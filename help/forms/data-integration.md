...
양식 데이터 모델을 만들면 이 모델을 사용하여 다음을 수행할 수 있습니다.

* 양식 데이터 모델을 기반으로 적응형 Forms 만들기
...
적응형 양식을 데이터베이스에 보내기 위한 데이터 통합을 구현하는 첫 번째 단계는 적응형 Forms에서 사용할 정보를 저장하는 데이터 소스를 식별하고 구성하는 것입니다. 그런 다음 하나 이상의 데이터 소스의 데이터 모델 개체, 속성 및 서비스를 사용하는 양식 데이터 모델을 만듭니다. 적응형 양식 필드가 각 데이터 소스 속성에 바인딩되는 양식 데이터 모델을 기반으로 적응형 Forms을 만들 수 있습니다.
