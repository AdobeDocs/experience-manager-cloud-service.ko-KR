...
description: AEM Forms에서 Azure Blob 스토리지 구성을 만들고 효율적인 데이터 스토리지를 위해 적응형 Forms 내에서 사용하는 방법에 대해 알아봅니다.
keywords: Azure Blob Storage와 AEM Forms 통합, Azure Storage에 데이터 제출, AEM Forms에서 Azure 스토리지 구성 생성, 적응형 Forms 제출 작업에서 Azure Blob 스토리지 사용
...
**[!UICONTROL Azure Blob Storage에 제출]** 제출 액션은 적응형 양식을 Microsoft® Azure 포털과 연결합니다. 양식 데이터, 파일, 첨부 파일 또는 기록 문서를 연결된 Azure 저장소 컨테이너에 제출할 수 있습니다.
...
* 적응형 양식 데이터, 파일, 첨부 파일 및 기록 문서를 Azure 스토리지 컨테이너에 제출하는 프로세스를 간소화하는 데 도움이 됩니다.
* 적응형 양식 제출을 위한 중앙 집중식 및 조직화된 스토리지에는 Azure Blob 스토리지를 사용합니다.

## AEM Forms을 Microsoft® Azure Blob 스토리지와 연결

적응형 Forms 제출 작업에서 Azure Blob 저장소를 사용하려면 다음을 수행하십시오.
...
기초 구성 요소를 기반으로 하는 적응형 양식에서 Azure 스토리지 컨테이너 구성을 다음과 같이 사용하려면 다음 단계를 수행하십시오.

1. 편집할 적응형 양식을 열고 적응형 양식 컨테이너 속성의 **[!UICONTROL 제출]** 섹션으로 이동합니다.
1. **[!UICONTROL 제출 액션]** 드롭다운 목록에서 **[!UICONTROL Azure Blob 저장소에 제출]**&#x200B;을 선택합니다.

   ![Azure Blob 저장소 GIF](/help/forms/assets/submit-to-azure-blob-fc.png){width=50%,height=50%}

   Azure Blob 저장소에 기록 문서(DoR)를 저장할 수도 있습니다.
...
다음 단계를 수행하여 핵심 구성 요소를 기반으로 하는 적응형 양식에서 Azure 스토리지 컨테이너 구성을 다음으로 사용하십시오.
...
범용 편집기에서 작성한 적응형 양식에서 Azure 스토리지 컨테이너 구성을 사용하려면 다음 단계를 수행하십시오.
...
1. **제출** 탭을 클릭하고 **[!UICONTROL Azure Blob 저장소에 제출]** 제출 액션을 선택합니다.
   ...
   **원래 이름으로 첨부 파일 저장**&#x200B;을 선택하면 첨부 파일은 원래 파일 이름을 사용하여 폴더에 저장됩니다. Azure Blob 저장소에 기록 문서(DoR)를 저장할 수도 있습니다.
