...
1. 양식 템플릿을 선택합니다. 적응형 Forms 또는 기초 구성 요소 기반 적응형 Forms 또는 적응형 에 대한 양식 조각을 만들 수 있습니다.
   ...
   * Foundation 구성 요소 기반 양식에 대한 양식 조각을 만들려면 Foundation 구성 요소 템플릿을 선택합니다. 예: /libs/fd/af/templateForFragment/defaultFragmentTemplate.

   핵심 구성 요소 기반 양식에 대한 양식 조각을 만들 때 양식 테마 선택 옵션을 사용하여 핵심 구성 요소 기반 테마를 선택합니다.

1. Form Model **탭을 클릭하여 열고** Select From **드롭다운 메뉴에서**&#x200B;조각에 대해 다음 모델 중 하나를 선택합니다.
   ...
   * **없음**: 양식 모델을 사용하지 않고 조각을 처음부터 만들도록 지정합니다.
     ...
     >[!NOTE]
     >
     >기본 구성 요소 기반 조각에 비해 핵심 구성 요소 기반 조각의 장점은 단일 적응형 양식의 양식 모델에 연결되지 않은 여러 핵심 구성 요소 기반 조각을 사용할 수 있다는 것입니다.

   * **스키마**: AEM Forms에 업로드된 XML 또는 JSON 스키마를 사용하여 조각을 만들도록 지정합니다. 사용 가능한 XML 또는 JSON 스키마를 업로드하거나 해당 스키마에서 조각을 위한 양식 모델로 선택할 수 있습니다. XML 스키마를 선택하면 XML 스키마 복합 유형&#x200B;]**드롭다운 상자에서 선택한 스키마**[!UICONTROL &#x200B;에 있는 complexType을 선택하여 적응형 양식 조각을 만들 수도 있습니다. JSON 스키마를 선택하면 JSON 스키마 정의&#x200B;]**드롭다운 상자에서 선택한 스키마에 있는 스키마**[!UICONTROL &#x200B;정의를 선택하여 적응형 양식 조각을 만들 수도 있습니다.
   * **양식 데이터 모델**: 양식 데이터 모델을 사용하여 조각을 만들도록 지정합니다. 양식 데이터 모델에서 하나의 데이터 모델 개체만 기반으로 적응형 양식 조각을 만들 수 있습니다. 양식 데이터 모델 정의 드롭다운을 확장합니다. 지정된 양식 데이터 모델의 모든 데이터 모델 개체를 나열합니다. 목록에서 데이터 모델 개체를 선택합니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td><p>사전 추가</p> </td>
   <td><p>선택한 부분을 현지화하기 위한 사전을 생성합니다. 자세한 내용은 적응형 Forms</a> 현지화를 참조하십시오<a>.<br /> <br /> </p> </td>
  </tr>
  <tr>
   <td><p>게시/게시 취소</p> </td>
...
## 조각으로 작업할 때 기억해야 할 요점 {#key-points-to-remember-when-working-with-fragments}

* 조각 이름이 고유한지 확인합니다. 이름이 같은 기존 부분이 있는 경우 조각을 만들지 못합니다.
* XDP 기반 적응형 양식에서 패널을 다른 XDP 조각을 포함하는 조각으로 저장하면 결과 조각이 하위 XDP 조각에 자동으로 바인딩됩니다. XSD 기반 적응형 양식의 경우 결과 조각이 스키마 루트에 바인딩됩니다.
* 적응형 양식 조각을 만들면 CRXDe Lite에서 적응형 양식의 guideContainer 노드와 유사한 조각 노드가 만들어집니다.
...
* 확인 구성 요소가 포함된 적응형 양식은 익명 사용자를 지원하지 않습니다. 또한 적응형 양식 조각에서 구성 요소 확인 을 사용하는 것이 수정되지 않습니다.
...
양식을 만드는 데 사용할 수 있는 참조 적응형 양식 조각을 사용할 수 있습니다.
