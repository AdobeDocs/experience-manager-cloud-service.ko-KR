...
1. 만들기 > 적응형 양식 단편 **을 클릭** 합니다.
1. 조각에 대 한 제목, 이름, 설명 및 태그를 지정 합니다. 조각에 고유한 이름을 지정 했는지 확인 합니다. 이름이 같은 다른 조각이 이미 있는 경우, 조각이 생성 되지 않습니다.
1. 양식 템플릿을 선택합니다. 적응형 Forms 또는 기초 구성 요소 기반 적응형 Forms 또는 적응형 에 대한 양식 조각을 만들 수 있습니다.
   ...
   * 기초 구성 요소 기반 양식에 대 한 양식 단편을 만들려면 템플릿 기초 구성 요소를 선택 합니다. 예:/libs/fd/af/templateForFragment/defaultFragmentTemplate.

   핵심 구성 요소 기반 양식에 대 한 양식 단편을 만들 때 [양식 테마 선택] 옵션을 사용 하 여 핵심 구성 요소 기반 테마를 선택 합니다.
...
1. 을 클릭 하 여 **양식 모델** 탭 클릭 하 고 선택 위치 **드롭다운 메뉴에서** 조각에 대해 다음 모델 중 하나를 선택 합니다.
   ...
   * **없음** : 양식 모델을 사용 하지 않고 처음부터 단편을 생성 하도록 지정 합니다.
     ...
     >[!NOTE]
     >
     >기본 구성 요소 기반 조각보다 핵심 구성 요소 기반 조각의 이점은 단일 적응형 양식의 양식 모델에 연결되지 않은 여러 핵심 구성 요소 기반 조각을 사용할 수 있다는 점입니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td><p>미리보기</p> </td>
   <td><p>XML 파일의 데이터를 조각과 병합 하 여 단편을 HTML 또는 사용자 지정 미리 보기로 미리 보는 옵션을 제공 합니다. 자세한 내용은 양식 </a> 미리 보기를 참조 <a> 하십시오. <br /> <br /> </p> </td>
  </tr>
  <tr>
...
* 다른 양식 데이터 모델을 사용하는 적응형 양식의 조각은 지원되지 않습니다. 예를 들어, XDP 기반 조각은 XSD 기반 적응형 양식에서 지원 되지 않으며 그 반대의 경우도 마찬가지입니다.
...
* 적응형 양식 내에서 참조로 삽입 되는 적응형 양식 단편은 편집할 수 없습니다. 편집 하려면 독립형 적응형 양식 단편을 편집 하거나 적응형 양식에서 단편을 포함 합니다.
...
* ( **Mac만** 해당) 양식 조각 기능이 모든 시나리오에서 완벽 하 게 작동 하도록 하려면/private/etc/hosts 파일에 다음 항목을 추가 합니다.
  `127.0.0.1 <Host machine>`**호스트 컴퓨터** : AEM Forms 배포 되는 Apple Mac 컴퓨터입니다.
