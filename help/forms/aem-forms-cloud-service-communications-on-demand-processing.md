---
title: Forms Communications 동기 API는 어떻게 설정합니까?
...
# AEM Forms Communications API에 대한 OAuth 서버 간 액세스 구성
...
* [AEM 릴리스 2024.10.18459.20241031T210302Z 이상](#update-aem-instance)
* 2024년 11월 이전에 환경이 생성된 경우 제품 프로필 업데이트
...
>[!NOTE]
>
> Adobe Developer Console을 사용한 OAuth 서버 간 인증에 대한 자세한 내용을 보려면 [여기를 클릭](/help/forms/oauth-api-authetication.md)하십시오.
...
![프로그램 개요 페이지](/help/forms/assets/program-overview.png)
...
3. 저장소 URL 복사
   ...
   ![저장소 URL 복사](/help/forms/assets/copy-repo-url.png)
...
2. _API 추가_ 대화 상자에서 **Experience Cloud**(으)로 필터링합니다.
3. **&quot;Forms 통신 API&quot;** 선택
...
>[!NOTE]
>
> Adobe Developer Console을 통한 OAuth 서버 간 인증에 대한 자세한 내용은 [OAuth 서버 간 인증](/help/forms/oauth-api-authetication.md) 문서를 참조하십시오.
...
3. `api.yaml` 디렉터리에 이름이 `config`인 새 파일을 만듭니다.
...
2. **&quot;추가&quot;** 단추 클릭
   ...
   ![파이프라인 추가](/help/forms/assets/add-pipeline.png)
...
>[!TAB A. API 테스트에 Swagger UI 사용]

Swagger UI는 코드를 작성하지 않고 API를 테스트하기 위한 대화형 인터페이스를 제공합니다. **사용해 보기** 기능을 사용하여 [PDF 생성](https://developer.adobe.com/experience-cloud/experience-manager-apis/api/experimental/document/#operation/renderPDFForm) Forms 통신 API를 호출하고 테스트하십시오.

1. [Forms Communication API 참조](https://developer.adobe.com/experience-manager-forms-cloud-service-developer-reference/)&#x200B;(으)로 이동하여 브라우저에서 [Forms Communication API](https://developer.adobe.com/experience-cloud/experience-manager-apis/api/experimental/document) 설명서를 엽니다.
...
4. 다음 값을 입력합니다.
   ...
   | **섹션** | **매개 변수** | **값** |
   |--------------|---------------|------------|
   | 버킷 | AEM 인스턴스 | Adobe 도메인 이름(`.adobeaemcloud.com`)이 없는 AEM 인스턴스 이름. 예를 들어 `pXXXXX-eYYYYY`을(를) 버킷으로 사용합니다. |
   | 보안 | 전달자 토큰 | Adobe Developer Console 프로젝트의 OAuth 서버 간 자격 증명에서 [액세스 토큰 사용](/help/forms/oauth-api-authetication.md#how-to-generate-an-access-token-using-oauth-server-to-server-authentication) |
...
6. **응답** 탭에서 응답을 확인하십시오.
   ...
   * 응답 코드가 `403`이면 인증 오류가 있음을 의미합니다.
...
>[!TAB B. 프로그래밍 방식으로 NodeJS 응용 프로그램 개발]
...
AEM 환경을 업데이트하려면 [AEM 인스턴스 업데이트](#update-aem-instance) 섹션을 참조하십시오.
