...
다음 **[!UICONTROL AEM 워크플로우 호출]** 제출 액션은 적응형 양식을 AEM 워크플로와 연결합니다. 양식이 제출되면 연결된 워크플로가 작성자 인스턴스에서 자동으로 시작됩니다. 데이터 파일, 첨부 파일 및 기록 문서를 워크플로우의 페이로드 위치 또는 변수에 저장할 수 있습니다. 워크플로가 외부 데이터 스토리지로 표시되고 외부 데이터 스토리지로 구성되면 변수 옵션만 제공됩니다. 워크플로 모델에 제공되는 변수 목록에서 선택할 수 있습니다. 워크플로 생성 시점이 아닌 이후 단계에서 워크플로가 외부 데이터 스토리지로 표시되면 필수 변수 구성이 마련되었는지 확인합니다.

>[!NOTE]
>
>  방법 알아보기 [워크플로우 모델 만들기](https://experienceleague.adobe.com/docs/experience-manager-65/developing/extending-aem/extending-workflows/workflows-models.html?lang=ko#extending-aem) 사용자가 워크플로우를 시작할 때 실행되는 일련의 단계를 정의합니다. 워크플로가 일시적인지 또는 여러 리소스를 사용하는지 여부와 같은 모델 속성을 정의할 수도 있습니다.
...
을 사용하여 자동화된 프로세스를 설정하려면 [AEM 워크플로](https://experienceleague.adobe.com/docs/experience-manager-65/developing/extending-aem/extending-workflows/workflows-models.html?lang=ko#extending-aem) 적응형 양식의 경우 다음 단계를 수행하십시오.
...
1. 다음에서 **[!UICONTROL 제출 액션]** 드롭다운 목록에서 다음을 선택합니다. **[!UICONTROL AEM 워크플로우 호출]** .
   ![이메일 보내기 작업 구성](/help/forms/assets/configure-invoke-aem-workflow.png)

1. 다음에서 워크플로 모델 선택 **[!UICONTROL 워크플로 모델]** 드롭다운 목록입니다.
1. 다음에서 옵션 선택: **[!UICONTROL 다음을 사용하여 데이터 파일 저장]** 드롭다운 목록입니다.
...
1. 다음에서 옵션 선택: **[!UICONTROL 다음을 사용하여 첨부 파일 저장]** 드롭다운 목록입니다.
...
1. 다음에서 옵션 선택: **[!UICONTROL 다음을 사용하는 기록 문서]** 드롭다운 목록입니다.
...
>[!NOTE]
>
> 자세히 알아보기 [Forms 중심 AEM 워크플로 - 비즈니스 프로세스 자동화를 위한 단계 참조](/help/forms/aem-forms-workflow-step-reference.md).
