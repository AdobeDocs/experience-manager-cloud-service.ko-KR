...
| 버전 | 문서 링크 |
| -------- | ---------------------------- |
| AEM 6.5 | [여기 클릭](https://experienceleague.adobe.com/docs/experience-manager-65/forms/manage-administer-aem-forms/manage-form-metadata.html?lang=ko) |
| AEM as a Cloud Service | 이 문서 |

메타데이터를 사용하면 에셋을 보다 쉽게 분류하고 구성할 수 있으며 특정 에셋을 찾는 사용자에게 도움이 됩니다.

[!DNL AEM Forms]은(는) 기본적으로 각 에셋 유형에 대해 정의된 메타데이터 집합을 제공합니다. 기본 메타데이터 외에도 각 에셋 유형에 사용자 지정 메타데이터를 추가할 수 있습니다. [!DNL AEM Forms]은(는) 폼에 대해 이 모든 메타데이터를 효율적으로 만들고, 관리하고, 교환할 수 있는 적절한 수단도 제공합니다.
...
[!DNL AEM Forms]에서 에셋과 연결된 메타데이터 속성 목록은 해당 유형에 따라 다릅니다. 또한 사용자 지정 메타데이터 속성을 추가하면 해당 사용자 지정 메타데이터가 추가된 유형의 모든 에셋에 추가됩니다.

### 에셋 유형 {#asset-types}

[!DNL AEM Forms]에서 지원되는 자산 유형은 다음과 같습니다.
...
다음은 [!DNL AEM Forms]에서 지원되는 메타데이터 속성의 광범위한 목록입니다.
...
<table>
 <tbody> 
  ...
  <tr> 
   ...
   <td>자산의 표시 이름입니다.<br /> </td> 
  </tr> 
  <tr> 
   <td>설명</td> 
   <td>리소스를 제외한 모든 항목</td> 
   <td>에셋에 대한 설명. 사용자가 이 값을 지정할 수 있습니다.<br /> </td> 
  </tr> 
  <tr> 
   <td>유형</td> 
   <td>모두</td> 
...
1. 다음 방법 중 하나를 사용하여 속성 페이지를 엽니다.
   ...
   * 빠른 작업에서 **[!UICONTROL 속성]** ![속성](assets/Smock_Info_18_N.svg) 아이콘을 클릭합니다.

     >[!NOTE]
     >
     >빠른 작업은 마우스를 가져가면 썸네일 위에 표시되는 작업 항목입니다.

   * 양식을 선택하고 도구 모음에 나타나는 **[!UICONTROL 속성]** ![속성](assets/Smock_Info_18_N.svg) 아이콘을 클릭합니다.
   * 선택 모드에 있지 않을 때 양식 썸네일을 클릭하여 양식 세부 정보 페이지로 이동합니다. 이제 오른쪽 상단의 ![속성](assets/Smock_Info_18_N.svg) 눈 아이콘을 클릭한 다음 아래 목록에서 속성을 클릭합니다.
...
적응형 Forms의 경우 사용자가 현재 적응형 양식 미리 보기의 스냅숏으로 썸네일을 생성할 수 있는 추가 기능이 제공됩니다. [!DNL AEM Forms]은(는) 적응형 Forms 작성도 지원하므로 적응형 양식을 변경할 때마다 적응형 양식의 미리 보기가 변경될 수 있습니다. 이 썸네일 생성 기능을 사용하면 현재 미리보기 상태를 기반으로 적응형 양식에 대한 최신 썸네일을 가져올 수 있습니다. 이 작업을 수행하려면 **[!UICONTROL 미리 보기 생성]**&#x200B;을 클릭하십시오.
...
기본적으로 제공되는 메타데이터 외에도 [!DNL AEM Forms]은(는) 새로운 사용자 지정 메타데이터를 지원합니다.
...
[!DNL AEM Forms]은(는) 이 도구에서 지원되는 양식 형식의 메타데이터 스키마를 노출합니다. 이렇게 하면 이러한 스키마에 액세스하고 메타데이터 스키마 편집기에 제공된 기능을 사용하여 사용자 지정 속성을 추가할 수 있습니다.
...
1. `extendedmetadata` 옆의 확인란을 선택한 다음 도구 모음에 나타나는 ![편집](assets/Smock_Edit_18_N.svg) 아이콘을 클릭합니다.

1. [!DNL AEM Forms]에서 선택한 자산 유형(이 경우 적응형 양식)의 메타데이터 스키마 편집기/양식 빌더를 엽니다.
...
1. 방금 드래그한 구성 요소를 클릭합니다. 오른쪽 패널에 열리는 설정 탭에서 다음 필드에 대한 정보를 입력합니다.
   ...
   1. 추가한 필드를 삭제하려면 필드를 선택한 다음 삭제 ![삭제](assets/Smock_Delete_18_N.svg) 아이콘을 클릭합니다.

1. 필요한 경우 1-3단계에 따라 다른 속성을 추가합니다.
1. 모든 변경 작업을 수행한 후 **[!UICONTROL 저장]**&#x200B;을 클릭합니다.

   사용자 지정 메타데이터 속성을 추가했습니다.

이제 [!DNL AEM Forms]의 모든 적응형 Forms에 이 추가 메타데이터 속성이 포함됩니다. 속성 페이지에서 편집할 수 있습니다.
