...
1. 작성자 인스턴스 https://[*hostname*]:[*포트*]/aem/forms.html 에 [!DNL AEM Forms] 로그인합니다.
1. 적응형 양식 단편&#x200B;**만들기 > 클릭합니다**.
1. 조각의 제목, 이름, 설명 및 태그를 지정합니다.

   >[!NOTE]
   >
   >조각에 고유한 이름을 지정해야 합니다. 같은 이름의 다른 조각이 이미 있으면 해당 조각이 만들어지지 않습니다.
...
1. **확인**&#x200B;을 클릭합니다.
   ...
   패널이 저장소의 지정된 위치 또는 기본 위치에 저장됩니다. 적응형 양식에서 패널은 조각의 스냅샷으로 대체됩니다. 아래 표시된 대로 [일반 정보] 패널과 하위 패널인 [개인 정보 및 주소]는 조각으로 저장됩니다.

   조각을 편집하려면 패널 도구 모음에서 [자산&#x200B;]**편집]을 클릭합니다**[!UICONTROL . 조각이 편집 모드의 새 탭이나 창에서 열립니다.
...
사용자가 만든 적응형 양식 단편은 AEM 컨텐츠 파인더의 적응형 양식 단편 탭 나타납니다. 적응형 양식에 적응형 양식 단편을 삽입하려면:

1. 적응형 양식 단편을 삽입할 적응형 양식을 편집 모드에서 엽니다.
1. 사이드바에서 자산 브라우저](assets/assets-browser.png) Assets **![를 클릭합니다**. 자산 브라우저의 드롭다운에서 적응형 양식 단편&#x200B;**을 선택합니다**.

   모든 적응형 양식 단편을 표시하거나 양식 모델(양식 템플릿, XML 스키마 또는 기본)에 따라 필터링하도록 선택할 수도 있습니다.
...
적응형 양식 단편이 포함된 적응형 양식을 현지화하려면 조각과 양식을 별도로 현지화해야 합니다. 아이디어는 조각을 한 번 현지화하고 여러 적응형 Forms에서 재사용하는 것입니다.
...
* 적응형 양식을 게시 때 적응형 양식에 참조로 삽입된 독립형 적응형 양식 단편을 게시해야 합니다.
* 업데이트된 적응형 양식 조각을 다시 게시하면 조각이 사용되는 적응형 양식의 게시된 인스턴스에 변경 사항이 반영됩니다.
* Verify 구성 요소가 포함된 적응형 양식은 익명 사용자를 지원하지 않습니다. 또한 적응형 양식 단편에서 Verify 구성 요소를 사용하도록 변경되지 않았습니다.
