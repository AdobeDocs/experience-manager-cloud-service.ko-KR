...
1. 클릭하여 열기 **양식 모델** 탭 및 **다음에서 선택** 드롭다운 메뉴에서 조각에 대해 다음 모델 중 하나를 선택합니다.
   ...
   * **없음** : 양식 모델을 사용 하지 않고 처음부터 단편을 생성 하도록 지정 합니다.
     ...
     >[!NOTE]
     >
     > 주요 구성 요소를 기반으로 한 적응형 Forms에서는 양식에서 단일 양식 단편을 여러 번 사용할 수 있습니다. none 기반 양식 조각과 schema 기반 양식 조각을 모두 지원합니다.
   ...
   * **양식 템플릿**: 업로드된 XDP 템플릿을 사용하여 조각을 만들도록 지정합니다. [!DNL AEM Forms]. 조각의 양식 모델로 적절한 XDP 템플릿을 선택합니다.
   ...
   선택한 양식 템플릿에 조각으로 표시 된 하위 양식도 표시 됩니다. 드롭다운 목록에서 적응형 양식 단편에 대 한 하위 양식을 선택할 수 있습니다.

   ![지정한 양식에서 하위 폼을 선택 템플릿](assets/fragment-subform.png)

   또한 드롭다운 상자의 하위 양식에 대 한 SOM 표현식을 지정 하 여 양식 템플릿에 조각으로 표시 되지 않은 하위 양식을 사용 하 여 적응형 양식 단편을 만들 수 있습니다.

   * **XML 스키마** :에 [!DNL AEM Forms] 업로드 된 XML 스키마를 사용 하 여 단편을 만들도록 지정 합니다. 사용 가능한 XML 스키마를 조각의 양식 모델로 업로드 하거나 선택할 수 있습니다.

   ![XML 스키마를 기반으로 한 적응형 양식 단편을 모델로 만들기](assets/xml-schema-model.png)
...
다음 예제 이미지와 같이 추가 된 조각의 패널 도구 모음에서 [자산 포함: *fragmentName* > **버튼를 클릭** 하 여 적응형 양식에 적응형 양식 단편을 포함 하도록 선택할 수 있습니다.
...
중첩 된 적응형 양식 조각을 만들 수 있으며,이는 다른 조각에서 조각을 끌어서 놓을 수 있고 중첩 된 단편 구조를 가질 수 있다는 것을 의미 합니다.
...
>[!NOTE]
>
> * 적응형 양식에서 비 기반 양식 조각을 여러 번 사용하는 경우 조각 필드 간에 데이터 동기화가 발생합니다. 단일 을 사용할 수 있습니다. [양식 조각(핵심 구성 요소 기반)](/help/forms/adaptive-form-fragments-core-components.md)  양식에서 여러 번. 데이터 동기화 문제가 없는 없음 기반 및 스키마 기반 양식 조각을 모두 지원 합니다.
...
UI를 [!DNL AEM Forms] 사용 하 여 적응형 양식 조각에 대해 여러 작업을 수행할 수 있습니다.

1. `https://[hostname]:'port'/aem/forms.html`로 이동합니다.

1. [UI] 도구 모음에서 [!DNL AEM Forms] [선택 **]을 클릭** 하 고 적응형 양식 단편을 선택 합니다. 도구 모음에는 선택한 적응형 양식 단편에서 수행할 수 있는 다음 작업이 표시 됩니다.
...
* 적응형 양식 내에서 참조로 삽입 되는 적응형 양식 단편은 편집할 수 없습니다. 편집하려면 독립 실행형 적응형 양식 조각을 편집하거나 적응형 양식에 해당 조각을 임베드하십시오.
