---
title: 자산을  [!DNL AEM Forms](으)로 가져오고 내보내는 방법
...
다른 [!DNL AEM Forms] 인스턴스 간에 양식, 테마, 템플릿, 문서 단편, 테마 및 기타 자산을 이동할 수 있습니다. 시스템을 마이그레이션하거나 개발 또는 스테이징 서버에서 프로덕션 서버로 양식을 이동할 때 이러한 이동이 필요합니다.

[!DNL AEM Forms] UI를 통한 업로드 및 가져오기가 지원되는 자산의 경우 내보내기 또는 가져오기에 Forms UI를 사용하는 것이 좋습니다. 이러한 에셋을 내보내거나 가져오기 위해 AEM 패키지 관리자를 사용하는 것은 권장되지 않습니다.

## Forms 및 문서 에셋 다운로드 또는 업로드 {#download-or-upload-forms-amp-documents-assets}

[!DNL AEM Forms] 사용자 인터페이스를 사용하면 자산을 AEM CRX 패키지 또는 이진 파일로 다운로드하여 AEM 인스턴스에서 내보낼 수 있습니다. 그런 다음 다운로드한 AEM CRX 패키지 또는 이진 파일을 다른 AEM 인스턴스로 가져올 수 있습니다.

[!DNL AEM Forms] 사용자 인터페이스를 통한 내보내기 및 가져오기는 적응형 양식 템플릿 및 적응형 양식 콘텐츠 정책을 제외한 모든 자산에 대해 지원됩니다. 따라서 [!DNL AEM Forms] UI에서 적응형 양식을 내보낼 때 관련 적응형 양식 템플릿 및 콘텐츠 정책이 다른 관련 에셋처럼 자동으로 내보내지지 않습니다.

이러한 에셋 유형의 경우 AEM 패키지 관리자를 사용하여 소스 AEM 서버에 CRX 패키지를 만들고 대상 서버에 패키지를 설치해야 합니다. 패키지를 만들고 설치하는 방법에 대한 자세한 내용은 [AEM as a Cloud Service에 배포](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/deploying/overview.html)를 참조하십시오.

### Forms 및 문서 에셋 다운로드 {#download-forms-amp-documents-assets}

Forms 및 Documents 에셋을 다운로드하려면:

1. [!DNL AEM Forms] 인스턴스에 로그인합니다.
1. Experience Manager ![adobeexperiencemanager](assets/adobeexperiencemanager.png) 아이콘 > 탐색 ![나침반](assets/Smock_Compass_18_N.svg) 아이콘> **[!UICONTROL Forms]** > **[!UICONTROL Forms 및 문서]**&#x200B;를 선택합니다.
1. 양식 자산을 선택하고 **[!UICONTROL 다운로드]** 아이콘을 선택합니다.
1. 에셋 다운로드에서 다음 옵션 중 하나를 선택하고 **[!UICONTROL 다운로드]**&#x200B;를 선택합니다.

   * **CRX 패키지로 다운로드:** 선택한 모든 에셋과 관련 종속성을 다운로드하고 [!DNL AEM Forms] 인스턴스에서 다른 인스턴스로 이동하려면 옵션을 사용합니다. 모든 에셋과 폴더를 crx 패키지로 다운로드합니다. AEM(적응형 Forms 및 적응형 양식 단편), PDF 문서 및 리소스(XSD, XFS, 이미지)에서 작성된 양식을 포함한 모든 양식 자산은 [!DNL AEM Forms] UI에서 패키지로 다운로드할 수 있습니다.
자산을 패키지로 다운로드하면 다운로드하도록 선택한 자산이 사용한 자산도 다운로드된다는 이점이 있습니다. 예를 들어 양식 템플릿, XSD 및 이미지를 사용하는 적응형 양식이 있는 경우. 이 적응형 양식을 선택하고 패키지로 다운로드하면 다운로드된 패키지에는 양식 템플릿, XSD 및 이미지도 포함됩니다. 자산과 연결된 모든 메타데이터 속성(사용자 지정 속성 포함)도 다운로드됩니다.
   ...
   * **에셋을 이진 파일로 다운로드:** 양식 서식 파일(XDP), PDF forms(PDF), 문서(PDF) 및 리소스(이미지, 스키마, 스타일시트)만 다운로드하려면 옵션을 사용합니다. 외부 애플리케이션으로 이러한 에셋을 편집할 수 있습니다. XSD, XDP, 이미지, PDF 및 XDP와 같은 바이너리가 있는 양식 에셋을 .zip 파일로 다운로드합니다.
**[!UICONTROL 이진 파일로 에셋 다운로드]** 옵션을 사용하면 적응형 Forms, 적응형 양식 단편 및 테마를 다운로드할 수 없습니다. 이러한 에셋을 다운로드하려면 **[!UICONTROL CRX 패키지로 다운로드]** 옵션을 사용해야 합니다.
...
1. [!DNL AEM Forms] 인스턴스에 로그인합니다.
1. Experience Manager ![adobeexperiencemanager](assets/adobeexperiencemanager.png) 아이콘 > 탐색 ![나침반](assets/Smock_Compass_18_N.svg) 아이콘> **[!UICONTROL Forms]** > **[!UICONTROL Forms 및 문서]**&#x200B;를 선택합니다.
1. **만들기** >**파일 업로드**&#x200B;를 선택합니다. 양식 또는 패키지 업로드 대화 상자가 나타납니다.
1. 대화 상자에서 가져올 패키지 또는 아카이브를 찾아 선택합니다. PDF 문서, XSD, 이미지, 스타일시트 및 XDP 양식을 선택할 수도 있습니다. **[!UICONTROL 열기]**&#x200B;를 선택합니다. 선택한 폴더 또는 파일 이름에는 특수 문자가 포함되지 않아야 합니다.
...
[!DNL AEM Forms]을(를) 사용하면 테마를 만들거나 다운로드하거나 업로드할 수 있습니다. 테마는 양식, 문서 및 편지와 같은 다른 에셋처럼 만들어집니다. 테마를 만들고, 다운로드한 다음 별도의 인스턴스에 업로드하여 재사용할 수 있습니다. 테마에 대한 자세한 내용은 [!DNL AEM Forms]의 [테마](themes.md)를 참조하세요.
...
1. [!DNL AEM Forms] 인스턴스에 로그인합니다.
1. Experience Manager ![adobeexperiencemanager](assets/adobeexperiencemanager.png) 아이콘 > 탐색 ![나침반](assets/Smock_Compass_18_N.svg) 아이콘> **[!UICONTROL Forms]** > **[!UICONTROL 테마]**&#x200B;를 선택합니다.
1. 테마를 선택하고 **[!UICONTROL 다운로드]**&#x200B;를 선택합니다. 테마는 보관 파일(.zip 파일)로 다운로드됩니다.
...
1. Experience Manager에서 **[!UICONTROL Forms]** > **[!UICONTROL Forms 테마]**&#x200B;로 이동합니다.
1. 테마 페이지에서 **[!UICONTROL Forms 만들기]** > **[!UICONTROL Forms 파일 업로드]**&#x200B;를 클릭합니다.
1. 파일 업로드 프롬프트에서 컴퓨터에서 테마 패키지를 찾아 선택하고 **[!UICONTROL Forms 업로드]**&#x200B;를 클릭합니다. 테마가 업로드되었습니다.
...
1. [!DNL AEM Forms] 패키지 관리자를 엽니다.
1. **[!UICONTROL 패키지 만들기]**&#x200B;를 클릭합니다. **[!UICONTROL 새 패키지]** 대화 상자가 나타납니다.
1. 패키지의 이름, 버전 및 그룹을 지정합니다. **[!UICONTROL 확인]**&#x200B;을 클릭합니다.
1. **[!UICONTROL 편집]**&#x200B;을 클릭하고 **[!UICONTROL 필터]** 탭을 엽니다. **[!UICONTROL 필터 추가]**&#x200B;를 클릭합니다. 워크플로우 응용 프로그램의 경로를 지정합니다. 예: /etc/fd/dashboard/startpoints/homemortgage. **[!UICONTROL 규칙 추가]**&#x200B;를 클릭합니다.

1. **[!UICONTROL 고급]** 탭을 엽니다. ACL 처리 필드에서 **[!UICONTROL 병합]** 또는 **[!UICONTROL 덮어쓰기]**&#x200B;를 선택합니다. **[!UICONTROL 저장]**&#x200B;을 클릭합니다.
1. 패키지를 만들려면 **[!UICONTROL 빌드]**&#x200B;를 클릭합니다.
...
[!DNL AEM Forms] 사용자 인터페이스는 폴더를 사용하여 자산을 정렬합니다. 이러한 폴더는 [!DNL AEM Forms] 사용자 인터페이스 내에서 만들어진 자산을 정렬하는 데 사용됩니다. 이름을 바꾸고 하위 폴더를 만들고 이러한 폴더에 에셋과 문서를 저장할 수 있습니다. 폴더에 문서 및 에셋을 구성하면 파일을 함께 그룹화하여 간편하게 관리할 수 있습니다. 폴더를 선택하여 다운로드하거나 삭제할 수 있습니다.

폴더를 만들려면 다음 단계를 완료하십시오.

### 폴더 만들기 {#create-a-folder}

1. `https://<server>:<port>/aem/forms.html`의 [!DNL AEM Forms] 사용자 인터페이스에 로그인합니다.
1. 폴더를 만들 위치로 이동합니다.
...
>[!MORELIKETHIS]
>
>* [내보내기 양식 템플릿 가져오기](/help/forms/import-export-forms-templates.md)
