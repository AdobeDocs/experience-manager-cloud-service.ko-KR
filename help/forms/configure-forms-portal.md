...
* 검색 및 목록: 이 구성 요소를 사용하여 양식 리포지토리의 양식을 포털 페이지에 나열할 수 있고 지정된 기준에 따라 양식을 나열할 수 있는 구성 옵션을 제공합니다.
...
AEM Project Archetype에서 [기본 Forms 포털 구성 요소를 가져올 수 있습니다](#import-forms-portal-components-aem-archetype). 가져온 후 다음 구성을 수행합니다.

* [외부 스토리지 구성](#configure-azure-storage-adaptive-forms)

* [Forms 포털 구성 요소 사용](#enable-forms-portal-components)
...
1. **로컬 개발 인스턴스에서 Cloud Manager Git 저장소를 복제합니다.** Cloud Manager Git 저장소에 기본 AEM 프로젝트가 포함되어 있습니다. [AEM Archetype](https://github.com/adobe/aem-project-archetype/)을(를) 기반으로 합니다. Cloud Manager UI의 셀프서비스 Git 계정 관리를 사용하여 Cloud Manager Git 저장소를 복제하여 프로젝트를 로컬 개발 환경에서 가져옵니다. 저장소 액세스에 대한 자세한 내용은 [저장소 액세스](https://experienceleague.adobe.com/docs/experience-manager-cloud-manager/using/managing-code/accessing-repos.html)를 참조하십시오.

1. **[!DNL Experience Manager Forms]을(를) [Cloud Service] 프로젝트로 만들기:** [AEM Archetype 27](https://github.com/adobe/aem-project-archetype/releases/tag/aem-project-archetype-27) 이상을 기반으로 [!DNL Experience Manager Forms]을(를) [Cloud Service] 프로젝트로 만들기 as a Cloud Service Archetype을 통해 개발자는 [!DNL AEM Forms]에 대한 개발을 쉽게 시작할 수 있습니다. 또한 빠르게 시작하는 데 도움이 되는 몇 가지 샘플 테마 및 템플릿이 포함되어 있습니다.

   as a Cloud Service [!DNL Experience Manager Forms] 프로젝트를 만들려면 명령 프롬프트를 열고 아래 명령을 실행하십시오. [!DNL Forms]개의 특정 구성, 테마 및 템플릿을 포함하려면 `includeForms=y`을(를) 설정하십시오.
   ...
   프로젝트가 준비되면 Archetype 프로젝트의 최상위 `pom.xml`에서 `<core.forms.components.version>x.y.z</core.forms.components.version>` 속성을 업데이트하여 `AEM Archetype` 프로젝트에 최신 버전의 [core-forms-components](https://github.com/adobe/aem-core-forms-components)을(를) 반영합니다.

1. **로컬 개발 환경에 프로젝트 배포:** 다음 명령을 사용하여 로컬 개발 환경에 배포할 수 있습니다
...
1. as a Cloud Service [코드를  [!DNL AEM Forms] 환경에 배포](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/developing/aem-project-content-package-structure.html#embeddeds).


## 적응형 Forms을 위한 Azure 스토리지 구성 {#configure-azure-storage-adaptive-forms}

[[!DNL Experience Manager Forms] 데이터 통합](data-integration.md)은(는) [!DNL Azure] 저장소 서비스와 양식을 통합할 수 있도록 [!DNL Azure] 저장소 구성을 제공합니다. 양식 데이터 모델(FDM)을 사용하여 [!DNL Azure] 서버와 상호 작용하여 비즈니스 워크플로를 가능하게 하는 적응형 Forms을 만들 수 있습니다.

### 스토리지 구성 경로 {#create-azure-storage-configuration}

이 단계를 실행하기 전에 Azure 저장소 계정과 [!DNL Azure] 저장소 계정에 대한 액세스 권한을 부여하는 액세스 키가 있는지 확인하십시오.
...
Adobe Experience Manager(AEM) 사이트에서 핵심 구성 요소(기본 포털 구성 요소 포함)를 사용하려면 프록시 구성 요소를 만들어 사이트에 맞게 활성화해야 합니다. 프록시 구성 요소를 만들고 포털 구성 요소를 사용하려면 [핵심 구성 요소 사용](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/get-started/using.html?lang=en#create-proxy-components)을 참조하세요.

포털 구성 요소가 활성화되면 사이트 페이지의 작성자 인스턴스에서 사용할 수 있습니다.

## Forms 포털 구성 요소 추가 및 구성 {#configure-forms-portal-components}

포털 구성 요소를 추가 및 구성하여 AEM을 사용하여 작성된 웹 사이트에서 Forms 포털을 만들고 사용자 지정할 수 있습니다. Forms 포털에서 사용하기 전에 [구성 요소가 활성화되었는지](#enable-forms-portal-components)확인하십시오.

구성 요소를 추가하려면 구성 요소 창에서 페이지의 레이아웃 컨테이너로 구성 요소를 끌어서 놓거나 레이아웃 컨테이너에서 추가 아이콘을 선택하고 [!UICONTROL 새 구성 요소 삽입] 대화 상자에서 구성 요소를 추가하십시오.

### 초안 및 제출 구성 요소 구성 {#configure-drafts-submissions-component}

초안 및 제출 구성 요소에는 나중에 제출한 양식을 완료하기 위해 초안으로 저장된 양식이 표시됩니다. 구성하려면 구성 요소를 선택한 다음 ![구성 아이콘](assets/configure_icon.png)을 선택합니다. [!UICONTROL 초안 및 제출] 대화 상자에서 초안 또는 제출된 양식으로 양식 목록을 나타내는 제목을 지정합니다. 구성 요소에 초안 양식이나 제출된 양식을 카드 또는 목록 형식으로 나열할지 여부도 선택합니다.
...
구성하려면 구성 요소를 선택한 다음 ![구성 아이콘](assets/configure_icon.png)을 선택합니다. [!UICONTROL 검색 및 목록 작성기] 대화 상자가 열립니다.

1. [!UICONTROL 디스플레이] 탭에서 다음을 구성합니다.
   * **[!UICONTROL 제목]**&#x200B;에서 검색 및 목록 구성 요소의 제목을 지정합니다. 직설적인 제목을 사용하면 양식 목록에서 빠른 검색을 수행할 수 있습니다.
   * **[!UICONTROL 레이아웃]** 목록에서 카드 또는 목록 형식으로 양식을 나타내는 레이아웃을 선택하십시오.
   * 검색 및 정렬 기준 기능을 숨기려면 **[!UICONTROL 검색 숨기기]** 및 **[!UICONTROL 정렬 숨기기]**&#x200B;를 선택하십시오.
   * **[!UICONTROL 도구 설명]**&#x200B;에서 구성 요소를 마우스로 가리키면 표시되는 도구 설명을 제공합니다.
1. [!UICONTROL 자산 폴더] 탭에서 양식을 가져와서 페이지에 나열하는 위치를 지정합니다. 여러 폴더 위치를 구성할 수 있습니다.
1. [!UICONTROL 결과] 탭에서 페이지당 표시할 최대 양식 수를 구성합니다. 기본값은 페이지당 8개의 양식입니다.

### 링크 구성 요소 구성 {#configure-link-component}

링크 구성 요소를 사용하여 페이지에서 적응형 양식에 대한 링크를 제공할 수 있습니다. 구성하려면 구성 요소를 선택한 다음 ![구성 아이콘](assets/configure_icon.png)을 선택합니다. [!UICONTROL 링크 구성 요소 편집] 대화 상자가 열립니다.

1. [!UICONTROL 표시] 탭에서 링크 캡션 및 도구 설명을 입력하여 링크가 나타내는 양식을 쉽게 식별할 수 있습니다.
1. [!UICONTROL 자산 정보] 탭에서 자산이 저장되는 저장소 경로를 지정합니다.
1. [!UICONTROL 쿼리 매개 변수] 탭에서 키-값 쌍 형식으로 추가 매개 변수를 지정합니다. 링크를 클릭하면 이러한 추가 매개 변수가 양식과 함께 전달됩니다.
...
1. 왼쪽 창에서 속성 아이콘을 선택하고 **[!UICONTROL 전자 서명]** 옵션을 확장합니다.
1. **[!UICONTROL Adobe Sign 사용]**&#x200B;을 선택합니다. 다양한 구성 옵션이 표시됩니다.
1. [!UICONTROL 양식 제출] 섹션에서 **[!UICONTROL 모든 수신자가 서명식을 완료한 후]** 옵션을 선택하여 양식 제출 동작을 구성하십시오. 여기서 서명할 모든 수신자에게 먼저 양식이 전송됩니다. 모든 수신자가 양식에 서명하면 양식이 제출됩니다.
...
1. [!UICONTROL When] 섹션에서 &quot;클릭됨&quot;을 선택하고 [!UICONTROL Then] 섹션에서 &quot;양식 저장&quot; 옵션을 선택합니다.
...
1. 작성자 인스턴스에서 편집 모드로 적응형 양식을 엽니다.
1. 왼쪽 창에서 ![속성 아이콘](assets/configure_icon.png)을 선택하고 [!UICONTROL 자동 저장] 옵션을 확장합니다.
1. **[!UICONTROL 사용]** 확인란을 선택하여 양식 자동 저장을 사용하도록 설정합니다. 다음을 구성할 수 있습니다.
* 기본적으로 [!UICONTROL 적응형 양식 이벤트]은(는) &quot;true&quot;로 설정되어 있으며, 이는 모든 이벤트 후에 양식이 자동 저장됨을 의미합니다.
* [!UICONTROL 트리거]에서 이벤트 발생 또는 특정 시간 이후에 자동 저장을 트리거하도록 구성합니다.
