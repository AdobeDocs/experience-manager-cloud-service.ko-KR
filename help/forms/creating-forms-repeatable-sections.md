...
1. 편집 모드에서 패널을 선택한 다음 를 선택합니다 ![cmppr](assets/cmppr.png). 사이드바의 속성에서 **[!UICONTROL 패널이 반복 가능하도록 설정]**. 다음에 대한 값 지정 **[!UICONTROL 최대]** 및 **[!UICONTROL 최소]** 필드.
...
1. 반복할 패널의 상위에는 반복 가능한 패널의 인스턴스를 관리할 수 있는 추가 및 삭제 단추가 포함되어야 합니다. 상위에 단추를 삽입하고 단추에 스크립트를 활성화하려면 다음 단계를 수행하십시오.
   ...
   1. 사이드바에서 버튼 구성 요소를 패널의 상위로 드래그 앤 드롭합니다. 구성 요소를 선택하고 을 선택합니다. ![규칙 편집](assets/edit-rules.png). 버튼의 규칙이 규칙 편집기에 열립니다.
   1. 규칙 편집기 창에서 **만들기**.
...
1. 반복할 패널의 상위에서 을 선택합니다. ![cmppr](assets/cmppr.png). 사이드바에서 속성을 볼 수 있습니다. 다음에서 **레이아웃** 드롭다운, 선택 **어코디언**.
1. 반복할 패널에서 을 선택합니다. ![cmppr](assets/cmppr.png). 사이드바에서 패널 속성을 볼 수 있습니다. 활성화 **패널이 반복 가능하도록 설정** 탭을 클릭하고 다음에 대한 값을 지정합니다 **최대** 및 **최소** 필드.
...
반복 가능한 하위 양식은 적응형 Forms의 반복 가능한 패널과 유사합니다. 위치 [!DNL AEM Forms] 디자이너에서 다음 단계를 수행하여 반복되는 하위 양식을 만듭니다.
...
1. 이제 양식 템플릿을 적응형 양식에 연결합니다. 자세한 단계는 를 참조하십시오. [템플릿을 기반으로 적응형 양식 만들기](creating-adaptive-form.md#create-an-adaptive-form-based-on-a-template).
...
XML 스키마와 복합 유형 요소의 minOccours 및 maxOccurs 속성에서 반복 가능한 패널을 만들 수 있습니다. XML 스키마에 대한 자세한 내용은 [XML 스키마를 양식 모델로 사용하여 적응형 Forms 만들기](adaptive-form-xml-schema-form-model.md).
