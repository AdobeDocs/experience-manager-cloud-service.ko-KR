...
데이터 모델 개체 및 속성을 만든 후에는 양식 데이터 모델(FDM)을 기반으로 적응형 Forms <!--and interactive communications-->을(를) 계속 만들 수 있습니다. 나중에 사용 가능하고 구성된 데이터 소스가 있으면 양식 데이터 모델(FDM)을 데이터 소스와 바인딩할 수 있습니다. 연결된 응용 Forms <!--and interactive communications-->에서 바인딩이 자동으로 업데이트됩니다. FDM(양식 데이터 모델)을 사용하여 적응형 Forms <!--and interactive communications-->을(를) 만드는 방법에 대한 자세한 내용은 [양식 데이터 모델 사용](using-form-data-model.md)을 참조하십시오.
