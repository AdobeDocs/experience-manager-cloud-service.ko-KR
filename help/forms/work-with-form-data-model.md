...
>[!NOTE]
>
>두 그룹의 멤버여야 합니다. **fdm-author** 및 **forms-user** 양식 데이터 모델(FDM)을 만들고 작업할 수 있는 그룹입니다. 다음으로 연락 [!DNL Experience Manager] 관리자가 그룹의 구성원이 되도록 합니다.
...
1. 에 로그인합니다 [!DNL Experience Manager] 작성자 인스턴스, 다음으로 이동 **[!UICONTROL Forms > 데이터 통합]**&#x200B;을 클릭하고 데이터 모델 개체를 추가할 양식 데이터 모델(FDM)을 엽니다.
1. 데이터 소스 창에서 데이터 소스를 확장하여 사용 가능한 데이터 모델 개체 및 서비스를 봅니다.
1. 양식 데이터 모델(FDM)에 추가할 데이터 모델 개체 및 서비스를 선택하고 을 선택합니다 **[!UICONTROL 선택 항목 추가]**.

   ![selected-object](assets/selected-objects.png)

   선택한 데이터 모델 개체 및 서비스

   다음 **[!UICONTROL 모델]** 탭에는 양식 데이터 모델(FDM)에 추가된 모든 데이터 모델 객체와 해당 등록 정보의 그래픽 표현이 표시됩니다. 각 데이터 모델 개체는 양식 데이터 모델(FDM)의 상자로 표시됩니다.
...
1. 선택 **[!UICONTROL 저장]** 를 클릭하여 양식 모델 개체를 저장합니다.

   >[!NOTE]
   >
   >적응형 양식 규칙을 사용하여 FDM(양식 데이터 모델)의 서비스 탭에서 구성한 서비스를 호출할 수 있습니다. 구성된 서비스는 규칙 편집기의 서비스 호출 작업에서 사용할 수 있습니다. 적응형 양식 규칙에서 이러한 서비스를 사용하는 방법에 대한 자세한 내용은 서비스 호출 및 규칙 값 설정 을 참조하십시오. [규칙 편집기](rule-editor.md).
...
1. 에 로그인합니다 [!DNL Experience Manager] 작성자 인스턴스, 다음으로 이동 **[!UICONTROL Forms > 데이터 통합]**&#x200B;을 클릭하고 데이터 모델 개체 또는 엔티티를 생성할 양식 데이터 모델(FDM)을 엽니다.
1. 선택 **[!UICONTROL 엔티티 만들기]**.
1. 다음에서 [!UICONTROL 데이터 모델 만들기] 대화 상자에서 데이터 모델 개체의 이름을 지정하고 을 선택합니다 **[!UICONTROL 추가]**. 데이터 모델 개체가 양식 데이터 모델(FDM)에 추가됩니다. 새로 추가된 데이터 모델 개체는 데이터 소스에 바인딩되지 않으며 다음 이미지에 표시된 대로 어떤 속성도 가지고 있지 않습니다.
...
1. 데이터 모델 개체가 데이터 소스에 바인딩되면 추가된 자식 속성은 동일한 이름 및 데이터 형식의 부모 데이터 모델 개체의 속성에 자동으로 바인딩됩니다.
   ...
   하위 속성을 데이터 모델 개체 속성과 수동으로 바인딩하려면 **[!UICONTROL 바인드 참조]** 필드. 다음 **[!UICONTROL 개체 선택]** 이 대화 상자에는 상위 데이터 모델 개체의 모든 속성이 나열됩니다. 바인딩할 속성을 선택하고 확인 표시 아이콘을 선택합니다. 하위 속성과 동일한 데이터 유형의 속성만 선택할 수 있습니다.

1. 선택 **[!UICONTROL 완료]** 하위 속성을 저장하고 **[!UICONTROL 저장]** 양식 데이터 모델(FDM)을 저장하는 방법 이제 하위 속성이 데이터 모델 개체에 추가됩니다.

데이터 모델 개체 및 속성을 만든 후에도 적응형 Forms을 계속 만들 수 있습니다 <!--and interactive communications--> 양식 데이터 모델(FDM) 기반 나중에 사용 가능하고 구성된 데이터 소스가 있으면 양식 데이터 모델(FDM)을 데이터 소스와 바인딩할 수 있습니다. 바인딩은 연결된 적응형 Forms에서 자동으로 업데이트됩니다 <!--and interactive communications-->. 적응형 Forms 만들기에 대한 자세한 정보 <!--and interactive communications--> 양식 데이터 모델(FDM) 사용을 참조하십시오. [양식 데이터 모델 사용](using-form-data-model.md).

### 데이터 모델 개체 및 속성 바인딩 {#bind-data-model-objects-and-properties}

양식 데이터 모델(FDM)과 통합할 데이터 소스를 사용할 수 있으면에서 설명한 대로 양식 데이터 모델(FDM)에 추가할 수 있습니다 [데이터 소스 업데이트](create-form-data-models.md#update). 그런 다음 바인딩되지 않은 데이터 모델 개체와 속성을 바인딩하려면 다음을 수행합니다.

1. 양식 데이터 모델에서 데이터 소스와 바인딩할 바인딩되지 않은 데이터 소스를 선택합니다.
1. **[!UICONTROL 속성 편집]**&#x200B;을 선택하십시오.
1. 다음에서 **[!UICONTROL 속성 편집]** 창에서 다음 옆에 있는 찾아보기 아이콘을 선택합니다 **[!UICONTROL 바인딩]** 필드. 열리면 **[!UICONTROL 개체 선택]** 양식 데이터 모델(FDM)에 추가된 데이터 소스를 나열하는 대화 상자

   ![select-object](assets/select-object.png)

1. 데이터 소스 트리를 확장하고 바인딩할 데이터 모델 개체를 선택하고 확인 표시 아이콘을 선택합니다.
1. 선택 **[!UICONTROL 완료]** 속성을 저장한 다음 **[!UICONTROL 저장]** 을 클릭하여 양식 데이터 모델을 저장합니다. 이제 데이터 모델 개체가 데이터 소스로 바인딩됩니다. 데이터 모델 개체가 더 이상 바인딩되지 않음으로 표시되지 않습니다.
...
1. 데이터 모델 개체 상단의 확인란을 선택하여 선택하고 **[!UICONTROL 속성 편집]**.
...
1. 선택 ![aem_6_3_edit](assets/edit.svg) 에 대한 서비스 인수 읽기 [인수를 사용자 프로필 속성, 요청 속성 또는 리터럴 값에 바인딩합니다.](#bindargument) 바인딩 값을 지정합니다.
1. 선택 **[!UICONTROL 완료]** 인수를 저장하려면 **[!UICONTROL 완료]** 속성을 저장한 다음 **[!UICONTROL 저장]** 양식 데이터 모델(FDM)을 저장하는 방법
...
1. 데이터 모델 개체 상단의 확인란을 선택하여 선택하고 **[!UICONTROL 연결 추가]**. 연결 추가 대화 상자가 열립니다.

   ![연결 추가](assets/add-association.png)

   >[!NOTE]
   >
   >데이터 모델 개체 및 서비스 외에도 OData 서비스 메타데이터 문서에는 두 데이터 모델 개체 간의 연결을 정의하는 탐색 속성이 포함됩니다. FDM(양식 데이터 모델)에서 연결을 추가할 때 이러한 탐색 속성을 사용할 수 있습니다. 자세한 내용은 [OData 서비스의 탐색 속성 작업](#work-with-navigation-properties-of-odata-services).
...
1. 연결 추가 창에서 다음을 수행합니다.
   ...
   * 선택한 모델 개체에서 데이터를 읽을 읽기 서비스를 선택합니다. 서비스 읽기 인수가 나타납니다. 를 편집하여 필요한 경우 인수를 변경하고 연결할 데이터 모델 개체의 속성에 바인딩합니다.
   ...
   선택 **[!UICONTROL 완료]** 를 클릭하여 인수를 저장합니다.

1. 선택 **[!UICONTROL 완료]** 연결을 저장한 다음 **[!UICONTROL 저장]** 양식 데이터 모델(FDM)을 저장하는 방법
...
>
>연결에 대한 확인란을 선택하고 을 선택하여 연결을 편집할 수 있습니다 **[!UICONTROL 연결 편집]**.
...
1. 선택 **[!UICONTROL 속성 편집]**. 다음 **[!UICONTROL 속성 편집]** 선택한 모델 개체, 속성 또는 서비스에 대한 창이 열립니다.
...
1. 선택 **[!UICONTROL 완료]** 속성을 저장한 다음 **[!UICONTROL 저장]** 양식 데이터 모델(FDM)을 저장하는 방법
...
1. 사용 **[!UICONTROL 계산됨]** 및 선택 **[!UICONTROL 완료]** 속성을 만듭니다.
...
1. 규칙 편집기 창에서 다음을 선택합니다 **[!UICONTROL 만들기]**. A **[!UICONTROL 값 설정]** 규칙 창이 열립니다.
...
1. 수학 표현식에서 을 선택합니다. **[!UICONTROL 이름]** 및 **[!UICONTROL 성]** 첫 번째 및 두 번째 객체에서 각각. 선택 **[!UICONTROL 플러스]** 를 연산자로 사용하십시오.
   ...
   선택 **[!UICONTROL 완료]** 다음을 선택합니다. **[!UICONTROL 닫기]** 을 눌러 규칙 편집기 창을 닫습니다. 이 규칙은 다음과 유사합니다.

   ![규칙](assets/rule.png)

1. 양식 데이터 모델(FDM)에서 다음을 선택합니다 **[!UICONTROL 저장]**. 계산된 속성이 구성되었습니다.
...
FDM(양식 데이터 모델)에서 OData 서비스를 구성할 경우 엔티티 컨테이너의 모든 탐색 속성을 FDM(양식 데이터 모델)의 서비스를 통해 사용할 수 있습니다. TripPin OData 서비스의 이 예에서는 `Person` 엔티티 컨테이너는 1개를 사용하여 읽을 수 있습니다. `GET LINK` FDM(양식 데이터 모델)의 서비스.

다음에서는 `GET LINK of Person /People` 양식 데이터 모델(FDM)의 서비스: 의 세 가지 탐색 속성에 대한 결합 서비스 `Person` tripPin OData 서비스의 엔터티입니다.

![nav-prop-service](assets/nav-prop-service.png)

을(를) 추가하고 나면 `GET LINK` 서비스를 양식 데이터 모델(FDM)의 서비스 탭으로 보내면 등록 정보를 편집하여 서비스에 사용할 출력 모델 객체와 탐색 등록 정보를 선택할 수 있습니다. 예를 들어, 다음 `GET LINK of Person /People` 다음 예제의 서비스에서는 Trip을 출력 모델 객체로 사용하고 탐색 속성을 Trip으로 사용합니다.
...
마찬가지로 다음을 선택할 수 있습니다 `GET LINK` 양식 데이터 모델(FDM)에서 연결을 추가할 때 해당 탐색 속성을 서비스하고 구성합니다. 단, 탐색 속성을 선택하려면 **[!UICONTROL 필드에 바인딩]** 이(가) (으)로 설정됨 **[!UICONTROL 리터럴]**.
...
1. 양식 데이터 모델(FDM)을 열고 다음을 선택합니다 **[!UICONTROL 샘플 데이터 편집]**. 샘플 데이터 편집 창에서 샘플 데이터를 생성하고 표시합니다.

   ![샘플 데이터 생성](assets/form_data_model_generate_sample_data_new.png)

1. 위치 **[!UICONTROL 샘플 데이터 편집]** 창, 필요에 따라 데이터 편집 및 선택 **[!UICONTROL 저장]**.
...
1. 양식 데이터 모델(FDM)에서 데이터 모델 객체 또는 서비스를 선택하고 다음을 선택합니다 **[!UICONTROL 테스트 모델 개체]** 또는 **[!UICONTROL 테스트 서비스]**, 각각
...
1. 테스트 코드에서 인수 값을 지정하고 다음을 선택합니다. **[!UICONTROL 테스트]**. 테스트가 성공하면 [출력] 창의 출력이 반환됩니다.
...
양식 데이터 모델(FDM)은 DermisBridge API를 호출하는 동안 입력으로 받은 데이터의 유효성을 검사합니다(양식 데이터 모델에서 사용할 수 있는 유효성 검사 기준에 따라). 유효성 검사는 다음을 기반으로 합니다 `ValidationOptions` api를 호출하는 데 사용되는 쿼리 개체에 설정된 플래그.
...
양식 데이터 모델(FDM)은 데이터 유형 제약 조건 목록을 기반으로 입력 데이터의 유효성을 검사합니다. 입력 데이터에 대한 제약 조건 목록은 데이터 소스에 따라 달라질 수 있습니다.
...
이제 적응형 Forms에서 사용할 준비가 된 작업 양식 데이터 모델(FDM)이 있습니다 <!--and interactive communications--> 워크플로. 자세한 내용은 [양식 데이터 모델(FDM) 사용](using-form-data-model.md).
