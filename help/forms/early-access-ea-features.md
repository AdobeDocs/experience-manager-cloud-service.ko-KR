...
description: ' [!DNL Adobe Experience Manager Forms] as a Cloud Service의 최신 조기 액세스 기능입니다.'
...
* 제품 팀과 함께 혁신하여 제품의 미래를 함께 형성하십시오. 사용 사례를 활용하여 개발을 안내하고, 혁신이 성숙해지도록 하며, 실제 과제를 해결하십시오.
* 출시 전에 피드백을 제공하고 문제를 해결하여 전체 출시 시 더욱 원활한 사용자 경험을 제공할 수 있습니다.

## Early Access 프로그램에 참여하는 방법

Early Access의 혁신을 위해 Early Access 프로그램에 참여하려는 경우 [aem-forms-ea@adobe.com](mailto:aem-forms-ea@adobe.com)(으)로 메일을 보내 액세스를 요청하면 됩니다. 전체 또는 특정 혁신에 대한 액세스를 요청할 수 있습니다.
...
* **제품 쿼리용 AI 어시스턴트**: AEM 양식 관련 질문에 대한 즉각적인 답변을 얻습니다. 당사의 AI 도우미는 자체 개인 지식 베이스로 작동하여 플랫폼 내에서 통찰력 있는 지침과 추천을 직접 제공합니다.
...
시작할 준비가 되셨습니까? [Adobe Workfront Fusion에 적응형 양식 제출](/help/forms/submit-adaptive-form-to-workfront-fusion.md) 설명서는 혁신을 사용하기 위한 단계별 안내서를 제공합니다.

<!-- <br/> ![Adobe Workfront](/help/forms/assets/adobe-workfront.png) <br/> -->

### 대화형 Forms

대화형 Forms 혁신을 통해 모든 AEM Sites 페이지에서 친근한 챗봇과 같은 양식과 상호 작용할 수 있습니다.

간단히 대화 Forms 구성 요소를 AEM Sites 페이지로 끌어다 놓고 시작할 수 있습니다. 코딩이 필요하지 않으므로 누구나 몰입형 및 사용자 친화적인 대화형 Forms 경험을 빠르게 만들 수 있습니다.
...
데이터 전송 프로세스를 간소화함으로써 커넥터는 웹 사이트의 변환율을 향상하는 데 도움이 될 수 있습니다. 리드 데이터를 Marketo으로 자동으로 보내면 자격을 갖춘 리드가 신속하게 육성 프로그램에 입력되도록 할 수 있습니다.

기본적으로 AEM Forms에서 Marketo으로 연결되는 커넥터는 두 플랫폼의 장점을 모두 활용하여 보다 효율적이고 효과적인 마케팅 깔때기를 만들 수 있도록 도와줍니다.

시작할 준비가 되셨습니까? 다음 문서에서는 혁신을 사용하기 위한 자세한 지침을 제공합니다.
...
* **가져오기 및 내보내기:** 환경 전반에 걸쳐 해당 조각과 데이터 모델과 함께 대화형 통신을 원활하게 마이그레이션하고 다시 사용합니다.

* **템플릿 잠금**: 템플릿 내의 콘텐츠 및 레이아웃 요소를 잠그어 브랜드 무결성을 유지하고 무단 수정을 방지합니다.
...
* **게시 인스턴스에서 연결 UI를 호출합니다**: 이제 게시 인스턴스에서 직접 연결 UI를 호출할 수 있습니다. 이 기능은 필요한 구성, 페이로드 구조 및 호출 흐름을 정의하므로 통합을 단순화하고 환경 전반을 신속하게 배포할 수 있습니다.

##### 동적 페이지 번호 지정

다중 페이지 문서에서 명확하고 일관된 페이지 매김을 위해 마스터 페이지에 자동으로 &quot;페이지 번호 # of ##&quot;를 표시합니다.
...
AEM Forms HTML5 Forms을 사용하면 기존 XFA(XML Forms 아키텍처) 양식 템플릿을 HTML5 형식으로 렌더링하여 XFA 기반 PDF이 지원되지 않는 최신 브라우저 및 모바일 디바이스에서 액세스할 수 있습니다. 이 기능은 기존 PDF forms과 최신 웹 경험 간의 차이를 해소합니다.

![HTML 5 Forms 미리 보기](/help/forms/assets/mobile_form_on_an_ipad_date_14.png)

**주요 기능:**

* **XFA 양식 템플릿의 HTML5 기반 렌더링**: 기존 XFA 기반 양식을 HTML5 형식으로 렌더링하여 클라이언트 플랫폼을 HTML5는 지원하지만 XFA Forms을 사용하는 Adobe Reader은 지원하지 않는 모바일 장치(Apple iPad, Android 태블릿, 스마트폰 등)로 확장합니다.

* **모바일 지원 양식**: HTML5 Forms은 모바일 지원 기능을 제공하여 HTML5 브라우저를 사용하는 태블릿이나 스마트폰에 현재 솔루션과 워크플로우를 확장할 수 있도록 지원합니다.
...
* **개발자 생산성**: CLI가 반복 설정 작업을 처리하는 동안 비즈니스 논리 및 UI 구축에 집중합니다.
* **원활한 통합**: 생성된 구성 요소는 기존 AEM Forms 프로젝트를 사용하고 통합할 수 있습니다.

시작할 준비가 되셨습니까? AEM Forms CLI 도구는 Forms as a Cloud Service 배포를 위한 조기 액세스 프로그램에서 사용할 수 있습니다. 액세스 권한을 요청하려면 공식 주소에서 [aem-forms-ea@adobe.com](mailto:aem-forms-ea@adobe.com)(으)로 조직 ID 및 프로그램 세부 사항을 전자 메일로 보내십시오.
...
이제 적응형 Forms은 새로운 RESTful 웹 서비스 제출 동작을 통해 캡처된 데이터를 인증된 외부 REST 종단점으로 원활하게 전송할 수 있습니다.

* 지원되는 표준: Swagger 2.0 및 3.0을 통해 API 통합이 용이
...
![오른쪽에서 왼쪽(RTL) 언어 지원](mobile-responsive.png)

적응형 Forms의 RTL(오른쪽에서 왼쪽) 언어 지원을 통해 다음을 수행할 수 있습니다.

* **사용자 기반 확장**: 이제 RTL 언어에 익숙한 전 세계 20억 명 이상의 사용자에게 비즈니스를 제공할 수 있습니다.

* **향상된 사용자 경험 제공**: Forms은 오른쪽에서 왼쪽으로 자연스러운 텍스트 흐름, 적절한 UI 요소 정렬 및 사용자 읽기 습관을 미러링하는 직관적인 레이아웃으로 완벽하게 렌더링합니다. 이렇게 하면 혼동이 줄어들고 양식 완료율이 높아집니다.
...
AEM Forms 문서 Assurance API는 AEM Forms Cloud Service 통신 API 내의 도구 세트로서, PDF 문서의 보안 및 사용자 상호 작용을 관리할 수 있습니다.

다음은 Doc Assurance API가 수행할 수 있는 작업에 대한 분류입니다.

* **문서 암호화 및 암호 해독**: 암호화로 내용을 읽을 수 없게 하여 문서를 보호합니다. 전체 문서, 특정 컨텐츠, 메타데이터 또는 첨부 파일을 포함하여 PDF의 암호화되는 부분을 제어할 수 있습니다.
...
문서 생성 API에는 생성된 PDF을 Azure Blob 저장소에 직접 업로드할 수 있는 선택적 기능이 포함되어 있습니다. 문서 생성 API를 사용하여 PDF를 Azure Blob Storage에 직접 업로드하면 다음과 같은 몇 가지 주요 이점이 있습니다.
...
* [JSON 배열에서 속성 값 검색](/help/forms/invoke-service-enhancements-rule-editor.md#retrieve-property-values-from-a-json-array): 사용자 지정 함수로 API를 호출하여 JSON 배열에서 값을 추출하고 양식 필드에 직접 바인딩합니다.

### Forms Manager의 버전 관리 지원

Forms Manager는 이제 적응형 Forms(핵심 구성 요소 및 기초 구성 요소), 양식 단편, 테마, XDP 템플릿 및 바이너리 자산에 대한 버전 관리를 지원합니다. Forms 및 문서 콘솔에서 버전을 만들고, 기록을 보고, 이전 상태를 복원할 수 있습니다. [Forms 관리자에서 양식 버전 관리](/help/forms/manage-form-versions-forms-manager.md)를 참조하십시오.

### 양식 구성 요소에 대한 액세스 가능성 개선

적응형 Forms 핵심 구성 요소는 확인란 그룹, 라디오 버튼 그룹 및 패널에 대한 WCAG 호환 의미 체계 마크업을 도입합니다. 이러한 구성 요소는 이제 `<fieldset>` 및 `<legend>` 요소를 활용하여 그룹 레이블과 보조 기술 옵션 간의 의미 있는 관계를 설정합니다. 적응형 Forms에서 [필드 집합 및 범례 지원](/help/forms/creating-accessible-adaptive-forms.md#fieldset-legend-accessibility)을 참조하세요.

## 추가 참조

* [AEM Forms의 최신 혁신](/help/forms/latest-innovations.md)
