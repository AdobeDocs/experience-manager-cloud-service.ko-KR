...
하나 이상의 템플릿과 각 템플릿에 대한 여러 XML 데이터 레코드가 있는 시나리오를 고려하십시오. 커뮤니케이션 API를 사용하여 각 레코드에 대한 인쇄 문서를 생성할 수 있습니다. <!-- You can also combine the records into a single document. --> 결과가 비대화형 PDF 문서입니다. 비대화형 PDF 문서에서는 사용자가 해당 필드에 데이터를 입력할 수 없습니다.
...
* **기본 인증**: 기본 인증은 HTTP 프로토콜에 내장된 간단한 인증 구성표입니다. 클라이언트는 Basic이라는 단어 다음에 공백과 base64로 인코딩된 문자열 사용자 이름 :password이 포함된 Authorization 헤더로 HTTP 요청을 보냅니다. 예를 들어, 클라이언트가 관리자/관리자로 권한을 부여하려면 기본 [base64로 인코딩된 문자열 사용자 이름]: [base64로 인코딩된 문자열 암호]를 보냅니다.

* **토큰 기반 인증:** 토큰 기반 인증은 액세스 토큰(전달자 인증 토큰)을 사용하여 Experience Manager as a Cloud Service에 요청을 수행합니다. AEM Forms as a Cloud Service은 액세스 토큰을 안전하게 검색할 수 있는 API를 제공합니다. 토큰을 검색하고 사용하여 요청을 인증하려면 다음을 수행하십시오.
   ...
   1. [환경에 Experience Manager as a Cloud Service 자격 증명을 설치](https://experienceleague.adobe.com/docs/experience-manager-learn/getting-started-with-aem-headless/authentication/service-credentials.html?lang=ko). (Application Server, 웹 서버 또는 기타 비 AEM 서버)가 Cloud Service에 요청을 전송(호출)하도록 구성되었습니다.
   ...
   1. [Experience Manager 환경에서 기술 계정 사용자에 대한 적절한 권한을 설정합니다](https://experienceleague.adobe.com/docs/experience-manager-learn/getting-started-with-aem-headless/authentication/service-credentials.html?lang=ko#configure-access-in-aem).

  >[!NOTE]
  >
  >Adobe에서는 프로덕션 환경에서 토큰 기반 인증을 사용하는 것이 좋습니다.

  >[!IMPORTANT]
  >
  > 자세한 내용은 [OAuth 서버 간 인증](/help/forms/oauth-api-authetication.md) 및 [JWT 서버 간 인증](/help/forms/jwt-api-authentication.md)을 참조하십시오.
<!-- 
...
[API 참조 설명서](https://developer.adobe.com/experience-manager-forms-cloud-service-developer-reference/)에서는 API에서 제공하는 모든 매개 변수, 인증 방법 및 다양한 서비스에 대한 자세한 정보를 제공합니다. API 참조 설명서는 API 정의 파일도 .yaml 형식으로 제공합니다. .yaml 파일을 다운로드하고 [Postman](https://www.postman.com/)에 업로드하여 API의 기능을 확인할 수 있습니다.

>[!VIDEO](https://video.tv.adobe.com/v/335771)

>[!NOTE]
>
> AEM Forms Communication API를 호출하는 자세한 단계는 [OAuth 서버 간 인증을 사용하여 AEM Forms Communications API 호출](/help/forms/aem-forms-cloud-service-communications-on-demand-processing.md) 문서를 참조하십시오.
