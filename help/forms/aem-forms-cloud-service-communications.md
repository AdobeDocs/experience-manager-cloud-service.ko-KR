...
* **토큰 기반 인증:** 토큰 기반 인증은 액세스 토큰(전달자 인증 토큰)을 사용하여 Experience Manager as a Cloud Service에 요청을 수행합니다. AEM Forms as a Cloud Service은 액세스 토큰을 안전하게 검색할 수 있는 API를 제공합니다. 토큰을 검색하고 사용하여 요청을 인증하려면 다음을 수행하십시오.
   ...
   1. [환경에 Experience Manager as a Cloud Service 자격 증명을 설치](https://experienceleague.adobe.com/docs/experience-manager-learn/getting-started-with-aem-headless/authentication/service-credentials.html). (Application Server, 웹 서버 또는 기타 비 AEM 서버)가 Cloud Service에 요청을 전송(호출)하도록 구성되었습니다.
   ...
   1. [Experience Manager 환경에서 기술 계정 사용자에 대한 적절한 권한을 설정합니다](https://experienceleague.adobe.com/docs/experience-manager-learn/getting-started-with-aem-headless/authentication/service-credentials.html?lang=en#configure-access-in-aem).
