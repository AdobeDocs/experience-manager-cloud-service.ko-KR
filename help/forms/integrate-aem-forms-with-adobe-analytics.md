...
- **전환 Funnel 여정**: 진행 및 중단 패턴을 보여 주는 여러 단계 양식을 통한 사용자 분석
...
AEM Forms은 [Adobe Analytics](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/overview.html?lang=en)과(와) 원활하게 통합되어 게시된 양식의 성능 지표를 자동으로 캡처하고 추적합니다. 추가 구성 없이 인증된 사용자와 익명의 사용자 모두의 동작을 모니터링할 수 있습니다.
...
- **전환 Funnel 분석**: 특정 전환 지점 및 드롭오프 단계를 측정하도록 사용자 지정 이벤트를 설정합니다.
...
- **Funnel 시각화**: 여러 단계 양식을 통해 사용자 진행 상황을 시각화하고 최적화 기회를 식별합니다
...
- **Funnel 최적화**: 양식 진행에서 병목 현상을 식별하고 해결합니다.
- **A/B 테스트 통합**: 양식 변형 간에 funnel 성능 비교
...
Adobe Launch 구성을 만들기 전에 Adobe Launch를 클라우드 솔루션으로 사용하여 [Adobe IMS 구성을 만드십시오](https://experienceleague.adobe.com/docs/experience-manager-learn/sites/integrations/experience-platform-launch/connect-aem-launch-adobe-io.html).
...
적응형 양식에 대해 [!DNL Adobe Analytics]을(를) 사용하도록 설정한 후 AEM Forms과 [ 사이에 적절한 데이터 이벤트 흐름이 있으면 ](https://experienceleague.adobe.com/docs/launch-learn/implementing-in-websites-with-launch/implement-solutions/analytics.html?lang=en#validate-the-page-view-beacon)유효성 검사[!DNL Adobe Analytics]할 수 있습니다. AEM Forms과 Adobe Analytics 통합이 완료되었습니다. 이제 [Adobe Analytics에서 보고서를 구성하고 볼 수 있습니다](#view-reports-adobe-analytics).
...
- **양식 전환 Funnel**: 양식 완료 프로세스를 통해 사용자 진행 시각화
...
A: Form Analytics는 특히 양식 내의 사용자 상호 작용에 중점을 두며 필드 수준 동작, 완료 패턴 및 포기 지점에 대한 자세한 통찰력을 제공합니다. 일반 웹 사이트 분석은 페이지 보기 및 전체 사용자 여정을 추적하는 반면, 양식 분석에서는 양식별 사용자 경험, 유효성 검사 오류, 필드 완료 시간 및 양식 자체 내의 변환 funnel 분석에 대한 세분화된 데이터를 제공합니다.
...
A: 여러 단계 양식에는 각 단계를 통한 진행률을 추적하기 위한 특별한 구성이 필요합니다. 단계 완료를 위한 사용자 지정 이벤트를 구현하고, 단계 간 드롭오프점을 시각화하도록 funnel 분석을 구성하고, 사용자 지정 변수를 사용하여 양식 마법사를 통해 사용자 경로를 추적합니다. Adobe Analytics은 다중 페이지 양식 추적에 대한 특정 지침을 제공합니다.
...
A: 분석 스크립트를 비동기적으로 로드하고, 중요하지 않은 추적을 위해 소극적 로드를 구현하고, 사용자 지정 변수 및 이벤트 수를 줄이고, Adobe Launch에서 효율적인 규칙 구성을 사용하고, Core Web Vitals을 모니터링하여 분석이 사용자 경험에 부정적인 영향을 주지 않도록 하여 분석 구현을 최적화합니다.
