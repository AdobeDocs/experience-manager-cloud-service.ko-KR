...
규칙 편집기를 사용하면 가로 탭, 세로 탭, 아코디언 또는 마법사와 같은 탐색 단추를 패널 레이아웃에 추가할 수 있습니다. 이러한 버튼은 양식의 서로 다른 패널 간에 전환을 단순화하고 포커스를 선택한 패널로 전환하여 사용자 경험을 향상시킵니다.

탐색이 탭이 아닌 버튼으로 용이한 애플리케이션의 프로필 설정 섹션과 상호 작용하고 있다고 가정해 보십시오. 기본 대시보드에서 프로필 설정을 입력하면 프로필의 다양한 측면을 다루는 일련의 패널이 표시됩니다. **개인 정보**, **계정 보안** 및 **알림 환경 설정**.

각 패널에는 특정 정보를 업데이트하는 관련 필드와 옵션이 포함되어 있습니다. `Next` 및 `Back`과(와) 같은 탐색 단추를 눈에 띄게 배치하여 이 패널 사이를 이동할 수 있습니다. `Next`을(를) 클릭하여 사용자를 **계정 보안** 패널로 이동하고 `Back`을(를) 클릭하여 **개인 정보** 패널로 돌아갑니다. 이러한 탐색 방식은 문맥을 잃지 않고 섹션 간의 원활한 전환을 보장하여 부드럽고 직관적인 사용자 경험을 제공합니다. 탐색 버튼을 사용하면 프로필 설정 관리 프로세스가 간소화되어 상호 작용이 보다 체계적이고 사용자 친화적으로 이루어집니다.

`Navigate among the panels` 규칙을 사용하여 다른 패널 간에 전환할 수 있는 단추에 대한 탐색 규칙을 만들 수 있습니다.  `Shift focus to the next item` 특성을 선택하여 포커스를 레이아웃의 다음 패널로 이동합니다.

![다음 패널 규칙](/help/forms/assets/rule-editor-navigate-in-panel-next.png){width=50%}

`Next` 단추를 클릭하면 포커스가 레이아웃의 다음 패널로 이동합니다.

![다음 단추를 사용하여 패널에서 탐색](/help/forms/assets/navigate-in-panel.gif)

마찬가지로 `Previous` 단추에 대한 규칙을 만들어 포커스를 이전 패널로 이동할 수 있습니다.

![이전 패널 규칙](/help/forms/assets/rule-editor-navigate-in-panel-previous.png){width=50%}
