...
description: 적응형 Forms 규칙 편집기를 사용하면 코딩 또는 스크립팅 없이 다이내믹 동작을 추가하고 복잡한 논리를 양식에 빌드할 수 있습니다. 규칙 구성 선택을 위한 규칙 및 지침을 이해합니다. 규칙 편집기에서 사용 가능한 연산자 유형 및 이벤트에 대해 자세히 알아보십시오.
...
규칙 편집기 기능은 forms 비즈니스 사용자 및 개발자가 적응형 양식 개체에 대 한 규칙을 작성 하는 기능을 제공 합니다. 이러한 규칙은 양식에서 사전 설정 된 조건, 사용자 입력 및 사용자 작업을 기반으로 양식 개체를 트리거하는 작업을 정의 합니다. 정확도와 속도를 보장 하 경험 양식 채우기를 보다 효율적으로 간소화할 수 있습니다.

규칙 편집기에서는 규칙을 작성할 수 있는 직관적인 단순화 된 사용자 인터페이스를 제공 합니다. 규칙 편집기는 모든 사용자에 대 한 시각적 편집기를 제공 합니다.<!-- In addition, only for forms power users, rule editor provides a code editor to write rules and scripts. --> 규칙을 사용 하 여 적응형 양식 객체에 대해 수행할 수 있는 몇 가지 주요 작업은 다음과 같습니다.
...
* **변경 됨 (이벤트):** 사용자가 입력 한 값 또는 양식 개체에 대해 선택 된 옵션을 변경 하는 경우 true를 반환 합니다.
* **탐색 (이벤트):** 탐색 개체를 사용자 클릭 하면 true를 반환 합니다. 탐색 객체는 패널 간에 이동하는 데 사용됩니다.
...
규칙 편집기에서는 규칙을 작성 하는 데 사용할 수 있는 미리 정의 된 규칙 유형 세트를 제공 합니다. 각 규칙 유형을 자세히 살펴보겠습니다. 규칙 편집기에서 규칙 작성에 대 한 자세한 내용은 규칙 ](rule-editor.md#p-write-rules-p) 작성을 참조 [ 하십시오.

### [!UICONTROL http 화이트보드] {#whenruletype}

규칙 유형 **-작업-대체 작업** 규칙 생성 또는 조건 작업 **구문을 사용할]** 수도 있습니다 **.**[!UICONTROL  이 규칙 유형에 서 먼저 평가 조건을 지정 하 고 조건이 충족 되는 경우 트리거할 작업을 시작 합니다 ( `True` ). When 규칙 유형을 사용하는 동안 여러 AND 및 OR 연산자를 사용하여 다음을 만들 수 있습니다 [중첩된 표현식](#nestedexpressions).
...
다음 예제는 값을 로 가져옵니다. `dependentid` 필드를 입력으로 설정하고 값을 `Relation` 출력 필드 `Relation` 의 인수 `getDependent` 양식 데이터 모델 서비스.
...
>[!NOTE]
>
>또한 규칙의 값 집합을 사용 하 여 드롭다운 목록 구성 요소의 모든 값을 양식 데이터 모델 서비스 또는 웹 서비스의 출력에서 채울 수 있습니다. 그러나 선택하는 출력 인수는 배열 형식이어야 합니다. 배열에 반환 되는 모든 값은 지정 된 드롭다운 목록에서 사용할 수 있게 됩니다.

### [!UICONTROL 표시] {#show}

**[!UICONTROL 보기]** 규칙 유형을 사용 하 여 조건이 충족 되는지 여부에 따라 양식 개체를 표시 하거나 숨기기 하는 규칙를 작성할 수 있습니다. 조건이 충족 되지 않거나 반환 `False` 되지 않는 경우에도 보기 규칙 유형으로 숨김 작업이 트리거됩니다.

일반적인 보기 규칙은 다음과 같이 구성 됩니다.
...
토글 버튼 탭을 누르면 양식 개체 및 함수 창을 전환 합니다.
...
1. 클라이언트 만들기라이브러리에 대해 다음 단계를 수행합니다. 자세한 내용은 [클라이언트측 라이브러리 사용](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/developing/full-stack/clientlibs.html#developing).
