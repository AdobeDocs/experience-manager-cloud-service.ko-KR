...
**Action-Condition** 이 구문에서 규칙는 먼저 트리거할 작업을 정의한 다음 평가 조건을 정의합니다. 이 구문의 또 다른 변형은 action-condition-alternate action으로, 조건이 False를 반환하는 경우 트리거할 대체 작업도 정의합니다.

작업의 보기, Hide, Enable, Disable, Set 값 Of 및 Validate규칙 형식을 규칙 편집기 작업 조건 규칙 적용합니다. 기본적으로 보기에 대한 대체 작업은 Hide(숨기기)이고 Enable(사용)에 대한 대체 작업은 Disable(사용 안 함)이며 그 반대입니다. 기본 대체 동작은 변경할 수 없습니다.
...
다음 **[!UICONTROL 날짜]** 규칙 유형은 **condition-action-alternate action** 규칙 구문 또는 경우에 따라 **조건-작용** 구문입니다. 이 규칙 유형에서는 먼저 평가할 조건을 지정하고 조건이 충족되면 트리거할 작업을 지정합니다( `True`). When 규칙 형식을 사용하는 동안 여러 AND 및 OR 연산자를 사용하여 중첩 식을 만들 [수 있습니다](#nestedexpressions).

When 규칙 유형을 사용하면 양식 개체에 대한 조건을 평가하고 하나 이상의 개체에 대해 작업을 수행할 수 있습니다.

간단히 말해서 일반적인 When 규칙은 다음과 같이 구성됩니다.
...
라디오 단추 또는 목록과 같은 다중 값 구성 요소가 있는 경우 해당 구성 요소에 대한 규칙 만드는 동안 옵션이 자동으로 검색되고 규칙 작성자가 사용할 수 있습니다. 옵션 값을 다시 입력할 필요는 없습니다.
...
**[!UICONTROL 서비스 호출]** 양식 데이터 모델로 구성된 서비스를 호출합니다. [Invoke Service] 작업을 선택하면 필드가 나타납니다. 필드를 탭하면 인스턴스의 모든 양식 데이터 모델에 [!DNL Experience Manager] 구성된 모든 서비스가 표시됩니다. 양식 데이터 모델 서비스를 선택하면 양식 개체를 지정된 서비스에 대한 입력 및 출력 매개 변수와 매핑할 수 있는 필드가 더 많이 나타납니다. 양식 데이터 모델 서비스를 호출하는 예제 규칙을 참조하십시오.
...
(조건 1 AND 조건 2 AND 조건 3)은 TRUE입니다.

다음 예제에서는 필드의 값을 입력으로 사용하고 필드 값을 `dependentid` `Relation` 양식 데이터 모델 서비스의 인수 `getDependent` 출력 `Relation` 으로 설정합니다.
