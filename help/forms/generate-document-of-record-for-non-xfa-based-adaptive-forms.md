---
title: AEM Forms에 대한 제출 PDF(이전 기록 문서) 생성
description: 적응형 Forms에 대한 양식 제출에서 제출 PDF을 생성하는 방법을 알아봅니다. 보관 또는 참조를 위해 제출된 양식의 PDF을 만듭니다.
...
# 적응형 Forms을 위한 제출 PDF(이전 기록 문서) 생성
...
양식을 작성하거나 제출할 때 인쇄 또는 문서 형식으로 양식의 기록을 유지할 수 있습니다. 이 레코드를 제출 PDF(이전 Document of Record 또는 DoR)라고 합니다. 제출된 양식의 인쇄용 PDF 입니다. 고객이 나중에 제출한 정보에 대해서는 제출 PDF 를 참조하거나 제출 PDF 를 사용하여 양식 및 컨텐츠를 PDF 형식으로 함께 보관할 수도 있습니다.

![제출 PDF(이전 기록 문서)](assets/document-of-record.png)

제출 서류 PDF을 만들기 위해 XFA 또는 Acroform 기반 템플릿을 적응형 양식을 통해 수집된 데이터와 병합합니다. 제출 서류 PDF을 자동으로 또는 온디맨드로 생성할 수 있습니다.
온디맨드 옵션을 사용하면 사용자 지정 XFA 또는 Acroform 기반 템플릿을 지정하여 제출 서류 PDF에 사용자 지정 모양을 제공할 수 있습니다.

다음과 같은 작업을 수행할 수 있습니다.

* [XFA 기반 제출 PDF 생성](#generate-an-XFA-based-document-of-record)
* [Acroform 기반(Acrobat Form PDF) 제출 PDF 생성](#generate-an-Acroform-based-document-of-record)
* [제출 서류 PDF 자동 생성](#auto-generate-a-document-of-record)

## 시작하기 전 {#components-to-automatically-generate-a-document-of-record}

제출 PDF에 필요한 에셋에 대한 학습을 시작하고 준비하기 전에:

**기본 템플릿:** Forms Designer 또는 Acrobat 양식(AcroForm)에서 만든 XFA 템플릿(XDP 파일)입니다. [기본 템플릿](#base-template-of-a-document-of-record)은(는) 제출 PDF의 스타일 및 브랜딩 정보를 지정하는 데 사용됩니다. 이전에 XFA 템플릿(XDP 파일)을 AEM Forms 인스턴스에 업로드했습니다

**적응형 양식:** 제출 PDF을 생성할 적응형 양식입니다.

## XFA 기반 제출 PDF 생성 {#generate-an-XFA-based-document-of-record}

XFA 템플릿(XDP 파일)을 AEM Forms 인스턴스에 업로드합니다. XFA 템플릿(XDP 파일)을 제출 PDF의 템플릿으로 사용하도록 적응형 양식을 구성하려면 다음 단계를 수행하십시오.
...
이제 적응형 양식이 XDP 파일을 제출 PDF의 템플릿으로 사용하도록 구성되었습니다. 다음 단계는 [적응형 양식 구성 요소를 해당 템플릿 필드와 바인딩](#bind-adaptive-form-components-with-template-fields)하는 것입니다.

## Acroform 기반 제출 PDF 생성 {#generate-an-Acroform-based-document-of-record}

Adobe Acrobat PDF(Acroform)를 AEM Forms 인스턴스에 업로드합니다. Adobe Acrobat PDF(Acroform)를 제출 PDF의 템플릿으로 사용하도록 적응형 양식을 구성하려면 다음 단계를 수행하십시오.
...
이제 적응형 양식이 제출 PDF용 템플릿으로 Acroform을 사용하도록 구성되었습니다. 다음 단계는 [적응형 양식 구성 요소를 해당 템플릿 필드와 바인딩](#bind-adaptive-form-components-with-template-fields)하는 것입니다.

## 제출 서류 자동 생성 PDF {#auto-generate-a-document-of-record}

적응형 양식이 제출 PDF을 자동으로 생성하도록 구성된 경우 양식이 변경될 때마다 해당 제출 PDF이 즉시 업데이트됩니다. 예를 들어 필드가 기존 적응형 양식에서 제거되는 경우 해당 필드도 제거되며 제출 PDF에 표시되지 않습니다. 제출 서류 PDF을 자동으로 생성할 수 있는 많은 다른 이점이 있습니다.

* 양식 개발자는 데이터 바인딩을 수동으로 유지 관리할 필요가 없습니다. 자동 생성된 제출 PDF은 데이터 바인딩 관련 업데이트를 처리합니다.
* 양식 개발자는 제출 PDF에서 제외로 표시된 필드를 수동으로 숨길 필요가 없습니다. 자동 생성된 제출 PDF은 이러한 필드를 제외하도록 사전 구성되어 있습니다.
* 자동 생성된 제출 PDF 옵션을 사용하면 제출 PDF에 대한 양식 템플릿을 만드는 데 필요한 시간이 절약됩니다.
* 자동 생성된 제출 PDF 옵션을 사용하면 다양한 기본 템플릿을 사용하여 다양한 스타일 및 모양을 사용할 수 있습니다. 조직의 Submission PDF에 가장 적합한 스타일과 모양을 선택하는 데 도움이 됩니다. 스타일을 지정하지 않으면 시스템 스타일이 기본값으로 설정됩니다.
* 자동 생성된 제출 PDF은 양식의 모든 변경 사항이 제출 PDF에 즉시 반영되도록 합니다.

제출 PDF을 자동으로 생성하도록 적응형 양식을 구성하려면 다음 단계를 수행하십시오.
...
적응형 양식 필드를 템플릿 필드와 바인딩하여 캡처된 양식 데이터를 해당 제출 PDF 필드에 표시합니다. 적응형 양식 구성 요소를 해당 제출 PDF 템플릿 필드와 바인딩하려면 다음을 수행하십시오.
...
전자 메일 보내기, Experience Manager Workflow 제출 액션을 [기록 문서 단계와 함께 사용하거나 기타 제출 액션](configuring-submit-actions.md)을 사용하여 제출 PDF을 받을 수 있습니다.

## 제출 PDF 템플릿에 대한 증분 업데이트 {#document-of-record-template-incremental-updates}

적응형 양식 및 해당 제출 PDF 템플릿은 시간이 지남에 따라 발전할 수 있습니다. 적응형 양식 또는 제출 PDF 템플릿에 필드를 추가, 제거 또는 수정하도록 선택할 수 있습니다.

제출 서류 PDF 템플릿을 변경하고 변경된 템플릿을 AEM Forms에 업로드하면 적응형 Forms 편집기가 변경된 바인딩을 자동으로 감지하고 새 바인딩이 필요한 적응형 양식 구성 요소에 대해 알려 줍니다. 제출 PDF 템플릿을 증분 업데이트할 수 있습니다.

예를 들어 조직 *We.Retail*&#x200B;에는 AcroForm 기반 제출 PDF 템플릿 *we-retail-invoice.pdf*&#x200B;이 있습니다. 템플릿은 다음과 같습니다.
...
양식 개발자는 적응형 Forms 필드를 해당 제출 PDF 템플릿과 바인딩합니다.

>[!VIDEO](assets/we-retail-binding.mp4)

이제 적응형 양식이 제출되면 업데이트된 제출 PDF이 만들어집니다.

![업데이트됨-](assets/we-retail-new-invoice-sent-to-customer.png)

## 제출 PDF 작업 시 주요 고려 사항 {#key-considerations-when-working-with-document-of-record}

적응형 Forms용 제출 PDF 작업을 할 때 다음 고려 사항 및 제한 사항을 기억하십시오.

* **서식 있는 텍스트 지원**: 제출 PDF은 서식 있는 텍스트 필드의 HTML 마크업 태그를 지원합니다. 지원되는 태그 및 액세스 가능성 고려 사항에 대한 자세한 내용은 [제출 PDF에서 지원되는 HTML 마크업 태그](html-markup-tags-support-in-document-of-record.md)를 참조하십시오.
* 적응형 양식의 문서 조각은 제출 PDF에 표시되지 않습니다. 그러나 적응형 양식 조각은 지원됩니다.
* XML 스키마 기반 적응형 양식에 대해 생성된 제출 PDF의 컨텐츠 바인딩은 지원되지 않습니다.
* 현지화된 버전의 Submission PDF은 사용자가 Submission PDF 렌더링을 요청할 때 로케일에 대한 요청에 따라 생성됩니다. 제출 PDF의 현지화는 적응형 양식의 현지화와 함께 발생합니다. <!-- For more information on localization of Document of Record and Adaptive Forms see Using AEM translation workflow to localize Adaptive Forms and Document of Record.-->
...
다음 표에서는 적응형 양식 구성 요소와 해당 XFA 구성 요소 및 제출 PDF에 표시되는 경우에 대해 설명합니다.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td> </td>
   <td>false</td>
   <td>제출 PDF 템플릿에서 사용할 수 없습니다. 첨부 파일을 통해 제출 PDF에서만 사용할 수 있습니다.</td>
  </tr>
 </tbody>
</table>
...
|---|---|---|
| 이미지 | 이미지 | TextDraw 및 Image 구성 요소는 바인딩되거나 바인딩되지 않았더라도 제출 PDF 설정을 사용하여 제외되지 않는 한 XSD 기반 적응형 양식용 제출 PDF에 항상 나타납니다. |

### 표 {#tables}

머리글, 바닥글 및 행과 같은 적응형 Forms 테이블 구성 요소는 해당 XFA 구성 요소에 매핑됩니다. 반복 가능한 패널을 Submission PDF의 테이블에 매핑할 수 있습니다.

## 제출 서류 PDF의 기본 템플릿 {#base-template-of-a-document-of-record}

기본 템플릿은 Submission PDF에 스타일 및 모양 정보를 제공합니다. 자동 생성된 제출 PDF의 기본 모양을 사용자 지정할 수 있습니다. 예를 들어 기본 템플릿을 사용하여 제출 PDF의 머리글에 회사 로고를 추가하고 바닥글에 저작권 정보를 추가할 수 있습니다.

기본 템플릿의 마스터 페이지는 제출 PDF 템플릿의 마스터 페이지로 사용됩니다. 마스터 페이지에는 제출 PDF에 적용할 수 있는 페이지 머리글, 페이지 바닥글 및 페이지 번호와 같은 정보가 있을 수 있습니다. 제출 서류 PDF의 자동 생성을 위한 기본 템플릿을 사용하여 제출 서류 PDF에 이러한 정보를 적용할 수 있습니다. 기본 템플릿을 사용하면 필드의 기본 속성을 변경할 수 있습니다.

기본 템플릿을 디자인할 때는 항상 [기본 템플릿 규칙](#base-template-conventions)을 따르십시오.

## 기본 템플릿 규칙 {#base-template-conventions}

기본 템플릿은 제출 서류 PDF의 머리글, 바닥글, 스타일 및 모양을 정의하는 데 사용됩니다. 머리글과 바닥글에는 회사 로고와 저작권 텍스트와 같은 정보가 포함될 수 있습니다. 기본 템플릿의 첫 번째 마스터 페이지가 복사되어 제출 PDF의 마스터 페이지로 사용됩니다. 이 페이지에는 머리글, 바닥글, 페이지 번호 또는 제출 PDF의 모든 페이지에 표시되어야 하는 기타 모든 정보가 포함되어 있습니다. 기본 템플릿 규칙을 준수하지 않는 기본 템플릿을 사용하는 경우 기본 템플릿의 첫 번째 마스터 페이지가 여전히 제출 PDF 템플릿에서 사용됩니다. 규칙에 따라 기본 템플릿을 디자인하고 제출 서류 PDF의 자동 생성에 사용하는 것이 좋습니다.
...
* 제출 서류 PDF의 필드에 스타일을 적용하려면 기본 템플릿에서 `AF_FIELDSSUBFORM` 루트 하위 양식 아래의 `AF_METATEMPLATE` 하위 양식에 있는 필드를 제공합니다.

* 이러한 필드의 속성은 제출 서류 PDF의 필드에 적용됩니다. 이러한 필드는 `AF_<name of field in all caps>_XFO` 명명 규칙을 따라야 합니다. 예를들어 확인란의 필드 이름은 `AF_CHECKBOX_XFO`이어야 합니다.
...
1. (선택 사항) 제출 서류 PDF의 필드에 적용할 필드의 스타일 및 모양을 수정합니다.
1. 양식을 저장합니다.

이제 저장된 양식을 제출 PDF의 기본 템플릿으로 사용할 수 있습니다. 기본 템플릿에 있는 스크립트를 수정하거나 제거하지 마십시오.
...
## 제출 PDF의 브랜딩 정보 사용자 지정 {#customize-the-branding-information-in-document-of-record}

제출 서류 PDF을 생성하는 동안 기록 문서 탭에서 제출 서류 PDF에 대한 브랜딩 정보를 변경할 수 있습니다. 기록 문서 탭에는 로고, 모양, 레이아웃, 머리글과 바닥글, 면책조항, 선택하지 않은 확인란과 라디오 단추 옵션 포함 여부 등의 옵션이 포함되어 있습니다.

기록 문서 탭에 입력하는 브랜딩 정보를 현지화하려면 브라우저의 로케일이 적절하게 설정되어 있는지 확인하십시오. 제출 서류 PDF의 브랜딩 정보를 사용자 정의하려면 다음 단계를 수행하십시오.

1. 제출 서류 PDF에서 패널(루트 패널)을 선택한 다음 ![구성](assets/configure.png)을 선택합니다.
1. ![dortab](assets/dortab.png)을(를) 선택합니다. 기록 문서 탭이 나타납니다.
1. 제출 서류 PDF 렌더링에 사용할 기본 템플릿 또는 사용자 지정 템플릿을 선택합니다. 기본 템플릿을 선택하면 템플릿 드롭다운 아래에 제출 PDF의 썸네일 미리 보기가 표시됩니다.
1. 기본 또는 사용자 지정 템플릿 선택 여부에 따라 다음 속성 중 일부 또는 모든 속성이 기록 문서 탭에 나타납니다. 제출 서류 PDF의 모양을 정의하려면 아래에 언급된 속성을 지정합니다.
   ...
   1. **기본 속성**:
      ...
      * **강조 색상**: 제출 PDF에서 머리글 텍스트 및 구분 기호를 렌더링하는 색상입니다.
      * **글꼴 모음**: 제출 PDF 텍스트의 글꼴 모음.

        >[!NOTE]
        >
        > AEM Forms은 PDF 파일과 원활하게 통합되는 다양한 기본 제공 글꼴을 제공합니다. 지원되는 글꼴 목록을 보려면 [여기를 클릭](/help/forms/supported-out-of-the-box-fonts.md)하세요.

      * **데이터 모델에 바인딩되지 않은 양식 개체 포함**: 속성을 설정하면 제출 PDF의 스키마 기반 적응형 양식에서 바인딩되지 않은 필드가 포함됩니다.
      * **기록 문서에서 숨겨진 필드 제외**: 속성을 설정하면 제출 PDF에서 제외할 숨겨진 필드가 식별됩니다.
   ...
   2. **양식 필드 속성**:
      ...
      **다중 선택 드롭다운에 대한 레이블 표시**
      ...
      이제 제출 PDF에 내부 저장 값 대신 다중 선택 드롭다운 구성 요소에 대해 선택한 표시 레이블이 표시됩니다. 예를 들어 사용자가 드롭다운에서 &quot;California&quot; 및 &quot;New York&quot;을 선택하면 제출 PDF에 `CA` 및 `NY`과(와) 같은 내부 값 대신 선택한 레이블이 표시됩니다.
   ...
   3. **기본 페이지 속성**:
      ...
      * **면책조항 텍스트**: 면책조항 텍스트.
      ...
      ![기본 페이지 속성](/help/forms/assets/masterpagepropertiesdor.png)
...
>[!NOTE]
> 
> 제출 서류 PDF에서 사용자 정의 양식 제목을 표시하려면 **기록 문서 속성** > **기본 페이지 속성**&#x200B;에서 **사용자 정의 양식 제목**&#x200B;을 편집하십시오. 이 사용자 정의 제목:
> 
> * 생성된 PDF의 헤더에 표시됩니다.
...
1. **[!UICONTROL 기록 문서 템플릿]** 탭을 열고 다음 옵션 중 하나를 선택합니다.
   ...
   * **[!UICONTROL 양식 서식 파일을 기록 문서 서식 파일로 연결]**:When 이 옵션을 선택하면 XFA 양식이 제출 PDF의 서식 파일로 사용됩니다.
...
## Submission PDF의 패널에 대한 표 및 열 레이아웃 {#table-and-column-layouts-for-panels-in-document-of-record}

적응형 양식은 여러 양식 필드가 있는 긴 양식일 수 있습니다. 제출 서류 PDF을 적응형 양식의 정확한 사본으로 저장하지 않을 수 있습니다. 이제 제출 서류 PDF에 하나 이상의 적응형 양식 패널을 저장하기 위한 테이블 또는 열 레이아웃을 선택할 수 있습니다.

제출 서류 PDF을 생성하기 전에 패널의 설정에서 해당 패널의 기록 문서에 대한 레이아웃을 테이블 또는 열로 선택합니다. 패널의 필드는 그에 따라 제출 PDF에서 구성됩니다.

제출 서류 PDF의 테이블 레이아웃에 렌더링된 패널의 ![필드](assets/dortablelayout.png)

제출 서류 PDF의 표 레이아웃에 렌더링된 패널의 필드

제출 서류 PDF의 열 레이아웃에서 렌더링되는 패널의 ![필드](assets/dorcolumnlayout.png)

제출 서류 PDF의 열 레이아웃으로 렌더링된 패널의 필드

## 제출 PDF 설정 {#document-of-record-settings}

제출 PDF 설정을 사용하여 제출 PDF에 포함할 옵션을 선택할 수 있습니다. 예를 들어 은행에서 이름, 연령, 주민등록번호, 전화번호를 양식으로 입력할 수 있습니다. 양식은 은행 계좌 번호와 지점 세부 사항을 생성합니다. 제출 PDF에서 이름, 주민등록번호, 은행 계좌 및 지점 상세내역만 표시하도록 선택할 수 있습니다.
...
* **기록 문서에서 제외**: 속성을 true로 설정하면 제출 PDF에서 필드가 제외됩니다. 이름이 `excludeFromDoR`인 스크립트 가능 속성입니다. 이 동작은 **숨겨진 경우 DoR에서 필드 제외** 양식 수준 속성에 따라 다릅니다.

* **패널을 테이블로 표시:** 패널에 6개 이하의 필드가 있는 경우 등록 정보를 설정하면 패널이 제출 PDF에서 테이블로 표시됩니다. 패널에만 적용됩니다.
* **기록 문서에서 제목 제외:** 속성을 설정하면 제출 PDF에서 패널/테이블의 제목이 제외됩니다. 패널 및 테이블에만 적용할 수 있습니다.
* **기록 문서에서 설명 제외:** 속성을 설정하면 제출 PDF에서 패널/테이블의 설명이 제외됩니다. 패널 및 테이블에만 적용할 수 있습니다.

**양식 수준 설정**

* **DoR에 바인딩되지 않은 필드 포함:** 속성을 설정하면 제출 PDF의 스키마 기반 적응형 양식에서 바인딩되지 않은 필드가 포함됩니다. 기본적으로 true입니다.
* **숨겨진 경우 DoR에서 필드 제외:** 양식 제출 시 제출 PDF에서 숨겨진 필드를 제외하도록 속성을 설정합니다. [서버에서 다시 유효성 검사](/help/forms/configuring-submit-actions.md#server-side-revalidation-in-adaptive-form-server-side-revalidation-in-adaptive-form)를 사용하도록 설정하면 서버에서 숨겨진 필드를 다시 계산한 다음 제출 PDF에서 해당 필드를 제외합니다.
...
1. Cloud Service 환경에 프로젝트를 배포합니다.
