...
1. [!UICONTROL 구성 값 편집] 대화 상자에서 AEM [!DNL Forms Server]의 인스턴스가 **[!UICONTROL 적응형 Forms 수]** 필드에 캐시할 수 있는 양식 또는 문서의 최대 수를 지정하십시오. 기본값은 100입니다.

   >[!NOTE]
   >
   >캐시를 비활성화하려면 적응형 Forms 수 필드의 값을 **0**(으)로 설정합니다. 캐시 구성을 비활성화하거나 변경하면 캐시가 재설정되고 모든 양식 및 문서가 캐시에서 제거됩니다.

   ![적응형 Forms HTML 캐시에 대한 구성 대화 상자](assets/cache-configuration-edit.png)
...
* 현지화된 적응형 Forms에 대한 고려 사항:
   ...
   * `http://host:port/content/forms/af/<adaptivefName>.html` 형식의 URL에 대해 브라우저 로캘 <!-- [Disable using browser locale](supporting-new-language-localization.md#how-localization-of-adaptive-form-works) -->을(를) 사용하지 않도록 설정하십시오.
...
1. [dispatcher.any 파일에 다음 내용을 추가하십시오](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html?lang=en#automatically-invalidating-cached-files):
...
1. [아래의 규칙 dispatcher.any 또는 사용자 지정 규칙 파일을 추가하십시오](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html?lang=en#specifying-the-documents-to-cache). 캐싱을 지원하지 않는 URL은 제외합니다. 예를 들어 대화형 통신.
...
1. [URL 매개 변수 무시 목록에 다음 매개 변수를 추가하십시오](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html?lang=en#ignoring-url-parameters):
...
AEM 환경이 적응형 Forms을 캐시하도록 구성되었습니다. 모든 유형의 적응형 Forms을 캐시합니다. 캐시된 페이지를 전달하기 전에 페이지에 대한 사용자 액세스 권한을 확인해야 하는 경우 [보안 콘텐츠 캐싱](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/permissions-cache.html?lang=ko-KR)을 참조하십시오.
...
에셋 브라우저를 통해 적응형 양식에 이미지 또는 비디오를 선택하고 Assets 편집기에서 편집한 경우, 그러한 에셋은 Dispatcher 캐시에서 자동으로 무효화되지 않습니다.
...
적응형 양식에 컨텐츠 조각 또는 경험 조각을 추가하고 이러한 자산을 독립적으로 편집 및 게시할 경우, 이러한 자산이 포함된 적응형 Forms은 Dispatcher 캐시에서 자동으로 무효화되지 않습니다.
...
적응형 양식 URL에 현지화 정보가 없고 구성 관리자에서 **[!UICONTROL 브라우저 로케일 사용]**&#x200B;을 사용하도록 설정된 경우. 적응형 양식의 지역화된 버전이 제공되며 적응형 양식의 첫 번째 인스턴스만 캐시되고 모든 후속 사용자에게 전달됩니다.
...
>[!MORELIKETHIS]
>
>* [AEM Formsas a Cloud Service 에 대한 캐싱 관련 문제 해결](/help/forms/troubleshooting-caching-performance.md)