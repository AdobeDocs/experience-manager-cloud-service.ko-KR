...
적응형 Forms[의 &#x200B;](/help/forms/rule-editor-core-components.md)규칙 편집기는 비동기 기능을 지원하므로, 양식과의 사용자 상호 작용을 방해하지 않고 외부 프로세스나 데이터 검색을 기다리는 작업을 통합 및 관리할 수 있습니다.
...
>[!NOTE]
>
> * `Promise`비동기 함수 호출&#x200B;**규칙 유형을 선택하면 양식의 규칙 편집기에 반환 형식이**&#x200B;인 함수만 표시됩니다.
...
1. **Then** 섹션의 **[!UICONTROL 작업 선택]** 드롭다운 목록에서 **비동기 함수 호출**&#x200B;을 선택합니다.
**[!UICONTROL 비동기 함수 호출]**&#x200B;을(를) 선택하면 `Promise` 반환 형식의 함수가 나타납니다.
1. 목록에서 비동기 함수를 선택합니다. 예를 들어 `matchOTP()` 및 `Add success callback`이(가) 나타나므로 `add failure callback` 함수와 해당 콜백을 선택하십시오.
...
이제 `Success` 함수에 대한 콜백 `Failure` 및 `matchOTP`의 구현을 계속할 수 있습니다.
...
OTP가 값 `userdetails`과(와) 일치하는 경우 `111` 패널을 표시하는 규칙을 만들어 보겠습니다.
...
1. **Then** 섹션의 **[!UICONTROL 작업 선택]** 드롭다운 목록에서 **표시**&#x200B;를 선택합니다. 예를 들어 `userdetails` 패널을 표시합니다.
1. **[!UICONTROL 문 추가]**&#x200B;를 클릭합니다.
1. **[!UICONTROL 작업 선택]** 드롭다운 목록에서 **숨기기**&#x200B;를 선택합니다. 예를들어 `error message` 텍스트 상자를 숨깁니다.
1. **[!UICONTROL 완료]**&#x200B;를 클릭합니다.

![성공 호출](/help/forms/assets/rule-editor-success-callback.png){width=50%, height=50%}

사용자가 OTP를 `111`(으)로 입력하고 `User Details` 단추를 클릭하면 `Confirm` 패널이 표시되는 아래 스크린샷을 참조하십시오.
...
1. **Then** 섹션의 **[!UICONTROL 작업 선택]** 드롭다운 목록에서 **표시**&#x200B;를 선택합니다. 예를들어 `error message` 텍스트 상자를 표시합니다.
1. **[!UICONTROL 문 추가]**&#x200B;를 클릭합니다.
1. **[!UICONTROL 작업 선택]** 드롭다운 목록에서 **숨기기**&#x200B;를 선택합니다. 예를 들어 `userdetails` 패널을 숨깁니다.
1. **[!UICONTROL 완료]**&#x200B;를 클릭합니다.
