...
* **[!UICONTROL 을 사용하여 템플릿 파일 선택]**: 템플릿 파일의 경로를 지정합니다. 페이로드를 기준으로 하거나 절대 경로에 저장하거나 문서 데이터 유형의 변수를 사용하여 템플릿 파일을 선택할 수 있습니다. 예, [Payload_Directory]/Workflow/data.xml 경로가 crx-repository에 없는 경우 관리자는 경로를 사용하기 전에 만들 수 있습니다. 또한 페이로드를 입력 데이터 파일로 허용할 수도 있습니다.

* **[!UICONTROL 을 사용하여 데이터 문서 선택]**: 입력 데이터 파일의 경로를 지정합니다. 페이로드에 상대적인 경로를 사용하거나 절대 경로에 저장하거나 문서 데이터 유형의 변수를 사용하여 입력 데이터 파일을 선택할 수 있습니다. 예, [Payload_Directory]/Workflow/data.xml 경로가 crx-repository에 없는 경우 관리자는 경로를 사용하기 전에 만들 수 있습니다.

* **[!UICONTROL 프린터 형식]**: XDC 파일이 제공되지 않을 때 사용할 페이지 설명 언어를 지정하여 출력 스트림을 생성하는 인쇄 형식 값입니다. 리터럴 값을 제공하는 경우 다음 값 중 하나를 선택합니다.

   * **[!UICONTROL 색상 PCL]**: 옵션을 사용하여 PCL에 대한 XDC 파일을 지정합니다.
   * **[!UICONTROL 일반 PostScript]**: PostScript용 일반 XDC 파일을 지정하려면 옵션을 사용합니다.
   ...
   * **[!UICONTROL TPCL 600DPI]**: TPCL 600DPI를 사용합니다. tpcl600.xdc 파일이 사용됩니다.
   * **[!UICONTROL PostScript 일반]**: PostScript용 일반 텍스트 XDC 파일을 지정하려면 옵션을 사용합니다.
   * **[!UICONTROL DPL300DPI]**: DPL 300DPI를 사용합니다. dpl300.xdc가 사용됩니다.
   * **[!UICONTROL DPL400DPI]**: DPL 400DPI를 사용합니다. dpl400.xdc가 사용됩니다.
...
* **[!UICONTROL 다음 방법으로 출력 문서 저장]**: 출력 파일을 저장할 위치를 지정합니다. 출력 파일을 페이로드를 기준으로 하는 위치에 저장하거나, 변수에 절대 위치를 지정하여 출력 파일을 저장할 수 있습니다. 경로가 crx-repository에 없는 경우 관리자는 경로를 사용하기 전에 만들 수 있습니다.

**[!UICONTROL 고급 속성]**

* **[!UICONTROL 을 사용하여 컨텐츠 루트 위치 선택]**: 컨텐츠 루트는 양식 디자인에서 사용되는 상대 자산을 검색할 수 있도록 저장소의 URI, 절대 참조 또는 위치를 지정하는 문자열 값입니다. 예를 들어 양식 디자인이 ../myImage.gif과 같이 이미지를 참조하는 경우 myImage.gif는 repository://에 있어야 합니다. 기본값은 repository:// 이며, 이 값은 저장소의 루트 수준을 가리킵니다.
...
* **[!UICONTROL 로케일]**: PDF 문서를 생성하는 데 사용되는 언어를 지정합니다. 리터럴 값을 제공하는 경우 목록에서 언어를 선택하거나 다음 값 중 하나를 선택합니다.
   * **[!UICONTROL 서버 기본값을 사용하려면]**: (기본값) [!DNL AEM Forms] 서버. 로케일 설정은 관리 콘솔을 사용하여 구성됩니다. (자세한 내용은 [디자이너 도움말](http://www.adobe.com/go/learn_aemforms_designer_65_kr))

   * **[!UICONTROL 사용자 지정 값을 사용하려면]**: 리터럴 상자에 로케일 코드를 입력하거나 로케일 코드를 포함하는 문자열 변수를 선택합니다. 지원되는 로케일 코드의 전체 목록은 http://java.sun.com/j2se/1.5.0/docs/guide/intl/locale.doc.html 을 참조하십시오.
...
* **[!UICONTROL 이중 인쇄]**: 양면 인쇄 또는 양면 인쇄 사용 여부를 지정하는 페이지 매김 값입니다. PostScript 및 PCL을 지원하는 프린터에서는 이 값을 사용합니다.리터럴 값을 제공하는 경우 다음 값 중 하나를 선택하십시오.
   ...
   * **[!UICONTROL 심플렉스]**: 양면 인쇄를 사용합니다.

## 비대화형 PDF 출력 생성 단계   {#generatePDFdocuments}
