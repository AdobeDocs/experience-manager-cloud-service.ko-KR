...
이 샘플은 대화형 통신 세부 사항을 입력하고 한 번의 클릭으로 연결 UI를 시작할 수 있는 간단한 양식 인터페이스를 제공합니다.
...
2. **미리 채우기 서비스 구성**(선택 사항): IC에 동적 데이터를 미리 채우려면 **미리 채우기 서비스** 필드에 양식 데이터 모델 서비스 이름을 입력하십시오. 예를 들어 샘플 데이터에는 `FdmTestData`을(를) 사용하고 테스트 데이터에는 `IC-FDM`을(를) 사용합니다.

3. **서비스 매개 변수 추가**(선택 사항): **서비스 매개 변수(JSON)** 필드에 미리 채우기 서비스에 필요한 매개 변수와 함께 JSON 개체를 입력합니다. 예:

   ```json
   {"customerId": "101", "accountNumber": "ACC-98765"}
   ```

4. **PDF 옵션 설정**(선택 사항): **옵션(JSON)** 필드에서 로케일, 첨부 파일 또는 접근성 설정과 같은 렌더링 옵션을 구성합니다.

5. **UI 연결 시작**: **UI 연결 시작** 단추를 클릭합니다. 대화형 통신과 함께 미리 로드된 연결 UI가 있는 새 브라우저 창이 열립니다.
