...
* 사이트 테마에 대해 **프론트엔드 파이프라인**&#x200B;이(가) 구성된 [AEM as a Cloud Service](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/developing/developing-with-front-end-pipelines.html)
...
>[!NOTE]
>
>**테마 이름:** Forms 테마를 사이트 테마에 포함하고 프론트엔드 파이프라인을 통해 배포하는 경우 **테마 이름을 변경할 필요가 없습니다**. 양식 스타일은 현재 이름으로 빌드되고 배포되는 기존 사이트 테마의 일부가 됩니다. 테마 이름 변경(예: `package.json`에서)은 전용 테마 리포지토리에서 **독립 실행형** Forms 테마를 배포하는 경우에만 필요합니다. 해당 시나리오는 [테마를 사용하여 핵심 구성 요소 기반 적응형 Forms 스타일링](/help/forms/using-themes-in-core-components.md)에 설명되어 있습니다.
...
1. **구성 요소 폴더 복사**\
   ...
   ![적응형 양식 구성 요소 추가](/help/forms/assets/theme-add-adaptiveform-component.png)
...
2. **이미지 복사**\
   ...
   ![이미지 추가](/help/forms/assets/theme-add-images.png)
...
![변수 및 mixin 추가](/help/forms/assets/theme-add-mixin-variable.png)

## 4단계: SCSS에서 이미지 경로 수정 {#step-4-fix-image-paths}

Forms 테마에서 구성 요소 SCSS 파일은 종종 `./resources/` 또는 `url(resources/`과(와) 같은 경로를 사용하는 이미지를 참조합니다. 사이트 테마에 복사한 후 해당 경로는 `theme/src/components/adaptiveform/resources/images/`을(를) 가리켜야 합니다.

**표준 사이트 템플릿 테마**&#x200B;이(가) Parcel을 사용하여 `url()`에서 `theme/src/`개 경로를 확인합니다. 따라서 이미지가 `theme/src/components/adaptiveform/resources/images/`에 있으면 **`components/adaptiveform/resources/images/`**&#x200B;을(를) 기준으로 하여 `theme/src/` 경로를 사용하십시오.
...
**이후**(사이트 테마, `adaptiveform/resources/images/`의 이미지):
...
![이미지 URL 변경](/help/forms/assets/theme-change-url.png)

이미지(단추, 아코디언, 마법사, 컨테이너, 스크리블 등)를 참조하는 `adaptiveform/` 아래의 모든 SCSS 파일에 대해 이 작업을 반복합니다. `theme/src/components/adaptiveform/`을(를) 초과하는 IDE에서 프로젝트 전체 찾기/바꾸기를 권장합니다.
...
![적응형 양식 scss](/help/forms/assets/theme-adaptive-form-scss.png)
...
![적응형 양식 scss 추가](/help/forms/assets/theme-add-adaptive-form-scss-theme.png)
...
1. 테마 루트에서 사이트 테마를 빌드합니다.
   ...
   ![빌드 실행](/help/forms/assets/theme-mpm-run-build.png)

2. 기존 [프론트엔드 파이프라인](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/developing/developing-with-front-end-pipelines.html)을 통해 배포합니다. 배포 후 동일한 테마 CSS가 사이트 페이지와 포함된 적응형 Forms 모두에 적용됩니다.

## 문제 해결 {#troubleshooting}

| 문제 | 확인할 사항 |
|-------|-------------------------------|
| 빌드 실패: 이미지에 대한 &quot;파일을 찾을 수 없음&quot; | 모든 양식 이미지가 `theme/src/components/adaptiveform/resources/images/`에 있는지 확인하십시오. `.scss`의 `adaptiveform/`마다 `url(components/adaptiveform/resources/images/...)`을(를) 사용하여 `theme/src/`에서 경로가 확인됩니다(Plural을 사용한 표준 사이트 테마 빌드에 필요). 번들로 파일당 경로를 확인하지 않는 한 `../resources/` 또는 `resources/`만 사용하지 마십시오. 그런 다음 이미지 폴더와 일치하는 경로를 사용하십시오. |
...
* [프론트엔드 파이프라인으로 개발](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/developing/developing-with-front-end-pipelines.html)
