---
title: 적응형 Forms에서 테마를 구축하고 사용하는 방법
description: 테마를 사용하여 스타일을 지정하고 핵심 구성 요소를 사용하여 적응형 양식에 시각적 ID를 제공할 수 있습니다. 여러 적응형 Forms에서 테마를 공유할 수 있습니다.
keywords: 양식 빌더 테마, 적응형 양식 스타일링 핵심 구성 요소, 양식 테마 빌더, 적응형 양식 스타일링, 테마 맞춤화, 양식 테마 빌드
...
# 테마를 사용하여 적응형 Forms 기반의 핵심 구성 요소 스타일 지정
...
[이러한 테마를 사용자 지정하여 새 테마를 만들 수 있습니다](#customize-a-theme).

>[!NOTE]
>
>적응형 Forms이 AEM Sites 페이지에 임베드된 경우 적응형 Forms 테마를 사이트 테마에 포함하면 사이트와 양식 모두에 단일 테마를 사용할 수 있습니다. [AEM Sites 테마에 적응형 Forms 테마 포함](/help/forms/embed-adaptive-forms-theme-in-site-theme.md)을 참조하십시오.
...
1. [테마 복제](#1-clone-a-theme)
1. [테마 이름 설정](#2-set-name-of-a-theme)
1. [테마 맞춤화](#3-customize-a-theme)
1. [테마 테스트](#4-test-a-customized-theme)
1. [테마 배포](#5-deploy-a-theme)
...
>[!NOTE]
>
>이 단계는 프론트엔드 파이프라인(예: 전용 Forms 테마 저장소)을 통해 테마를 **독립 실행형** 패키지로 배포하는 경우에 적용됩니다. 대신 Forms 테마를 AEM Sites 테마에 **포함**&#x200B;하는 경우 양식 테마에 대해 별도의 테마 이름이나 버전을 설정할 필요가 없습니다. 사이트 테마 이름이 사용됩니다. [AEM Sites 테마에 적응형 Forms 테마 포함](/help/forms/embed-adaptive-forms-theme-in-site-theme.md)을 참조하십시오.
...
* [테마 수준 스타일 설정](#set-theme-level-styles)

* [구성 요소 수준 스타일 설정](#set-component-level-styles)
...
* 4.1 [테스트를 위한 로컬 환경 구성](#41-configure-a-local-environment-for-testing)
* 4.2 [로컬 환경을 사용하여 테마 테스트](#42-test-the-theme-using-a-local-environment)

##### 4.1. 테스트를 위한 로컬 환경 구성
...
* 5.1 [테마에 대한 저장소 만들기](#51-create-a-repository-for-theme)
* 5.2 [변경 내용을 리포지토리에 푸시](#52-push-the-changes-to-the-repository)
* 5.3 [프론트엔드 파이프라인 추가](#53-add-the-frontend-pipeline)
* 5.4 [Node.js 버전을 20](#54-set-the-nodejs-version-to-20)(으)로 설정
* 5.5 [프론트엔드 파이프라인 실행](#55-run-the-frontend-pipeline)
...
1. **[!UICONTROL 저장]**&#x200B;을 클릭합니다.
   ...
   >[!NOTE]
   >
   > Cloud Manager에서 프론트엔드 파이프라인이 실패하지 않도록 [Node.js 버전을 20](#54-set-the-nodejs-version-to-20)(으)로 설정합니다.

##### 5.4 Node.js 버전을 20으로 설정

파이프라인 구성을 사용하여 Node.js 버전을 20으로 설정하려면 다음을 수행하십시오.

1. **파이프라인** 섹션으로 이동하여 프론트엔드 파이프라인을 찾습니다.
2. 파이프라인의 오른쪽에서 세 점 메뉴 **⋯**&#x200B;을(를) 클릭하고 드롭다운에서 **변수 보기/편집**&#x200B;을(를) 선택합니다.
3. **변수 구성** 대화 상자에서 다음과 같이 필드를 채웁니다.
   * **이름** - NODE_VERSION
   * **값** - 20
   * **단계 적용됨** - 빌드
   * **유형** - 변수
4. 구성을 적용하려면 **저장**&#x200B;을 클릭하세요.

![파이프라인 구성](/help/forms/assets/pipeline-config.png)
...
1. **[!UICONTROL 실행]** 을 클릭합니다.
   ...
   ![파이프라인 실행](/help/forms/assets/canvas-theme-run-pipeline.png)
...
* [AEM Sites 테마에 적응형 Forms 테마 포함](/help/forms/embed-adaptive-forms-theme-in-site-theme.md)
