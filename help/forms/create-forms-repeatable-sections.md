...
>[!NOTE]
>
>필드가 반복 가능 패널에 속해 있으면 스크립트에서 해당 이름을 사용하여 직접 액세스할 수 없습니다. 필드에 액세스하려면 `instances`의 `InstanceManager` API를 사용하여 필드가 속한 반복 가능한 인스턴스를 지정하십시오. `instances`에서 `InstanceManager` API를 사용하는 구문은 다음과 같습니다.
>
>
