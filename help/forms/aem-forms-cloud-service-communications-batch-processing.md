...
예를 통해 입력 데이터 파일 `record1.xml`과(와) 두 개의 출력 형식 `PDF` 및 `PCL`이(가) 있다고 가정해 보겠습니다. 그러면 대상 위치에 각 출력 형식에 대해 하나씩 두 개의 하위 폴더 `pdf` 및 `pcl`이(가) 포함됩니다. PDF 생성이 성공했다고 가정해 보겠습니다. 그러면 `pdf` 하위 폴더에 실제 생성된 PDF 문서 `success`을(를) 포함하는 `record1.pdf` 하위 폴더가 포함됩니다. PCL을 생성하지 못한 경우 `pcl` 하위 폴더에 `failure` 하위 폴더가 포함되고, 이 하위 폴더에는 오류 세부 정보가 포함된 오류 파일 `record1.error.txt`이(가) 포함됩니다. 또한 대상 위치에는 일괄 처리 실행 중에 필요한 특정 파일이 들어 있는 `__tmp__`(이)라는 임시 폴더가 있습니다. 대상 폴더를 참조하는 활성 배치 실행이 없는 경우 이 폴더를 삭제할 수 있습니다.
