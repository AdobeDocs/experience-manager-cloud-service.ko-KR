---
title: 간편한 벌크 PDF 만들기 - 일괄 처리를 통한 기본 아트 - 수백만 개의 PDF 문서 생성에 대한 자체 도움말 안내서!
...
# AEM Forms as a Cloud Service Communications 일괄 처리
...
* **구성(정의)**: 일괄 처리 구성은 생성된 문서에 대해 설정할 다양한 자산 및 속성에 대한 정보를 저장합니다. 예를 들어 출력 문서에 대한 다양한 속성을 지정하는 것과 함께 사용할 XDP 또는 PDF 템플릿과 고객 데이터의 위치에 대한 세부 정보를 제공합니다.

* **실행**: 일괄 처리 작업을 시작하려면 일괄 처리 구성 이름을 일괄 처리 실행 API에 전달합니다.

### 배치 작업의 구성 요소 {#components-of-a-batch-operations}

**클라우드 구성**: Experience Manager 클라우드 구성을 통해 Experience Manager 인스턴스를 고객이 소유한 Microsoft Azure 스토리지에 연결할 수 있습니다. 고객이 소유한 Microsoft Azure 계정에 연결할 자격 증명을 지정할 수 있습니다.
...
**일괄 처리 API**: 일괄 처리 구성을 만들고 이러한 구성을 기반으로 일괄 처리 실행을 실행하여 PDF 또는 XDP 템플릿과 데이터를 병합하고 PDF, PS, PCL, DPL, IPL 및 ZPL 형식의 출력을 생성할 수 있습니다. 통신은 구성 관리 및 배치 실행을 위한 배치 API를 제공합니다.
...
* **outputTypes**: 출력 형식 지정: PDF 및 PRINT. PRINT 출력 형식을 사용하는 경우 `printedOutputOptionsList` 속성에서 하나 이상의 인쇄 옵션을 지정하십시오. 인쇄 옵션은 해당 렌더링 유형으로 식별되므로, 현재 동일한 렌더링 유형의 여러 인쇄 옵션은 허용되지 않습니다. 지원되는 형식은 PS, PCL, DPL, IPL 및 ZPL입니다.
...
예를 통해 입력 데이터 파일 `record1.xml`과(와) 두 개의 출력 형식 `PDF` 및 `PCL`이(가) 있다고 가정해 보겠습니다. 그러면 대상 위치에 각 출력 형식에 대해 하나씩 두 개의 하위 폴더 `pdf` 및 `pcl`이(가) 포함됩니다. PDF 생성이 성공했다고 가정해 보겠습니다. 그러면 `pdf` 하위 폴더에 실제 생성된 PDF 문서 `record1.pdf`을(를) 포함하는 `success` 하위 폴더가 포함됩니다. PCL을 생성하지 못한 경우 `pcl` 하위 폴더에 `failure` 하위 폴더가 포함되고, 이 하위 폴더에는 오류 세부 정보가 포함된 오류 파일 `record1.error.txt`이(가) 포함됩니다. 또한 대상 위치에는 일괄 처리 실행 중에 필요한 특정 파일이 들어 있는 `__tmp__`(이)라는 임시 폴더가 있습니다. 대상 폴더를 참조하는 활성 배치 실행이 없는 경우 이 폴더를 삭제할 수 있습니다.
