...
* Cloud Manager 액세스
* AEM as a Cloud Service 인스턴스 액세스
* AEM as a Cloud Service 컨텐츠 작성

## 소개  {#introduction}

AEM as a Cloud Service 액세스는 컨텐츠 작성자 및 AEM 사용자를 위한 AEM 클라우드 여정의 중요한 첫 번째 단계입니다. 시기 적절한 액세스 권한을 가지고 있으면 생산성을 잃지 않으며 최종 사용자는 변화하는 디지털 경험에 훨씬 근접합니다.

시스템 관리자가 AEM 사용자 제품 프로필에 할당한 모든 팀원은 Cloud Manager에 대한 액세스 권한을 (읽기 전용)갖게 됩니다. 물론 이러한 사용자에게도 [Cloud Manager 제품 프로필](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/onboarding/onboarding-concepts/aem-cs-team-product-profiles.html?lang=en#cloud-manager-product-profiles)로 설정되면 AEM as a Cloud Service 및 Cloud Manager에서 이러한 권한의 합이 표시됩니다.

## 전제 조건  {#prerequisites}

시스템 관리자가 AEM as a Cloud Service 팀을 설정할 때 필요한 모든 단계를 완료하고 AEM 사용자 제품 프로필에 사용자를 할당해야 합니다.
...
1. 다음에서 Cloud Manager의 로그인 페이지로 직접 이동합니다. [my.cloudmanager.adobe.com](https://my.cloudmanager.adobe.com/).
   ...
   >[!NOTE]
   >책갈피를 지정하세요 [my.cloudmanager.adobe.com](https://my.cloudmanager.adobe.com/) 나중에 참조할 수 있도록 링크하고 Cloud Manager의 랜딩 페이지로 직접 이동하는 데 도움이 됩니다.
