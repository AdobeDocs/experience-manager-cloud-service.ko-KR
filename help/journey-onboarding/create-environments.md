...
여행 관련 미디어에 중점을 둔 임차인인 이론적 WKND Travel and Adventure Enterprises의 예를 생각해보면 두 가지 프로그램이 있을 수 있습니다. 즉, WKND매거진 사업부의 한 사이트 프로그램과 WKND미디어 사업부의 한 가지 자산 프로그램. 각 프로그램에는 사이트의 실제 트래픽을 처리하는 프로덕션 환경과 새로운 애플리케이션 코드를 테스트하기 위한 개발 환경과 같은 몇 가지 환경이 있을 수 있습니다.

다음과 같은 네 가지 유형의 환경이 있습니다.
...
* **신속한 개발** - RDE(Rapid Development Environment)를 통해 개발자가 변경 사항을 신속하게 배포하고 검토할 수 있으므로 로컬 개발 환경에서 작동하는 것으로 입증된 기능을 테스트하는 시간을 최소화할 수 있습니다.

이 온보딩 여정을 위해 최소한의 기능을 시작하기 위해 AEM as a Cloud Service의 기능을 탐색하는 데 사용할 수 있는 개발 환경을 만듭니다.

## 환경 만들기 {#creating-environments}

1. Cloud Manager에 로그온 위치 [my.cloudmanager.adobe.com](https://my.cloudmanager.adobe.com/) 적절한 조직을 선택합니다.
...
클라우드 리소스가 만들어지고 팀에서 액세스할 수 있습니다. 시스템 관리자는 먼저 팀 구성원을 Adobe Admin Console에서 as a Cloud Service으로 있는 AEM의 제품 프로필에 할당하여 해당 리소스에 액세스할 수 있어야 합니다.

따라서 다음 번에 문서를 검토하여 온보딩 여정을 계속해야 합니다 [AEM as a Cloud Service 제품 프로필에 팀 구성원 할당](assign-profiles-aem.md). 이 문서에서는 팀 구성원에게 새 환경에 액세스할 수 있는 권한을 부여하는 방법을 알아봅니다.
...
* [Cloud Manager 사용 - 환경](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/cloud-manager/environments.html) - Cloud Manager 환경은 AEM 작성, 게시 및 Dispatcher 서비스로 구성됩니다. 서로 다른 환경에서 역할을 지원하고 다양한 CI/CD 파이프라인을 사용하여 관여하는 방법을 알아봅니다.
