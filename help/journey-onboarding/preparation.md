...
>[!VIDEO](https://video.tv.adobe.com/v/3431508/?captions=kor&quality=12&learn=on)
...
* 액세스할 수 있는 환경의 수와 유형
* 서비스 시작 일자
* Premier Support 수준(해당되는 경우)입니다.
* 온보딩 담당자

이러한 세부 사항을 모르는 경우, 조직 내 약정 팀에 문의하십시오.

약정에 서명하면 Adobe에서 환경을 프로비저닝하고 약정에 정의된 온보딩 담당자에게 시작 이메일을 보내며 Adobe에서 온보딩 통화를 구성합니다. 이 워크플로우는 일반적으로 향후 서비스 시작 날짜가 지정되지 않는 한 계약 종료 며칠 후에 발생합니다.

## 시스템 프로비저닝 {#provisioning}

계약에 서명하면 계약에 포함된 모든 AEM 클라우드 환경이 Adobe에서 자동으로 프로비저닝됩니다. 이러한 환경은 마찬가지로 Cloud Manager의 프로그램에 연결됩니다(여정 뒷부분에서 자세히 설명). 각 구독에는 하나 이상의 프로덕션, 하나의 스테이징 및 선택적 개발 또는 테스트와 같은 AEM 클라우드 환경이 포함됩니다.

## 온보드 호출 {#onboarding-call}
...
>[!TIP]
>
>환영 이메일을 받았지만 시스템 관리자가 아닌 경우 시작 통화 중에 Adobe에 알리십시오. 또는 이 여정을 계속 진행하고 다음 단계에서 다른 동료를 직접 시스템 관리자로 지정할 수도 있습니다.
...
이제 이러한 준비 단계에 익숙해졌으므로 시스템에 직접 로그인할 준비가 거의 된 것입니다. 시스템 관리자의 관점에서 AEM as a Cloud Service 둘러보기를 시작합니다. 그런 다음 필수 용어를 알아봅니다. 여정의 다음 부분인 [AEM as a Cloud Service 용어](terminology.md)를 진행하십시오.
