...
# 컨텐츠 전송 확인(이전) {#validating-content-transfers}

## 시작하기 {#getting-started}

사용자는 콘텐츠 전송 도구에서 추출한 모든 콘텐츠가 대상 인스턴스에 성공적으로 수집되었는지 확인할 수 있습니다. 이 유효성 검사 기능은 추출 중에 포함된 노드의 요약과 수집 중에 포함된 노드의 요약을 비교하여 작동합니다. 추출 다이제스트에 포함된 노드 경로 중 수집 다이제스트에서 누락된 노드가 있는 경우 유효성 검사가 실패한 것으로 간주되며 추가 수동 유효성 검사가 필요할 수 있습니다.
...
유효성 검사 보고서는 수집 로그에 포함될 수 있을 뿐만 아니라 컨텐츠 전송 도구 사용자 인터페이스에서 액세스할 수도 있습니다. 이렇게 하려면 마이그레이션 세트를 선택하고 **유효성 검사** 작업 표시줄의 단추:
...
사용 **유효성 검사 게시/작성자 보고서** 대상 환경의 지정된 계층에 대한 가장 최근 수집에 대한 유효성 검사 보고서를 보기 위한 버튼입니다. 작은 게시 수집에서 아래를 참조하십시오.

![이미지](/help/journey-migration/content-transfer-tool/assets/CTTvalidationreport.png)

>[!NOTE]
>
>다음 **유효성 검사 게시/작성자 보고서** 수집이 완료되면 링크가 표시됩니다. 또한 유효성 검사 보고서는 유지되므로 섭취 로그와 마찬가지로 섭취가 완료된 후에 만료되지 않습니다.
