...
>additional-url="https://experienceleague.adobe.com/ko/docs/experience-manager-cloud-service/content/migration-journey/cloud-migration/content-transfer-tool/extracting-content#top-up-extraction-process" text="추가 추출"
...
1. 수집을 만드는 데 필요한 정보를 제공합니다.
   ...
   * **계층:** 계층을 선택합니다. (작성자/게시).
   ...
   >[!NOTE]
   > 대상 계층이 `Publish`인 경우 수집 중에 게시 인스턴스가 계속 실행되고 있습니다.  그러나 수집 중에 압축 프로세스가 실행 중인 경우 두 프로세스 간에 충돌이 발생할 수 있습니다.  이러한 이유로 수집 프로세스는 1) 수집 중에 압축이 시작되지 않도록 압축 시간 스크립트를 비활성화하고, 2) 압축이 현재 실행 중인지 확인하고, 실행 중이라면 수집이 진행되기 전에 완료될 때까지 기다립니다.  게시 수집이 예상보다 오래 걸리는 경우 수집 로그에서 관련 로그 문을 확인하십시오.
   ...
   * **지우기:** `Wipe` 값을 선택합니다.
      ...
      * 지우지 않는 수집은 추가 수집 사용 사례용으로 특별히 설계되었습니다. 이러한 수집은 기존 마이그레이션 세트의 마지막 수집 이후 변경된 새로운 컨텐츠 양을 늘리기 위한 것입니다. 이 사용 사례 외부에서 지우지 않는 수집을 수행하면 수집 시간이 매우 길어질 수 있습니다.
...
>additional-url="https://experienceleague.adobe.com/ko/docs/experience-manager-cloud-service/content/migration-journey/cloud-migration/content-transfer-tool/viewing-logs" text="로그 보기"
...
>additional-url="https://experienceleague.adobe.com/ko/docs/experience-manager-cloud-service/content/migration-journey/cloud-migration/content-transfer-tool/validating-content-transfers" text="콘텐츠 전송 확인"
...
* Cloud Manager을 통해 [IP 허용 목록이 적용](/help/implementing/cloud-manager/ip-allow-lists/apply-allow-list.md)된 경우 Cloud Acceleration Manager이 마이그레이션 서비스에 도달하는 것이 차단됩니다. 주소가 동적이므로 수집에 IP 주소를 추가할 수 없습니다. 현재 수집 및 색인 지정 프로세스가 실행되는 동안 임시로 IP 허용 목록허용 목록에 추가하다 에 0.0.0.0/0을(를) 추가하여 수집 및 색인 지정 프로세스 동안 IP 수집을 비활성화하는 것이 유일한 방법입니다.
...
작성자 환경을 사용할 수 있는지 확인하고 몇 분 정도 기다린 후 수집을 다시 시도합니다. 문제가 지속되면 발생한 오류 상태와 함께 고객 지원 센터에 문의하십시오.
...
>additional-url="https://experienceleague.adobe.com/ko/docs/experience-manager-cloud-service/content/migration-journey/cloud-migration/content-transfer-tool/ingesting-content#top-up-ingestion-process" text="추가 수집"
...
>java.lang.RuntimeException: org.apache.jackrabbit.oak.api.CommitFailedException: OakConstraint0030: 고유성 제약 조건이 값 a1a1a1a1-b2b2-c3c3-d4d4-e5e5e5e5e5e5: /some/path/jcr[, /some/other/path/jcr:uuid]을 가진 :contentjcr:content 속성을 위반했습니다.
...
>additional-url="https://experienceleague.adobe.com/ko/docs/experience-manager-cloud-service/content/migration-journey/cloud-migration/content-transfer-tool/prerequisites-content-transfer-tool" text="마이그레이션 사전 요구 사항"
...
* `Mongo took too long to start` - 매우 드문 경우이지만 수집 워크플로우에 사용되는 로컬 MongoDB를 시작하지 못할 수 있습니다. 나중에 다시 시도하면 문제가 해결될 가능성이 높습니다.

#### AZCopy 문제 {#azcopy-issues}

* `AZCopy critical failure` - 드물게 수집의 사전 복사 단계를 수행하는 데 사용되는 AZCopy 도구가 예기치 않게 실패할 수 있습니다. 이 경우 수집을 다시 시도해야 합니다.
...
### 대기 수집을 시작하지 못했습니다. {#waiting-ingestion-not-started}

>[!CONTEXTUALHELP]
>id="aemcloud_cam_ingestion_troubleshooting_waiting_ingestion_not_started"
>title="대기 수집이 시작되지 않았습니다"
>abstract="추출이 완료될 때까지 대기한 이후 수집을 시작하지 못했습니다."

실행 중인 추출을 소스 마이그레이션 세트로 사용하여 생성된 수집은 해당 추출이 성공할 때까지 대기하며, 이 시점에서 수집은 정상적으로 시작하려고 시도합니다. 수집이 시작되지 않으면 실패한 것으로 표시됩니다. 시작하지 않는 이유는 IP 허용 목록이 대상 작성자 환경에 구성되어 있고 다른 이유로 대상 환경을 사용할 수 없기 때문입니다.  이 경우 수집을 시작하지 못한 이유를 확인하고 문제를 해결한 다음 수집을 다시 시작합니다(추출을 다시 실행할 필요가 없음).
