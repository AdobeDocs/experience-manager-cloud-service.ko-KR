...
1. **추출**: 추출이란 소스 AEM 인스턴스에서 *마이그레이션 세트*&#x200B;라고 하는 임시 영역으로 콘텐츠를 추출하는 것입니다. *마이그레이션 세트*&#x200B;는 소스 AEM 인스턴스와 클라우드 서비스 AEM 인스턴스 간에 전송된 콘텐츠를 임시 저장할 수 있도록 Adobe가 제공하는 클라우드 저장소 영역입니다.
   ...
   >[!NOTE]
   >이제 사용자 매핑이 작성자의 추출 단계의 일부로 자동으로 실행됩니다(하지만 필요에 따라 작성자에서 비활성화하거나 게시에서 활성화할 수 있음). 자세한 내용은 [사용자 매핑 및 보안 주체 마이그레이션](/help/journey-migration/content-transfer-tool/using-content-transfer-tool/user-mapping-and-migration.md) 자세한 내용
...
모든 마이그레이션 세트는 약 90일 동안 장기간 동안 활동이 없으면 결국 만료됩니다. 일정 시간 동안 프로젝트 카드 및 마이그레이션 작업 테이블 행에 표시기가 표시되면 마이그레이션 세트가 만료되고 해당 데이터를 더 이상 사용할 수 없게 됩니다. 다음 방법으로 마이그레이션 세트에 따라 간편하게 만료 시간을 연장할 수 있습니다.
...
마이그레이션 세트의 만료는 마이그레이션 세트 행에서 모니터링할 수 있습니다. 마이그레이션 세트가 만료 날짜에 도달하고 있다는 유용한 시각적 표시기도 프로젝트 카드를 추가했습니다.
