...
* 디자인 실현
* 필요한 기능을 구현할 수 있도록 필요한 서비스 및 스크립트 개발

의 구성 [개발](/help/implementing/developing/introduction/development-guidelines.md) 환경은 다양한 요인에 따라 달라질 수 있지만 일반적으로 다음과 같이 구성됩니다.

* 통합된 코드 베이스를 제공하기 위해 버전 제어 기능을 갖춘 통합된 개발 시스템. 이 통합 코드 베이스는 각 개발자가 사용하는 개별 개발 환경에서 코드를 병합하고 통합하는 데 사용됩니다.
...
프로덕션 환경은 다음과 같은 환경으로 구성됩니다 [작성자 및 게시](/help/sites-cloud/authoring/getting-started/concepts.md) 구현.
...
프로젝트 규모에 따라 여러 명의 저자, 혹은 출판사, 혹은 두 사람 모두로 구성되는 경우가 많다. 하위 수준에서 저장소가 여러 인스턴스로 클러스터링될 수도 있습니다.

#### 작성 {#author}

일반적으로 작성자 인스턴스는 내부 방화벽 뒤에 있습니다. 이 내부 방화벽은 사용자 및 동료가 다음과 같은 작성 작업을 수행하는 환경입니다.
...
일반적으로 게시 환경은 &quot;DMZ&quot;(Demilitarized Zone)에 있습니다. 이 환경은 방문자가 귀하의 콘텐츠(예: 웹 사이트 또는 모바일 애플리케이션 형식)에 액세스하여 상호 작용하는 환경으로, 인트라넷 내에서 공개적입니다. 게시 환경:
...
AEM as a Cloud Service의 프로젝트는 종종 코드 배포를 트리거합니다.
...
콘텐츠는 낮은 환경에서 높은 환경으로 이동하는 코드를 따라서는 안 됩니다. 즉, 작성자가 로컬 컴퓨터나 낮은 환경에서 컨텐츠를 만든 다음 프로덕션 환경으로 이동하는 것은 좋지 않습니다. 그 이유는 오류와 불일치를 초래할 수 있기 때문이다.

프로덕션 콘텐츠는 스테이징 환경에서 효율적이고 정확한 테스트 환경을 제공하도록 프로덕션 환경에서 스테이징 환경으로 이동해야 합니다.

>[!NOTE]
>
>이 방법론은 스테이징 컨텐츠를 프로덕션과 계속 동기화해야 한다는 것을 의미하지 않습니다. 정기적인 업데이트로도 충분하지만, 특히 새로운 코드 반복을 테스트하기 전에 업데이트해야 합니다. QA 및 개발 환경에 있는 콘텐츠는 자주 업데이트하지 않아도 됩니다. 이는 프로덕션 콘텐츠를 잘 표현한 것이어야 합니다.

콘텐츠를 다음과 같이 전송할 수 있습니다.

* 다른 환경 간에 패키지를 내보내고 가져옵니다.
* AEM 다른 인스턴스 간에 HTTP 또는 HTTPS 연결을 사용하여 컨텐츠를 직접 복제(as a Cloud Service 복제)합니다.
