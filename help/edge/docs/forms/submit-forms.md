...
1. 양식 필드 이름(`shared-default` 시트의 `Name` 열 값)을 `incoming` 시트의 헤더에 미러링합니다.

   제출 버튼을 제외한 `shared-default` 시트의 각 `Name` 열 값은 `incoming` 시트에서 헤더 역할을 합니다. 그 예로 다음 이미지에서는 “문의” 양식의 헤더를 설명합니다.

   ![문의 양식의 필드](/help/edge/assets/contact-us-form-excel-sheet-fields.png)

1. Sidekick을 사용하여 시트를 미리 봅니다.
...
또한 스프레드시트에서 다음과 같은 변경 내용을 확인할 수 있습니다.
