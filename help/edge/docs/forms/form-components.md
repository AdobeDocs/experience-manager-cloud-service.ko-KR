...
양식 블록을 사용하면 캡처한 데이터를 캡처하고 저장할 양식을 쉽게 만들 수 있습니다. 모든 HTML을 기반으로 구성 요소를 지원합니다.5 [입력 유형](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input#input_types) 텍스트, 이메일, 번호, 날짜 등. 또한 다음을 지원합니다 [텍스트 영역](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/textarea), [선택](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/select), 및 [필드 세트](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/fieldset) 요소를 포함하고, HTML5 고유의 입력 유효성 검사 기능을 포함합니다.

양식 블록은 일관성을 보장하기 위해 모든 필드 유형 및 컨테이너(패널)에 대해 균일한 HTML 구조를 생성합니다. 이러한 일관된 구조를 통해 다음과 같은 작업을 쉽게 수행할 수 있습니다 [양식 스타일 지정](/help/edge/docs/forms/style-theme-forms.md).


## 구성 요소 속성

양식 블록의 양식 구성 요소에 대해 지원되는 속성은 다음과 같습니다.

<table>
 <tbody>
  <tr>
   <td><b>속성</b> </td>
   <td><b>양식 구성 요소(입력 유형)</b> </td>
   <td><b>세부 사항</b></td>
  </tr>
  <tr>
   <td>이름</td>
   <td>모든 구성 요소</td>
   <td> 구성 요소의 이름을 지정하는 문자열 구성 요소의 데이터가 이 이름에 대해 제출됩니다.
   </td>
  </tr>
  <tr>
   <td>레이블</td>
   <td>모든 구성 요소</td>
   <td> 레이블은 필드의 캡션 역할을 합니다.
   </td>
  </tr>
  <tr>
   <td>값</td>
   <td>모든 구성 요소</td>
   <td> 컨트롤의 초기값입니다. 라디오 및 확인란 구성 요소의 경우 선택할 때 제출되는 값/데이터입니다. 값 속성은 항상 선택 사항이지만 확인란 및 라디오에 대해서는 필수로 간주해야 합니다.
   </td>
  </tr>
    <tr>
   <td>플레이스홀더</td>
