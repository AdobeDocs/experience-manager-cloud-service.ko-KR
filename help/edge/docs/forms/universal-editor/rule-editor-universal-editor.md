...
# Edge Delivery Services 양식용 규칙 편집기

규칙 편집기를 사용하면 작성자가 코드를 작성하지 않고도 정적 양식을 반응형의 지능적인 경험으로 바꿀 수 있습니다. 조건부로 필드를 표시하고, 계산을 수행하고, 데이터를 유효성 검사하고, 플로우를 통해 사용자를 안내하고, 사용자가 입력할 때 조정되는 비즈니스 논리를 통합할 수 있습니다.
...
- **액션**: 조건이 참일 때 (선택적으로 거짓일 때) 발생하는 상황입니다.
...
구성 요소에 이미 규칙이 있는 경우 다음과 같은 작업을 수행할 수 있습니다.
...
- **When**: 복잡한 조건부 비헤이비어에 대한 기본 규칙 (조건 → 액션 ± Else)
- **숨기기/표시**: 조건에 따라 가시성 제어 (점진적 공개)
- **활성화/비활성화**: 필드가 대화형인지 여부 제어 (예: 필수 필드가 유효해질 때까지 제출 비활성화)
...
2. **다음 순서대로 양식 구성 요소를 추가합니다.**
   ...
   - 숫자 입력: “추가 공제” (필수: 아니요, 플레이스홀더: “추가 공제 입력”)
   - 숫자 입력: “과세 소득” (읽기 전용: 예)
...
### 사용자 정의 함수에 대한 정적 가져오기

범용 편집기의 규칙 편집기가 정적 가져오기를 지원하므로 여러 파일과 양식에서 재사용 가능한 논리를 구성할 수 있습니다. 모든 사용자 정의 함수를 단일 파일(/blocks/form/functions.js)에 보관하는 대신 다른 모듈에서 함수를 가져올 수 있습니다.
예: 외부 파일에서 함수 가져오기
다음 폴더 구조 고려:
...
아래와 같이 `commonLib/functions.js`에서 주요 `functions.js` 파일로 함수를 가져올 수 있습니다.
...
### 다양한 양식에서 사용자 정의 함수 구성

다양한 함수 세트를 별도의 파일이나 폴더에 생성하고 필요에 따라 내보낼 수 있습니다.

- 특정 함수를 특정 양식에서만 사용할 수 있도록 하고자 하는 경우 양식 구성에 함수 파일의 경로를 입력할 수 있습니다.

- 경로에 대한 텍스트 상자가 비어 있으면 규칙 편집기는 기본적으로 `/blocks/form/functions.js`에서 함수 로드

![UE의 사용자 정의 함수](/help/forms/assets/custom-function-in-ue.png){width=50%}

위 스크린샷의 사용자 정의 함수 경로가 사용자 정의 함수 경로 텍스트 상자에 추가되었습니다. 이 양식의 사용자 정의 기능은 지정된 파일(`cc_function.js`)에서 로드됩니다.

이를 통해 여러 양식에서 함수를 공유하거나 양식별로 분리하여 유연성을 확보할 수 있습니다.
