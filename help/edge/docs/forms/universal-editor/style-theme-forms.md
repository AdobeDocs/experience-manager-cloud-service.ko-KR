...
AEM Forms용 Edge Delivery Services에서 양식 스타일을 지정하려면 CSS 사용자 정의 속성, 블록 기반 아키텍처 및 구성 요소별 타기팅 전략에 대한 정교한 이해가 필요합니다. 기존의 양식 스타일 지정 접근 방식과 달리, 적응형 양식 블록은 Edge Delivery Services의 성능 및 접근성 이점을 유지하면서 일관된 테마를 가능하게 하는 체계적인 디자인 토큰 시스템을 구현합니다.

적응형 양식 블록 아키텍처는 모든 양식 구성 요소에 표준화된 HTML 구조를 생성하여 CSS 타기팅 및 사용자 정의를 위한 예측 가능한 패턴을 만듭니다. 이러한 일관성은 개발자가 복잡한 양식 구현 전반에 걸쳐 확장되는 포괄적인 스타일 지정 시스템을 구현하는 동시에 Edge Delivery Services를 매우 빠르게 만드는 블록 기반 성능 최적화를 보존할 수 있도록 합니다.

이 포괄적인 안내서는 CSS 사용자 정의 속성 시스템, 구성 요소 HTML 구조 패턴 및 고급 스타일 지정 기술을 포함하여 Edge Delivery Services 에코시스템 내 양식 스타일 지정의 기술적 기반을 다룹니다. 이 설명서는 정교하고 브랜드화된 양식 경험을 만들기 위한 이론적 이해와 실제 구현 지침을 모두 제공합니다.

## 학습 내용

**CSS 사용자 정의 속성 마스터**: 색상 구성, 타이포그래피 크기, 간격 시스템 및 레이아웃 매개변수를 포함하여 양식 모양을 제어하는 전체 변수 시스템을 이해합니다. 이러한 속성을 재정의하고 확장하여 포괄적인 브랜드 테마를 구현하는 방법에 대해 알아봅니다.

**구성 요소 아키텍처 이해**: 각 양식 구성 요소 유형에서 사용되는 HTML 구조 패턴에 대한 심층적인 지식을 얻어, 기본 기능 또는 접근성 기능을 손상시키지 않고 정확한 CSS 타기팅 및 사용자 정의를 가능하게 합니다.

**고급 스타일 지정 기술**: 상태 기반 스타일 지정, 반응형 디자인 통합 및 Edge Delivery Services의 빠른 로딩 특성을 유지하는 성능 최적화 사용자 정의 전략을 포함한 정교한 스타일 지정 패턴을 구현합니다.

**전문가 수준의 구현 전략**: 디자인 시스템 통합, 유지 관리 가능한 CSS 아키텍처 및 복잡한 스타일 지정 시나리오에 대한 문제 해결 기술을 포함한 양식 스타일 지정에 대한 업계 표준 접근 방식에 대해 알아봅니다.
...
## CSS 사용자 정의 속성을 사용한 포괄적인 양식 스타일 지정

적응형 양식 블록은 모든 양식 구성 요소에 걸쳐 체계적인 테마 지정과 일관된 스타일 지정을 가능하게 하는 사용자 정의 속성(CSS 변수)을 기반으로 구축된 정교한 CSS 아키텍처를 활용합니다. 이 구조를 이해하는 것은 효과적인 양식 사용자 정의 및 브랜딩에 필수적입니다.

### forms.css 아키텍처 이해

기본 양식 스타일은 프로젝트 저장소의 `/blocks/form/form.css`에 있으며, 유지 관리성, 일관성 및 사용자 정의 유연성을 우선시하는 구조화된 접근 방식을 따릅니다. 아키텍처는 여러 핵심 구성 요소로 구성됩니다.

**CSS 사용자 정의 속성 기반**: 스타일 지정 시스템은 `:root` 수준에서 정의된 CSS 사용자 정의 속성을 기반으로 구축되어, 모든 양식 구성 요소 전체에 종속되는 중앙 집중식 테마 시스템을 제공합니다. 이러한 변수는 색상, 타이포그래피, 간격 및 레이아웃 속성에 대한 디자인 토큰을 설정합니다.

**블록 기반 CSS 구조**: Edge Delivery Services는 `.form` 클래스가 모든 양식 관련 스타일의 기본 네임스페이스 역할을 하는 블록 기반 아키텍처를 사용하며, 적절한 범위 격리를 보장하고 다른 페이지 구성 요소와의 CSS 충돌을 방지합니다.

**구성 요소별 스타일 지정**: 개별 양식 구성 요소는 일관된 래퍼 패턴(`.{Type}-wrapper`)을 사용하여 스타일이 지정되며, 이는 전체 디자인 시스템의 무결성을 유지하면서 다양한 필드 유형에 대한 예측 가능한 타기팅을 제공합니다.

### CSS 사용자 정의 속성 참조 및 사용자 정의

양식 스타일 지정 시스템에는 양식 모양 및 동작의 모든 측면을 제어하는 50개 이상의 CSS 사용자 정의 속성이 포함됩니다. 이러한 속성을 이해하면 디자인 일관성을 유지하면서 포괄적인 사용자 정의가 가능합니다.

+++ 색상 및 테마 변수

색상 시스템은 신중하게 구성된 사용자 정의 속성을 통해 양식을 위한 완전한 시각적 기반을 설정합니다.
...
**실용적인 사용자 정의 예**: 양식에 어두운 테마를 구현하려면 기본 색상 변수를 재정의합니다.
...
이 단일 변경 사항은 시스템이 하드코딩된 값 대신 변수 참조를 사용하므로 모든 양식 구성 요소를 통해 전파됩니다.

+++

+++ 타이포그래피 및 간격 변수

타이포그래피 및 간격 변수는 텍스트 표시 및 레이아웃 간격에 대한 포괄적인 제어를 제공합니다.
...
**실용적인 사용자 정의 예**: 더 작은 타이포그래피로 더 콤팩트한 양식 레이아웃을 만들려면 다음 작업을 수행하십시오.
...
레이아웃 변수는 양식 차원, 그리드 비헤이비어 및 구성 요소 배열을 제어합니다.
...
**실용적인 사용자 정의 예**: 향상된 시각적 깊이로 카드 스타일 양식을 만들려면 다음 작업을 수행하십시오.
...
### CSS 스타일 지정 패턴 및 모범 사례

적응형 양식 블록은 모든 구성 요소에서 유지 관리 가능하고 성능이 뛰어나며 일관된 스타일 지정을 보장하는 특정 CSS 패턴을 따릅니다.

+++ 기본 스타일 지정 패턴

**블록 수준 양식 컨테이너**: 전체 레이아웃 및 배경 스타일 지정을 위해 기본 양식 컨테이너를 타기팅합니다.
...
**구성 요소 래퍼 패턴**: 일관된 래퍼 클래스를 사용하여 특정 필드 유형을 타기팅합니다.
...
**필드별 타기팅**: 고유한 스타일 지정 요구 사항을 위해 개별 필드를 이름별로 타기팅합니다.
...
**상태 기반 스타일 지정**: 유효성 검사 및 상호 작용 상태를 구현합니다.
...
전체 양식 스타일 지정 참조는 AEM Forms 상용구 저장소에서 사용할 수 있습니다.

```
https://github.com/adobe-rnd/aem-boilerplate-forms/blob/main/blocks/form/form.css
```

이 파일은 CSS 사용자 정의 속성 시스템의 표준 구현 역할을 하며 모든 양식 스타일 지정의 기반을 제공합니다. 여기에는 모든 CSS 변수, 구성 요소 스타일 지정 패턴 및 반응형 디자인 구현에 대한 포괄적인 정의가 포함됩니다.

+++

+++ 프로젝트 통합

Edge Delivery Services 프로젝트에서 이 구조화된 접근 방식을 통해 양식 스타일 지정을 구현합니다.
...
**CSS 사용자 정의 속성 재정의**: 글로벌 스타일에서 양식 변수를 재정의하여 브랜드별 테마를 구현합니다.
...
**구성 요소별 사용자 정의**:
CSS 변수 시스템을 유지하면서 구성 요소별 스타일 지정을 추가합니다.
...
**반응형 디자인 통합**: 일관된 반응형 비헤이비어를 위해 미디어 쿼리 내에서 CSS 사용자 정의 속성을 활용합니다.
...
이 섹션에서는 CSS 사용자 정의 속성을 사용하여 최신 브랜드 양식을 만드는 방법을 보여줍니다. 이해와 탐색을 쉽게 할 수 있도록 구현은 명확한 하위 섹션으로 나뉩니다.



+++ &#x200B;1. 브랜드 테마 변수

CSS 사용자 정의 속성을 사용하여 브랜드의 색상 팔레트, 간격 및 타이포그래피를 정의합니다.
...
시각적으로 매력적인 레이아웃을 위해 양식 컨테이너에 최신 배경, 테두리 반경 및 그림자를 적용합니다.
...
깔끔하고 현대적인 모양을 위해 텍스트, 이메일 및 숫자 입력 필드 스타일을 지정합니다.
...
필요에 따라 특정 필드, 상태 또는 구성 요소를 타기팅하여 양식 스타일 지정을 확장할 수 있습니다. 고급 패턴에 대해서는 이전 섹션을 참조합니다.
...
이 포괄적인 접근 방식은 CSS 사용자 정의 속성이 적응형 양식 블록 시스템의 구조적 무결성과 접근성 기능을 유지하면서 정교한 테마 지정을 가능하게 하는 방법을 보여줍니다.
