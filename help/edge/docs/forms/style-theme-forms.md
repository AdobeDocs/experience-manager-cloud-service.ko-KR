...
적응형 양식 블록은 모든 양식 필드에 대해 일관된 구조를 생성합니다. 이러한 일관된 구조를 사용하면 필드 유형 및 필드 이름을 기반으로 양식 필드를 선택하고 스타일을 지정하는 CSS 선택기를 더 쉽게 개발할 수 있습니다.

이 문서는 다양한 양식 구성 요소에 대한 HTML 구조를 간략하게 설명하며, 적응형 양식 블록의 양식 필드 스타일을 지정하기 위해 다양한 양식 필드의 CSS 선택기를 만드는 방법을 이해하는 데 도움이 됩니다.

문서가 작성되면 다음과 같은 작업을 수행할 수 있습니다.
...
- 확인란 그룹: 여러 옵션을 선택하는 데 사용됨
- 라디오 그룹: 그룹에서 하나의 옵션만 선택하는 데 사용됨
- 드롭다운: 선택 상자라고도 하며 목록에서 하나의 옵션을 선택하는 데 사용됨
- 패널/컨테이너: 관련 양식 요소를 그룹화하는 데 사용됨
...
- **기본 스타일 업데이트**: `/blocks/form/form.css` 파일을 편집하여 양식의 기본 스타일을 수정할 수 있습니다. 이 파일은 다단계 마법사 양식을 지원하여 양식에 대한 포괄적인 스타일을 제공합니다. 양식 전반에 걸쳐 손쉬운 사용자 정의, 유지 관리 및 균일한 스타일 지정을 위해 사용자 정의 CSS 변수 사용을 강조합니다. 프로젝트에 적응형 양식 블록을 추가하는 방법에 대한 지침은 [양식 만들기](/help/edge/docs/forms/create-forms.md)를 참조하십시오.
...
- **양식 스타일 지정:** 이 섹션에서는 선택기를 사용하여 양식 구성 요소의 스타일을 지정해 특정 HTML 요소를 타기팅하는 데 중점을 둡니다. 입력 필드, 텍스트 영역, 확인란, 라디오 버튼, 파일 입력, 양식 레이블 및 설명에 대한 스타일을 정의합니다.
