...
* **클래스 이름** - `classes` 속성이 다음으로 처리됨 [블록 옵션](/help/edge/developer/markup-sections-blocks.md#block-options) 테이블 헤더에 렌더링됩니다. [단순 블록,](#simple) 또는 의 항목에 대한 값 목록으로 [컨테이너 블록.](#container) 원하는 경우 유용합니다. [블록 스타일을 다르게 지정,](/help/edge/aem-authoring/create-block.md#block-options) 그러나 완전히 새로운 블록을 만들 필요는 없습니다.
