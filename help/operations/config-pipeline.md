...
* [지원되는 구성](#configurations) - 구성 파이프라인으로 배포할 수 있는 구성 목록입니다.
...
* [비밀 환경 변수](#secret-env-vars) - 구성에 비밀을 표시하지 않기 위해 환경 변수를 사용하는 예입니다.
* [비밀 파이프라인 변수](#secret-pipeline-vars) - Edge Delivery Services 프로젝트 전 구성에 비밀을 공개하지 않기 위해 환경 변수를 사용하는 예입니다.
...
| [로그 전달](/help/implementing/developing/introduction/log-forwarding.md) | `LogForwarding` | Azure Blob Storage, Datadog, HTTPS, Elasticsearch, Splunk 등 다양한 대상에 로그를 전달하기 위한 엔드포인트 및 자격 증명을 구성합니다. | X | X |
...
## 구성 암호  {#secret-in-configuration}

중요한 정보를 소스 제어에 저장할 필요가 없도록 구성 파일은 구성 파이프라인 변수 또는 환경 변수의 비밀을 참조하는 것을 지원합니다. 로그 전달을 비롯한 일부 구성의 경우 특정 속성에 대해 비밀 변수가 필수입니다. CDN 구성에서 암호 사용에 대한 자세한 내용은 [CDN 자격 증명 및 인증 구성](/help/implementing/dispatcher/cdn-credentials-authentication.md)을 참조하십시오.

다음 코드 조각은 구성에 비밀 변수 `${{SPLUNK_TOKEN}}`을(를) 사용하는 방법의 예입니다.
...
**기본 방법**&#x200B;은(는) 형식이 **secret**&#x200B;인 Cloud Manager 파이프라인 변수를 사용하는 것이므로 중요한 정보를 소스 제어에 저장할 필요가 없습니다. **단계 적용됨** 선택 상자는 **배포** 옵션을 사용해야 합니다.

파이프라인 변수를 사용하는 방법에 대한 자세한 내용은 [Cloud Manager의 파이프라인 변수](/help/implementing/cloud-manager/configuring-pipelines/pipeline-variables.md)를 참조하십시오.


### 비밀 환경 변수 {#secret-env-vars}

환경별로 다른 비밀 값을 사용하려는 경우 비밀 환경 변수를 사용하십시오.

환경 변수를 사용하는 방법에 대한 자세한 내용은 [Cloud Manager 환경 변수](/help/implementing/cloud-manager/environment-variables.md)를 참조하십시오.

>[!NOTE]
>비밀 환경 변수를 사용하는 것은 더 번거롭고 엄격한 규율을 수반합니다. 환경 변수는 구성 파이프라인과 함께 배포되지 않습니다. 파이프라인을 실행하기 전에 이를 배포해야 하며 파이프라인 구성이 여전히 이를 참조하는 동안에는 제거하지 말아야 합니다. 따라서 파이프라인 비밀이 선호됩니다.
