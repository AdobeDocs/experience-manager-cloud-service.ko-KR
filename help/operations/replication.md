...
Adobe Experience Manager as a Cloud Service은 [Sling 콘텐츠 배포](https://sling.apache.org/documentation/bundles/content-distribution.html) 기능을 통해 콘텐츠를 이동하여 AEM 런타임 외부에 있는 Adobe I/O에서 실행되는 파이프라인 서비스를 복제합니다.
...
>게시해야 하는 컨텐츠만 게시하는 것이 좋으며 필요하지 않은 경우 많은 컨텐츠를 게시하지 않는 것이 신중한 경우가 많지만 컨텐츠 트리 게시 작업 과정을 통해 보낼 수 있는 컨텐츠의 양에 제한은 없습니다.

### 빠른 게시 취소/게시 - 예정된 게시 취소/게시 {#publish-unpublish}

이렇게 하면 게시 관리 접근 방식을 통해 가능한 추가 옵션 없이 선택한 페이지를 즉시 게시할 수 있습니다.
...
이에 대해 자동 복제를 구현하려면 [OSGi 구성](/help/implementing/deploying/configuring-osgi.md) **설정/해제 트리거 구성**&#x200B;에서 **자동 복제**&#x200B;를 활성화해야 합니다.
...
모든 워크플로와 마찬가지로 API를 통해 호출할 수도 있습니다. 자세한 내용은 [프로그래밍 방식으로 워크플로와 상호 작용](https://experienceleague.adobe.com/docs/experience-manager-65/developing/extending-aem/extending-workflows/workflows-program-interaction.html?lang=ko-kr#extending-aem)을 참조하십시오.

또는 `Publish Content Tree` 프로세스 단계를 사용하는 워크플로 모델을 만들어 이를 수행할 수도 있습니다.

1. AEM as a Cloud Service 홈페이지에서 **도구 - 워크플로 - 모델**&#x200B;로 이동합니다.
...
1. 목록에서 새로 생성된 모델을 선택한 다음 **편집**&#x200B;을 누릅니다.
...
1. **프로세스** 탭을 클릭한 다음 드롭다운 목록에서 `Publish Content Tree`를 선택합니다.을(를) 클릭한 다음 **핸들러 진행** 확인란

   ![트리 활성화](/help/operations/assets/newstep.png)

1. **인수** 필드에서 추가 매개변수를 설정합니다. 여러 개의 쉼표로 구분된 인수를 함께 열거할 수 있습니다. 예:
...
* `includeChildren`(부울 값, 기본값: `false`) false는 경로만 게시됨을 의미합니다. true는 하위 항목도 게시됨을 의미합니다.
...
트리 활성화 워크플로 단계가 시작되면 INFO 로그 수준에 구성 매개변수가 기록됩니다. 경로가 활성화되면 INFO 문도 기록됩니다.

그런 다음 워크플로 단계가 모든 경로를 복제한 후 최종 INFO 문이 기록됩니다.

또한 `com.day.cq.wcm.workflow.process.impl` 아래 로거의 로그 수준을 DEBUG/TRACE로 높여 더 훨씬 많은 로그 정보를 가져올 수 있습니다.

오류가 발생한 경우 워크플로 단계는 `WorkflowException`으로 종료되며, 기본 예외가 래핑됩니다.

아래에서는 샘플 콘텐츠 트리 게시 워크플로 중에 생성된 로그 예제를 찾을 수 있습니다.
...
워크플로는 콘텐츠를 청크 단위로 처리하며, 각 청크는 게시할 전체 콘텐츠의 하위 집합을 나타냅니다. 어떤 이유로든 워크플로가 시스템에 의해 중지되면 해당 워크플로는 시작되고 아직 처리되지 않은 청크가 처리됩니다. 로그 문에는 특정 경로에서 콘텐츠가 다시 시작되었다고 표시됩니다.
...
위의 예와 같이 리소스를 복제할 때는 기본적으로 활성 상태인 에이전트만 사용됩니다. AEM as a Cloud Service에서 이는 작성자를 게시 계층에 연결하는 “게시”라는 에이전트입니다.

미리보기 기능을 지원하기 위해 “미리보기”라는 새 에이전트가 추가되었으며, 이는 기본적으로 활성화되어 있지 않습니다. 이 에이전트는 작성자를 미리보기 계층에 연결하는 데 사용됩니다. 미리보기 에이전트를 통해서만 복제하려면 `AgentFilter`를 통해 이 미리보기 에이전트를 명시적으로 선택해야 합니다.
...
복제 작업에 기본적으로 활성 상태인 에이전트가 하나 이상 포함된 경우 리소스의 전체 `ReplicationStatus`만 수정됩니다. 위의 예에서는 복제가 “미리보기” 에이전트를 사용하고 있기 때문에 해당되지 않습니다. 따라서 특정 에이전트 상태의 쿼리를 허용하는 새 `getStatusForAgent()` 메서드를 사용해야 합니다. 이 메서드는 “게시” 에이전트에도 작동합니다. 제공된 에이전트를 사용하여 복제 작업을 수행한 경우 null이 아닌 값이 반환됩니다.
...
한 번에 100개 이하의 경로를 복제하고 500개를 엄격한 제한으로 설정하는 것이 좋습니다. 엄격한 제한을 초과하면 `ReplicationException`이 표시됩니다.
애플리케이션 논리에 작은 단위의 복제가 필요하지 않는 경우 `ReplicationOptions.setUseAtomicCalls`를 false로 설정함으로써 이 제한을 극복할 수 있습니다. 이렇게 하면 원하는 수의 경로가 허용되며, 내부적으로 버킷이 생성되어 경로 수가 이 제한보다 낮게 유지됩니다.

복제 호출당 전송되는 콘텐츠 크기는 `10 MB`를 초과해서는 안 됩니다. 여기에는 노드 및 속성이 포함되지만 바이너리는 포함되지 않습니다(워크플로 패키지 및 콘텐츠 패키지는 바이너리로 간주됨).
...
이 경우 편집 가능한 기본 대기열은 빨간색 상태로 표시되며 이를 검토하여 어떤 항목이 게시를 취소했는지 확인해야 합니다. 해당 대기열을 클릭하면 보류 중인 항목이 표시되며 필요한 경우 단일 항목 또는 모든 항목을 지울 수 있습니다.
