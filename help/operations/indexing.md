...
>[!NOTE]
>
>CRX DE 패키지 관리자(`damAssetLucene-8`)를 사용하여 *Cloud Service 환경*&#x200B;에서 최신 기본 제공 인덱스 정의(예: `/crx/packmgr/`)를 복사할 수 있습니다. 이름을 `damAssetLucene-8-custom-1` 이상으로 바꾸고 XML 파일 내에 사용자 지정을 추가합니다. 클라우드 환경의 인덱스가 `elasticsearch` 유형인 경우 `type` 속성을 `lucene`(으)로 변경하고, `async` 속성을 `[async,nrt]`(으)로 변경하고, `similarityTags` 속성을 `true`(으)로 변경하는 등 추가 변경이 필요합니다. 이렇게 하면 필요한 구성이 실수로 제거되지 않도록 할 수 있습니다. 예를 들어 `tika` 아래의 `/oak:index/damAssetLucene-8/tika` 노드는 AEM Cloud Service 환경에 배포된 사용자 지정된 인덱스에 필요하지만 로컬 AEM SDK에는 없습니다.
...
제한 섹션에서 언급한 바와 같이 패키지 관리자를 사용하여 추출된 색인 정의가 다른 유형(예: `type`)인 경우에도 사용자 지정된 색인 정의의 `lucene`을(를) 항상 `elasticsearch`(으)로 설정해야 합니다.
추출된 색인 정의가 `async`(으)로 설정된 경우에도 `elastic-async` 속성을 변경해야 합니다. 사용자 지정된 인덱스 정의에 대해 `async` 속성을 `[async]`, `[async,nrt]` 또는 `[fulltext-async]` 중 하나로 설정해야 합니다.
...
환경은 서로 다른 AEM 버전에 있을 수 있습니다. 예를 들어 `dev`에 필요한 테스트를 수행한 후 스테이징 및 프로덕션이 아직 릴리스 `X+1`에 있고 릴리스 `X`(으)로 업그레이드되기를 기다리는 동안 `X+1` 환경이 릴리스 `dev`에 있습니다. 릴리스 `X+1`이(가) 사용자 지정된 새 버전의 제품 색인과 함께 제공되며 해당 색인의 새 사용자 지정이 필요한 경우 다음 표에서 AEM 릴리스를 기반으로 하는 환경에 설정해야 하는 버전을 설명합니다.
...
색인 관리는 `lucene`이(가) `compatVersion`(으)로 설정된 `2` 유형의 색인에 대해서만 지원됩니다. 내부적으로 다른 색인을 구성하고 쿼리에 사용할 수 있습니다(예: Elasticsearch 색인). `damAssetLucene` 색인에 대해 작성된 쿼리는 AEM as a Cloud Service에서 이 색인의 Elasticsearch 버전에 대해 실행될 수 있습니다. 이 차이는 응용 프로그램 사용자에게는 보이지 않지만 `explain` 기능과 같은 특정 도구에서는 다른 색인을 보고합니다. Lucene 색인과 Elasticsearch 색인의 차이에 대해서는 [Apache Jackrabbit Oak의 Elasticsearch 설명서](https://jackrabbit.apache.org/oak/docs/query/elastic.html)를 참조하십시오. 고객은 Elasticsearch 색인을 직접 구성할 수 없으며 구성할 필요가 없습니다.
...
OOTB 인덱스의 정의를 새 버전으로 사용하여 [변경 실행 취소](#undoing-a-change-undoing-a-change)에 설명된 단계를 따릅니다. 예를 들어 이미 `damAssetLucene-8-custom-3`을(를) 배포했지만 더 이상 맞춤화가 필요하지 않고 기본 `damAssetLucene-8` 색인으로 다시 바꾸고 싶다면 `damAssetLucene-8-custom-4`의 색인 정의가 포함된 색인 `damAssetLucene-8`을(를) 추가해야 합니다.
