...
>* 백업을 복원하면 백업 시점과 현재 시점 사이에 최근 데이터가 손실됩니다. 또한 스테이징이 이전 버전으로 복원됩니다.
...
* 삭제된 경로에 대한 패키지를 사용할 수 있는 경우 [패키지 관리자.](/help/implementing/developing/tools/package-manager.md)
* 삭제된 경로가 Sites의 페이지인 경우 [트리 복원 기능입니다.](/help/sites-cloud/authoring/sites-console/page-versions.md)
* 삭제된 경로가 에셋 폴더이고 원래 파일을 사용할 수 있는 경우 를 통해 다시 업로드하십시오. [에셋 콘솔.](/help/assets/add-assets.md)
* 삭제 콘텐츠가 자산인 경우 다음을 고려하십시오. [자산의 이전 버전을 복원하는 중입니다.](/help/assets/manage-digital-assets.md)

위의 옵션 중 어느 것도 작동하지 않고 삭제된 경로의 내용이 중요한 경우 다음 섹션에 설명된 대로 콘텐츠 복원을 수행합니다.

## 사용자 역할 만들기 {#user-role}

기본적으로 개발, 프로덕션 또는 스테이징 환경에서 컨텐츠 복원을 실행할 수 있는 권한은 사용자에게 없습니다. 이러한 일반 단계에 따라 특정 사용자 또는 그룹에 이 권한을 위임하려면 다음을 수행하십시오.

1. 콘텐츠 복원을 참조하는 표현적 이름으로 제품 프로필을 만듭니다.
1. 다음을 제공합니다 **프로그램 액세스** 필수 프로그램에 대한 권한.
1. 다음을 제공합니다 **콘텐츠 복원** 사용 사례에 따라 프로그램의 필수 환경 또는 모든 환경에 대한 권한.
1. 해당 프로필 프로필에 사용자를 할당합니다.

권한 관리에 대한 자세한 내용은 을 참조하십시오. [사용자 지정 권한](/help/implementing/cloud-manager/custom-permissions.md) 설명서를 참조하십시오.
...
>[!NOTE]
>
>사용자에게 다음이 있어야 합니다. [적절한 권한](#user-role) 복원 작업을 시작합니다.
...
백업에 대한 정보 아이콘을 클릭하면 해당 백업에 대한 로그를 다운로드하고 복원이 시작된 시점의 스냅샷과 데이터 간의 차이점을 포함한 코드 세부 정보를 검사할 수 있습니다.
