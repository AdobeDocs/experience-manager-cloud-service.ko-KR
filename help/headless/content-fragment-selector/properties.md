...
| 속성 | 유형 | 필수 | 기본값 | 설명 |
|--- |--- |--- |--- |--- |
| `ref` | 조각 선택기 참조 | 아니요 | | `ContentFragmentSelector` 인스턴스를 참조하여 `reload`과(와) 같이 제공된 기능에 액세스할 수 있습니다. |
...
| `allowedRepositoryIds` | 문자열[] | 아니요 | | 콘텐츠 조각 선택기에서 저장소 및 콘텐츠 조각을 필터링할 저장소 ID 목록입니다. 저장소 ID가 제공되면 이러한 저장소만 저장소 선택기에 표시됩니다. 제공되지 않거나 빈 배열인 경우 사용자가 액세스할 수 있는 모든 저장소를 사용할 수 있습니다. |
...
| `getImsToken` | 자산 표현물 생성과 같은 다른 서비스에 대한 요청을 인증하는 데 사용할 수 있는 현재 로그인한 사용자에 대한 인증 `imsToken`을(를) 검색합니다. |
