...
* SITES-20149: RTC: `[cq-wcm-launches-core]` CF용 Launch에 대한 새 API를 내보냅니다.
* SITES-20150: RTC: `[cq-command]` 기존 API에 새 메서드를 추가합니다.
* SITES-20238: `[RTC]` Pfizer MVP - CF 경로를 ID로 또는 그 반대로 해결하려면 CF API를 추가하십시오.
* SITES-20333: 콘텐츠 조각을 만들 때 유효성 검사를 개선합니다.
* SITES-20334: 콘텐츠 조각 모델을 편집할 때 유효성 검사를 개선합니다.
* SITES-20342: `[Backend]` 폴더 수준에서 게시합니다. CF만 게시하려면 필터를 추가하십시오.
...
* SITES-20495: `[BE]` 폴더 수준에서 게시할 권한을 받는 기능입니다.
...
* SITES-20601: `[OpenAPI]` 직접 하위 콘텐츠 조각만 가져오도록 CF 검색 API를 업데이트합니다.
* SITES-20653: 실험 시작 날짜 및 종료 날짜를 추가합니다.
* SITES-20656: `[BE]` 문자열을 바꿀 때 대/소문자를 일치시키는 옵션을 제공합니다.
* SITES-20666: 작성 시 기본적으로 실험이 꺼져 있어야 합니다.
* SITES-20752: CF용 Apple 실행 `[cq-wcm-core]`개.
* SITES-20763: 사이트 통합에서 게재 API 엔드포인트를 업데이트합니다.
* SITES-20946: LIST 모델 엔드포인트의 속성으로 etag를 추가합니다.
* SITES-20947: `[persistence]` 콘텐츠 조각 ID별로 하위 작업을 가져옵니다.
* SITES-21012: 모델 메타데이터 스키마를 제품에 병합합니다.
* SITES-21043: Cloud Service의 `[CF][launches]` 측면 포트 성능이 개선되었습니다.
* SITES-21044: `[CF][launches]` Side-port 비동기 Cloud Service에 대한 페이로드 구현 편집.
* SITES-21550: 사용자 지정 메타데이터: 숫자, 날짜, 날짜, 시간 필드.
* SITES-21769: ID별 검색을 위해 /jcr:id/ 경로 접두사를 사용합니다.
...
* SITES-11727: `[GQL]` 콘텐츠 조각 참조의 전체 하이드레이션에 데이터가 없습니다.
...
* SITES-20559: `[MSM][XF][Lufthansa]` 마스터/언어에서 국가/언어로 경험 조각을 롤아웃하면 참조가 업데이트되지 않습니다.
...
* SITES-20691: 경험 조각 템플릿 제한이 cq:allowedTemplates을(를) 준수하지 않습니다.
* SITES-20816: CF OpenAPI - 참조된 조각에 대한 누락된 모델과 출력 불일치가 발생합니다.
* SITES-21122: AEM CS 콘텐츠 조각에 Live Copy 결함이 있습니다.
* SITES-21233: SITES로 업그레이드한 후 GS1 US에 대해 `[CoreCmp]` 아코디언이 15860.
...
* ASSETS-35559: DM 배치 업로드 실패 로그를 WARN으로 감소시킵니다.
