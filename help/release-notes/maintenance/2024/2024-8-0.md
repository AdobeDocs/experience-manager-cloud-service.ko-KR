...
* SITES-12209 - 색인에 cq:policy을(를) 추가하여 정책 편집기 성능을 개선합니다.
...
* FORMS-14327 - Forms 서비스 API : 비대화형 PDF가 입력에 제공되면 데이터 추출:-에서 500개의 응답 코드를 제공합니다.
...
* SITES-23662 - 게시를 트리거하는 사용자를 서버 로그의 JCR 로그 구문에서 추출할 수 없습니다. 이는 개발 중인 기능으로 인해 로그에 간헐적이지만 무해한 “OSGI 이벤트 배치에서 유효한 사용자 ID를 찾을 수 없습니다.” 오류가 발생할 수 있습니다.
