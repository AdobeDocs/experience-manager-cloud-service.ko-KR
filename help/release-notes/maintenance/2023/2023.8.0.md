...
2023.8.0 기능 활성화는 이 유지 관리 릴리스에 대한 전체 기능 세트를 제공합니다. 자세한 내용은 [Experience Manager 릴리스 로드맵](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/update-releases-roadmap.html)을 참조하십시오.
...
- SITES-14171: GraphQL - 캐시된 데이터에 대한 순환 참조가 끊기지 않는 경우가 있습니다.
...
- ASSETS-24478: 인덱스에서 잠재적으로 큰 5개의 속성 제거(고객 인덱스 데이터 분석을 기반으로 함)
...
따라서 현재 사용자가 액세스할 수 없는 에셋을 포함하는 패싯 카운트 값이 사용자에게 제공될 수 있습니다. 이때 사용자의 해당 자산 액세스, 다운로드 또는 읽기가 불가하며 사용자가 자산의 존재에 대한 추가 정보를 얻을 수 없습니다.
...
- ASSETS-25832: 전체 액세스 폴더에서 읽기 전용 액세스 폴더로 에셋을 연결할 때 발생하는 문제를 해결했습니다.
...
- ASSETS-26700: 변경 사항 없이 공용 폴더의 속성 페이지를 저장하면 세 개의 불필요한 그룹이 생성되던 문제를 해결했습니다
