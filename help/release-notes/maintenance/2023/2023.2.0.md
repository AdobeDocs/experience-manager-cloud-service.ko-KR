...
- SITES-13636 - 일부 고객이 이미지와 회전 사이트 clientlib의 clientlib 종속성을 누락한 것으로 보고된 문제 고객이 이미지나 회전식 clienlib을 포함하는 다른 clientlib을 임베드한 경우 종속성이 해결되지 않습니다.
- SITES-13475 - 2.22.10 핵심 구성 요소 버전의 PanelContainer.js에 대한 이전 버전과의 호환성 저하 문제
...
- SITES-12573 - 하나의 변수를 지정하지 않으면 필터 내에 변수를 사용하는 GraphQL 쿼리가 실패합니다. GraphQL을 AEM as a Cloud Service&#39;와 함께 사용하려면 이 릴리스로 업데이트하지 마십시오.
