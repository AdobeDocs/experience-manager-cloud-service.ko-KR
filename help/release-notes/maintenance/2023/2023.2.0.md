...
다음 섹션에서는 2023.2.0 기능 활성화와 연결된 as a Cloud Service Experience Manager의 유지 관리 릴리스에 대한 기술 릴리스 노트를 간략하게 설명합니다.
...
>[!IMPORTANT]
>
> CloudManager 사용자 인터페이스에 불일치가 있으며 &quot;2023.3.11382&quot;을 표시하지만 공식 릴리스는 &quot;2023.02&quot;입니다. 이러한 불일치는 2023.02 기능의 활성화가 지연되었기 때문입니다.
> Adobe이 예정된 릴리스에 대해 이 문제를 해결하기 위해 노력 중입니다.
...
- SKYOPS-49280 - RDE를 사용하여 구성 또는 번들 업데이트를 Publish에 설치할 때 Publish Dispatcher 캐시가 무효화되지 않으므로 결과를 관찰할 수 없습니다

### 알려진 문제 {#known-issues-11382}

- SITES-12573 - 하나의 변수가 지정되지 않으면 필터 내부 변수를 사용하는 GraphQL 쿼리가 실패합니다. AEMas a Cloud Service 와 함께 GraphQL을 사용하는 경우 이 릴리스를 업데이트하지 마십시오.
- SKYOPS-51970 - 이미지 작성 단계에서 사용된 FACT 버전의 확인된 회귀, 불일치 사용자 매핑 초래
- GRANITE-44542 - 문제가 패키지 필터에 포함된 폴더를 위한 패키지 노드 유형을 지정하지 않은(.content.xml with jcr:primaryType을 입력해서) 고객을 위해서 보고되었습니다. 이 문제로 인해 이러한 폴더가 nt:folder로 처리되어 다양한 경우에 문제가 발생했습니다.
...
CORS를 사용하는 경우 업그레이드하지 마십시오. 이 릴리스에서 GraphQL 콘텐츠 게재 기능에 영향을 미치는 문제가 확인되었습니다. GraphQL 지속 쿼리가 캐시되는 방식에 대한 기본 AEM Dispatcher 구성의 변경으로 인해 이러한 쿼리의 GraphQL 콘텐츠 전달이 중단될 수 있습니다. 이 문제는 Adobe의 다음 유지 관리 릴리스에서 수정됩니다.
