...
- GRANITE-44542 - 패키지 필터에 포함된 폴더에 대해 패키지 노드 유형을 지정하지 않은 고객(jcr:primaryType에 .content.xml을 제공하여)에 대해 문제가 보고되었습니다. 이로 인해 이러한 폴더가 nt:folder(으)로 처리되어 다양한 경우에 문제가 발생했습니다.
