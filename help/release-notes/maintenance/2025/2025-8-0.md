...
* Apache HTTPD 버전 2.4.65는 보안 수정의 일환으로 구현된 새로운 제한으로 인해 특정 구성에 영향을 줄 수 있는 변경 사항을 도입합니다. 이러한 수정 사항은 `RequestHeader set`, `edit`, `edit_r`과 같은 콘텐츠 유형 헤더를 수정하는 지시문이 이제 요청 헤더로만 올바르게 제한되도록 하여 취약점을 해결합니다. 이 변경 사항은 특히 정적 콘텐츠의 경우 응답 헤더에 대한 의도하지 않은 수정을 방지합니다.
