...
* AEM [SPA Editor.](/help/implementing/developing/hybrid/introduction.md)에서 [Launches](/help/sites-cloud/authoring/launches/overview.md)를 만들 수 있습니다.
...
* 비디오 코드 변환 시 이제 자산 마이크로서비스에서 지원됩니다. [!UICONTROL 처리 프로필] 구성의 새 섹션을 통해 비디오 비트율 및 차원을 설정할 수 있습니다. 출력 형식은 H.264 코덱이 있는 MP4입니다. 자세한 내용은 [비디오 자산 관리](/help/assets/manage-video-assets.md#transcode-video)를 참조하십시오. 더 많은 코드 변환 옵션과 비디오 전달을 위해 [!DNL Dynamic Media] 추가 기능을 사용하십시오.

* 새 [!DNL Experience Manager Assets] 배포에서 이제 스마트 태그 지정 기능이 기본적으로 구성됩니다. 를 [!DNL Adobe Developer Console]과 수동으로 통합할 필요가 없습니다. 기존 배포에서 관리자는 이전처럼 스마트 태그 통합을 구성합니다.
...
* 향상된 액세스 가능성 지원이 [!DNL Assets]의 사용자 인터페이스 컨트롤, 탐색, 검색 및 검색 경험에 추가되었습니다.

   * [!UICONTROL 표현물 추가] 옵션을 선택한 후 Esc 키를 누르면 포커스가 도구 모음으로 돌아갑니다.<!-- via CQ-4293594-->
...
* [!DNL AEM Desktop app] 이제 2.0.3 릴리스를 사용할 수 있습니다. 이 기능은 [!DNL Experience Manager] 6.5.5 서비스 팩과의 호환성을 향상하고 클라이언트 OS 호환성 목록을 업데이트했습니다. [!DNL Windows] 7 및  [!DNL macOS] 10.14 이전 버전은 지원되지 않습니다.

### [!DNL Assets] {#bugs-fixed}에 수정된 버그
...
* AEM Dispatcher Converter 는 온-프레미스 및 Adobe Managed Services 디스패처 구성을 AEM과 Cloud Service 호환 디스패처 구성으로 변환하는 기능을 지원합니다. [Git 리소스 를 참조하십시오.AEM Cloud Service Dispatcher 변환기](https://github.com/adobe/aem-cloud-service-source-migration/tree/master/packages/dispatcher-converter) 를 참조하십시오.
