...
* 콘텐츠 모델, 새로운 탭 자리 표시자 데이터 유형 - 새로운 탭 자리 표시자 데이터 유형을 사용하면 데이터 유형을 콘텐츠 조각 편집기의 탭 아래에 렌더링될 섹션으로 그룹화할 수 있습니다.
...
* 많은 에셋의 메타데이터는 CSV 파일을 사용하여 일괄로 가져올 수 있으며 CSV 파일로 내보낼 수 있습니다. 이제 기본 날짜 형식은 입니다 `yyyy-MM-dd'T'HH:mm:ss.SSSXXX`. 사용자는 열 헤더를 업데이트하여 다른 형식을 활용할 수 있습니다. 예: 추가 `Date: DateFormat: yyyy-MM-dd'T'HH:mm:ssXXX` csv 파일의 열 머리글로 사용됩니다(단어 대신). `Date`.
