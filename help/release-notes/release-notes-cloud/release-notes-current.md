...
참여를 위해 선정된 고객은 Adobe이 직접 통보한다. 참여는 고객 라이선싱과 제한된 프로그램 용량을 포함한 자격 고려 사항이 적용됩니다. 모든 요청을 초기에 수용할 수는 없지만 향후 베타 웨이브에서 추가 고객을 고려할 수 있습니다.
...
채팅 애플리케이션(예를 들어, ChatGPT) 및 최신 IDE(예를 들어, 커서)는 모델 컨텍스트 프로토콜(MCP)을 사용하여 MCP 서버에 의해 노출된 대형 언어 모델(LLM) 호출 도구를 허용할 수 있다. 낮은 수준의 API 사양에 따라 코드를 작성하는 대신 원하는 내용을 자연어로 설명하고 LLM으로 해당 의도를 적절한 API 호출로 변환하도록 할 수 있습니다. 또한 경우에 따라 프롬프트가 사용자 인터페이스를 탐색하는 것보다 더 편리할 수 있습니다.

예를 들어, ChatGPT를 입력하라는 메시지를 표시할 수 있습니다. *AEM에서 /en/coffee.html을 편집합니다. 컨텐츠의 Fresccopa Coffee 텍스트를 20세가 원하는 것으로 변경합니다.*

AEM은 **페이지**, **콘텐츠 조각** 및 **Assets** 작업을 위한 도구를 사용하여 두 개의 콘텐츠 MCP 서버(**읽기 전용** 및 **읽기/쓰기**)를 제공합니다. 이러한 서버는 **ChatGPT**, **Claude**, **Cursor** 및 **Microsoft Copilot Studio** MCP 클라이언트에서 사용할 수 있습니다.
...
AI Search는 사용자 쿼리의 의미와 의도를 파악하여 기존의 키워드 일치를 넘어서는 지능적이고 상황에 맞는 검색 경험을 제공합니다. AI와 머신 러닝을 기반으로, 쿼리가 다르게 표현되거나, 철자가 틀리거나, 동의어를 사용하거나, 다른 언어로 제출되는 경우에도 보다 정확한 결과를 제공하여 사용자가 적은 노력으로 관련 콘텐츠를 더 빨리 찾을 수 있도록 지원합니다.

자세한 내용은 [Assets 보기](/help/assets/search-assets-view.md#ai-search) 및 [관리자 보기](/help/assets/search-assets.md#ai-search)에서 AI 검색을 참조하십시오.
