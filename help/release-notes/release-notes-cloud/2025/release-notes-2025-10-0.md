...
유니버설 편집기는 사용자 지정 제출 작업, Microsoft SharePoint에 제출, Microsoft OneDrive에 제출, Azure Blob Storage에 제출, REST 끝점에 제출, AEM 워크플로 호출, Power Automate 흐름 호출, Marketo Engage에 제출, Adobe Experience Platform(AEP)에 제출, 스프레드시트에 제출, 양식 데이터 모델(FDM)을 사용하여 제출, Workfront Fusion에 제출 및 이메일 보내기 등 광범위한 제출 워크플로를 지원합니다.
