...
>[!IMPORTANT]
>
>**2026년 6월 11일**&#x200B;에 여러 [더 이상 사용되지 않는 API](#aem-apis)가 제거됩니다. 다음 주요 날짜 및 영향을 검토하십시오.
>
...
>* **2026년 4월 14일**: 이러한 API를 사용하는 코드가 포함된 Cloud Manager 파이프라인은 **코드 품질** 단계 동안 **실패**&#x200B;됩니다. 더 이상 사용되지 않는 API 사용이 제거될 때까지 배포가 차단됩니다. *시간이 중요한 업데이트를 릴리스할 수 없으며 비즈니스 운영에 영향을 줄 수 있습니다.*
>* **2026년 6월 11일**: 더 이상 사용되지 않는 API를 사용하는 환경은 중요한 Adobe 릴리스 업데이트를 받지 않습니다&#x200B;**. 따라서 성능 및 가용성에 대한 Adobe의 표준 약정이 적용되지 않습니다.**&#x200B;그 결과, 새로운 기능이나 버그 수정을 받지 않고, 애플리케이션 안정성과 가동 시간에 부정적인 영향을 미치고, 보안 위험 노출이 더 증가할 수 있습니다. Adobe 릴리스 업데이트를 다시 받으려면 전체 스택 파이프라인을 성공적으로 실행해야 합니다. 그러면 며칠 내에 업데이트가 적용됩니다.
>
...
| Sites | [SPA 편집기](/help/implementing/developing/hybrid/introduction.md) | AEM에서 헤드리스 콘텐츠 관리에 권장되는 편집기는 <br>- [범용 편집기](https://www.aem.live/docs/aem-authoring)&#x200B;(시각적 편집용) <br>- [콘텐츠 조각 편집기](/help/assets/content-fragments/content-fragments-managing.md)(양식 기반 편집용)입니다. |
...
>* **2026년 2월 26일**: 이러한 API를 사용하는 코드가 포함된 Cloud Manager 파이프라인은 **코드 품질** 단계 동안 **일시 중지**&#x200B;됩니다. 배포 관리자, 프로젝트 관리자 또는 비즈니스 소유자는 파이프라인을 진행할 수 있도록 문제를 재정의할 수 있습니다. *이로 인해 코드 변경 내용의 유효성을 검사하고 해제하는 기능이 느려질 수 있습니다.*
>* **2026년 4월 14일**: 이러한 API를 사용하는 코드가 포함된 Cloud Manager 파이프라인은 **코드 품질** 단계 동안 **실패**&#x200B;됩니다. 더 이상 사용되지 않는 API 사용이 제거될 때까지 배포가 차단됩니다. *시간이 중요한 업데이트를 릴리스할 수 없으며 비즈니스 운영에 영향을 줄 수 있습니다.*
>* **2026년 6월 11일**: 더 이상 사용되지 않는 API를 사용하는 환경은 중요한 Adobe 릴리스 업데이트를 받지 않습니다&#x200B;**. 따라서 성능 및 가용성에 대한 Adobe의 표준 약속이 적용되지 않습니다.**&#x200B;그 결과, 새로운 기능이나 버그 수정을 받지 않고, 애플리케이션 안정성과 가동 시간에 부정적인 영향을 미치고, 보안 위험 노출이 더 증가할 수 있습니다.
>
...
ACS AEM Commons를 사용하는 경우, 최소 버전 6.17.2(최신 버전 권장)를 사용하고, 콘텐츠 패키지에 대해 분류자 `cloud`을(를) 지정하여 [Cloud Service의 버전을 포함](https://adobe-consulting-services.github.io/acs-aem-commons/pages/maven.html)해야 합니다.
...
* ACS AEM Commons 최신 버전(최소 6.17.2)으로 업데이트하고 `cloud` 분류자를 사용해야 하는 경우.
...
* 이 목록에서 필요한 번들을 선택하고 프로젝트에 추가합니다. 패키지를 추가하는 경우 해당 가져오기를 수행하기 위해 이러한 목록에서 다른 번들을 추가해야 할 수 있으므로 해당 번들의 패키지 가져오기를 확인해야 합니다. 이러한 번들의 버전 9.x를 사용하려면 아래에서 버전을 정확히 배포하고 *및* 패키지를 내보내지 않도록 하십시오.
...
* ACS AEM Commons 최신 버전(최소 6.17.2)으로 업데이트하고 `cloud` 분류자를 사용해야 하는 경우.
* `io.wcm:io.wcm.caconfig.extensions`을(를) 사용하는 경우 버전 1.9.2 이상으로 업데이트하십시오.
...
* ACS AEM Commons 최신 버전(최소 6.17.2)으로 업데이트하고 `cloud` 분류자를 사용해야 하는 경우.
* Apache Kafka 클라이언트를 사용 중이며 Apache ServiceMix(`org.apache.servicemix.bundles.kafka-clients`)의 OSGi 래퍼 번들을 포함하는 경우 [AEM Apache Kafka 클라이언트 래퍼](https://repo.maven.apache.org/maven2/com/adobe/aem/osgi/com.adobe.aem.osgi.kafka-clients/4.0.0_1.0/)&#x200B;(으)로 바꾸십시오. 이 버전은 해당 두 패키지의 사용만 제거된 Apache ServiceMix의 버전과 동일합니다.
* 최신 버전(최소 2.0.12)으로 `com.adobe.aio.aem:aio-lib-osgi` 업데이트를 사용하는 경우.
...
다음 OSGi 구성 요소 PID에 대한 속성은 수정할 수 없으므로 구성해서는 안 됩니다.
