...
>[!NOTE]
>
> SEO(검색 엔진 최적화)는 많은 마케터의 주요 관심사가 되었습니다. 따라서 Adobe Experience Manager(AEM) as a Cloud Service의 많은 프로젝트에서 SEO 문제를 해결해야 합니다. 자세한 내용은 [SEO 및 URL 관리 모범 사례](/help/overview/seo-and-url-management.md)를 참조하세요.
...
>[!VIDEO](https://video.tv.adobe.com/v/38586/?captions=kor&quality=12)
...
>[!NOTE]
> 
> `url_path`은(는) `url_keys` 슬래시로 구분된 제품이나 범주의 상위 항목 `url_key`과(와) 제품이나 범주의 `/`의 연결입니다. 각 `url_key`은(는) 지정된 저장소 내에서 고유한 것으로 간주됩니다.
...
카탈로그의 특정 하위 집합 또는 제품에 대해서만 [다중 범주 및 제품 페이지](/help/commerce-cloud/cif-storefront/authoring/multi-template-usage.md)를 만들 수 있습니다.
...
#### SKU가 포함된 제품 페이지 URL 형식을 사용합니다. {#use-sku}
...
#### 카테고리 컨텍스트가 포함된 제품 페이지 URL 형식을 사용합니다. {#use-url}
...
#### URL 길이와 인코딩된 정보 간의 균형. {#balance-url}
...
>[!NOTE]
>
>프로젝트에서 사용하는 외부 도메인을 사용하여 이 구성을 조정해야 합니다. Sling 매핑은 호스트 이름과 도메인을 기반으로 작동합니다. 따라서 이 구성은 기본적으로 비활성화되며 배포 전에 활성화해야 합니다. 이렇게 하려면 사용된 도메인 이름에 따라 `hostname.adobeaemcloud.com`에서 Sling 매핑 `ui.content/src/main/content/jcr_root/etc/map.publish/https` 폴더의 이름을 바꾸고 프로젝트의 `resource.resolver.map.location="/etc/map.publish"` 구성에 `JcrResourceResolver`을(를) 추가하여 이 구성을 사용하도록 설정하십시오.
