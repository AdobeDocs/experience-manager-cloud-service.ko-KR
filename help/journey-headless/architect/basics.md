...
이 문서는 이러한 원칙을 기반으로 하므로 AEM Headless 프로젝트에 대한 콘텐츠를 모델링하는 방법을 이해할 수 있습니다.
...
>[!NOTE]
>
>편집기에서 UUID 참조는 참조된 리소스에 대한 경로를 지정합니다. 내부적으로 이러한 참조는 리소스를 참조하는 UUID(범용 고유 ID)로 유지됩니다.
