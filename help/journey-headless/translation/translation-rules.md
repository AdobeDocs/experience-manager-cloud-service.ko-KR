...
>[!TIP]
>
>일반적으로 콘텐츠 설계자는 번역 전문가에게 **속성 이름**&#x200B;번역을 위해 필요한 모든 필드 중 하나. 변환 규칙을 구성하는 데 해당 이름이 필요합니다. 번역 전문가는 [앞서 이 여정에서 설명한 대로 해당 **속성 이름**&#x200B;을 ](getting-started.md#content-modlels)검색할 수 있습니다.
...
다음에서 **번역 구성** UI에는 번역 규칙에 사용할 수 있는 몇 가지 옵션이 있습니다. 여기에서 기본 Headless 현지화 구성에 가장 필요하고 일반적인 단계를 집중적으로 다룹니다.

1. 선택 **컨텍스트 추가** 경로를 추가합니다. 이는 규칙의 영향을 받는 콘텐츠 경로입니다.
   ![컨텍스트 추가](assets/add-translation-context.png)
1. 경로 브라우저를 사용하여 필요한 경로를 선택한 다음 를 선택합니다. **확인** 저장. Headless 콘텐츠를 보유하는 콘텐츠 조각은 일반적으로 `/content/dam/<your-project>` 아래에 위치합니다.
   ![경로 선택](assets/select-context.png)
1. 방금 만든 컨텍스트를 선택한 다음 을 선택합니다. **편집**. 이렇게 하면 **번역 규칙 편집기**가 열려 속성을 구성할 수 있습니다.
...
1. 선택하지 않은 경우 **일반** 목록의 섹션에서, 원하는 콘텐츠 조각 모델의 속성 이름을 추가합니다. [이전에는 번역을 위한 필드로 식별되었습니다.](getting-started.md#content-models)
   1. 다음에서 **새 속성** 필드에 속성 이름을 입력합니다. 옵션은 다음과 같습니다 **번역** 및 **상속** 은 자동으로 선택됩니다.
   1. 선택 **추가**.
...
번역 규칙의 일부로 구성할 수 있는 몇 가지 추가 속성이 있습니다. 또한 XML로 직접 규칙을 지정할 수 있으므로 보다 구체적이고 유연하게 규칙을 지정할 수 있습니다.
