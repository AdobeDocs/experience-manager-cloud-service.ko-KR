---
title: 번역 규칙 구성
description: 현지화를 위한 콘텐츠를 식별하는 번역 규칙을 정의하는 방법을 알아봅니다.
...
현지화를 위한 콘텐츠를 식별하는 번역 규칙을 정의하는 방법을 알아봅니다.

## 지금까지 그 이야기 {#story-so-far}

AEM 헤드리스 로컬라이제이션 여정의 이전 문서에서, [번역 커넥터 구성](configure-connector.md) 에서는 번역 커넥터를 설치하고 구성하는 방법을 알아보았으며, 이제 다음을 수행해야 합니다.
...
번역 규칙은 번역 프로젝트에 포함되거나 번역 프로젝트에서 제외되는 콘텐츠를 식별합니다. 컨텐츠가 번역되면 AEM은 이러한 규칙에 따라 컨텐츠를 추출하여 이미 설정한 커넥터를 통해 번역 서비스로 전송할 수 있습니다.
...
컨텐츠 조각 모델은 고유한 프로젝트이므로 AEM에서 번역할 컨텐츠 모델의 요소를 파악할 수 있도록 번역 규칙을 설정하는 것이 중요합니다.

## 번역 규칙 만들기 {#creating-rules}

복잡한 번역 요구 사항을 지원하도록 여러 규칙을 만들 수 있습니다. 예를 들어, 작업 중인 프로젝트 중 하나에 대해서는 모델의 모든 필드를 번역해야 하지만, 다른 설명 필드에서는 제목을 번역하지 않은 상태로 두어야 합니다.
...
1. AEM에서 구성을 저장합니다.
1. 방금 만든 컨텍스트를 선택한 다음 **편집**&#x200B;을 탭하거나 클릭해야 합니다. 속성을 구성하려면 **번역 규칙 편집기**를 엽니다.
...
1. 선택 취소하면 목록의 **일반** 섹션 아래에서 이전에 번역 필드로 식별한 [속성 이름을 추가합니다.](getting-started.md#content-models)
...
이러한 지식을 바탕으로 AEM 헤드리스 로컬라이제이션 여정을 계속 진행하려면 [콘텐츠 번역](translate-content.md) 문서를 참조하여 커넥터와 규칙이 어떻게 상호 작용하여 헤드리스 콘텐츠를 번역하는지 학습합니다.

## 추가 리소스 {#additional-resources}

[컨텐츠 번역 문서를 검토하여 헤드리스 로컬라이제이션 여정의 다음 부분으로 이동하는 것이 좋지만,](translate-content.md) 다음은 이 문서에서 언급된 일부 개념에 대해 자세히 설명하는 추가 선택적 리소스입니다. 하지만 헤드리스 여정을 계속 진행할 필요는 없습니다.
