...
아래 두 목록은 고객이 구성할 수 있는 사항을 설명하는 AEM as a Cloud Service OSGi 구성 면과 같습니다.
...
이러한 규칙은 Cloud Manager 빌드 프로세스 중에 검증됩니다. 시간이 지남에 따라 추가 규칙을 추가할 수 있으며 예상 적용 날짜가 표에 설명되어 있습니다. 고객은 대상 적용 날짜별로 이러한 규칙을 준수해야 합니다. 제거 날짜 이후에 규칙을 준수하지 않으면 Cloud Manager 빌드 프로세스에서 오류가 생성됩니다. Maven 프로젝트에는 로컬 SDK 개발 중에 OSGI 구성 오류를 플래그를 지정하려면 [AEM as a Cloud Service SDK Build Analyzer Maven 플러그인](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/build-analyzer-maven-plugin.html)을 포함해야 합니다.

OSGI 구성에 대한 추가 정보는 [이 위치](/help/implementing/deploying/configuring-osgi.md)에 있습니다.
...
* **`org.apache.felix.eventadmin.impl.EventAdmin`** (발표 날짜: 4/30/2021, 적용 날짜: 7/31/2021)
   ...
   * `org.apache.felix.eventadmin.IgnoreTimeout`
      * 필수
      * 유형: 문자열 배열
      * 필수 범위: `org.apache.felix*`, `org.apache.sling*`, `come.day*`, `com.adobe*` 중 적어도 하나를 포함해야 합니다.
