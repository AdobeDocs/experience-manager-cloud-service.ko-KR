...
[AEM 버전](/help/implementing/deploying/aem-version-updates.md) 업데이트는 항상 [사용자 정의 코드](#customer-releases) 푸시와는 별도의 배포 이벤트입니다. 다른 방법으로 볼 때, 사용자 지정 코드 릴리스는 맨 위에 배포되기 때문에 프로덕션에 있는 AEM 버전에 대해 테스트해야 합니다. 이후 발생하는 AEM 버전 업데이트로, 빈번하며 자동으로 적용됩니다. 이미 배포된 사용자 정의 코드와 역으로 호환하기 위해 업데이트됩니다.
...
기존 비 클라우드 AEM 버전과 마찬가지로 특정 빠른 시작을 기반으로 하는 로컬 오프라인 개발이 지원되며 대부분의 경우 디버깅을 위한 선택 도구가 될 것으로 예상됩니다.
...
내부 릴리스에 대한 사용자 지정 코드를 개발하려면 [AEM AS A CLOUD SERVICE SDK](/help/implementing/developing/introduction/aem-as-a-cloud-service-sdk.md) 을 다운로드하여 설치해야 합니다. AEM as a Cloud Service Dispatcher 도구로 AEM을 사용하는 방법에 대한 자세한 내용은 [이 페이지](/help/implementing/dispatcher/disp-overview.md)를 참조하십시오.
...
변경 불가능한 저장소에서 지속되는 모든 콘텐츠는 Git에 체크인하고 Cloud Manager를 통해 배포해야 합니다. 즉, 현재 AEM 솔루션과 달리 코드는 실행 중인 AEM 인스턴스에 직접 배포하지 않습니다. 이렇게 하면 클라우드 환경의 특정 릴리스에 대해 실행 중인 코드가 동일하므로 프로덕션에서 의도하지 않게 코드 변형이 발생할 위험이 제거됩니다. 예를 들어 OSGI 구성은 런타임 시 AEM 웹 콘솔의 구성 관리자를 통해 관리하는 대신 소스 제어에 커밋해야 합니다.

배포 패턴으로 인한 애플리케이션 변경 사항은 스위치에 의해 활성화되므로 서비스 사용자, 해당 ACL, 노드 유형 및 색인 정의 변경 사항을 제외하고는 변경 가능한 저장소의 변경 사항에 따라 달라질 수 없습니다.
...
일부 경우에 환경을 업데이트할 때마다 Cloud Manager에서 배포할 수 있도록 소스 제어에서 콘텐츠 변경을 준비하는 데 유용할 수 있습니다. 예를 들어 특정 루트 폴더 구조를 시드하거나 편집 가능한 템플릿의 변경 사항을 정렬하여 애플리케이션에서 업데이트된 구성 요소의 정책을 활성화하는 것이 합리적일 수 있습니다.

Cloud Manager에서 변경 가능한 저장소에 배포하는 콘텐츠, 변경 가능한 콘텐츠 패키지 및 repoinit 문을 설명하는 두 가지 전략이 있습니다.

### 변경 가능한 콘텐츠 패키지 {#mutable-content-packages}

일반적으로 폴더 경로 계층, 서비스 사용자와 액세스 제어(ACL) 등 콘텐츠는 Maven Archetype 기반 AEM 프로젝트에 커밋합니다. 기법에는 AEM에서 내보내기 또는 직접 XML로 작성하기가 포함됩니다. 빌드 및 배포 프로세스 도중 Cloud Manager는 최종 변경 가능한 콘텐츠 패키지를 패키지화합니다. 파이프라인의 배포 단계에서 변경 가능한 콘텐츠를 3가지 다른 시간대에 설치합니다.
...
* 다른 모든 콘텐츠는 jackrabbit vault를 통해 정의할 수 있음. 예:
   * 폴더 (추가, 수정, 제거)
   * 편집 가능한 템플릿 (추가, 수정, 제거)
...
위에서 언급했듯이 기존 코드 베이스를 보유한 고객은 [AEM 6.5 설명서](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/restructuring/repository-restructuring.html)에 설명된 6.5 저장소 변경에 필요한 저장소 재구성 연습을 준수해야 합니다.
...
* Repoinit는 시작 시 리소스를 생성하므로 논리에 의해 해당 리소스가 있다는 사실이 당연시될 수 있습니다. 변경 가능한 콘텐츠 패키지 방식에서는 시작 후 리소스가 생성되므로 리소스를 사용하는 애플리케이션 코드가 실패할 수 있습니다.
* Repoinit는 수행할 액션을 명시적으로 제어하므로 비교적 안전한 지침 세트입니다. 또한 사용자, 서비스 사용자 및 그룹을 제거할 수 있는 몇 가지 보안 관련 사례를 제외하고 유일하게 지원되는 추가 작업입니다. 반면 변경 가능한 콘텐츠 패키지 접근 방식에서는 명시적으로 무언가를 제거합니다. 필터를 정의하면 필터에 의해 가려지는 모든 것이 삭제됩니다. 단, 새 콘텐츠의 존재 여부에 따라 애플리케이션 비헤이비어를 변경할 수 있는 시나리오가 모든 콘텐츠에 있으므로 주의해야 합니다.
* Repoinit는 빠르고 작은 단위의 작업을 수행합니다. 반면, 변경 가능한 콘텐츠 패키지의 성능은 필터에 포함된 구조에 따라 상당히 달라질 수 있습니다. 단일 노드를 업데이트하더라도 대형 트리의 스냅샷이 생성될 수 있습니다.
* OSGi 구성이 등록될 때 실행되므로 런타임 시 로컬 개발 환경에서 repoinit 문의 유효성을 검사할 수 있습니다.
...
>[!WARNING]
>
>ACL이 `/apps` 또는 `/libs` 아래의 노드에 정의된 경우 Repoinit 실행은 빈 저장소에서 시작됩니다. 패키지는 Repoinit가 시작된 후 설치되므로 명령문은 패키지에 정의된 항목을 신뢰할 수 없지만 아래의 상위 구조와 같은 전제 조건을 정의해야 합니다.

>[!TIP]
>
>ACL의 경우 심층 구조 생성이 번거로울 수 있으므로 더 높은 수준에서 ACL을 정의하고 rep:glob 제한을 통해 작업해야 하는 위치를 제한하는 것이 보다 합리적입니다.

Repoinit에 대한 자세한 내용은 [Sling 설명서](https://sling.apache.org/documentation/bundles/repository-initialization.html)에서 확인할 수 있습니다.
...
패키지 관리자는 런타임 개념입니다. 변경 불가능한 저장소에 콘텐츠나 코드를 설치할 수 없으므로 해당 콘텐츠 패키지는 변경 가능한 콘텐츠로만 구성되어야 합니다(주로 `/content` 또는 `/conf`). 콘텐츠 패키지에 혼합 콘텐츠(변경 가능한 콘텐츠 및 변경 불가능한 콘텐츠 모두)가 포함된 경우 변경 가능한 콘텐츠만 설치됩니다.
...
Cloud Manager를 통해 설치된 콘텐츠 패키지(변경 가능과 변경 불가능 모두)는 AEM Package Manager의 사용자 인터페이스에서 중단 상태로 표시됩니다. 해당 패키지는 다시 설치하고 다시 빌드하거나 다운로드할 수도 없으며 **“cp2fm”** 접미사로 나열되어 설치가 Cloud Manager에서 관리되었음을 나타냅니다.

### 서드파티 패키지 포함 {#including-third-party}

일반적으로 고객은 Adobe의 번역 파트너와 같은 소프트웨어 공급업체 등 서드파티 소스에서 사전 빌드된 패키지를 포함합니다. 원격 저장소에서 해당 패키지를 호스팅하고 `pom.xml`에서 참조하는 것이 좋습니다. [암호로 보호된 Maven 저장소](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/setting-up-project.md#password-protected-maven-repositories)의 설명에 따라 암호 보호가 설정된 공용 저장소와 개인 저장소의 경우에도 가능합니다.

패키지를 원격 저장소에 저장할 수 없는 경우 고객은 프로젝트의 일부로 SCM에 커밋하고 이에 종속된 항목에서 참조하는 로컬 파일 시스템 기반 Maven 저장소에 배치할 수 있습니다. 저장소는 아래 그림과 같이 프로젝트 pom에서 선언될 수 있습니다.
...
포함된 서드파티 패키지는 이 문서에 설명된 AEM as a Cloud Service 서비스 코딩 및 패키징 지침을 준수해야 합니다. 그렇지 않으면 해당 패키지가 포함되어 결국 배포가 실패할 수 있습니다.

다음 Maven `POM.xml` 조각은 서드파티 패키지가 **filevault-package-maven-plugin Maven** 플러그인 구성을 통해 프로젝트의 “컨테이너(일반적으로 **“모두”**&#x200B;라고 함)” 패키지에 임베드할 수 있는 방식을 보여 줍니다.
...
AEM 업데이트와 마찬가지로 고객 릴리스는 롤링 배포 전략을 사용하여 배포되므로 적절한 상황에서 작성자 클러스터 다운타임을 제거할 수 있습니다. 일반적인 이벤트 순서는 아래에 설명되어 있습니다. 여기서 고객 코드의 이전 버전 및 새 버전의 노드는 동일한 버전의 AEM 코드를 실행 중입니다.
...
* 이전 버전의 노드가 실행 중이고 서비스를 계속 제공하는 동시에 상태 검사를 통해 새 버전의 노드는 준비 상태를 확인합니다.
...
신규 또는 수정된 색인으로 인해 새 버전이 트래픽을 처리하기 전에 인덱싱 단계 또는 리인덱싱 단계가 추가될 수 있습니다. AEM as a Cloud Service의 색인 관리에 대한 자세한 내용은 [이 문서](/help/operations/indexing.md)에서 확인할 수 있습니다. Cloud Manager 빌드 페이지에서 인덱싱 작업의 상태를 확인할 수 있으며 새 버전이 트래픽을 가져올 수 있게 되면 알림을 받게 됩니다.

>[!NOTE]
>
>새 버전은 신규 색인이 생성될 때까지 트래픽을 허용할 수 없으므로 롤링 배포에 필요한 시간은 색인 크기에 따라 달라집니다.

현재 AEM as a Cloud Service는 ACS Commons Ensure Oak 인덱스 도구 등 색인 관리 도구와 구동하지 않습니다.
...
Cloud ready AEM 빠른 시작을 사용하여 복제를 개발하고 테스트하려면 작성자/게시 설정과 함께 클래식 복제 기능을 사용해야 합니다. 클라우드에 대한 AEM 작성자의 UI 진입점이 제거된 경우 사용자는 `http://localhost:4502/etc/replication`으로 이동하여 구성합니다.
...
콘텐츠나 코드에 액세스하는 데 필요한 서비스 사용자나 ACL을 변경할 때 이전 AEM 버전에 오류가 발생하여 이전 서비스 사용자가 해당 콘텐츠나 코드에 액세스할 수 있습니다. 이 비헤이비어를 해결하려면 후속 릴리스에서 정리하기 전에 브리지 역할을 하는 첫 번째 릴리스로 변경 사항이 최소 두 개의 릴리스에 분산되어야 합니다.
...
배포 후 오류가 보고되거나 감지되면 이전 버전으로 롤백해야 할 수 있습니다. 새 구조(변경 가능한 콘텐츠)는 롤백되지 않으므로 새 코드가 새 버전에서 생성된 새 구조와 호환되는지 확인하는 것이 좋습니다. 이전 코드가 호환되지 않는 경우 후속 고객 릴리스에 수정 사항이 적용되어야 합니다.
...
**도구 > 작업** 화면은 클라우드 환경에서 더 이상 사용할 수 없으므로 소스 제어에서 유지 관리 작업 구성을 지속해야 합니다. 변경 사항이 반응적으로 적용되고 잊혀지기 보다는 의도적으로 지속될 수 있다는 이점이 있습니다. 자세한 내용은 [유지 관리 작업 문서](/help/operations/maintenance.md)를 참조하십시오.
