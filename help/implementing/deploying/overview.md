...
코드 개발의 기본 사항은 AEM On Premise 및 Managed Services 솔루션과 비교하여 Cloud Service 로서 AEM에 있습니다. 개발자가 코드를 작성하고 로컬로 테스트한 다음 원격 AEM에 Cloud Service 환경으로 푸시됩니다. Managed Services용 선택적 콘텐츠 전달 도구인 Cloud Manager가 필요합니다. 이제 Cloud Service 환경으로 AEM에 코드를 배포하는 유일한 메커니즘입니다.

[AEM 버전](/help/implementing/deploying/aem-version-updates.md)의 업데이트는 항상 [사용자 지정 코드](#customer-releases)를 푸시하지 않고 별도의 배포 이벤트입니다. 다른 방식으로 보면, 사용자 지정 코드 릴리스는 맨 위에 배포되기 때문에 프로덕션 상태에 있는 AEM 버전에 대해 테스트해야 합니다. 이후 발생하는 AEM 버전 업데이트로, 자주 업데이트되며 자동으로 적용됩니다. 이미 배포된 고객 코드와 역호환되도록 하기 위한 것입니다.
...
내부 릴리스에 대한 사용자 지정 코드를 개발하려면 관련 버전의 [AEM as a Cloud Service SDK](/help/implementing/developing/introduction/aem-as-a-cloud-service-sdk.md)를 다운로드하여 설치해야 합니다. AEM을 Cloud Service Dispatcher 도구로 사용하는 방법에 대한 자세한 내용은 [이 페이지](/help/implementing/dispatcher/disp-overview.md)를 참조하십시오.

다음 비디오에서는 코드를 AEM as a Cloud Service에 배포하는 방법에 대한 높은 수준의 개요를 제공합니다.
...
AEM as a Cloud Service 애플리케이션용으로 작성된 컨텐츠 패키지는 변경할 수 없는 컨텐츠와 변경할 수 있는 컨텐츠 간에 완전히 구분되어 있어야 하며 Cloud Manager에서는 변경할 수 있는 컨텐츠만 설치하고 다음과 같은 메시지도 출력합니다.
...
이러한 코드 패키지에 대해 일부 추가 제한 사항이 적용됩니다. 예를 들어 [설치 후크](http://jackrabbit.apache.org/filevault/installhooks.html)는 지원되지 않습니다.
...
포함된 모든 타사 패키지는 Cloud Service 서비스 호환으로서 AEM의 유효성을 검사해야 하며, 그렇지 않으면 배포 오류가 발생합니다.

위에서 언급했듯이 기존 코드 베이스를 사용하는 고객은 [AEM 6.5 설명서에 설명된 6.5 저장소 변경 사항으로 필요한 저장소 구조 변경 연습을 준수해야 합니다.](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/restructuring/repository-restructuring.html)

## 포인트 {#repoinit}

다음 경우 OSGI 공장 구성에서 명시적 컨텐츠 생성 `repoinit` 문을 수동으로 코딩하는 방법을 사용하는 것이 좋습니다.
...
1. 프로젝트의 구성 폴더에 공장 PID `org.apache.sling.jcr.repoinit.RepositoryInitializer`에 대한 OSGi 구성을 추가합니다. **org.apache.sling.jcr.repointeit.RepositoryInitializer~initstructure**&#x200B;와 같은 구성에 대해 수사적 이름을 사용합니다.
1. 구성의 스크립트 속성에 repoint 문을 추가합니다. 구문 및 옵션은 [Sling 설명서](https://sling.apache.org/documentation/bundles/repository-initialization.html)에 설명되어 있습니다. 하위 폴더 앞에 상위 폴더를 명시적으로 만들어야 합니다. 예를 들어 `/content/myfolder` 앞에 `/content`을 명시적으로 만들면 `/content/myfolder/mysubfolder` 앞에 낮은 수준 구조에서 설정되는 ACL의 경우 더 높은 수준에서 설정하고 `rep:glob` 제한을 사용하는 것이 좋습니다.  예 `(allow jcr:read on /apps restriction(rep:glob,/msm/wcm/rolloutconfigs))`.
...
>[!WARNING]
>
>`/apps` 또는 `/libs` 아래의 노드에 대해 정의된 ACL의 경우 리디렉션 실행이 빈 저장소에서 시작됩니다. 패키지는 응답 후 설치되므로 명령문은 패키지에 정의된 어떤 것도 사용할 수 없지만 아래의 상위 구조와 같은 전제 조건을 정의해야 합니다.
...
컨텐츠 패키지를 &quot;일회성&quot;으로 설치해야 하는 사용 사례가 있습니다. 예를 들어 프로덕션 문제를 디버깅하기 위해 프로덕션에서 스테이징으로 특정 콘텐츠를 가져오는 경우가 있습니다. 이러한 시나리오의 경우 패키지 관리자를 AEM에서 Cloud Service 환경으로 사용할 수 있습니다.

패키지 관리자는 런타임 개념이므로 컨텐츠나 코드를 변경할 수 없는 저장소에 설치할 수 없으므로 이러한 컨텐츠 패키지는 변경할 수 있는 컨텐츠(주로 `/content` 또는 `/conf`)로만 구성되어야 합니다. 콘텐츠 패키지에 혼합(변경할 수 있는 콘텐츠와 변경할 수 없는 콘텐츠 모두)인 콘텐츠가 포함되어 있으면 가변 콘텐츠만 설치됩니다.

Cloud Manager를 통해 설치된 모든 컨텐츠 패키지(변경할 수 있고 변경할 수 없음)는 AEM Package Manager의 사용자 인터페이스에 고정된 상태로 표시됩니다. 이러한 패키지를 다시 설치하거나 다시 빌드하거나 다운로드할 수 없으며 **&quot;cp2fm&quot;** 접미사가 표시되어 해당 설치가 Cloud Manager에서 관리되었음을 나타냅니다.
...
포함된 타사 패키지는 이 문서에 설명된 Cloud Service 서비스 코딩 및 패키징 지침으로서 AEM을 준수해야 합니다. 그렇지 않으면 포함 시 배포 오류가 발생합니다.

다음 Maven POM XML 코드 조각은 **filevault-package-maven-plugin** Maven 플러그인 구성을 통해 일반적으로 **&#39;all&#39;**&#x200B;이라고 하는 프로젝트의 &quot;Container&quot; 패키지에 타사 패키지를 임베드하는 방법을 보여줍니다.
...
새 색인이나 수정된 색인은 새로운 (녹색) 버전에서 트래픽을 발생시키기 전에 추가적인 색인 지정 또는 재인덱싱 단계를 생성합니다. AEM as a Cloud Service의 색인 관리에 대한 자세한 내용은 [이 문서](/help/operations/indexing.md)에 나와 있습니다. Cloud Manager 빌드 페이지에서 색인 작업의 상태를 확인할 수 있으며 새 버전에서 트래픽을 받을 준비가 되면 알림을 받게 됩니다.

>[!NOTE]
>
>롤링 배포에 필요한 시간은 새 색인이 생성될 때까지 녹색 버전에서 트래픽을 허용할 수 없으므로 인덱스 크기에 따라 달라집니다.

현재 AEM as a Cloud Service은 ACS Commons Secure Oak Index 도구와 같은 색인 관리 도구에서 작동하지 않습니다.

## 복제 {#replication}

게시 메커니즘은 [AEM Replication Java API](https://helpx.adobe.com/experience-manager/6-3/sites/developing/using/reference-materials/diff-previous/changes/com.day.cq.replication.Replicator.html)와 이전 버전과 호환됩니다.
...
기존 AEM 솔루션에서는 고객이 임의 실행 모드로 인스턴스를 실행하고 OSGI 구성을 적용하거나 특정 인스턴스에 OSGI 번들을 설치할 수 있습니다. 일반적으로 정의된 실행 모드에는 *서비스*(작성 및 게시)와 환경(개발, 스테이지, prod)이 포함됩니다.

반면 AEM은 사용 가능한 실행 모드와 OSGI 번들 및 OSGI 구성을 매핑하는 방법에 대해 더 적합합니다.

* OSGI 구성 실행 모드는 개발, 스테이지, 환경을 참조하거나, 작성자를 참조하고, 서비스에 대해 게시해야 합니다. `<service>.<environment_type>` 조합이 지원되지만 이 조합을 이 특정 순서(예: `author.dev` 또는 `publish.prod`)에서 사용해야 합니다. OSGI 토큰은 런타임 시 더 이상 `environment_type`을 포함하지 않는 `getRunModes` 메서드를 사용하지 않고 코드에서 직접 참조해야 합니다. 자세한 내용은 [AEM as a Cloud Service](/help/implementing/deploying/configuring-osgi.md)에 대한 OSGi 구성 을 참조하십시오.
* OSGI 번들 실행 모드는 서비스(작성자, 게시)로 제한됩니다. 실행 당 모드 OSGI 번들은 `install/author` 또는 `install/publish` 아래의 컨텐츠 패키지에 설치해야 합니다.
...
* **구성** (*기본값은 모든 AEM Services에 적용*)
* **config.author** (*모든 AEM 작성자 서비스에 적용*)
* **config.author.dev** (*AEM Dev Author 서비스에 적용*)
...
**도구 > 작업** 화면은 클라우드 환경에서 더 이상 사용할 수 없으므로 유지 관리 작업 구성은 소스 제어에서 유지되어야 합니다. 따라서 변경 사항이 적극적으로 적용되거나 잊혀지지 않고 의도적으로 유지되도록 하는 이점이 있습니다. 자세한 내용은 [유지 관리 작업 문서](/help/operations/maintenance.md)를 참조하십시오.
