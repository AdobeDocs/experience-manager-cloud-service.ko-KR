...
파란색-녹색 배포 패턴으로 인한 애플리케이션 변경 사항은 스위치에 의해 활성화되므로 서비스 사용자, 해당 ACL, 노드 유형 및 색인 정의 변경 사항을 제외하고는 변경 가능한 저장소의 변경 사항에 따라 달라질 수 없습니다.
...
* 다른 모든 콘텐츠는 jackrabbit vault를 통해 정의할 수 있음. 예:
   ...
   * 스크립트 (패키지는 패키지 설치의 여러 설치 프로세스 단계에서 설치 후크를 트리거할 수 있습니다. <!-- MISDIRECTED REQUEST, 421 ERROR, CAN'T FIND CORRECT PATH See the [Jackrabbit filevault documentation](https://jackrabbit.incubator.apache.org/filevault/installhooks.html) about install hooks. --> AEM CS는 현재 Filevault 버전 3.4.0을 사용합니다. 이 버전은 설치 후크를 관리 사용자, 시스템 사용자 및 관리자 그룹의 멤버로 제한)).
...
* Repoinit는 수행할 액션을 명시적으로 제어하므로 비교적 안전한 지침 세트입니다. 또한 사용자, 서비스 사용자 및 그룹을 제거할 수 있는 몇 가지 보안 관련 사례를 제외하고 유일하게 지원되는 추가 작업입니다. 반면에 필터를 정의하면 필터에 포함된 모든 항목이 삭제되므로 변경 가능한 콘텐츠 패키지 방식으로 항목을 명시적으로 제거할 수 있습니다. 단, 새 콘텐츠의 존재 여부에 따라 애플리케이션 비헤이비어를 변경할 수 있는 시나리오가 모든 콘텐츠에 있으므로 주의해야 합니다.
...
AEM 업데이트와 마찬가지로 적절한 상황에서 작성 클러스터 중단 시간을 없애려면 롤링 배치 전략을 사용하여 고객 릴리스를 배포합니다. 일반적인 이벤트 순서는 아래에 설명되어 있습니다. 여기서 **파란색**&#x200B;은 고객 코드의 이전 버전이고 **녹색**&#x200B;은 새 버전입니다. 파란색과 녹색은 모두 동일한 버전의 AEM 코드를 실행 중입니다.

* 파란색 버전은 활성 상태이고 녹색의 릴리스 후보는 빌드되고 사용 가능함
* 신규 또는 업데이트된 색인 정의가 있는 경우 해당 색인이 처리됩니다. 파란색 배포는 항상 이전 색인을 사용하고 녹색 배포는 항상 신규 색인을 사용합니다.
* 녹색이 시작되면 동시에 파란색이 계속 제공됨
* 파란색이 실행 중이고 제공되는 동시에 상태 검사를 통해 녹색 준비 단계 확인 중
* 준비가 된 녹색 노드로 트래픽 허용 및 중단된 파란색 노드 바꾸기
* 시간이 지나면서 파란색 노드는 녹색 노드만 남을 때까지 녹색 노드로 바뀌고 배포가 완료됨
* 신규 또는 수정된 가변 콘텐츠가 배포됨

## 색인 {#indexes}

신규 또는 수정된 색인으로 인해 새 (녹색) 버전이 트래픽을 처리하기 전에 인덱싱 단계 또는 리인덱싱 단계가 추가될 수 있습니다. AEM as a Cloud Service의 색인 관리에 대한 자세한 내용은 [이 문서](/help/operations/indexing.md)에서 확인할 수 있습니다. Cloud Manager 빌드 페이지에서 인덱싱 작업의 상태를 확인할 수 있으며 새 버전이 트래픽을 가져올 수 있게 되면 알림을 받게 됩니다.

>[!NOTE]
>
>녹색 버전은 신규 색인이 생성될 때까지 트래픽을 허용할 수 없으므로 롤링 배포에 필요한 시간은 색인 크기에 따라 달라집니다.
...
콘텐츠나 코드에 액세스하는 데 필요한 서비스 사용자나 ACL을 변경할 때 이전 AEM 버전에 오류가 발생하여 이전 서비스 사용자가 해당 콘텐츠나 코드에 액세스할 수 있습니다. 이 비헤이비어를 해결하려면 후속 릴리스에서 정리하기 전에 브리지 역할을 하는 첫 번째 릴리스로 변경 사항이 2개 이상의 릴리스에 분산되어야 합니다.

### 색인 변경 {#index-changes}

색인이 변경되면 파란색 버전은 종료될 때까지 색인을 계속 사용하고 녹색 버전은 자체 수정된 색인 세트를 사용해야 합니다. 개발자는 [이 문서](/help/operations/indexing.md)에 설명된 색인 관리 기법을 따라야 합니다.
