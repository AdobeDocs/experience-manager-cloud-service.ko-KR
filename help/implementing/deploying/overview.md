...
AEM 버전 [업데이트는](/help/implementing/deploying/aem-version-updates.md) 사용자 지정 코드 [푸시와는 항상 별도의 배포 이벤트입니다](#customer-releases). 다른 방법으로 볼 때 사용자 지정 코드 릴리스는 제작 중인 AEM 버전에 대해 테스트되어야 합니다. 이 버전은 최상위 버전에 배포되기 때문입니다. AEM 버전 업데이트 이후 발생하는 업데이트로 자주 업데이트되며 자동으로 적용됩니다. 이미 배포된 고객 코드와 역호환됩니다.
