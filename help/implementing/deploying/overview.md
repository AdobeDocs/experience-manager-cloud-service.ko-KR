...
배포 패턴으로 인한 애플리케이션 변경은 스위치에 의해 활성화되므로 서비스 사용자, 해당 ACL, 노드 유형 및 색인 정의 변경을 제외하고 변경 가능한 저장소의 변경 사항에 따라 달라질 수 없습니다.
...
AEM 업데이트와 마찬가지로 적절한 상황에서 작성 클러스터 중단 시간을 없애려면 롤링 배치 전략을 사용하여 고객 릴리스를 배포합니다. 이전 버전과 새 버전의 고객 코드가 모두 있는 노드에서 동일한 버전의 AEM 코드를 실행하는 일반적인 이벤트 시퀀스가 아래에 설명되어 있습니다.

* 이전 버전의 노드가 활성 상태이고 새 버전에 대한 릴리스 후보가 작성되어 사용할 수 있게 됩니다.
* 신규 또는 업데이트된 색인 정의가 있는 경우 해당 색인이 처리됩니다. 이전 버전의 노드는 항상 이전 색인을 사용하는 반면 새 버전의 노드는 항상 새 색인을 사용합니다.
* 이전 버전이 여전히 트래픽을 제공하는 동안 새 버전이 있는 노드가 시작됩니다.
* 이전 버전의 노드가 실행 중이며 새 버전의 노드가 상태 검사를 통해 준비 여부를 확인하는 동안 계속 제공됩니다.
* 준비가 완료된 새 버전이 있는 노드는 트래픽을 수락하고 노드가 이전 버전으로 대체되며, 이전 버전은 다운됩니다.
* 시간이 지남에 따라 새 버전이 있는 노드만 남을 때까지 이전 버전이 있는 노드가 새 버전이 있는 노드로 교체되어 배포가 완료됩니다.
* 그런 다음 변경 가능한 새로운 콘텐츠 또는 수정된 콘텐츠가 배포됩니다.

## 색인 {#indexes}

새 색인이나 수정된 색인으로 인해 추가 색인화 또는 색인 재지정 단계가 수행되어 새 버전이 트래픽을 초래할 수 있습니다. AEM as a Cloud Service의 색인 관리에 대한 자세한 내용은 [이 문서](/help/operations/indexing.md)에서 확인할 수 있습니다. Cloud Manager 빌드 페이지에서 인덱싱 작업의 상태를 확인할 수 있으며 새 버전이 트래픽을 가져올 수 있게 되면 알림을 받게 됩니다.

>[!NOTE]
>
>새 버전은 새 색인이 생성될 때까지 트래픽을 허용할 수 없으므로 롤링 배포에 필요한 시간은 색인 크기에 따라 달라집니다.
...
콘텐츠나 코드에 액세스하는 데 필요한 서비스 사용자나 ACL을 변경할 때 이전 AEM 버전에 오류가 발생하여 이전 서비스 사용자가 해당 콘텐츠나 코드에 액세스할 수 있습니다. 이 비헤이비어를 해결하기 위해 최소 두 개의 릴리스에 걸쳐 변경 작업을 수행하는 것이 좋습니다. 첫 번째 릴리스는 후속 릴리스에서 정리하기 전에 브리지 역할을 합니다.

### 색인 변경 {#index-changes}

색인을 변경한 경우 새 버전은 종료될 때까지 해당 색인을 계속 사용하는 반면 이전 버전은 수정된 자체 색인 세트를 사용하는 것이 중요합니다. 개발자는 [이 문서](/help/operations/indexing.md)에 설명된 색인 관리 기법을 따라야 합니다.

### 롤백에 필요한 일반적인 코딩 {#conservative-coding-for-rollbacks}

배포 후 오류가 보고되거나 감지되면 이전 버전으로 롤백해야 할 수 있습니다. 새 구조(변경 가능한 콘텐츠 콘텐츠)가 롤백되지 않으므로 새 코드가 해당 새 버전에서 만든 새 구조와 호환되는지 확인하는 것이 좋습니다. 이전 코드가 호환되지 않는 경우 후속 고객 릴리스에 수정 사항이 적용되어야 합니다.
