...
AEM 버전의 업데이트는 사용자 지정 코드를 푸시하는 것과 항상 별개의 배포 이벤트입니다. 다른 방법으로 볼 때 사용자 지정 코드 릴리스는 제작 중인 AEM 버전에 대해 테스트되어야 합니다. 이 버전은 최상위 버전에 배포되기 때문입니다. 현재 Managed Services에 비해 자주 업데이트되는 AEM 버전 업데이트는 자동으로 적용됩니다. 이미 배포된 고객 코드와 역호환됩니다.
...
AEM은 자주 업데이트되며, 잠재적으로 하루에 한 번 정도 업데이트되며 버그 수정 및 성능 향상에 중점을 둘 것임을 이해하는 데 중요합니다. 업데이트는 중단 시간을 발생시키지 않고 투명하게 진행됩니다. 이 업데이트는 이전 버전과 호환되므로 사용자 지정 코드를 수정할 필요가 없습니다. 실제로 AEM 업데이트는 고객 코드 배포와 관련된 독립적인 이벤트입니다. AEM 업데이트는 마지막으로 성공한 코드 푸시 상단에 배포되므로 마지막 프로덕션에서 푸시된 모든 변경 사항이 배포되지 않습니다.
...
상태 검사는 응용 프로그램의 상태를 모니터링하는 데 사용됩니다. AEM에서 Cloud Service 업데이트으로 이러한 검사가 실패하는 경우 해당 환경에서 릴리스되지 않으며 Adobe에서 업데이트로 인해 예기치 않은 동작이 발생한 이유를 조사합니다.

### 복합 노드 저장소 {#composite-node-store}

위에 언급했듯이 대부분의 경우 업데이트로 인해 노드 클러스터인 작성자를 비롯하여 다운타임이 0으로 줄어듭니다. Oak의 &quot;복합 노드 스토어&quot; 기능으로 인해 롤링 업데이트가 가능합니다. 이 기능을 사용하면 AEM에서 여러 저장소를 동시에 참조할 수 있습니다. 순환 배포에서 새로운 Green AEM 버전은 고유한(TarMK 기반 변경 불가능한 저장소) `/libs` 를 포함하고 있으며, 이전 Blue AEM 버전과 구별됩니다. 두 버전은 변경 가능, 기타 등의 영역을 포함하는 공유 DocumentMK 기반 저장소 `/content`를 참조하고 `/conf``/etc` 있습니다. 파란색과 녹색 둘 다 자체 버전이 있으므로 롤링 업데이트 동안 둘 다 활성 상태일 수 `/libs`있으며, 파란색이 완전히 녹색으로 바뀔 때까지 둘 다 트래픽을 수행할 수 있습니다.

## 고객 릴리스 {#customer-releases}

### 올바른 AEM 버전에 맞게 코딩 {#coding-against-the-right-aem-version}

이전 AEM 솔루션의 경우, 최신 AEM 버전은 가끔(약 분기별 서비스 팩이 있는 경우 대략 매년) 변경되었으며, 고객은 API Jar를 참조하여 프로덕션 인스턴스를 최신 빠른 시점으로 업데이트합니다. 그러나 Cloud Service 애플리케이션으로 AEM은 최신 버전의 AEM으로 자동 업데이트되므로 최신 AEM 인터페이스에 대해 내부 릴리스에 대한 사용자 정의 코드를 구축해야 합니다.
...
내부 릴리스에 대한 사용자 정의 코드를 개발하려면 Cloud Service SDK로서 [AEM의 관련 버전을](/help/implementing/developing/introduction/aem-as-a-cloud-service-sdk.md) 다운로드하여 설치해야 합니다. AEM을 Cloud Service 발송자 도구로 사용하는 방법에 대한 자세한 내용은 [이 페이지를 참조하십시오](/help/implementing/dispatcher/overview.md).
...
Cloud ready AEM Quickstart를 사용한 복제를 개발하여 테스트하려면 Author/Publish 설정과 함께 클래식 복제 기능을 사용해야 합니다. 클라우드에 대해 AEM 작성자의 UI 시작 지점이 제거된 경우 사용자가 구성을 위해 이동하게 `http://localhost:4502/etc/replication` 됩니다.
