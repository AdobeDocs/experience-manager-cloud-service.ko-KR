...
## 유니버설 편집기를 열 콘텐츠 경로 또는 `sling:resourceType`과(와) 방법을 정의합니다. (선택 사항) {#content-paths}

[페이지 편집기](/help/sites-cloud/authoring/page-editor/introduction.md)를 사용하는 기존 AEM 프로젝트가 있는 경우, 콘텐츠 작성자가 페이지를 편집하면 해당 페이지가 페이지 편집기로 자동 열립니다. 콘텐츠 경로 또는 `sling:resourceType`에 따라 AEM에서 열어야 하는 편집기와 여는 방법을 정의할 수 있으므로 선택한 콘텐츠에 필요한 편집기에 관계없이 작성자에게 원활한 경험을 제공합니다.

1. 구성 관리자를 엽니다.

   `http://<host>:<port>/system/console/configMgr`

1. 목록에서 **`Universal Editor URL Service`**&#x200B;을(를) 찾은 다음 **구성 값 편집**&#x200B;을 클릭합니다.

1. 범용 편집기를 여는 콘텐츠 경로 또는 `sling:resourceType`을 정의합니다.

   * **`mappings`** 필드에서 유니버설 편집기를 열 경로를 제공합니다.
   * **`resourcetypes`** 필드에서 유니버설 편집기에서 직접 연 리소스 목록을 제공합니다.

1. 편집기를 여는 방법을 정의합니다.

   * **`aemdomain`** 옵션을 선택하여 `https://author-pXX-eYYY.adobeaemcloud.com...`과(와) 같은 AEM 도메인에서 편집기를 엽니다. 그렇지 않으면 `https://experience.adobe.com...`에서 열립니다.
   * **`editorreleasepreview`**&#x200B;미리 보기 환경에서 자동으로 유니버설 편집기를 열려면 [ 옵션을 선택하십시오. ](/help/release-notes/universal-editor/preview.md) 그렇지 않으면 현재 릴리스 환경에서 열립니다.
