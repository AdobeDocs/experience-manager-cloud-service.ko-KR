...
1. **CDN에 도메인 매핑** 대화 상자에서 다음 CDN 유형 중 하나를 선택합니다.

   * **Adobe 관리 CDN(권장)** - 이 구성에 Adobe 관리 CDN이 사용됩니다. 여기에는 자동 설정 및 관리, 기본 제공 보안 기능이 포함됩니다.
   * **기타 CDN 공급자** - 이 구성에 자체 관리되는 CDN 공급자 네트워크가 사용됩니다.

1. 이전 단계에서 선택한 CDN 유형에 따라 다음을 수행합니다.

   * **Adobe 관리 CDN**

     ![Adobe 관리 CDN 라디오 단추가 선택된 상태에서 CDN에 도메인 매핑 대화 상자](/help/implementing/cloud-manager/assets/cdn/map-domain-to-cdn-adobe-managed.png)

      1. **원본** 드롭다운 목록에서 다음 중 하나를 선택합니다.
      ...
      1. **도메인** 드롭다운 목록에서 사용할 도메인 이름을 선택합니다.<br>드롭다운 목록에 사용 가능한 확인된 도메인이 없습니까? [사용자 정의 도메인 이름 추가](/help/implementing/cloud-manager/custom-domain-names/add-custom-domain-name.md)를 참조하십시오.

      1. **SSL 인증서** 드롭다운 목록에서 사용할 인증서를 선택합니다.<br>드롭다운 목록에서 사용할 수 있는 SSL 인증서가 없습니까? [SSL 인증서 추가](/help/implementing/cloud-manager/managing-ssl-certifications/add-ssl-certificate.md)를 참조하십시오.

      1. **저장**&#x200B;을 클릭합니다.

   * **다른 CDN 공급자**

     ![Adobe 관리 CDN 라디오 단추가 선택된 상태에서 CDN에 도메인 매핑 대화 상자](/help/implementing/cloud-manager/assets/cdn/map-domain-to-cdn-other-provider.png)

     나열된 구성 단계를 사용하여 CDN에 필요한 설정을 적용하고 매핑을 확인합니다. [사용자 지정 도메인 이름 추가](/help/implementing/cloud-manager/custom-domain-names/add-custom-domain-name.md)도 참조하세요.

      1. **내 CDN을 구성했습니다**&#x200B;를 클릭합니다.
...
1. Adobe에서는 도메인 매핑을 테스트할 것을 권장합니다.
