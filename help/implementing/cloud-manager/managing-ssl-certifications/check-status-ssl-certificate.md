...
* **주황색**
인증서가 60일 이내에 만료될 예정임을 나타냅니다. 사이트 액세스나 정전이 발생하지 않도록 Cloud Manager UI를 통해 인증서를 갱신하고 대체할 계획이 있는지 확인해야 합니다. Cloud Manager는 UI에 알림 메시지를 전송하여 인증서 만료 시간이 임박했음을 알려줍니다.

* **빨간색**
여러 알림을 보내더라도 SSL 인증서가 만료되었음을 나타냅니다.