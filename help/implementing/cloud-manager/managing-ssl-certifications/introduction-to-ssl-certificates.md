...
* 여러 환경에서 OV/EV 인증서를 사용할 수 있습니다.
   * 즉, 한 번 추가할 수 있지만 여러 번 사용됩니다.
...
자체 EV/OV 인증서를 업로드하려면 다음 요구 사항을 충족해야 합니다.

* AEM as a Cloud Service은 OV(조직 유효성 검사) 또는 EV(확장 유효성 검사) 정책을 준수하는 인증서를 수락합니다.
   * Cloud Manager에서는 자체 DV(도메인 유효성 검사) 인증서 업로드를 지원하지 않습니다.
