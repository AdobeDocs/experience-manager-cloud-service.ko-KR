...
즉시 사용 가능한 여러 AEM Oak 색인에는 tika 구성과 이러한 인덱스 **반드시**&#x200B;에 tika 구성을 포함해야 합니다. 이 규칙은 `damAssetLucene`, `lucene` 및 `graphqlConfig` 의 사용자 지정을 확인하고 `tika` 중 하나가 있으면 문제를 발생시킵니다  노드가 없거나 `tika` 노드에 이름이 `config.xml`인 하위 노드가 있는 경우

인덱스 정의 사용자 지정에 대한 자세한 내용은 [색인 지정 설명서](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/operations/indexing.html?lang=en#preparing-the-new-index-definition)를 참조하십시오.
...
lucene 유형의 Oak 인덱스  항상 비동기식으로 인덱싱해야 합니다. 이렇게 하지 않으면 시스템이 불안정해질 수 있습니다. lucene 인덱스 구조에 대한 자세한 내용은 [Oak 설명서](https://jackrabbit.apache.org/oak/docs/query/lucene.html#index-definition)에 있습니다.
...
AEM Assets에서 자산 검색이 올바르게 작동하려면 `damAssetLucene` Oak 인덱스의 사용자 지정은 이 인덱스에 해당하는 지침 세트를 따라야 합니다. 이 규칙은 인덱스 정의에 `visualSimilaritySearch` 값을 포함하는 `tags` 이라는 다중 값 속성이 있어야 함을 확인합니다.
...
**다음** 이후: 버전 2021.2.0(2021.8.0에서 유형 및 심각도가 변경됨)
