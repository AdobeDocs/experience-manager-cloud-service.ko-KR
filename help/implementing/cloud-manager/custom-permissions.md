...
>[!NOTE]
>
>Cloud Manager에 대한 프로필을 만들고 권한을 관리하려면 Admin Console for Adobe Experience Manager as a Cloud Service에 제품 관리자 권한이 있어야 합니다.
...
1. Cloud Manager의 권한과 사용자를 관리할 수 있는 Admin Console의 **제품** 탭으로 리디렉션됩니다. Admin Console에서 **새 프로필** 단추를 선택합니다.
...
1. Admin Console에서 사용자 지정 권한을 할당한 [새 제품 프로필의 이름을 선택합니다](#assign-permissions).
...
| 권한 | 설명 |
| --- | --- |
| 프로그램 제작 | 사용자가 프로그램을 만들 수 있도록 허용합니다. |
| 프로그램 액세스 | 사용자가 프로그램에 액세스할 수 있도록 허용합니다. |
| 프로그램 편집 | 사용자가 프로그램을 편집할 수 있도록 허용합니다. |
| 환경 만들기 | 사용자가 환경을 만들 수 있도록 허용합니다. |
| 환경 편집 | 사용자가 환경을 업데이트하고 편집할 수 있도록 허용합니다. |
| 환경 로그 읽기 | 사용자가 환경 로그를 읽을 수 있도록 허용합니다. |
| 환경 변수 관리 | 사용자가 환경 구성을 생성/편집/삭제할 수 있도록 허용합니다. |
| 환경 복원 만들기 | 사용자가 환경 복원을 만들 수 있도록 허용합니다. |
| 신속한 개발 환경 재설정 | 사용자가 RDE(신속한 개발 환경)를 재설정할 수 있도록 해줍니다. |
| 콘텐츠 복사 관리 | 사용자가 콘텐츠 복사 작업을 관리할 수 있도록 허용합니다. |
| 파이프라인 만들기 | 사용자가 파이프라인을 만들 수 있도록 허용합니다. |
| 파이프라인 삭제 | 사용자가 파이프라인을 삭제할 수 있도록 허용합니다. |
| 파이프라인 편집 | 사용자가 파이프라인을 편집할 수 있도록 허용합니다. |
| 프로덕션 배포 승인/거부 | 사용자가 프로덕션 배포 단계를 승인하거나 거부할 수 있습니다. |
| 파이프라인 실행 취소 | 사용자가 파이프라인 실행을 취소할 수 있습니다. |
| 파이프라인 실행 시작 | 사용자가 새 파이프라인 실행을 시작할 수 있도록 허용합니다. |
| 중요한 지표 오류 재정의/거부 | 사용자가 중요한 지표 오류를 재정의/거부할 수 있습니다. |
| 프로덕션 배포 예약 | 사용자가 프로덕션 배포 단계를 예약하도록 허용합니다. |
| 저장소 정보 액세스 | 사용자가 저장소 정보에 액세스하고 액세스 암호를 생성할 수 있도록 허용합니다. |
| 저장소 생성 | 사용자가 Git 저장소를 만들 수 있도록 허용합니다. |
| 저장소 삭제 | 사용자가 Git 저장소를 삭제할 수 있도록 허용합니다. |
| 저장소 편집 | 사용자가 Git 저장소를 편집할 수 있도록 허용합니다. |
| 저장소 코드 생성 | 사용자가 Archetype에서 프로젝트를 생성할 수 있도록 허용합니다. |
| 도메인 이름 관리 | 사용자가 도메인 이름을 생성/편집/삭제할 수 있도록 허용합니다. |
| IP 허용 목록 관리 | 사용자가 IP 허용 목록 및 IP 허용 목록 바인딩을 편집/삭제할 수 있도록 허용합니다. |
| 네트워크 인프라 관리 | 사용자가 네트워크 인프라를 생성/편집/삭제할 수 있도록 허용합니다. |
| SSL 인증서 관리 | 사용자가 SSL 인증서를 생성/편집/삭제할 수 있도록 허용합니다. |
| New Relic 하위 계정 사용자 관리 | 사용자가 New Relic 하위 계정 사용자를 읽고 편집할 수 있도록 허용합니다. |
...
* Cloud Manager에서 만든 프로그램, 환경 및 파이프라인과 같은 리소스는 권한 구성을 위해 Admin Console에 표시되는 데 2분이 걸릴 수 있습니다.
