...
1. [제품 프로필에 사용자 지정 권한을 할당합니다.](#assign-permissions)
1. [사용자를 제품 프로필에 할당합니다.](#assign-users)

이 섹션에서는 이러한 단계를 자세히 설명합니다. 다음을 확인하는 것이 유용할 수 있습니다. [용어](#terms) 및 [구성 가능한 권한](#configurable-permissions) 섹션을 통해 나만의 사용자 지정 권한을 만들 수 있습니다.

>[!NOTE]
>
>Adobe Experience Manager as a Cloud Service에 대한 프로필을 만들고 권한을 관리하려면 Cloud Manager Admin Console에 제품 관리자 권한이 있어야 합니다.
...
1. Cloud Manager 랜딩 페이지에서 **액세스 관리** 단추를 클릭합니다.
...
1. 더하기(`+`) 아이콘을 클릭하여 열에 추가할 수 있습니다 **포함된 권한 항목**.

   * 다음 항목 선택 `i` 에 대해 자세히 알아보려면 권한 항목 옆에 있는 아이콘을 클릭합니다.

1. 다음 항목 선택 **모두 추가** 단추(맨 위) **사용 가능한 권한** 모든 권한을 추가할 수 있도록 열을 표시합니다.
...
1. Admin Console에서 [사용자 지정 권한을 할당한 새 제품 프로필입니다.](#assign-permissions)
...
| 용어 | 설명 |
|---|---|
| 미리 정의된 권한 | 다음과 같이 사전 정의된 역할 **비즈니스 소유자** 및 **배포 관리자** 를 사용하여 Cloud Manager의 다양한 기능을 관리할 수 있습니다. 사전 정의된 역할에 대한 자세한 내용은 [AEM as a Cloud Service 팀 및 제품 프로필.](/help/onboarding/aem-cs-team-product-profiles.md) |
| 사용자 정의 권한 | Cloud Manager 기능을 사용하면 권한 프로필을 만들어 Cloud Manager의 지원되는 기능을 제어하는 역할을 정의할 수 있습니다 |
| 제품 프로필 | 권한 프로필의 일부인 사용자에게 적용할 수 있는 구성 가능한 권한을 관리하기 위해 Admin Console에 작성됨 |
...
이 경우 사용자에게 환경 또는 파이프라인에 액세스할 수 없는 경우 **프로그램 액세스** 환경 또는 파이프라인이 포함된 권한.
