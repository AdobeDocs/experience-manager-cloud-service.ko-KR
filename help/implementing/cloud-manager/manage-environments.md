...
1. 기다려 주십시오. Cloud Manager는 계속해서 지역을 복구하고 있으며 언제든지 사용할 수 있습니다.
1. 몇 시간이 지나도 문제가 지속되면 추가 게시 지역을 제거하고 다시 추가(동일 지역 또는 다른 지역)하여 전체 배포를 트리거할 수 있습니다.

추가 작업을 수행하기 전에 시스템이 자체적으로 복구될 때까지 대기하는 시간은 해당 지역의 장애가 시스템에 미치는 영향에 따라 다릅니다.

어떠한 경우에도 [트래픽은 언제든 가장 가까운 온라인 지역으로 라우팅되어야 합니다.](/help/operations/additional-publish-regions.md) 문제가 지속되면 Adobe 고객 지원 센터에 문의하십시오.
