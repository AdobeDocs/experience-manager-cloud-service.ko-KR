...
1. 다음에서 **[내 프로그램](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/editing-programs.md#my-programs)** 환경을 추가할 프로그램을 화면, 탭 또는 클릭합니다.

1. **[프로그램 개요](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/editing-programs.md#program-overview)** 페이지에서 **환경** 카드의 **환경 추가**&#x200B;를 클릭하여 환경을 추가합니다.
...
1. 드롭다운 아래에 선택한 지역을 추가하여 옵션을 표시합니다.
   * 다음 항목 선택 `X` 선택한 영역 옆에 있으므로 선택을 취소할 수 있습니다.
1. **추가 게시 지역** 드롭다운에서 다른 지역을 선택하여 다른 지역을 추가합니다.
1. 선택 **저장** 환경을 만들 준비가 되면
...
>[!NOTE]
> 2024년 현재, 개발 인스턴스 및 일부 샌드박스 프로그램은 이미 자동으로 업데이트되므로 업데이트를 수동으로 관리할 필요가 없습니다. 이 전환으로 인해 에서는 개발 인스턴스에 대해 환경을 수동으로 업데이트 하는 옵션을 사용할 수 없습니다. _일부_ 을 참조하십시오.

다음 **업데이트** 옵션은 다음 위치에서 사용할 수 있습니다. **환경** 환경의 줄임표 버튼을 클릭하여 샌드박스 프로그램의 일부 개발 환경 및 환경에 대한 카드입니다.
...
* 적절한 파이프라인이 존재하지 않으면 **업데이트** 옵션을 선택하면 하나를 만들라는 메시지가 표시됩니다.
...
1. IP 허용 목록 허용 목록에 추가하다 테이블 상단에 있는 입력 필드를 사용하여 IP 라이브러리를 선택하고 적용할 작성자 또는 게시 서비스를 선택할 수 있습니다.
