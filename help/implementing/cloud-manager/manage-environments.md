...
1. **환경 추가** 대화 상자가 나타나면 다음 작업을 수행하십시오.
   ...
   * 환경 **이름**&#x200B;을 제공합니다.
   * 환경 **설명**&#x200B;을 제공합니다.
   * 을(를) 추가하는 경우 **프로덕션 + 스테이징** 환경에서는 프로덕션 환경과 스테이징 환경 모두에 환경 이름과 설명을 제공해야 합니다.
...
을(를) 가진 사용자 **비즈니스 소유자** 역할은 기본 영역 외에 최대 3개의 추가 게시 영역을 포함하도록 프로덕션 및 스테이징 환경을 구성할 수 있습니다. 추가 게시 영역은 가용성을 향상시킬 수 있습니다. 다음을 참조하십시오. [추가 게시 영역 설명서](/help/operations/additional-publish-regions.md) 을 참조하십시오.

>[!TIP]
>
>다음을 사용할 수 있습니다. [Cloud Manager API](https://developer.adobe.com/experience-cloud/cloud-manager/guides/api-usage/creating-programs-and-environments/#creating-aem-cloud-service-environments) 사용 가능한 영역의 현재 목록을 쿼리합니다.

### 새 환경에 여러 게시 영역 추가 {#add-regions}

새 환경을 추가할 때 기본 영역 외에 추가 영역을 구성하도록 선택할 수 있습니다.

1. 다음 항목 선택 **기본 영역**.
   * 환경 생성 후에는 변경할 수 없습니다.
1. 옵션 선택 **추가 게시 영역 추가** 및 새 항목 **추가 게시 영역** 드롭다운이 나타납니다.
1. 다음에서 **추가 게시 영역** 드롭다운에서 추가 영역을 선택합니다.
1. 선택한 영역이 드롭다운 아래에 추가되어 해당 선택을 나타냅니다.
   * 선택한 영역 옆에 있는 X를 탭하거나 클릭하여 선택을 해제합니다.
1. 에서 다른 지역 선택 **추가 게시 영역** 드롭다운을 클릭하여 다른 지역을 추가합니다.
1. 탭 또는 클릭 **저장** 환경을 만들 준비가 되면

![여러 영역 선택](assets/select-multiple-regions.png)

선택한 영역은 프로덕션 환경과 스테이징 환경 모두에 적용됩니다.

추가 지역을 지정하지 않은 경우 [환경이 생성된 후 나중에 그렇게 할 수 있습니다.](#edit-regions)

프로비전하려는 경우 [고급 네트워킹](/help/security/configuring-advanced-networking.md) 프로그램의 경우 Cloud Manager API를 사용하여 환경에 게시 영역을 추가하기 전에 이 작업을 수행하는 것이 좋습니다. 그렇지 않으면 추가 게시 영역의 트래픽이 기본 영역의 프록시를 통해 이동합니다.

### 여러 게시 영역 편집 {#edit-regions}

처음에 추가 영역을 지정하지 않은 경우 필요한 권한이 있으면 환경이 생성된 후에 지정할 수 있습니다.

추가 게시 영역을 제거할 수도 있습니다. 그러나 한 트랜잭션에서 영역만 추가하거나 제거할 수 있습니다. 한 영역을 추가하고 한 영역을 제거해야 하는 경우 먼저 추가하고 변경 사항을 저장한 다음 제거합니다(또는 그 반대).

1. 프로그램의 프로그램 개요 콘솔에서 프로덕션 환경의 줄임표 버튼을 클릭하고 을 선택합니다 **편집** 메뉴에서 삭제할 수 있습니다.

   ![환경 편집](assets/select-edit-environment.png)

1. 다음에서 **프로덕션 환경 편집** 대화 상자에서 추가 게시 영역에 필요한 사항을 변경합니다.
   * 사용 **추가 게시 영역** 드롭다운을 클릭하여 추가 영역을 선택합니다.
   * 선택한 추가 게시 영역 옆에 있는 X 를 클릭하여 선택을 해제합니다.

   ![환경 편집](assets/edit-environment.png)

1. 탭 또는 클릭 **저장** 변경 내용을 저장합니다.

프로덕션 환경에 대한 변경 사항은 프로덕션 및 스테이징 환경 모두에 적용됩니다. 여러 게시 영역에 대한 변경 내용은 프로덕션 환경에서만 편집할 수 있습니다.

프로비전하려는 경우 [고급 네트워킹](/help/security/configuring-advanced-networking.md) 프로그램의 경우 환경에 게시 영역을 추가하기 전에 이 작업을 수행하는 것이 좋습니다. 그렇지 않으면 추가 게시 영역의 트래픽이 기본 영역의 프록시를 통해 이동합니다.
