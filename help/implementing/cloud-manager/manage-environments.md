...
프로그램에 [고급 네트워킹](/help/security/configuring-advanced-networking.md)을 프로비전하려면 Cloud Manager API를 사용하여 환경에 게시 영역을 추가하기 전에 이 프로비전을 수행하는 것이 좋습니다. 그렇지 않은 경우, 추가 게시 지역의 트래픽이 기본 지역의 프록시로 이동합니다.
...
프로그램에 [고급 네트워킹](/help/security/configuring-advanced-networking.md)을 프로비전하려면 환경에 게시 영역을 추가하기 전에 이 프로비전을 수행하는 것이 좋습니다. 그렇지 않은 경우, 추가 게시 지역의 트래픽이 기본 지역의 프록시로 이동합니다.
...
## 이전에 배포된 코드 복원 {#restore-previous-code-deployed}
...
브라우저에서 새 탭이 열리고 **Developer Console**&#x200B;에 대한 로그인 페이지가 표시됩니다.
