...
* **신속한 개발** - 신속한 개발 환경(RDE)을 통해 개발자는 변경 사항을 신속하게 배포하고 검토할 수 있으므로 로컬 개발 환경에서 작동하는 것으로 입증된 기능을 테스트하는 데 필요한 시간을 최소화할 수 있습니다. 다음을 참조하십시오 [신속한 개발 환경 설명서](/help/implementing/developing/introduction/rapid-development-environments.md) RDE 사용 방법에 대한 자세한 내용.
...
1. 다음에서 **프로그램 개요** 페이지, 클릭 **환경 추가** 다음에 있음 **환경** 카드를 사용하여 환경을 추가할 수 있습니다.
...
1. **환경 추가** 대화 상자가 나타나면 다음 작업을 수행하십시오.
   ...
   * 환경 **이름**&#x200B;을 제공합니다.
   * 환경 **설명**&#x200B;을 제공합니다.
   * 을(를) 추가하는 경우 **프로덕션 + 스테이징** 환경은 프로덕션 환경과 스테이징 환경 모두에 환경 이름과 설명을 제공해야 합니다.
   * 드롭다운에서 **기본 지역**&#x200B;을 선택합니다.
      * 기본 영역은 생성 후 변경할 수 없습니다.
      * 사용 가능한 권한에 따라 다음을 구성할 수 있습니다. [여러 지역](#multiple-regions).
...
을(를) 가진 사용자 **비즈니스 소유자** 역할은 기본 영역 외에 최대 3개의 추가 게시 영역을 포함하도록 프로덕션 및 스테이징 환경을 구성할 수 있습니다. 추가 게시 지역은 가용성을 높일 수 있습니다. 자세한 내용은 [추가 게시 지역 설명서](/help/operations/additional-publish-regions.md)를 참조하십시오.
...
환경을 추가할 때 기본 영역 외에 추가 영역을 구성하도록 선택할 수 있습니다.

1. **기본 지역**&#x200B;을 선택합니다.
   * 환경을 만든 후에는 기본 영역을 변경할 수 없습니다.
1. 옵션 선택 **추가 게시 영역 추가** 및 새 항목 **추가 게시 영역** 옵션 드롭다운이 나타납니다.
...
선택한 영역은 프로덕션 환경과 스테이징 환경 모두에 적용됩니다.

추가 지역을 지정하지 않으면 [환경을 만든 후 나중에 지정할 수 있습니다.](#edit-regions)

프로비전하려는 경우 [고급 네트워킹](/help/security/configuring-advanced-networking.md) 프로그램의 경우 Cloud Manager API를 사용하여 환경에 추가 게시 영역을 추가하기 전에 이 프로비저닝이 수행되는 것이 좋습니다. 그렇지 않으면 추가 게시 영역의 트래픽이 기본 영역의 프록시를 통해 이동합니다.

### 여러 게시 지역 편집 {#edit-regions}

처음에 추가 지역을 지정하지 않으면 환경을 만들고 필요한 권한이 있는 경우 지정할 수 있습니다.

추가 게시 지역을 제거할 수도 있습니다. 단, 한 번의 트랜잭션에서만 지역을 추가 또는 제거할 수 있습니다. 한 영역을 추가하고 한 영역을 제거해야 하는 경우 먼저 추가하고 변경 사항을 저장한 다음 제거합니다(또는 반대로).
...
프로덕션 환경에 대한 변경 사항은 프로덕션 및 스테이징 환경 모두에 적용됩니다. 여러 게시 지역에 대한 변경 사항은 프로덕션 환경에서만 편집할 수 있습니다.

프로비전하려는 경우 [고급 네트워킹](/help/security/configuring-advanced-networking.md) 프로그램의 경우 환경에 게시 영역을 추가하기 전에 이 프로비저닝이 수행되는 것이 좋습니다. 그렇지 않으면 추가 게시 영역의 트래픽이 기본 영역의 프록시를 통해 이동합니다.

## 환경 세부 정보 {#viewing-environment}

다음을 사용할 수 있습니다. **환경** 두 가지 방법으로 환경의 세부 사항에 액세스할 수 있는 개요 페이지의 카드입니다.

1. 다음에서 **개요** 페이지를 클릭하고 **환경** 화면 상단의 탭입니다.

   ![환경 탭](assets/environments-tab2.png)

   * 또는 **모두 표시** 단추 **환경** 로 바로 이동할 카드 **환경** 탭.
...
1. 세부 정보를 표시할 수 있도록 목록에서 환경을 클릭합니다.
...
>[!NOTE]
>
>**환경** 카드에는 세 가지 환경만 나열됩니다. 클릭 **모두 표시** 앞에서 설명한 대로 프로그램의 모든 환경을 확인합니다.

### 미리보기 서비스 액세스 {#access-preview-service}

Cloud Manager는 각 AEM as a Cloud Service 환경에 미리보기 서비스(추가 게시 서비스로 제공)를 제공합니다.

서비스를 사용하면 웹 사이트의 최종 경험이 실제 게시 환경에 도달하고 공개적으로 사용할 수 있게 되기 전에 이를 미리 볼 수 있습니다.

생성 시 미리보기 서비스에는 레이블이 지정된 기본 IP 허용 목록이 적용됩니다 `Preview Default [<envId>]`: 미리보기 서비스에 대한 모든 트래픽을 차단합니다. 미리보기 서비스에서 기본 IP 허용 목록을 적용 취소하여 액세스를 활성화할 수 있습니다.
...
활성화되면 AEM 내 게시 관리 UI를 사용하여 미리보기 서비스에 콘텐츠를 게시할 수 있습니다. 다음을 참조하십시오 [컨텐츠 미리 보기](/help/sites-cloud/authoring/fundamentals/previewing-content.md) 을 참조하십시오.

>[!NOTE]
>
>환경이 AEM 버전 `2021.05.5368.20210529T101701Z` 이상이어야 미리보기 서비스를 사용할 수 있습니다. 미리보기 서비스를 사용할 수 있도록 업데이트 파이프라인이 환경에서 성공적으로 실행되었는지 확인하십시오.
...
Cloud Manager는 파이프라인과 함께 마지막으로 배포된 버전보다 최신 버전의 AEM이 있음을 감지하면 다음을 표시합니다. **업데이트 사용 가능** 환경에 대한 상태입니다.
...
다음 **업데이트** 옵션은 다음 위치에서 사용할 수 있습니다. **환경** 환경의 줄임표 버튼을 클릭하여 표시되는 샌드박스 프로그램의 개발 환경 및 환경에 대한 카드

![환경 카드의 업데이트 옵션](assets/environ-update2.png)

이 옵션은 다음을 클릭하여 사용할 수도 있습니다. **환경** 프로그램의 탭을 클릭한 다음 환경의 줄임표 버튼을 선택합니다.
...
삭제 옵션은 프로그램 **개요** 창의 **환경** 탭에서도 사용할 수 있습니다. 환경의 줄임표 버튼을 클릭하고 을(를) 선택합니다 **삭제**.
...
>[!TIP]
>
>다음을 참조하십시오 [AEM as a Cloud Service 팀 및 제품 프로필](/help/onboarding/aem-cs-team-product-profiles.md) AEM as a Cloud Service 팀 및 제품 프로필로 라이선스가 부여된 Adobe 솔루션에 대한 액세스 권한을 부여하고 제한하는 방법에 대해 알아보려면
...
다음을 참조하십시오 [샌드박스 환경 최대 절전 모드 설정 및 해제](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/using-cloud-manager/programs/introduction-sandbox-programs.html#hibernation) 을 참조하십시오.

이 옵션은 개별 환경의 줄임표 메뉴를 클릭할 때 표시되는 **개요** 창의 **환경** 탭에서도 사용할 수 있습니다.

## 로컬 로그인 {#login-locally}

선택 **로컬 로그인** 을 클릭합니다. **환경** Adobe Experience Manager에 로컬로 로그온할 수 있도록 카드를 만듭니다.

![로컬 로그인](assets/environ-login-locally.png)

또한 **환경** 의 탭 **개요** 페이지를 가리키도록 업데이트하는 중입니다.
...
사용자 정의 도메인 이름을 구성하려면 **환경** 환경 세부 정보를 보려면 탭을 클릭하고 환경을 클릭합니다.
...
IP 허용 목록을 관리하려면 **환경** 의 탭 **개요** 프로그램의 페이지입니다. 세부 정보를 관리할 수 있도록 개별 환경을 클릭합니다.

### IP 허용 목록 적용 {#apply-ip-allow-list}

IP 허용 목록을 적용하면 허용 목록에 추가하다 정의에 포함된 모든 IP 범위가 환경의 작성자 또는 게시 서비스와 연결됩니다. 의 사용자 **비즈니스 소유자** 또는 **배포 관리자** ip 허용 목록을 적용하려면 역할을에 로그인해야 합니다.

IP 허용 목록을 환경에 적용하려면 Cloud Manager에 있어야 합니다. Cloud Manager의 IP 허용 목록에 대한 자세한 내용은 [Cloud Manager의 IP 허용 목록 소개](/help/implementing/cloud-manager/ip-allow-lists/introduction.md).

**IP 허용 목록을 적용하려면**

1. 프로그램 **개요** 화면의 **환경** 탭에서 특정 환경으로 이동하고 **IP 허용 목록** 표로 이동합니다.
1. IP 허용 목록 허용 목록에 추가하다 테이블 상단에 있는 입력 필드를 사용하여 IP 라이브러리를 선택하고 적용하려는 작성자 또는 게시 서비스를 선택할 수 있습니다.
1. **적용**&#x200B;을 클릭하여 제출을 확인합니다.

### IP 허용 목록 적용 취소 {#unapply-ip-allow-list}

IP 허용 목록허용 목록에 추가하다 를 적용 취소하면 작성자 또는 게시자의 서비스 정의에 포함된 모든 IP 범위가 제거됩니다. 의 사용자 **비즈니스 소유자** 또는 **배포 관리자** ip 허용 목록에 추가하다를 적용 취소하려면 역할이 로그인해야 합니다.

**IP 허용 목록을 적용 취소하려면**

1. 프로그램 **개요** 화면의 **환경** 탭에서 특정 환경으로 이동하고 **IP 허용 목록** 표로 이동합니다.
1. 목록에서 제거할 IP 허용 목록 규칙이 나열된 행을 식별합니다.
