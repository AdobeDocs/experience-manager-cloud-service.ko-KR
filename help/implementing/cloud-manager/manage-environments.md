...
필수 권한을 가진 사용자는 다음 환경 유형을 만들 수 있습니다(특정 테넌트에 사용할 수 있는 영역의 경계 내에서).

* **프로덕션 및 스테이지 환경**:프로덕션 및 스테이지는 두 제품으로 제공되며 테스트 및 제작 용도로 사용됩니다.
...
* **개발**:개발 환경은 개발 및 테스트 목적으로 만들 수 있으며 비프로덕션 파이프라인에만 연결됩니다.
   ...
   | 이름 | 작성 계층 | 게시 계층 | 사용자가 만들 수 있음 | 사용자가 | 환경과 연결할 수 있는 파이프라인 |
   |--- |--- |--- |--- |---|---|
   | 프로덕션 | 예 | 예 포함된 사이트 | 예 | 아니오 | 프로덕션 파이프라인 |
   | 단계 | 예 | 예 포함된 사이트 | 예 | 아니오 | 프로덕션 파이프라인 |
   | 개발 | 예 | 예 포함된 사이트 | 예 | 예 | 비프로덕션 파이프라인 |

   >[!NOTE]
   >프로덕션 및 스테이지는 두 제품으로 제공되며 테스트 및 제작 용도로 사용됩니다.  사용자는 스테이지 또는 프로덕션 환경만 만들 수 없습니다.
...
1. 환경을 추가하려면 **환경 추가**&#x200B;를 클릭합니다. 이 단추는 **환경** 화면에서 액세스할 수 있습니다.
   ...
   프로그램에 제로 환경이 있는 경우 **환경 추가** 옵션도 **환경** 카드에서 사용할 수 있습니다.

   ![](assets/no-environments.png)

   >[!NOTE]
   >**환경 추가** 옵션은 권한이 없거나 계약을 체결할 수 있는 항목에 따라 비활성화됩니다.

1. **환경 추가** 대화 상자가 나타납니다.사용자는 **환경 유형** 및 **환경 이름** 및 **환경 설명**(특정 테넌트에 사용할 수 있는 항목 범위 내에서 환경을 만드는 사용자의 목적에 따라) 등의 세부 정보를 제출해야 합니다.
...
1. 채워진 기준이 있는 환경을 추가하려면 **저장**&#x200B;을 클릭합니다.  이제 *개요* 화면에 파이프라인을 설정할 수 있는 카드가 표시됩니다.
   ...
   >[!NOTE]
   >아직 비프로덕션 파이프라인을 설정하지 않은 경우 *개요* 화면에 비프로덕션 파이프라인을 만들 수 있는 카드가 표시됩니다.
...
1. 환경 세부 사항을 보려면 목록에서 환경 중 하나를 선택합니다.
...
개발 환경에 대한 업데이트는 프로그램 사용자가 관리합니다. 환경에서 공개적으로 사용 가능한 최신 AEM 릴리스를 실행하고 있지 않으면 홈 화면의 환경 카드의 상태에 **UPDATE AVAILABLE**&#x200B;이 표시됩니다.

![](assets/environ-update.png)


**업데이트** 옵션은 **환경** 카드에서 사용할 수 있습니다.
이 옵션은 **환경** 카드에서 **세부 사항**&#x200B;을 클릭하는 경우에도 사용할 수 있습니다. **환경** 페이지가 열리고 개발 환경을 선택하면 **를 클릭합니다...**&#x200B;을 선택하고 아래 그림과 같이 **업데이트**&#x200B;를 선택합니다.
...
**환경** 카드에서 **세부 사항**&#x200B;을 클릭하면 삭제 옵션도 사용할 수 있습니다. **환경** 페이지가 열리고 개발 환경을 선택하면 **를 클릭합니다...**&#x200B;을 선택하고 아래 그림과 같이 **삭제**&#x200B;를 선택합니다.
...
**환경** 카드의 드롭다운 메뉴에서 **개발자 콘솔**&#x200B;을 선택합니다. 그러면 브라우저에서 **개발자 콘솔**&#x200B;에 대한 로그인 페이지가 있는 새 탭이 열립니다.

개발자 역할의 사용자만 **개발자 콘솔**&#x200B;에 액세스할 수 있습니다. 클라우드 관리자 샌드박스 프로그램에 대한 액세스 권한이 있는 모든 사용자가 **개발자 콘솔**&#x200B;에 액세스할 수 있는 샌드박스 프로그램에 대한 예외입니다.
...
이 옵션은 **환경** 카드에서 **세부 사항**&#x200B;을 클릭하는 경우에도 사용할 수 있습니다. **환경** 페이지가 열리고 환경을 선택하면 **를 클릭합니다...**&#x200B;을 선택하고 **개발자 콘솔**&#x200B;을 선택합니다.

## 로컬로 로그인 {#login-locally}

Adobe Experience Manager에 로컬로 로그인하려면 **환경** 카드의 드롭다운 메뉴에서 **로컬 로그인**&#x200B;을 선택합니다.
...
아래 설명에 따라 환경에 대한 게시 서비스에서 다음 작업을 수행할 수 있습니다.
...
1. [사용자 지정 도메인 이름 또는 ](/help/implementing/cloud-manager/custom-domain-names/check-domain-name-status.md#pre-existing-cdn) SSL 인증서 [의 상태를 확인합니다](/help/implementing/cloud-manager/managing-ssl-certifications/check-status-ssl-certificate.md#pre-existing-cdn).
...
환경 요약 페이지에서 환경 세부 사항 페이지로 이동합니다. 게시 및/또는 작성 서비스에서 사용자 환경에 대해 다음 작업을 수행할 수 있습니다.
...
IP 허용 목록 적용 취소는 허용 목록 정의에 포함된 모든 IP 범위가 환경의 작성자 또는 게시자 서비스와 연결되지 않는 프로세스입니다. IP 허용 목록을 적용 취소하려면 비즈니스 소유자 또는 배포 관리자 역할의 사용자가 로그인해야 합니다.

IP 허용 목록 적용을 취소하려면 아래 절차를 따르십시오.
