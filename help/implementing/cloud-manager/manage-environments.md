...
1. 드롭다운 아래에 선택한 지역을 추가하여 옵션을 표시합니다.
   * 선택한 지역 옆의 `X`를 탭하거나 클릭하여 지역 선택을 해제합니다.
...
>[!NOTE]
>
>환경이 AEM 버전 `2021.05.5368.20210529T101701Z` 이상이어야 미리보기 서비스를 사용할 수 있습니다. 미리보기 서비스를 사용할 수 있도록 사용자 환경에서 업데이트 파이프라인이 실행되었는지 확인하십시오.

### 추가 게시 영역 상태 {#additional-region-status}

추가 게시 영역을 활성화한 경우 다음에서 이러한 영역의 상태를 확인할 수 있습니다. **환경** 카드.

1. 다음에서 **개요** 페이지를 찾은 다음 **환경** 카드.

1. 다음에서 **환경** 카드, **상태** 열은 구성된 추가 게시 영역에 문제가 있는 경우 반영합니다. 다음을 클릭합니다. **정보** 지역 세부 정보 아이콘.

   ![환경 카드의 추가 게시 영역 상태 정보](assets/additional-publish-region-status-environments-card.png)

또는 다음에서 동일한 정보에 액세스할 수 있습니다. **환경** 탭.

1. 다음에서 **개요** 페이지에서 **환경** 탭.

1. 다음에서 **환경** 탭에서 왼쪽 탐색 패널에서 쿼리할 환경을 선택합니다.

1. 환경이 선택되면 다음 작업을 수행합니다.

   * 다음 **환경 정보** 이 표에서는 선택한 환경에 대해 구성된 영역을 보여 줍니다.
   * 다음 **상태** 열 **환경 세그먼트** 구성된 추가 게시 영역에 문제가 있는 경우 표가 반영됩니다. 문제에 대한 세부 사항을 보려면 상태 위로 마우스를 가져갑니다.

   ![환경 탭의 추가 게시 영역 상태 정보](assets/additional-publish-region-status-environments-tab.png)

추가 게시 영역과 함께 보고된 문제가 있는 경우:

1. 조금만 기다려 주세요. Cloud Manager는 계속해서 지역을 복구하려고 시도하며 언제든지 사용할 수 있습니다.
1. 몇 시간 후에도 문제가 지속되면 추가 게시 영역을 제거하고 다시 추가하여(동일한 영역 또는 다른 영역) 전체 배포를 트리거할 수 있습니다.

추가 조치를 취하기 전에 시스템이 자체적으로 복구될 때까지 기다리는 시간은 해당 지역의 장애가 시스템에 미치는 영향에 따라 다릅니다.
...
단, 샌드박스 프로그램의 환경과 개발 환경의 업데이트도 프로그램 내에서 관리됩니다. 이러한 환경에서 공개적으로 사용 가능한 최신 AEM 버전을 실행하지 않는 경우, 프로그램의 **개요** 화면에 있는 **환경** 카드의 상태에 **업데이트 사용 가능**&#x200B;이 표시됩니다.
...
환경에 IP 허용 목록을 적용하려면 Cloud Manager에 IP 허용 목록이 있어야 합니다. Cloud Manager의 IP 허용 목록에 대한 자세한 내용은 [Cloud Manager의 IP 허용 목록 소개](/help/implementing/cloud-manager/ip-allow-lists/introduction.md)를 참조하십시오.

**IP 허용 목록을 적용하는 방법:**

1. 프로그램 **개요** 화면의 **환경** 탭에서 특정 환경으로 이동하고 **IP 허용 목록** 표로 이동합니다.
1. IP 허용 목록 표 상단의 입력 필드를 사용하여 IP 허용 목록 및 이를 적용할 작성자 또는 게시 서비스를 선택합니다.
1. **적용**&#x200B;을 클릭하여 제출을 확인합니다.

### IP 허용 목록의 적용 취소 {#unapply-ip-allow-list}

IP 허용 목록의 적용을 취소하면 환경의 작성자 또는 게시자 서비스에서 허용 목록 정의에 포함된 모든 IP 범위의 연결이 해제됩니다. IP 허용 목록의 적용을 취소하려면 **비즈니스 소유자** 또는 **배포 관리자** 역할이 있는 사용자가 로그인해야 합니다.

**IP 허용 목록의 적용을 취소하는 방법:**
