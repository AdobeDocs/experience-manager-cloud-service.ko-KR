...
AEM as a Cloud Service은 콘텐츠 요청을 계산하기 위해 서버측 수집 규칙을 적용합니다. 이러한 규칙은 인식된 AI/LLM 크롤러 및 사이트를 정기적으로 ping하는 모니터링 서비스 세트를 포함하여 잘 알려진 보트(예: 검색 엔진 크롤러)를 제외합니다. 이 제외 목록에 없는 기타 가상, 자동화된 또는 모니터링 유형 트래픽은 청구 가능한 콘텐츠 요청으로 계산됩니다.
...
| 잘 알려진 AI/LLM 크롤러(Fastly에 의해 태그됨) | 제외됨 | 잘 알려진 보트로 식별되는 인식된 AI/LLM 크롤러의 요청(예: `User-Agent` 또는 기타 보트 분류 신호). 이러한 요청은 청구할 수 없습니다.<br><br>AI 에이전트가 잘 알려진 봇으로 식별되지 않은 경우(예: 일반 브라우저 `User-Agent`을(를) 사용하는 경우) 해당 요청은 청구 가능한 콘텐츠 요청으로 카운트될 수 있습니다. |
...
* 사이트와 통합한 모든 사용자 정의 검색 크롤러에 대한 AEM 라이선스 비용과 콘텐츠의 신선도에 대한 요구 사항의 균형을 맞출 수 있습니다.  지나치게 공격적인 크롤러는 많은 콘텐츠 요청을 소비할 수 있습니다.
* 리디렉션을 클라이언트측(JavaScript 리디렉션을 사용하는 경우 상태 200)이 아닌 서버측(상태 301 또는 302)으로 처리하여 두 개의 별도 콘텐츠 요청을 방지합니다.
* 페이지를 렌더링하기 위해 로드될 수 있는 AEM의 JSON 응답인 API 호출을 결합하거나 줄입니다.
* 브라우저의 사용자 에이전트가 AEM에 올바르게 전달되었는지 확인합니다. 이렇게 하면 위에서 설명한 &quot;잘 알려진 검색 엔진&quot; 콘텐츠 요청 제외 규칙을 활용합니다.  특정 Headless 구현 또는 CDN 구성으로 인해 원래 사용자 에이전트가 손실되는 경우가 있습니다. 그렇게 되면 제외가 방지되고 사용자 에이전트가 전달된 경우보다 더 높은 콘텐츠 요청으로 이어질 수 있습니다.
