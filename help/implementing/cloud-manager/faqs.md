...
`sling-distribution-importer` 사용자는 `ui.content package` 패키지에 정의된 콘텐츠 경로에 대한 추가 권한이 필요합니다.  이는 일반적으로 `/conf` 및 `/var` 모두에 대한 권한을 추가해야 함을 의미합니다.
...
* 작성자에서 게시 인스턴스로 콘텐츠 패키지를 복제(Sling 배포)하는 동안 다른 오류로 인해 배포가 실패할 수 있습니다.
   * 다음 단계에 따라 로컬 설치에서 문제를 시뮬레이션하십시오.
