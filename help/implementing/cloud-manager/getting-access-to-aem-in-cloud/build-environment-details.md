...
Java 21 또는 Java 17로 빌드하기 위해 Cloud Manager은 이제 이러한 Java 버전과 호환되는 SonarQube 9.9를 사용합니다. 이 변경 사항은 Cloud Manager 릴리스 2025.1.0에 도입되었습니다. SonarQube를 업그레이드하기 위해 고객 작업이 필요하지 않습니다. 자세한 내용을 보고 변경 내용을 이해하려면 [Cloud Manager 2025.1.0용 릴리스 노트](/help/implementing/cloud-manager/release-notes/2025/2025-1-0.md)를 참조하십시오.
...
Java 21 런타임은 아래의 기준을 충족하는 AEM 릴리스 17098 이상의 환경인 모든 적격 환경에 적용되었습니다. 환경이 이 기준을 충족하지 못할 경우 성능, 가용성 및 보안을 보장하기 위해 조정하는 것이 중요합니다.

* **ASM의 최소 버전:**
`org.objectweb.asm` 아티팩트에 번들로 제공되는 Java 패키지 `org.ow2.asm.*`의 사용을 버전 9.5 이상으로 업데이트하여 최신 JVM 런타임 지원을 보장합니다.
...
* **런타임 매개 변수 편집:**
Java 21을 사용하여 로컬로 AEM을 실행하는 경우 `crx-quickstart/bin/start` 매개 변수로 인해 시작 스크립트(`crx-quickstart/bin/start.bat` 또는 `MaxPermSize`)가 실패합니다. 해결 방법으로 스크립트에서 `-XX:MaxPermSize=256M`을(를) 제거하거나 환경 변수 `CQ_JVM_OPTS`을(를) 정의하여 `-Xmx1024m -Djava.awt.headless=true`(으)로 설정합니다.

  이 문제는 AEM Cloud Service SDK 버전 19149 이상에서 해결되었습니다.

>[!IMPORTANT]
>
>환경이 아직 Java 21 런타임으로 자동으로 업데이트되지 않은 경우 Java 17 또는 21을 사용하여 환경을 빌드하여 트리거할 수 있습니다. 이 작업은 `.cloudmanager/java-version`을(를) `21` 또는 `17`(으)로 설정하여 수행됩니다. 질문이 있는 경우 [aemcs-java-adopter@adobe.com](mailto:aemcs-java-adopter@adobe.com)(으)로 Adobe에 문의하십시오.
...
최신 JVM 실행 시간을 지원하려면 `aemanalyser-maven-plugin`의 사용을 버전 1.6.16 이상으로 업데이트하십시오.
