...
| 빌드 및 단위 테스트 | 컨테이너화된 빌드 프로세스를 실행합니다.빌드 환경에 대한 자세한 내용은 <br>빌드 환경 세부 정보[를 참조하십시오.](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/build-environment-details.md) |
...
| 스테이지 테스트 단계 | 설명 |
| --- | --- |
| 제품 기능 테스트 | Cloud Manager 파이프라인은 스테이지 환경에 대해 실행되는 테스트를 실행합니다.<br>제품 기능 테스트[도 참조하세요.](/help/implementing/cloud-manager/functional-testing.md#product-functional-testing) |
| 사용자 정의 기능 테스트 | 파이프라인의 이 단계는 항상 실행되며 건너뛸 수 없습니다. 빌드가 테스트 JAR을 생성하지 않으면 테스트가 자동으로 통과됩니다.<br>사용자 지정 기능 테스트[도 참조하세요.](/help/implementing/cloud-manager/functional-testing.md#custom-functional-testing) |
