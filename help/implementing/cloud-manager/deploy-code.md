...
* **유효성 검사** - 이 단계에서는 파이프라인이 현재 사용 가능한 리소스를 사용하도록 구성되었는지 확인합니다. 예를 들어 구성된 분기가 존재하며 환경을 사용할 수 있는지 테스트합니다.
...
* **경험 감사** - 파이프라인의 이 단계는 항상 실행되며 건너뛸 수 없습니다. 프로덕션 파이프라인이 실행되면 검사를 실행할 사용자 정의 기능 테스트 후에 경험 감사 단계가 포함됩니다.
   ...
   * 이 인사이트는 현재 배포에 도입된 회귀가 있는지 확인하는 데 유용합니다.
...
* 프로덕션 배포 단계를 다시 실행하는 것은 마지막 실행에서만 가능합니다.
...
실행이 재실행인지 식별하기 위해 트리거 필드를 검사할 수 있습니다. 값은 입니다. *RE_EXECUTE*.
...
위의 HAL 링크의 _href_ 값 구문은 참조 지점으로 사용하기 위한 것이 아닙니다. 실제 값은 항상 HAL 링크에서 읽어야 하며 생성되지 않아야 합니다.

제출 *PUT* 이 끝점에 대한 요청으로 인해 *201* 성공한 경우 응답이며, 응답 본문은 새 실행을 나타냅니다. 이는 API를 통해 일반 실행을 시작하는 것과 유사합니다.
