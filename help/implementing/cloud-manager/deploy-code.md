...
* **유효성 검사** - 이 단계에서는 파이프라인이 현재 사용 가능한 리소스를 사용하도록 구성되었는지 확인합니다. 예를 들어 구성된 분기가 있고 환경을 사용할 수 있는지 테스트합니다.
...
* **경험 감사** - 파이프라인의 이 단계는 항상 실행되며 건너뛸 수 없습니다. 프로덕션 파이프라인이 실행되면 검사를 실행할 사용자 정의 기능 테스트 후에 경험 감사 단계가 포함됩니다.
   ...
   * 결과는 정보 제공용이며 점수, 현재 점수와 이전 점수 간의 변화를 보여줍니다.
...
실행이 재실행인지 식별하기 위해 트리거 필드를 검사할 수 있습니다. 이 값은 *RE_EXECUTE*&#x200B;가 됩니다.
