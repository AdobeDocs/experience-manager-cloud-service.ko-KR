...
드문 경우지만 일시적인 이유로 프로덕션 배포 단계가 실패할 수 있습니다. 이러한 경우 완료 유형(예: 취소 또는 실패)에 관계없이 프로덕션 배포 단계가 완료된 한 프로덕션 배포 단계의 재실행이 지원됩니다. 재실행은 세 단계로 구성된 동일한 파이프라인을 사용하여 새 실행을 만듭니다.
...
재실행을 트리거하려면 HAL 링크에 대한 PUT 요청을 수행합니다 `https://ns.adobe.com/adobecloud/rel/pipeline/reExecute` 프로덕션 배포 단계 상태에서

* 이 링크가 있으면 해당 단계에서 재실행할 수 있습니다.
* 이 링크가 없으면 해당 단계에서 재실행할 수 없습니다.

이 링크는 프로덕션 배포 단계에만 사용할 수 있습니다.
...
HAL 링크의 href 값 구문은 예제일 뿐입니다. 실제 값은 항상 HAL 링크에서 읽어야 하며 생성되지 않아야 합니다.
