...
AEM as a Cloud Service의 Cloud Manager 2025.10.0 릴리스 일자는 2025년 10월 2일 금요일입니다.
...
* **AEM 클라우드 상태 평가 서비스**
  ...
  Adobe은 AEM as a Cloud Service 환경을 최적화하고, 안전하며, 모범 사례에 맞게 유지하는 자동화된 비침습적 검사 도구인 AEM 클라우드 상태 평가 서비스를 제공합니다.

  이 서비스는 다음을 수행합니다.

   * 환경을 스캔하여 성능 병목 현상, 비효율성 및 잠재적 위험을 표시합니다.
   * 콘텐츠 구조(블루프린트, 라이브 카피) 및 사용자 지정 구성을 분석합니다.
   * 오래된 종속성(AEM SDK, 타사 라이브러리)을 식별합니다.
   * 코드 품질 문제에 플래그를 지정합니다(잘못된 주석, 비효율적인 패턴).
   * **작업 센터**&#x200B;와 같은 대시보드를 통해 실행 가능한 지침을 제공합니다.
   * 초기 문제 감지 및 치료를 통해 사전 예방적 최적화 지원.

  팀은 보다 원활한 성능, 보다 강력한 보안 및 장기 유지 관리를 위해 AEM 환경을 지속적으로 모니터링하고 개선할 수 있습니다.

  [프로덕션 및 스테이징 환경에 대한 상태 평가](/help/implementing/cloud-manager/reports/report-health-assessment.md)를 참조하십시오.

* **파이프라인 지원 구성**

  이제 Edge Delivery Services를 사용하여 빌드한 사이트에서도 구성 파이프라인이 지원되며 Cloud Service 환경 그 이상으로 기능이 확장되었습니다. **Config Pipelines**&#x200B;을(를) 사용하여 트래픽 필터 규칙 및 원본 선택기를 포함한 CDN 구성과 같은 설정을 관리할 수 있습니다. [지원되는 구성](/help/operations/config-pipeline.md#configurations)을 참조하십시오.

  Edge Delivery 구성 파이프라인은 Cloud Manager 파이프라인 변수를 통해 비밀도 지원합니다.
...
* **도메인 매핑-CDN 설정 대화 상자 간소화**

  Cloud Manager이 혼동을 줄이고 구성 속도를 높이기 위해 **도메인을 CDN에 매핑** 흐름을 간소화했습니다. 이제 대화 상자에서 **Adobe 관리 CDN**(&quot;권장&quot; 배지 포함)을 강조합니다.

  ![Adobe 관리 CDN 라디오 단추가 선택된 상태에서 CDN에 도메인 매핑 대화 상자](/help/implementing/cloud-manager/assets/cdn/map-domain-to-cdn-dialog-box-adobe-managed-cdn.png).
   ...
   * CDN 원본을 `publish-p<PROGRAM_ID>-e<ENV_ID>.adobeaemcloud.com`(으)로 지정합니다.
   * 원래 호스트를 전달하려면 **Host/SNI**&#x200B;을(를) 설정하십시오.
   * Cloud Manager에서 키를 배포한 후 `X-AEM-Edge-Key`을(를) 추가합니다.
   * `X-Forwarded-Host`을(를) 고객 응대 도메인으로 설정합니다.
   * AEM에 도달하기 전에 다른 `X-Forwarded-*` 헤더를 지우십시오.

  ![다른 CDN 공급자 라디오 단추가 선택된 상태에서 CDN에 도메인 매핑 대화 상자](/help/implementing/cloud-manager/assets/cdn/map-domain-to-cdn-dialog-box-other-cdn-provider.png)

  <!-- (no redundant `Origin` field or "Learn more" clutter) -->함께 제공되는 바닥글에는 두 개의 유용한 링크(주요 CDN에 대한 샘플 구성 및 전체 설명서에 대한 링크)가 있습니다. 단일 확인 버튼 - 내 CDN을 구성했습니다. 흐름이 완료됩니다.

  AEM as a Cloud Service의 [CDN](/help/implementing/dispatcher/cdn.md#point-to-point-CDN)을(를) 참조하십시오.
...
[Experience Hub](/help/experience-hub.md)은(는) 조직의 요구 사항에 맞게 사용자 지정된 AEM에 대한 진입점 역할을 합니다. 최소한의 노력으로 Adobe에서 활성화할 수 있도록 기존 AEM UI 확장에 대해 Experience Hub에 알려 주십시오.

![Experience Hub 확장성 및 사용자 지정 워크플로의 다이어그램](/help/implementing/cloud-manager/release-notes/assets/experience-hub-extensibility-customization.png)
...
관심 있으십니까? Adobe OrgID 및 프로그램 ID를 사용하여 [beta_quickbuild_cmpipelines@adobe.com](mailto:beta_quickbuild_cmpipelines@adobe.com)에 전자 메일을 보냅니다.
