...
description: Adobe Experience Manager as a Cloud Service의 Cloud Manager 2025.9.0 릴리스에 대해 알아봅니다.
...
# Adobe Experience Manager as a Cloud Service의 Cloud Manager 2025.9.0 릴리스 정보 {#release-notes}

<!-- https://wiki.corp.adobe.com/display/DMSArchitecture/%5BKT%5D+Cloud+Manager+2025.08.0+Release -->

AEM (Adobe Experience Manager) as a Cloud Service의 Cloud Manager 2025.9.0 릴리스에 대해 알아봅니다.
...
AEM as a Cloud Service의 Cloud Manager 2025.9.0 릴리스 일자는 2025년 9월 4일 금요일입니다.

다음 릴리스는 2025년 10월 2일 금요일에 예정되어 있습니다.

## 새로운 기능 {#what-is-new}

* **Adobe 관리 도메인 유효성 검사 인증서를 수동으로 갱신**

  이제 Cloud Manager 또는 공개 API에서 수동으로 Adobe 관리 DV(도메인 유효성 검사) 인증서를 갱신하여 인증서를 미리 새로 고칠 수 있습니다. <!-- CMGR-68738 -->

  ![SSL 인증서 갱신](/help/implementing/cloud-manager/release-notes/assets/ssl-certificate-adobedv-renew.png)

* **이제 개인 저장소에 대한 Azure DevOps 지원이 추가되었습니다**

  설명서 업데이트에는 Azure DevOps로 자체 Git 가져오기 및 가져오기 요청 유효성 검사에 대한 구성 단계가 포함됩니다. [Cloud Manager에 외부 저장소 추가](/help/implementing/cloud-manager/managing-code/external-repositories.md)를 참조하십시오.

* **개인 저장소에 대한 가져오기 요청 확인**

  이제 Cloud Manager은 GitHub, Bitbucket, Azure DevOps 및 GitLab에서 개인 저장소를 사용하는 구성 파이프라인을 지원합니다. [개인 저장소에 대한 가져오기 요청 검사](/help/implementing/cloud-manager/managing-code/github-check-config.md)를 참조하십시오.
...
[스테이징 전용 및 프로덕션 전용 파이프라인](/help/implementing/cloud-manager/configuring-pipelines/stage-prod-only.md)에 대한 지원이 도입되어 전체 스택 프로덕션 배포 파이프라인을 더 작고 전문적인 배포로 분할할 수 있습니다.
...
* 이제 Edge Delivery 구성 파이프라인은 Cloud Manager 파이프라인 변수를 통해 암호를 지원합니다.
...
9월 Cloud Manager 릴리스에는 중요한 버그 수정이 없습니다.
