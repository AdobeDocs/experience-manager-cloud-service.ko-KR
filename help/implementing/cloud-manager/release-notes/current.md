...
AEM as a Cloud Service의 Cloud Manager 2025.8.0 릴리스 일자는 2025년 8월 7일 목요일입니다.
...
* **Adobe Experience Hub 곧 제공 예정**
  ...
  Experience Hub는 사용자가 목표를 더 빠르게 달성할 수 있도록 개인화되고 상황에 맞는 경험을 제공하는 통합 시작점입니다. 롤아웃은 2025년 8월 26일까지 완료되어 모든 사용자가 사용할 수 있게 됩니다. 새로운 Experience Hub는 [experience.adobe.com](https://experience.adobe.com/)에서 직접 액세스할 수 있습니다. 자세한 내용은 [Experience Hub](/help/experience-hub.md)을 참조하세요.

* **Edge Delivery Services 라이선스는 셀프서비스 방식으로 HIPAA 프로그램에 포함될 수 있습니다.**

  헬스 케어 또는 중요한 데이터 요구 사항이 있는 조직은 이제 셀프서비스 방식으로 Edge Delivery Services를 사용하여 엄격한 규제 표준을 충족하는 HIPAA 규정을 준수할 수 있습니다. <!-- CMGR-70016 -->

* **BYOG는 이제 Edge Delivery Services에서 사용 가능합니다.**

  이제 Cloud Manager를 사용하면 외부 Git 저장소를 구성하여 유연한 코드 관리 워크플로를 구현할 수 있습니다. <!--(CMGR‑69010, CMGR‑70988) --> 또한 Cloud Manager UI에서 선택한 분기 코드를 직접 가져와서 수동 저장소 작업을 줄일 수 있습니다. [외부 Git 저장소를 사용하도록 Edge Delivery 사이트 구성](/help/implementing/cloud-manager/edge-delivery/config-edge-delivery-site-with-byog.md)<!-- (CMGR‑68085)(CMGR-69015) -->을 참조하십시오. <!-- KT: https://wiki.corp.adobe.com/display/DMSArchitecture/%5B2025%5D+Cloud+Manager+-+Bring+Your+Own+Git+with+EDS -->
...
* **새로운 Forms Add‑on에 대한 자동 프로비저닝**
   ...
   * 간단한 마케팅 양식 사용 사례를 대상으로 합니다.
   * IMS 조직이 사용 가능한 양식 추가 기능 라이선스를 보유하고 있을 때만 프로덕션 프로그램 생성 또는 프로덕션 프로그램 편집 중 **솔루션 및 추가 기능** 목록에 나타납니다.

     ![Forms 추가 기능](/help/implementing/cloud-manager/release-notes/assets/forms-add-on.png) *Forms 추가 기능은 IMS 조직에서 Forms 추가 기능 라이선스를 사용할 수 있는 경우에만 프로그램에 추가할 수 있습니다.*

     ![프로덕션 프로그램을 생성할 때 솔루션 및 추가 기능의 Forms 추가 기능](/help/implementing/cloud-manager/release-notes/assets/forms-add-on-creating-production-program.png) *프로그램 생성 중에 사이트 솔루션 내에서 Forms 추가 기능을 선택할 수 있습니다.*

     ![생산 프로그램 편집 시 양식 추가 기능](/help/implementing/cloud-manager/release-notes/assets/forms-add-on-editing-production-program.png) *~ 안에&#x200B;**프로그램 편집**, 사이트 프로그램에 대한 Forms 추가 기능을 선택한 다음 파이프라인을 실행하여 환경에서 활성화합니다.*
...
* Edge Delivery Services 파이프라인은 이제 **배포된 코드** 열에 **구성**&#x200B;을 표시하여 구성 전용 배포를 즉시 식별할 수 있습니다.<!-- CMGR‑69681 -->
* Cloud Manager는 프로그램에 하나 이상의 Edge Delivery Services 사이트와 매핑된 도메인이 포함되면 **Edge Delivery Pipeline 추가**&#x200B;를 표시합니다. 그렇지 않으면 해당 옵션이 비활성화된 것으로 표시되고, 도구 설명에서 누락된 요구 사항을 설명합니다. <!-- CMGR‑69680 -->
* **Edge Delivery** 탭에는 각 파이프라인의 이름, 상태, 저장소 및 분기를 나열하는 새로운 **Edge Delivery 파이프라인** 위젯이 표시됩니다. <!-- (CMGR-69052) -->
...
* **필터** 패널에 **게재 유형** 섹션이 추가되었으며, 여기에는 **Edge 게재** 및 **게시 게재** 확인란이 포함됩니다. <!-- (CMGR-69682) -->
...
* 파이프라인은 이제 활성 Edge Delivery Services 도메인 구성에만 변수를 전달하고 파이프라인 재생성 중에 제거된 모든 구성을 건너뜁니다. <!-- (CMGR‑70039) -->
* 파이프라인 실행이 이제 안정적으로 시작됩니다. 내부 리소스 처리 오류로 인해 일부 파이프라인이 시작되지 않는 문제가 해결되었습니다. <!-- (CMGR‑58167) -->
* 콘텐츠 복사는 Cloud Manager 권한을 검증하고 배포 관리자 또는 관리자 권한이 없는 사용자의 시작을 차단합니다. <!-- (CMGR‑62097) -->
