...
* **BYOG는 이제 Edge Delivery Services에서 사용 가능합니다.**
  ...
  이제 Cloud Manager를 사용하면 외부 Git 저장소를 구성하여 유연한 코드 관리 워크플로를 구현할 수 있습니다. <!--(CMGR‑69010, CMGR‑70988) --> 또한 Cloud Manager UI에서 선택한 분기 코드를 직접 가져와서 수동 저장소 작업을 줄일 수 있습니다. [외부 Git 저장소를 사용하도록 Edge Delivery 사이트 구성](/help/implementing/cloud-manager/edge-delivery/config-edge-delivery-site-with-byog.md)을 참조하십시오.
...
* **새로운 Forms Add‑on에 대한 자동 프로비저닝**
   ...
   * IMS 조직이 사용 가능한 양식 추가 기능 라이선스를 보유하고 있을 때만 프로덕션 프로그램 생성 또는 프로덕션 프로그램 편집 중 **솔루션 및 추가 기능** 목록에 나타납니다.
     ...
     ![프로덕션 프로그램 편집 시 양식 추가 기능](/help/implementing/cloud-manager/release-notes/assets/forms-add-on-editing-production-program.png) *~ 안에&#x200B;**프로그램 편집**, 사이트 프로그램에 대한 Forms 추가 기능을 선택한 다음 파이프라인을 실행하여 환경에서 활성화합니다.*
