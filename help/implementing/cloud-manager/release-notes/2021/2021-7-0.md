...
description: Adobe Experience Manager(AEM)의 Cloud Manager 릴리스 정보 as a Cloud Service 릴리스 2021.7.0
...
이 페이지에서는 Adobe Experience Manager(AEM) as a Cloud Service 2021.7.0의 Cloud Manager 릴리스 정보에 대해 간략히 설명합니다.
...
Azul JDK를 사용하도록 전환하는 고객은 모든 기존 애플리케이션이 Azul JDK에서 오류 없이 컴파일되지 않는다는 것을 알고 있어야 합니다. 전환하기 전에 로컬에서 테스트하는 것이 좋습니다.
