...
>title="UI 테스트"
>abstract="사용자 정의 UI 테스트는 애플리케이션에 대한 UI 테스트를 만들고 자동으로 실행할 수 있는 선택적 기능입니다. UI 테스트는 언어와 프레임워크에서 폭넓은 선택을 할 수 있도록 Docker 이미지로 패키지된 Selenium 기반 테스트입니다. Java 및 Maven, Node와 WebDriver.io 또는 Selenium을 기반으로 한 기타 프레임워크와 기술입니다."
