...
AEM as a Cloud Service의 Cloud Manager을 사용하면 프로젝트에 대해 Edge Delivery 서비스를 활성화할 수 있습니다.

>[!TIP]
>
>Edge Delivery Services에 대한 자세한 내용과 AEM에서 사용할 수 있는 방법은 [Edge Delivery Services 개요](/help/edge/overview.md#how-does-it-work)를 참조하십시오.
...
Cloud Manager은 Adobe Managed CDN에서 Edge Delivery Services에 대해 고유한 기능을 가진 두 가지 배포 유형을 제공합니다. [자세히 알아보기](#edge-delivery-deployment-options).

>[!NOTE]
>
>Edge Delivery Services은 구성 파이프라인 및 원본 선택기를 사용하여 기존 AEM Sites as a Cloud Service 환경에 통합할 수도 있습니다. 자세한 내용은 [Edge Delivery Services으로 프록시 설정](/help/implementing/dispatcher/cdn-configuring-traffic.md#proxying-to-edge-delivery) 및 [기존 환경에서 프록시 설정](https://www.aem.live/docs/byo-cdn-adobe-managed#option-1-setup-a-proxy-from-an-existing-environment)을 참조하십시오.

## Adobe Managed CDN의 Edge Delivery Services 배포 옵션 {#edge-delivery-deployment-options}

Adobe Managed CDN에는 두 가지 Edge Delivery Services 배포 유형이 있습니다.

1. **기존 AEMaaCS 환경에서** — 기존 AEM Sites as a Cloud Service 환경에서 HTTP 프록시를 설정합니다. 이 방법은 일반적으로 기존 환경이 있고 사이트의 일부를 Edge Delivery Services으로 마이그레이션하려는 경우에 사용됩니다. [기존 환경에서 프록시 설정](https://www.aem.live/docs/byo-cdn-adobe-managed#option-1-setup-a-proxy-from-an-existing-environment)을 참조하십시오.

1. **기존 AEMaaCS 환경(Edge 환경) 없이** — AEM Sites as a Cloud Service 환경과 독립적으로 새 Edge Delivery 사이트를 설정합니다. 이 접근 방식은 AEM 작성자 또는 게시 환경이 없고 Edge Delivery Services을 단독으로 사용하려는 경우에 사용됩니다. [기존 환경 없이 Edge Delivery 사이트 설정](https://www.aem.live/docs/byo-cdn-adobe-managed#option-2-setup-an-edge-delivery-site-without-an-existing-environment)을 참조하십시오.

다음 두 옵션에는 서로 다른 기능도 있습니다.

* **파이프라인 구성**&#x200B;을 AEM as a Cloud Service 환경에 사용할 수 있습니다.
* **파이프라인 구성**&#x200B;은(는) 현재 제한된 Beta 프로그램을 통해서만 Edge 환경에 사용할 수 있습니다.

전체 설정 지침은 [Adobe 관리 CDN](https://www.aem.live/docs/byo-cdn-adobe-managed)을 참조하세요.


## AEM 작성을 통한 Edge Delivery Services 정보(Beta) {#eds-aem-authoring}

>[!NOTE]
>
>여기에 설명된 유연한 게시 계층 및 AEM 작성 횡단 보도 기능은 Beta에 있습니다. Beta에 가입하려면 Adobe 조직 ID와 프로그램 ID로 [grp-beta_xwalk-publish_config@adobe.com](mailto:grp-beta_xwalk-publish_config@adobe.com)에 전자 메일을 보내십시오.

최신 웹 경험은 고성능 전달을 필요로 하지만, 많은 조직은 확립된 AEM 작성 워크플로, 거버넌스 및 콘텐츠 재사용 패턴을 사용합니다. Cloud Manager은 작성에 영향을 주지 않고 팀이 게재를 현대화할 수 있도록 지원하기 위해 다음과 같은 기능을 제공합니다.

* Edge Delivery Services을 사용하여 경험을 전달합니다.
* 컨텐츠 작성에 AEM 작성자를 계속 사용합니다.
* 아키텍처에 필요한 인프라만 프로비저닝합니다.

이러한 기능을 통해 조직은 기존 워크플로우의 영향을 받지 않고 최신 게재를 점진적으로 채택할 수 있습니다.

### Edge Delivery 사이트 작성 옵션 {#authoring-options-eds}

Cloud Manager에서 Edge Delivery 사이트를 만들 때 선호하는 작성 접근 방식을 선택할 수 있습니다.

* 문서 기반 작성 - Google Drive 또는 SharePoint에서 콘텐츠를 작성합니다. AEM 환경은 필요하지 않습니다.
* AEM 작성 — 범용 편집기를 사용하여 AEM에서 컨텐츠를 작성합니다. 이 메서드를 사용하려면 AEM 작성자 환경이 필요합니다. 이 옵션을 사용하면 Edge Delivery에서 컨텐츠 전달을 처리할 때 게시 계층이 필요하지 않습니다.

조직은 워크플로 환경 설정에 따라 이러한 접근 방식 중에서 선택하거나 두 방식을 점진적으로 사용할 수 있습니다. [한 번의 클릭으로 첫 번째 Edge Delivery 사이트 만들기](/help/implementing/cloud-manager/edge-delivery/create-edge-delivery-site.md)를 참조하십시오.

### 유연한 게시 계층 {#flexible-publish-tier}

Cloud Manager을 사용하면 게시 계층이 프로그램 환경에 프로비저닝되는지 여부를 구성할 수 있습니다. 다음 표에 표시된 것처럼 모든 아키텍처에 게시 계층이 필요한 것은 아닙니다.
...
필요한 경우에만 게시 계층을 활성화함으로써 팀은 환경을 보다 빠르게 프로비저닝하고 인프라를 단순화하며 불필요한 구성 요소를 줄일 수 있습니다. [유연한 게시 계층(Beta)](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/creating-production-programs.md#flexible-publish-tier)을 참조하십시오.
...
* [외부 Git 저장소를 사용](/help/implementing/cloud-manager/managing-code/external-repositories.md)(고유한 Git 가져오기)하여 Edge Delivery Services 사이트 코드를 동기화하고 배포합니다. 이 기능을 활용하려면 먼저 [Cloud Manager에서 사이트를 온보딩](/help/implementing/cloud-manager/edge-delivery/add-edge-delivery-site.md)해야 합니다. <!-- NEW from CQDOC-22867 -->
* [Edge Delivery 구성 파이프라인을 사용](/help/implementing/dispatcher/cdn-configuring-traffic.md)하여 트래픽 필터, 원본 선택기 및 리디렉션과 같은 규칙을 정의하여 Edge Delivery 사이트에 대한 Adobe 관리 CDN 설정을 구성합니다. <!-- NEW from CQDOC-22867 -->
* CRUD(만들기, 읽기, 업데이트, 삭제) 작업을 수행할 때 [API 우선](https://developer.adobe.com/experience-cloud/experience-manager-apis/)의 이점을 활용할 수 있습니다.
* [SLA 보고에 액세스](/help/implementing/cloud-manager/reports/report-sla.md).
...
## 프로덕션 프로그램 또는 샌드박스 프로그램에 Edge Delivery Services 추가에 대한 정보 {#about-adding-eds-to-prod-sandbox}
...
| AEM 작성을 사용하는 Edge Delivery 사이트를 만들고 싶습니다 | [Edge Delivery 사이트 만들기](/help/implementing/cloud-manager/edge-delivery/create-edge-delivery-site.md#one-click-edge-delivery-site)를 참조하십시오. Edge Delivery과 함께 AEM 작성을 사용하는 경우 게시 계층은 선택 사항입니다. [유연한 게시 계층(Beta)](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/creating-production-programs.md#flexible-publish-tier)을 참조하십시오. |
