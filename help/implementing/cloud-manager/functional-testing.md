---
title: 기능 테스트
description: 코드의 품질과 신뢰성을 보장하기 위해 AEM as a Cloud Service 배포 프로세스에 내장된 세 가지 유형의 기능 테스트에 대해 알아보십시오.
...
>title="기능 테스트"
>abstract="AEM as a Cloud Service 배포 프로세스에 포함된 세 가지 유형의 기능 테스트에 대해 알아봅니다. 테스트는 코드의 품질과 신뢰성을 보장합니다."
...
기본 제공 품질 게이트는 주로 AEM 애플리케이션의 컨텍스트 내에서 AEM 제품의 기능을 확인합니다. 반면 사용자가 설정한 사용자 정의 품질 게이트는 애플리케이션의 중요한 기능과 사용자 상호 작용이 의도한 대로 수행되는지 확인하도록 설계되었습니다. 이러한 두 세트의 품질 게이트는 함께 작동하여 코드 수정 및 AEM 제품 업데이트 모두에 대해 강력하고 안전한 자동 배포를 보장합니다.
...
제품 기능 테스트는 작성 및 복제 작업을 포함하여 핵심 AEM 기능에 대한 안정적인 HTTP 통합 테스트(IT)입니다. Adobe은 즉시 사용할 수 있도록 이러한 기능을 제공하고 유지 관리합니다. 이는 AEM 제품의 핵심 기능이 손상된 경우 사용자 정의 애플리케이션 코드에 대한 변경 사항이 배포되는 것을 방지하기 위한 것입니다.
...
제품 테스트와 마찬가지로 고객 기능 테스트는 JUnit로 구현하고 Maven을 사용하여 실행하며 공식 [AEM 테스트 클라이언트](https://github.com/adobe/aem-testing-clients)의 맨 위에 빌드된 HTTP 통합 테스트(IT)입니다.
...
효율적인 파이프라인 실행을 유지하기 위해 Adobe에서는 약 15분 이하의 기능 테스트 런타임을 목표로 주요 기능 및 기본 사용자 상호 작용 흐름에 중점을 두는 것이 좋습니다. 이 시간을 초과하는 전체 기능 테스트 세트는 개발 프로세스 동안 일반적인 고객 유효성 검사 파이프라인의 일부로 실행해야 합니다.
...
고객별 개발에 대한 위험 제어를 극대화하기 위해 Adobe에서는 중요한 UI 테스트를 AEM as a Cloud Service에 캡처할 것을 권장합니다. 제한적이지만 고객 경험에 미치는 영향을 극대화하는 데 중점을 둡니다.
...
파이프라인 실행을 효율적으로 유지하기 위해 Adobe에서는 주요 기능 및 주요 사용자 상호 작용 흐름에 중점을 둘 것을 권장합니다. 이 품질 게이트를 초과하는 전체 UI 테스트 세트는 일반 고객 유효성 검사 파이프라인의 일부로 실행해야 합니다. 고객의 개발 프로세스에 통합합니다.
...
여기에서 원하는 도구 및 프레임워크를 선택할 수 있습니다. 고객 기능 테스트 및 사용자 정의 UI 테스트와 달리 AEM as a Cloud Service 관련 제한은 없습니다. 따라서 Adobe에서는 여기에서 장기 실행 기능 및 UI 테스트를 수행할 것을 권장합니다.

모든 도구와 프레임워크를 선택할 수 있지만, Adobe에서는 HTTP 기반 통합 및 UI 테스트를 사용자 정의 기능 및 UI 테스트 품질 게이트에 사용되는 도구 및 프레임워크에 맞추는 것을 제안합니다. 또한 Adobe은 [신속한 개발 환경(RDE)](/help/implementing/developing/introduction/rapid-development-environments.md)을 로컬 테스트 전략에 통합하여 AEM 클라우드 환경을 면밀하게 미러링하는 것을 권장합니다.
