...
description: Java&trade를 작성하는 방법 알아보기; AEM as a Cloud Service 기능 테스트
...
AEM as a Cloud Service에 대한 Java™ 기능 테스트를 작성하는 방법에 대해 알아봅니다
...
사용자 정의 기능 테스트를 위한 코드는 의 Java™ 코드입니다. `it.tests` 프로젝트의 폴더입니다. 모든 기능 테스트와 함께 단일 JAR을 생성해야 합니다. 빌드가 둘 이상의 테스트 JAR을 생성하는 경우 선택되는 JAR은 비결정적입니다. 테스트 JAR이 0이면 테스트 단계가 기본적으로 통과합니다. 샘플 테스트는 [AEM Project Archetype](https://github.com/adobe/aem-project-archetype/tree/develop/src/main/archetype/it.tests)을 참조하십시오.

테스트는 최소 2개의 작성자 인스턴스, 2개의 게시 인스턴스 및 Dispatcher 구성을 포함하여 Adobe이 유지 관리하는 테스트 인프라에서 실행됩니다. 이 설정은 사용자 정의 기능 테스트가 전체 AEM 스택에 대해 실행됨을 의미합니다.
...
다음을 참조하십시오 [`aem-testing-clients` GitHub 저장소](https://github.com/adobe/aem-testing-clients) 을 참조하십시오.
...
| 메모리 | 0.5Gi | 테스트에 할당된 메모리 양, 값(GB) |
| 시간 초과 | 30m | 테스트가 종료되는 기간입니다. |
| 권장 기간 | 15m | 이 시간보다 오래 걸리지 않도록 테스트를 작성하는 것이 좋습니다. |

>[!NOTE]
>
> 추가 리소스가 필요한 경우, 고객 지원 사례를 만들고 사용 사례를 설명합니다. Adobe 팀이 요청을 검토하고 적절한 지원을 제공합니다.
...
| 속성 | 설명 | 예 |
|-------------------------------------|------------------------------------------------------------------|-------------------------|
| `sling.it.instances` | 클라우드 서비스를 일치시키려면 인스턴스 수를 로 설정해야 합니다. `2` | `2` |
| `sling.it.instance.url.1` | 작성자 URL로 설정해야 합니다. | `http://localhost:4502` |
| `sling.it.instance.runmode.1` | 첫 번째 인스턴스의 실행 모드를 로 설정해야 합니다. `author` | `author` |
...
| `sling.it.instance.runmode.2` | 두 번째 인스턴스의 실행 모드를 로 설정해야 합니다. `publish` | `publish` |
