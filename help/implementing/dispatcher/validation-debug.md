...
>[!NOTE]
>클라우드의 Dispatcher 및 Dispatcher 도구를 다운로드하는 방법에 대한 자세한 내용은 Cloud](/help/implementing/dispatcher/disp-overview.md) 페이지의 [Dispatcher 를 참조하십시오. Dispatcher 구성이 이전 모드에 있는 경우 [기존 모드 설명서](/help/implementing/dispatcher/validation-debug-legacy.md)를 참조하십시오.

다음 섹션에서는 유연한 모드 파일 구조, 로컬 유효성 검사, 디버깅 및 기존 모드에서 유연한 모드로 마이그레이션하는 방법에 대해 설명합니다.

이 문서에서는 프로젝트의 Dispatcher 구성에 파일 `opt-in/USE_SOURCES_DIRECTLY`이 포함되어 있다고 가정합니다. 그러면 SDK 및 런타임에서 레거시 모드와 비교하여 향상된 방법으로 구성을 확인하고 배포하며 파일 수 및 크기에 대한 제한을 제거합니다.

이와 같이, Dispatcher 구성에 위의 파일이 포함되어 있지 않으면 **권장** 기존 모드에서 유연한 모드](#migrating) 섹션에 설명된 대로 레거시 모드에서 유연한 모드로 마이그레이션하는 것이 좋습니다.[
...
3. [파일 구조 섹션](##flexible-mode-file-structure)에 설명된 대로 변경할 수 없도록 만들어진 Dispatcher SDK 구성 파일의 하위 세트가 수정되지 않았는지 확인합니다.
...
이 단계는 `bin/docker_run.sh src/dispatcher host.internal.docker:4503 8080`을 통해 독립적으로 실행할 수도 있습니다.
...
이 단계는 `bin/docker_immutability_check.sh src/dispatcher`을 통해 독립적으로 실행할 수도 있습니다.

## Apache 및 Dispatcher 구성 디버깅 {#debugging-apache-and-dispatcher-configuration}

`./bin/docker_run.sh src/dispatcher docker.for.mac.localhost:4503 8080` 을 사용하여 Apache 디스패처를 로컬로 실행할 수 있습니다.
...
Cloud Manager 2021.7.0 릴리스를 통해 새 Cloud Manager 프로그램은 [AEM Archetype 28](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/overview.html?lang=en) 이상을 사용하여 maven 프로젝트 구조를 생성합니다. 이 구조에는 **opt-in/USE_SOURCES_직접** 파일이 포함됩니다. 이렇게 하면 파일 수와 크기 주위에 [기존 모드](/help/implementing/dispatcher/validation-debug-legacy.md)의 이전 제한 사항이 제거되어 SDK 및 런타임에서 개선된 방식으로 구성을 확인하고 배포합니다. Dispatcher 구성에 이 파일이 없는 경우에는 마이그레이션하는 것이 좋습니다. 안전한 전환을 확인하려면 다음 단계를 사용하십시오.
...
2. **클라우드 개발 테스트:**
   * 비프로덕션 파이프라인에 의해 배포되는 Git 분기에 파일 `opt-in/USE_SOURCES_DIRECTLY`을 커밋합니다.
...
3. **프로덕션에 배포:**
   * 프로덕션 파이프라인에 의해 Cloud 스테이지 및 프로덕션 환경에 배포되는 Git 분기에 파일 `opt-in/USE_SOURCES_DIRECTLY`을 커밋합니다.
