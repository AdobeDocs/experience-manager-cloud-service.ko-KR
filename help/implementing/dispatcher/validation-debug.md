...
이 문서에서는 프로젝트의 Dispatcher 구성에 파일이 포함되어 있다고 가정합니다 `opt-in/USE_SOURCES_DIRECTLY`를 사용하면 SDK 및 런타임에서 기존 모드에 비해 개선된 방식으로 구성을 확인 및 배포하여 파일 수 및 크기에 대한 제한을 제거할 수 있습니다.

따라서 Dispatcher 구성에 위의 파일이 포함되지 않은 경우 **매우 권장됨** 레거시 모드에서 다음에 설명된 유연한 모드로 마이그레이션하는 경우 [레거시 모드에서 유연한 모드로 마이그레이션](#migrating) 섹션.
...
ServerAlias와 일치하는 가상 호스트를 항상 하나 이상 사용할 수 있는지 확인하십시오 `\*.local`, `localhost` 및 `127.0.0.1` dispatcher 무효화에 필요합니다. 서버 별칭 `*.adobeaemcloud.net` 및 `*.adobeaemcloud.com` 하나 이상의 vhost 구성에도 필요하며 내부 Adobe 프로세스에 필요합니다.
...
>Windows와 macOS 모두에 Docker Desktop을 사용하는 것이 좋습니다.
...
로컬 변경 불가능한 파일은 `bin/update_maven.sh src/dispatcher` dispatcher 폴더의 스크립트. 여기서 `src/dispatcher` 는 dispatcher 구성 디렉터리입니다. 이렇게 하면 상위 디렉터리의 pom.xml 파일도 업데이트되므로 Maven 불변 검사도 업데이트됩니다.

## Apache 및 Dispatcher 구성 디버깅 {#debugging-apache-and-dispatcher-configuration}

다음을 사용하여 Apache Dispatcher를 로컬에서 실행할 수 있습니다. `./bin/docker_run.sh src/dispatcher docker.for.mac.localhost:4503 8080`.
...
출력의 첫 번째 행은 다음에 대해 실행되는 것과 비슷합니다. `docker_run.sh`, 예를 들면 다음과 같습니다.
...
Cloud Manager 2021.7.0 릴리스를 통해 새로운 Cloud Manager 프로그램은 [AEM Archetype 28](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/overview.html?lang=ko) 또는 그 이상(파일을 포함) **opt-in/USE_SOURCES_DIRECTLY**. 이렇게 하면 의 이전 제한 사항이 제거됩니다. [레거시 모드](/help/implementing/dispatcher/validation-debug-legacy.md) 또한 SDK 및 런타임에서 개선된 방법으로 구성을 확인하고 배포하는 문제가 발생할 수 있습니다. Dispatcher 구성에 이 파일이 없는 경우 마이그레이션하는 것이 좋습니다. 안전한 전환을 보장하려면 다음 단계를 따르십시오.

1. **로컬 테스트.** 최신 Dispatcher 도구 SDK를 사용하여 폴더 및 파일을 추가합니다 `opt-in/USE_SOURCES_DIRECTLY`. 이 문서의 &quot;로컬 유효성 검사&quot; 지침에 따라 Dispatcher가 로컬에서 작동하는지 테스트합니다.
1. **클라우드 개발 테스트:**
