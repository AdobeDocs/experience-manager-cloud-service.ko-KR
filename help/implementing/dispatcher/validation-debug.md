...
ServerAlias와 일치하는 가상 호스트를 항상 사용할 수 있는지 확인합니다. `\*.local` 내부 Adobe 프로세스에 필요한 localhost도 포함됩니다.
...
3. 에 설명된 대로 변경할 수 없도록 설계된 Dispatcher SDK 구성 파일의 하위 집합이 있는지 확인합니다. [파일 구조 섹션](##flexible-mode-file-structure)은(는) 수정되지 않았습니다.
