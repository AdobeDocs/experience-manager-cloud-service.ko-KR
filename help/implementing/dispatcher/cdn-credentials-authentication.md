...
## 암호 배포: 환경 변수 및 파이프라인 변수 {#deploying-secrets}
...
동일한 구성에 대해 파이프라인 비밀과 환경 비밀을 혼합할 수 없습니다. 배포 단계에 대해 파이프라인 비밀 변수가 정의된 경우 환경 설정에서 사용됩니다.

다음 이미지는 Cloud Manager에서 파이프라인 비밀 변수를 구성하는 방법을 보여 줍니다.

![파이프라인 암호 변수 구성](/help/implementing/dispatcher/assets/pipeline-secrets-configuration.png)

파이프라인 변수(암호 포함)의 추가, 편집 및 관리에 대한 자세한 내용은 [Cloud Manager의 파이프라인 변수](/help/implementing/cloud-manager/configuring-pipelines/pipeline-variables.md)를 참조하십시오.

## 비밀 작업 지침 {#secrets-guidelines}
