...
이 페이지에서는 Dispatcher 도구와 지원되는 Apache 모듈을 다운로드하고 추출하는 방법을 설명하고 기존 및 유연한 모드에 대한 높은 수준의 개요를 제공합니다. 또한 유효성 검사 및 디버깅에 대한 추가 참조와 AMS에서 AEM으로 Dispatcher 구성을 as a Cloud Service으로 마이그레이션하는 방법에 대한 참조가 있습니다. <!-- ERROR: NOT FOUND (HTTP ERROR 404) Also, see [this video](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/cloud-5/cloud5-aem-dispatcher-cloud.html) for additional details about deploying dispatcher files in a cloud service environment. -->
...
* 고객이 Dispatcher 구성에 AEM as a Cloud Service 지원 지침만 포함되어 있는지 확인하는 도구입니다. 또한 툴은 Apache가 성공적으로 시작할 수 있도록 구문이 올바른지 확인합니다.
...
**macOS/Linux**&#x200B;의 경우 실행 가능한 Dispatcher 도구 아티팩트를 만들어 실행합니다. 이 편집기는 저장한 디렉터리 아래에 Dispatcher 도구 파일을 자동으로 추출합니다(여기서 `version` 는 Dispatcher 도구 버전입니다.
...
게시 계층의 경우 Blob를 제공하는 기본값은 첨부 파일로 제공됩니다. 표준을 사용하여 이 설정 재정의 [콘텐츠 처리 헤더](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Disposition) Dispatcher에서.
...
프로젝트의 Apache 및 Dispatcher 폴더 구조는 의 설명에 따라 프로젝트가 사용하는 모드에 따라 약간 다릅니다. [Dispatcher 도구를 사용한 유효성 검사 및 디버깅](#validation-debug) 위의 섹션.
