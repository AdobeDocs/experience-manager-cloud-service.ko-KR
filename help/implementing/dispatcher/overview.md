...
1. 브라우저에 삽입된 URL
2. DNS에 매핑된 CDN에 대한 요청
3. 컨텐츠가 CDN에 완전히 캐시되는 경우 CDN은 브라우저에 컨텐츠를 제공합니다
4. 컨텐츠가 완전히 캐시되지 않으면 CDN은 디스패처에 대해 콜아웃(역방향 프록시)합니다
5. 컨텐츠가 디스패처에 완전히 캐시되는 경우 디스패처는 이를 CDN에 제공합니다
6. 컨텐츠가 완전히 캐시되지 않으면 디스패처는 AEM 게시로 아웃(역방향 프록시)을 호출합니다
7. 브라우저로 렌더링된 컨텐츠

### CDN {#cdn}

AEM에서는 두 가지 옵션을 제공합니다.

1. 관리 CDN - AEM의 기본 CDN
2. 관리되지 않는 CDN - 고객은 자신의 CDN을 가져오고 이를 전적으로 관리합니다.

첫 번째 옵션은 매우 권장되며 Adobe는 두 번째 옵션을 사용할 때 잘못된 구성의 결과에 대해 책임을 지지 않습니다.

#### 관리 CDN {#managed-cdn}

Adobe에서 CDN을 프로비전한 후에는 CNAME을 만들어 관리되는 CDN 도메인에서 호스팅되는 Adobe 제어 도메인에 애플리케이션의 도메인을 매핑해야 합니다.

CDN은 SSL을 종료해야 하는 레이어 7 웹 애플리케이션 방화벽 역할을 하므로 고객 서명 SSL 인증서가 필요합니다. 출시 전 단계에서는 수동 프로세스를 통해 Adobe에 인증서를 제공해야 합니다.

GA 시점에 Cloud Manager에 인증서를 업로드해야 합니다. 그러면 인증서가 CDN에 업로드됩니다. SSL 인증서가 만료되면 Cloud Manager에서 인증서를 새로 고칠 수 있도록 알림 메시지가 표시됩니다.

#### 관리되지 않는 CDN {#unmanaged-cdn}

제공된 고유한 CDN을 관리할 수 있습니다.

1. 기존 CDN이 있습니다.
2. 관리
3. 애플리케이션이 AEM 아키텍처에 아직 수용되지 않은 CDN에 대한 광범위한 API 호출을 수행하지 않습니다.
4. 클라우드 서비스로서 AEM은 엔드 투 엔드 시스템이 제대로 작동하도록 설정할 수 있습니다.
5. 허용하려면 Adobe에 CDN URL의 허용 목록을 제공해야 합니다.

Adobe는 CDN의 출처로 사용할 AEM Cloud URL을 제공합니다.
...
AEM을 클라우드 서비스로 사용하기 전에는 디스패처 캐시를 무효화하는 2가지 방법이 있었습니다.

1. 게시 디스패처 플러시 에이전트 지정 복제 API 호출
...
이 게시 트리거 무효화는 빠른 시작과 동일합니다.게시 인스턴스가 작성자로부터 (복제 및 파이프라인 대기열을 통해) 새 버전의 페이지 또는 자산을 받으면 플러시 에이전트를 사용하여 디스패처의 적절한 경로를 무효화합니다. 업데이트된 경로는 상태 파일 수준으로 구성할 수 있는 수준까지 부모와 함께 [디스패처 캐시에서 제거됩니다](https://docs.adobe.com/content/help/en/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html#invalidating-files-by-folder-level).
