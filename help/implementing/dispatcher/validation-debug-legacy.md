...
>[!NOTE]
>클라우드의 Dispatcher 및 Dispatcher 도구를 다운로드하는 방법에 대한 자세한 내용은 [클라우드의 디스패처](/help/implementing/dispatcher/disp-overview.md) 페이지.

다음 섹션에서는 레거시 모드 파일 구조, 로컬 유효성 검사, 디버깅 및 기존 모드에서 로 마이그레이션하는 방법에 대해 설명합니다 [유연한 모드](/help/implementing/dispatcher/validation-debug.md).
...
그렇습니다 **매우 권장** 마이그레이션 섹션에 설명된 대로 레거시 모드에서 유연한 모드로 마이그레이션합니다. [기존 모드에서 유연한 모드로 마이그레이션](#migrating-flexible). 또한 유연한 모드를 사용하면 SDK 및 런타임에서 개선된 방식으로 구성을 확인하고 배포합니다.
...
3. 에 설명된 대로 변경할 수 없도록 만들어진 Dispatcher SDK 구성 파일의 하위 집합을 확인합니다. [파일 구조 섹션](##legacy-mode-file-structure)이 수정되지 않았습니다. 이 확인은 AEM SDK 버전 v2021.1.4738에서 도입되었으며 Dispatcher 도구 버전 2.0.36도 포함되어 있습니다. 이 업데이트 전에 고객은 변경할 수 없는 파일의 로컬 SDK 수정 사항이 클라우드 환경에도 적용된다고 잘못 간주했을 수 있습니다.
...
이 단계는 를 통해 독립적으로 실행할 수도 있습니다. `validator full -d out src/dispatcher`다음 명령에 필요한 외부 디렉토리를 생성합니다. `bin/docker_run.sh out host.docker.internal:4503 8080`.
...
이 단계는 를 통해 독립적으로 실행할 수도 있습니다. `validator full -d out src/dispatcher`다음 명령에 필요한 외부 디렉토리를 생성합니다. `bin/docker_immutability_check.sh out`.

## Apache 및 Dispatcher 구성 디버깅 {#debugging-apache-and-dispatcher-configuration}

를 사용하여 Apache 디스패처를 로컬로 실행할 수 있습니다 `./bin/docker_run.sh out docker.for.mac.localhost:4503 8080`.
...
Cloud Manager 2021.7.0 릴리스를 통해 새로운 Cloud Manager 프로그램은 파일을 포함하는 AEM 원형 28 이상을 사용하여 maven 프로젝트 구조를 생성합니다 **opt-in/USE_SOURCES_DIRECT**. 따라서 파일 수와 크기 주변에 이전 모드의 제한 사항이 제거되어 SDK 및 런타임에서 개선된 방식으로 구성을 확인하고 배포합니다. Dispatcher 구성에 이 파일이 없는 경우에는 마이그레이션하는 것이 좋습니다. 에 설명된 메서드를 사용합니다. [유연한 모드](/help/implementing/dispatcher/validation-debug.md#migrating) 페이지.
