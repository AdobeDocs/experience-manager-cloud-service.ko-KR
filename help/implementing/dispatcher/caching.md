...
* 캐시된 HTML에서 더 이상 사용할 수 없는 긴 캐시 URL을 계속 참조하는 경우 [짧은 캐시 clientlib URL로 대체](#content-consistency)하는 등 자세한 내용은 [클라이언트 측 라이브러리 및 버전 일관성](#clientlib-shortcache-fallback) 섹션을 참조하십시오.
...
일반적으로 Dispatcher 캐시를 무효화할 필요는 없습니다. 대신 콘텐츠가 다시 게시될 때 Dispatcher이 캐시를 새로 고치고 CDN이 캐시 만료 헤더를 준수하도록 해야 합니다.
...
결론적으로, Dispatcher 캐시를 무효화하려는 경우 권장되는 옵션은 작성자의 SCD API 무효화 작업을 사용하는 것입니다. 또한 이벤트를 수신하여 추가 다운스트림 작업을 트리거할 수도 있습니다.
...
엄격한 clientlib 버전 관리를 사용하면 장기 해시 키가 선택기로 클라이언트 라이브러리에 추가됩니다. 따라서 clientlib 참조는 다음과 같습니다.

```
<link rel="stylesheet" href="/etc.clientlibs/wkndapp/clientlibs/clientlib-base.lc-7c8c5d228445ff48ab49a8e3c865c562-lc.css" type="text/css">
```

AEM as a Cloud Service에서 엄격한 clientlib 버전 관리가 기본적으로 활성화되어 있습니다.
...
### 긴 캐시 clientlib을 사용할 수 없는 경우 짧은 캐시 URL로 폴백 {#clientlib-shortcache-fallback}

중간 캐시(예: Adobe CDN 또는 브라우저 캐시)는 새 콘텐츠가 게시된 후 잠시 동안 **이전 HTML** 응답을 계속 제공할 수 있습니다. 배포가 이러한 아티팩트를 대체했으므로 캐시된 HTML에서 게시 계층에 더 이상 존재하지 않는 긴 캐시 clientlib URL(`lc-` 선택기 패턴)을 계속 참조할 수 있습니다.

이 경우 AEM as a Cloud Service은 **short-cache** clientlib URL에 대해 **HTTP 리디렉션**&#x200B;을 발행합니다. 짧은 캐시 URL은 `sc-` 선택기 패턴 및 시간 기반 세그먼트를 사용하므로 요청이 현재 사용 가능한 클라이언트 라이브러리의 **최신** 버전으로 확인될 수 있습니다.
...
짧은 캐시 URL에 대한 **리디렉션**&#x200B;과(와) **짧은 캐시 clientlib 응답** 자체가 모두 짧은 수명 상태입니다. 응답은 **최대 캐시 수명 60초**(`max-age=60`)을 사용합니다. **다운스트림 캐시**(고객 관리 CDN, 프록시 또는 유사)은 **이러한 응답을 이보다 오래 캐시해서는 안 됩니다**. TTL을 증가시키거나 짧은 캐시 clientlib URL을 변경할 수 없는 장기 자산으로 처리하는 사용자 지정 캐시 규칙을 사용하지 마십시오. 그렇게 하면 오래된 JS 또는 CSS에 사용자를 연결할 수 있습니다.

이 동작은 **클라이언트 라이브러리가 HTML과 역호환되어야 함**&#x200B;을 의미합니다. 이 라이브러리는 HTML이 캐시되는 동안 캐시에서 계속 제공될 수 있습니다. HTML 캐싱에서 보통 TTL을 사용할 때는 거의 문제가 되지 않습니다([위의 &#x200B;](#html-text)에 설명된 기본값 포함). **매우 긴** HTML 캐시 수명을 구성하는 경우 사용자가 짧은 캐시 대체 항목을 통해 전달된 최신 JS 및 CSS와 함께 이전 마크업을 로드할 수 있습니다.
