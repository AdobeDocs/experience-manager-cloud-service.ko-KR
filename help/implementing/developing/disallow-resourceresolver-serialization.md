...
슬링 모델 내보내기 기능을 사용하면 슬링 모델 개체를 JSON 형식으로 serialize할 수 있습니다. 이 기능은 SPA(단일 페이지 애플리케이션)에서 AEM의 데이터에 쉽게 액세스할 수 있도록 하므로 널리 사용됩니다. 구현 측면에서는 Jackson Databind 라이브러리를 사용하여 이러한 개체를 serialize합니다.
...
Adobe은 두 단계 접근 방식으로 `ResourceResolvers`의 serialization을 비활성화할 계획입니다.
...
1. 나중에 Adobe에서 `ResourceResolver`을(를) JSON으로 직렬화할 수 없게 됩니다.
...
Adobe은 모든 고객에게 애플리케이션 로그와 코드 베이스를 확인하여 이 문제의 영향을 받는지 확인하고 필요한 경우 사용자 지정 애플리케이션을 변경하여 이 경고 메시지가 더 이상 로그에 표시되지 않도록 합니다.
...
슬링 모델이 이 문제의 영향을 받지만 변경되지 않는 경우, `ResourceResolver` 개체의 serialization을 명시적으로 비활성화하면(두 번째 단계로 Adobe에서 실행됨) JSON 출력이 변경됩니다.
