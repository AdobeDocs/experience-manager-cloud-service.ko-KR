...
## SPA 개발 원칙 {#spa-development-principles-for-aem}

AEM에서 단일 페이지 애플리케이션을 개발하는 경우 프런트 엔드 개발자가 SPA을 만들 때 표준 모범 사례를 준수한다고 가정합니다. 프런트 엔드 개발자는 이러한 일반적인 모범 사례와 AEM별 원칙을 거의 따르지 않는 경우 SPA은 [AEM 및의 컨텐츠 작성 기능과 함께 사용할 수 있습니다](introduction.md#content-editing-experience-with-spa).
...
렌더링과 마찬가지로 모든 라우팅도 동적입니다. AEM [에서 SPA은 항상 라우팅](routing.md) 및 AEM을 소유하고에서 수신 대기하며 이에 따라 컨텐츠를 가져옵니다.
...
AEM에서 SPA [를 개발하는 원칙을](#spa-development-principles-for-aem) 따를 경우 SPA은 지원되는 모든 AEM 컨텐츠 제작 기능과 함께 작동합니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td>AEM은 SPA <a href="/help/implementing/developing/spa/reference-materials.md">Editor SDK 프레임워크를 사용하지 않고 헤드리스 CMS로 사용됩니다.</a></td>
...
>[!NOTE]
>
>모든 모델이 AEM에서 지원되지만 컨텐츠 작성자는 AEM에서 세 번째(그리고 AEM에서 [권장되는](#spa-development-principles-for-aem)SPA 개발 원칙)를 구현해야만 익숙하다면 SPA의 컨텐츠를 사용하면서 편집할 수 있습니다.

## 기존 SPA에서 AEM으로 마이그레이션 {#migrating-existing-spas-to-aem}

일반적으로 SPA에서 AEM의 [SPA 개발 원칙을](#spa-development-principles-for-aem)따를 경우 SPA은 AEM에서 작동하고 AEM 편집기를 사용하여 편집할 수 있습니다.
...
1. **응답형 구성 요소에서`this.props.cqModel`**
   ...
   구성 요소가 동의되고 JSON 모델이 적용되면 프런트 엔드 개발자는 SPA을 무료로 개발하고 JSON 모델에 간편하게 액세스할 수 있습니다 `this.props.cqModel`.
...
AEM용 SPA `MapTo` 을 일반적으로 사용하고 구축하는 방법에 대한 자세한 내용은 선택한 프레임워크에 대한 시작하기 안내서를 참조하십시오.
...
* [AEM에서 React를 사용하여 SPA 시작하기를](getting-started-react.md) 통해 React를 사용하여 AEM에서 SPA 편집기로 작동하는 기본 SPA이 어떻게 만들어지는지 보여줍니다.
