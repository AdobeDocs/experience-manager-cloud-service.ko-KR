...
이 연습은 표준 AEM 기능 및 샘플 WKND SPA Project 앱을 기반으로 합니다. 팔로우하려면 여기에서 GitHub에서 샘플 WKND SPA Project 앱을 [다운로드하여 설치하십시오.](https://github.com/adobe/aem-guides-wknd-spa)

>[!CAUTION]
>
>이 문서에서는 데모용으로만 [WKND SPA Project 앱을](https://github.com/adobe/aem-guides-wknd-spa) 사용합니다. 어떤 프로젝트 작업에도 사용해서는 안 됩니다.
...
## SPA의 컨텐츠 편집 경험 {#content-editing-experience-with-spa}
...
>
>이 작동 방식을 확인하려면 [SPA Apps 및 AEM Editor의 다음 섹션으로 계속하십시오](#spa-apps-and-the-aem-spa-editor).
...
1. WKND SPA Project 응용 프로그램을 게시 서버에 로드하거나 페이지 편집기의 [페이지 정보] 메뉴 **에서 [게시됨으로** 보기] 옵션 **을 사용하여** 로드합니다.
...
다음 섹션인 SPA [애플리케이션](#loading-a-spa-application)로드, SPA 로딩 역학 및 컨텐츠를 동기적으로 그리고 비동기적으로 로드하는 방법에 대해 자세히 알아봅니다.
...
1. 새 탭 `home.model.json` 에서 을 로드합니다.
   ...
   AEM SPA 편집기는 [AEM 컨텐츠](/help/assets/content-fragments/content-fragments.md) 서비스를 활용하여 페이지의 전체 컨텐츠를 JSON 모델로 전달합니다.
...
1. 편집기에서 WKND SPA Project 애플리케이션을 로드하고 미리 보기 **모드로** 전환합니다.
...
* [AEM에서 SPA 시작하기 - React를](getting-started-react.md) 사용하여 기본 SPA이 AEM에서 SPA 편집기와 연동되도록 구축된 방식을 살펴봅니다.
* [AEM에서 SPA 시작(Getting Started with Angular](getting-started-angular.md) )은 AEM에서 SPA 편집기로 작업할 기본 SPA이 어떻게 만들어지는지를 보여줍니다
* [SPA 편집자](editor-overview.md) 개요에서는 AEM과 SPA 간의 통신 모델에 대해 더 자세히 설명합니다.
* [AEM용 SPA](developing.md) 개발 과정에서는 프런트 엔드 개발자가 AEM용 SPA을 개발하는 방법뿐만 아니라 SPA과 AEM 아키텍처가 작동하는 방식을 설명합니다.
