...
SPA(단일 페이지 애플리케이션)은 종종 기본 애플리케이션과 마찬가지로 익숙한 방식으로 반응하고 동작하는 풍부하고 동적인 경험을 사용자에게 제공할 수 있습니다. [이는 클라이언트가 컨텐츠를 전방으로 로드한 다음, 사용자 인터랙션을 처리하](introduction.md#how-does-a-spa-work) 는 데 크게 의존하여 클라이언트와 서버 사이에 필요한 통신 양을 최소화함으로써, 앱을 보다 반응적으로 만들 수 있다.
...
[프로젝트에서 SSR](#when-to-use-ssr)을 구현해야 한다고 확신하는 경우 Adobe의 권장 해결 방법은 Adobe I/O Runtime을 사용하는 것입니다.
...
SSR을 사용할 때 AEM에서 SPA의 [구성 요소 상호 작용 워크플로우](introduction.md#interaction-with-the-spa-editor)에는 앱의 초기 컨텐츠가 Adobe I/O Runtime에서 생성되는 단계가 포함됩니다.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>
    <ul>
     <li>CSS 및 JavaScript와 같은 애플리케이션에 필요한 Clientlib 리소스는 <code><a href="/help/implementing/developing/introduction/clientlibs.md">allowProxy</a></code> 속성<br />을 통해 AEM 개발자가 사용할 수 있어야 합니다. </li>
