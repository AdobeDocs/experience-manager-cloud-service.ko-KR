...
AEM용 Javascript SPA SDK에서 구성 요소에 대한 동적 모델 매핑이 발생하는 방법에 대한 자세한 내용은 SPA[용 Dynamic Model to Component Mapping 문서를 참조하십시오.](model-to-component-mapping.md)
...
이렇게 하려면 모델 공급자가 [`PageModelManager`](#pagemodelmanager)에 등록해야 합니다. 그런 다음 변경 사항이 발생하면 업데이트된 데이터를 수신하여 위임 구성 요소에 전달합니다. 규칙으로 모델의 조각을 전달하는 위임 구성 요소에 사용할 수 있는 속성의 이름은 `cqModel`입니다. 구현은 구성 요소에 이 속성을 제공하는 데 무료이지만 프레임워크 아키텍처와의 통합, 발견 가능성 및 사용 편이성 등의 측면을 고려해야 합니다.
...
`Page` 구성 요소는 `Container` 구성 요소를 확장합니다. 컨테이너는 하위 페이지를 포함한 하위 구성 요소를 포함하고 렌더링하기 위한 구성 요소입니다. 이렇게 하려면 컨테이너가 모델의 `:itemsOrder`, `:items` 및 `:children` 속성을 반복합니다. `Page` 구성 요소는 [`ComponentMapping`](#componentmapping) 라이브러리의 저장소에서 자식 구성 요소를 동적으로 가져옵니다. `Page`은(는) 하위 구성 요소를 인스턴스화할 책임이 있습니다.
...
참고 항목: npm 리소스 [@adobe/aem-response-editable-components](https://www.npmjs.com/package/@adobe/aem-react-editable-components)
...
기본 [`Component Mapping`](#componentmapping) 라이브러리 및 그 `MapTo` 함수를 캡슐화하고 확장하여 현재 구성 요소 클래스와 함께 제공된 편집 구성에 관련된 기능을 제공할 수 있습니다.
...
위의 구현에서 프로젝트 구성 요소는 [구성 요소 매핑](#componentmapping) 저장소에 실제로 등록되기 전에 비어 있는 기능으로 확장됩니다. 이 작업은 `EditConfig` 구성 개체에 대한 지원을 제공하기 위해 [`ComponentMapping`](#componentmapping) 라이브러리를 캡슐화하고 확장함으로써 수행됩니다.
...
기본 [`PageModelManager`](#pagemodelmanager) 라이브러리 및 그 [`ModelRouter`](routing.md) 모듈(기본적으로 활성화됨)은 주어진 리소스 경로와 연관된 모델에 대한 사전 반입과 액세스를 제공할 책임이 있습니다.

두 개체는 라우팅 개념과 관련되지만 [`ModelRouter`](routing.md)은(는) 현재 응용 프로그램 상태와 동기화되는 데이터 모델을 사용하여 [`PageModelManager`](#pagemodelmanager)을(를) 로드할 책임이 있습니다.
...
* [SPA ](editor-overview.md) Editor 개요(AEM 및 통신 모델)
