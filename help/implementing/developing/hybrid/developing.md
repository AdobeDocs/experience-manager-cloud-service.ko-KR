...
AEM 작성 기능을 지원하지 않아도 되는 경우에는 다른 것을 고려하십시오 [SPA 디자인 모델](#spa-design-models).
...
특정 콘텐츠를 가리키는 명시적 렌더링은 정적 렌더링으로 간주되며 지원되지만 AEM 콘텐츠 작성 기능과 호환됩니다. 그것은 또한 의 원칙에 위배된다 [이식성](#portability).
...
>[!NOTE]
>
>모든 모델은 AEM에서 지원되지만, 세 번째 모델(권장 사항에 따라)을 구현해야만 합니다 [SPA 개발 원칙](#spa-development-principles-for-aem))는 AEM에서 SPA의 콘텐츠와 상호 작용하고 편집할 수 있는 콘텐츠 작성자입니다.

## 기존 SPA에서 AEM으로 마이그레이션 {#migrating-existing-spas-to-aem}

일반적으로 SPA이 [AEM용 SPA 개발 원칙](#spa-development-principles-for-aem)를 설치한 다음 SPA은 AEM에서 작동하며 AEM SPA 편집기를 사용하여 편집할 수 있습니다.
...
1. **구성 요소 구현 `render()` 방법**
   ...
   프론트엔드 개발자는 `render()` 메서드를 사용하여 조건에 맞는 `cqModel` 속성. 이 메서드는 페이지에 삽입된 DOM 및 HTML 조각을 출력합니다. 이 방법은 React에서 앱을 빌드하는 표준 방법이기도 합니다.
...
1. **HTML5를 사용하는 라우팅 솔루션 구현 `History` API.**
   ...
   다음의 경우 `ModelRouter` 이(가) 활성화됨, 호출 `pushState` 및 `replaceState` 함수는 다음에 대한 요청을 트리거합니다. `PageModelManager` 모델의 누락된 조각을 가져올 수 있습니다.

   의 현재 버전 `ModelRouter` 에서는 슬링 모델 진입점의 실제 리소스 경로를 가리키는 URL만 사용할 수 있습니다. vanity URL 또는 별칭의 사용을 지원하지 않습니다.
...
* **AEM 작성자**
  ...
  작성 환경에서 SPA 편집기를 사용하여 SPA을 편집하는 경우:
   ...
   1. 다음 `cq-data` 속성을 사용하면 편집기에서 구성 요소에 사용할 수 있는 편집 구성 사항을 알 수 있도록 추가 페이지 정보를 로드할 수 있습니다.
...
* [SPA 블루프린트](blueprint.md) 는 React 또는 Angular 이외의 프레임워크에 대해 SPA을 AEM에 구현하려는 경우 AEM용 SPA SDK가 작동하는 방식에 대해 자세히 알아봅니다. 또는 더 깊은 이해를 원할 수도 있습니다.
