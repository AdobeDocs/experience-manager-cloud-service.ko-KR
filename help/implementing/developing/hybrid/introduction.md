...
이 연습은 표준 AEM 기능과 샘플 WKND SPA Project 앱을 기반으로 합니다. 따라가려면 여기 GitHub에서 [샘플 WKND SPA Project 앱을 다운로드하여 설치하십시오.](https://github.com/adobe/aem-guides-wknd-spa)

>[!CAUTION]
>
>이 문서에서는 데모용으로만 [WKND SPA Project 앱](https://github.com/adobe/aem-guides-wknd-spa)을 사용합니다. 프로젝트 작업에 사용해서는 안 됩니다.
...
방문자가 모바일, 기본 앱에서 예상하는 것에 접근하는 보다 유연한 경험을 위해 SPA은 첫 번째 로드 시 방문자를 위해 필요한 모든 데이터를 로드합니다. 처음에는 시간이 조금 더 걸릴 수 있지만 그렇게 되면 추가 서버 호출이 필요하지 않습니다.
...
이 작동 방식을 보려면 이 문서 [SPA Apps 및 AEM SPA Editor](#spa-apps-and-the-aem-spa-editor)의 다음 섹션으로 이동하십시오.
...
1. 게시 서버에서 또는 페이지 편집기의 **페이지 정보** 메뉴에서 **게시됨으로 보기** 옵션을 사용하여 WKND SPA Project 애플리케이션을 로드합니다.
...
다음 섹션인 [SPA Application](#loading-a-spa-application)을 로드하면 SPA을 로드하는 역학과 컨텐츠를 동기적으로 그리고 비동기적으로 로드하는 방법에 대해 자세히 살펴봅니다.

### SPA 애플리케이션 로드 중 {#loading-a-spa-application}

1. 아직 로드되지 않은 경우 게시 서버에서 또는 페이지 편집기의 **페이지 정보** 메뉴에서 **게시됨으로 보기** 옵션을 사용하여 WKND SPA Project 앱을 로드하십시오.
...
1. 소스의 컨텐츠는 제한되어 있습니다.
   * 페이지의 본문 내에 콘텐츠가 없습니다. 주로 스타일시트와 `clientlib-react.min.js` 등의 다양한 스크립트에 대한 호출로 구성됩니다.
...
1. 새 탭에서 `home.model.json`을(를) 로드합니다.
...
1. 또한 모델은 페이지를 동기식으로 로드되도록 함께 그룹화하여 필요한 페이지 다시 로드 수를 줄일 수 있습니다.
   ...
   WKND SPA Project 앱의 예에서 방문자는 일반적으로 모든 페이지를 방문하므로 `home`, `page-1`, `page-2` 및 `page-3` 페이지가 동기식으로 로드됩니다.

   이 동작은 필수가 아니며, 완전히 정의할 수 있습니다.

   ![WKND SPA 프로젝트 항목 그룹화](assets/wknd-pages.png)

1. 동작의 이러한 차이를 보려면 `home` 페이지를 다시 로드하고 관리자의 네트워크 활동을 지웁니다. 페이지 메뉴에서 `page-1`으로 이동하여 `page-1` 이미지에 대한 요청만 네트워크 활동인지 확인합니다. `page-1` 자체적으로 로드할 필요가 없습니다.
...
1. 브라우저의 기본 제공 개발자 도구를 사용하여 페이지의 콘텐츠를 검사합니다. 선택 도구를 사용하여 페이지에서 편집 가능한 구성 요소를 선택하고 요소 세부 사항을 봅니다.
   ...
   >[!NOTE]
   이 동작은 편집 가능한 각 구성 요소에 대해 `cq` 요소가 삽입되는 AEM의 서버측 렌더링 페이지와 다릅니다.
...
* [Angular를 사용하여 AEM에서 SPA](getting-started-angular.md) 로 시작하기 는 Angular을 사용하여 AEM에서 SPA 편집기로 작동하기 위해 기본 SPA을 어떻게 빌드하는지 보여줍니다
* [SPA ](editor-overview.md) 편집기 개요는 AEM과 SPA 간의 통신 모델에 대해 자세히 설명합니다.
* [SPA for ](developing.md) AEM을 위한 개발 에서는 프런트엔드 개발자와 협력하여 AEM용 SPA을 개발하는 방법과 SPA이 AEM 아키텍처와 상호 작용하는 방법을 설명합니다.
