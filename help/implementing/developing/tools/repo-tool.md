...
파일 시스템의 `jcr_root` FileVault 구조 내에 지정된 경로에 대해 AEM Repo Tool은 전체 하위 트리에 대해 단일 필터를 사용하여 패키지를 만들고 이를 서버로 푸시하고(FTP `put`과(와) 유사), 서버에서 가져오거나(`get`) 차이점(`status` 및 `diff`)을 비교합니다.
