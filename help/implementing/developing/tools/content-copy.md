---
title: 콘텐츠 복사 도구
description: 콘텐츠 복사 도구를 사용하면 사용자가 필요할 때 AEM as a Cloud Service의 프로덕션 환경에서 낮은 환경으로 테스트 목적으로 변경 가능한 콘텐츠를 복사할 수 있습니다.
...
콘텐츠 복사 도구를 사용하면 사용자가 필요할 때 AEM as a Cloud Service의 프로덕션 환경에서 낮은 환경으로 테스트 목적으로 변경 가능한 콘텐츠를 복사할 수 있습니다.
...
콘텐츠 세트가 복사할 콘텐츠를 정의합니다. 콘텐츠 세트는 JCR 경로 목록으로 구성됩니다. 이러한 경로에는 동일한 Cloud Manager 프로그램 내에서 소스 작성 서비스 환경에서 타겟 작성 서비스 환경으로 복사할 수 있는 변경 가능한 콘텐츠가 포함되어 있습니다. 콘텐츠 세트에서 허용되는 경로는 다음과 같습니다.
...
* 소스 경로와 대상 경로가 일치하는 경우 소스의 컨텐츠가 대상 환경의 수정된 컨텐츠를 덮어씁니다.
...
1. 콘텐츠 세트를 세분화하거나 제한해야 하는 경우 하위 경로를 제외할 수 있습니다.
   ...
   1. 포함된 경로 목록에서 제한할 경로 옆에 있는 **제외 하위 경로 추가** 옵션을 클릭합니다.
...
1. 콘텐츠 단계를 생성할 때와 유사한 단계를 수행합니다. **콘텐츠 세트 추가**&#x200B;를 클릭하는 대신 콘솔에서 기존 세트를 선택하고 줄임표 메뉴에서 **편집**&#x200B;을 선택합니다.

![콘텐츠 세트 편집](assets/edit-content-set.png)

1. 콘텐츠 세트를 편집할 때 구성된 경로를 확장하여 제외된 하위 경로를 표시할 수 있습니다.
...
1. **개요** 페이지에서 **환경** > **콘텐츠 집합**&#x200B;으로 이동합니다.

1. 콘솔에서 콘텐츠 세트를 선택합니다.

1. 줄임표 메뉴에서 **콘텐츠 복사**&#x200B;를 클릭합니다.

   ![콘텐츠 복사](assets/copy-content.png)

   >[!NOTE]
   >
   >다음 중 하나에 해당하는 경우 환경을 선택할 수 없습니다.
   >
   >* 사용자에게 적절한 권한이 없습니다.
...
1. (선택 사항) 원하는 경우 다음을 설정합니다.

   * **액세스 제어 목록 포함** - 콘텐츠와 함께 콘텐츠의 액세스 제어 권한을 복사하려면 선택합니다.
   * **지우기** - 가져오기를 시작하기 전에 대상에서 기존 콘텐츠를 삭제하려면 선택하십시오. 따라서 깨끗한 상태에서 시작하여 기존 콘텐츠와의 충돌을 피할 수 있습니다. **지우기**&#x200B;를 선택 해제한 상태로 두면 Cloud Manager은 기존 대상 콘텐츠 위에 새 콘텐츠를 가져옵니다. 지우기가 시작되기 전에 확인 메시지가 나타나고 Cloud Manager은 지우기 작업과 추적을 위한 가져오기 세부 사항을 기록합니다.
     ![콘텐츠 복사](assets/content_copy_wipe-destination.png)
      * **가져오기 전에 대상 지우기** 옵션을 선택하고 **복사**&#x200B;를 클릭하면 팝업이 표시되고 옵션을 사용할 수 있습니다.
         * **취소**(이 경우 **콘텐츠 복사** 흐름이 시작되지 않음)
         * **확인**(**콘텐츠 복사** 흐름이 시작되고 대상의 콘텐츠가 지워짐)
           ![콘텐츠 복사](assets/content-copy-wipe-destination-warning.png)

      * **가져오기 전에 대상 지우기**&#x200B;를 선택하지 않으면 **콘텐츠 복사** 흐름이 이전과 같이 작동합니다.
...
### 액세스 로그 {#accessing-logs}

완료된 콘텐츠 복사 프로세스에 대해 소스 및 대상 환경의 로그를 모두 확인할 수 있습니다.

**로그에 액세스하려면:**

1. **콘텐츠 복사 활동** 페이지의 줄임표 메뉴에서 검토할 복사 프로세스에 대한 **로그**&#x200B;를 클릭합니다. 그런 다음 환경을 선택합니다.

![콘텐츠 복사 프로세스에 대한 로그 액세스](assets/copy-content-logs.png)

로그는 로컬 시스템으로 다운로드됩니다.

1. 다운로드가 시작되지 않으면 팝업 차단 설정을 확인하십시오.
...
* [콘텐츠 조각 모델](/help/sites-cloud/administering/content-fragments/content-fragment-models.md#data-types)은(는) UUID(범용 고유 ID)를 기반으로 참조 필드를 지정할 수 있습니다. 이러한 UUID는 저장소별로 다르므로 컨텐츠 복사 도구는 컨텐츠 조각을 복사할 때 대상 환경에서 이러한 UUID를 다시 계산합니다.
