---
title: 콘텐츠 복사 도구
description: 컨텐츠 복사 도구를 사용하면 AEM as a Cloud Service 프로덕션 환경에서 온디맨드로 변경할 수 있는 컨텐츠를 테스트 목적으로 낮은 환경에 복사할 수 있습니다.
...
컨텐츠 복사 도구를 사용하면 AEM as a Cloud Service 프로덕션 환경에서 온디맨드로 변경할 수 있는 컨텐츠를 테스트 목적으로 낮은 환경에 복사할 수 있습니다.

## 소개 {#introduction}

현재의 실제 데이터는 테스트, 검증 및 사용자 승인 목적에 유용합니다. 컨텐츠 복사 도구를 사용하여 프로덕션 AEM 환경의 컨텐츠를 스테이징, 개발 또는 스테이징에 복사할 수 있습니다 [신속한 개발 환경(RDE)](/help/implementing/developing/introduction/rapid-development-environments.md) 환경 을 참조하십시오.

복사할 콘텐츠는 콘텐츠 세트에 의해 정의됩니다. 컨텐츠 세트는 소스 작성 서비스 환경에서 동일한 Cloud Manager 프로그램 내의 대상 작성 서비스 환경으로 복사할 가변 컨텐츠가 들어 있는 JCR 경로 목록으로 구성됩니다. 콘텐츠 세트에서 허용되는 경로는 다음과 같습니다.
...
콘텐츠 복사 도구를 사용하려면 소스 환경과 대상 환경 모두에서 특정 권한이 필요합니다.
...
컨텐츠를 복사하려면 먼저 컨텐츠 세트를 정의해야 합니다. 정의되면 콘텐츠 세트를 재사용하여 콘텐츠를 복사할 수 있습니다. 콘텐츠 세트를 만들려면 다음 단계를 따르십시오.
...
1. **콘텐츠 복사** 대화 상자에서 콘텐츠 복사 작업의 소스 및 대상을 지정합니다.
   ...
   * 컨텐츠는 상위 환경에서 하위 환경으로 또는 환경의 계층 구조가 다음과 같은 개발/RDE 환경 사이에서만 복사할 수 있습니다(가장 높은 환경에서 가장 낮은 환경에서만).
      * 프로덕션
      * 스테이징
      * 개발/RDE

1. 필요한 경우 다음을 선택할 수도 있습니다 **액세스 제어 목록 포함** 복사 프로세스에서 사용할 수 있습니다.
...
| 취소됨 | 사용자가 컨텐츠 복사 작업을 시작한 후 작업을 취소합니다 |

### 복사 프로세스 취소 {#cancelling}

컨텐츠 복사 작업을 시작한 후 중지해야 하는 경우에는 취소할 수 있습니다.

이렇게 하려면 **컨텐츠 활동 복사** 페이지에서 을 선택합니다 **취소** 이전에 시작한 복사 프로세스의 줄임표 메뉴에서 작업을 수행합니다.

![컨텐츠 복사 취소](assets/content-copy-cancel.png)

>[!NOTE]
>
>컨텐츠 복사 작업을 취소하면 대상 환경에서 컨텐츠의 부분 복사본이 발생할 수 있습니다. 이렇게 하면 대상 환경을 사용할 수 없는 상태로 만들 수 있습니다.
>
>취소로 인해 환경이 이러한 상태에 있는 경우 Adobe 고객 지원 센터에 문의하여 지원을 받으십시오.

## 제한 사항 {#limitations}

콘텐츠 복사 도구에는 다음과 같은 제한 사항이 있습니다.

* 컨텐츠는 하위 환경에서 더 높은 환경으로 복사할 수 없습니다.
* 컨텐츠는 작성 서비스에서만 복사할 수 있습니다.
...
* 콘텐츠 복사 도구에는 버전 관리 기능이 없으며 마지막 콘텐츠 복사 작업 이후 콘텐츠 세트의 소스 환경에서 변경되거나 새로 생성된 콘텐츠를 자동으로 감지할 수 없습니다.
   * 마지막 컨텐츠 복사 작업 이후에 컨텐츠 변경 사항만 사용하여 대상 환경을 업데이트하려면 컨텐츠 세트를 만들고 마지막 컨텐츠 복사 작업 이후 변경된 소스 인스턴스에 경로를 지정해야 합니다.
