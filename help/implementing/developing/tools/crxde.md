...
**홈** 탭에서는 컨텐츠 및/또는 설명서를 검색하고 개발자 설명서 및 Adobe 지원에 액세스할 수 있습니다.
...
* **서버 구성**  - Felix Console에 액세스할 수 있습니다(다음 링크도  `https://<host>:<port>/system/console/configMgr`있음).
* **쿼리**  - 저장소 쿼리
...
클릭하여 로그인하거나 다른 사용자로 다시 로그인합니다. `@crx.default`은 사용자가 저장소의 기본(및 전용) 작업 공간에 있음을 나타냅니다.
...
1. 브라우저에서 CRXDE Lite을 엽니다.
1. [**Explorer 창**,](#explorer-pane)에서 새 노드를 만들려는 노드를 마우스 오른쪽 단추로 클릭하고 **만들기**&#x200B;를 선택한 다음 **노드 만들기**&#x200B;를 선택합니다.
1. **이름**&#x200B;을 입력하고 **유형**&#x200B;을 선택합니다.
1. **확인**&#x200B;을 클릭합니다.
1. 서버에 변경 내용을 저장하려면 [**모든 저장 단추**](#save-all-button)&#x200B;를 클릭합니다.

이제 속성을 수정하거나 새 노드를 만들어 노드를 필요에 맞게 적용할 수 있습니다.

>[!NOTE]
**노드 만들기**&#x200B;를 비롯한 대부분의 편집 작업은 메모리에 모든 변경 사항을 저장하고 저장 시 저장소에만 저장합니다([**모든 단추 저장**](#save-all-button) 사용). 그러나 이동 등의 일부 작업은 자동으로 지속됩니다.
...
1. 브라우저에서 CRXDE Lite을 엽니다.
1. [**익스플로러 창**,](#explorer-pane)에서 새 속성을 추가할 노드를 선택합니다.
...
1. 브라우저에서 CRXDE Lite을 엽니다.
1. [**Exploader 창**,](#explorer-pane)에서 파일을 만들 구성 요소를 마우스 오른쪽 단추로 클릭하고 **만들기**&#x200B;를 선택한 다음 **파일 만들기**&#x200B;를 선택합니다.
1. 확장명을 포함하는 **이름** 파일을 입력합니다.
1. **확인**&#x200B;을 클릭합니다.
1. 새 파일이 [**편집 창**&#x200B;에서 탭으로 열립니다.](#edit-pane)
1. 파일을 편집합니다.
1. [**모든 저장 단추**](#save-all-button)&#x200B;를 클릭하여 변경 내용을 저장합니다.
...
1. [**편집 창**](#edit-pane)&#x200B;가져오기 노드 유형&#x200B;**이라는 레이블이 지정된 새 탭이 열립니다.**
...
1. 브라우저에서 CRXDE Lite을 엽니다.
1. 창 아래쪽에 있는 [**콘솔 탭**](#console-tab) 오른쪽의 드롭다운 메뉴에서 **서버 로그**&#x200B;를 선택합니다.
