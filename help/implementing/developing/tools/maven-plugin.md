...
>[!TIP]
>
>자세한 내용은 AEM의 [AEM 프로젝트 구조](https://docs.adobe.com/content/help/ko-KR/experience-manager-cloud-service/implementing/developing/aem-project-content-package-structure.html) 아티클을 Cloud Service 설명서로 참조하고 [AEM Project Tranype](https://docs.adobe.com/content/help/ko-KR/experience-manager-core-components/using/developing/archetype/overview.html) 설명서를 참조하십시오. 두 가지 모두 AEM 6.5에서 완벽하게 지원됩니다.

## Content Package Maven Plugin {#obtaining-the-content-package-maven-plugin} 입수
...
Maven이 플러그인을 다운로드할 수 있도록 하려면 이 페이지의 [Obging the Content Package Maven Plugin](#obtaining-the-content-package-maven-plugin) 섹션에 제공된 프로필을 사용하십시오.
...
별도의 언급이 없는 한 플러그인 목표와 매개 변수는 다음 예와 같이 `vault` 접두어를 사용합니다.
...
다음 표의 매개 변수는 **Goals** 열에 언급된 경우를 제외하고 모든 목표에 공통입니다.
...
| `password` | `String` | 예 | `admin` | AEM 인증에 사용되는 암호 | `package`을 제외한 모든 목표 |
...
| `useProxy` | `boolean` | 아니오 | `true` | `true` 값이 있으면 Maven이 패키지 관리자에 대한 프록시 요청을 위해 검색된 첫 번째 활성 프록시 구성을 사용하게 됩니다. | `package`을 제외한 모든 목표 |
...
다음 매개 변수 외에 [공통 매개 변수](#common-parameters) 섹션의 `name` 매개 변수에 대한 설명을 참조하십시오.
...
| `failOnMissingEmbed` | `boolean` | 예 | `false` | `true`의 값은 포함된 아티팩트가 프로젝트 종속성에서 없으면 빌드가 실패합니다. 값이 `false`이면 빌드가 이러한 오류를 무시합니다. |
...
| `finalName` | `java.lang.String` | 예 | Maven 프로젝트에 정의된 `finalName`(빌드 단계) | 생성된 패키지 ZIP 파일의 이름( `.zip` 파일 확장자 없음) |
| `group` | `java.lang.String` | 예 | Maven 프로젝트에 정의된 `groupID` | 콘텐츠 패키지의 대상 설치 경로에 포함된 생성된 콘텐츠 패키지의 `groupId` |
...
| `properties` | `java.util.Map` | 아니오 | 없음 | 이러한 매개 변수는 `properties.xml` 파일에서 설정할 수 있는 추가 속성을 정의합니다. 이러한 속성은 다음과 같은 사전 정의된 속성을 덮어쓸 수 없습니다.`group`(설정하는 동안 `group` 매개 변수 사용), `name`(설정하는 데 `name` 매개 변수 사용), `version`(설정하는 데 `version` 매개 변수 사용), `description`(프로젝트 설명에 의해 설정됨), `groupId`(`groupId` MAVEN 프로젝트 설명자의 `artifactId`), &lt;a9/> 마스터 프로젝트 설명자), `dependencies`(설정할 `dependencies` 매개 변수 사용), `createdBy`(시스템 속성의 값), `created`(현재 시스템 시간), `requiresRoot` (설정할 `requiresRoot` 매개 변수 사용), `packagePath`(그룹 및 패키지 이름에서 자동으로 생성)`artifactId``user.name` |
...
필터 요소를 사용하여 패키지 컨텐츠를 정의합니다. 필터는 패키지의 `META-INF/vault/filter.xml` 파일의 `workspaceFilter` 요소에 추가됩니다.
...
소스 파일은 파일 시스템의 어느 위치에서나 찾을 수 있습니다. POM 파일에서 소스 파일을 `target/vault-work/META-INF`에 복사하여 패키지에 포함할 빌드 리소스를 정의합니다.
...
다음 POM 코드는 패키지에 축소판 이미지만 추가합니다. 축소판 이미지의 이름은 `thumbnail.png`이어야 하며 패키지의 `META-INF/vault/definition` 폴더에 있어야 합니다. 이 예에서 소스 파일은 프로젝트의 `/src/main/content/META-INF/vault/definition` 폴더에 있습니다.
...
## AEM 프로젝트 원형 사용을 사용하여 AEM 프로젝트 생성 {#using-archetypes}

최신 AEM Project Tranype은 온-프레미스와 AMS 구현 모두에 대한 모범 사례 패키지 구조를 구현하며 모든 AEM 프로젝트에 권장됩니다.

>[!TIP]
>
>자세한 내용은 AEM의 [AEM 프로젝트 구조](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/developing/aem-project-content-package-structure.html) 아티클을 Cloud Service 설명서로 참조하고 [AEM Project Tranype](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/overview.html) 설명서를 참조하십시오. 두 가지 모두 AEM 6.5에서 완벽하게 지원됩니다.
