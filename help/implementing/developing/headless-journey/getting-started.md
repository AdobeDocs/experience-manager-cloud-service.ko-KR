...
# AEM Headless를 Cloud Service {#getting-started} 로 시작하기
...
AEM Headless는 다음 섹션에 설명된 [의 헤드리스 컨텐츠를 관리하기 위한 강력한 도구를 제공하여 이 기술 기반을 구축합니다.](#aem-headless-basics)

## AEM Headless 기본 사항 {#aem-headless-basics}

AEM의 헤드리스 기능은 몇 가지 주요 기능을 기반으로 합니다. 이러한 내용은 여정 뒷부분에서 자세히 설명합니다. 이제 그들이 무엇을 하고 그들이 뭐라고 불리는지에 대한 기본 사항만 아는 것이 중요하다.
...
그러나 AEM 헤드리스 기능을 이해하면 열려 있는 추가 하이브리드 가능성을 알고 있어야 합니다. 우리는 이 사건들을 당신의 인식을 위해 아래와 같이 정리합니다. 여정 종료 시 프로젝트에 이러한 유연성이 필요한 경우 이러한 개념을 보다 자세히 소개하게 됩니다.
...
#### 레벨 2:AEM - 하이브리드 모델 {#level-2}에 SPA 포함
...
#### 레벨 3:AEM에서 SPA 포함 및 완전히 활성화 - 하이브리드 모델 {#level-3}
...
프로젝트 시작 부분에 번역 및 지역화를 계획합니다. &quot;국제화 프로젝트 관리자&quot;를 번역해야 하는 컨텐츠와 번역되지 않아야 하는 내용, 지역 또는 지역 컨텐츠 제작자가 수정할 수 있는 번역된 컨텐츠를 정의하는 책임을 지는 별도의 성향으로 간주합니다.
...
[컨텐츠 계층](#content-hierarchy)을 활용하여 현지화를 보다 쉽게 할 수 있습니다.

AEM 워크플로우 및 로컬라이제이션 도구에 대한 추가 설명서는 [추가 리소스](#additional-resources) 섹션을 참조하십시오.

##### 컨텐츠 계층 구조 활용 {#content-hierarchy}

폴더 계층 구조 는 컨텐츠 관리와 관련하여 두 가지 주요 문제를 해결할 수 있습니다.

* [로컬라이제이션](#localization)  - AEM은 로케일 특정 폴더에 컨텐츠 사본을 유지 관리하여 컨텐츠의 로컬라이제이션을 관리합니다.
* 조직 - 폴더는 로컬라이제이션 요구 사항을 지원하고 컨텐츠 조각을 논리적으로 관리하는 데 필요한 컨텐츠 계층 구조를 정의하는 데 사용됩니다.

AEM을 사용하면 매우 유연한 컨텐츠 구조를 만들 수 있으며 계층 구조가 임의로 클 수 있습니다. 그러나 폴더 구조의 모든 변경 사항에 [이 컨텐츠 경로에 의존하는 기존 쿼리에 의도하지 않은 결과가 발생할 수 있다는 것을 알고 있어야 합니다.](#developer) 따라서 미리 명확하게 설정된 잘 정의된 계층 구조가 컨텐츠 작성자에게 매우 유용할 수 있습니다.

폴더를 특정 컨텐츠 유형(컨텐츠 조각 모델 기반)만 허용하도록 제한할 수도 있습니다. 일반적으로 계층의 모든 폴더에 대해 허용되는 모델을 항상 명시적으로 지정하는 것이 좋습니다. 지정된 폴더에 대해 허용되는 컨텐츠 지정:

* 컨텐츠 작성자가 폴더에 속하지 않는 컨텐츠를 작성할 수 없습니다.
* 만드는 동안 폴더에 허용된 컨텐츠 유형을 필터링하여 유효한 컨텐츠 유형만 표시하도록 컨텐츠 생성 프로세스를 최적화합니다.

적절한 컨텐츠 구조를 만들면 컨텐츠 재사용을 최대화하기 위해 채널 간에 헤드리스 컨텐츠 작성을 조정하기가 쉬워집니다. 여러 채널에 컨텐츠를 활용하면 컨텐츠 제작 효율성과 변경 관리 기능이 크게 향상됩니다.
...
##### 컨텐츠 중첩 {#content-nesting} 과별하지 않음

[컨텐츠 ](#content-fragments) 조각은 헤드리스 컨텐츠를 만드는 데 AEM에서 사용됩니다. AEM에서는 컨텐츠 조각에 대해 최대 10개의 컨텐츠 중첩 수준을 지원합니다. 하지만 AEM에서는 상위 컨텐츠 조각에 정의된 각 참조를 반복적으로 확인한 다음 모든 동일 수준에 하위 참조가 있는지 확인해야 한다는 것을 명심해야 합니다. 이러한 작업은 신속하게 추가되어 성능 문제가 될 수 있습니다.

일반적인 경험상 컨텐츠 조각 참조는 5개 수준 이상으로 중첩되어서는 안 됩니다.

#### 컨텐츠 모델러 {#content-modeler}

컨텐츠 모델러는 헤드리스 없이 전달해야 하는 데이터에 대한 요구 사항을 분석하고 이 데이터의 구조를 정의합니다. 이러한 구조를 AEM에서 [컨텐츠 조각 모델](#content-fragment-models)이라고 합니다. 컨텐츠 조각 모델은 컨텐츠 작성자가 만드는 컨텐츠 조각의 기초로 사용됩니다.

컨텐츠 조각 모델을 정의할 때 유용한 접근 방법은 컨텐츠를 사용할 애플리케이션의 UX 구성 요소에 매핑되는 모델을 만드는 것입니다.
...
* GraphQL은 &quot;필요한 사항을 정확히 확인하고 정확하게 파악할 수 있습니다&quot;라는 모토에 따라 선언적으로 설계되었습니다. 즉, GraphQL 쿼리를 만들 때 항상 관계형 데이터베이스에서 만들 수 있는 `select *` 유형 쿼리를 사용하지 않습니다.
...
경험의 여러 섹션에서 콘텐츠 업데이트 주파수가 서로 다른 경우가 많습니다. CDN 및 캐시 구성을 미세 조정할 수 있어야 합니다. 또한 [컨텐츠 모델러](#content-modeler)는 컨텐츠를 표시하기 위한 모델을 설계하므로 이 입력도 중요합니다. 다음 사항을 고려하십시오.
