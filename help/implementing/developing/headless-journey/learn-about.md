...
전체 스택 접근 방식은 모든 경험이 하나의 시스템에 있는 사일로를 생성합니다. 사일로의 한 수준에서 변경 또는 추가 작업을 수행하려면 다른 레이어에서 변경 사항이 필요하므로 시간이 많이 걸리고 비용이 많이 소요됩니다.
...
이러한 디커플링은 각 소비 서비스(프런트 엔드)가 API를 통해 전달되는 동일한 컨텐츠를 기반으로 경험을 구축할 수 있으므로 컨텐츠를 재사용하고 일관성을 유지할 수 있음을 의미합니다. 그런 다음 서비스를 사용하면 고유한 프레젠테이션 레이어를 구현할 수 있으므로 컨텐츠 관리 레이어(백엔드)를 가로로 손쉽게 확장할 수 있습니다.
