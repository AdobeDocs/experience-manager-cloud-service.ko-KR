...
AEM 헤드리스 여정의 이전 문서인 [컨텐츠를 모델링하는 방법](model-your-content.md) AEM에서 데이터 모델링에 대한 기본 사항을 배웠으므로 이제 컨텐트 구조를 모델링하는 방법을 이해하고 AEM 컨텐츠 조각 모델 및 컨텐츠 조각을 사용하여 해당 구조를 구현해야 합니다.

* [데이터 모델링](#data-modeling)과 관련된 개념과 용어를 인식합니다.
* 헤드리스 컨텐츠 전달](#data-modeling-for-aem-headless)에 데이터 모델링이 필요한 이유를 파악합니다.[
* AEM 컨텐츠 조각 모델](#create-structure-content-fragment-models)을 사용하여 이 구조를 실현하는 방법을 이해합니다(및 [컨텐츠 조각](#use-content-to-author-content)을 사용하여 컨텐츠를 작성하십시오).[
* [컨텐트 모델 지정 방법 이해](#getting-started-examples);기본 견본과 원칙.
...
## 데이터에 액세스하시겠습니까?{#so-youd-like-to-access-your-data}
...
즉, AEM과 함께 사용할 때 필요한 데이터만 정확하고 안전하게 효율적으로 선택할 수 있습니다.
...
AEM GraphQL API는 컨텐츠 조각에 대한 (복잡한) 쿼리를 수행할 수 있도록 특별히 구성된 표준 GraphQL API의 사용자 정의된 버전입니다.
...
GraphQL은 강력한 형식의 API로, 데이터를 유형별로 명확히 구조화하고 구성해야 합니다. GraphQL 사양에서는 특정 인스턴스에서 데이터를 심문하기 위한 강력한 API를 만드는 방법에 대한 일련의 지침을 제공합니다. 이렇게 하려면 클라이언트는 쿼리에 필요한 모든 유형을 포함하는 스키마를 반입해야 합니다.
...
테넌트 끝점의 경우 경로는 비교할 수 있습니다.

`/content/cq:graphql/your-tenant/endpoint`
`/content/_cq_graphql/your-tenant/endpoint.json`

사용하기 전에 모든 끝점을 활성화해야 합니다. AEM용 GraphQL에 대해 끝점, 글로벌 또는 테넌트를 활성화하려면 다음을 수행해야 합니다.
...
GraphQL 끝점을 활성화하려면 먼저 구성 브라우저에서 적절한 구성이 있어야 합니다.
...
### AEM GraphQL 스키마 {#aem-graphql-schemas}

데이터에 액세스하려면 먼저 필요한 컨텐츠 조각 모델 유형(GraphQL 스키마로 표시됨)을 선택해야 합니다. AEM GraphQL 스키마는 GraphQL 쿼리에서 사용하기 위해 컨텐츠 조각 모델에서 파생됩니다.

<!--
Confirm is the schema city or CityModel? -->

`City`이라는 컨텐츠 조각 모델이 있는 경우 `city`이라는 해당 스키마가 있습니다. 다음 쿼리를 사용하여 이 모델을 기반으로 모든 콘텐츠 조각의 `name`을(를) 나열할 수 있습니다.
...
다음 쿼리를 사용하여 모든 사용 가능한 스키마에 대해 모든 `types`의 `name` 및 `description`을(를) 나열할 수 있습니다.
...
### AEM GraphQL 필드 {#aem-graphql-fields}

필요한 스키마를 선택하면 스키마의 필드로 표시되는 특정 데이터에 액세스하게 됩니다.
...
* AEM용 GraphQL은 또한 여러 개의 도우미 필드를 생성합니다.
...
| 열거 | 문자열 | 모델 생성 시 정의된 옵션 목록에서 옵션을 표시하는 데 사용됩니다. |
...
>[!NOTE]
>
>메타데이터에 대한 샘플 쿼리 - 제목이 GB인 시상식에 대한 메타데이터 목록을 참조하십시오.
...
>[!NOTE]
>
>샘플 쿼리 - 명명된 변화가 있는 모든 도시를 참조하십시오.

### AEM GraphQL 변수 {#aem-graphql-variables}

변수는 여러 값을 한 위치에 저장할 수 있으므로 프로그래밍 또는 쿼리 형식의 모든 형식으로 유용합니다. AEM GraphQL의 경우, 각 값에 대해 별도의 쿼리를 작성하지 않고도 하나의 변수에 대한 쿼리를 작성할 수 있으며 이 변수의 값은 필요에 따라 변경할 수 있습니다.

GraphQL은 쿼리에 변수를 배치할 수 있도록 허용합니다.

>[!NOTE]
>
>자세한 내용은 변수에 대한 GraphQL 설명서를 참조하십시오.

예를 들어 특정 변형이 있는 `Article` 유형의 모든 컨텐츠 조각을 가져오려면 GraphiQL에서 `variation` 변수를 지정할 수 있습니다. 모든 인스턴스는 `GetArticlesByVariation`에서 가져온 다음 `articleList`에서 사용하도록 전달됩니다.

![GraphQL ](assets/graphqlapi-variables.png "변수GraphQL 변수")
...
### AEM GraphQL 지시문 {#aem-graphql-directives}
...
!![GraphQL Directives](assets/graphqlapi-directives .png &quot;GraphQL 지시어&quot;)
...
### AEM GraphQL 필터 {#aem-graphql-filters}
...
* AEM 확장용 GraphQL 상세 정보

* 이 샘플 컨텐츠 및 구조를 사용한 샘플 쿼리

### AEM GraphQL 확장 {#aem-graphql-extensions}
...
지속적인 쿼리는 항상 적절한(테넌트) 구성과 관련된 끝점을 사용해야 합니다.둘 중 하나 또는 둘 다를 사용할 수 있습니다.
...
>[!NOTE]
>
>자세한 내용은 구성 브라우저에서 컨텐츠 조각 기능 활성화를 참조하십시오.
>
...
>[!NOTE]
>
>AEM의 CORS 리소스 공유 정책에 대한 자세한 개요는 CORS(교차 도메인 리소스 공유) 이해를 참조하십시오.
...
### AEM GraphQL 인증 {#aem-graphql-authentication}

AEM(Cloud Service) GraphQL API for Content Fragment 배달을 위한 Adobe Experience Manager의 기본 사용 사례는 제3자 응용 프로그램 또는 서비스의 원격 쿼리를 승인하는 것입니다. 이러한 원격 쿼리는 헤드리스 컨텐츠 전달을 보호하기 위해 인증된 API 액세스가 필요할 수 있습니다.

>[!NOTE]
>
>테스트 및 개발의 경우 GraphiQL 인터페이스 인터페이스를 사용하여 AEM GraphQL API에 직접 액세스할 수도 있습니다.

인증을 위해 제3자 서비스는 액세스 토큰을 사용해야 하며, 이 토큰은 GraphQL 요청에서 사용할 수 있습니다.

#### 액세스 토큰 {#retrieving-access-token} 검색

자세한 내용은 서버측 API에 대한 액세스 토큰 생성을 참조하십시오.
...
## AEM GraphQL 쿼리 샘플 {#samples-aem-graphql-queries}

포괄적인 샘플 쿼리에 대해서는 AEM에서 GraphQL 사용 방법 학습 - 샘플 컨텐츠 및 쿼리를 참조하십시오.
