...
[경험 조각](/help/sites-cloud/authoring/fragments/content-fragments.md)은(는) 페이지 내에서 참조할 수 있는 콘텐츠 및 레이아웃을 포함한 하나 이상의 구성 요소 그룹입니다.

경험 조각 기본, 변형 또는 두 가지 모두 는 다음을 사용합니다.

* `sling:resourceType` : `/libs/cq/experience-fragments/components/xfpage`

`/libs/cq/experience-fragments/components/xfpage/xfpage.html`이(가) 없으므로 다음으로 되돌아감
...
>
>자세한 내용은 [URL 외부화](/help/implementing/developing/tools/externalizer.md)를 참조하십시오.

![일반 HTML 렌디션](assets/xf-14.png)

일반 렌디션 선택기에서는 추가 스크립트와 반대로 변환기를 사용합니다. [Sling 재작성기](https://sling.apache.org/documentation/bundles/output-rewriting-pipelines-org-apache-sling-rewriter.html)가 변환기로 사용됩니다. 이 변환기는 다음 위치에 구성됩니다.
...
Adobe은 오버레이를 사용하여 재작성기를 구성할 것을 권장합니다. [AEM as a Cloud Service의 오버레이](/help/implementing/developing/introduction/overlays.md)를 참조하세요.

## 경험 조각용 템플릿 {#templates-for-experience-fragments}

>[!CAUTION]
>
>경험 조각에는 ***편집 가능한 템플릿***&#x200B;만 지원됩니다.
>
...
1. [허용된 템플릿](/help/sites-cloud/authoring/fragments/content-fragments.md#configure-allowed-templates-folder)은(는) 경험 조각 콘솔에서 구성할 수 있습니다.
...
기본적으로 Target HTML 오퍼를 구성하면 요청이 AEM의 사용자 지정 Sling 선택기로 전송됩니다. 이 선택기를 `.nocloudconfigs.html`이라고 합니다. 이름에서 알 수 있듯이, 경험 조각의 일반 HTML 렌더링을 만들지만, 클라우드 구성(불필요한 정보)은 포함하지 않습니다.
...
2. AEM은 HTML에 있는 모든 내부 링크를 수정하여 게시된 리소스를 가리키도록 합니다.
   ...
   1. `src` 특성
   2. `href` 특성
   3. `*-src` 특성(예: `data-src` 및 `custom-src`)
   4. `*-href` 특성(예: `data-href`, `custom-href` 및 `img-href`)

   >[!NOTE]
   >
   >HTML의 내부 링크는 상대 링크이지만 사용자 지정 구성 요소가 HTML에 전체 URL을 제공하는 경우가 있을 수 있습니다. 기본적으로 AEM은 이러한 완전한 URL을 무시하고 수정하지 않습니다.

   이러한 특성의 링크는 AEM Link 외부화 `publishLink()`을(를) 통해 실행되어 게시된 인스턴스에 있는 것처럼 URL을 다시 만들 수 있습니다. 따라서 공개적으로 사용할 수 있습니다.
...
[default](#default-link-rewriting)에서 다루지 않는 더 복잡한 경우에는 AEM에서 링크 재작성기 공급자 인터페이스를 제공합니다. 이 인터페이스는 번들에서 서비스로 구현할 수 있는 `ConsumerType` 인터페이스입니다. AEM이 경험 조각에서 렌더링한 대로 HTML 오퍼의 내부 링크에 대해 수행하는 수정 사항을 무시합니다. 이 인터페이스를 사용하면 비즈니스 요구 사항에 맞게 내부 HTML 링크를 다시 작성하는 프로세스를 사용자 지정할 수 있습니다.
...
* `sling:alias mechanisms`이(가) 리소스에 대해 배치되어 있습니다.
...
* `link`
처리 중인 링크의 `String` 표현입니다. 이 표현은 일반적으로 작성자 인스턴스의 리소스를 가리키는 상대 URL입니다.
