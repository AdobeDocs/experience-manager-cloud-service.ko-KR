---
title: 경험 구성요소
description: 클라우드 서비스 경험 조각으로 Adobe Experience Manager 확장
...
이러한 그룹에 대한 사용 사례 중 하나는 Adobe Target과 같은 타사 터치포인트에 컨텐츠를 포함시키는 것입니다.
...
링크 외부화 기능은 Target 오퍼의 HTML 버전을 만들 때 필요한 올바른 URL을 결정하는 데 사용되며, 이 URL은 이후 Adobe Target으로 전송됩니다. 이는 Adobe Target에서 Target HTML 오퍼 내의 모든 링크에 공개적으로 액세스할 수 있어야 하기 때문에 필요합니다. 즉, 링크 참조 및 경험 조각 자체는 게시되어야 사용할 수 있습니다.

기본적으로 타겟 HTML 오퍼를 만들 때 AEM의 사용자 지정 Sling 선택기로 요청이 전송됩니다. 이 선택기를 호출합니다 `.nocloudconfigs.html`. 이름이 암시하듯이 경험 조각에 대한 일반 HTML 렌더링을 만들지만 클라우드 구성(탁월한 정보)은 포함하지 않습니다.
...
1. 요소 `html`, `head`및 `body` 요소는 `div` 요소로 대체됩니다. 및 `meta`요소는 제거됩니다(원래 `noscript` 요소의 하위 요소이며, 이 요소가 `title` `head` `div` 요소로 바뀐 경우에는 고려되지 않습니다.).
   ...
   이 작업은 HTML 타겟 오퍼를 타겟 활동에 포함할 수 있도록 해줍니다.
...
기본 구현을 사용하는 경우 위에 설명된 프로세스에서는 경험 조각에서 Target 오퍼를 생성한 다음 이를 Adobe Target으로 내보내기에 충분해야 합니다. 그러나 이 프로세스에서 고려하지 않은 일부 사용 사례가 있습니다. 다음과 같습니다.
...
이 서비스는 다양한 링크에 액세스하기 위해 경험 조각 내보내기를 타겟 재작성에 연결하는 데 사용됩니다.
...
이 메서드는 매개 변수로 대상 시스템으로 내보내기가 현재 재작성하고 있는 경험 조각 변형을 받습니다.
...
Target으로 내보내기 시스템을 통과하는 기타 모든 경험 조각은 무시되며 본 서비스에서 구현된 변경 사항에 영향을 받지 않습니다.
...
예를 들어 Target으로 내보내기 시스템이 현재 이 요소를 처리 중인 경우 다음을 `CSSInclude` 사용하여 정의할 수 있습니다.
...
예를 들어 URL의 `/etc.clientlibs` 부분을 제거하고 적절한 외부 도메인을 추가하려고 합니다. 간단한 작업을 위해 다음과 같이 귀하의 서비스에 대한 리소스 해결 프로그램에 대한 액세스 권한이 있는 것으로 간주합니다. `rewriteLinkExample2`
