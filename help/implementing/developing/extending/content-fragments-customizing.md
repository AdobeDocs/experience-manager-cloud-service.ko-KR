...
* [자산 관리](/help/assets/manage-digital-assets.md) 및 [자산 편집기](/help/assets/extend-asset-editor.md) 사용자 지정 및 확장을 참조하십시오.
...
* 컨텐츠 조각 템플릿 - **단순 조각**:
   ...
   * 템플릿이 작성되면 조편으로 복사됩니다.

   * 새 변형 등을 추가하는 함수는 해당 조각을 업데이트해야 합니다.

   * 컨텐츠 조각 템플릿(**단순 조각**)은 AEM 에코시스템 내의 다른 템플릿 메커니즘과 다른 방식으로 작동합니다(예: 페이지 템플릿 등). 그러므로 그것은 별도로 고려되어야 한다.

   * 단순 **조각** 템플릿을 기반으로 하는 경우 컨텐츠의 MIME 유형은 실제 컨텐츠에서 관리됩니다. 즉, 각 요소와 변형에 다른 MIME 유형이 있을 수 있습니다.
...
* 메타데이터 및 관련 컨텐츠는 아래 `jcr:content/metadata`에 저장됩니다. 제목 및 설명은 일반 메타데이터로 간주되지 않고 `jcr:content`
간단한 컨텐츠 조각을 자산에 매핑 {#mapping-simple-content-fragments-to-assets}

#### ![자산을 간단하게](assets/content-fragment-to-assets-simple.png)

간단한 컨텐츠 조각( **단순 조각** 템플릿 기반)은 기본 자산 및 (선택 사항) 하위 자산으로 구성된 합성에 매핑됩니다.
...
#### 표준 자산과 마찬가지로 컨텐츠 조각도 다음 아래에 보관됩니다.{#asset-location}
...
기능 통합 {#feature-integration}](/help/assets/content-fragments/content-fragments-delete.md)

#### 자산 코어와 통합하려면{#feature-integration}
...
>컨텐츠 [조각 구성 요소는 핵심 구성 요소의 일부입니다[#$tu71]. 자세한 내용은 [핵심 구성 요소[#$tu72] 개발을 참조하십시오.
>
>



* 구성 요소를 사용하면 표시할 변형을 선택할 수 있습니다.`fragmentPath``fragmentPath`
...
>**컨텐츠 조각 템플릿 - 단순 조각:**
>
>페이지의 컨텐츠 조각 템플릿 **단순 조각** 을 기반으로 한 컨텐츠 조각을 사용하는 경우 조각을 생성할 때 템플릿이 복사되었기 때문에 참조가 없습니다.
>
>다른 프레임워크와의 통합 {#integration-with-other-frameworks}**

### 컨텐츠 조각 통합:{#integration-with-other-frameworks}
...
* **컨텐츠 조각은 AEM 번역 워크플로우와 완전히 통합됩니다. 건축적 수준에서, 이것은 다음을 의미한다:**
   ...
   * 그들은 다른 언어 루트 아래 위치한다. 관련 언어 루트 아래에서 동일한 상대 경로를 공유합니다.
      ...
      * `/content/dam/<path>/en/<to>/<fragment>`
         ...
         `/content/dam/<path>/de/<to>/<fragment>`규칙 기반 경로 외에도 컨텐츠 조각의 다른 언어 버전 간에 더 이상 연결되지 않습니다. 언어 변형을 탐색하는 수단을 제공하지만 두 개의 개별 조각으로 처리됩니다.
   * [!NOTE]
...
* 컨텐츠 조각(re)은 표준 자산으로 정의할 수 있는 [메타데이터 스키마를](/help/assets/metadata-schemas.md)사용합니다.
   ...
   * CFM은 고유한 특정 스키마를 제공합니다.[](/help/assets/metadata-schemas.md)

   * `/libs/dam/content/schemaeditors/forms/contentfragment`

      `/libs/dam/content/schemaeditors/forms/contentfragment`필요한 경우 확장할 수 있습니다.

      해당 스키마 양식이 조각 편집기와 통합됩니다.

   * 컨텐츠 조각 관리 API - 서버측 {#the-content-fragment-management-api-server-side}

## 서버측 API를 사용하여 컨텐츠 조각에 액세스할 수 있습니다. see:{#the-content-fragment-management-api-server-side}

[com.adobe.cq.dam.cfm[#$tu111]



>[!CAUTION]컨텐츠 구조에 직접 액세스하는 대신 서버측 API를 사용하는 것이 좋습니다.
>
>주요 인터페이스 {#key-interfaces}

### 다음 세 가지 인터페이스는 시작 지점으로 사용할 수 있습니다.{#key-interfaces}
...
* 
   ...
   * 목록 요소
      ...
      * 요소 데이터에 액세스(참조 `ContentElement`)](#caveats)

      * 조각에 대해 정의된 목록 변형`ContentElement`
   * 전역적으로 새로운 변형 만들기
   * 관련 컨텐츠 관리:
   * 목록 컬렉션
   ...
   * 
      ...
      * 목록 변형
         ...
         * 변형 데이터 액세스(참조 `ContentVariation`)](#caveats)
         * 변형에 대한 해결 단축키(지정한 변형을 요소에 사용할 수 없는 경우 구현 전용 폴백 논리 추가 적용)`ContentVariation`
      * **컨텐츠 변형** ([ContentVariation[#$tu147])
   * 
   ...
   요소의 새 버전 만들기`ContentFragment``ContentElement``ContentVariation``Versionable`
...
### 다음을 조정할 수 있습니다.{#adapting-using-adaptto}
...
* `Resource` - 기본 Sling 리소스; 기본 개체를 `Resource` 직접 업데이트하려면 개체를 다시 `ContentFragment` 구성해야 합니다.
   ...
   * `ContentElement` 을(를) 다음과 같이 적용할 수 있습니다.`Asset``Asset``ContentFragment`
...
* 새 변형을 만들어 데이터 구조 `ContentFragment` 를 업데이트합니다.
   ...
   * 컨텐츠 조각 관리 API - 클라이언트측 {#the-content-fragment-management-api-client-side}`ContentFragment.removeVariation()`

## [!CAUTION]

>[!CAUTION]클라이언트측 API는 내부입니다.
>
>추가 정보 {#additional-information}

### 다음을 참조하십시오.{#additional-information}
...
기술적으로 모든 편집 작업은 다른 모든 AEM 편집 작업처럼 *라이브* 컨텐츠에서 수행됩니다. 편집 세션이 시작되면 편집되지 않은 현재 상태의 버전이 만들어집니다. 사용자가 편집을 취소하면 해당 버전이 복원됩니다. 사용자가 [ **저장**]을 클릭하는 *경우 모든 편집 작업이* 라이브컨텐츠에서 실행되므로 모든 변경 사항이 이미 유지됩니다. 또한 **저장을** 클릭하면 일부 백그라운드 처리가 트리거됩니다(전체 텍스트 검색 정보 만들기 및/또는 혼합 미디어 자산 처리).
...
### 이를 위해 API를 나타내는 리소스를 다음과 같은 용도로 적용할 수 있습니다.{#example-accessing-an-existing-content-fragment}

`com.adobe.cq.dam.cfm.ContentFragment`

`com.adobe.cq.dam.cfm.ContentFragment`예:
...
### 프로그래밍 방식으로 새 컨텐츠 조각을 만들려면 모델 또는 템플릿 리소스에서`FragmentTemplate` 수정한 컨텐츠를 사용해야 합니다.
...
노드: `<conf-root>/settings/dam/cfm/jcr:content`](/help/assets/content-fragments/content-fragments-managing.md#save-cancel-and-versions)

* 속성 이름: `autoSaveInterval`
* 유형: `Long`
* 기본값: `600` 10분) this is defined on `/libs/settings/dam/cfm/jcr:content`

* 자동 저장 간격을 5분으로 설정하려면 노드에서 속성을 정의해야 합니다. 예를 들면 다음과 같습니다.`600``/libs/settings/dam/cfm/jcr:content`
...
## 자세한 내용은{#components-for-page-authoring}

[핵심 구성 요소 - 컨텐츠 조각 구성 요소[#$tu206] (권장)
