...
이를 최소화하려면 장기간에 걸친 작업은 가급적 뒤로 하고 최소한 재개해야 한다. 이러한 작업을 실행하는 경우, 적어도 한 번 보증이 있어 중단되는 경우 가능한 한 빨리 다시 실행됩니다. 그러나 처음부터 다시 시작해서는 안 될 것이다. 이러한 작업을 예약하는 경우, [Sling 작업](https://sling.apache.org/documentation/bundles/apache-sling-eventing-and-job-handling.html#jobs-guarantee-of-processing) 스케줄러를 다시 한 번 이상 실행할 때 사용하는 것이 가장 좋습니다.
...
Adobe은 HTTP 연결을 만들려면 제공된 [Apache HttpComponents Client 4.x library](https://hc.apache.org/httpcomponents-client-ga/)을 사용하는 것이 좋습니다.

효과가 있지만 직접 종속성을 제공해야 할 수 있는 대체 방법은 다음과 같습니다.

* [java.net.](https://docs.oracle.com/javase/7/docs/api/java/net/URL.html) URL 및/ [또는 java.net.URLConnection](https://docs.oracle.com/javase/7/docs/api/java/net/URLConnection.html) (AEM 제공)
* [Apache Commons HttpClient 3.x](https://hc.apache.org/httpclient-3.x/) (이전 버전이며 버전 4.x로 대체되므로 권장되지 않음)
...
## Cloud Service {#no-streaming-binaries}으로서 AEM을 통해 바이너리를 스트리밍하지 않음
...
>[!NOTE]
>
>아래에 나열된 구성 변경 사항을 수행하려면 로컬 개발 환경에서 구성 변경 사항을 만든 다음 AEM에 Cloud Service 인스턴스로 푸시해야 합니다. 이 방법에 대한 자세한 내용은 [Cloud Service](/help/implementing/deploying/overview.md)으로 AEM에 배포를 참조하십시오.
...
로컬 개발 시(클라우드 지원 빠른 시작 사용), `/apps` 및 `/libs`은(는) 직접 작성할 수 있습니다. 이는 최상위 폴더를 변경할 수 없는 클라우드 환경과는 다릅니다.
...
예상되는 전용 IP 주소에서 트래픽이 실제로 전송되는지 확인하려면, 가능한 경우 대상 서비스에서 로그를 확인하십시오. 그렇지 않으면 호출 IP 주소를 반환하는 [https://ifconfig.me/ip](https://ifconfig.me/ip)과 같은 디버깅 서비스로 전화하는 것이 유용할 수 있습니다.

## 이메일 {#sending-email} 전송

AEM을 Cloud Service으로 사용하려면 아웃바운드 메일을 암호화해야 합니다. 아래 섹션에서는 이메일을 요청, 구성 및 전송하는 방법에 대해 설명합니다.
...
1. SMTP 액세스가 작성자, 게시 또는 둘 다에 필요한지 여부

### 이메일 전송 {#sending-emails}

[일 CQ 메일 서비스 OSGI 서비스](https://docs.adobe.com/content/help/en/experience-manager-65/administering/operations/notification.html#configuring-the-mail-service)를 사용해야 하며 이메일을 받는 사람에게 직접 보내는 것이 아니라 지원 요청에 지정된 메일 서버로 보내야 합니다.
...
AEM의 이메일은 [일 CQ 메일 서비스 OSGi 서비스](https://docs.adobe.com/content/help/en/experience-manager-65/administering/operations/notification.html#configuring-the-mail-service)를 사용하여 보내야 합니다.
...
* `smtp.port`을(를) `465` 설정
* `smtp.ssl`을(를) `true` 설정
* `smtp.starttls`을(를) `false` 설정

포트 587이 요청된 경우(메일 서버가 포트 465를 지원하지 않는 경우에만 허용):

* `smtp.port`을(를) `587` 설정
* `smtp.ssl`을(를) `false` 설정
* `smtp.starttls`을(를) `true` 설정