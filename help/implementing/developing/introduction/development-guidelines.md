...
CQ [Mail Service OSGI 서비스를](https://docs.adobe.com/content/help/en/experience-manager-65/administering/operations/notification.html#configuring-the-mail-service) 사용하고 이메일을 수신자에게 직접 보내는 대신 지원 요청에 지정된 메일 서버로 보내야 합니다.
...
AEM의 이메일은 [Day CQ Mail Service OSGi 서비스를 사용하여 보내야 합니다](https://docs.adobe.com/content/help/en/experience-manager-65/administering/operations/notification.html#configuring-the-mail-service).

이메일 설정 구성에 대한 자세한 내용은 [AEM 6.5 설명서를](https://docs.adobe.com/content/help/en/experience-manager-65/administering/operations/notification.html) 참조하십시오. AEM CS의 경우 `com.day.cq.mailer.DefaultMailService OSGI` 서비스를 다음과 같이 조정해야 합니다.

포트 465가 요청된 경우:

* 설정 `smtp.port` `465`
* 설정 `smtp.ssl` `true`
* 설정 `smtp.starttls` `false`

포트 587이 요청된 경우(메일 서버가 포트 465를 지원하지 않는 경우에만 허용):

* 설정 `smtp.port` `587`
* 설정 `smtp.ssl` `false`
* 설정 `smtp.starttls` `true`