...
AEM as a Cloud Service 개발자 콘솔에 대한 액세스 권한이 있는 사용자는 주어진 환경에 대해 Developer Console의 통합 탭을 볼 수 있습니다. AEM as a Cloud Service 환경 관리자 역할이 있는 사용자는 자격 증명을 만들거나, 보거나, 관리할 수 있습니다.

**새 기술 계정 만들기**&#x200B;를 클릭하면 Pod 선택에 관계없이 환경의 작성자 및 게시 계층에 대한 클라이언트 ID, 클라이언트 암호, 개인 키, 인증서 및 구성을 포함하는 자격 증명 집합이 만들어집니다.
...
자격 증명이 만들어지면 **통합** 섹션의 **기술 계정** 탭에 표시됩니다.
...
>[!IMPORTANT]
>
>AEM 작성자의 AEM 사용자 또는 AEM 관리자 제품 프로필 의 멤버이기도 한 IMS 조직 관리자(일반적으로 Cloud Manager을 통해 환경을 프로비저닝한 동일한 사용자)는 먼저 Developer Console에 액세스해야 합니다. 그런 다음 AEM as a Cloud Service 환경에 대한 관리자 권한이 있는 사용자가 자격 증명을 생성하고 나중에 검색할 수 있도록 **새 기술 계정 만들기**&#x200B;를 클릭합니다. IMS 조직 관리자가 아직 기술 계정을 만들지 않은 경우 IMS 조직 관리자 역할이 필요하다는 메시지가 표시됩니다.

### 비 AEM 서버에 AEM 서비스 자격 증명 설치 {#install-the-aem-service-credentials-on-a-non-aem-server}

AEM을 호출하는 애플리케이션은 AEM as a Cloud Service의 자격 증명에 액세스하여 이를 비밀로 취급할 수 있어야 합니다.
...
1. [https://adminconsole.adobe.com/](https://adminconsole.adobe.com/)의 Adobe Admin Console으로 이동합니다.
1. 왼쪽의 **제품 및 서비스** 열 아래에 있는 **관리** 링크를 누릅니다.
1. **AEM as a Cloud Service**&#x200B;을(를) 선택합니다.
1. **새 프로필** 단추를 누릅니다.

   ![새 프로필](/help/implementing/developing/introduction/assets/s2s-newproductprofile.png)

1. 프로필 이름을 지정하고 **저장**&#x200B;을 누릅니다.
...
1. 만든 기술 계정(이 경우 `84b2c3a2-d60a-40dc-84cb-e16b786c1673@techacct.adobe.com`)을 추가하고 **저장**&#x200B;을 클릭합니다.
...
API를 호출하면 제품 프로필이 AEM as a Cloud Service 작성자 인스턴스에서 해당 그룹의 구성원으로 적절한 기술 계정을 사용하여 사용자 그룹으로 표시됩니다.

이 정보를 확인하려면 다음을 수행하십시오.

1. 작성자 인스턴스에 로그온합니다.
1. **도구** > **보안**(으)로 이동한 다음 **그룹** 카드를 클릭합니다.
1. 그룹 목록에서 생성한 프로필의 이름을 찾아 클릭합니다.

   ![그룹 프로필](/help/implementing/developing/introduction/assets/s2s-groupprofile.png)

1. 다음 창에서 **구성원** 탭으로 전환하고 기술 계정이 올바르게 나열되어 있는지 확인하십시오.
...
1. **도구** > **보안** > **사용자**(으)로 이동합니다.
1. 기술 계정이 사용자 목록인지 확인하고 선택합니다.
1. **그룹** 탭을 클릭하면 사용자가 제품 프로필에 해당하는 그룹의 일부인지 확인할 수 있습니다. 이 사용자는 기여자를 포함하여 소수의 다른 그룹에도 속합니다.

   ![그룹 구성원](/help/implementing/developing/introduction/assets/s2s-groupmembership.png)

>[!NOTE]
>
>2023년 중반 이전에는 여러 자격 증명을 만들 수 있었지만 Adobe Admin Console에서 제품 프로필을 만들도록 고객에게 안내되지 않았습니다. 따라서 기술 계정은 AEM as a Cloud Service 인스턴스의 &quot;기여자&quot; 이외의 그룹과 관련이 없었습니다. 일관성을 위해 이 기술 계정의 경우 위에서 설명한 대로 Adobe Admin Console에서 제품 프로필을 만들고 기존 기술 계정을 해당 그룹에 추가하는 것이 좋습니다.

<u>**적절한 그룹 권한 설정**</u>

마지막으로, API를 적절하게 호출하거나 잠글 수 있도록 필요한 적절한 권한으로 그룹을 구성합니다.

1. 적절한 작성자 인스턴스에 로그온하고 **설정** > **보안** > **권한**(으)로 이동합니다.
...
1. 사용 권한을 적절하게 수정하고 **저장**&#x200B;을 클릭하세요.

   ![권한 편집 확인](/help/implementing/developing/introduction/assets/s2s-confirmeditpermissions.png)

>[!INFO]
>
>Adobe IMS(Identity Management System) 및 AEM 사용자 및 그룹에 대해 자세히 알아보십시오. [설명서](/help/security/ims-support.md)를 참조하세요.

## 개발자 흐름 {#developer-flow}

개발자는 개발 AEM as a Cloud Service 개발 환경을 요청하는 비 AEM 애플리케이션(랩톱에서 실행되거나 호스팅됨)의 개발 인스턴스를 사용하여 테스트하려고 할 수 있습니다. 그러나 개발자에게 반드시 IMS 관리자 역할 권한이 있는 것은 아니기 때문에 Adobe은 일반 서버 간 흐름에서 설명한 JWT 전달자를 생성할 수 있다고 간주할 수 없습니다. 따라서 Adobe은 개발자가 액세스 권한이 있는 AEM as a Cloud Service의 환경에 대한 요청에 사용할 수 있는 액세스 토큰을 직접 생성하는 메커니즘을 제공합니다.
...
1. **통합**&#x200B;의 **로컬 토큰**(으)로 이동
1. 액세스 토큰을 생성할 수 있도록 Developer Console에서 **로컬 개발 토큰 가져오기**&#x200B;를 클릭합니다.
...
AEM as a Cloud Service의 자격 증명은 기본적으로 1년 후에 만료됩니다. 서비스 연속성을 보장하기 위해 개발자는 자격 증명을 새로 고치고 1년 동안 가용성을 확장할 수 있습니다.

이 새로 고침 확장을 수행하려면 다음을 수행합니다.

* 아래와 같이 Developer Console의 **통합** - **기술 계정**&#x200B;에서 **인증서 추가** 단추를 사용합니다
...
1. 먼저 새 키를 추가합니다. 이 키는 새 개인 키 및 새 인증서를 사용하여 자격 증명을 생성합니다. 새 개인 키가 사용자 인터페이스에 **current**(으)로 표시되어 앞으로 이 기술 계정의 모든 새 자격 증명에 사용됩니다. 이전 개인 키와 연결된 자격 증명은 해지될 때까지 계속 유효합니다. 해지하려면 현재 기술 계정에서 세 점(**..**)을 선택한 다음 **새 개인 키 추가**&#x200B;를 선택하십시오.

   ![새 개인 키 추가](/help/implementing/developing/introduction/assets/s2s-addnewprivatekey.png)

1. 다음 프롬프트에서 **추가**&#x200B;를 선택합니다.

   ![새 개인 키 추가 확인](/help/implementing/developing/introduction/assets/s2s-addprivatekeyconfirm.png)

   새 자격 증명이 포함된 새 찾아보기 탭이 열리고 사용자 인터페이스가 **current**(으)로 표시된 새 개인 키가 모두 표시되도록 업데이트됩니다.
...
1. 인증서 오른쪽에 있는 세 점(**..**)을 선택하고 **취소**&#x200B;를 선택하여 이전 인증서를 취소합니다.
   ...
   그런 다음 **취소** 단추를 눌러 다음 프롬프트에서 취소를 확인합니다.
