...
* `allowProxy`: 모든 clientlibs는 아래에 저장되어야 하므로 `apps`로 지정하는 경우 이 속성을 사용하면 프록시 서블릿을 통해 clientlibraries에 액세스할 수 있습니다. 자세한 내용은 [클라이언트 라이브러리 폴더 찾기 및 프록시 클라이언트 라이브러리 서블릿 사용](#locating-a-client-library-folder-and-using-the-proxy-client-libraries-servlet) 아래의 제품에서 사용할 수 있습니다.
...
1. 정적 리소스를 관리해야 하는 경우 이름이 인 하위 폴더를 만듭니다. `resources` 클라이언트 라이브러리 폴더 아래에 있습니다.
   * 폴더 아래에 정적 리소스를 저장하는 경우 `resources`를 호출하면 게시 인스턴스에서 참조할 수 없습니다.
...
모든 애플리케이션 관련 파일을 /apps 아래의 애플리케이션 폴더에 보관하는 것이 가장 좋습니다. 또한 /apps 폴더에 대한 웹 사이트 방문자의 액세스를 거부하는 것이 좋습니다. 두 가지 우수 사례를 모두 충족하려면 /apps 아래에 있는 클라이언트 라이브러리를 포함하는 /etc 폴더 아래에 클라이언트 라이브러리 폴더를 만드십시오.
