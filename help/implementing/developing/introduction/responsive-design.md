...
다양한 창 크기 및 방향에 맞는 HTML5를 생성하는 AEM(Adobe Experience Manager) 애플리케이션을 개발합니다. 예를 들어, 다음 뷰포트 너비 범위는 다양한 장치 유형 및 방향에 해당합니다
...
* 페이지를 렌더링할 때 어셈블된 CSS를 정의하려면 [클라이언트 라이브러리 폴더](clientlibs.md)를 사용하십시오.
...
[WKND 자습서](develop-wknd-tutorial.md)에서는 이 전략을 사용하여 사이트 디자인에서 스타일을 정의합니다. WKND에서 사용하는 CSS 파일은 `/apps/wknd/clientlibs/clientlib-grid/less/grid.less`에 있습니다.
