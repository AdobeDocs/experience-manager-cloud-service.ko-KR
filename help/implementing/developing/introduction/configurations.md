...
>[!NOTE]
>
>사용자에게 다음이 있는 경우 구성 브라우저를 사용하여 구성을 만들 수 있습니다. `admin` 권한. 다음과 같음 `admin` 액세스 권한을 구성에 할당하거나 구성을 수정하는 데도 권한이 필요합니다.
...
콘텐츠는 다음을 통해 연결된 구성을 참조합니다. `cq:conf` 속성. AEM은 콘텐츠 및 해당 컨텍스트를 기반으로 조회를 수행합니다 `cq:conf` 속성을 사용하여 적절한 구성을 찾을 수 있습니다.
...
모든 구성 조회의 시작점은 콘텐츠 리소스이며 `/content`. 페이지, 페이지 내의 구성 요소, 에셋 또는 DAM 폴더일 수 있습니다. 이 컨텍스트에서 적용되는 올바른 구성을 찾고 있는 실제 콘텐츠입니다.

이제 `Conf` 개체를 사용하면 관심 있는 특정 구성 항목을 검색할 수 있습니다. 이 경우 `dam/imageserver`: 와 관련된 설정의 컬렉션입니다. `imageserver`. 다음 `getItem` 호출 반환: `ValueMap`. 그런 다음 을 읽습니다. `bgkcolor` 문자열 속성을 사용하고 속성(또는 전체 구성 항목)이 없는 경우 &quot;FFFFFF&quot;의 기본값을 제공합니다.
...
이 예에서는 여기에서 WKND별 DAM 폴더와 해당 구성을 가정할 수 있습니다. 해당 폴더에서 시작 `/content/dam/wknd`, 이름이 인 문자열 속성이 있음을 알 수 있습니다. `cq:conf` 하위 트리에 적용되는 구성을 참조합니다. 속성은 `jcr:content` 에셋 폴더 또는 페이지 다음 `conf` 링크는 명시적이므로 CRXDE의 콘텐츠만 보면 쉽게 따라갈 수 있습니다.

안에서 뛰기 `/conf`를 클릭하고 참조를 따라 다음 항목이 있는지 확인합니다. `/conf/wknd` 노드. 이는 구성입니다. 조회는 애플리케이션 코드를 통과시킵니다. 예제 코드에는 전용 참조가 없습니다. `Conf` 개체. 적용되는 구성은 JCR 콘텐츠를 통해 제어됩니다.

구성에 고정 이름이 포함된 것이 표시됩니다 `settings` 다음을 포함한 실제 항목을 포함하는 노드 `dam/imageserver` 이 경우에 필요합니다. 이러한 항목은 &quot;설정 문서&quot;로 생각할 수 있으며 `cq:Page` 다음 포함 `jcr:content` 실제 컨텐츠 유지.

마지막으로 속성이 표시됩니다. `bgkcolor` 이 샘플 코드에는 가 필요합니다. 다음 `ValueMap` 다음에서 돌아옵니다. `getItem` 페이지의 `jcr:content` 노드.
...
클릭 **해결** 해결된 구성을 확인하고 해당 구성을 해결하는 데 도움이 되는 코드 샘플을 가져올 수 있습니다.
...
클릭 **해결** 선택한 구성에 대해 연관된 컨텍스트 경로 및 속성을 검색할 수 있습니다.
