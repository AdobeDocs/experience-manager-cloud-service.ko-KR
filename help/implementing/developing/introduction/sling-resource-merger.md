...
* **[](/help/implementing/developing/introduction/overlays.md)** 검색 경로 [를 사용하는 리소스 ](/help/implementing/developing/introduction/overlays.md#search-paths)오버레이입니다.
...
* 사용자 지정된 정의의 컨텐츠의 우선 순위가 원본보다 높습니다(즉, *overlays* 또는 *overrides*).
...
>
>이는 인스턴스에 업그레이드가 적용될 때마다 `/libs`의 내용을 덮어쓸 수 있기 때문입니다.
>
...
>
>
하지만, Cloud Service은 `/apps`에서 사용자 지정을 정의하는 것이 가장 좋은 방법이므로, 무시는 종종 `/apps` 아래에 정의됩니다.이는 `/libs` 아래의 어떤 것도 변경하지 않아야 하기 때문입니다.
