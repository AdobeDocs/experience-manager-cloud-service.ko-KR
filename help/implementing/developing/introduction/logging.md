...
AEM as a Cloud Service는 고객이 고유한 고객 경험을 만들기 위해 사용자 지정 코드를 포함할 수 있는 플랫폼입니다. 이러한 점을 염두에 두고, 로깅은 클라우드 환경에서 사용자 지정 코드를 디버깅하기 위한 중요한 기능이며, 특히 로컬 개발 환경을 위한 기능입니다.
...
## 클라우드 서비스 로깅으로 AEM {#aem-as-a-cloud-service-logging}
...
## AEM을 클라우드 서비스로 사용 Java 로깅 {#aem-as-a-cloud-service-java-logging}
...
* 특정 작성기에 연결하지 않으므로 기본 구성이 있는 암시적 작성기를 만들어 사용합니다.
...
**AEM을 클라우드 서비스로 사용 HTTP 요청/응답 액세스 로깅**
...
로그에 액세스하고 관리하는 방법에 대한 자세한 내용은 Cloud Manager [설명서를](/help/implementing/cloud-manager/manage-logs.md)참조하십시오.