...
>[!TIP]
>
>이 문서가 이러한 학습 및 개념을 기반으로 빌드됨에 따라 기본 [AEM Project Archetype 사용](https://experienceleague.adobe.com/ko/docs/experience-manager-core-components/using/developing/archetype/overview#) 및 [FileVault Content Maven 플러그인](/help/implementing/developing/tools/maven-plugin.md)을 숙지하십시오.
...
AEM의 `/apps` 및 `/libs` 영역은 **변경할 수 없음**&#x200B;입니다. AEM이 시작된 후에는 런타임 시 이러한 영역에서 컨텐츠를 만들거나 업데이트하거나 삭제할 수 없습니다. 런타임 시 변경할 수 없는 영역을 변경하려고 하면 실패합니다.
...
AEM as a Cloud Service 배포 프로세스에서 Oak 인덱스(`/oak:index`)를 관리합니다. Cloud Manager은 새 코드 이미지로 전환하기 전에 새 색인이 배포되고 완전히 다시 색인화될 때까지 기다려야 하기 때문입니다.

이러한 이유로 Oak 색인은 런타임 시 변경할 수 있지만, 변경 가능한 패키지를 설치하기 전에 설치할 수 있도록 코드로 배포해야 합니다. 따라서 `/oak:index` 구성은 코드 패키지의 일부이며 아래 설명된 [과(와) 같이 &#x200B;](#recommended-package-structure) 콘텐츠 패키지의 일부가 아닙니다.
...
+ `ui.apps` 패키지에는 배포할 모든 코드가 포함되어 있으며 `/apps`에만 배포됩니다. `ui.apps` 패키지의 일반적인 요소는 다음을 포함하지만 이에 국한되지 않습니다.
   + [구성 요소 정의 및 HTL](https://experienceleague.adobe.com/ko/docs/experience-manager-htl/content/overview#) 스크립트
   ...
   + [미리 컴파일된 번들형 스크립트](https://experienceleague.adobe.com/ko/docs/experience-manager-core-components/using/developing/archetype/precompiled-bundled-scripts#)
...
+ `ui.content` 패키지에 모든 콘텐츠와 구성이 포함되어 있습니다. 콘텐츠 패키지에 `ui.apps` 또는 `ui.config` 패키지에 없는 모든 노드 정의, 즉 `/apps` 또는 `/oak:index`에 없는 모든 노드 정의가 포함되어 있습니다. `ui.content` 패키지의 일반적인 요소는 다음을 포함하지만 이에 국한되지 않습니다.
   ...
   + 관리되는 태그 지정 분류
      + `/content/cq:tags`
   + 레거시 `etc` 노드(이상적으로는 이러한 노드를 비/기타 위치로 마이그레이션함)
      + `/etc`

### 컨테이너 패키지

+ `all` 패키지는 컨테이너 패키지입니다. OSGi 번들 JAR 파일과 `ui.apps`, `ui.config` 및 `ui.content` 패키지를 임베드하는 배포 가능한 아티팩트만 포함됩니다. `all` 패키지에는 자체 **콘텐츠 또는 코드**&#x200B;가 없어야 하며, 대신 모든 배포를 저장소에 하위 패키지 또는 OSGi 번들 Jar 파일로 위임해야 합니다.

  이제 패키지가 [&#x200B; 구성이 아닌 Maven &#x200B;](#embeddeds)FileVault 패키지 Maven 플러그인의 임베드된 구성`<subPackages>`을 사용하여 포함됩니다.

  복잡한 Experience Manager 배포의 경우 AEM의 특정 사이트 또는 테넌트를 나타내는 `ui.apps`, `ui.config` 및 `ui.content`개의 프로젝트/패키지를 여러 개 만드는 것이 좋습니다. 이 방법을 사용하는 경우 변경할 수 있는 콘텐츠와 변경할 수 없는 콘텐츠 간의 분할을 준수하십시오. 또한 필요한 콘텐츠 패키지와 OSGi 번들 JAR 파일을 `all` 컨테이너 콘텐츠 패키지에 하위 패키지로 포함합니다.
...
+ `ui.config` 패키지에 모든 [OSGi 구성](/help/implementing/deploying/configuring-osgi.md)이 포함되어 있습니다.
   + OSGi 번들에 속하지만 일반 콘텐츠 노드를 포함하지 않는 코드로 간주됩니다. 따라서 컨테이너 패키지로 표시됩니다
   + 실행 모드별 OSGi 구성 정의가 포함된 조직 폴더
      + `/apps/my-app/osgiconfig`
   + 모든 대상 AEM as a Cloud Service 배포 대상에 적용되는 기본 OSGi 구성이 포함된 일반적인 OSGi 구성 폴더
...
+ `all` 콘텐츠 패키지는 단일 배포 아티팩트를 만들기 위해 다음 패키지를 임베드합니다.
   + AEM 애플리케이션에 필요한 `core` OSGi 번들 Jar
...
Repo Init의 주요 이점은 스크립트가 정의한 모든 작업을 수행할 수 있는 암시적 권한이 있다는 것입니다. 또한 이러한 스크립트는 배포 수명 주기 초기에 호출되어 코드를 실행할 때까지 모든 필수 JCR 구조가 존재하도록 합니다.
...
저장소 초기화 스크립트는 `scripts` OSGi 팩토리 구성의 `RepositoryInitializer` 항목으로 저장됩니다. 실행 모드는 암시적으로 이들을 타깃팅할 수 있습니다. 이 타깃팅을 사용하면 AEM Author와 AEM Publish Services 사이 또는 개발, 스테이지 및 프로덕션 환경 사이에서도 저장소 초기화 스크립트를 구분할 수 있습니다.

저장소 초기 OSGi 구성은 다중 라인을 지원하므로 [`.config` OSGi 구성 형식](https://sling.apache.org/documentation/bundles/configuration-installer-factory.html#configuration-files-config-1)로 작성되었습니다. 다중 라인은 [`.cfg.json`을(를) 사용하여 OSGi 구성을 정의하는 모범 사례](https://sling.apache.org/documentation/bundles/configuration-installer-factory.html#configuration-files-cfgjson-1)의 예외입니다.
...
콘텐츠 또는 코드 패키지는 특수 사이드카 폴더에 배치됩니다. FileVault Maven 플러그인의 `<embeddeds>` 구성을 사용하여 AEM 작성자, AEM 게시 또는 둘 다에 설치할 대상으로 지정합니다. `<subPackages>` 구성을 사용하지 마십시오.
...
+ 두 번째 수준 폴더는 폴더 이름에 사후 수정된 `-packages`이(가) 있는 응용 프로그램을 나타냅니다. 대부분의 경우 모든 하위 패키지가 임베드된 하나의 두 번째 수준 폴더만 있지만 응용 프로그램의 최상의 논리 구조를 나타내기 위해 원하는 수만큼 두 번째 수준 폴더를 만들 수 있습니다.
  ...
  >[!WARNING]
  >
  >규칙에 따라 하위 패키지 포함 폴더의 이름은 `-packages` 접미사를 사용하여 지정됩니다. 이 이름을 지정하면 배포 코드 및 콘텐츠 패키지가 **아래의 하위 패키지의 대상 폴더에 배포되지**&#x200B;않습니다`/apps/<app-name>/...`. 이 방법은 파괴적이고 순환적인 설치 동작을 방지합니다.

+ 세 번째 수준 폴더는 다음 중 하나여야 합니다
  `application`, `content` 또는 `container`
   + `application` 폴더에 코드 패키지가 있습니다.
   + `content` 폴더에 콘텐츠 패키지가 있습니다.
   + `container` 폴더에는 AEM 응용 프로그램에 포함되지 않았을 수 있는 [추가 응용 프로그램 패키지](#extra-application-packages)가 있습니다.
...
+ 네 번째 수준 폴더는 하위 패키지를 포함하며 다음 중 하나여야 합니다.
   ...
   + `install.publish` 따라서 AEM 게시에서 **only**&#x200B;을(를) 설치합니다.
...
패키지를 생성하는 모든 프로젝트에서 컨테이너(**) 프로젝트에 대해** except`all`을(를) `<cloudManagerTarget>none</cloudManagerTarget>` 플러그 인 선언의 `<properties>` 구성에 `filevault-package-maven-plugin`을(를) 추가하여 Adobe Cloud Manager에서 이** 배포하지 않도록 합니다. 컨테이너(`all`) 패키지는 Cloud Manager을 통해 배포된 단일 패키지여야 하며, 이렇게 되면 필요한 모든 코드 및 콘텐츠 패키지가 임베드됩니다.
...
Repo Init 스크립트가 포함된 Repo Init 스크립트는 `RepositoryInitializer` 속성을 통해 `scripts` OSGi 팩터리 구성에 정의되어 있습니다. OSGi 구성 내에서 이러한 스크립트를 정의하므로 일반적인 `../config.<runmode>` 폴더 의미 체계를 사용하여 실행 모드별로 이러한 스크립트의 범위를 쉽게 지정할 수 있습니다.
