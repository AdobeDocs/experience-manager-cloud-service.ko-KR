...
description: AEM이 JCR, Sling 및 OSGi와 같은 구조화되고 기본적인 기술인 방식을 포함하여 AEM의 기술 기반에 대한 개요입니다.
...
AEM은 강력하고 확장 가능하고 유연한 기술을 기반으로 구축되었습니다. 이 문서는 AEM을 구성하는 다양한 부품에 대한 상세 개요를 제공하며 전체 스택 AEM 개발자를 위한 기술 부록으로 작성된 것입니다. 이것은 시작하기 안내서가 아니다. AEM 개발을 처음 사용하는 경우 첫 번째 단계로 [AEM Sites 개발 시작하기 - WKND 자습서](develop-wknd-tutorial.md)를 참조하십시오.

>[!TIP]
>
>AEM의 핵심 기술에 들어가기 전에, Adobe은 [AEM Sites 개발 시작하기 - WKND 자습서를 완료할 것을 권장합니다.](develop-wknd-tutorial.md)
...
JCR(Java Content Repository) 표준인 [JSR 283](https://docs.adobe.com/content/docs/en/spec/jcr/2.0/index.html)은 컨텐츠 저장소 내의 세부 수준에서 컨텐츠 양방향 액세스를 위한 공급업체 독립적이고 구현 독립적인 방법을 지정합니다. 사양 리드는 Adobe Research(스위스) AG에 의해 소유됩니다.
...
[Apache Jackrabbit ](https://jackrabbit.apache.org/oak/) Oako는 JCR 표준을 준수하는 최신 세계적인 웹 사이트 및 기타 까다로운 컨텐츠 애플리케이션의 기초로 사용할 수 있는 확장 가능한 고성능의 계층 컨텐츠 저장소를 구현합니다.

Jackrabbit Oak(즉, Oak)는 AEM이 빌드되는 JCR 표준의 구현입니다.

## Sling 요청 처리 {#sling-request-processing}

AEM은 컨텐츠 중심의 애플리케이션을 쉽게 개발할 수 있는 REST 원칙을 기반으로 하는 웹 애플리케이션 프레임워크인 [Sling](https://sling.apache.org/site/index.html)을 사용하여 구축되었습니다. Sling은 Apache Jackrabbit Oak와 같은 JCR 저장소를 데이터 저장소로 사용합니다. Sling은 Apache Software Foundation에 기여했습니다. 자세한 내용은 Apache에서 확인할 수 있습니다.
...
이러한 유연성의 이점은 다양한 컨텐츠 요소가 있는 애플리케이션이나 사용자 정의할 수 있는 페이지가 필요할 때 분명히 나타날 수 있습니다. 특히 AEM과 같은 웹 컨텐츠 관리 시스템을 구현할 때
...
다음 다이어그램은 저장소에서 노드를 생성, 수정, 삭제, 복사 및 이동하는 데 대한 끝없는 옵션을 제공하는 모든 POST 요청을 처리할 때 사용할 수 있는 숨겨진 강력한 요청 매개 변수인 `SlingPostServlet`을 설명합니다.
...
* 둘째, 표현 또는 스크립트는 요청의 특정 부분(예: 선택기 및/또는 확장)과 결합하여 리소스 속성에서 찾을 수 있습니다.

### RESTful Sling {#restful-sling}

Sling은 컨텐츠 중심의 철학으로 REST 지향 서버를 구현하므로 웹 애플리케이션 프레임워크에 새로운 개념을 제공합니다. 이점은 다음과 같습니다.

* 표면이 아닌 매우 안전하게.리소스 및 표현이 서버 내에서 올바르게 모델링됩니다.
* 하나 이상의 데이터 모델 제거
   * 다른 컨텐츠 관리 프레임워크에서 리소스에 액세스하려면 URL 구조, 비즈니스 객체, DB 스키마가 필요할 수 있습니다.
...
* **프로토콜**  - HTTPS
* **호스트**  - 사이트의 도메인
* **컨텐츠 경로**  - 렌더링할 컨텐츠를 지정하고 확장명과 함께 사용할 경로를 지정합니다.이 예에서는  `tools/spy.html`
* **선택기**  - 컨텐츠를 렌더링하는 대체 방법에 사용됩니다.이 예제에서는 A4 형식의 프린터 지원 버전
* **확장**  - 컨텐트 형식또한 렌더링에 사용할 스크립트를 지정합니다.
* **접미사**  - 추가 정보를 지정하는 데 사용할 수 있습니다.
* **param(s)** - 동적 내용에 필요한 모든 매개 변수
...
다음 그림은 다음 섹션에서 자세히 설명될 때 사용하는 메커니즘을 보여 줍니다.
...
* 첫 번째 슬링은 요청에 지정된 위치에 노드가 있는지 여부를 확인합니다.예:`../content/corporate/jobs/developer.html`
...
모든 Sling 스크립트는 `/apps`(변경 가능한 사용자 스크립트) 또는 `/libs`(변경 불가능한 시스템 스크립트)의 하위 폴더에 저장되므로 이러한 순서로 검색됩니다.
...
이전 예를 사용하면 `sling:resourceType`이(가) `hr/jobs`인 경우

* `.html`으로 끝나는 GET/HEAD 요청 및 URL(기본 요청 유형, 기본 형식)
   * 스크립트는 `/apps/hr/jobs/jobs.esp`;입니다.`sling:resourceType`의 마지막 섹션은 파일 이름을 형성합니다.
* POST 요청(GET/HEAD을 제외한 모든 요청 유형, 메서드 이름은 대문자여야 함)
   * POST은 스크립트 이름에 사용됩니다.
   * 스크립트는 `/apps/hr/jobs/jobs.POST.esp`입니다.
* `.html`으로 끝나지 않고 다른 형식의 URL
...
* `sling:resourceType`이(가) 정의되지 않은 경우 다음을 수행합니다.
   * 콘텐트 경로는 적절한 스크립트를 검색하는 데 사용됩니다(경로 기준 `ResourceTypeProvider`이(가) 활성 상태인 경우).
...
* 스크립트가 전혀 없는 경우 기본 스크립트가 사용됩니다.
   * 기본 변환은 현재 일반 텍스트(`.txt`), HTML(`.html`) 및 JSON(`.json`)으로 지원되며, 이 모두 노드의 속성(적절한 형식)을 나열합니다. 확장 `.res` 또는 요청 확장이 없는 요청에 대한 기본 변환은 리소스를 스풀링하는 것입니다(가능한 경우).
* http 오류 처리(코드 403 또는 404) Sling은 다음 중 하나에서 스크립트를 찾습니다.
   * 사용자 지정된 스크립트의 `/apps/sling/servlet/errorhandler` 위치
   * 또는 표준 스크립트 `/libs/sling/servlet/errorhandler/404.jsp` 위치
...
이렇게 하려면 `sling:include("/<path>/<resource>")` 명령을 사용할 수 있습니다. 참조된 리소스의 정의를 효과적으로 포함합니다.

## OSGi {#osgi}

OSGi(Open Services Gateway Initiative)는 모듈식 응용 프로그램 및 라이브러리를 개발 및 배포하는 데 필요한 아키텍처(Java용 Dynamic Module System)를 정의합니다. OSGi 컨테이너를 사용하면 애플리케이션을 개별 모듈(추가 메타 정보가 있는 jar 파일 및 OSGi 용어로 번들이라고 함)로 분류하고 애플리케이션 간의 상호 종속성을 관리할 수 있습니다.
...
이 아키텍처에서는 응용 프로그램 특정 모듈로 Sling을 확장할 수 있습니다. 따라서 AEM은 OSGi의 [Apache Felix](https://felix.apache.org/) 구현을 사용합니다. 이 두 번들 모두 OSGi 프레임워크에서 실행되는 OSGi 번들 모음입니다.
...
자세한 내용은 [AEM용 OSGi를 Cloud Service](/help/implementing/deploying/configuring-osgi.md)으로 구성을 참조하십시오.

## 저장소 내 구조 {#structure-within-the-repository}

다음 목록은 저장소 내에 표시되는 구조에 대한 개요를 제공합니다.

* `/apps` - 응용 프로그램 관련;웹 사이트 고유의 구성 요소 정의를 포함합니다. 개발하는 구성 요소는 `/libs/core/wcm/components`에서 사용할 수 있는 기본 구성 요소를 기반으로 할 수 있습니다.
* `/content` - 웹 사이트용으로 만든 컨텐츠
* `/etc`
* `/home` - 사용자 및 그룹 정보.
* `/libs` - AEM의 핵심에 속하는 라이브러리 및 정의 `/libs`의 하위 폴더는 기본 AEM 기능을 나타냅니다. `/libs`의 내용은 수정할 수 없습니다. 웹 사이트 전용 기능은 `/apps`에서 작성해야 합니다.
* `/tmp` - 임시 작업장
* `/var` - 시스템에 의해 변경 및 업데이트되는 파일감사 로그, 통계, 이벤트 처리 등

>[!CAUTION]
>
>이 구조나 그 안에 있는 파일들에 대한 변경은 주의해서 해야 한다. 변경 내용이 미치는 영향을 완전히 이해했는지 확인합니다.
>
