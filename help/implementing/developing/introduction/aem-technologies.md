...
AEM은 입증된 범위 및 확장 가능한 유연한 기술을 기반으로 구축된 강력한 플랫폼입니다. 이 문서에서는 AEM을 구성하는 다양한 부품에 대한 상세 개요를 제공하며 전체 스택 AEM 개발자를 위한 기술 부록으로 작성되었습니다. 이것은 시작하기 안내서가 아니다. AEM 개발을 처음 사용하는 경우 첫 번째 단계로 [개발 시작 AEM Sites - WKND 자습서](develop-wknd-tutorial.md)를 참조하십시오.

>[!TIP]
>
>AEM의 핵심 기술에 들어가기 전에, Adobe은 [AEM Sites 개발 시작 - WKND 자습서를 완료할 것을 권장합니다.](develop-wknd-tutorial.md)
...
JCR(Java Content Repository) 표준인 [JSR 283](https://docs.adobe.com/content/docs/en/spec/jcr/2.0/index.html)은 컨텐츠 저장소 내의 세부 수준에서 컨텐츠 양방향 액세스를 위한 공급업체 독립적이고 구현에 독립적인 방법을 지정합니다. 사양 리드는 Adobe 리서치(스위스) AG에 의해 소유됩니다.
...
[Apache Jackrabbit ](https://jackrabbit.apache.org/oak/) Okie는 JCR 표준을 준수하는 최신 세계적 수준의 웹 사이트 및 기타 까다로운 컨텐츠 애플리케이션의 기초로 사용할 수 있는 확장 가능한 고성능의 계층 컨텐츠 저장소를 구현한 솔루션입니다.

Jackrabbit Oak(또는 Oak라고도 함)는 AEM이 빌드되는 JCR 표준의 구현입니다.

## Sling 요청 처리 {#sling-request-processing}

AEM은 컨텐츠 중심 애플리케이션을 쉽게 개발할 수 있는 REST 원칙을 기반으로 하는 웹 애플리케이션 프레임워크인 [Sling](https://sling.apache.org/site/index.html)을 사용하여 구축되었습니다. Sling은 Apache Jackrabbit Oak와 같은 JCR 저장소를 데이터 저장소로 사용합니다. Sling은 Apache Software Foundation에 기여했으며, 자세한 내용은 Apache에서 확인할 수 있습니다.
...
다음 다이어그램은 저장소에서 노드를 만들고, 수정하고, 삭제하고, 복사하고, 이동하는 데 필요한 끝없는 옵션을 제공하는 모든 POST 요청을 처리할 때 사용할 수 있는 숨겨진 강력한 요청 매개 변수인 `SlingPostServlet`을 설명합니다.
...
* **프로토콜**  - HTTPS
* **호스트**  - 사이트의 도메인
* **컨텐츠 경로**  - 렌더링할 컨텐츠를 지정하고 확장자와 함께 사용할 경로를 지정합니다.이 예에서는  `tools/spy.html`
* **selector(s)** - 컨텐츠를 렌더링하는 대체 방법에 사용됩니다.이 예에서는 A4 형식의 프린터 지원 버전입니다.
* **익스텐션**  - 콘텐츠 형식또한 렌더링에 사용할 스크립트를 지정합니다
...
이전 예를 사용하는 경우 `sling:resourceType`이 `hr/jobs`이면 다음

* GET/HEAD 요청 및 `.html`으로 끝나는 URL(기본 요청 유형, 기본 형식)
...
* 선택기가 있는 URL
   ...
   * 선택기가 `print`일 수 있는 프린터 친화적인 버전을 살펴보는 경우`../content/corporate/jobs/developer.print.html`
   * 스크립트는 `/apps/hr/jobs/jobs.print.esp`입니다.선택기가 스크립트 이름에 추가됩니다.
* `sling:resourceType`이(가) 정의되지 않은 경우 다음을 수행합니다.
   * 콘텐트 경로는 적절한 스크립트를 검색하는 데 사용됩니다(경로 기반 `ResourceTypeProvider`이 활성).
...
* 스크립트를 전혀 찾을 수 없으면 기본 스크립트가 사용됩니다.
   * 기본 변환은 현재 일반 텍스트(`.txt`), HTML(`.html`) 및 JSON(`.json`)으로 지원되며, 모두 노드의 속성(적절한 형식)을 나열하게 됩니다. 요청 확장명이 없는 요청이나 `.res` 확장명에 대한 기본 변환은 리소스를 스풀링하는 것입니다(가능한 경우).
* http 오류 처리(코드 403 또는 404) Sling은 다음 중 하나에서 스크립트를 찾습니다.
   * 사용자 지정된 스크립트에 대한 `/apps/sling/servlet/errorhandler` 위치
...
이 아키텍처에서는 응용 프로그램별 모듈로 Sling을 확장할 수 있습니다. Sling 및 therwise AEM은 OSGi의 [Apache Felix](https://felix.apache.org/) 구현을 사용합니다. 두 OSGi 번들 모두 OSGi 프레임워크에서 실행되는 OSGi 번들 모음입니다.
...
자세한 내용은 [AEM용 OSGi를 Cloud Service](/help/implementing/deploying/configuring-osgi.md)로 구성을 참조하십시오.

## 저장소 내의 구조 {#structure-within-the-repository}

다음 목록은 저장소 내에 표시되는 구조에 대한 개요를 제공합니다.

* `/apps` - 응용 프로그램 관련에는 웹 사이트 고유의 구성 요소 정의가 포함됩니다. 개발하는 구성 요소는 `/libs/core/wcm/components`에서 사용할 수 있는 기본 구성 요소를 기반으로 할 수 있습니다.
* `/content` - 웹 사이트용으로 만든 컨텐츠
* `/etc`
* `/home` - 사용자 및 그룹 정보.
* `/libs` - AEM의 핵심에 속하는 라이브러리 및 정의 `/libs`의 하위 폴더는 기본적으로 AEM 기능을 나타냅니다. `/libs`의 콘텐트는 수정할 수 없습니다. 웹 사이트에 적합한 기능은 `/apps` 아래에 제공해야 합니다.
* `/tmp` - 임시 작업 영역
