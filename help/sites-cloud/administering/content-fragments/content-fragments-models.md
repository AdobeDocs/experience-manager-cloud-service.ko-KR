...
1. **편집**&#x200B;에 필요한 모델을 엽니다. 빠른 작업을 사용하거나, 모델을 선택한 후 도구 모음에서 작업을 선택하십시오.
   ...
   * 오른쪽: 필드를 만드는 데 사용할 수 있는 **데이터 유형**(필드가 만들어지면 사용할 **속성**)
   ...
   >[!NOTE]
   >
   >필드가 로 표시되는 경우 **필수**, **레이블** 왼쪽 창에 표시된 것은 별표( )로 표시됩니다.**&#42;**).
...
1. **필드를 제거하려면**
   ...
   필요한 필드를 선택한 후 휴지통 아이콘을 클릭/탭합니다. 작업을 확인하라는 메시지가 표시됩니다.
...
* **렌더링 형식**
조각의 필드를 구현하거나 렌더링하기 위한 다양한 옵션입니다. 이를 통해 작성자에게 필드의 단일 인스턴스가 표시되는지 또는 작성자가 여러 인스턴스를 만들 수 있는지를 정의할 수 있는 경우가 많습니다.

* **필드 레이블**
입력 **필드 레이블** 이(가) 을(를) 자동 생성합니다. **속성 이름**&#x200B;필요한 경우 수동으로 업데이트할 수 있습니다.
...
특정 필드의 경우, 콘텐츠는 현재 모델에서 만들어진 모든 콘텐츠 조각에서 고유해야 합니다.
  ...
  >[!CAUTION]
  >
  >(콘텐츠 조각 사본을 만드는) MSM을 사용하려면 각 콘텐츠 조각 모델에 사용되는 모든 데이터 유형에서 **고유한** 제한 사항을 제거해야 합니다. 콘텐츠 조각의 MSM은 **자산** 콘솔에서만 사용할 수 있습니다.
...
1. 모델을 선택한 후 도구 모음에서 **삭제**&#x200B;를 클릭합니다.
   ...
   >[!NOTE]
   >
   >모델을 참조하면 경고가 표시됩니다. 적절하게 조치하십시오.
...
* 콘솔
   ...
   * 편집 사용을 위해 모델의 **잠금을 해제**&#x200B;할 수 있습니다.
     ...
     다음을 선택하는 경우 **잠금 해제**&#x200B;에 경고가 표시되며 다음을 확인해야 합니다. **잠금 해제** 작업:
