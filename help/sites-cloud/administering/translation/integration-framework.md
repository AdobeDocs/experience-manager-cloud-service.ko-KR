...
1. [번역 서비스 제공업체에 연결합니다.](#connecting-to-a-translation-service-provider)
...
## 번역 서비스 제공업체에 연결 {#connecting-to-a-translation-service-provider}

AEM을 번역 서비스 제공업체에 연결하는 클라우드 구성을 만듭니다. AEM에는 기본적으로 [Microsoft Translator에 연결](connect-ms-translator.md)할 수 있는 기능이 포함되어 있습니다.
...
웹 사이트의 각 부분에 서로 다른 번역 요건이 있는 경우 그에 따라 여러 프레임워크 구성을 만듭니다. 예를 들어 다국어 웹 사이트에는 영어, 스페인어, 일본어 복사본이 포함될 수 있습니다. 사이트 소유자는 스페인어와 일본어 번역에 다른 번역 서비스 제공업체를 사용합니다. 따라서 프레임워크의 구성 두 개를 진행합니다. 각 구성은 다른 번역 서비스 제공업체를 사용합니다.
...
1. 필드에 다음 정보를 입력한 후 **만들기**&#x200B;를 클릭하거나 탭합니다.
   1. 드롭다운 메뉴에서 **구성 유형**&#x200B;을 선택합니다.
   1. 구성의 **제목**&#x200B;을 입력합니다. **제목**&#x200B;을 통해 **Cloud Services** 콘솔과 페이지 속성 드롭다운 목록에서 구성을 식별합니다.
   1. 필요한 경우 구성을 저장하는 저장소 노드에 사용하기 위해 **이름**&#x200B;을 입력합니다.
...
| 속성 | 설명 |
|---|---|
| 번역 방법 | 이 속성은 프레임워크가 사이트 콘텐츠에 대해 수행하는 번역 방법을 정의합니다. <br>- 기계 번역: 번역 제공업체가 기계 번역을 사용해 실시간으로 번역을 진행합니다.<br>- 사람 번역: 콘텐츠를 번역사가 번역할 수 있도록 번역 제공업체로 보냅니다.<br>- 번역하지 않음: 콘텐츠를 번역하도록 보내지 않습니다. 번역하지는 않지만 최신 콘텐츠로 업데이트할 수 있는 특정 콘텐츠 분기를 건너뛰기합니다. |
| 번역 제공업체 | 이 속성은 번역을 수행할 번역 제공업체를 정의합니다. 해당 커넥터를 설치하면 목록에 제공업체가 표시됩니다. |
...
에셋 속성은 에셋 구성 방법을 제어합니다. 에셋 번역에 대한 자세한 내용은 [에셋에 대한 언어 복사본 만들기](/help/assets/translate-assets.md)를 참조하십시오.

![사이트에 대한 번역 구성](../assets/translation-configuration-assets.png)

| 속성 | 설명 |
|---|---|
| 번역 방법 | 이 속성은 프레임워크가 에셋에 대해 수행하는 번역 유형을 선택합니다. <br>- 기계 번역: 번역 제공업체가 기계 번역을 사용해 즉시 번역을 진행합니다.<br>- 사람 번역: 콘텐츠를 사람이 직접 번역할 수 있도록 자동으로 번역 제공업체에 보냅니다.<br>- 번역하지 않음: 에셋을 번역하도록 보내지 않습니다. |
| 번역 제공업체 | 이 속성은 번역을 수행할 번역 제공업체를 정의합니다. 해당 커넥터를 설치하면 목록에 제공업체가 표시됩니다. |
...
* AEM을 번역 제공업체에 연결하는 클라우드 구성
* 번역의 세부 정보를 구성하는 번역 통합 프레임워크

번역 통합 프레임워크 클라우드 구성이 서비스 제공업체와의 연결에 사용하는 클라우드 구성을 식별한다는 점을 참고하십시오. 소스 페이지를 프레임워크 클라우드 구성과 연결할 때 프레임워크 클라우드 구성이 사용하는 서비스 제공업체 클라우드 구성과 페이지를 연결해야 합니다.
...
### 페이지를 번역 제공업체와 연결 {#associating-a-page-with-a-translation-provider}

페이지와 하위 페이지를 번역하기 위해 이용하는 번역 제공업체에 페이지를 연결합니다.
