...
AEM을 번역 서비스 공급업체에 연결하는 클라우드 구성을 만듭니다. AEM에는 기본적으로 Microsoft® Translator](connect-ms-translator.md)에 연결하는 기능이 [포함되어 있습니다.
...
커넥터 패키지를 설치하면 커넥터에 대한 클라우드 구성을 만들 수 있습니다. 일반적으로 번역 서비스를 사용하여 인증하려면 자격 증명을 제공해야 합니다. Microsoft Translator 커넥터에 대한 클라우드 구성을 추가하는 방법에 대한 자세한 내용은 Microsoft®® Translator](connect-ms-translator.md)와 통합을 참조하세요[.
...
컨텐츠 번역 방법을 지정할 수 있도록 번역 통합 프레임워크 구성을 만들기. 구성에는 다음 정보가 포함됩니다.
...
1. [글로벌 탐색 메뉴에서](/help/sites-cloud/authoring/basic-handling.md#global-navigation) 도구 > Cloud Services 및 번역 Cloud Services **을 선택합니다**.
1. 콘텐츠 구조에서 구성을 만들고자 하는 위치로 이동합니다. 보통 특정 사이트를 토대로 하거나 전역일 수 있습니다.
1. 필드에 다음 정보를 제공한 다음, 만들기&#x200B;**.를 선택합니다**.
...
1. **편집 구성 창의 Sites** 및 **Assets** 탭에서 속성을 **구성**&#x200B;한 다음 저장 및 닫기&#x200B;**을 선택합니다**.
...
| 업데이트 전용 번역 사용 중지 | 이 옵션을 선택하면 번역 프로젝트를 업데이트하면 마지막 번역 이후 변경된 필드뿐만 아니라 번역 가능한 모든 필드가 번역을 위해 제출됩니다. |
...
| 메타데이터 번역 | 자산 메타데이터 번역할 수 있도록 이 속성 활성화합니다. |
| 태그 번역 | 이 속성을 활성화하면 자산과 연결된 태그를 번역할 수 있습니다. |
| 자동 실행 번역 | 번역 프로젝트를 만든 후 자동으로 번역 작업을 실행할 수 있도록 이 속성 속성을 선택합니다. 이 옵션을 선택하면 번역 작업을 검토하거나 살필 수 없습니다. |
| 업데이트 전용 번역 사용 중지 | 이 옵션을 선택하면 번역 프로젝트를 업데이트하면 마지막 번역 이후 변경된 필드뿐만 아니라 번역 가능한 모든 필드가 번역을 위해 제출됩니다. |
| 번역을 위해 콘텐츠 모델 필드 활성화 | 이 옵션을 활성화하면 컨텐츠 조각 모델의](/help/sites-cloud/administering/content-fragments/content-fragment-models.md#properties) 번역 가능&#x200B;**필드를 [사용하여**&#x200B;필드가 번역되었는지 확인하고 그에 따라 번역 규칙을](rules.md) 자동으로 만듭니다[. 이 옵션은 사용자가 만든 모든 번역 규칙을 대체합니다. |
...
번역 통합 프레임워크 클라우드 구성은 서비스 제공자에 연결하는 데 사용할 클라우드 구성을 식별합니다. 소스 페이지를 프레임워크 클라우드 구성과 연결할 때 프레임워크 클라우드 구성이 사용하는 서비스 공급업체 클라우드 구성과 페이지를 연결해야 합니다.

페이지를 클라우드 구성과 연결할 때 페이지의 하위 영역은 연결에 상속됩니다. 예를 들어 페이지를 `/content/wknd/language-masters/en/magazine` 번역 통합 프레임워크와 연결한 경우 그 아래의 페이지와 하위 페이지는 프레임워크 `magazine` 에 따라 번역됩니다.

필요에 따라 하위 페이지에 연결을 재정의할 수 있습니다. 웹 사이트의 콘텐츠가 주로 여행과 라이프스타일에 관한 내용인 경우를 예로 들어 보겠습니다. 페이지 중 한 분기가 회사에 대해 설명합니다. 이 경우 사이트의 루트 페이지 는 라이프스타일 카테고리를 사용하여 기계 번역을 지정하는 번역 통합 프레임워크와 연결될 수 있습니다. 회사를 설명하는 분기 일반 범주를 사용하여 기계 번역을 수행하는 프레임워크를 사용합니다.

### 페이지를 번역 공급업체와 연결 {#associating-a-page-with-a-translation-provider}

페이지와 하위 페이지를 번역하기 위해 이용하는 번역 공급업체에 페이지를 연결합니다.

1. 사이트 콘솔에서 구성할 페이지 선택하고 보기 속성&#x200B;**선택합니다**.
1. **클라우드 서비스** 탭을 선택합니다.
1. Add Configuration(컨피그레이션 추가) **드롭다운 목록에서 컨피그레이션** 을 선택합니다.
1. **저장 후 닫기**&#x200B;를 선택합니다.
