...
AEM을 번역 서비스 공급자에 연결하는 클라우드 구성을 만듭니다. AEM에는 다음과 같은 기능이 포함되어 있습니다. [Microsoft Translator에 연결](connect-ms-translator.md) 기본적으로 제공됩니다.
...
번역 통합 프레임워크를 구성한 후 다음을 수행할 수 있습니다 [페이지와 연결](preparation.md) 그걸 사용해요

>[!TIP]
>
>AEM의 컨텐츠 번역 기능에 대한 개요는 다음을 참조하십시오 [다국어 사이트의 컨텐츠 번역](overview.md).

프레임워크의 단일 구성은 페이지 컨텐츠와 자산이 번역되는 방식을 제어합니다. 새 번역 구성을 만들려면 다음을 수행하십시오.

1. 에서 [전역 탐색 메뉴,](/help/sites-cloud/authoring/getting-started/basic-handling.md#global-navigation) 클릭 또는 탭 **도구 -> Cloud Services - 및 번역 Cloud Services**.
...
1. 에서 **구성 편집** 창에서 **Sites** 및 **자산** 탭을 클릭한 다음 탭하거나 탭합니다 **저장 및 닫기**.

### 사이트 구성 속성 {#sites-configuration-properties}

다음 **Sites** 탭은 페이지 컨텐츠 번역 수행 방식을 제어합니다.

![사이트에 대한 번역 구성](../assets/translation-configuration.png)

| 속성 | 설명 |
|---|---|
| 번역 방법 | 이 속성은 프레임워크가 사이트 컨텐츠에 대해 수행하는 번역 방법을 정의합니다.<br>- 기계 번역: 번역 제공자는 기계 번역을 실시간으로 이용하여 번역을 수행한다.<br>- 인간 번역: 번역자가 번역하기 위해 콘텐츠가 번역 공급자에게 전송됩니다.<br>- 번역하지 않음: 컨텐츠를 번역하도록 전송되지 않습니다. 이는 번역되지 않지만 최신 콘텐츠로 업데이트할 수 있는 특정 콘텐츠 분기를 건너리기 위한 것입니다. |
...
| 페이지 자산 번역 | 이 속성은 파일 시스템에서 구성 요소에 추가되거나 자산에서 참조되는 자산을 변환하는 방법을 정의합니다.<br>- 번역하지 않음: 페이지 자산은 번역되지 않습니다.<br>- 사이트 번역 워크플로우 사용: 자산은 의 구성 속성에 따라 처리됩니다 **Sites** 탭.<br>- 자산 번역 워크플로우 사용: 자산은 **자산** 탭. |
...
자산 속성은 자산을 구성하는 방법을 제어합니다. 자산 번역에 대한 자세한 내용은 [자산에 대한 언어 사본 만들기](/help/assets/translate-assets.md).

![사이트에 대한 번역 구성](../assets/translation-configuration-assets.png)

| 속성 | 설명 |
|---|---|
| 번역 방법 | 이 속성은 프레임워크가 자산에 대해 수행하는 번역 유형을 선택합니다.<br>- 기계 번역: 번역 공급자는 기계 번역을 사용하여 즉시 번역을 수행합니다.<br>- 인간 번역: 컨텐츠를 수동으로 번역하려면 번역 공급자에게 자동으로 전송됩니다.<br>-번역하지 않음: 자산은 번역을 위해 전송되지 않습니다. |
| 번역 공급자 | 이 속성은 번역을 수행할 번역 공급자를 정의합니다. 해당 커넥터가 설치되면 공급자가 목록에 나타납니다. |
...
페이지를 클라우드 구성과 연결할 때 페이지의 하위 항목은 연결을 상속합니다. 예를 들어, `/content/wknd/language-masters/en/magazine` 페이지가 번역된 통합 프레임워크인 `magazine` 아래의 페이지 및 하위 페이지는 프레임워크에 따라 번역됩니다.
...
1. 사이트 콘솔에서 구성할 페이지를 선택하고 클릭 또는 탭합니다 **속성 보기**.
1. 을 클릭하거나 탭합니다 **Cloud Services** 탭.
1. 에서 **구성 추가** 드롭다운에서 구성을 선택합니다.
1. 클릭 또는 탭 **저장 및 닫기**.
