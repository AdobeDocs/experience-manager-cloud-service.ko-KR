...
예를 들어, 작성자가 페이지의 모든 텍스트 구성 요소에 추가하는 컨텐츠를 변환하는 규칙을 만들 수 있습니다. 이 규칙은 `/content` 노드 및 `core/wcm/components/text/v2/text` 구성 요소에 대한 `text` 속성을 식별할 수 있습니다.
...
다음 예제 규칙은 모든 `text` 속성의 컨텐츠가 `/content` 노드 아래의 모든 페이지에 대해 번역됩니다. 이 규칙은 텍스트 구성 요소와 같이 `text` 속성에 컨텐츠를 저장하는 구성 요소에 적용됩니다.
...
다음 예제에서는 모든 `text` 속성의 콘텐츠를 변환하고 이미지 구성 요소의 다른 속성도 변환합니다. 다른 구성 요소에 이름이 같은 속성이 있는 경우 규칙이 적용되지 않습니다.
...
1. 그런 다음 컨텍스트를 선택하고 **편집**&#x200B;을 클릭해야 합니다. 그러면 번역 규칙 편집기가 열립니다.
...
예를 들어, 초안 컨텐츠에 플래그를 지정하려면 상위 노드에 속성 `draftOnly`이 true로 설정된 경우에도 하위 노드가 번역 작업에 추가됩니다. 여기서 `isDeep`은(는) 재생되며 상위 노드에 속성 `draftOnly`이 true인지 확인하고 해당 하위 노드를 제외합니다.

편집기에서 **필터** 탭에서 **심층**&#x200B;을 선택/선택 취소할 수 있습니다.

![규칙 필터링](../assets/translation-rules-editor-filters.png)

다음은 UI에서 **Is Deep** 가 선택 취소되는 결과 XML의 예입니다.
...
**`updateDestinationLanguage`** 텍스트가 아닌 언어 코드가 있는 속성(예:  `jcr:language`)에 사용됩니다. 사용자가 텍스트를 번역하지 않고 언어 로케일을 원본에서 대상으로 합니다. 이러한 속성은 번역용으로 전송되지 않습니다.

UI에서는 **속성** 탭에서 **변환**&#x200B;을 선택/선택 취소하여 이 값을 수정할 수 있지만, 언어 코드를 값으로 갖는 특정 속성에 대해서는 이 값을 수정할 수 있습니다.
...
`translation_rules.xml` 파일을 편집하는 경우 컨텐츠 패키지에 백업 복사본을 유지합니다. 특정 AEM 패키지를 다시 설치하면 현재 `translation_rules.xml` 파일을 원본으로 바꿀 수 있습니다. 이러한 상황에서 규칙을 복원하려면 백업 복사본이 포함된 패키지를 설치할 수 있습니다.
