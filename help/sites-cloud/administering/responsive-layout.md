...
>[!NOTE]
>
>에서 사이트 생성 시 [Project Archetype](#addlink) 또는 [표준 사이트 템플릿](#addlink): 일반적으로 응답형 레이아웃이 구성됩니다. 그렇지 않으면 다음을 수행해야 합니다. [레이아웃 컨테이너 구성 요소 활성화](#enable-the-layout-container-component-for-page) 을 참조하십시오.

## 에뮬레이터 활성화 {#enabling-emulator}

다음 [Project Archetype](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/overview.html) 및 [표준 사이트 템플릿](/help/sites-cloud/administering/site-creation/site-templates.md#standard-site-template) 는 이미 에뮬레이터를 사용할 수 있도록 설정되어 있습니다. 핵심 구성 요소 또는 Archetype을 기반으로 하지 않고 직접 콘텐츠를 개발한 경우 문서를 참조하십시오 [반응형 디자인](/help/implementing/developing/introduction/responsive-design.md) 구성 요소를 개발하면서 이러한 기능을 활용하는 방법에 대한 자세한 내용

## 사이트에 대한 레이아웃 모드 활성화 {#activate-layout-mode-for-your-site}

**레이아웃** 모드에서는 에뮬레이터를 사용하여 디바이스별로 콘텐츠의 레이아웃을 조정할 수 있습니다. WKND 샘플 사이트가에 대해 이미 활성화되었습니다. **레이아웃** 모드. 다음 단계에 따라 사이트를 활성화하십시오.

### 중단점 구성 {#configure-breakpoints}

중단점은 반응형 디자인에 중요하며 콘텐츠를 타겟 디바이스에 조정하는 방법 및 시기를 정의합니다. 하지만 소개하는 각 중단점은 콘텐츠를 수용할 수 있도록 작성자에게 추가 작업을 생성하므로 주의하십시오. 기본 중단점이 항상 있는 경우를 포함하여 두 개의 중단점으로 충분할 수 있습니다. Adobe은 기본값을 포함하여 3개 이상의 중단점을 생성하지 않는 것을 권장합니다(즉, 아래에 2개 이하의 노드) `cq:responsive/breakpoint`.
...
구성 요소 크기 조정 **레이아웃** 모드는 WKND 샘플 사이트에서 사용할 수 있는 응답형 디자인의 중요한 부분입니다. 다음 단계에 따라 사이트를 활성화하십시오.
...
다음을 만들어야 합니다. [클라이언트 라이브러리](/help/implementing/developing/introduction/clientlibs.md) 추가 구성 및 함수 호출을 제공합니다. 다음 LESS 추출은 프로젝트에 추가해야 하는 최소값의 예입니다.
