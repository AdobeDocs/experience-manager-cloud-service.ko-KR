...
중단점은 반응형 디자인에 중요하며 콘텐츠를 타겟 디바이스에 조정하는 방법 및 시기를 정의합니다. 하지만 소개하는 각 중단점은 콘텐츠를 수용할 수 있도록 작성자에게 추가 작업을 생성하므로 주의하십시오. 기본 중단점이 항상 있는 경우를 포함하여 두 개의 중단점으로 충분할 수 있습니다. Adobe에서는 기본값을 포함하여 중단점을 3개 이상 만들지 않는 것이 좋습니다(즉, `cq:responsive/breakpoint` 아래에 있는 노드를 2개 이하로 만드는 것이 좋습니다).
...
* CRXDE Lite 또는 XML을 사용하여 중단점을 정의할 수 있습니다.
