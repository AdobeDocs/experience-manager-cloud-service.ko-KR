...
* 특정 오해에 대한 설명과 함께 여러 팁에 대한 [MSM 모범 사례](best-practices.md)를 확인하십시오.
...
* `cq:LiveSync` 믹스인 유형
   ...
   * 이들 페이지에는 다음 속성을 통해 Live Copy에 대한 기본 정보 및 필수 정보를 포함하는 유형 `cq:LiveCopy`의 `cq:LiveSyncConfig` 하위 노드가 있습니다.
...
이러한 속성에 대해 알고 있고 있으면 저장소를 쿼리하고 특정 상태에 있는 페이지 세트를 찾는 데에도 유용할 수 있습니다. 예:
...
이 항목에 대한 자세한 내용은 [MSM 모범 사례](best-practices.md)를 참조하세요.

### 작성자 그룹에 대해 롤아웃 권한을 제거하려면 어떻게 해야 합니까? {#remove-rollout-permissions}

Adobe Experience Manager 주체(사용자 또는 그룹)에 대해 삭제하거나 제거할 수 있는 **롤아웃** 권한은 없습니다.
...
블루프린트 페이지가 롤아웃되면 해당 Live Copy 페이지가 업데이트되거나 아직 존재하지 않는 경우 Live Copy 페이지가 생성됩니다. 예: 처음 롤아웃되었거나 Live Copy 페이지가 수동으로 삭제된 경우.

그러나 후자의 경우, 같은 이름의 `cq:LiveRelationship` 속성이 없는 페이지가 존재하면 Live Copy 페이지가 생성되기 전에 이 페이지의 이름이 그에 따라 변경됩니다.

기본적으로 롤아웃되면 블루프린트 업데이트에 연결된 Live Copy 페이지가 롤아웃됩니다. 또는 페이지가 없거나, Live Copy 페이지가 생성됩니다.
...
이 문제를 방지하려면 **분리**&#x200B;가 아닌 Live Copy **일시 중단** 기능을 사용합니다. **분리** 작업에 대한 자세한 내용은 [이 문서](creating-live-copies.md)에서 확인할 수 있습니다.
