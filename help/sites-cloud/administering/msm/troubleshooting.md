...
다음을 참조하십시오 [이 문서](best-practices.md) 을 참조하십시오.
...
블루프린트 페이지가 롤아웃되면 해당 라이브 카피 페이지가 업데이트되거나 아직 존재하지 않는 경우(예: 처음 롤아웃되었거나 라이브 카피 페이지가 수동으로 삭제된 경우) 라이브 카피 페이지가 생성됩니다.
...
이 문제를 방지하려면 **분리**&#x200B;가 아닌 Live Copy **일시 중단** 기능을 사용하십시오. **분리** 작업에 대한 자세한 내용은 [이 문서](creating-live-copies.md)를 참조하십시오.
