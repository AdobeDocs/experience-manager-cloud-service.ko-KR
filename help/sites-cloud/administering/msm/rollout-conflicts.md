...
* 블루프린트: `/b`
   ...
   1개의 하위 페이지가 있는 마스터 페이지, `bp-level-1`

* Live Copy: `/b`

   1개의 하위 페이지가 있는 Live Copy 분기에서 수동으로 만든 페이지, `lc-level-1`

   * 다음으로 게시 시 활성화됨 `/b`와 하위 페이지 모두 포함
...
|---|---|---|---|
| 값 | `b` | `b` | `b` |
| 주석 | 블루프린트 분기에 만들어짐, 롤아웃 준비 | Live Copy 분기에서 수동으로 만들기 | 페이지의 컨텐츠를 포함합니다 `b` Live Copy 분기에서 수동으로 생성된 |
| 값 | `/bp-level-1` | `/lc-level-1` | `/lc-level-1` |
| 주석 |  | Live Copy 분기에서 수동으로 만들기 | 에는 페이지의 컨텐츠가 포함되어 있습니다 `child-level-1` Live Copy 분기에서 수동으로 생성된 |

## 롤아웃 관리자 및 충돌 처리 {#rollout-manager-and-conflict-handling}

롤아웃 관리자를 사용하여 충돌 관리를 활성화하거나 비활성화할 수 있습니다.

이 작업은 [OSGi 구성](/help/implementing/deploying/configuring-osgi.md) 의 **Day CQ WCM Rollout Manager**. 값 설정 **수동으로 만든 페이지와 충돌 처리** ( `rolloutmgr.conflicthandling.enabled`)를 true로 설정합니다. 롤아웃 관리자가 블루프린트에 있는 이름으로 Live Copy에서 만든 페이지의 충돌을 처리해야 합니다.

AEM에 있음 [충돌 관리가 비활성화되었을 때 미리 정의된 동작입니다.](#behavior-when-conflict-handling-deactivated)
...
* 다음 [기본 충돌 처리기](#default-conflict-handler):
...
기본 충돌 처리기는 다음과 같습니다. `ResourceNameRolloutConflictHandler`

* 이 핸들러를 사용하면 블루프린트 페이지에 우선 순위가 지정됩니다.
* 이 처리기의 서비스 등급이 낮음으로 설정되어 있습니다. 즉, `service.ranking` 속성은 사용자 지정된 핸들러가 더 높은 등급을 필요로 한다고 가정합니다. 그러나 필요할 때 등급을 유연하게 사용하기 위한 절대 최소는 아닙니다.

이 충돌 처리기가 블루프린트에 우선합니다. 예: Live Copy 페이지 `/b` 는 Live Copy 분기 내에서 `/b_msm_moved`.

* Live Copy: `/b`

   Live Copy 내에서 로 이동됩니다. `/b_msm_moved`. 이렇게 하면 백업 기능이 수행되며 컨텐츠가 손실되지 않습니다.
...
|---|---|---|---|---|
| 값 | `b` | `b` | `b_msm_moved` | `b` |
| 주석 |  | 블루프린트 페이지의 컨텐츠가 있습니다 `b` 그 말이 | 페이지의 컨텐츠가 있습니다. `b` Live Copy 분기에서 수동으로 생성된 | 변경 사항이 없으며 원래 페이지의 컨텐츠가 포함됩니다 `b` Live Copy 분기에서 수동으로 만든 후 이제 라고 합니다. `b_msm_moved` |
...
* 을 사용하여 구성할 수 있도록 설계할 수 있습니다. [OSGi 구성](/help/implementing/deploying/configuring-osgi.md). 특히
   * **서비스 등급** 다른 충돌 핸들러와 관련된 순서를 정의합니다( `service.ranking`).
      * 기본값은 `0`입니다.

### 충돌 처리가 비활성화된 경우 동작 {#behavior-when-conflict-handling-deactivated}

수동으로 [충돌 처리 비활성화,](#rollout-manager-and-conflict-handling) AEM에서는 충돌하는 페이지에서 작업을 수행하지 않습니다. 충돌하지 않는 페이지는 예상대로 롤아웃됩니다.

>[!CAUTION]
>
>충돌 처리가 비활성화되면 AEM에서 충돌이 무시되고 있음을 표시하지 않습니다. 이러한 경우 이 동작은 명시적으로 구성해야 하므로 원하는 동작으로 간주됩니다.
...
|---|---|---|---|
| 값 | `b` | `b` | `b` |
| 주석 |  | 변경 사항이 없으며 페이지의 컨텐츠가 있습니다. `b` Live Copy 분기에서 수동으로 생성된 | 변경 사항이 없으며 페이지의 컨텐츠가 포함되어 있습니다 `b` Live Copy 분기에서 수동으로 생성된 |
| 값 | `/bp-level-1` | `/lc-level-1` | `/lc-level-1` |
| 주석 |  | 변경 없음 | 변경 없음 |

### 서비스 등급 {#service-rankings}

다음 [OSGi](https://www.osgi.org/) 서비스 등급을 사용하여 개별 충돌 처리기의 우선 순위를 정의할 수 있습니다.
