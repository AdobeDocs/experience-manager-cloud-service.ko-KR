...
1. 선택 **인스턴스** 현재 진행 중인 실행 중인 워크플로 인스턴스 목록을 표시합니다.
1. 상단 레일의 오른쪽 모서리에 워크플로 인스턴스가 표시됩니다 **실행 중인 워크플로우**, **상태**, 및 **세부 사항**.
1. **실행 중인 워크플로우** 실행 중인 워크플로의 수와 상태를 표시합니다. 예를 들어 주어진 이미지에서 은 의 수입니다. **실행 중인 워크플로우** 및 **상태** AEM 인스턴스의 입니다.
...
1. 대상 **상태 세부 정보** 워크플로우 인스턴스 중 **세부 사항**&#x200B;을 클릭하여 다음을 표시합니다. **실행 중인 워크플로 인스턴스 수**, **완료된 워크플로 인스턴스**, **중단된 워크플로 인스턴스**, **실패한 워크플로 인스턴스**&#x200B;등. 예를 들어 아래에 표시된 주어진 이미지가 있습니다. **상태 세부 정보** 포함 **상태: 정상** 및 **상태: 비정상**.
   ...
   >[!NOTE]
   >
   > 워크플로우 인스턴스를 정상 상태로 유지하려면 의 모범 사례를 따르십시오. [정기적인 워크플로 인스턴스 제거](#regular-purging-of-workflow-instances) 또는 [워크플로우 모범 사례](https://experienceleague.adobe.com/docs/experience-manager-65/developing/extending-aem/extending-workflows/workflows-best-practices.html?lang=en).
...
1. 특정 항목을 선택한 다음 **종료**, **일시 중단** 또는 **재시작**&#x200B;을 적절히 사용합니다. 확인 및/또는 세부 정보가 필요합니다.
   ...
   >
   >
   >워크플로우를 종료하거나 중단하려면 참가자 단계와 같이 사용자 작업을 기다리는 상태여야 합니다. 현재 실행 중인 작업(실행 중인 활성 스레드)을 중단하려고 하면 예상한 결과가 나오지 않을 수 있습니다.
...
1. **보관**&#x200B;을 선택하여 정상적으로 완료된 워크플로 인스턴스 목록을 표시합니다.
   ...
   ![보관된 인스턴스](/help/sites-cloud/administering/assets/archived-instances.png)
...
* **종료 후 다시 시도** 과 유사 **종료** 단, 새 워크플로우 인스턴스가 원래 페이로드, 제목 및 설명을 사용하여 시작됩니다.
...
![워크플로 실패](/help/sites-cloud/administering/assets/workflow-failure.png)
...
여러 서비스 구성을 생성하여 서로 다른 기준을 충족하는 워크플로 인스턴스를 제거할 수도 있습니다. 예를 들어 특정 워크플로 모델의 인스턴스가 예상 시간보다 오래 실행될 때 해당 인스턴스를 제거하는 구성을 생성할 수 있습니다. 저장소 크기를 최소화하기 위해 며칠 후에 완료된 모든 워크플로를 제거하는 다른 구성을 생성할 수도 있습니다.
...
변수를 외부에 저장하려면 워크플로에서 표시하는 API를 구현해야 합니다.
