...
이 레이아웃은 응답형 격자 내에 구성 요소를 배치할 수 있도록 해주는 단락 시스템을 제공합니다. 이 격자를 사용하면 장치/창 크기 및 형식에 따라 레이아웃을 다시 정렬할 수 있습니다. The component is used in conjunction with the [**Layout **mode](/help/sites-cloud/authoring/fundamentals/environment-tools.md#page-modes), which allows you to create and edit your responsive layout dependent on device.
...
* [**레이아웃 컨테이너&#x200B;**](#adding-a-layout-container-and-its-content-edit-mode)구성 요소
   ...
   This component is available in the [component browser](/help/sites-cloud/authoring/fundamentals/environment-tools.md#components-browser) and provides a grid-paragraph system to allow you to add and position components within a responsive grid. 또한 페이지에서 기본 단락 시스템으로 설정할 수도 있습니다.
...
>[!NOTE]
>
>위의 메커니즘은 템플릿에 대한 구성으로 사용할 수 있게 됩니다. See Configuring Responsive Layout for further information. <!-- Use of the above mechanisms is enabled by configuration on the template. See [Configuring Responsive Layout](/help/sites-administering/configuring-responsive-layout.md) for further information.-->
...
1. 에뮬레이터 도구 모음이 열립니다.
   ...
   * **장치** 회전 - 장치를 세로(세로) 방향에서 가로(가로) 방향으로 또는 그 반대로 회전할 수 있습니다.
   ![장치 가로 회전 단추](/help/sites-cloud/authoring/assets/responsive-layout-rotate-device-landscape-button.png)
   ![장치 세로 회전 단추](/help/sites-cloud/authoring/assets/responsive-layout-rotate-device-portrait-button.png)
...
1. 에뮬레이션할 특정 장치를 선택하기 위해 다음 중 하나를 수행할 수 있습니다.
   ...
   * 에뮬레이터 도구 모음에서 장치 표시기 클릭/탭
   ![장치 드롭다운 선택](/help/sites-cloud/authoring/assets/responsive-layout-select-device-dropdown.png)
...
1. 특정 장치를 선택하면 다음 작업을 수행할 수 있습니다.
   ...
   * The blue dotted line represents the *fold* for the selected device (here an **iPhone 6 Plus** in landscape).
   ![접힌 것](/help/sites-cloud/authoring/assets/responsive-layout-fold.png)
...
>[!NOTE]
>
>아직 사용할 수 없는 경우 단락 **시스템** /페이지에 대해 레이아웃 컨테이너를 명시적으로 활성화해야 합니다. <!-- If not already available, the **Layout Container** must be explicitly [activated for a paragraph system/page](/help/sites-administering/configuring-responsive-layout.md).-->
...
1. 레이아웃 구성 요소가 중첩된 경우 **상위** 옵션을 선택하면 드롭다운 선택 사항이 표시되어 중첩된 레이아웃 컨테이너 또는 상위 항목을 선택할 수 있습니다.
   ...
   * 가장 낮은 중첩 레이아웃 컨테이너는 파란색으로 윤곽이 표시됩니다.
   * 연속적인 모든 컨테이너는 연한 파란색이 됩니다.
...
* 컨텐츠 구성 요소를 클릭/탭합니다. 도구 모음에서는 다음 작업을 수행할 수 있습니다.
   * **상위** - 전체 레이아웃 컨테이너 구성 요소를 선택하여 전체 작업을 수행할 수 있습니다.
   * **새 라인으로** 이동 - 구성 요소는 격자 내에서 사용 가능한 공간에 따라 새 라인으로 이동합니다.
...
* In **Layout** mode you can tap/click on the **Drag components here** to select the entire component. 그러면 이 모드에 대한 도구 모음이 표시됩니다.
   ...
   * **숨겨진 구성 요소** 표시 - 모든 또는 개별 구성 요소를 표시합니다.숫자는 현재 존재하는 숨겨진 구성 요소의 수를 나타냅니다. 카운터는 숨겨진 구성 요소 수를 표시합니다.
   ...
   * **새 라인으로 이동** - 공간이 허용되는 경우 구성 요소를 위로 이동합니다.
      ...
      ![새 선 단추로 이동](/help/sites-cloud/authoring/assets/responsive-layout-float-button.png)
