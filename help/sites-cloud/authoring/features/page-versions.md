...
* 상단 [작업 도구 모음](/help/sites-cloud/authoring/getting-started/basic-handling.md#actions-toolbar)의 **복원** 옵션
   ...
   * **버전 복원**
     ...
     현재 선택한 폴더 내에 있는 지정된 페이지의 버전을 복원합니다. 또한 이전에 삭제된 페이지를 복원할 수도 있습니다.

   * **트리 복원**

     지정된 날짜 및 시간의 전체 트리 버전을 복원합니다. 이전에 삭제된 페이지도 포함할 수 있습니다.

>[!NOTE]
>
>페이지를 복원하면 생성된 버전은 새 분기의 일부입니다.
>
>예시:
>
>1. 페이지의 버전을 임의로 만듭니다.
...
이 방법을 사용하여 현재 폴더 내에 지정된 페이지의 버전을 복원할 수 있습니다. 또한 이전에 삭제된 페이지를 복원할 수도 있습니다.
...
1. 특정 페이지의 경우 아래의 드롭다운 선택기를 사용합니다. **버전으로 복원** 을 눌러 해당 페이지에 필요한 버전을 선택합니다.
...
이 방법을 사용하여 지정된 날짜 및 시간의 트리 버전을 복원할 수 있습니다. 이전에 삭제된 페이지도 포함할 수 있습니다.
...
1. 에서 날짜 및 시간 선택기 사용 **날짜의 최신 버전** 복원할 트리의 다른 버전을 선택할 수 있습니다.
...
1. 대화 상자에서 대상 날짜 및 시간을 설정하고 을 클릭합니다. **날짜 설정**. 시간을 선택하지 않으면 기본적으로 현재 시간을 사용합니다.
...
타임워프는 선택한 시점의 페이지를 재현하는 데 최선의 노력을 다합니다. 그러나 AEM에서 콘텐츠를 지속적으로 작성하는 과정의 복잡성으로 인해 항상 이러한 복제가 가능한 것은 아닙니다. 타임워프를 사용할 때는 이러한 제한 사항을 염두에 두십시오.

* **타임워프는 게시된 페이지를 기반으로 작동합니다** - 이전에 페이지를 게시한 경우에만 타임워프가 완전히 작동합니다. 그렇지 않으면 작성 환경에 현재 페이지가 표시됩니다.
...
* **타임워프는 읽기 전용입니다.** 페이지의 이전 버전은 편집할 수 없으며, 볼 수만 있습니다. 이전 버전을 복원하려면 다음을 사용하여 수동으로 복원해야 합니다 [복원](#revert-to-a-version).
* **타임워프는 페이지 콘텐츠를 기반으로 합니다.** - 웹 사이트를 렌더링하기 위한 요소(예: 코드, css 및 에셋)가 변경된 경우 보기가 원래와 다릅니다. 이러한 항목은 저장소에서 버전이 관리되지 않습니다.
