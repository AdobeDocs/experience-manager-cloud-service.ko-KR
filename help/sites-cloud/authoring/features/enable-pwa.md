...
1. **점진적 웹 앱** 탭을 선택하고 해당 속성을 구성합니다. 최소 다음 작업을 수행할 수 있습니다.
   1. **PWA 사용** 옵션을 선택합니다.
   1. **시작 URL**&#x200B;을 정의합니다.

      ![PWA 사용](../assets/pwa-enable.png)

   1. DAM에 512x512 png 아이콘을 업로드하고 이를 앱의 아이콘으로 참조합니다.

      ![PWA 정의 아이콘](../assets/pwa-icon.png)

   1. 서비스 워커에서 오프라인 상태로 전환하려는 경로를 구성합니다. 일반적인 경로는 다음과 같습니다.
      * `/content/<sitename>`
...
## PWA 사용 사이트 사용 {#using-pwa-enabled-site}

사이트 PWA을 지원하도록 [을 구성했다면 직접 체험해 볼 수 있습니다.](#enabling-pwa-for-your-site)
...
1. 브라우저의 주소 표시줄에 사이트가 로컬 앱으로 설치될 수 있음을 나타내는 `+` 아이콘이 표시됩니다.
   * 브라우저에 따라 로컬 앱으로 설치할 수 있음을 알리는 알림(예: 배너 또는 대화 상자)을 표시할 수도 있습니다.
1. 앱을 설치합니다.
...
* **PWA**  활성화 - 사이트의 PWA을 활성화하는 주요 토글입니다.
* **시작 URL**  - 사용자가 로컬로 설치된 응용 프로그램을 로드할 때 응용 프로그램이  [여는 기본 설정 시작 ](https://developer.mozilla.org/en-US/docs/Web/Manifest/start_url) URL입니다.
   * 이는 콘텐츠 구조의 모든 경로가 될 수 있습니다.
   * 루트가 될 필요는 없으며 앱의 전용 시작 페이지인 경우가 많습니다.
   * 이 URL이 상대적이면 매니페스트 URL이 이를 확인하기 위한 기본 URL로 사용됩니다.
   * 비어 있으면 이 기능은 앱이 설치된 웹 페이지의 주소를 사용합니다.
   * 값을 설정하는 것이 좋습니다.
* **표시 모드**  - PWA을 사용하는 앱은 여전히 브라우저를 통해 제공되는 AEM 사이트입니다. [이러한 표시 ](https://developer.mozilla.org/en-US/docs/Web/Manifest/display) 옵션은 로컬 장치의 사용자에게 브라우저를 숨기거나 표시하는 방법을 정의합니다.
   * **독립 실행형**  - 브라우저가 사용자로부터 완전히 숨겨져 있으며 기본 앱처럼 나타납니다. 이 값이 기본값입니다.
      * 이 옵션을 사용하면 브라우저의 내비게이션 컨트롤을 사용하지 않고 사이트의 페이지에 있는 링크와 구성 요소를 사용하여 전체 컨텐츠의 앱 탐색을 수행할 수 있습니다.
   * **브라우저**  - 사이트를 방문할 때와 같은 방식으로 브라우저가 나타납니다.
   * **최소 UI**  - 브라우저는 대부분 기본 앱과 같이 숨겨지지만 기본 탐색 컨트롤은 노출됩니다.
   * **전체 화면**  - 브라우저가 기본 앱과 마찬가지로 완전히 숨겨져 있지만 전체 화면 모드로 렌더링됩니다.
      * 이 옵션을 사용하면 브라우저의 내비게이션 컨트롤을 사용하지 않고 사이트의 페이지에 있는 링크와 구성 요소를 사용하여 전체 컨텐츠의 앱 탐색을 수행할 수 있습니다.
* **화면 방향**  - PWA은 로컬 앱으로  [장치 방향을 처리하는 방법을 알아야 합니다.](https://developer.mozilla.org/en-US/docs/Web/Manifest/orientation)
   * **임의**  - 사용자의 장치 방향에 따라 응용 프로그램이 조정됩니다. 이 값이 기본값입니다.
   * **세로**  - 사용자 장치의 방향에 관계없이 응용 프로그램이 세로 레이아웃에서 강제로 열립니다.
   * **가로**  - 이렇게 하면 사용자의 장치 방향에 관계없이 앱이 가로 레이아웃으로 열립니다.
* **테마 색상**  - 로컬 사용자의 운영 체제에서 기본 UI 도구 모음 및 탐색 컨트롤을 표시하는 방법에 영향을 주는  [응용 프로그램의 ](https://developer.mozilla.org/en-US/docs/Web/Manifest/theme_color) 색상을 정의합니다. 브라우저에 따라 다른 앱 프레젠테이션 요소에 영향을 줄 수 있습니다.
   * 색상 웰 팝업을 사용하여 색상을 선택합니다.
   * 색상은 hex 또는 RGB 값으로 정의할 수도 있습니다.
* **배경색**  - 응용 프로그램이 로드될 때  [표시되는 응용 프로그램의 ](https://developer.mozilla.org/en-US/docs/Web/Manifest/background_color) 배경색을 정의합니다.
   * 색상 웰 팝업을 사용하여 색상을 선택합니다.
   * 색상은 hex 또는 RGB 값으로 정의할 수도 있습니다.
   * 특정 브라우저 [는 앱 이름, 배경색 및 아이콘에서 자동으로 시작 화면 ](https://developer.mozilla.org/en-US/docs/Web/Manifest#Splash_screens)을 만듭니다.
* **아이콘**  - 사용자 장치 [의 응용 프로그램을 ](https://developer.mozilla.org/en-US/docs/Web/Manifest/icons) 나타내는 아이콘을 정의합니다.
   * 아이콘은 512x512픽셀 크기의 png 파일이어야 합니다.
...
* **캐시 전략 및 내용 새로 고침**  빈도 - 이 설정은 PWA에 대한 캐싱 모델을 정의합니다.
   * **중간**  -  [이 ](https://web.dev/stale-while-revalidate/) 설정은 대부분의 사이트에서 사용되는 설정이며 기본값입니다.
      * 이 설정을 사용하면 사용자가 처음 본 내용은 캐시에서 로드되고 사용자가 해당 내용을 사용하는 동안 캐시에 있는 나머지 컨텐트의 유효성을 다시 검사합니다.
   * **자주**  - 경매 사이트와 같이 업데이트가 매우 빨리 필요한 사이트의 경우 이 경우가 해당합니다.
      * 이 설정을 사용하면 응용 프로그램이 네트워크를 통해 가장 최근의 내용을 먼저 찾고, 사용할 수 없는 경우 로컬 캐시에 저장됩니다.
   * **거의**  없음 - 참조 페이지와 같이 거의 정적인 사이트의 경우
      * 이 설정을 사용하면 응용 프로그램이 먼저 캐시에서 내용을 찾고, 사용할 수 없는 경우 다시 네트워크로 이동하여 검색합니다.
* **파일 사전 캐시**  - AEM에서 호스팅되는 이러한 파일은 서비스 워커가 설치되고 사용되기 전에 로컬 브라우저 캐시에 저장됩니다. 이렇게 하면 오프라인에서 웹 앱이 완전히 정상적으로 작동합니다.
* **경로 포함**  - 정의된 경로에 대한 네트워크 요청이 가로화되고 캐시된 내용은 구성된  **캐싱 전략 및 내용 새로 고침 빈도에 따라 반환됩니다**.
* **캐시 제외**  - 이러한 파일은  **파일 사전** 캐시 및  **경로 포함**&#x200B;의 설정에 관계없이 캐시되지 않습니다.

>[!TIP]
>
>개발자 팀에서는 오프라인 구성 설정 방법에 대한 유용한 정보를 얻을 수 있습니다.

## 제한 사항 {#limitations}

AEM Sites에 대해 일부 PWA 기능을 사용할 수 있는 것은 아닙니다. 이것은 몇 가지 주목할 만한 한계이다.

* 사용자는 페이지가 오프라인으로 캐시되기 전에 적어도 한 번 이상 페이지를 검색해야 합니다.
