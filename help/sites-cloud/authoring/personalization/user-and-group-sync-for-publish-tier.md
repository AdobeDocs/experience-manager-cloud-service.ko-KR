...
고객은 기본 SAML IdP를 통해 SAML 기반 인증을 사용할 수 있습니다. AEM에서 IdP를 사용할 때, IdP는 SAML 어설션에 설명된 대로 사용자의 자격 증명을 인증하고, AEM으로 사용자 인증을 브로커링하고, 필요에 따라 AEM에서 사용자 레코드를 생성하고, AEM에서 사용자 그룹 멤버십을 관리합니다.
...
### 사용자 지정 코드 및 마이그레이션 요구 사항 {#custom-code-and-migration-requirements}

다음 요구 사항은 사용자 지정 코드를 사용하여 로컬 사용자 또는 로컬 그룹을 만드는 경우에만 적용됩니다. 데이터 동기화를 활성화한 경우 동적 그룹 구성원이 포함된 외부 사용자 및 그룹을 만들려면 이러한 사용자 지정 코드를 업데이트해야 합니다.

**필수 단계:**

* **사용자 지정 코드에 대한 수정**: 사용자 또는 그룹을 만드는 사용자 지정 논리를 업데이트해야 합니다.

   * `rep:externalId` 속성을 설정하여 외부 사용자 만들기
   * `rep:externalId` 속성을 설정하여 외부 그룹 만들기
   * 직접적인 사용자-그룹 관계를 사용하지 않고 `rep:externalPrincipalNames` 속성을 사용하여 동적 그룹 멤버십을 구현합니다.

* **기존 데이터의 마이그레이션**: 프로덕션 환경에서 데이터 동기화를 사용하려면 먼저 기존 로컬 사용자 및 그룹을 모두 외부 ID 모델로 마이그레이션해야 합니다.

사용자 지정 구현 업데이트 및 기존 사용자 및 그룹 마이그레이션에 대한 자세한 기술 지침은 [외부 ID 및 동적 그룹 멤버십으로 마이그레이션](/help/security/migrating-to-external-identity.md)을 참조하세요.
