...
# ContextHub{#configuring-segmentation-with-contexthub}을(를) 사용하여 세그멘테이션 구성
...
그런 다음 이러한 세그먼트를 사용하여 방문자에게 특정 타깃팅된 컨텐츠를 제공합니다. [여기에 ](activities.md) 정의된 활동은 모든 페이지에 포함될 수 있으며 특정 컨텐트에 적용할 수 있는 방문자 세그먼트를 정의할 수 있습니다.

AEM을 사용하면 사용자의 경험을 손쉽게 개인화할 수 있습니다. 세그먼트 정의 결과를 확인할 수도 있습니다.

## 세그먼트 액세스 {#accessing-segments}

[Audiences](audiences.md) 콘솔은 Adobe Target 계정의 대상과 ContextHub에 대한 세그먼트를 관리하는 데 사용됩니다. 이 설명서에서는 ContextHub용 세그먼트 관리에 대해 설명합니다.
...
**세그먼트 편집기**&#x200B;에서는 세그먼트를 쉽게 수정할 수 있습니다. 세그먼트를 편집하려면 세그먼트 목록에서 세그먼트를 선택하고 **편집** 단추를 클릭합니다.

![세그먼트 편집기](../assets/contexthub-segment-editor.png)

구성 요소 브라우저를 사용하여 **AND** 및 **OR** 컨테이너를 추가하여 세그먼트 로직을 정의한 다음 추가 구성 요소를 추가하여 속성 및 값 또는 참조 스크립트 및 기타 세그먼트를 비교하여 선택 기준을 정의할 수 있습니다([새 세그먼트 만들기](#creating-a-new-segment) 참조). 세그먼트를 선택할 때 정확한 시나리오를 정의할 수 있습니다.

전체 문이 true로 평가되면 세그먼트가 해결됩니다. 여러 세그먼트를 적용할 수 있는 경우 **증폭** 계수도 사용됩니다. 증폭 요인에 대한 자세한 내용은 [새 세그먼트 만들기](#creating-a-new-segment)를 참조하십시오.
...
바로 사용할 수 있는 다음 컨테이너에서는 부울 평가를 위해 비교와 참조를 그룹화할 수 있습니다. 구성 요소 브라우저에서 편집기로 끌 수 있습니다. 자세한 내용은 다음 섹션 [AND 및 OR 컨테이너](#using-and-and-or-containers)를 참조하십시오.
...
>
>속성 `profile/age`을 비교할 때 이미 비교 유형이 **number**&#x200B;이(가) 될 것임을 알고 있으므로, `profile/age`이(가) 설정되어 있지 않더라도 30보다 작으면 **false**&#x200B;가 반환됩니다.`profile/age`
...
1. 세그먼트](#accessing-segments)에 액세스한 후 [세그먼트를 만들 폴더](#organizing-segments)로 이동하거나 루트에 그대로 둡니다.[

1. **만들기** 단추를 누르거나 클릭하고 **ContextHub 세그먼트 만들기**&#x200B;를 선택합니다.

   ![세그먼트 추가](../assets/contexthub-create-segment.png)

1. **새 ContextHub 세그먼트**&#x200B;에서 필요한 경우 세그먼트의 제목과 증폭 값을 입력한 다음 **만들기**&#x200B;를 탭하거나 클릭합니다.
...
1. 필요에 따라 구성 요소를 더 추가합니다. AND 및 OR 비교용 컨테이너 구성 요소를 사용하여 부울 표현식을 만들 수 있습니다(아래 [AND 및 Or 컨테이너 사용](#using-and-and-or-containers) 참조). 세그먼트 편집기를 사용하면 더 이상 필요하지 않은 구성 요소를 삭제하거나 문 내의 새 위치로 드래그할 수 있습니다.
...
스크립트가 추가 속성에 의존하는 경우 스크립트는 `this.dependOn()`을(를) 호출해야 합니다. 예를 들어 스크립트가 `profile/age`에 의존하는 경우:
...
1. **스크립트 참조** 구성 요소의 편집 대화 상자를 엽니다. [이(가) 올바르게 구성된 경우 **스크립트 이름** 드롭다운에서 스크립트를 사용할 수 있어야 합니다.](#defining-a-script-to-reference)
...
1. [이 세그먼트](#accessing-segments)에 액세스한 후 **만들기** 단추를 클릭하거나 탭하고 **폴더**&#x200B;를 선택합니다.
...
1. 도구 모음에서 **이름 변경**&#x200B;을 탭하거나 클릭하여 폴더 이름을 변경합니다.
...
이러한 테스트는 컨텐츠 페이지에 대해 수행할 수도 있으며 타깃팅된 컨텐츠 및 관련 **활동** 및 **경험**&#x200B;과 함께 수행할 수도 있습니다.
...
세그먼트는 특정 대상 대상이 보는 실제 컨텐츠를 제어하는 데 사용됩니다. 대상 및 세그먼트에 대한 자세한 내용은 [대상 관리](audiences.md)와 대상 및 세그먼트를 사용하여 컨텐츠를 타깃팅하는 방법에 대한 [타깃팅된 컨텐츠 작성](targeted-content.md)을 참조하십시오.
