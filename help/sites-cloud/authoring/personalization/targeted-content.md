...
![대상 드롭 영역의 아이콘](/help/sites-cloud/authoring/assets/targeted-drop-target-icon.png)
...
1. 라이브러리에서 오퍼를 선택한 다음, 확인 표시 아이콘을 클릭하거나 탭합니다.
   ...
   * **가장 최근에서 가장 오래된 순으로** 옆에 있는 화살표를 클릭하거나 탭하여 오퍼를 정렬하는 방법을 변경합니다. 오퍼를 가장 최근 오퍼에서 가장 오래된 오퍼순으로 또는 가장 오래된 오퍼에서 가장 최근 오퍼순으로 정렬할 수 있습니다.
   ...
   ![다른 이름으로 보기 단추](/help/sites-cloud/authoring/assets/targeted-view-as-button.png)
...
![AEM을 대상 엔진으로](/help/sites-cloud/authoring/assets/targeted-goals.png)
...
| 지표 | 설명 | 옵션 |
|---|---|---|
| 변환 | 테스트되는 경험의 일부를 클릭한 방문자 비율. 전환은 방문자당 한 번씩 또는 방문자가 전환을 완료할 때마다 계산됩니다. 전환 지표는 다음 중 하나로 설정됩니다 | 페이지 보기 - URL을 선택한 다음 URL 또는 여러 URL을 정의하거나 URL 포함 사항을 선택한 다음 경로나 키워드를 추가하여 대상이 본 페이지를 정의할 수 있습니다. mbox 확인됨 - mbox의 이름을 입력하여 대상이 본 mbox를 정의할 수 있습니다. mbox 추가를 클릭하여 여러 mbox를 입력할 수 있습니다. |
| 수입 | 방문으로 생성된 매출입니다. 나열된 매출 지표 중에서 선택할 수 있습니다. 이러한 옵션의 경우 mbox를 보았는지 여부는 목표에 도달했음을 나타냅니다. mbox 또는 여러 mbox를 정의할 수 있습니다. | 방문자당 매출(RPV), 평균 주문 가격(AOV), 총 판매, 주문 수 |
...
| 옵션 | 설명 |
|---|---|
| 위치 | 위치는 타깃팅된 컨텐츠 위치에 이름을 지정하고 오퍼를 배치할 페이지의 위치(또는 위치 또는 구성 요소)와 오퍼를 연결하는 문자열입니다. 이 필드는 일반 값입니다. 오퍼를 구성 요소에 넣으면 오퍼는 위치 ID를 기억합니다. 페이지가 실행되면 엔진은 사용자의 세그먼트를 평가하고 이를 기반으로 하여 표시해야 하는 활성 캠페인의 경험을 해결합니다. 그런 다음, 페이지에서 위치 ID를 확인하고 해당 위치 ID를 사용하는 오퍼를 위치 ID에 대응시킵니다. |
