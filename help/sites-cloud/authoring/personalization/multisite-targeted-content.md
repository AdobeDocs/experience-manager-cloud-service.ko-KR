...
사용 사례에 따라 다양한 방법으로 타깃팅된 컨텐츠에 대한 다중 사이트 지원을 설정할 수 있습니다. 이 섹션에서는 이 지원이 어떻게 하나의 브랜드에 이론적으로 실행되는지를 설명합니다. 또한 [예:사이트](#example-targeting-content-based-on-geography)를 기반으로 컨텐츠를 타깃팅하면 여러 사이트에서 컨텐츠를 타깃팅하는 실제 애플리케이션을 볼 수 있습니다.
...
>[!NOTE]
>
>이 기능에 대한 자세한 내용은 [타깃팅된 컨텐츠에 대한 다중 사이트 관리 구성 방식](/help/sites-cloud/authoring/personalization/multisite-structure.md)을 참조하십시오.
