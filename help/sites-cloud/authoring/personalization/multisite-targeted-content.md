...
1. [새 영역 만들기](#creating-new-areas) 또는 [영역을 live copy로 만들기](#creating-new-areas). 영역에는 구성 요소가 타겟팅되는 위치인 페이지의 *영역*&#x200B;에 사용할 수 있는 모든 활동이 포함됩니다. 새 영역을 만들면 빈 영역이 만들어지지만 영역을 Live Copy로 만들면 사이트 구조에서 콘텐츠를 상속할 수 있습니다.
...
>[!NOTE]
>
>브랜드 이름을 선택하면 기본 영역이라는 기본 영역이 기본적으로 축소됩니다 **종료 시간** 다른 영역을 만듭니다. Then, when you select a brand in either the **Activity** or **Offers** console, you see the **Area** console.
...
1. 에서 영역 선택 **영역 참조** 드롭다운 메뉴 및 선택 **저장**.
...
1. 상속을 분리하거나 일시 중단하려는 페이지로 이동하고 을 선택합니다 **타겟팅** ( 모드 드롭다운 메뉴).
...
1. 선택 **일시 중단** 활동을 일시 중단합니다. 일시 중단된 활동은 빨간색으로 표시됩니다.
...
1. 마스터에서 Live Copy를 분리하려는 페이지로 이동하고 을 선택합니다 **타겟팅** ( 모드 드롭다운 메뉴).
1. 페이지가 Live Copy인 영역에 연결되어 있으면 상속 상태가 표시됩니다. **타겟팅 시작**&#x200B;을 선택합니다.
1. Select **Detach Live Copy** from the drop-down menu in the toolbar. AEM confirms that you want to detach the Live Copy.
1. 선택 **분리** 활동에서 라이브 카피를 분리합니다. 분리된 후에는 상속에 대한 드롭다운 메뉴가 더 이상 표시되지 않습니다. 이제 이 활동은 로컬 활동입니다.
...
1. 상속을 복원하려는 페이지로 이동하여 을 선택합니다 **타겟팅** ( 모드 드롭다운 메뉴).
1. **타겟팅 시작**&#x200B;을 선택합니다.
1. 도구 모음의 드롭다운 메뉴에서 **Live Copy 다시 시작**&#x200B;을 선택합니다.

   ![Live Copy 다시 시작](/help/sites-cloud/authoring/assets/multisite-resume.png)

1. 선택 **다시 시작** 라이브 카피 상속을 다시 시작할지 확인하는 데 사용됩니다. 상속을 다시 시작하는 경우 현재 활동에 대한 수정 사항은 모두 잃게 됩니다.
