...
1. [새 영역을 만들거나](#creating-new-areas) [영역을 Live Copy로 만들기](#creating-new-areas). 영역에는 구성 요소가 타겟팅되는 위치인 페이지의 *영역*&#x200B;에 사용할 수 있는 모든 활동이 포함됩니다. 새 영역을 만들면 빈 영역이 만들어지지만 영역을 Live Copy로 만들면 사이트 구조에서 콘텐츠를 상속할 수 있습니다.
...
사용 사례에 따라 여러 가지 방법으로 타겟팅된 콘텐츠에 대한 다중 사이트 지원을 설정할 수 있습니다. 이 섹션에서는 이 지원이 어떻게 하나의 브랜드에 이론적으로 실행되는지를 설명합니다. 또한 [예: 지역에 따른 콘텐츠 타겟팅](#example-targeting-content-based-on-geography)에서, 여러 사이트에서 콘텐츠를 타겟팅하는 실제 애플리케이션을 확인할 수 있습니다.
...
>[!NOTE]
>
>다른 영역을 만들 때까지 **브랜드 이름을 선택하면 기본적으로 기본 영역이라는 기본 영역이 축소됩니다**. Then, when you select a brand in either the **Activity** or **Offers** console, you see the **Area** console.
...
1. 사이트 또는 페이지를 선택하고 **속성 보기**&#x200B;를 선택합니다.
1. **Personalization** 탭을 선택합니다.
...
1. **영역 참조** 드롭다운 메뉴에서 영역을 선택하고 **저장**&#x200B;을 선택합니다.
...
1. 상속을 분리하거나 일시 중단하려는 페이지로 이동하고 모드 드롭다운 메뉴에서 **타깃팅**&#x200B;을 선택합니다.
1. 페이지가 Live Copy인 영역에 연결되어 있으면 상속 상태가 표시됩니다. **타겟팅 시작**&#x200B;을 선택합니다.
...
1. 활동을 일시 중단하려면 **일시 중단**&#x200B;을(를) 선택하십시오. 일시 중단된 활동은 빨간색으로 표시됩니다.
...
1. 마스터에서 Live Copy를 분리하려는 페이지로 이동하고 모드 드롭다운 메뉴에서 **타깃팅**&#x200B;을(를) 선택합니다.
1. 페이지가 Live Copy인 영역에 연결되어 있으면 상속 상태가 표시됩니다. **타겟팅 시작**&#x200B;을 선택합니다.
1. Select **Detach Live Copy** from the drop-down menu in the toolbar. AEM confirms that you want to detach the Live Copy.
1. 활동에서 Live Copy를 분리하려면 **분리**&#x200B;를 선택하십시오. 분리된 후에는 상속에 대한 드롭다운 메뉴가 더 이상 표시되지 않습니다. 이제 이 활동은 로컬 활동입니다.
...
1. 상속을 복원하려는 페이지로 이동하고 모드 드롭다운 메뉴에서 **타깃팅**&#x200B;을 선택합니다.
1. **타겟팅 시작**&#x200B;을 선택합니다.
1. 도구 모음의 드롭다운 메뉴에서 **Live Copy 다시 시작**&#x200B;을 선택합니다.

   ![Live Copy 다시 시작](/help/sites-cloud/authoring/assets/multisite-resume.png)

1. **다시 시작**&#x200B;을 선택하여 Live Copy 상속을 다시 시작할지 확인합니다. 상속을 다시 시작하는 경우 현재 활동에 대한 수정 사항은 모두 잃게 됩니다.
...
1. **개인화** > **활동** 또는 **오퍼**&#x200B;로 이동한 다음 내 브랜드로 이동합니다.
1. 삭제하려는 영역 옆에 있는 아이콘을 선택합니다.
1. **삭제**&#x200B;를 선택하고 영역을 삭제할지 확인하십시오.
