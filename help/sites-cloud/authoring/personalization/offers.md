...
* 다음 권한이 있는 고객만 사용 가능: *legacy* 오퍼가 이미 정의되었습니다(즉, 기존).
...
1. 다음 항목 선택 **오퍼 페이지** 템플릿을 선택한 다음 선택 **다음**.
1. 오퍼의 제목을 입력하고 선택적으로 오퍼와 연결할 태그를 하나 이상 선택하거나 입력한 다음 를 선택합니다 **만들기**.
1. 확인 대화 상자에서 편집할 오퍼를 열려면 을 선택합니다. **페이지 열기**.
...
1. 오퍼 콘솔에서 오퍼 옆에 있는 아이콘을 선택하고 을 선택합니다 **편집**.
...
1. 오퍼 콘솔에서 오퍼 옆에 있는 아이콘을 선택하고 을 선택합니다 **삭제**.
...
1. 오퍼가 하나 이상의 경험에서 사용되는 경우 오퍼가 참조되고 있음을 나타내는 대화 상자가 표시됩니다.
   ...
   * 오퍼를 삭제하고 경험에서 제거하려면 을 선택합니다. **강제 삭제**.
   * 오퍼를 유지하려면 다음을 선택합니다. **취소**.
