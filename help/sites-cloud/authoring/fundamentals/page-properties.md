...
페이지에 필요한 속성을 정의할 수 있습니다. 이러한 속성은 페이지의 특성에 따라 다를 수 있습니다. 예를 들어 일부 페이지는 라이브 카피에 연결되어 있을 수 있지만 다른 페이지는 연결되지 않고 라이브 카피 정보를 적절하게 사용할 수 있습니다.
...
이 탭은 블루프린트 역할을 하는 페이지에만 표시됩니다. 블루프린트는 라이브 카피의 기반이 되며 [다중 사이트 관리](/help/sites-cloud/administering/msm/overview.md).

* **현재 라이브 카피** - 이 블루프린트 페이지를 기반으로 하는(즉, 의 라이브 카피인) 페이지를 나열합니다.

* **롤아웃 구성** - 수정 사항이 Live Copy에 반영되는 상황을 제어합니다.

### Live Copy {#live-copy}

이 탭은 Live Copy로 구성된 페이지에만 표시됩니다. 블루프린트와 마찬가지로 라이브 카피는 [다중 사이트 관리](/help/sites-cloud/administering/msm/overview.md).
