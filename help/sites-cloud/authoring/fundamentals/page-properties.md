...
* **제목 및 태그**
   ...
   * **제목**  - 페이지의 제목이 다양한 위치에 표시됩니다. 예를 들어 웹 사이트  **** 탭 목록 및 사이트  **** 카드/목록 보기가 있습니다.
      * 필수 필드입니다.
   ...
   * **탐색에서 숨기기**  - 결과 사이트의 페이지 탐색에 페이지가 표시되거나 숨겨지는지 여부를 나타냅니다.
...
* **기타 제목 및 설명**
   ...
   * **자막**  - 페이지에서 사용할 자막입니다.
   * **설명**  - 페이지 설명, 목적 또는 추가하려는 기타 모든 세부 사항입니다.
...
* **설정/해제 시간**
   ...
   >[!NOTE]
   >**On Time** 또는 **Off Time**&#x200B;이 과거 에 있고 자동 복제가 구성된 경우 관련 작업이 즉시 트리거됩니다.

   * **설정 시간**  - 게시된 페이지를 게시 환경에 표시(렌더링)할 날짜와 시간입니다. 페이지는 수동으로 또는 사전 구성된 자동 복제에서 게시해야 합니다.

      * 이미 [게시된 경우(수동으로)](/help/sites-cloud/authoring/fundamentals/publishing-pages.md)이 페이지는 지정된 시간에 렌더링될 때까지 비활성 상태로 유지됩니다.
   ...
   * **해제 시간**  - 과 유사하며  **설정 시간**&#x200B;과 함께 사용되는 경우가 많으며, 게시 환경에 게시된 페이지가 숨겨지는 시간을 정의합니다.

   * 즉시 게시하고 비활성화될 때까지 게시 환경(일반적인 시나리오)에서 사용할 수 있는 페이지에 대해서는 이러한 필드(**On Time** 및 **Off Time**)를 비워 둡니다.
...
* **별칭 URL**
   ...
   * **추가**  - 페이지의 별칭 URL을 정의하는 필드를 표시하려면 탭하거나 클릭합니다.
      ...
      * **제거** 아이콘을 탭하거나 클릭하여 별칭 URL을 삭제합니다.
...
* **템플릿 설정**
   ...
   * **허용된 템플릿**  -  [이 하위 분기 내에서 사용할 ](/help/sites-cloud/authoring/features/templates.md#enabling-and-allowing-a-template-template-author) 수 있는 템플릿 목록을 정의합니다
...
* **미리 보기 생성**  - 미리 보기로 사용할 페이지의 미리 보기를 생성합니다
...
* **타깃팅 구성**
   ...
   * **브랜드**  - 타깃팅할  [범위를 지정할 브랜드를 정의합니다](/help/sites-cloud/authoring/personalization/targeted-content.md).
   >[!NOTE]
...
이 탭은 블루프린트로 사용되는 페이지에만 표시됩니다. Live Copy의 기반이 되는 블루프린트는 [다중 사이트 관리의 일부입니다.](/help/sites-cloud/administering/msm/overview.md)
...
* **재설정**  - Live Copy를 블루프린트 상태로 재설정, 로컬 수정 사항 제거
