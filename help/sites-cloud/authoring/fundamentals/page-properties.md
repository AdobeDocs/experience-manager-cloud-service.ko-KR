...
* **제목 및 태그**
   ...
   * **제목** - 페이지의 제목이 다양한 위치에 표시됩니다. 예: **웹 사이트** 탭 목록 및 **Sites** 카드/목록 보기.
      * 필수 필드입니다.
   ...
   * **탐색 시 숨기기** - 결과 사이트의 페이지 탐색에 페이지가 표시되거나 숨겨지는지 여부를 나타냅니다.

* **브랜딩**

   각 페이지 제목에 브랜드 슬러그를 추가하여 페이지 간에 일관된 브랜드 이미지를 적용합니다. 이 기능을 사용하려면 릴리스 2.14.0 이상에서 페이지 구성 요소를 사용해야 합니다 [핵심 구성 요소.](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/introduction.html?lang=ko)

   * **무시** - 이 페이지에서 브랜드 슬러그를 정의하려면 을 선택합니다.
      * 이 값은 하위 페이지에도 상속되지 않는 한 상속됩니다 **무시** 값을 설정합니다.
...
* **기타 제목 및 설명**
   ...
   * **탐색 제목** - 탐색에 사용할 별도의 제목을 지정할 수 있습니다(예를 들어 더욱 간결한 제목을 원할 경우). 비어 있으면 **제목** 이 사용됩니다.
   * **자막** - 페이지에서 사용할 소제목입니다.
   * **설명** - 페이지 설명, 목적 또는 추가하고 싶은 기타 모든 세부 사항입니다.

* **설정/해제 시간**

   >[!NOTE]
   >
   > 자세한 내용은 [설정 및 해제 시간 - 트리거 구성](/help/operations/replication.md#on-and-off-times-trigger-configuration) 관련 자동 복제를 구성하는 방법에 대한 자세한 내용은

   >[!NOTE]
   >다음 중 하나가 **시간** 또는 **해제 시간** 가 과거 이고 자동 복제가 구성된 경우 관련 작업이 즉시 트리거됩니다.

   * **시간** - 게시된 페이지를 게시 환경에 표시(렌더링됨)할 날짜와 시간입니다. 페이지는 수동으로 또는 사전 구성된 자동 복제에서 게시해야 합니다.

      * 이미 있는 경우 [게시됨(수동으로)](/help/sites-cloud/authoring/fundamentals/publishing-pages.md) 이 페이지는 지정된 시간에 렌더링될 때까지 비활성 상태로 유지됩니다(숨김).
      * 게시되지 않고 자동 복제를 위해 구성된 경우, 페이지가 자동으로 게시되고 지정된 시간에 렌더링됩니다.
      * 게시되지 않았거나 자동 복제에 대해 구성되지 않은 경우 페이지가 자동으로 게시되지 않으므로 페이지에 액세스하려고 하면 404가 표시됩니다.
   * **해제 시간** -비슷하고 주로 와 조합하여 사용 **시간**, 게시 환경에서 게시된 페이지를 숨길 시간을 정의합니다.

   * 다음 필드를 그대로 둡니다(**시간** 및 **해제 시간**)를 비워 두면 즉시 게시하며, 비활성화될 때까지 게시 환경에서 사용할 수 있습니다(일반 시나리오).
...
* **별칭 URL**
   ...
   * **추가** - 페이지의 별칭 URL을 정의하는 필드를 표시하려면 탭하거나 클릭합니다.
      * 여러 항목을 추가하려면 다시 탭하거나 클릭합니다.
      * 을(를) 탭하거나 클릭합니다 **제거** 아이콘을 클릭하여 별칭 URL을 삭제합니다.
   * **별칭 URL 리디렉션** - 페이지에서 별칭 URL을 사용할지를 지정합니다.
...
* **설정**
   ...
   * **리디렉션** - 이 페이지가 HTML으로 자동으로 리디렉션되는 페이지를 나타냅니다. `302 Found` 상태.
   ...
   * **별칭** - 이 페이지에 사용할 별칭을 지정합니다
      ...
      * 별칭을 만들면 `sling:alias` 저장소 경로가 아니라 리소스에만 영향을 주는 페이지 노드의 속성입니다.
      * 편집기에서 별칭으로 액세스한 페이지를 게시할 수 없습니다. [게시 옵션](/help/sites-cloud/authoring/fundamentals/publishing-pages.md) 편집기에서 는 실제 경로를 통해 액세스되는 페이지에만 사용할 수 있습니다.
...
* **템플릿 설정**
   ...
   * **허용된 템플릿** - [사용할 수 있는 템플릿 목록을 정의합니다](/help/sites-cloud/authoring/features/templates.md#enabling-and-allowing-a-template-template-author) 이 하위 분기 내에서

* **인증 요구 사항**

   * **활성화** - 페이지에 액세스할 수 있도록 인증 사용
...
* **내보내기**
   ...
   * **구성 내보내기** - 내보내기 구성을 지정합니다

### 썸네일 {#thumbnail}

페이지 축소판 구성

* **미리 보기 생성** - 축소판으로 사용할 페이지의 미리 보기 생성
* **이미지 업로드** - 축소판으로 사용할 이미지를 업로드합니다.
* **이미지 선택** - 축소판으로 사용할 기존 자산을 선택합니다
* **되돌리기** - 축소판을 변경한 후 이 옵션을 사용할 수 있습니다. 변경 사항을 유지하지 않으려면 저장하기 전에 해당 변경 사항을 되돌릴 수 있습니다.
...
* **ContextHub 구성**
   ...
   * **ContextHub 경로** - 정의 [ContextHub 구성](/help/sites-cloud/authoring/personalization/contexthub.md)
   * **세그먼트 경로** - 정의 [세그먼트 경로](/help/sites-cloud/authoring/personalization/contexthub-segmentation.md)

* **타깃팅 구성**

   * **브랜드** - 을 정의합니다. [타깃팅할 범위를 지정하는 브랜드](/help/sites-cloud/authoring/personalization/targeted-content.md).
   >[!NOTE]
...
이 탭은 블루프린트로 사용되는 페이지에만 표시됩니다. Live Copy의 기반이 되는 블루프린트가 [다중 사이트 관리.](/help/sites-cloud/administering/msm/overview.md)

* **현재 Live Copy** - 이 블루프린트 페이지를 기반으로 하는(즉, Live Copy) 페이지를 나열합니다

* **롤아웃 구성** - 수정 사항이 Live Copy에 전파되는 환경을 제어합니다

### Live Copy {#live-copy}

* **동기화** - Live Copy를 블루프린트와 동기화, 로컬 수정 유지
* **재설정** - Live Copy를 블루프린트 상태로 재설정, 로컬 수정 제거
* **일시 중단** - 추가 롤아웃 수정 시 Live Copy 일시 중단
* **분리** - 블루프린트에서 Live Copy 분리

* **소스**

   * 이 Live Copy에 대한 블루프린트의 경로를 표시합니다
...
* **구성**
   ...
   * **롤아웃 구성 선택** - 수정 사항이 블루프린트에서 전파되고 다음 경우에만 사용할 수 있는 상황을 정의합니다 **상위 항목에서 롤아웃 구성 상속** 이(가) 선택되어 있지 않습니다.
