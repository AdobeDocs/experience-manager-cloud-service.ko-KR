...
1. 주석 대화 상자를 열고 상세 정보를 확인하려면 주석을 클릭하거나 탭합니다.
...
1. 필요한 구성 요소(파란색 테두리로 강조 표시되고 주석을 추가할 수 있는 구성 요소)를 클릭하거나 탭하여 주석을 추가하고 대화 상자를 엽니다.
   ...
   * 주석 텍스트 입력
   * 스케치(선과 모양)를 만들어 구성 요소의 영역을 강조 표시합니다.
...
1. 특정 주석의 편집이 끝나면 다음 작업을 할 수 있습니다.
   ...
   * 텍스트 마커를 클릭하거나 탭하여 주석을 엽니다. 열면 전체 텍스트 보기, 변경 작업 또는 [주석 삭제](#deleting-annotations)를 할 수 있습니다.
   * 텍스트 마커 위치 변경.
   * 스케치 선을 클릭하거나 탭하여 스케치를 선택하고 원하는 위치로 드래그합니다.
   * 구성 요소 이동 또는 복사
      * 모든 관련 주석과 해당 스케치가 이동 또는 복사되지만 단락을 기준으로 해당 위치가 그대로 유지됩니다.
...
>[!NOTE]
>
>구성 요소를 삭제하면 해당 리소스에 첨부된 모든 주석과 스케치가 페이지에서의 위치와 관계 없이 모두 삭제됩니다.

## 스케치 삭제 전용 {#deleting-sketches}

모든 관련 스케치가 포함된 전체 주석 대신 특정 스케치만 삭제할 수 있습니다.
