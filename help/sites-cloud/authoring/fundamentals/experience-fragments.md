...
경험 조각의 개인화를 사용하면 마케터가 경험 조각의 타겟 대상을 한 번만 정의한 다음, 모든 페이지에서 조각을 다시 사용할 수 있습니다. 이 기능은

* 조각을 사용할 때마다 각 대상에 필요한 변형을 지정할 필요가 없습니다
* 오퍼에서 스타일을 유지 관리합니다

이 단일 조각 내에 그룹화된 여러 구성 요소로 경험 조각을 만들 수 있습니다. 각 특정 대상 세그먼트에 대해 조각의 변형을 만든 다음 필요한 채널에서 이러한 경험 조각을 재사용할 수도 있습니다.

개인화는 **개인화** 경험 조각 또는 변형 또는 조각을 포함하는 폴더의 속성 이것은 상속이 개인화 속성을 무시할 수 있음을 의미합니다.

이러한 속성을 구성하면 **타깃팅** 경험 조각 편집기에서 모드입니다.

### 경험 조각에 대한 개인화 정의 {#defining-personalization-experience-fragment}

조각을 개인화하려면:

1. 에서 필요한 위치로 이동합니다 **경험 조각** 콘솔.

1. 폴더 또는 조각을 선택한 다음, **속성** 를 클릭합니다.

   >[!NOTE]
   >
   >폴더에 정의된 개인화 속성은 하위 트리를 통해 모든 하위 폴더와 해당 하위 트리 내의 경험 조각(및 변형)에 의해 상속됩니다. 상속을 중단하여 대체할 수 있습니다.

1. 를 엽니다. **개인화** 탭을 클릭하여 설정을 정의하고 저장합니다. 예를 들어, 폴더:

   ![경험 조각 - 개인화 속성](/help/sites-cloud/authoring/assets/xf-personalization-properties.png)

   >[!CAUTION]
   >
   >조각을 사이트 페이지에 포함시키면 **개인화** 을 구성했으면 페이지의 개인화 버전만 페이지 렌더링 시 사용됩니다.
   >
   >조각의 구성 요소에서 수행된 타깃팅이 페이지 렌더링 시 작동하려면 다음 조건을 충족해야 합니다.
   >
   >다음 **ContextHub 경로** 에서 선택됨 **개인화** 탭은 다음 중 하나여야 합니다.
   >
   >* 조각을 렌더링할 페이지에 대해 구성된 경로와 동일합니다
      >또는:
   >* 페이지에 대해 구성된 ContextHub에 정의된 저장소의 하위 집합을 포함하는 경로입니다

   >
   > 
다음 **세그먼트 경로** 에서 선택됨 **개인화** 탭은 다음 중 하나여야 합니다.
   * 조각을 렌더링할 페이지에 대해 구성된 경로와 동일한 경로입니다.
   * 페이지에 대해 구성된 세그먼트의 하위 집합을 포함하는 경로


### 경험 조각에 대한 타깃팅 정의 {#defining-targeting-experience-fragment}

개인화 속성이 구성되면 편집을 위해 조각을 열면 타깃팅 모드를 사용할 수 있습니다.

![경험 조각 편집기 - 타깃팅 모드](/help/sites-cloud/authoring/assets/xf-targeting-mode.png)

이 모드는 페이지 편집과 동일한 방식으로 작동합니다. 자세한 내용은 [페이지 편집기의 타깃팅 모드](/help/sites-cloud/authoring/personalization/targeted-content.md) 자세한 내용
