---
title: 컨텐츠 조각을 Adobe Target으로 내보내기
description: 컨텐츠 조각을 Adobe Target으로 내보내기
...
# 컨텐츠 조각을 Adobe Target으로 내보내기 {#exporting-content-fragments-to-adobe-target}

>[!CAUTION]
>
>* AEM 컨텐츠 조각을 Adobe Target의 기본 작업 공간으로 내보냅니다.
>* [Adobe Target과 통합](/help/sites-cloud/integrating/integrating-adobe-target.md)의 지침에 따라 Adobe Target과 AEM을 통합해야 합니다.


내보낼 수 있습니다 [컨텐츠 조각](/help/sites-cloud/authoring/fundamentals/content-fragments.md): Adobe Experience Manager as a Cloud Service(AEM)에서 만든 Adobe Target(Target)으로 이동합니다. 그런 다음 Target 활동의 오퍼로 사용하여 경험을 대규모로 테스트하고 개인화할 수 있습니다.

Adobe Target으로 컨텐츠 조각을 내보내는 데 사용할 수 있는 옵션이 있습니다.

* JSON: 헤드리스 콘텐츠 전송 지원

<!-- * GraphQL query ??? -->

Adobe Target으로 AEM 컨텐츠 조각을 내보낼 인스턴스를 준비하려면 다음을 수행해야 합니다.
...
* [Adobe Target으로 컨텐츠 조각 내보내기](#exporting-a-content-fragment-to-adobe-target)
* [Adobe Target에서 컨텐츠 조각 사용](#using-your-content-fragments-in-adobe-target)
* 그리고 [Adobe Target으로 이미 내보낸 컨텐츠 조각을 삭제합니다](#deleting-a-content-fragment-already-exported-to-adobe-target)

컨텐츠 조각을 Adobe Target의 기본 작업 공간으로 내보내거나 Adobe Target의 사용자 정의 작업 공간으로 내보낼 수 있습니다.
...
* 컨텐츠 조각에서 참조를 재작성하기 위한 외부 도우미 도메인을 선택합니다(선택 사항).
...
1. 적절한 폴더 또는 조각에 대한 **페이지 속성**&#x200B;을 엽니다.
   ...
   >[!NOTE]
   >
   >컨텐츠 조각 상위 폴더에 클라우드 구성을 추가하는 경우 구성이 모든 하위 폴더에 의해 상속됩니다.
   >
   >클라우드 구성을 컨텐츠 조각 자체에 추가하는 경우, 구성은 모든 변형에서 상속됩니다.
...
1. **클라우드 서비스 구성** 아래의 드롭다운 목록에서 **Adobe Target**&#x200B;을 선택합니다.
   ...
   >[!NOTE]
   >
   >컨텐츠 조각 오퍼의 JSON 형식은 사용자 지정할 수 있습니다. 이렇게 하려면 고객 컨텐츠 조각 구성 요소를 정의한 다음 구성 요소 Sling 모델에서 해당 속성을 내보내는 방법에 주석을 답니다.
   >
   >핵심 구성 요소를 참조하십시오. [핵심 구성 요소 - 컨텐츠 조각](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/wcm-components/content-fragment-component.html)
...
1. **Adobe Target**&#x200B;에서 다음을 선택합니다.
   ...
   * 필요한 경우 - 외부화 도메인
   ...
   >
   > 또한 외부자 도메인은 Target으로 전송되는 컨텐츠 조각의 컨텐츠와만 관련되며 오퍼 컨텐츠 보기와 같은 메타데이터는 연관되지 않습니다.
...
>[!IMPORTANT]
>
>새 기존 구성을 추가하는 것은 컨텐츠 조각 내보내기에만 지원되는 특별한 사례 시나리오입니다.
...
>
>이로 인해 Target으로 내보내기 전에 모든 관련 자산이 있는 컨텐츠 조각을 게시해야 합니다.

AEM에서 Target으로 컨텐츠 조각을 내보내려면(클라우드 구성을 지정한 후):

1. 에서 컨텐츠 조각으로 이동합니다 **자산** 콘솔.
1. 대상으로 내보낼 컨텐츠 조각을 선택합니다.

1. 탭/클릭 **Adobe Target 오퍼으로 내보내기**.
...
1. 필요에 따라 **게시하지 않고 내보내기** 또는 **게시**&#x200B;를 탭/클릭합니다.
   ...
   >[!NOTE]
   >
   >표시되는 실제 작업은 조각 및 관련 자산의 상태에 따라 달라집니다.
   >
   >모든 항목이 이미 게시되어 있고 그 이후에 수정된 내용이 없다면 이 단계가 전달됩니다.

   >[!NOTE]
   >
   >선택 **게시** 컨텐츠 조각을 즉시 게시하여 Target에 전송합니다.

1. 확인 대화 상자에서 **확인**&#x200B;을 탭/클릭합니다.

   이제 컨텐츠 조각이 Target에 있어야 합니다.
   ...
   >[!NOTE]
   >
   >Adobe Target에서 컨텐츠 조각을 볼 때 *마지막 수정 날짜* 보이는 날짜는 조각이 Adobe Target에 마지막으로 내보낸 날짜가 아니라 AEM에서 마지막으로 조각을 수정한 날짜입니다.
...
앞의 작업을 수행하면 컨텐츠 조각이 Target의 오퍼 페이지에 표시됩니다. 여기에서 수행할 수 있는 작업에 대해 알아보려면 [관련 Target 설명서](https://experienceleague.adobe.com/docs/target/using/integrate/aem/fragments/content-fragments-aem.html)를 살펴보십시오.

>[!NOTE]
>
>Adobe Target에서 컨텐츠 조각을 볼 때 *마지막 수정 날짜* 보이는 날짜는 조각이 Adobe Target에 마지막으로 내보낸 날짜가 아니라 AEM에서 마지막으로 조각을 수정한 날짜입니다.

## 이미 Adobe Target으로 내보낸 컨텐츠 조각 삭제 {#deleting-a-content-fragment-already-exported-to-adobe-target}

내보내기 와 마찬가지로, Adobe Target의 상단 도구 모음에서 컨텐츠 조각을 삭제해도 됩니다 **자산** 조각을 선택하면 콘솔:

![Adobe Target에서 삭제](assets/cfm-export-target-02.png)

이미 Target으로 내보낸 컨텐츠 조각을 삭제하면 Target의 오퍼에서 조각을 이미 사용하고 있는 경우 문제가 발생할 수 있습니다. AEM에서 조각 콘텐츠를 전달 중이므로 조각을 삭제하면 오퍼를 사용할 수 없습니다.

<!-- if the information about deleting-if-used correct, or is it not allowed at all? -->

이러한 상황을 방지하는 방법:

* 컨텐츠 조각이 현재 활동에서 사용되고 있지 않으면 AEM에서 경고 메시지 없이 조각을 삭제할 수 있습니다.
* 컨텐츠 조각이 현재 Target의 활동에서 사용 중인 경우 오류 메시지는 조각을 삭제하면 활동에 발생할 수 있는 결과에 대해 AEM 사용자에게 경고합니다.

   AEM의 오류 메시지에는 사용자가 컨텐츠 조각을 삭제할 수 없습니다(강제). 컨텐츠 조각이 삭제되는 경우:

   * AEM 컨텐츠 조각을 사용한 Target 오퍼에 원치 않는 동작이 표시될 수 있습니다

      * 컨텐츠 조각을 Target에 푸시했기 때문에 오퍼는 여전히 렌더링될 수 있습니다
      * 참조된 자산이 AEM에서도 삭제된 경우 컨텐츠 조각의 모든 참조가 제대로 작동하지 않을 수 있습니다.
   * 물론 컨텐츠 조각이 AEM에 더 이상 존재하지 않으므로 컨텐츠 조각을 추가로 수정할 수는 없습니다.
...
* [핵심 구성 요소 - 컨텐츠 조각](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/wcm-components/content-fragment-component.html)

* [Adobe Target 개발](https://developers.adobetarget.com/)

* [Adobe Target - Target 활동에서 AEM 컨텐츠 조각을 사용하여 최적화 또는 개인화를 지원합니다](https://experienceleague.adobe.com/docs/target/using/integrate/aem/fragments/content-fragments-aem.html)

* [Adobe Target - AEM 경험 구성요소 및 컨텐츠 조각 개요](https://experienceleague.adobe.com/docs/target/using/integrate/aem/fragments/aem-experience-and-content-fragments.html)
