...
>abstract="다양한 Experience Manager 배포에서 Experience Manager으로 고객을 전환하고 기존 고객이 연결되고 지속적인 경험을 제공할 수 있도록 지원하기 위한 권장되는 단계별 접근 방법을 설명합니다."
...
AEM(Adobe Experience Manager)은 Cloud Service의 재설계 기반으로서, 컨테이너 기반 인프라, API 기반 개발 및 안내 DevOps 프로세스를 기반으로 구축된 Experience Manager의 기반 구조를 제공하므로 마케터와 개발자는 고객 경험 관리 혁신을 항상 앞서나갈 수 있습니다.

Cloud Service은 브랜드 기업이 끊임없이 진화하는 소비자 요구를 충족할 수 있는 최신 클라우드 기본 아키텍처의 민첩성을 활용하여 Adobe Experience Manager의 풍부한 기능과 확장성을 제공합니다.

이 1페이지 분량의 문서는 다양한 Experience Manager 배포에서 Experience Manager으로 고객을 전환시키기 위한 권장되는 단계별 접근 방식을 소개하고, 기존 고객이 경험 관리를 위해 특별히 고안된 최신 플랫폼을 통해 지속적으로 연결된 경험을 전달할 수 있도록 합니다.
...
Managed Services의 선택적 컨텐츠 전달 도구인 Cloud Manager가 필요합니다. 이제 Cloud Service 환경으로 AEM에 코드를 배포하는 유일한 메커니즘입니다.

Cloud Service 환경으로 AEM을 구성하고 배포하는 방법에 대한 자가 도움말 리소스를 참조하십시오.
...
5. [운영 및 유지 관리](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/operations/home.html?lang=en)
   * [OSGI 구성 구성](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/deploying/configuring-osgi.html?lang=en#deploying)
   * [백업 및 복원](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/operations/backup.html?lang=en)

>[!TIP]
> [Experience Manager Cloud Service에 WKND를 배포하는 방법에 대한 자습서](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/develop-wknd-tutorial.html?lang=en) 참조
...
>2. [Experience Manager 커뮤니티 및 포럼 탐색](https://experienceleaguecommunities.adobe.com/t5/adobe-experience-manager/ct-p/adobe-experience-manager-community)
...
>title="Cloud Service으로 Adobe Experience Manager으로 이동"
>abstract="이 1페이지 분량의 문서는 다양한 Experience Manager 배포에서 Experience Manager으로 고객을 전환시키기 위한 권장되는 단계별 접근 방식을 소개하고, 기존 고객이 경험 관리를 위해 특별히 고안된 최신 플랫폼을 통해 지속적으로 연결된 경험을 전달할 수 있도록 합니다."

**Cloud Service은 Experience Manager 사이트 및 자산을 확장 가능하고 안전하며 민첩한 기술 기반을 제공하므로 마케터와 IT 담당자는 규모에 맞게 효과적인 경험을 전달하는 데 집중할 수 있습니다.**
...
Cloud Service으로 전환 과정을 시작하기 전에 Cloud Service에 익숙해지고 Experience Manager에 수행된 주목할 만한 변경 사항을 검토하고 교체 또는 폐기된 기능을 검토해야 합니다.

<table>
<tr>
<td>프로젝트 검색 및 평가</td>
<td><ul><li>Cloud Service과 Experience Manager 6.x로 Adobe Experience Manager 간의 중요한 차이점을 이해하려면 <a href="https://experienceleague.adobe.com/docs/experience-manager-cloud-service/release-notes/aem-cloud-changes.html?lang=en">Cloud Service에 대한 주목할 만한 변경 사항</a>을 참조하십시오.</li><li>사용되지 않음으로 표시된 기능 및 기능에 대한 자세한 내용은 <a href="https://experienceleague.adobe.com/docs/experience-manager-cloud-service/release-notes/deprecated-removed-features.html?lang=en#deprecated-features">사용되지 않는 기능</a>을 참조하십시오.</li><li>[Cloud Service 마이그레이션만 해당] Cloud Service 준비 상태 평가:소스 환경에서 <a href="https://experienceleague.adobe.com/docs/experience-manager-cloud-service/moving/cloud-migration/best-practices-analyzer/overview-best-practices-analyzer.html?lang=en#cloud-migration">Best Practices Analyzer(BPA)</a>를 실행합니다. </li><li>Experience Manager CS에서 주목할 만한 변경 사항 및 더 이상 사용되지 않는 기능에 대한 평가 완료</li></ul></td>
</tr>
<tr>
<td>리뷰</td>
<td><ul><li>검색, 노력 평가 및 리소스 작업 수행</li></ul></td>
</tr>
<tr>
<td>측정</td>
...
프로젝트의 실행 단계를 시작하기 전에 Cloud Service에 온보드 방식을 사용해야 합니다. 또한 Cloud Manager에 익숙해져야 합니다. Experience Manager Cloud Service 인스턴스에 프로젝트 코드를 배포하는 메커니즘입니다.

Cloud Manager를 사용하면 조직에서 클라우드에서 Experience Manager을 자체 관리할 수 있습니다. IT 팀 및 구현 파트너가 성능이나 보안을 손상하지 않고 사용자 지정 또는 업데이트를 신속하게 전달할 수 있도록 해주는 지속적인 통합 및 지속적인 전달([CI/CD](https://experienceleague.adobe.com/docs/experience-manager-cloud-manager/using/overview/ci-cd-pipeline.html?lang=en#overview)) 프레임워크가 포함되어 있습니다.

#### 콘텐츠 마이그레이션

1. [내용 전송 도구](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/migration/content-transfer-tool.html?lang=en#migration) :기존 컨텐츠를 소스 AEM 인스턴스(온-프레미스 또는 AMS)에서 대상 AEM Cloud Service 인스턴스로 이동하는 데 사용됩니다.
