...
이 환경은 품질 보증 팀이 새 시스템의 디자인과 기능을 모두 종합적으로 테스트하기 위해 사용합니다. 적절한 컨텐츠를 사용한 작성자 및 게시 환경이 모두 있어야 하며, 전체 테스트를 활성화하는 데 필요한 모든 서비스를 제공해야 합니다.
...
프로덕션 환경은 구현을 실제로 [작성 및 게시](/help/sites-cloud/authoring/getting-started/concepts.md)하는 데 필요한 환경으로 구성됩니다.
...
* 다른 인스턴스 간에 HTTP 또는 HTTPS 연결을 사용하여 컨텐츠를 직접 복제(AEM as a Cloud Service 복제)합니다.

![컨텐츠 이동](assets/content-movement.png)