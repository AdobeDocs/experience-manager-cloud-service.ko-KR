...
이 정보를 고려하여 Adobe은 현재 AEM 구성을 다시 평가하고 고급 네트워킹 설정과 함께 연결 풀링을 의도적으로 통합하는 것을 고려해 볼 것을 권장합니다. 병렬 연결 수를 관리하고 부실 연결 가능성을 최소화함으로써 프록시 서버가 연결 한도에 도달할 위험이 줄어듭니다. 따라서 이 전략적 구현은 요청이 외부 끝점에 도달하지 못할 가능성을 줄이기 위해 설계되었습니다.
