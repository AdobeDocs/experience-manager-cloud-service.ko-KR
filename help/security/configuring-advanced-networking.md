...
이 문서에서는 AEM as a Cloud Service에서 사용할 수 있는 고급 네트워킹 기능을 소개합니다. 이러한 기능에는 VPN, 비표준 포트 및 전용 이그레스 IP 주소의 셀프서비스 및 API 프로비저닝이 포함됩니다.

이 설명서 외에도 각 고급 네트워킹 옵션에 대해 설명하기 위해 설계된 일련의 튜토리얼이 있습니다. [고급 네트워킹](https://experienceleague.adobe.com/ko/docs/experience-manager-learn/cloud-service/networking/advanced-networking)을 참조하세요.
...
* 프로그램에 네트워크 인프라를 추가하고 구성하려면 사용자에게 **관리자** 역할이 있어야 합니다.
...
>[!TIP]
>
>유연한 포트 이그레스와 전용 이그레스 IP 주소 사이에서 결정할 때 특정 IP 주소가 필요하지 않은 경우 유연한 포트 이그레스를 선택하는 것이 좋습니다. 그 이유는 Adobe에서 유연한 포트 이그레스 트래픽의 성능을 최적화할 수 있기 때문입니다.
...
새 레코드가 사이드 패널의 **네트워크 인프라** 머리글 아래에 나타납니다. 여기에는 인프라 유형, 상태, 지역 및 인프라가 활성화된 환경과 같은 세부 정보가 포함됩니다.
...
* HTTP용: `AEM_PROXY_HOST`/ `AEM_HTTP_PROXY_PORT `(AEM 릴리스 &lt; 6094에서 기본값 `proxy.tunnel:3128`)
* https용: `AEM_PROXY_HOST`/ `AEM_HTTPS_PROXY_PORT `(AEM 릴리스 &lt; 6094에서는 기본값 `proxy.tunnel:3128`)
...
전용 IP 주소를 사용하면 CRM 공급업체와 같은 SaaS 공급업체 또는 IP 주소 허용 목록을 제공하는 AEM as a Cloud Service 외부의 다른 통합과 통합 시 보안을 강화할 수 있습니다. 전용 IP 주소를 허용 목록에 추가하면 AEM Cloud Service의 트래픽만 외부 서비스로 연결되도록 허용할 수 있습니다. 이 접근 방식은 허용된 다른 IP의 트래픽에 추가됩니다.

동일한 전용 IP가 프로그램의 모든 환경에 적용되며 작성자 및 게시 서비스 모두에 적용됩니다.

전용 IP 주소 기능을 활성화하지 않으면 AEM as a Cloud Service의 트래픽이 공유 IP 집합을 통해 흐릅니다. 이러한 IP는 AEM as a Cloud Service의 다른 고객이 사용합니다.

전용 이그레스 IP 주소를 구성하는 것은 [유연한 포트 이그레스](#flexible-port-egress)와 비슷합니다. 주요 차이점은 구성한 후에 트래픽이 항상 전용 고유 IP에서 이그레스된다는 것입니다. 해당 IP를 찾으려면 DNS Resolver를 사용하여 `p{PROGRAM_ID}.external.adobeaemcloud.com`과 연계된 IP 주소를 식별하십시오. 해당 IP 주소는 변경되지 않지만 변경해야 하는 경우 고급 알림이 제공됩니다.

>[!TIP]
>
>유연한 포트 이그레스와 전용 이그레스 IP 주소 사이에서 결정할 때 특정 IP 주소가 필요하지 않은 경우 유연한 포트 이그레스를 선택하십시오. 그 이유는 Adobe에서 유연한 포트 이그레스 트래픽의 성능을 최적화할 수 있기 때문입니다.

>[!NOTE]
>
>2021년 9월 30일 이전(즉, 2021년 9월 릴리스 이전)에 전용 이그레스 IP로 프로비저닝된 경우 전용 이그레스 IP 기능은 HTTP 및 HTTPS 포트만 지원합니다.
>
>이 결과에는 암호화 시 HTTP/1.1 및 HTTP/2가 포함됩니다. 또한 하나의 전용 이그레스 엔드포인트는 각각 포트 80/443에서 HTTP/HTTPS를 통해서만 타겟과 통신할 수 있습니다.
...
1. 열려 있는 **네트워크 인프라 추가** 마법사에서 **전용 이그레스 IP 주소**&#x200B;를 클릭합니다.
1. **지역** 드롭다운 메뉴에서 원하는 지역을 선택한 다음 **계속**&#x200B;을 클릭합니다.
...
해당 기능은 프록시 구성에 표준 Java™ 시스템 속성을 사용하는 경우 아웃바운드 트래픽을 생성하는 Java™ 코드 또는 라이브러리와 호환됩니다. 실제로 이 접근 방식에는 가장 일반적인 라이브러리가 포함되어야 합니다.
...
VPN을 사용하면 작성자, 게시 또는 미리보기 인스턴스에서 On-Premise 인프라 또는 데이터센터에 연결할 수 있습니다. 이 기능은 예를 들어 데이터베이스에 대한 액세스 보안을 유지하는 데 유용할 수 있습니다. 또한 VPN을 지원하는 CRM 공급업체 등의 SaaS 공급업체에 연결할 수도 있습니다.
...
프로그램당 한 번씩 POST `/program/<programId>/networkInfrastructures` 엔드포인트가 호출됩니다. 구성 정보의 페이로드를 전달합니다. 해당 정보에는 `kind` 매개 변수, 지역, 주소 공간(CIDR 목록 - 이 값은 나중에 수정할 수 없음), DNS Resolver(네트워크의 이름 확인을 위한)에 대한 **vpn** 값이 포함됩니다. 또한 게이트웨이 구성, 공유 VPN 키 및 IP 보안 정책과 같은 VPN 연결 정보도 포함합니다. 해당 엔드포인트는 `network_id`와 상태 등의 기타 정보에 응답합니다.
...
<tbody>
  <tr>
    ...
    <td>VPN으로부터의 연결은 CDN에 이 IP로부터 오는 연결로 표시됩니다. VPN에서 AEM으로 이동하는 연결만 허용하려면 이 IP만 허용하고 다른 모든 IP는 차단하도록 Cloud Manager을 구성하십시오. 자세한 내용은 “VPN 연결 인그레스 제한” 섹션을 확인하십시오.</td>
  </tr>
  <tr>
    <td><code>p{PROGRAM_ID}.{REGION}-gateway.external.adobeaemcloud.com</code></td>
    <td>해당 사항 없음</td>
    <td>AEM측 VPN 게이트웨이의 IP입니다. 네트워크 엔지니어링 팀은 이 IP를 사용하여 특정 IP 주소의 VPN 게이트웨이에 대한 VPN 연결만 허용할 수 있습니다. </td>
  </tr>
</tbody>
</table>
...
* **포트 전달** - 포트 전달 규칙은 http 또는 https 프로토콜을 사용하지 않는 경우에만 80/443 이외의 모든 대상 포트에 대해 선언해야 합니다.
   ...
   * http/https를 통해 포트 80/443을 사용하는 클라이언트 연결은 연결에 고급 네트워킹 속성을 적용하려면 해당 연결에서 여전히 프록시 설정을 사용해야 합니다.
...
* **비프록시 호스트** - 비프록시 호스트를 사용하면 전용 IP가 아닌 공유 IPS 주소 범위를 통해 라우팅해야 하는 호스트 세트를 선언할 수 있습니다.
   * 공유된 IP들을 통해 이그레스되는 트래픽이 더욱 최적화될 수 있기 때문에, 이 접근법은 유용할 수 있다.
...
1. 삭제할 인프라 옆에 있는 ![추가 아이콘, 줄임표](https://spectrum.adobe.com/static/icons/workflow_18/Smock_More_18_N.svg)를 클릭합니다.
...
1. 다음 중 하나를 수행하십시오.
   ...
   * **삭제**&#x200B;을(를) 선택한 경우 **삭제**&#x200B;와(과) 함께 **네트워크 구성 삭제** 대화 상자에서 삭제를 확인하거나 **취소**&#x200B;와(과) 함께 중단하십시오.
...
고급 네트워킹이 이미 구성된 환경에 추가 영역을 추가하면 추가 게시 영역의 트래픽이 기존 규칙을 따릅니다. 기본적으로 일치하는 트래픽은 기본 영역을 통해 라우팅됩니다. 단, 기본 지역을 사용할 수 없을 경우 고급 네트워킹이 추가 지역에서 활성화되지 않으면 고급 네트워킹 트래픽이 삭제됩니다. 지역 중 하나가 작동이 중단된 경우 지연 시간을 최적화하고 가용성을 높이려면 추가 게시 지역의 고급 네트워킹을 활성화해야 합니다. 다음 섹션에서 서로 다른 두 가지 시나리오를 설명합니다.
...
1. 전용 AEM IP 주소가 허용 목록에 추가되도록 인프라를 잠갔다면 해당 인프라에서 거부 규칙을 일시적으로 비활성화합니다. 이 단계를 건너뛰면 인프라에서 새 지역의 IP 주소에서 요청을 일시적으로 거부합니다. `p1234.external.adobeaemcloud.com`과(와) 같은 FQDN(정규화된 도메인 이름)을 사용하여 인프라를 잠근 경우에는 이 단계가 필요하지 않습니다. 모든 AEM 지역은 동일한 FQDN의 고급 네트워킹 트래픽을 이그레스합니다.
...
1. AEM 트래픽을 허용하기 위해 IP별로 인프라를 잠가야 하는 경우 `p1234.external.adobeaemcloud.com`에 해당하는 IP를 추가하십시오. 지역별로 하나의 IPS가 있어야 합니다.
...
1. [Cloud Manager 네트워크 인프라 API 만들기](https://developer.adobe.com/experience-cloud/cloud-manager/reference/api/#tag/Network-infrastructure/operation/createNetworkInfrastructure)에 대한 POST 호출을 통해 모든 지역의 네트워킹 인프라를 만듭니다. 기본 영역에 대한 페이로드의 JSON 구성의 유일한 차이는 영역 속성입니다.
...
절차는 전용 이그레스 IP 주소 지침과 거의 동일합니다. 유일한 차이점은 지역 속성이 기본 지역과 다르게 구성된다는 것입니다. 또한 선택적으로 `connections.gateway` 필드를 구성할 수 있습니다. 구성은 조직에서 운영하는 다른 VPN 엔드포인트로 라우팅할 수 있습니다(새 지역에 지리적으로 더 가깝게).
...
연결 풀링은 연결 저장소를 생성하고 유지하도록 맞춤화된 기술로, 연결 저장소를 필요로 하는 모든 스레드에서 즉시 사용할 수 있습니다. 여러 온라인 플랫폼과 리소스에서 수많은 연결 풀링 기술을 찾을 수 있으며 각각 고유한 장점과 고려 사항이 있습니다. Adobe은 고객이 이러한 방법론을 조사하여 시스템 아키텍처와 가장 호환되는 방법을 파악할 것을 권장합니다.

적절한 연결 풀링 전략을 구현하면 종종 최적이 아닌 성능으로 이어지는 시스템 구성의 일반적인 감독을 수정하기 위한 사전 조치가 됩니다. Adobe Experience Manager(AEM)는 연결 풀을 올바르게 설정해 외부 호출의 효율성을 향상시킬 수 있습니다. 이 접근 방식은 리소스 소비를 줄일 뿐만 아니라 서비스 중단 위험을 완화하고 업스트림 서버와 통신할 때 실패한 요청이 발생할 확률을 줄입니다.

이 정보를 기반으로 Adobe에서는 현재 AEM 구성을 검토하는 것이 좋습니다. 또한 고급 네트워킹 설정과 함께 의도적으로 연결 풀링을 사용하는 것도 고려해 보십시오. 병렬 연결 수를 관리하고 오래된 연결을 줄이면 네트워크 성능을 최적화하는 데 도움이 됩니다. 이러한 작업은 프록시 서버가 연결 한도에 도달할 위험을 낮춥니다. 따라서 이 전략적 구현은 요청이 외부 엔드포인트에 도달하지 못할 가능성을 줄이도록 설계되었습니다.

#### 연결 제한 FAQ

고급 네트워킹을 사용할 때 연결 수는 환경 전반의 안정성을 보장하고 낮은 환경에서 사용 가능한 연결이 소모되는 것을 방지하기 위해 제한됩니다.

연결은 AEM 인스턴스당 1000개로 제한되며 숫자가 750개에 도달하면 고객에게 경고가 전송됩니다.

##### 연결 제한은 비표준 포트의 아웃바운드 트래픽에만 적용됩니까, 아니면 모든 아웃바운드 트래픽에 적용됩니까?

제한은 고급 네트워킹을 사용하는 연결(비표준 포트의 이그레스, 전용 이그레스 IP 또는 VPN을 사용하는 연결)에만 해당됩니다.

##### 발신 연결 수가 크게 증가하지 않는 것으로 보입니다. 지금 알림이 수신되는 이유는 무엇입니까?

고객이 연결을 동적으로 생성하는 경우(예: 각 요청에 대해 하나 이상의 연결 생성) 트래픽 증가로 인해 연결에 스파이크가 발생할 수 있습니다.

##### 경고를 트리거하지 않고 과거에 유사한 상황이 발생했을 수 있습니까?
...
예. JVM 수준 및 네트워킹 인프라의 다른 지점에서 연결이 닫힙니다. 그러나 이 워크플로우는 모든 프로덕션 서비스에는 너무 늦습니다. 더 이상 필요하지 않은 경우 연결을 명시적으로 종료하거나 연결 풀링을 사용할 때 풀에 반환해야 합니다. 그렇지 않으면 자원 소비가 너무 많아 자원 고갈의 원인이 될 수 있다.

##### 최대 연결 제한에 도달하면 라이선스에 영향을 미치며 추가 비용이 발생합니까?

아니요. 이 제한과 관련된 라이선스나 비용은 없습니다. 이는 기술적 제한입니다.

##### 현재 사용량이 한도에 얼마나 가깝습니까? 최대 허용 한도는 얼마입니까?

연결이 750개를 초과하면 경고가 트리거됩니다. 최대 제한은 AEM 인스턴스당 연결 1000개입니다.

##### 이 제한이 VPN에 적용됩니까?

예. 이 제한은 VPN을 비롯한 고급 네트워킹을 사용하는 연결에 적용됩니다.
