...
고급 네트워킹을 사용할 때 여러 환경에서 안정성을 보장하고 낮은 환경에서 사용 가능한 연결이 소모되는 것을 방지하기 위해 연결 수가 제한됩니다.

연결은 AEM 인스턴스당 1000개로 제한되며 숫자가 750에 도달하면 고객에게 경고가 전송됩니다.

##### 연결 제한이 비표준 포트의 아웃바운드 트래픽 또는 모든 아웃바운드 트래픽에만 적용됩니까?

제한은 고급 네트워킹(비표준 포트의 이그레스, 전용 이그레스 IP 또는 VPN 사용)을 사용하는 연결에만 해당됩니다.

##### 우리는 발신 접속 수에서 큰 차이를 보이지 않는다. 지금 알림을 받는 이유는 무엇입니까?

고객이 연결을 동적으로 만드는 경우(예: 각 요청에 대해 하나 이상), 트래픽이 증가하면 연결이 급증할 수 있습니다.

##### 우리가 과거에 경고 받지 않고 비슷한 상황을 겪었을 가능성이 있는가?

경고는 소프트 제한에 도달한 경우에만 전송됩니다.

##### 최대 한도에 도달하면 어떻게 됩니까?

하드 제한에 도달하면 고급 네트워킹을 통한 AEM의 새 이그레스 연결(비표준 포트의 이그레스, 전용 이그레스 IP 또는 VPN을 사용하는 이그레스)이 삭제되어 DoS 공격으로부터 보호됩니다.

##### 한도를 높일 수 있습니까?

아니요. 연결 수가 많으면 Pod 및 환경에서 성능에 상당한 영향을 주고 DoS를 초래할 수 있습니다.

##### 특정 기간이 지나면 AEM 시스템에서 연결을 자동으로 닫습니까?

예. JVM 수준 및 네트워킹 인프라의 다양한 지점에서 연결이 닫힙니다. 그러나 이 작업은 모든 프로덕션 서비스에는 너무 늦습니다. 더 이상 필요하지 않은 경우 연결을 명시적으로 닫거나 연결 풀링을 사용할 때 풀로 반환해야 합니다. 그렇지 않으면 자원 소모량이 너무 많아 자원 고갈의 원인이 될 수 있다.

##### 최대 연결 한도에 도달하면 라이선스에 영향을 주고 추가 비용이 발생합니까?

아니요. 이 제한과 관련된 라이선스 또는 비용은 없습니다. 이는 기술적 제한입니다.

##### 한계에 얼마나 근접해 있나요? 최대 한도는 얼마입니까?

연결이 750을 초과하면 경고가 트리거됩니다. 최대 제한은 AEM 인스턴스당 1000개의 연결입니다.

##### 이 제한을 VPN에 적용할 수 있습니까?

예. 이 제한은 VPN을 포함한 고급 네트워킹을 사용하는 연결에 적용됩니다.

##### 전용 이그레스 IP를 사용하는 경우 이 제한을 계속 적용할 수 있습니까?

예. 전용 이그레스 IP를 사용하는 경우에도 이 제한을 적용할 수 있습니다.
