...
* [전용 송신 IP 주소](#dedicated-egress-IP-address) - 고유한 IP에서 발생하도록 AEM as a Cloud Service에서 트래픽을 구성합니다
* [가상 사설 네트워크(VPN)](#vpn) - VPN 기술을 보유한 고객을 위해 고객의 인프라와 AEM as a Cloud Service 간 트래픽 보안
...
자세한 내용은 [Cloud Manager API 설명서](https://developer.adobe.com/experience-cloud/cloud-manager/reference/api/#operation/disableEnvironmentAdvancedNetworkingConfiguration).
...
<tbody>
  ...
  <tr>
    ...
    <td>클라이언트가 <code>AEM_PROXY_HOST</code> 를 사용하는 환경 변수 <code>portOrig</code> 에 선언됨 <code>portForwards</code> API 매개 변수.</td>
    <td>임의</td>
    <td>허용</td>
...
>[!NOTE]
>
>2021년 9월 릴리스(10/6/21) 전에 전용 송신 IP를 프로비저닝된 경우, 다음을 참조하십시오. [기존 전용 송신 주소 고객](#legacy-dedicated-egress-address-customers).
...
전용 송신 IP 주소 구성이 다음과 동일합니다 [유연한 포트 송신](#configuring-flexible-port-egress-provision).
...
<tbody>
  ...
  <tr>
    <td></td>
    <td>와 일치하는 호스트 <code>nonProxyHosts</code> 매개 변수</td>
    <td>80 또는 443</td>
    <td>공유 클러스터 IP 사용</td>
    <td></td>
  </tr>
  ...
  <tr>
    <td><b>http가 아니거나 https가 아님</b></td>
    <td>클라이언트가 <code>AEM_PROXY_HOST</code> 를 사용한 env 변수 <code>portOrig</code> 에 선언됨 <code>portForwards</code> API 매개 변수</td>
    <td>임의</td>
    <td>전용 송신 IP 사용</td>
    <td><code>mysql.example.com:3306</code></td>
  </tr>
...
예상되는 전용 IP 주소에서 트래픽이 실제로 전송되는지 확인하려면 대상 서비스의 로그(사용 가능한 경우)를 확인하십시오. 그렇지 않으면 다음과 같은 디버깅 서비스를 호출하는 것이 유용할 수 있습니다. [https://ifconfig.me/IP](https://ifconfig.me/IP): 호출 IP 주소를 반환합니다.
...
IPSec 기술을 사용하는 대부분의 VPN 장치가 지원됩니다. 다음 위치에서 장치 목록을 참조하십시오. [이 페이지](https://docs.microsoft.com/en-us/azure/vpn-gateway/vpn-gateway-about-vpn-devices#devicetable)으로 설정되면, **RouteBased 구성 지침** 열. 표에 설명된 대로 장치를 구성합니다.
...
프로그램당 한 번, POST `/program/<programId>/networkInfrastructures` 엔드포인트가 호출되어 다음을 포함한 구성 정보의 페이로드를 전달합니다. 에 대한 &quot;vpn&quot; 값 `kind` 매개 변수, 지역, 주소 공간(CIDR 목록 - 나중에 수정할 수 없음), DNS 해상도(고객 네트워크에서 이름 확인) 및 게이트웨이 구성, 공유 VPN 키, IP 보안 정책 등의 VPN 연결 정보. 엔드포인트는 `network_id`뿐만 아니라 상태를 포함한 기타 정보도 포함됩니다. 전체 매개 변수 세트와 정확한 구문은 [API 설명서](https://developer.adobe.com/experience-cloud/cloud-manager/reference/api/#operation/createNetworkInfrastructure).
...
자세한 내용은 [API 설명서](https://developer.adobe.com/experience-cloud/cloud-manager/reference/api/#operation/enableEnvironmentAdvancedNetworkingConfiguration) 추가 정보.
...
특정 환경에 대해 VPN을 사용하지 않도록 설정하려면 다음을 호출합니다. `DELETE /program/{programId}/environment/{environmentId}/advancedNetworking`. 자세한 내용은 [API 설명서](https://developer.adobe.com/experience-cloud/cloud-manager/reference/api/#operation/disableEnvironmentAdvancedNetworkingConfiguration).
...
<tbody>
  ...
  <tr>
    <td></td>
    <td>IP가 <i>VPN 게이트웨이 주소</i> 공간 범위 및 http 프록시 구성(표준 Java HTTP 클라이언트 라이브러리를 사용하는 http/s 트래픽에 대해 기본적으로 구성됨)을 통해</td>
    <td>임의</td>
    <td>VPN 사용</td>
  ...
  <tr>
    <td></td>
    <td>IP가 <i>VPN 게이트웨이 주소 공간</i> 범위 및 http 프록시 구성을 통해(표준 Java HTTP 클라이언트 라이브러리를 사용하는 http/s 트래픽에 대해 기본적으로 구성됨)</td>
    <td>임의</td>
    <td>전용 송신 IP 사용</td>
    <td></td>
  </tr>
  ...
  <tr>
    <td><b>http가 아니거나 https가 아님</b></td>
    <td>IP가 <i>VPN 게이트웨이 주소 공간</i> 범위 및 클라이언트가 <code>AEM_PROXY_HOST</code> 를 사용한 env 변수 <code>portOrig</code> 에 선언됨 <code>portForwards</code> API 매개 변수</td>
    <td>임의</td>
    <td>VPN 사용</td>
  ...
  <tr>
    <td></td>
    <td>IP가 <i>VPN 게이트웨이 주소 공간</i> 범위 및 클라이언트 연결 <code>AEM_PROXY_HOST</code> 를 사용한 env 변수 <code>portOrig</code> 에 선언됨 <code>portForwards</code> API 매개 변수</td>
    <td>임의</td>
    <td>전용 송신 IP 사용</td>
    <td></td>
  </tr>
...
<tbody>
  ...
  <tr>
    ...
    <td>고객이 AEM에 대한 VPN 액세스만 허용하려면 매핑할 CNAME DNS 항목을 구성해야 합니다 <code>author-p{PROGRAM_ID}-e{ENVIRONMENT_ID}.adobeaemcloud.com</code>  및/또는 <code>publish-p{PROGRAM_ID}-e{ENVIRONMENT_ID}.adobeaemcloud.com</code> 아래와 같이 변경하는 것을 의미합니다.</td>
  </tr>
</tbody>
</table>
