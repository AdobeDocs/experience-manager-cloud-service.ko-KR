...
대부분의 IPSec 기술이 내장된 VPN 디바이스가 지원됩니다. [이 디바이스 목록의 **RouteBased 구성 지침** 열에 기재된 정보를 참조하십시오.](https://learn.microsoft.com/en-us/azure/vpn-gateway/vpn-gateway-about-vpn-devices#devicetable) 표에 설명된 대로 디바이스를 구성합니다.
...
다음 사항은 유용한 지침으로 제공되며 문제 해결을 위한 우수 사례를 포함합니다. 이러한 권장 사항은 문제를 효과적으로 진단하고 해결하는 데 도움을 주기 위한 것입니다.

### 연결 풀링 {#connection-pooling-advanced-networking}

연결 풀링은 필요한 스레드에서 즉시 사용할 수 있도록 준비되는 연결 저장소를 만들고 유지할 수 있도록 맞춤화된 기술입니다. 수많은 연결 풀링 기법은 다양한 온라인 플랫폼 및 리소스에 걸쳐 발견될 수 있으며, 각각의 고유한 장점과 고려 사항이 있습니다. EMC는 고객이 이러한 방법론을 조사하여 시스템 아키텍처와 가장 잘 호환되는 방법을 파악할 것을 권장합니다.

적절한 연결 풀링 전략을 구현하는 것은 시스템 구성에서 일반적인 감독을 수정하기 위한 사전 예방적 조치이며, 이는 종종 차선적인 성능으로 이어진다. Adobe Experience Manager(AEM)는 연결 풀을 올바르게 설정함으로써 외부 호출의 효율성을 높일 수 있습니다. 이렇게 하면 리소스 소모가 줄어들 뿐만 아니라 서비스 중단 위험을 완화하고 업스트림 서버와 통신할 때 실패한 요청이 발생할 확률을 줄일 수 있습니다.

이 정보를 고려하여 현재 AEM 구성을 재평가하고 고급 네트워킹 설정과 함께 연결 풀링을 의도적으로 통합하는 것을 고려하는 것이 좋습니다. 이러한 조치를 구현함으로써 프록시가 오버로드되지 않도록 솔루션을 설계하여 요청이 외부 끝점에 도달하지 못할 가능성을 줄일 수 있습니다.

이 정보를 고려하여 현재 AEM 구성을 재평가하고 고급 네트워킹 설정과 함께 연결 풀링을 의도적으로 통합하는 것을 고려하는 것이 좋습니다. 병렬 연결 수를 관리하고 부실 연결 가능성을 최소화함으로써 프록시 서버가 연결 한도에 도달할 위험이 줄어듭니다. 따라서 이 전략적 구현은 요청이 외부 끝점에 도달하지 못할 가능성을 줄이기 위해 설계되었습니다.
