...
포트 전달 규칙은 80/443 이외의 대상 포트에 대해 선언되어야 하지만, http 또는 https 프로토콜을 사용하지 않는 경우에만 대상 호스트 집합(이름 또는 IP, 포트 포함)을 지정하여 선언해야 합니다. 각 대상 호스트에 대해 고객은 원하는 대상 포트를 30000에서 30999 사이의 포트에 매핑해야 합니다.
