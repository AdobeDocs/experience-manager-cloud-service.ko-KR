...
이 문서에서는 AEM as a Cloud Service에서 사용할 수 있는 고급 네트워킹 기능을 소개합니다. 이러한 기능에는 VPN의 셀프서비스 및 API 프로비저닝, 비표준 포트, 전용 이그레스 IP 주소가 포함됩니다.

이 문서 외에도 각 고급 네트워킹 옵션을 안내하도록 고안된 일련의 튜토리얼도 있습니다. [고급 네트워킹](https://experienceleague.adobe.com/ko/docs/experience-manager-learn/cloud-service/networking/advanced-networking)을 참조하십시오.

>[!IMPORTANT]
>
>Cloud Manager UI를 통하거나 Cloud Manager API(예: cURL)를 사용하여 AEM as a Cloud Service에서 고급 네트워킹을 구성할 수 있습니다.
>
>이 문서는 UI 방법 사용에 중점을 둡니다. API를 통해 구성을 자동화하는 것을 선호하는 경우 [VPN(Virtual Private Network) 튜토리얼](https://experienceleague.adobe.com/ko/docs/experience-manager-learn/cloud-service/networking/vpn)을 참조하십시오.
>
>**API를 사용하여 고급 네트워킹 자동화**
>고급 네트워킹 설정(예: VPN 생성)을 자동화하기 위해 Cloud Manager API를 사용할 수 있습니다.
>
>```bash
...
>```
>
>전체 튜토리얼과 더 많은 API 예시는 [VPN(Virtual Private Network) 튜토리얼](https://experienceleague.adobe.com/ko/docs/experience-manager-learn/cloud-service/networking/vpn)에서 확인하십시오.
>
...
1. **프로그램 개요** 페이지에서 **환경** 탭으로 이동하여 왼쪽 패널에서 **네트워크 인프라**&#x200B;를 선택합니다.

   ![네트워크 인프라 추가](assets/advanced-networking-ui-network-infrastructure.png)

1. **네트워크 인프라 추가** 마법사에서 **유연한 포트 이그레스**&#x200B;를 선택합니다.
1. **지역** 드롭다운 메뉴에서 원하는 지역을 선택한 다음 **계속**&#x200B;을 클릭합니다.
...
측면 패널의 **네트워크 인프라** 제목 아래에 새 레코드가 나타납니다. 여기에는 인프라 유형, 상태, 지역 및 활성화 환경과 같은 세부 사항이 포함됩니다.

![네트워크 인프라의 새 항목](assets/advanced-networking-ui-flexible-port-egress-new-entry.png)
...
`portForwards` 매개 변수에 선언된 포트를 통한 대상이 있는 비 HTTP/S 트래픽은 매핑된 포트와 함께 `AEM_PROXY_HOST` 속성을 참조해야 합니다. 예:

```java
DriverManager.getConnection("jdbc:mysql://" + System.getenv("AEM_PROXY_HOST") + ":53306/test");
```

아래 테이블에서는 트래픽 라우팅에 대해 설명합니다.
...
전용 IP 주소를 사용하면 CRM 공급업체와 같은 SaaS 공급업체 또는 IP 주소 허용 목록을 제공하는 AEM as a Cloud Service 외부의 다른 통합과 통합 시 보안을 강화할 수 있습니다. 전용 IP 주소를 허용 목록에 추가하면 AEM Cloud Service의 트래픽만 외부 서비스로 연결되도록 허용할 수 있습니다. 이 접근 방식에는 다른 허용된 IP의 트래픽도 포함됩니다.

동일한 전용 IP가 프로그램의 모든 환경에 적용되며 작성자 및 게시 서비스 모두에 적용됩니다.

전용 IP 주소 기능을 활성화하지 않으면 AEM as a Cloud Service의 트래픽이 공유된 IP 집합을 통해 흐릅니다. 이 IP는 AEM as a Cloud Service의 다른 고객이 사용하고 있습니다.

전용 이그레스 IP 주소의 구성은 [유연한 포트 이그레스](#flexible-port-egress)와 유사합니다. 주요 차이점은 구성 후 트래픽은 항상 전용 고유 IP에서 이그레스된다는 것입니다. 해당 IP를 찾으려면 DNS Resolver를 사용하여 `p{PROGRAM_ID}.external.adobeaemcloud.com`과 연계된 IP 주소를 식별하십시오. 해당 IP 주소는 변경되지 않지만 변경해야 하는 경우 고급 알림이 제공됩니다.
...
>
>이 결과에는 암호화 시 HTTP/1.1 및 HTTP/2가 포함됩니다. 또한 하나의 전용 이그레스 엔드포인트는 각각 포트 80/443에서 HTTP / HTTPS를 통해서만 대상과 통신할 수 있습니다.
...
1. **네트워크 인프라 추가** 마법사가 열리면 **전용 이그레스 IP 주소**&#x200B;를 클릭합니다.
1. **지역** 드롭다운 메뉴에서 원하는 지역을 선택한 다음 **계속**&#x200B;을 클릭합니다.
...
새 레코드가 사이드 패널의 **네트워크 인프라** 머리글 아래에 나타납니다. 여기에는 인프라 유형, 상태, 지역 및 활성화 환경과 같은 세부 사항이 포함됩니다.
...
해당 기능은 프록시 구성에 표준 Java™ 시스템 속성을 사용하는 경우 아웃바운드 트래픽을 생성하는 Java™ 코드 또는 라이브러리와 호환됩니다. 따라서 이 접근 방식에는 가장 일반적인 라이브러리가 포함되어야 합니다.
...
에 대한 명시적 호출이 필요한 Apache HttpClient를 사용하는 예제
[`HttpClientBuilder.useSystemProperties()`](https://hc.apache.org/httpcomponents-client-4.5.x/current/httpclient/apidocs/org/apache/http/impl/client/HttpClientBuilder.html) 또는 사용
...
VPN을 사용하면 작성자, 게시 또는 미리보기 인스턴스에서 On-Premise 인프라 또는 데이터센터에 연결할 수 있습니다. 예를 들면 이 기능은 데이터베이스에 대한 액세스를 보호하는 데 유용할 수 있습니다. 또한 VPN을 지원하는 CRM 공급업체 등의 SaaS 공급업체에 연결할 수도 있습니다.

대부분의 IPSec 기술이 내장된 VPN 디바이스가 지원됩니다. [이 디바이스 목록](https://learn.microsoft.com/ko-kr/azure/vpn-gateway/vpn-gateway-about-vpn-devices#devicetable)의 **RouteBased 구성 지침** 열에 기재된 정보를 참조하십시오. 테이블에 설명된 대로 디바이스를 구성하십시오.
...
프로그램당 한 번씩 POST `/program/<programId>/networkInfrastructures` 엔드포인트가 호출됩니다. 구성 정보의 페이로드를 전달합니다. 해당 정보는 `kind` 매개변수, 지역, 주소 공간(CIDR 목록 - 나중에 값을 수정할 수 없음)에 대한 **vpn** 값, 고객 네트워크의 이름 확인을 위한 DNS Resolver를 포함합니다. 또한 게이트웨이 구성, 공유 VPN 키 및 IP 보안 정책과 같은 VPN 연결 정보도 포함합니다. 해당 엔드포인트는 `network_id`와 상태 등의 기타 정보에 응답합니다.
...
다음 다이어그램은 구성 및 개발에 유용한 도메인 및 관련 IP 집합을 시각적으로 보여 줍니다. 다이어그램 아래의 테이블에서는 이들 도메인과 IP에 대해 설명합니다.
...
<tbody>
  <tr>
    ...
    <td>비공개 네트워크를 통하지 않고 인터넷으로 이동하는 트래픽에 대한 전용 이그레스 IP 주소입니다.</td>
  </tr>
  <tr>
...
* **프록시가 아닌 호스트** - 프록시가 아닌 호스트를 사용하면 전용 IP가 아닌 공유 IP 주소 범위를 통해 라우팅해야 하는 호스트 집합을 선언할 수 있습니다.
   * 공유된 IP 주소들을 통해 이그레스되는 트래픽이 추가로 최적화될 수 있기 때문에 이 접근법은 유용할 수 있다.
...
1. 전용 이그레스 IP 주소 및 VPN의 경우 **비프록시 호스트** 탭에서 필요에 따라 호스트 세트를 정의할 수 있습니다. 이러한 정의된 호스트는 **비프록시 호스트** 필드에 호스트 이름을 제공하고 **추가**&#x200B;를 클릭하여 전용 IP가 아닌 공유 IP 주소 범위를 통해 라우팅되어야 합니다.

   * 호스트가 탭의 호스트 목록에 추가됩니다.
   * 여러 호스트를 추가하려면 이 단계를 반복합니다.
   * 호스트를 제거하려면 행 오른쪽에 있는 X 를 클릭합니다.
...
전용 이그레스 IP 주소와 VPN 고급 네트워킹 유형은 `nonProxyHosts` 매개변수를 지원합니다. 이 지원을 통해 전용 IP가 아닌 공유 IP 주소 범위를 통해 라우팅해야 하는 호스트 집합을 선언할 수 있습니다. `nonProxyHost` URL은 `example.com` 또는 `*.example.com`의 패턴을 따르며, 여기서 와일드카드는 도메인의 시작 위치에서만 지원됩니다.
...
### UI로 편집, 테스트 또는 삭제 {#delete-ui}

1. [my.cloudmanager.adobe.com](https://my.cloudmanager.adobe.com/)에서 Cloud Manager에 로그인한 다음 적절한 조직을 선택합니다.

1. **[내 프로그램](/help/implementing/cloud-manager/navigation.md#my-programs)** 콘솔에서 프로그램을 선택합니다.

1. **프로그램 개요** 페이지에서 **환경** 탭으로 이동합니다.
1. 왼쪽 패널에서 **네트워크 인프라**&#x200B;를 클릭합니다.
1. 편집, 테스트 또는 삭제할 인프라 옆에 있는 ![추가 아이콘, 줄임표](https://spectrum.adobe.com/static/icons/workflow_18/Smock_More_18_N.svg)를 클릭합니다.

   ![프로그램 수준에서 고급 네트워킹 편집 또는 삭제 선택](assets/advanced-networking-ui-delete-infrastructure.png)

1. **편집**, **테스트** 또는 **삭제**&#x200B;를 클릭합니다.

1. 다음 중 하나를 수행하십시오.

   * **편집**&#x200B;을 선택하면 **네트워크 인프라 편집** 마법사가 열립니다. 인프라를 만들 때 설명된 단계대로 필요에 따라 편집합니다.

   * **최근 테스트** 드롭다운 메뉴에서 연결을 활성화하기 전에 연결을 자체 테스트하기 위해 **테스트**&#x200B;를 선택한 경우 구성을 다시 로드할 항목을 선택한 다음 **테스트**&#x200B;를 클릭합니다. 테스트를 실행하지 않은 경우 메뉴에 *최근 테스트 없음*&#x200B;이 표시됩니다.

     또는 **호스트** 텍스트 필드에 필요한 대상 호스트 이름을 입력합니다. 그런 다음 **포트** 드롭다운 메뉴에서 필요한 해당 포트를 선택합니다. **테스트**&#x200B;를 클릭합니다. 대화 상자의 **테스트 결과** 섹션에 결과가 표시됩니다.
...
프로그램에 대해 한 번에 한 가지 유형의 고급 네트워킹 인프라만 구성할 수 있습니다. 고급 네트워킹 인프라는 유연한 포트 이그레스, 전용 이그레스 IP 주소 또는 VPN이어야 합니다.

이미 구성한 것과 다른 고급 네트워킹 인프라 유형이 필요하다고 판단되는 경우 기존 네트워킹 인프라 유형을 삭제하고 다른 네트워킹 인프라 유형을 만듭니다. 다음 작업을 수행합니다.
...
## 다른 게시 영역에 대한 고급 네트워킹 구성 {#advanced-networking-configuration-for-additional-publish-regions}

고급 네트워킹이 이미 구성된 환경에 추가 지역을 추가하면 추가 게시 지역의 트래픽이 기존 규칙을 따릅니다. 기본적으로 일치하는 트래픽은 기본 지역을 통해 라우팅됩니다. 단, 기본 지역을 사용할 수 없을 경우 고급 네트워킹이 추가 지역에서 활성화되지 않으면 고급 네트워킹 트래픽이 삭제됩니다. 지역 중 하나가 작동이 중단된 경우 지연 시간을 최적화하고 가용성을 높이려면 추가 게시 지역의 고급 네트워킹을 활성화해야 합니다. 다음 섹션에서 서로 다른 두 가지 시나리오를 설명합니다.
...
1. 전용 AEM IP 주소가 허용 목록에 추가되도록 인프라를 잠갔다면 해당 인프라에서 거부 규칙을 일시적으로 비활성화합니다. 이 단계를 건너뛰면 인프라가 새 지역의 IP 주소 요청을 일시적으로 거부합니다. 이 단계는 `p1234.external.adobeaemcloud.com`과 같은 FQDN(Fully Qualified Domain Name)을 사용하여 인프라를 잠그는 경우에는 필요하지 않습니다. 모든 AEM 지역은 동일한 FQDN에서 고급 네트워킹 트래픽을 전송합니다.
...
1. AEM 트래픽을 허용하도록 IP별로 인프라를 잠가야 하는 경우 `p1234.external.adobeaemcloud.com`에 해당하는 IP 주소를 추가하십시오. 지역별로 하나의 IPS가 있어야 합니다.
...
절차는 전용 이그레스 IP 주소 지침과 거의 동일합니다. 유일한 차이점은 지역 속성이 기본 지역과 다르게 구성된다는 점입니다. 또한 선택적으로 `connections.gateway` 필드를 구성할 수 있습니다. 구성은 조직에서 운영하는 다른 VPN 엔드포인트로 라우팅할 수 있습니다(새 지역에 지리적으로 더 가깝게).

## 문제 해결

다음 사항은 유용한 지침으로 제공되며 문제 해결에 대한 우수 사례를 포함합니다. 이러한 권장 사항은 문제를 효과적으로 진단하고 해결하는 데 도움이 되도록 하기 위한 것입니다.

### 연결 풀링 {#connection-pooling-advanced-networking}

연결 풀링은 연결 저장소를 생성하고 유지하도록 맞춤화된 기술로, 연결 저장소를 필요로 하는 모든 스레드에서 즉시 사용할 수 있습니다. 여러 온라인 플랫폼과 리소스에서 수많은 연결 풀링 기술을 찾을 수 있으며 각각 고유한 장점과 고려 사항이 있습니다. Adobe는 고객이 이러한 방법론을 파악하여 시스템 아키텍처와 가장 호환되는 방법을 식별하도록 권장합니다.

적절한 연결 풀링 전략을 구현하면 종종 최적이 아닌 성능으로 이어지는 시스템 구성의 일반적인 감독을 수정하기 위한 사전 조치가 됩니다. Adobe Experience Manager(AEM)는 연결 풀을 올바르게 설정해 외부 호출의 효율성을 향상시킬 수 있습니다. 이 접근 방식으로 리소스 소비를 줄일 수 있을 뿐만 아니라 서비스 중단의 위험을 완화하고 업스트림 서버와 통신할 때 요청이 실패할 확률을 줄일 수 있습니다.

Adobe에서는 이러한 정보를 바탕으로 현재 AEM 구성을 검토할 것을 권장합니다. 또한 고급 네트워킹 설정과 함께 의도적으로 연결 풀링을 사용하는 것도 고려해 보십시오. 병렬 연결 수를 관리하고 오래된 연결을 줄이면 네트워크 성능을 최적화하는 데 도움이 됩니다. 이러한 액션을 수행하면 프록시 서버가 연결 제한에 도달할 위험이 낮아집니다. 따라서 이 전략적 구현은 요청이 외부 엔드포인트에 도달하지 못할 가능성을 줄이도록 설계되었습니다.
...
##### 발신 연결 수에는 큰 증가가 없는 것 같습니다. 지금 알림을 받는 이유는 무엇입니까?

고객이 연결을 동적으로 생성하는 경우(예: 각 요청에 대해 하나 이상의 연결 생성) 트래픽 증가로 인해 연결에 스파이크가 발생할 수 있습니다.

##### 비슷한 상황이 경보가 발생하지 않고도 과거에 발생할 수 있었을까요?

경고는 가벼운 제한에 도달한 경우에만 전송됩니다.

##### 최대 한도에 도달하면 어떻게 됩니까?

하드 제한에 도달하면 고급 네트워킹(비표준 포트의 이그레스, 전용 이그레스 IP 또는 VPN 사용)을 통한 AEM의 새 이그레스 연결이 DoS 공격으로부터 보호하기 위해 삭제됩니다.
...
예, 연결은 JVM 수준과 네트워킹 인프라의 다양한 지점에서 종료됩니다. 그러나 이 워크플로는 모든 프로덕션 서비스에 너무 늦습니다. 더 이상 필요하지 않은 경우 연결을 명시적으로 종료하거나 연결 풀링을 사용할 때 풀에 반환해야 합니다. 그렇지 않으면 리소스 소비가 너무 높아 리소스가 소진될 수 있습니다.

##### 최대 연결 제한에 도달하면 라이선스에 영향을 미치며 추가 비용이 발생합니까?

아니요. 이 제한과 관련된 라이선스나 비용은 없습니다. 이는 기술적 제한입니다.

##### 현재 사용량이 한도에 얼마나 근접했습니까? 최대 허용 제한은 얼마입니까?
...
##### 전용 이그레스 IP를 사용하는 경우에도 제한이 적용됩니까?
