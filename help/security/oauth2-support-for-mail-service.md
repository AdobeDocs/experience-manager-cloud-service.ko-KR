...
여러 이메일 공급자에 대해 OAuth를 구성할 수 있습니다. 다음은 Microsoft® Office 365 Outlook과 함께 OAuth2를 통해 인증하도록 AEM 메일 서비스를 구성하는 방법에 대한 단계별 지침입니다. 다른 공급업체가 유사한 방식으로 구성될 수 있습니다.
...
1. 검색 대상 **Azure Active Directory** 검색 창에서 결과를 클릭합니다. 또는 [https://portal.azure.com/#blade/Microsoft_AAD_IAM/ActiveDirectoryMenuBlade/Overview](https://portal.azure.com/#blade/Microsoft_AAD_IAM/ActiveDirectoryMenuBlade/Overview)에서 바로 검색할 수 있습니다.
1. 클릭 **앱 등록** > **새 등록**.
...
1. 아래의 앱에 대한 권한을 선택한 다음 **권한 추가**&#x200B;를 클릭합니다.
   ...
   * `email`
   * `profile`
1. 다음으로 이동 **인증** > **플랫폼 추가** > **웹**, 및 **리디렉션 Url** 섹션에서 슬래시가 있는 URL과 슬래시가 없는 URL을 추가합니다.
   * `http://localhost/`
   * `http://localhost`
1. 누르기 **구성** 각 URL을 추가하고 요구 사항에 따라 설정을 구성합니다.
1. 다음으로 이동 **인증서 및 암호**, 클릭 **새 클라이언트 암호** 화면에 표시되는 단계에 따라 암호를 만드십시오. 나중에 사용할 수 있도록 이 암호를 메모해 두십시오.
1. 누르기 **개요** 왼쪽 창에서 다음 값을 복사합니다. **애플리케이션(클라이언트) ID** 및 **디렉터리(테넌트) ID** 나중에 사용합니다.

재점검하려면 다음 정보를 사용하여 AEM 측의 메일 서비스에 대한 OAuth2를 구성하십시오.

* 테넌트 ID로 구성되는 인증 URL 형식은 다음과 같습니다. `https://login.microsoftonline.com/<tenantID>/oauth2/v2.0/authorize`
* 테넌트 ID로 구성되는 토큰 URL 형식은 다음과 같습니다. `https://login.microsoftonline.com/<tenantID>/oauth2/v2.0/token`
* 테넌트 ID로 구성되는 새로 고침 URL 형식은 다음과 같습니다. `https://login.microsoftonline.com/<tenantID>/oauth2/v2.0/token`
* 클라이언트 ID
* 클라이언트 보안

### 새로 고침 토큰 생성 {#generating-the-refresh-token}

다음으로, 다음을 수행하여 후속 단계에서 OSGi 구성의 일부인 새로 고침 토큰을 생성합니다.

1. 을 교체한 후 브라우저에서 다음 URL을 엽니다. `clientID` 및 `tenantID` 계정과 관련된 값: `https://login.microsoftonline.com/%3ctenantID%3e/oauth2/v2.0/authorize?client_id=%3cclientId%3e&response_type=code&redirect_uri=http://localhost&response_mode=query&scope=https://outlook.office.com/SMTP.Send%20email%20openid%20profile%20offline_access&state=12345`.
1. 요청을 받으면 권한을 허용합니다.
1. 이 URL은 다음 형식으로 구성된 새 위치로 리디렉션됩니다. `http://localhost/?code=<code>&state=12345&session_state=4f984c6b-cc1f-47b9-81b2-66522ea83f81#`.
1. 값 복사 `<code>` 위의 예에서,
1. 다음 cURL 명령을 사용하여 refreshToken을 가져옵니다. tenantID, clientID 및 clientSecret을 계정에 대한 값과 `<code>`:
...
1. 다음 curl을 사용하여 이전 절차에서 생성된 refreshToken을 사용하여 accessToken을 생성하고 의 값을 바꿉니다. `<client_id>`,`<client_secret>`, 및 `<refreshToken>`:
...
1. 제대로 작동하는지 확인할 수 있도록 accessToken을 사용하여 메일을 보냅니다.
...
1. 다음을 입력합니다. `authUrl`, `tokenUrl`, 및 `refreshURL` 앞 절에서 설명한 바와 같이 그것들을 구성함으로써.
1. 구성에 다음 범위를 추가합니다.
   * `https://outlook.office.com/SMTP.Send`
   * `https://graph.microsoft.com/Mail.Read`
   * `https://graph.microsoft.com/Mail.Send`
...
메일 서비스가 제대로 작동하지 않으면 를 다시 생성해야 합니다. `refreshToken` 위에서 설명한 대로 Cloud Manager API를 통해 새 값을 전달합니다. 새 값을 배포하는 데 몇 분 정도 소요됩니다.
