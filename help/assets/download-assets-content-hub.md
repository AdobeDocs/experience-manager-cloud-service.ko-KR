...
* 여러 에셋 및 해당 렌디션을 다운로드하려면 다음을 수행하십시오.
   1. 에셋을 선택하고 ![다운로드](/help/assets/assets/download-icon.svg) **[!UICONTROL 다운로드]**&#x200B;를 클릭한 다음 **[!UICONTROL 에셋 다운로드]** 대화 상자에서 선택한 에셋 목록을 검토합니다. 목록에서 자산 선택을 취소하려면 자산 옆에 있는 ![선택 취소](/help/assets/assets/Close.svg)를 클릭합니다.
   1. 렌디션을 하나 이상 선택하여 ZIP 파일로 다운로드합니다. **[!UICONTROL 스마트 자르기]** 및 **[!UICONTROL 정적 렌디션]**&#x200B;을 선택하면 선택한 각 에셋의 사용 가능한 모든 정적 및 스마트 자르기 렌디션이 다운로드됩니다.
   1. 선택 사항: **[!UICONTROL 각 에셋에 대해 별도의 폴더 만들기]**&#x200B;를 선택 해제하여 선택한 에셋과 해당 렌디션을 zip 파일의 폴더 내에 있는 플랫 계층 구조로 다운로드합니다. 기본적으로 [!DNL Content Hub]은(는) 선택한 에셋과 해당 표현물을 zip 파일 내의 개별 폴더로 다운로드합니다.
...
에셋 렌디션은 에셋의 원본 파일을 다르게 표현한 것입니다. 이러한 렌디션에는 썸네일, 웹 또는 모바일용으로 최적화된 버전, 워터마크가 지정되거나 DRM 보호 파일을 포함하거나 스마트 자르기와 같은 동적 요소가 포함될 수 있습니다. 원본 파일 형식과 일치시킬 필요가 없으며, 대신 다양한 사용 사례에서 에셋을 나타내는 역할을 합니다.
...
## 자주 묻는 질문 {#faqs-download-assets-content-hub}

### AEM Assets Content Hub에서 단일 에셋 또는 여러 에셋을 다운로드하려면 어떻게 합니까?

AEM Assets Content Hub에서 단일 에셋을 다운로드하려면 에셋 카드에서 다운로드 아이콘을 클릭합니다. 여러 에셋의 경우 에셋을 선택하고 작업 표시줄에서 다운로드 를 클릭합니다. 관리자가 다운로드를 활성화한 경우 Content Hub에서 원본 렌디션의 에셋 다운로드를 시작합니다. 에셋 변환을 사용할 수 있는 경우 다운로드하기 전에 특정 변환을 선택할 수 있습니다.

### AEM Assets Content Hub에서 관리자가 관리하는 사용자가 원본 에셋을 다운로드하거나 렌디션의 가용성을 활성화할 수 있는 구성이 있습니까?

예. 관리자가 구성 UI의 **렌디션** 탭에서 사용할 수 있는 **렌디션의 사용 가능** 및 **원본 에셋의 다운로드 허용** 토글을 활성화하거나 비활성화하여 원본 에셋의 다운로드를 관리하거나 AEM Assets Content Hub에서 렌디션의 사용 가능 여부를 설정할 수 있습니다.

### 에셋 렌디션이란 무엇이며 AEM Assets Content Hub에서 사용할 수 있는 유형은 무엇입니까?

AEM Assets Content Hub의 에셋 렌디션은 다양한 사용 사례에 맞게 조정된 원래 에셋 파일을 서로 다르게 표현합니다. 유형에는 정적 렌디션(예: 썸네일, 웹 또는 모바일에 최적화된 버전, 인쇄용 고해상도 파일), 사용자 지정 정적 렌디션 및 동적 렌디션(스마트 자르기 버전 포함)이 포함됩니다. 정적 렌디션은 미리 생성되는 반면 동적 렌디션은 요청에 따라 실시간으로 생성됩니다.

### 스마트 자르기는 AEM Assets Content Hub의 에셋에서 어떻게 작동합니까?

AEM Assets Content Hub의 스마트 자르기는 이미지의 가장 중요한 부분을 식별하고 주변을 지능적으로 자르는 AI 기반 기능입니다. 이렇게 하면 다양한 화면 및 형식에서 시각적 개체가 잘 보입니다. 조직에서 Dynamic Media 기능을 지원하는 경우 이미지 에셋에 스마트 자르기 렌디션을 사용할 수 있습니다.

### AEM Assets Content Hub에서 동적 변환을 활성화하려면 어떻게 합니까?

AEM Assets Content Hub에서 동적 변환을 활성화하려면 다음 작업을 수행하십시오.

* OpenAPI 기능이 있는 Dynamic Media가 활성화되어 있는지 확인합니다. 그런 다음 승인된 자산을 공개 게재할 수 있습니다.

* 자산 승인 대상을 Content Hub으로 설정합니다.

* 구성 UI에서 렌디션 탭으로 이동하여 렌디션의 가용성을 활성화합니다.

* OpenAPI를 사용하여 Dynamic Media에 새로 온보딩하는 경우, 관리 보기 > 도구 > Assets > 이미지 사전 설정에서 기존 이미지 사전 설정을 다시 저장합니다(편집 > 저장).

동적 변환은 이미지 자산에 대해서만 지원됩니다.

### AEM Assets Content Hub에서 에셋 렌디션을 다운로드하려면 어떻게 합니까?

AEM Assets Content Hub에서 렌디션을 다운로드하려면 에셋을 선택하고 다운로드 작업을 클릭합니다. 관리자가 렌디션을 활성화한 경우 다운로드할 렌디션을 선택할 수 있는 대화 상자가 나타납니다. 라이센스가 부여된 에셋의 경우 다운로드하기 전에 라이센스 약관에 동의하십시오. 여러 에셋의 경우 에셋을 선택하고 렌디션을 선택한 다음 ZIP 파일로 다운로드합니다. 사용 가능한 모든 렌디션이나 정적 또는 동적 렌디션과 같은 특정 렌디션을 선택할 수 있습니다.
