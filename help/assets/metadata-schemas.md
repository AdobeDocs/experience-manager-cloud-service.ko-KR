...
1. 자산에 대한 속성 페이지를 보려면 카드 보기의 자산 타일의 **[!UICONTROL 빠른]** 작업에서 속성 보기 아이콘을 클릭하거나 탭합니다. 또는 UI에서 자산을 선택한 다음 도구 모음에서 **[!UICONTROL 속성]** 아이콘을 클릭하거나 탭합니다.
1. 다양한 탭에서 다양한 메타데이터 속성을 편집합니다. 하지만 속성 페이지에서 자산 유형을 수정할 수는 없습니다.
자산에 대한 MIME 유형을 수정하려면 사용자 지정 메타데이터 스키마 양식을 사용하거나 기존 양식을 수정합니다. 자세한 [내용은 메타데이터 스키마](#edit-metadata-schema-forms) 양식 편집을 참조하십시오. 특정 MIME 유형의 메타데이터 스키마를 수정하는 경우 현재 MIME 형식과 모든 자산 하위 형식의 자산에 대한 속성 페이지 레이아웃이 수정됩니다. 예를 들어, jpeg 스키마를 `default/image` 수정하면 MIME 유형의 자산에 대한 메타데이터 레이아웃(자산 속성)만 수정됩니다 `image/jpeg`. 그러나 기본 스키마를 편집하는 경우 변경 사항은 모든 유형의 자산에 대한 메타데이터 레이아웃을 수정합니다.
...
AEM에서는 즉시 다음과 같은 템플릿을 제공합니다.
   ...
   * **기본값**:자산에 대한 기본 메타데이터 스키마 양식입니다.
   다음 하위 양식은 기본 양식의 속성을 상속합니다.i. **이미지**:MIME 유형이 &quot;image&quot;인 에셋에 대한 스키마 양식입니다(예: `image/jpeg``image/png`등).
&quot;이미지&quot; 양식에는 다음과 같은 하위 양식 템플릿이 있습니다.a. **jpeg**:하위 유형이 있는 자산에 대한 스키마 양식입니다 `jpeg`.
b. **tiff**:하위 유형이 있는 자산에 대한 스키마 양식입니다 `tiff`.

   ii. **애플리케이션**:MIME 유형이 `application`있는 에셋에 대한 스키마 양식(예: `application/pdf``application/zip`등)
a. **pdf**:하위 유형이 있는 자산에 대한 스키마 양식입니다 `pdf`.

   iii. **비디오**:MIME 형식이 `video`있는 자산( `video/avi`예: `video/mp4`등)에 대한 스키마 양식입니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td>스마트 태그</td>
   <td>메타데이터 태그를 자동으로 추가하여 검색 기능을 확장할 수 있습니다.<br /> </td>
  </tr>
  <tr>
   <td>숨김 필드</td>
  ...
  <tr>
   <td>자산 참조</td>
   <td>자산을 참조하는 자산 목록을 표시하는 데 추가합니다.</td>
  </tr>
  <tr>
   <td>제품 참조</td>
  ...
  <tr>
   <td>상황에 맞는 메타데이터</td>
   <td>자산 속성 페이지에서 다른 메타데이터 탭의 표시를 제어하기 위해 추가합니다.</td>
  </tr>
 </tbody>
</table>
...
#### 스키마 양식의 탭 추가 및 삭제 {#add-delete-a-tab-in-the-schema-form}
...
>[!NOTE]
>
>기본 양식에 대한 사용자 지정 변경 내용을 삭제하면 해당 양식이 기본 상태로 돌아가도록 되었음을 나타내기 위해 메타데이터 스키마 인터페이스에서 해당 양식 앞에 잠금 아이콘이 다시 나타납니다.
...
적절한 양식 유형 아래에 새 양식을 만듭니다. 예를 들어 **이미지/png** 하위 유형에 대한 새 템플릿을 추가하려면 &quot;이미지&quot; 양식 아래에 양식을 만듭니다. 스키마 양식의 제목은 하위 유형 이름입니다. 이 경우 제목은 &quot;png&quot;입니다&#x200B;**.**
...
| `mimetypes` | 속성에 정의된 양식을 사용하는 MIME 형식 `exposedmimetype` 목록 | 문자열 | `image/png` |
...
메타데이터 스키마 기능은 관리자만 사용할 수 있습니다. 그러나 관리자는 일부 권한을 수정하여 비관리자에게 액세스할 수 있습니다. 관리자가 아닌 사용자에게는 `/conf` 폴더에 대한 만들기, 수정 및 삭제 권한이 있어야 합니다.
...
1. (선택 사항) `https://[server]:[port]/system/console/components/`액세스. 기본적으로 비활성화된 `com.day.cq.dam.core.impl.MissingMetadataNotificationJob` 구성 요소를 구성하고 활성화합니다. AEM에서 자산의 메타데이터의 유효성을 확인하는 빈도를 설정합니다.
   ...
   이 구성은 자산의 `hasValidMetadata` 속성을 추가합니다 `jcr:content` . AEM은 이 속성을 사용하여 검색 결과를 필터링할 수 있습니다.

   >[!NOTE]
   >
   >예약된 확인 후 자산이 추가되는 경우, 자산은 다음 예약된 확인 `hasValidMetadata` 때까지 플래그로 표시되지 않습니다. 자산은 중간 검색 결과에 표시되지 않습니다.
