...
자산에 대한 MIME 형식을 수정하려면 사용자 지정 메타데이터 스키마 양식을 사용하거나 기존 양식을 수정하십시오. 자세한 [내용은 메타데이터 스키마 양식](#edit-metadata-schema-forms) 편집을 참조하십시오. 특정 MIME 유형의 메타데이터 스키마를 수정하는 경우 현재 MIME 형식과 모든 자산 하위 형식의 자산에 대한 속성 페이지 레이아웃이 수정됩니다. 예를 들어 jpeg 스키마를 `default/image` 수정하면 MIME 형식의 자산에 대한 메타데이터 레이아웃(자산 속성)만 수정됩니다 `image/jpeg`. 그러나 기본 스키마를 편집하는 경우 변경 사항은 모든 유형의 자산에 대한 메타데이터 레이아웃을 수정합니다.

1. 양식/템플릿 목록을 보려면 AEM 로고를 클릭한 다음 도구 > **[!UICONTROL 자산]** > **[!UICONTROL 메타데이터 스키마]** 로 **[!UICONTROL 이동합니다]**.
AEM에서는 기본적으로 다음과 같은 템플릿을 제공합니다.

   * **기본값**: 자산에 대한 기본 메타데이터 스키마 양식입니다.

   다음 하위 양식은 기본 양식의 속성을 상속합니다.
i. **이미지**: MIME 유형이 &quot;image&quot;인 에셋에 대한 스키마 양식(예: `image/jpeg`, `image/png`등)입니다.
&quot;이미지&quot; 양식에는 다음과 같은 하위 양식 템플릿이 있습니다.
a. **jpeg**: 하위 형식의 자산에 대한 스키마 양식입니다 `jpeg`.
...
1. 스키마 양식 페이지에서 양식 앞에 있는 확인란을 선택한 다음 도구 모음에서 **편집 아이콘** 을 클릭합니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td>숨김 필드</td>
   <td>숨김 필드를 추가합니다. 자산이 저장되면 POST 매개 변수로 전송됩니다.</td>
  </tr>
  <tr>
   <td>자산 참조자</td>
...
적절한 양식 유형 아래에서 새 양식을 만듭니다. 예를 들어 **이미지/png** 하위 유형에 대한 새 템플릿을 추가하려면 &quot;이미지&quot; 양식에서 양식을 만듭니다. 스키마 양식의 제목은 하위 유형 이름입니다. 이 경우 제목은 &quot;png.**&quot;입니다**
...
1. AEM 로고를 클릭한 다음 도구 > **[!UICONTROL 자산]** > **[!UICONTROL 메타데이터]** 스키마 **[!UICONTROL 로 이동합니다]**. 메타데이터 **[!UICONTROL 스키마 양식]** 페이지가 표시됩니다.
...
1. [ **[!UICONTROL 메타데이터 스키마 양식]** ] 페이지에서 `my_default` 양식을 선택한 다음 **[!UICONTROL 편집]** 아이콘을클릭합니다.
...
1. **[!UICONTROL 저장]**&#x200B;을 클릭합니다. 수정된 양식이 [ **[!UICONTROL 메타데이터 스키마 양식] 페이지에]** 나열됩니다. 양식을 선택한 다음 도구 모음 **[!UICONTROL 에서 폴더에]** 적용을 클릭하거나 탭하여 사용자 지정 메타데이터를 폴더에 적용합니다.
1. 사용자 지정 양식에 추가한 필수 필드에 대한 메타데이터가 없는 일부 자산을 폴더로 이동하고 업로드합니다. 필수 필드에 대한 누락된 메타데이터에 대한 메시지가 자산의 카드 보기에 표시됩니다.
1. (선택 사항) 액세스 `https://[server]:[port]/system/console/components/`. 기본적으로 비활성화된 구성 `com.day.cq.dam.core.impl.MissingMetadataNotificationJob` 요소를 구성하고 활성화합니다. AEM이 자산에 대한 메타데이터의 유효성을 확인하는 빈도를 설정합니다.

   이 구성은 자산의 속성 `hasValidMetadata` 을 `jcr:content` 추가합니다. 이 속성을 사용하여 AEM에서 검색 결과를 필터링할 수 있습니다.
