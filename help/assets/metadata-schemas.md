...
자산에 대한 MIME 형식을 수정하려면 사용자 지정 메타데이터 스키마 양식을 사용하거나 기존 양식을 수정합니다. 자세한 내용은 [메타데이터 스키마 편집 Forms](#edit-metadata-schema-forms)을 참조하십시오. 특정 MIME 유형에 대한 메타데이터 스키마를 수정하는 경우 현재 MIME 유형 및 모든 자산 하위 유형의 자산에 대한 속성 페이지 레이아웃이 수정됩니다. 예를 들어 `default/image` 아래의 jpeg 스키마를 수정하면 MIME 유형이 `image/jpeg`인 자산에 대한 메타데이터 레이아웃(자산 속성)만 수정합니다. 그러나 기본 스키마를 편집하는 경우 변경 사항은 모든 유형의 자산에 대한 메타데이터 레이아웃을 수정합니다.

1. 양식/템플릿 목록을 보려면 AEM 로고를 클릭한 다음 **[!UICONTROL 도구]** > **[!UICONTROL 자산]** > **[!UICONTROL 메타데이터 스키마]**로 이동합니다.
AEM에서는 기본적으로 다음 템플릿을 제공합니다.

   * **기본값**:자산에 대한 기본 메타데이터 스키마 양식입니다.

   다음 하위 양식은 기본 양식의 속성을 상속합니다.
i.**이미지**:MIME 유형이 &quot;image&quot;인 에셋의 스키마 양식(예: `image/jpeg`, `image/png` 등)입니다.
&quot;이미지&quot; 양식에는 다음과 같은 하위 양식 템플릿이 있습니다.
a.**jpeg**:하위 유형이 `jpeg`인 자산에 대한 스키마 양식입니다.
b.**tiff**:하위 유형이 `tiff`인 자산에 대한 스키마 양식입니다.

   ii. **애플리케이션**:MIME 유형의 자산 `application`에 대한 스키마 양식(예:  `application/pdf`등) `application/zip`.
a.**pdf**:하위 유형이 `pdf`인 자산에 대한 스키마 양식입니다.

   iii. **비디오**:MIME 형식을 사용하는 자산 `video`의 스키마 양식( `video/avi`예:  `video/mp4`등)입니다.
...
1. 사용자 지정 템플릿을 목록에 추가하려면 도구 모음에서 **[!UICONTROL 만들기]**&#x200B;를 클릭합니다.
   ...
   >[!NOTE]
   >
   >편집하지 않은 템플릿에는 먼저 잠금 아이콘이 표시됩니다. 템플릿을 사용자 지정하면 템플릿이 사라지기 전에 잠금 아이콘이 표시됩니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td>자산 등급</td>
   <td>자산 등급을 지정하기 위한 표시 옵션에 추가합니다.</td>
  </tr>
  <tr>
   <td>상황에 맞는 메타데이터</td>
   <td>자산의 속성 페이지에서 다른 메타데이터 탭의 표시를 제어하는 데 추가를 클릭합니다.</td>
  </tr>
 </tbody>
</table>
...
#### 스키마 양식 {#add-delete-a-tab-in-the-schema-form}에서 탭 추가 및 삭제
...
적절한 양식 유형 아래에 새 양식을 만듭니다. 예를 들어 **image/png** 하위 유형에 대한 새 템플릿을 추가하려면 &quot;image&quot; 양식 아래에 양식을 만듭니다. 스키마 양식의 제목은 하위 유형 이름입니다. 이 경우 제목은 &quot;png.**&quot;**
...
| `mimetypes` | `exposedmimetype` 속성에 정의된 양식을 사용하는 MIME 유형 목록 | 문자열 | `image/png` |
...
메타데이터 스키마 기능은 관리자만 사용할 수 있습니다. 그러나 관리자는 일부 권한을 수정하여 관리자가 아닌 사람에게 액세스 권한을 제공할 수 있습니다. 관리자가 아닌 사용자는 `/conf` 폴더에 대한 권한을 만들고, 수정하고, 삭제해야 합니다.

## 폴더별 메타데이터 적용 중 {#applying-folder-specific-metadata}
