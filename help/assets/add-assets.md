...
[!DNL Adobe Experience Manager Assets]은(는) 다양한 소스의 다양한 디지털 에셋을 허용합니다. 바이너리와 생성된 렌디션을 저장하고, 다양한 워크플로우 및 [!DNL Adobe AI] 서비스를 사용하여 에셋 처리를 수행할 수 있으며, 여러 표면에 걸쳐 다양한 채널을 통해 배포할 수 있습니다.
...
| [[!DNL Experience Manager] 데스크탑 앱](https://experienceleague.adobe.com/docs/experience-manager-desktop-app/using/using.html?lang=ko) | 낮은 볼륨 에셋 수집이지만 마이그레이션용은 아닙니다. | 관리자, 마케터 |
...
* [[!DNL Experience Manager] 데스크톱 앱](https://experienceleague.adobe.com/docs/experience-manager-desktop-app/using/using.html?lang=ko): 로컬 파일 시스템에서 에셋을 업로드하는 크리에이티브 전문가 및 마케터에게 유용합니다. 로컬에서 사용할 수 있는 중첩된 폴더를 업로드하는 데 사용합니다.
...
이 도구는 Azure 또는 S3 데이터 저장소에서 자산을 대규모로 수집하는 데 사용할 관리자 그룹에만 제공됩니다. 구성 및 수집에 대한 자세한 내용은 비디오를 참조하십시오.

>[!VIDEO](https://video.tv.adobe.com/v/341391/?captions=kor&quality=12&learn=on)
...
이 기능을 사용하려면 Azure 또는 AWS의 외부 스토리지 계정이나 버킷이 필요합니다.
...
1. 데이터 소스와의 연결을 만드는 데 사용할 값을 제공합니다. 예를 들어 **Azure Blob 저장소**&#x200B;를 데이터 소스로 선택하는 경우 Azure 저장소 계정, Azure Blob 컨테이너 및 Azure 액세스 키의 값을 지정합니다.

1. 드롭다운 목록에서 필요한 인증 모드를 선택합니다. **Azure 액세스 키**&#x200B;는 Azure 저장소 계정에 대한 완전한 액세스를 제공하는 반면, **Azure SAS 토큰**&#x200B;을(를) 사용하면 관리자는 권한 및 만료 정책을 사용하여 토큰의 기능을 제한할 수 있습니다.
...
1. (선택 사항) **[!UICONTROL MIME 유형 제외]** 필드에서 수집에서 제외할 MIME 유형의 쉼표로 구분된 목록을 지정합니다. 예를 들어, `image/jpeg, image/.*, video/mp4`과 같이 입력합니다. [지원되는 모든 파일 형식](/help/assets/file-format-support.md)을 참조하세요.

1. **[!UICONTROL MIME 유형 포함]** 필드의 수집에서 포함할 MIME 유형의 쉼표로 구분된 목록을 지정하십시오. [지원되는 모든 파일 형식](/help/assets/file-format-support.md)을 참조하세요.

1. 파일을 **[!UICONTROL (으)로 가져온 후 원본 데이터 저장소에서 원본 파일을 삭제하려면]**&#x200B;가져온 후 원본 파일 삭제[!DNL Experience Manager] 옵션을 선택하십시오.
...
[!DNL Experience Manager]은(는) 웹 브라우저 사용자 인터페이스 외에도 데스크톱에서 다른 클라이언트를 지원합니다. 또한 웹 브라우저로 이동하지 않고도 업로드를 수행할 수 있습니다.
...
* [[!DNL Experience Manager] 데스크톱 앱](https://experienceleague.adobe.com/docs/experience-manager-desktop-app/using/using.html?lang=ko)은(는) 파일 형식이나 자산을 처리하는 기본 응용 프로그램에 관계없이 데스크톱에서 자산 작업을 간소화합니다. 브라우저 업로드는 플랫 파일 목록 업로드만 지원하므로 로컬 파일 시스템에서 중첩된 폴더 계층으로 파일을 업로드하는 것이 유용합니다.

## 업로드 시 에셋 처리 {#process-when-uploaded}

업로드된 에셋에 대해 추가 처리를 수행하려면 업로드 폴더에 처리 프로필을 적용할 수 있습니다. 프로필은 **[!UICONTROL 폴더의]**&#x200B;속성[!DNL Assets] 페이지에서 사용할 수 있습니다. 확장이 없거나 잘못된 디지털 자산은 원하는 대로 처리되지 않습니다. 예를 들어 이러한 에셋을 업로드할 때 아무것도 발생하지 않거나 잘못된 처리 프로필이 에셋에 적용될 수 있습니다. 사용자는 DAM에 이진 파일을 저장할 수 있습니다.
...
* [!DNL Experience Manager Assets]은(는) 이제 많은 하위 자산을 포함하는 폴더를 지원합니다. 폴더에 1000개가 넘는 직접 하위 폴더(에셋 또는 하위 폴더)가 포함되면 관리 UI는 비동기적으로 업데이트된 인덱스를 사용하여 폴더 콘텐츠를 나열합니다. 따라서 새로 생성된 폴더 및 에셋의 가시성이 잠시 지연될 수 있으며(일반적으로 몇 초), 관리 보기에서 이러한 폴더를 열 때 최종 사용자에게 다음과 같은 동작을 알리는 배너가 표시됩니다. &quot;이 디렉터리에는 1000개 이상의 항목이 포함되어 있습니다. 업로드 및 새 폴더 생성이 지연될 수 있습니다.&quot;
...
>[!MORELIKETHIS]
>
>* [[!DNL Adobe Experience Manager] 데스크탑 앱](https://experienceleague.adobe.com/docs/experience-manager-desktop-app/using/introduction.html?lang=ko)
