...
메타데이터 전략을 계획할 때 요구 사항에 따라 사용자 지정 메타데이터 스키마를 만드는 것이 중요합니다. 잘 설계된 스키마는 Experience Manager 내 에셋을 분류하고 구성하는 구조화된 프레임워크를 제공합니다.
...
저장소 전체에서 일관된 결과를 보장하기 위해 전략을 신중하게 계획합니다. 자세히 알아보기 [메타데이터 스키마](metadata-schemas.md).
...
예를 들어 과 같이 중복 이름을 사용하는 대신 *campaign_image_001* 및 *campaign_image_002*&#x200B;를 사용하여 다음과 같은 체계적인 이름 지정 규칙을 구현합니다 *event_promotion* 및 *product_unch*, 명확하고 순서가 지정된 식별을 보장합니다.

* **통제 어휘를 효과적으로 사용:** 태그에 표준화된 용어를 사용하여 제어된 어휘를 구현합니다. 구현 방법 알아보기 [AEM 태깅 프레임워크](/help/implementing/developing/introduction/tagging-framework.md) 효과적으로.  <br>
...
* **키워드 최적화:** 에셋과 연결된 키워드를 최적화하여 메타데이터 검색 기능을 개선합니다. 에서 특정 에셋에 대한 키워드의 관련성을 향상시킬 수 있습니다. [!UICONTROL 자산 관리자] 다음 단계를 수행합니다.

   1. 다음으로 이동 **[!UICONTROL 에셋]** > **[!UICONTROL 파일]** > **[!UICONTROL [자산 폴더]]**.
   1. 메타데이터를 업데이트할 자산을 선택한 다음 를 클릭합니다 **[!UICONTROL 속성]**.
   1. 다음 위치로 이동 **[!UICONTROL 고급]** 탭을 클릭한 다음 **[!UICONTROL 추가]** 다음 아래에 **[!UICONTROL 검색 키워드에 대한 개선]**. <br>검색 키워드를 향상시키려면 기본 메타데이터 스키마를 사용해야 합니다.
...
추가한 키워드를 사용하여 에셋을 검색합니다. 에셋이 상위 검색 결과에 나타납니다.
  ...
  방법 알아보기 [Experience Manager에서 검색 증폭](https://experienceleague.adobe.com/docs/experience-manager-learn/assets/search-and-discovery/search-boost.html).

* **사용자 지정 메타데이터 필드:** 메타데이터 필드를 사용자 정의하여 에셋에 대한 추가 정보를 캡처합니다. 예를 들어 프로젝트 세부 사항, 저작권 정보 또는 검색 기능을 향상시키는 기타 관련 데이터에 대한 특정 필드를 추가합니다. 학습 [사용자 지정 메타데이터를 편집하거나 추가하는 방법](meta-edit.md) Experience Manager Assets.


* **메타데이터 유효성 검사:** 메타데이터 항목에 대한 유효성 검사 를 구현하여 일관성 및 정확성을 보장합니다. 제어 어휘를 사용하면 유효성 검사 프로세스가 더 매끄러워지고 명확하지 않거나 일관되지 않은 항목이 나올 가능성이 줄어듭니다. 여기에는 모호하거나 일관되지 않은 정보를 방지하기 위해 특정 메타데이터 속성에 대한 지침을 설정하는 작업이 포함될 수 있습니다.
...
콘텐츠 공급망을 검사하여 메타데이터 소스를 찾고 맨 위부터 시작하여 모든 관련자를 참여시켜 철저한 모범 사례 접근 방식을 구현합니다. 조직 전반에서 완벽한 지원을 보장하기 위해 다양한 직원을 참여시킵니다. <br>다양한 단계에 메타데이터를 통합하여 업로드 중 에셋 세부 정보 제공 책임을 공유합니다. 예: 통합 [!DNL Experience Manager Assets] 및 [!DNL Workfront] 는 메타데이터 관리 측면에서 상당한 이점을 제공하여 콘텐츠 생성 및 관리의 효율성과 공동 작업을 향상시킵니다. 이 통합을 통해 연결된 에셋에 대한 효과적인 메타데이터 동기화를 보장하고에서 변경 사항이 발생하면 프로젝트 세부 사항을 자동으로 업데이트합니다. [!DNL Workfront].
