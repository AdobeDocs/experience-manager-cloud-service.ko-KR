...
**시나리오 2: `classic car` 폴더 내에서 `automobile` 키워드를 사용하여 모든 파일을 검색합니다.**
...
일부 키워드를 제외한 결과를 검색하려는 경우 NOT 연산자를 사용할 수 있습니다. NOT 연산자는 하이픈(-) 기호를 사용하여 검색 결과에서 제외할 항목을 AEM 검색으로 보냅니다. 예를 들어 `car - classic`이(가) 포함되어 있지만 `car`을(를) 제외한 메타데이터를 지정하는 `classic` 검색 쿼리입니다.
