...
3. 다음 중 하나를 선택합니다. **현재 페이지** 또는 **모든 페이지**. 내보낼 자산의 이름과 형식을 지정합니다. 캔버스 컨텐츠를 PNG, JPEG, PDF, MP4, MP4+PNG 또는 MP4+JPEG 형식으로 내보낼 수 있습니다. 캔버스 페이지의 자산에 따라 형식이 자동으로 조정됩니다.
선택 **현재 페이지** 현재 페이지의 자산을 대상 폴더에 저장합니다. 다음을 선택하는 경우 **모든 페이지** 내보내기 형식이 PDF이 아닌 경우, 모든 캔버스 페이지는 대상 폴더 내의 새 폴더에 별도의 파일로 저장됩니다. 내보내기 형식이 PDF 형식이면 모든 캔버스 페이지가 대상 폴더에 단일 PDF 파일로 저장됩니다.

4. 아래의 폴더 아이콘을 클릭합니다. **대상 폴더** 위치를 선택하고 에셋을 저장합니다.
