...
>
>콘텐츠 조각 작성에는 두 개의 편집기가 있습니다. 기본 기능은 동일하지만 몇 가지 차이점이 있습니다. 이 섹션에서는 주로 **Assets** 콘솔에서 액세스하는 원본 편집기에 대해 설명합니다. 새 편집기([콘텐츠 조각](/help/sites-cloud/administering/content-fragments/authoring.md) 콘솔에서 주로 액세스)에 대한 자세한 내용은 사이트 설명서 **콘텐츠 조각 - 작성**&#x200B;을 참조하십시오. 두 편집기 모두 상단 도구 모음에 토글 스위치가 있어 다른 편집기에 빠르게 액세스할 수 있습니다.
...
* 페이지에서 참조할 때 [중간 콘텐츠](#in-between-content-when-page-authoring-with-content-fragments)(으)로 사용됩니다.
...
Assets은 각각 고유한 장점이 있는 여러 가지 방법으로 컨텐츠 조각에 사용할 수 있습니다.
...
컨텐츠 조각을 만들려면 다음이 필요합니다.

* **콘텐츠 조각 모델**

   * [구성 브라우저를 사용하여 활성화됨](/help/assets/content-fragments/content-fragments-configuration-browser.md)입니다.
   * 은(는) [도구를 사용하여 만들었습니다](/help/assets/content-fragments/content-fragments-models.md).
   ...
   * 모델 정의는 제목과 하나의 데이터 요소를 필요로 합니다. 그 외의 모든 것은 선택 사항입니다.
   * 해당되는 경우 모델은 기본 콘텐츠를 정의할 수 있습니다.
...
* **콘텐츠 조각 구성 요소**
   ...
   * HTML 형식, JSON 형식 또는 둘 다로 조각을 게재하는 데 중요합니다.
...
변형은 작성 환경 및 전달 시에도 콘텐츠 조각에 처리 시간을 추가합니다.

다음 작업을 수행하는 것이 좋습니다.

* 변형 수를 관리 가능한 최소값으로 유지
* 콘텐츠 조각당 10개의 변형을 초과하지 마십시오.
