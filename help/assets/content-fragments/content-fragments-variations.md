...
**컨텐츠 조각 편집** 옵션은 새 편집기 탭에서 해당 조각을 엽니다(동일한 브라우저 탭 내).

원래 탭을 다시 선택합니다(예: **Little Phony Inc.**). 이 보조 탭을 닫습니다(이 경우 **Adam Smith**).
...
**마스터**&#x200B;는 컨텐츠 조각의 필수적인 부분이며, 정의에 따라 컨텐츠의 마스터 사본은 보존되지만 변형에는 해당 컨텐츠의 개별 업데이트 및 맞춤 버전이 포함됩니다. 업데이트기본이 되면 이러한 변경 내용이 변형과 관련성이 있으므로 해당 변경 사항에도 전파되어야 합니다.
