...
컨텐츠 조각은 **자산으로**&#x200B;저장되므로 주로 **자산** 콘솔에서관리됩니다.
...
자산 **콘솔에서** 컨텐츠 조각에 대해 다양한 작업을 사용할 수 있습니다.

* 도구 모음에서; 조각 선택 후 모든 적절한 작업을 사용할 수 있습니다.
* 빠른 [작업](/help/sites-cloud/authoring/getting-started/basic-handling.md#quick-actions); 개별 조각 카드에 사용할 수 있는 작업 하위 집합.

![액션](assets/cfm-managing-02.png)

조각을 선택하여 해당 작업이 있는 도구 모음을 표시합니다.

* **만들기**
* **다운로드**

   * 조각을 ZIP 파일로 저장; 요소, 변형, 메타데이터 포함 여부를 정의할 수 있습니다.
...
* **편집**
   ...
   * 요소, 변형, 관련 컨텐츠 및 메타데이터와 함께 컨텐츠를 [](/help/assets/content-fragments/content-fragments-variations.md) 편집할 조각을 열 수 있습니다.

* **태그 관리**
* **대상 컬렉션**

   * 컬렉션에 조각을 추가합니다.
   * 컬렉션을 조각과 [연결할 때도 이 작업을 수행할 수 있습니다](/help/assets/content-fragments/content-fragments-assoc-content.md#adding-associated-content).
...
>[!NOTE]
>
>이러한 작업 중 대부분은 자산 [및/또는](/help/assets/manage-digital-assets.md) AEM 데스크톱 앱에 대한 표준 [작업입니다](https://helpx.adobe.com/experience-manager/desktop-app/aem-desktop-app.html).
...
>[!CAUTION]
>
>컨텐츠 조각을 편집하려면 적절한 권한 [이 필요합니다](/help/implementing/developing/extending/content-fragments-customizing.md#asset-permissions). 문제가 발생하면 시스템 관리자에게 문의하십시오.
...
2. 다음 방법 중 하나를 사용하여 편집할 조각을 엽니다.
   ...
   * 조각을 선택한 다음 **도구 모음에서** 편집을 선택합니다.
   ...
   >[!NOTE]
   >
   >1. 조각을 컨텐츠 페이지에서 이미 참조하면 메시지가 표시됩니다.
      >
      >
      >
...
4. 변경한 후 필요에 따라 **저장** 또는 **취소를** 사용하십시오.

   >[!NOTE]
   >
   >저장 ********[및](#save-cancel-and-versions) 취소모두 편집기를 종료합니다. 두 옵션 모두 컨텐츠 조각에 대해 작동하는 방법에 대한 자세한 내용은저장, 취소 및 버전을 참조하십시오.
...
* **저장**
   ...
   >[!NOTE]
   >
   >[저장]을 선택하기 전에 편집기에서 일련의 변경 작업을 수행할 수 **있습니다**.

   >[!CAUTION]
   >
   >저장 **은 변경 사항을 저장하는 것 외에도 모든 참조를 업데이트하고** 필요에 따라 디스패처가 플러시되도록 합니다. 이러한 변경 사항을 처리하는 데 시간이 걸릴 수 있습니다. 이로 인해 대형/복잡한/과도하게 로드된 시스템에 성능에 영향을 줄 수 있습니다.
   >
   >
   >저장을 사용할 때 **이** 사항을 염두에 두고 조각 편집기를 신속하게 다시 입력하여 추가 변경 사항을 만들고 저장합니다.

* **취소**

   최신 변경 내용을 저장하지 않고 편집기를 종료합니다.

컨텐츠 조각을 편집하는 동안 AEM은 변경 사항을 **취소할** 경우 이전 컨텐츠를 복원할 수 있도록 버전을 자동으로 생성합니다.

1. AEM을 편집하기 위해 컨텐츠 조각을 열면 *편집 세션이 존재하는지 여부를 나타내는 쿠키 기반 토큰이 있는지 확인합니다* .
...
2. 활성 ** 편집 세션이 있는 동안 편집되는 컨텐츠는 600초마다 자동으로 저장됩니다(기본값).
   ...
   >[!NOTE]
   >
   >자동 저장 간격은 메커니즘을 사용하여 구성할 수 `/conf` 있습니다.
   >
   >기본값은 다음을 참조하십시오.
   >  `/libs/settings/dam/cfm/jcr:content/autoSaveInterval`

3. 사용자가 편집을 **취소하도록** 선택하면 편집 세션이 시작될 때 만들어진 버전이 복원되고 토큰이 제거되어 편집 세션이 종료됩니다.
4. 사용자가 편집 내용을 **저장하도록** 선택하면 업데이트된 요소/변형이 유지되고 토큰이 제거되어 편집 세션이 종료됩니다.

## 조각 컨텐츠 편집 {#editing-the-content-of-your-fragment}

조각을 열면 변형 [탭을 사용하여 컨텐츠를](/help/assets/content-fragments/content-fragments-variations.md) 작성할 수 있습니다.

## 조각 내 변형 생성 및 관리 {#creating-and-managing-variations-within-your-fragment}

마스터 콘텐츠를 만들고 나면 해당 콘텐츠의 [변형을](/help/assets/content-fragments/content-fragments-variations.md) 만들고 관리할 수 있습니다.

## 컨텐츠와 조각 연결 {#associating-content-with-your-fragment}

컨텐츠를 조각 [과](/help/assets/content-fragments/content-fragments-assoc-content.md) 연결할 수도 있습니다. 컨텐츠 페이지에 추가될 때 자산(예: 이미지)을 조각과 함께(선택 사항)사용할 수 있도록 연결을 제공합니다.

## 조각의 메타데이터(속성) 보기 및 편집 {#viewing-and-editing-the-metadata-properties-of-your-fragment}

메타데이터 탭을 사용하여 조각 속성을 보고 편집할 [수](/help/assets/content-fragments/content-fragments-metadata.md) 있습니다.

## 컨텐츠 조각에 대한 타임라인 {#timeline-for-content-fragments}

표준 옵션 외에도 [타임라인은](/help/assets/manage-digital-assets.md#timeline) 컨텐츠 조각에 관련된 정보와 작업을 모두 제공합니다.

* 버전, 주석 및 주석에 대한 정보 보기
* 버전에 대한 작업

   * **[이 버전으로](#reverting-to-a-version)**되돌리기(기존 조각, 특정 버전 선택)

   * **[현재 조각](#comparing-fragment-versions)**비교(기존 조각, 특정 버전 선택)

   * 레이블 **및** /또는 **주석** 추가(기존 조각, 특정 버전 선택)

   * **버전으로** 저장(기존 조각 선택, 타임라인 하단의 위쪽 화살표)
...
주석(컨텐츠 조각)은 다음과 같습니다.
* 조각 편집기에 입력됨
* 조각 내의 선택한 텍스트 세그먼트에 특정할 수 있습니다.
...
현재 **와** 비교 작업은 특정 버전을 선택한 후 [타임라인에서](/help/assets/content-fragments/content-fragments-managing.md#timeline-for-content-fragments) 사용할 수 있습니다.
...
>[!NOTE]
조각 비교 시 조각 컨텐츠를 편집할 수 없습니다.

![비교](assets/cfm-managing-06.png)

## Reverting to a Version  {#reverting-to-a-version}

조각 특정 버전으로 되돌릴 수 있습니다.

* 타임라인에서 바로 [액세스할 수 있습니다](/help/assets/content-fragments/content-fragments-managing.md#timeline-for-content-fragments).
...
>[!CAUTION]
조각이 모델을 기반으로 하는 경우 [모델이 게시되었는지 확인해야 합니다](/help/assets/content-fragments/content-fragments-models.md#publishing-a-content-fragment-model).
모델이 아직 게시되지 않은 컨텐츠 조각을 게시하는 경우 선택 목록에 이것이 표시되고 모델이 조각과 함께 게시됩니다.

게시 환경에서 사용하려면 컨텐츠 조각을 게시해야 합니다. 게시할 수 있습니다.

* 제작 후; 를 **클릭합니다** .
* 조각을 [사용하는 페이지를 게시할 때](/help/sites-cloud/authoring/fundamentals/content-fragments.md#publishing); 페이지 참조에 조각이 나열됩니다.
...
4. 삭제 **작업을** 확인합니다.
   ...
   >[!CAUTION]
   조각을 페이지에서 이미 참조한 경우 경고 메시지가 표시되고 강제 삭제 **로 진행할지 확인해야 합니다**. 해당 컨텐츠 조각 구성 요소와 함께 조각이 모든 컨텐츠 페이지에서 삭제됩니다.
