---
title: Dynamic Media Open API와 콘텐츠 관리자 통합
description: Content Advisor를 다양한 Adobe, Adobe 이외 애플리케이션 및 타사 애플리케이션과 통합합니다.
role: Admin, User
...
# OpenAPI 기능과 Dynamic Media 통합 {#integrate-dynamic-media-open-apis}

Content Advisor를 사용하면 다양한 Adobe 애플리케이션을 통합하여 원활하게 연동할 수 있습니다.

## 사전 요구 사항 {#prereqs-polaris}

Content Advisor를 Dynamic Media와 OpenAPI 기능을 통합하는 경우 다음 사전 요구 사항을 사용하십시오.

* [커뮤니케이션 방법](/help/assets/content-advisor-properties.md#prereqs)
...
Dynamic Media OpenAPI 프로세스와 컨텐츠 권고자를 통합하려면 사용자 지정된 Dynamic Media URL을 만들거나 Dynamic Media URL을 선택할 준비가 된 경우 등 다양한 단계가 필요합니다.

### OpenAPI 기능과 Dynamic Media용 Content Advisor 통합 {#integrate-dynamic-media}
...
이 구성을 사용하면 폴더 없이 또는 플랫 구조로 승인된 모든 에셋을 볼 수 있습니다. 자세한 내용을 보려면 [콘텐츠 관리자 속성](/help/assets/content-advisor-properties.md)의 `aemTierType` 속성으로 이동하십시오.


### 승인된 자산에서 동적 게재 URL 만들기 {#create-dynamic-media-url}

콘텐츠 관리자를 설정하면 선택한 에셋에서 동적 게재 URL을 만드는 데 오브젝트 스키마가 사용됩니다.
...
OpenAPI 기능이 있는 Dynamic Media용 콘텐츠 관리자를 사용하면 사용자 지정 속성 및 이를 기반으로 필터를 구성할 수 있습니다. `filterSchema` 속성은 이러한 속성을 구성하는 데 사용됩니다. 사용자 지정은 필터를 구성할 수 있는 `metadata.<metadata bucket>.<property name>.`(으)로 노출될 수 있습니다.
...
예를 들어 OpenAPI 기능이 있는 Dynamic Media용 콘텐츠 관리자에서 `asset jcr:content/metadata/client_name:market`의 속성이 필터 구성을 위해 `metadata.embedded.client_name:market`(으)로 변환됩니다.
