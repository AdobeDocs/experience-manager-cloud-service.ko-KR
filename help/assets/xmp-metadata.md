...
AEM Assets는 자산 이진에서 읽고 자산을 인제스트할 때 JCR에 저장된 XMP 메타데이터에 대한 속성/노드의 블랙 리스트 및 화이트 리스트 필터링을 모두 지원합니다.

블랙 리스트 필터링을 사용하면 제외에 지정된 속성을 제외한 모든 XMP 메타데이터 속성을 가져올 수 있습니다. 하지만 대량의 XMP 메타데이터가 있는 INDD 파일(예: 1000개의 노드가 10,000개 속성)과 같은 자산 유형의 경우 필터링할 노드 이름이 미리 알려지는 것은 아닙니다. 블랙 리스트 필터링을 통해 많은 XMP 메타데이터가 포함된 많은 자산을 가져올 수 있는 경우, AEM 인스턴스/클러스터에는 안정성 문제가 발생할 수 있습니다(예: 끊어진 관측 큐).

가져올 XMP 속성을 정의할 수 있어 XMP 메타데이터를 화이트 리스트 필터링하면 이 문제가 해결됩니다. 이렇게 하면 기타/알 수 없는 XMP 속성이 무시됩니다. 이러한 속성 중 일부를 이전 버전과의 호환성을 위해 블랙 리스트 필터에 추가할 수 있습니다.
...
1. 화이트 리스트 필터링을 적용하려면 **[!UICONTROL XMP 속성에 허용 목록 적용을]**&#x200B;선택하고 XMP 필터링을 위한 **[!UICONTROL 허용 목록 XML 이름 상자에 가져올 속성을]** 지정합니다.

1. 화이트 리스트 필터링을 적용한 후 블랙리스트에 추가된 XMP 속성을 필터링하려면 XMP 필터링을 위한 블랙리스트에 **[!UICONTROL 있는 XML 이름 상자에 해당 속성을]** 지정합니다.

   >[!NOTE]
   >
   >[ **[!UICONTROL XMP 속성에 블랙 리스트 적용]** ] 옵션이 기본적으로 선택되어 있습니다. 다시 말해 블랙 리스트 필터링은 기본적으로 활성화되어 있습니다. 블랙 리스트 필터링을 비활성화하려면 [XMP 속성에 **[!UICONTROL 블랙 리스트 적용] 옵션을 선택]** 취소합니다.
