...
XMP defines a [metadata](https://en.wikipedia.org/wiki/Metadata) model that can be used with any defined set of metadata items. XMP는 리소스가 여러 처리 단계를 거치는 동안(사진 촬영, [스캔](https://en.wikipedia.org/wiki/Image_scanner), 텍스트 작성, [자르기](https://en.wikipedia.org/wiki/Cropping_%28image%29) 또는 색상 조정과 같은 사진 편집 단계, 최종 이미지 조합까지) 그 이력을 기록하는 데 유용한 기본 속성을 위한 특정 [스키마](https://en.wikipedia.org/wiki/XML_schema)도 정의합니다. XMP allows each software program or device along the way to add its own information to a digital resource, which can then be retained in the final digital file.
...
이 경우 [!DNL Assets]은(는) 자산 계층 구조에 저장된 자산 메타데이터의 **[!UICONTROL 매개 변수에]** Title`dc:title` 속성의 변경 내용을 저장합니다.
