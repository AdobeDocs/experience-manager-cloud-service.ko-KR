...
* [Adobe AI](https://business.adobe.com/ai/adobe-genai.html)에서 제공하는 인공 지능을 사용하여 업로드된 모든 에셋에 관련 태그를 자동으로 적용합니다. 스마트 태그라고 명명된 이들 태그는 관련 에셋을 빠르게 찾을 수 있도록 지원하여 프로젝트의 콘텐츠 속도를 높입니다.
...
### AEM Assets Content Hub에 업로드할 수 있는 에셋 유형은 무엇이며 어디에서 사용할 수 있습니까? {#asset-types-upload-to-content-hub}

에셋을 추가할 수 있는 권한이 있는 AEM Assets Content Hub 사용자는 로컬 파일 시스템에서 브랜드로 승인된 에셋을 업로드할 수 있습니다. 원래 폴더 구조에 관계없이 업로드된 모든 에셋은 Content Hub의 최상위 수준에 표시되어 검색 기능을 향상시킬 수 있습니다.

### AEM Assets Content Hub은 자산 검색 및 구성을 어떻게 향상시킵니까? {#search-content-hub}

AEM Assets Content Hub은 사용자가 캠페인 이름, 키워드, 채널, 시간대 및 지역과 같은 각 업로드에 대한 주요 세부 정보를 정의할 수 있도록 하여 자산 검색 및 구성을 강화합니다. 또한 각 에셋에 대한 추가 속성(파일 크기, 형식 및 해상도 등)을 자동으로 생성하고 Adobe AI을 사용하여 스마트 태그를 적용하므로 관련 에셋을 보다 쉽고 빠르게 찾을 수 있습니다.

### 로컬 파일 시스템에서 AEM Assets Content Hub으로 에셋을 업로드하는 방법 {#upload-assets-content-hub}

로컬 파일 시스템에서 AEM Assets Content Hub으로 자산을 업로드하려면 **Assets 추가**&#x200B;를 클릭하여 업로드 대화 상자를 엽니다. 파일이나 폴더를 드래그 앤 드롭하거나 수동으로 검색하여 선택할 수 있습니다. 자산을 캠페인 이름으로 그룹화해야 하며, 더 나은 조직을 위해 키워드, 채널, 일정 및 지역과 같은 다른 필드를 채우는 것이 좋습니다. 준비가 되면 **업로드**&#x200B;를 클릭하고 세부 정보를 검토한 후 업로드를 시작하는지 확인하십시오.
...
### AEM Assets Content Hub에 자산을 업로드할 때 필수 또는 선택 사항인 필드를 구성할 수 있습니까? {#available-fields-while-uploading-assets-to-content-hub}

관리자는 구성 사용자 인터페이스를 사용하여 AEM Assets Content Hub에 에셋을 업로드하는 동안 필수 또는 선택 사항인 필드를 정의할 수 있습니다.

### 업로드한 에셋이 AEM Assets Content Hub에 자동으로 표시되지 않는 경우 어떻게 해야 합니까? {#assets-do-not-display-in-content-hub}

AEM Assets Content Hub에 자산이 자동으로 표시되지 않으면 자동 승인 전환이 비활성화되는 것입니다. 자산이 Assets as a Cloud Service 환경의 **하이드레이션된 자산** 폴더에 있습니다. Content Hub에 표시하려면 해당 상태를 **승인됨**(으)로 일괄 편집해야 합니다.
