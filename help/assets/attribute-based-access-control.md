...
>[!IMPORTANT]
>
>[AEM 거버넌스 에이전트](/help/ai-in-aem/agents/governance/overview.md#permission-management-in-content-hub)를 사용하여 특성 기반 액세스 제어 규칙을 설정할 수도 있습니다.
...
AEM Assets Content Hub의 속성 기반 액세스 제어(ABAC)를 사용하면 관리자가 메타데이터 기반 규칙을 정의하여 다양한 사용자 그룹이 디지털 에셋에 액세스하는 수준을 제어할 수 있습니다. 액세스는 에셋의 메타데이터가 규칙에 지정된 조건과 일치하는지 여부에 따라 결정되므로 에셋 가시성을 세부적이고 동적으로 관리할 수 있습니다.

### 관리자는 AEM Assets Content Hub에서 ABAC를 사용하여 액세스 규칙을 어떻게 정의합니까?

관리자는 브랜드 또는 지역과 같은 에셋 메타데이터를 기반으로 조건을 만들고 이를 특정 사용자 그룹 ID에 연결하여 액세스 규칙을 정의합니다. 이러한 규칙은 논리적 (AND, OR) 및 비교 (equals, not equals) 연산자를 사용하여 사용자 그룹에 표시되는 에셋을 정확히 지정합니다.
...
아니요. 현재로서는 관리자가 Content Hub 인터페이스에서 직접 ABAC 규칙을 만들 수 없습니다. 대신 템플릿 스프레드시트(이 문서에 제공된 다운로드 링크)를 다운로드하고 여기에서 규칙을 정의한 다음 구현을 위해 지원 티켓을 통해 Adobe 지원에 제출해야 합니다.

### AEM Assets Content Hub에서 ABAC 규칙을 설정할 때 사용할 수 있는 메타데이터 조건 유형은 무엇입니까?

AEM Assets Content Hub의 ABAC 규칙은 AND 및 OR와 같은 논리 연산자와 같음 및 같지 않음 과 같은 비교 연산자를 사용할 수 있습니다. 규칙에 사용되는 메타데이터 속성은 AEM 메타데이터 스키마에서 올바르게 정의되어야 하며 사용할 수 있어야 하며 지역, 브랜드 또는 게시 상태와 같은 필드를 포함할 수 있습니다.

### AEM Assets Content Hub ABAC가 대규모 팀과 다양한 자산 요구 사항을 가진 조직에 특히 유용한 이유는 무엇입니까?

ABAC는 사용자 역할, 지역 또는 브랜드를 기반으로 에셋에 대한 세분화된 규칙 기반 액세스를 가능하게 하므로 대규모 팀이 있는 조직에 유용합니다. 수동 권한 할당이나 자산의 과도한 중복 없이 사용자가 책임과 관련된 자산만 볼 수 있도록 합니다.

### 관리자는 Adobe 지원 센터에 제출하기 전에 AEM Assets Content Hub용 ABAC 스프레드시트를 어떻게 준비해야 합니까?

관리자는 Adobe Admin Console에서 사용자 그룹을 만들고, 그룹 ID를 기록하고, 스프레드시트에서 각 그룹에 대한 권한 및 조건을 명확히 정의해야 합니다. 모든 메타데이터 속성이 적절한 스키마에 올바르게 매핑되는지 확인하고 설명 열을 사용하여 각 규칙의 비즈니스 의도를 명확히 해야 Adobe에서 규칙을 보다 쉽게 확인하고 구현할 수 있습니다.
