...
Adobe 개발자 콘솔에서 프로젝트 및 API는 조직(브랜드 포털 테넌트) 수준에서 구성됩니다. API를 구성하면 Adobe 개발자 콘솔에서 JWT(서비스 계정) 연결이 만들어집니다. 키 쌍(개인 및 공개 키)을 생성하거나 공개 키를 업로드하여 API를 구성하는 두 가지 방법이 있습니다. 브랜드 포털에서 AEM Assets 클라우드 인스턴스를 구성하려면 AEM Assets 클라우드 인스턴스에서 공개 인증서(공개 키)를 생성하고 공개 키를 업로드하여 Adobe 개발자 콘솔에서 자격 증명을 만들어야 합니다. 이 공개 키는 선택한 브랜드 포털 조직에 대한 API를 구성하는 데 사용되며 서비스 계정에 대한 자격 증명과 JWT 페이로드를 생성합니다. 이러한 자격 증명은 AEM Assets 클라우드 인스턴스에서 IMS 계정을 구성하는 데 추가로 사용됩니다. IMS 계정이 구성되면 AEM Assets 클라우드 인스턴스에서 브랜드 포털 클라우드 서비스를 구성할 수 있습니다.
