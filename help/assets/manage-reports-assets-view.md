...
description: Assets 보기의 보고서 섹션에 있는 데이터에 액세스하여 제품 및 기능 사용을 평가하고 주요 성공 지표에 대한 인사이트를 도출합니다.
...
AEM Assets 보기 환경은 보고서 대시보드를 통해 포괄적인 보고 기능을 제공합니다. 이 기능을 통해 사용자는 지정된 기간(일회성부터 매일, 매주, 매월 또는 매년까지) 내에서 에셋 업로드 및 다운로드에 대한 세부 정보를 보여 주는 CSV 보고서를 생성하고 다운로드할 수 있습니다.
...
1. **보고서**&#x200B;로 이동하고 **보고서 만들기**(오른쪽 상단)를 클릭합니다. **보고서 만들기** 대화 상자에 아래와 같은 필드가 표시됩니다.
   ...
   **구성 탭:**
   ...
   1. **폴더 경로 선택:** 특정 폴더 내에서 업로드 및 다운로드된 에셋에 대한 보고서를 생성할 폴더 경로를 선택합니다. 예를 들어 폴더에 업로드된 에셋에 대한 보고서가 필요하다면 해당 폴더의 경로를 지정합니다.
...
>* 보고서는 처음 1백만 행에 대해 생성됩니다. 이 제한 내에 있는 모든 파일을 캡처하려면 더 작은 폴더의 레퍼러 열을 포함하는 것이 좋습니다.
...
1. 보고서 대화 상자에 아래와 같은 정보가 표시됩니다.
   ...
   1. **제목:** 보고서에 제목을 추가합니다.
   1. **설명**: 원하는 경우 보고서에 설명을 추가합니다.
   1. **폴더 경로 선택:** 나중에 특정 폴더에 업로드되거나 해당 폴더에서 다운로드될 에셋에 대한 보고서를 생성하려면 폴더 경로를 선택합니다.
...
* **크기별 자산 수:** 자산 보기 환경의 총 자산 수를 다양한 크기 범위로 세그먼트화하고 도넛 차트로 표시되는 각 크기 범위의 자산 수와 비율을 강조 표시합니다.
  ![인사이트-크기별-에셋-수](/help/assets/assets/insights-assets-count-by-size.svg)
* **자산 유형별 자산 수:** Assets 보기 환경의 총 자산 수를 세그먼트화하며, 파일 유형에 따라 자산의 수와 비율을 강조 표시합니다. 도넛 차트로 표시됩니다.
  ![인사이트-크기별-에셋-수](/help/assets/assets/insights-assest-count-by-asset-type1.svg)