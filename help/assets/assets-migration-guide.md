...
실제로 마이그레이션 단계를 수행하기 전에 성능 조정 지침을 검토하고 구현합니다. 최대 동시 작업 구성과 같은 많은 단계는 로드 중인 서버의 안정성과 성능을 크게 향상시킵니다. 파일 데이터 저장소 구성과 같은 다른 단계는 시스템이 에셋으로 로드된 후 수행하는 것이 훨씬 더 어렵습니다.
...
>
>
이러한 소프트웨어는 오픈 소스이며 Apache [v2 라이선스의](https://adobe-consulting-services.github.io/pages/license.html)적용을 받습니다. 질문하거나 문제를 보고하려면 ACS AEM 도구 [및 ACS AEM](https://github.com/Adobe-Consulting-Services/acs-aem-commons/issues) Commons에 대한 해당 GitHub 문제를 [참조하십시오](https://github.com/Adobe-Consulting-Services/acs-aem-tools/issues).
...
ACS [AEM 도구 CSV 자산 가져오기는](https://adobe-consulting-services.github.io/acs-aem-tools/features/csv-asset-importer/index.html) 자산을 가져오기 위해 파일 시스템에서 자산 및 CSV 파일에서 자산 메타데이터를 가져옵니다. AEM Asset Manager API는 자산을 시스템으로 가져오고 구성된 메타데이터 속성을 적용하는 데 사용됩니다. 에셋은 네트워크 파일 마운트 또는 외부 드라이브를 통해 서버에 마운트하는 것이 좋습니다.
...
1. Adobe 자산과 함께 변환을 마이그레이션하기 때문에 DAM Update 자산에 대한 워크플로우 런처를 비활성화할 수 있습니다.
...
1. 지침에 따라 AEM으로의 초기 마이그레이션에 대해 문서화된 자산을 [](#activating-assets) 활성화합니다.

1. 새 마이그레이션과 마찬가지로, 단일 게시 인스턴스를 로드하고 복제하는 것이 두 노드에서 컨텐츠를 활성화하는 것보다 더 효율적입니다. 게시 [복제를 참조하십시오.](#cloning-publish)

1. 마이그레이션을 완료한 후 DAM Update Asset 워크플로우용 방사선을 다시 활성화하여 지속적인 시스템 사용을 위해 변환 생성 및 메타데이터 추출을 지원합니다.
