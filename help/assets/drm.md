...
>[!NOTE]
>
>위치 `/etc/dam/drm/licences`은(는) [!DNL Experience Manager]의 이전 릴리스에서 라이선스를 저장하는 데 사용되었습니다. 해당 위치는 이제 더 이상 사용되지 않습니다. 라이선스 페이지를 만들거나 수정하거나 이전 [!DNL Experience Manager] 릴리스의 페이지를 포트하는 경우 `/apps/settings/dam/drm/licenses` 또는 `/conf/*/settings/dam/drm/licenses` 위치에 이러한 자산을 저장하는 것이 좋습니다.
