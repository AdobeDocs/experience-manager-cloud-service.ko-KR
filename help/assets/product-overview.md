...
## AEM Assets Content Hub을 선택해야 하는 이유

AEM Assets Content Hub은 다음과 같은 주요 이점을 제공합니다.
...
## AEM Assets Content Hub에 액세스하는 방법 {#access-content-hub}
...
1. [Experience Manager Assets as a DAM에서 작성자 또는 관리자가 승인한 자산](approve-assets.md)

1. [관리자는 다른 사용자를 위해 Content Hub 사용자 인터페이스 구성할 수 있습니다](configure-content-hub-ui-options.md).
...
AEM Assets Content Hub은 보다 광범위한 팀이 직관적인 포털을 통해 관련성이 있고 승인된 자산을 쉽게 검색하고 이러한 자산을 빠르게 요구 사항에 맞게 조정할 수 있는 Adobe Experience Manager as a Cloud Service 기능입니다. 규모에 맞게 에셋을 배포할 수 있고 온브랜드 콘텐츠 변형을 만들어 마케팅 민첩성을 향상시킵니다.

### AEM Assets Content Hub에 액세스하기 위한 사전 요구 사항은 무엇입니까? {#prerequisites-for-content-hub}

Content Hub에는 Experience Manager as a Cloud Service의 프로덕션 작성 환경, 특히 2024.6 릴리스 이상(최소 버전 2024.6.16799)이 필요합니다.

### AEM Assets Content Hub은 브랜드 승인 에셋에 대한 검색 경험을 어떻게 개선합니까? {#content-hub-improves-search-experience}

AEM Assets Content Hub은 모든 승인된 에셋을 단일 계층 구조로 표시하므로, 직관적인 포털을 통해 브랜드에서 승인한 에셋을 보다 쉽게 찾고 공유할 수 있습니다. 이 설정을 통해 검색 프로세스를 간소화하고 사용자가 필요한 에셋을 효율적으로 찾을 수 있습니다.

### AEM Assets Content Hub 사용자 인터페이스를 구성할 수 있는 사용자는 누구이며 어떤 측면을 구성할 수 있습니까? {#content-hub-configuration}

관리자는 검색을 위한 필터, 에셋을 추가하거나 가져오기 위한 필드, 에셋 속성 및 브랜딩을 위한 배너 콘텐츠를 포함하여 AEM Assets Content Hub 사용자 인터페이스를 구성할 수 있습니다. 이를 통해 조직 요구 사항을 기반으로 맞춤화를 수행할 수 있습니다.

### AEM Assets Content Hub은 크리에이티브가 아닌 사용자에게 콘텐츠를 편집하고 재혼합하는 권한을 어떻게 부여합니까? {#content-hub-edit-remix-content}
...
### 사용자가 AEM Assets Content Hub에 액세스하려면 어떻게 합니까? {#content-hub-access}

사용자는 직접 링크(https://experience.adobe.com/#/assets/contenthub)를 통해 또는 experience.adobe.com에 로그인하고 빠른 액세스 섹션에서 AEM Assets Content Hub을 선택하여 Experience Manager Assets Content Hub에 액세스할 수 있습니다.
