...
자산 게재 응답에는 다운스트림 캐싱 계층에 대해 **10분**&#x200B;의 기본 `max-age`을(를) 사용하는 `Cache-Control` 헤더가 포함됩니다. 이는 사용자 지정 *BYOCDN(Bring-Your-Own-CDN) 구성*, *최종 사용자 브라우저* 및 *중간 캐싱 프록시*&#x200B;에 적용되므로 전체 게재 경로에서 일관된 캐시 제어를 보장합니다.
