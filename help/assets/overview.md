...
AEM Assets as a Cloud Service은 기업이 디지털 자산 관리 및 Dynamic Media 작업을 수행할 뿐만 아니라 AI/ML과 같은 차세대 스마트 기능을 사용할 수 있도록 클라우드 기반 PaaS 솔루션을 제공합니다. 항상 최신이고 항상 이용 가능하며 항상 학습하는 시스템 내에서 모든 것이 이루어집니다.
...
* Adobe Asset Link를 사용하면 Adobe Photoshop, Adobe Illustrator 및 Adobe InDesign 데스크탑 애플리케이션에서 Experience Manager의 자산에 액세스할 수 있습니다. 열려 있는 문서를 Experience Manager에 업로드할 수 있습니다. 이러한 데스크탑 애플리케이션에서 찾을 수 있는 Adobe Asset Link 인터페이스를 통해 직접 작업할 수 있습니다.
...
**스마트 태그**: 스마트 태그는 Adobe AI의 인공 지능 프레임워크를 사용하여 태그 구조 및 비즈니스 분류에 대한 이미지 인식 알고리즘을 교육합니다. 이 콘텐츠 인텔리전스는 다른 자산 세트에 관련 태그를 적용하는 데 사용됩니다. AEM은 기본적으로 업로드된 자산에 스마트 태그를 자동 적용합니다.

**지능형 색상 기반 태그 지정 및 검색**: AEM Assets은 Adobe AI 기능을 사용하여 이미지의 색상을 구분하고 수집 시 이러한 트레이트를 자동으로 태그로 적용합니다. 이러한 태그를 통해 이미지 색상 컴포지션에 따라 향상된 검색 경험이 가능합니다.
...
<td>
   ...
   <p>
      <em>업로드된 자산에 스마트 태그를 자동으로 적용하는 방법에 대해 알아봅니다.</em>
   </p>
</td>
...
**상황별 검색**: AEM Assets에서 텍스트 프롬프트를 정의하여 저장소에서 사용 가능한 자산을 검색할 수 있습니다. Experience Manager Assets는 해당 텍스트 프롬프트를 검색 필터로 자동 변환하고 검색 결과를 표시합니다. 필터 창에서 자동 필터를 확인하여 수정하고 검색 결과의 범위를 더 좁힐 수 있습니다. 대화형 텍스트 프롬프트의 예는 다음과 같습니다.

* *해변과 맑은 하늘이 포함된 높이가 200px 이상, 너비가 100px 이상인 이미지*
* *높이가 1500 및 2500픽셀이고 지난 달에 생성되었으며 만료 및 승인되지 않은 푸른 하늘 이미지가 필요합니다*.
...
**스마트 자르기**: 모든 이미지 또는 비디오에서 자동으로 초점을 감지하고 자르기 유지 기능을 제공하는 Adobe AI 기능입니다. 화면 크기에 상관없이 원하는 관심 지점을 포착하여 번거로운 수작업을 할 필요가 없고, 어떤 디바이스나 화면에서도 보기 좋은 고품질로 빠르게 로딩되는 이미지와 비디오를 제공합니다.
...
<td>
   ...
   <p>
      <em>AI를 사용하여 비디오 콘텐츠에 자동으로 캡션을 생성합니다. </em>
   </p>
</td>
</table>
...
AEM Assets는 적절한 자산을 신속하게 찾을 수 있도록 도와주는 기능을 제공합니다. 이러한 기능에는 AI 생성 태그 지정(스마트 태그), 맞춤형 메타데이터, 향상된 검색 기능이 포함됩니다.
...
<td>
   ...
   <p>
      <em>AEM 사용자가 기본에서 고급 수준의 검색을 수행하는 데 도움이 되는 다양한 시나리오에 대해 알아봅니다.</em>
   </p>
</td>
</table>
...
**내 작업 영역**: Assets 보기에는 위젯을 제공하는 사용자 정의 작업 영역도 포함되어 있습니다. 이러한 위젯을 사용하면 자산 사용자 인터페이스의 주요 영역에 편리하게 액세스할 수 있으며 사용자에게 가장 관련성이 높은 정보에 액세스할 수 있습니다. 이 페이지는 작업 항목에 대한 개요 및 주요 워크플로에 대한 바로 가기를 제공하는 종합적인 솔루션 역할을 합니다.
...
<td>
   ...
   <p>
      <em>내 작업 영역을 사용하여 Assets 사용자 인터페이스의 주요 영역에 빠르게 액세스하는 방법에 대해 알아봅니다.</em>
   </p>
</td>
<td>
...
**알림**: Assets 보기 알림을 사용하여 저장소에서 사용할 수 있는 자산, 폴더 또는 컬렉션에 수행된 작업을 모니터링할 수 있습니다. 알림을 받을 콘텐츠를 선택하고 구독해야 합니다. 알림을 받을 카테고리를 구성할 수도 있습니다.
...
* **AEM Assets를 Creative Cloud에 연결**: Experience Manager Assets는 다른 IMS 조직에서 제공된 Creative Cloud 권한과 연결할 수 있습니다. 이 기능을 사용하면 Express 및 Creative Cloud Libraries를 포함하여 AEM Assets에서 최신 Creative Cloud 통합을 사용할 수 있습니다. Creative Cloud 제품 및 AEM Assets가 별도의 IMS 조직에 프로비저닝된 경우 다른 Creative Cloud 조직에 연결하여 두 솔루션 간의 통합 워크플로를 실행할 수 있습니다.
...
<td>
   ...
   <p>
      <em>AEM Assets를 문서 기반 및 범용 편집기 기반 작성 흐름과 통합합니다.</em>
   </p>
</td>
</table>
...
AEM as a Cloud Service은 콘텐츠 관리, 최적화 및 거버넌스를 향상시킬 수 있는 지능형 **에이전트**&#x200B;를 제공합니다. 이러한 에이전트를 통해 사용자는 콘텐츠를 신속하게 검색하고, 캠페인을 최적화하고, 디지털 자산 전반에서 규정 준수를 보장할 수 있습니다.

**검색 에이전트**

검색 에이전트는 간소화되고 클릭되지 않는 검색 환경을 위해 자연스럽고 대화식 프롬프트를 통해 AEM 콘텐츠를 온디맨드로 제공합니다. **Assets, 콘텐츠 조각 및 적응형 Forms**&#x200B;에서 지능적으로 검색하여 이미지, 비디오, PDF, 문서 및 양식 템플릿과 같은 관련 콘텐츠를 제공합니다. 자연어를 사용하면 AEM Assets 인터페이스에서 복잡한 쿼리를 작성하거나 필터를 적용하지 않고 검색할 수 있습니다. 에이전트는 프롬프트에 따라 조정된 결과를 에셋 메타데이터 및 게재 URL과 함께 반환하여 다른 애플리케이션에 임베드할 수 있도록 준비됩니다.

검색 에이전트의 몇 가지 주요 이점은 다음과 같습니다.

* **통합 콘텐츠 검색:** 단일 대화 인터페이스에서 이미지, 비디오, PDF 문서, 문서, 양식 등 모든 유형의 AEM 콘텐츠에 액세스합니다.
...
* **일관된 콘텐츠 사용률:** 승인된 에셋과 조각을 재사용하여 채널 전반에서 브랜드 일관성을 유지합니다.

**기술:** 자연어 콘텐츠 검색, 태그 기반 에셋 검색, 폴더 기반 콘텐츠 검색, 형식 및 방향 기반 에셋 검색\
**담당자:** 캠페인 관리자, 채널 마케터, DAM 사서, 에이전시 및 파트너\
AEM에서 AI Assistant를 통해 **액세스:**

**일반적인 사용 사례/샘플 프롬프트:**

* WKND 폴더에 &quot;office&quot;라는 태그가 지정된 이미지를 표시합니다.
* WKND 음료에 대해 게시된 모든 콘텐츠 조각을 나열합니다.
* 작업에 적용할 양식을 찾습니다.
* 가로 방향으로 사용자와 에셋을 표시합니다.

**콘텐츠 최적화 에이전트**

**콘텐츠 최적화 에이전트**&#x200B;는 자연어 프롬프트를 사용하여 자산을 세분화하고 조정하는 데 도움이 됩니다. 새로운 렌디션을 생성하고, 시각적 효과를 조정하고, 배경을 변경하고, 채널 준비가 가능한 변형을 자동으로 만들 수 있습니다. 원활한 최적화를 위해 검색 에이전트 및 OpenAPI가 있는 **Dynamic Media**&#x200B;와(과) 함께 작동합니다.

**주요 이점:**

* **간편한 자산 변환:** 이미지 크기 조정, 선명하게 하기, 다시 칠하기 또는 미러 이미지.
* **채널에 최적화된 출력:** Instagram, 웹 배너 및 기타 마케팅 채널에 대한 렌디션을 생성합니다.
* **대규모 Creative 개선 사항:** 대규모 워크플로우에 대한 배경 변경 사항 또는 오버레이를 적용합니다.

AEM의 AI Assistant를 통해 **액세스:**.
...
**제한 사항:** 일부 최적화는 PNG 자산에 대해 지원되지 않습니다.

**거버넌스 에이전트**

거버넌스 에이전트는 AEM 콘텐츠 전반에서 규정 준수, 브랜드 일관성 및 정책 시행을 보장하는 데 도움이 됩니다. 메타데이터, 접근성 또는 기업 지침을 충족하지 않는 컨텐츠를 식별합니다.

거버넌스 에이전트의 몇 가지 주요 이점은 다음과 같습니다.

* **준수 모니터링:** 콘텐츠에서 정책 위반을 감지했습니다.
* **메타데이터 적용:** 자산에 거버넌스에 필요한 메타데이터가 있는지 확인합니다.
* **브랜드 일관성:** 회사 표준을 충족하지 않는 콘텐츠에 플래그를 지정합니다.

**기술:** 정책 준수 확인, 메타데이터 유효성 검사, 접근성 감사, 위반에 대한 자동 경고\
**담당자:** DAM 관리자, 규정 준수 책임자, 브랜드 관리자\
AEM AI Assistant를 통해 **액세스:**

**일반적인 사용 사례/샘플 프롬프트:**

* WKND 폴더의 모든 에셋에 대한 메타데이터의 유효성을 검사합니다.
* 브랜드 지침이 누락된 자산을 식별합니다.
* 게시된 콘텐츠를 감사하여 접근성 규정을 준수하십시오.
...
<td>
   ...
   <img alt="검색 에이전트 개요" src="./assets/discovery-agent-overview.jpeg" />
   </a>
   <div>
      <a href="https://experienceleague.adobe.com/ko/docs/experience-manager-cloud-service/content/ai-in-aem/agents/discovery/overview">
   ...
   <p>
      <em>검색 에이전트 및 대화형 콘텐츠 검색 기능에 대한 개요</em>
   </p>
</td>
...
<td>
   ...
   <p>
      <em>콘텐츠 최적화 에이전트 및 지원되는 최적화 워크플로에 대한 개요</em>
   </p>
</td>
...
<td>
   ...
   <p>
      <em>준수 및 정책 적용을 위한 거버넌스 에이전트 개요</em>
   </p>
</td>
</table>

### **AEM에서 에이전트에 액세스하는 방법**

에이전트는 AEM Cloud Service의 **AI Assistant**&#x200B;를 통해 액세스할 수 있습니다. [experience.adobe.com](https://experience.adobe.com/)에 로그인하고 자연어 프롬프트를 사용하여 AI Assistant와 상호 작용합니다.
...
* **비-크리에이티브 사용자 또한 브랜드를 유지하면서 콘텐츠를 편집하고 리믹스할 수 있도록 지원**: Content Hub를 사용하여 Adobe Express로 새 콘텐츠를 만들 수 있습니다(Adobe Express 권한이 있는 경우). 사용하기 쉬운 도구로 기존 콘텐츠를 편집하고, 템플릿과 브랜드 요소로 브랜드에 맞는 변형을 만들고, Adobe Firefly의 최신 생성형 AI 기능으로 새로운 콘텐츠를 만들 수 있습니다.
...
* **스마트 자르기**: 모든 이미지 또는 비디오에서 초점을 자동으로 감지하고 자르기 방식으로 유지하는 Adobe AI 기능입니다. 화면 크기에 상관없이 원하는 관심 지점을 포착하여 번거로운 수작업을 할 필요가 없고, 어떤 디바이스나 화면에서도 보기 좋은 고품질로 빠르게 로딩되는 이미지와 비디오를 제공합니다.
...
* **웹에 최적화된 게재**: 디지털 자산은 웹에 최적화된 포맷으로 제공되어 디지털 경험의 핵심 웹 바이탈을 향상시킵니다. 이 최적화에는 이미지에 대한 WebP 렌디션 지원, 비디오에 대한 HLS 또는 DASH 프로토콜을 통한 적응형 스트리밍, 문서에 대한 원본 렌디션이 포함됩니다.

* **동적 자산 변환**: 해당 시스템은 이미지 수정자로 알려진 URL 매개변수를 사용하여 즉석에서 이미지를 변환할 수 있습니다. [예: 폭, 높이, 회전, 뒤집기, 품질, 자르기, 포맷 및 스마트 자르기](/help/assets/deliver-assets-apis.md). 변환된 렌디션은 동적으로 생성되며 CDN을 통해 원활하게 게재됩니다.
...
자산 보고는 관리자가 Adobe Experience Manager Assets 보기 환경의 활동을 조회할 수 있도록 해 줍니다. 이 데이터는 사용자가 콘텐츠 및 제품과 상호 작용하는 방법에 대한 유용한 정보를 제공합니다. 모든 사용자는 인사이트 대시보드에 액세스할 수 있으며 관리자의 제품 프로필에 할당된 사용자는 사용자 정의 보고서를 만들 수 있습니다.
