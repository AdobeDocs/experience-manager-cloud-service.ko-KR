...
클라우드 서비스인 Adobe Experience Manager는 Experience Manager 애플리케이션 및 기능을 활용하는 클라우드 기반의 방법을 제공합니다. 이 새로운 아키텍처의 핵심 요소 중 하나는 자산 처리 및 처리이며 자산 마이크로 서비스를 기반으로 합니다. 에셋 마이크로서비스는 클라우드 서비스를 사용하여 자산을 확장 가능하고 탄력적으로 처리할 수 있습니다. Adobe는 다양한 자산 유형 및 처리 옵션을 적절하게 처리하기 위해 클라우드 서비스를 관리합니다. 클라우드 기반의 자산 마이크로 서비스의 주요 이점은 다음과 같습니다.

* 리소스를 많이 사용하는 작업을 원활하게 처리할 수 있는 확장 가능한 아키텍처
* Experience Manager 환경의 성능에 영향을 주지 않는 효율적인 인덱싱 및 텍스트 추출
* 워크플로우가 Experience Manager 환경에서 자산 처리를 처리할 필요성을 최소화합니다. 따라서 리소스를 절약하고 Experience Manager의 로드를 최소화하며 확장성을 제공합니다.
...
* Adobe Experience Manager에서 자산 마이크로서비스에 처리 요청을 보냅니다. 요청 내용은 생성할 변환을 지정하는 Experience Manager의 처리 프로필 구성에 따라 달라집니다.
...
* 생성된 바이너리(변환)에 대한 직접 포인터와 함께 처리가 완료되었다는 알림을 Adobe Experience Manager에 보냅니다. 생성된 표현물은 Experience Manager에서 업로드된 자산에 대해 사용할 수 있습니다.

자산 수집 및 처리의 기본 흐름입니다. 구성된 경우, Experience Manager는 사용자 지정 워크플로우 모델을 시작하여 자산의 사후 처리를 수행할 수도 있습니다. 예를 들어 엔터프라이즈 시스템에서 정보를 가져오고 자산 속성에 추가하는 등 사용자 환경에 맞는 사용자 정의 단계를 실행합니다.

통합 및 처리 과정은 Experience Manager의 자산 마이크로 서비스 아키텍처의 핵심 개념입니다.

* **직접 바이너리 액세스**: 자산은 Experience Manager 환경용으로 구성된 클라우드 이진 스토어로 전송(및 업로드)되고, 그런 다음 AEM, 자산 마이크로 서비스 및 클라이언트가 직접 액세스하여 작업을 수행할 수 있습니다. 이렇게 하면 네트워크 로드와 저장된 바이너리의 복제가 최소화됩니다.
...
제품 제공 중 일부로 제공되는 Adobe Experience Manager 클라이언트는 기본적으로 모든 지원 기능이 바이너리(direct binary access)에 업로드됩니다. 여기에는 웹 인터페이스, Adobe Asset Link 및 AEM 데스크탑 앱을 사용한 업로드가 포함됩니다.
...
Adobe Experience Manager를 구성하여 자산 처리가 완료된 후 게시 워크플로우를 자동으로 트리거할 수 있습니다.
