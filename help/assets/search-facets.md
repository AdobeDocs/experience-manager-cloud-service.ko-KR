...
1. 검색 편집 Forms 페이지의 **[!UICONTROL 조건자 선택]** 탭으로 이동하여 기본 창을 표시합니다. 예: 드래그 **[!UICONTROL 속성 조건자]**.
...
1. 설정 탭에서 술어에 대한 필드 레이블, 자리 표시자 텍스트 및 설명을 입력합니다. 술어와 연결할 메타데이터 속성에 유효한 이름을 지정하십시오. 설정 탭의 헤더 레이블은 선택한 술어의 유형을 식별합니다.
   ...
   ![설정 탭을 사용하여 필요한 술어 옵션을 제공합니다](assets/settings.png)
...
1. In the **[!UICONTROL Property Name]** field, specify a valid name for the metadata property you want to associate with the predicate. It is the name based on which the search is performed. For example, enter `jcr:content/metadata/dc:description` or `./jcr:content/metadata/dc:description`. 선택 대화 상자에서 기존 노드를 선택할 수도 있습니다.
   ...
   *그림: 속성 이름 필드의 술어와 메타데이터 속성 연결*

1. 다음을 클릭합니다. **[!UICONTROL 미리 보기]** ![미리보기](assets/preview.png) 술어를 추가한 후 나타나는 필터 패널의 미리 보기를 생성하려면 다음을 수행합니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td><p>전체 텍스트</p> </td>
   <td>전체 에셋 노드에서 전체 텍스트 검색을 수행하는 검색 조건자입니다. 이 변수는 <code>jcr</code>:<code>contains</code> 연산자. 에셋 노드의 특정 부분에서 전체 텍스트 검색을 수행하려는 경우 상대 경로를 지정할 수 있습니다.</td>
   <td>
    <ul>
     <li>레이블</li>
     <li>자리 표시자</li>
...
기본적으로 앞에 잠금 아이콘이 나타납니다 **[!UICONTROL 에셋 관리자 검색 레일]** 다음에서 **[!UICONTROL Forms 검색]** 페이지를 가리키도록 업데이트하는 중입니다. 기본 양식이 수정되었음을 나타내는 검색 패싯을 양식에 추가하면 잠금 아이콘이 사라집니다.
...
| 작업 | 권한 |
|---|---|
| 편집 | 읽기 및 쓰기 권한 `/apps` crx의 노드 |
| 삭제 | 읽기, 쓰기 및 삭제 권한 `/apps` crx의 노드 |
| 미리보기 | 읽기, 쓰기 및 삭제 권한 `/var/dam/content` crx의 노드 또한 읽기 및 쓰기 권한도 `/apps` 노드. |
