...
예를 들어 파일 유형에서 &quot;이미지&quot;를 선택하면, 에셋 검색 쿼리는 `value` 속성을 OR 연산으로 결합하여 생성됩니다. 예를 들어 이미지 검색 쿼리는 *image/jpeg*, *image/gif*, *image/png*, *image/pjpeg*, and *image/tiff* 값과 일치하는 결과를 `jcr:content/metadata/dc:format` OR 연산으로 결합하여 생성됩니다.
...
1. **[!UICONTROL 설정]** 탭에서 속성의 레이블과 이름을 입력합니다. 예를 들어 해당 형식을 기준으로 자산을 검색하려면 레이블에 대해 사용자에게 친숙한 이름(예: **[!UICONTROL 파일 형식]**)을 지정하십시오. 속성 필드에서 검색을 수행할 기준 속성을 지정합니다(예: `jcr:content/metadata/dc:format.`).
1. 다음 중 하나를 수행하십시오.
...
1. 사용자가 여러 파일 유형(이미지, 문서, 멀티미디어, 아카이브)을 선택할 수 있도록 **[!UICONTROL 단일 선택]** 옵션을 해제하세요. If you select **[!UICONTROL Single Select]**, the user can select only one option for file types at a time.
