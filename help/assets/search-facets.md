...
AEM(Adobe Experience Manager) 자산의 전사적 배포에는 많은 자산을 저장할 수 있는 능력이 있습니다. 경우에 따라, AEM의 일반 검색 기능만 사용하는 경우 올바른 자산을 찾는 것이 어렵고 시간이 많이 소요될 수 있습니다.
...
1. AEM 로고를 탭/클릭한 다음 **[!UICONTROL 도구]** > **[!UICONTROL 일반]** > **[!UICONTROL 양식]**&#x200B;검색으로이동합니다.
1. 양식 검색 페이지에서 **[!UICONTROL 자산 관리 검색 레일을]**&#x200B;선택한 다음 **aemassets_edit**![](assets/aemassets_edit.png)편집을누릅니다.
...
1. 속성 **[!UICONTROL 이름]** 필드에서 술어와 연결할 메타데이터 속성의 유효한 이름을 지정합니다. 검색이 수행되는 이름입니다. 예를 들어, enter `jcr:content/metadata/dc:description` or `./jcr:content/metadata/dc:description`. 선택 대화 상자에서 기존 노드를 선택할 수도 있습니다.
...
1. 미리 보기를 닫으려면 미리 보기 **[!UICONTROL 의]** 오른쪽 위 모서리에 있는 [ ![닫기](assets/do-not-localize/close_icon.png) ]를 클릭합니다.
...
1. AEM 로고를 누른 다음 **[!UICONTROL 도구 > 일반 > 양식 검색으로 이동합니다]**.
1. 양식 **[!UICONTROL 검색]** 페이지에서 **[!UICONTROL 자산 관리 검색 레일을]**&#x200B;선택한 다음 편집 아이콘을 누릅니다.
...
1. 다음 중 하나를 수행하십시오.
   ...
   * 옵션 필드 ![](assets/do-not-localize/aem_assets_add_icon.png) 옆의 을 눌러 필터 패널에서 제공하려는 옵션에 대한 표시 텍스트와 값을 지정합니다. 다른 옵션을 추가하려면 탭/클릭 ![](assets/do-not-localize/aem_assets_add_icon.png) 을 하고 단계를 반복합니다.
...
조건자를 사용하면 자산을 여러 값으로 검색할 수 있습니다. `Multi Value Property` AEM Assets에 여러 제품의 이미지가 있고 각 이미지에 대한 메타데이터에 제품과 연관된 SKU 번호가 포함된 시나리오를 생각해 보십시오. 이 술어를 사용하여 여러 SKU 번호를 기반으로 제품 이미지를 검색할 수 있습니다.

1. AEM 로고를 클릭한 다음 **[!UICONTROL 도구]** > **[!UICONTROL 일반]** > 양식 **[!UICONTROL 검색으로]**&#x200B;이동합니다.
1. 양식 검색 페이지에서 **[!UICONTROL 자산 관리 검색 레일을]**&#x200B;선택하고 **aemassets_edit** ![편집을](assets/aemassets_edit.png)탭합니다.
...
조건자를 사용하면 `Tags` 자산을 위한 태그 기반 검색을 수행할 수 있습니다. 기본적으로 AEM Assets은 지정한 태그를 기준으로 일치하는 하나 이상의 태그를 자산을 검색합니다. 즉, 검색 쿼리는 지정된 태그를 사용하여 OR 작업을 수행합니다. 하지만 모든 태그 일치 옵션을 사용하여 지정한 모든 태그가 포함된 자산을 검색할 수 있습니다.

1. AEM 로고를 클릭한 다음 **[!UICONTROL 도구]** > **[!UICONTROL 일반]** > 양식 **[!UICONTROL 검색으로]**&#x200B;이동합니다.
1. 양식 검색 페이지에서 **[!UICONTROL 자산 관리 검색 레일을]** 선택한 다음 **aemassets_edit**![](assets/aemassets_edit.png)편집을누릅니다.
1. 검색 양식 편집 페이지의 설명 선택 **[!UICONTROL 탭에서]** 태그 설명조건자를 주 창으로 드래그합니다.
1. [설정] 탭에서 설명 자리 표시자 텍스트를 입력합니다. 예를 들어 속성 필드에서 검색을 수행할 속성 이름을 지정합니다 `jcr:content/metadata/cq:tags`. 또는 선택 대화 상자에서 CRXDE의 노드를 선택할 수 있습니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td><p>전체 텍스트</p> </td>
   <td>전체 자산 노드에서 전체 텍스트 검색을 수행하는 검색 조건자입니다. 이 변수는 다음과 <code>jcr</code>매핑됩니다.<code>contains</code> 연산자로 전환하십시오. 자산 노드의 특정 부분에서 전체 텍스트 검색을 수행하려는 경우 상대 경로를 지정할 수 있습니다.</td>
   <td>
    <ul>
     <li>레이블</li>
     <li>자리 표시자</li>
...
1. 도구 **[!UICONTROL 모음에서]** ![삭제 아이콘](assets/do-not-localize/deleteoutline.png) 을 누릅니다.
...
| 작업 | 권한 |
|---|---|
| 편집 | CRX의 노드에 대한 읽기 및 쓰기 권한 `/apps` . |
| 삭제 | CRX의 노드에서 사용 권한을 읽고, 작성하고, `/apps` 삭제합니다. |
| 미리 보기 | CRX의 노드에서 사용 권한을 읽고, 작성하고, `/var/dam/content` 삭제합니다. 또한 노드에 대한 읽기 및 쓰기 권한도 `/apps` 지정합니다. |
