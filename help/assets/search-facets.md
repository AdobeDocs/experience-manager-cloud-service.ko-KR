...
1. 미리 보기를 닫으려면 미리 보기의 오른쪽 위 모서리에 있는 **[!UICONTROL 닫기]** 를 ![탭](assets/do-not-localize/close_icon.png) /클릭합니다.
...
조건자를 사용하면 자산을 여러 값으로 검색할 수 있습니다. `Multi Value Property` AEM 자산에 여러 제품의 이미지가 있고 각 이미지에 대한 메타데이터에 제품과 연관된 SKU 번호가 포함된 시나리오를 생각해 보십시오. 이 술어를 사용하여 여러 SKU 번호를 기반으로 제품 이미지를 검색할 수 있습니다.
...
조건자를 사용하면 `Tags` 자산을 위한 태그 기반 검색을 수행할 수 있습니다. 기본적으로 AEM Assets는 지정한 태그를 기준으로 일치하는 하나 이상의 태그가 있는 자산을 검색합니다. 즉, 검색 쿼리는 지정된 태그를 사용하여 OR 작업을 수행합니다. 하지만 모든 태그 일치 옵션을 사용하여 지정한 모든 태그가 포함된 자산을 검색할 수 있습니다.
...
<table>
 <tbody>
  ...
  <tr>
   <td>편집 </td>
   <td>CRX의 노드에 대한 읽기 및 <code>/apps</code> 쓰기 권한<br /> </td>
  </tr>
  <tr>
   <td>삭제</td>
   <td>CRX의 노드에서 읽기, 쓰기 및 <code>/apps</code> 삭제 권한</td>
  </tr>
  <tr>
   <td>미리 보기</td>
   <td>CRX의 노드에 대한 읽기, 쓰기 및 삭제 권한 <code>/var/dam/content</code> 을 참조하십시오. 또한 노드에 대한 읽기 및 쓰기 권한도 <code>/apps</code> 지정합니다.</td>
  </tr>
 </tbody>
</table>
