...
* [Content Hub 관리자](#onboard-content-hub-administrator): Content Hub에서 브랜드 승인 에셋에 액세스, Content Hub에 에셋 업로드, 이미지 편집을 위한 Adobe Express 통합 외에도 [구성 사용자 인터페이스](/help/assets/configure-content-hub-ui-options.md)에 액세스(Adobe Express 권한이 있는 경우).
...
* [Experience Manager Assets 사용자](#experience-manager-assets-users): Experience Manager Assets as a Cloud Service에서 자산을 승인하여 해당 자산을 Content Hub에서 사용할 수 있도록 하는 기능.
...
>[!NOTE]
>
>[Experience Manager Assets 사용자](#experience-manager-assets-users)는 Experience Manager Assets as a Cloud Service 환경에서 자산을 승인하여 해당 자산을 Content Hub에서 사용할 수 있도록 할 수 있습니다. 이러한 사용자는 AEM > 프로덕션 작성자 인스턴스 > Admin Console을 사용하는 AEM 사용자 제품 프로필에 추가해야 합니다.

## 1단계: Cloud Manager을 사용하여 Experience Manager Assets용 Content Hub 활성화 {#enable-content-hub}


Content Hub 포털에 액세스하려면 먼저 관리자가 Cloud Manager을 사용하여 Experience Manager Assets as a Cloud Service용 Content Hub을 활성화해야 합니다.

### 권한 {#permissions-edit-program}

Cloud Manager에서 프로그램을 편집하려면 비즈니스 소유자 역할이 있어야 합니다. 자세한 내용은 [프로그램 편집](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/editing-programs.md)을 참조하세요.

Experience Manager Assets용 Content Hub을 활성화하려면 다음을 수행하십시오.

1. Cloud Manager에 로그온합니다. 로그인하는 동안 올바른 조직을 선택해야 합니다. Cloud Manager에 모든 프로그램이 나열됩니다.

1. Experience Manager Assets as a Cloud Service 프로그램으로 이동하고 기타 옵션 아이콘(...)을 클릭한 다음 **[!UICONTROL 프로그램 편집]**&#x200B;을 선택합니다.
...
이제 Content Hub이 Experience Manager Assets as a Cloud Service에 대해 활성화됩니다. 프로덕션 환경에서 Content Hub을 활성화한 후에는 셀프서비스 방식으로 비활성화할 수 없습니다.
...
[Cloud Manager을 사용하여 Assets as a Cloud Service용 Content Hub을 활성화](#enable-content-hub) 후 Admin Console의 AEM Assets as a Cloud Service 내에서 접미사 `delivery`을(를) 사용하여 만든 새 인스턴스가 있습니다.
...
Content Hub 관리자는 Content Hub에서 브랜드 승인 에셋에 액세스, Content Hub에 에셋 업로드, 이미지를 편집하기 위한 Adobe Express 통합(Adobe Express 권한이 있는 경우)과 함께 [구성 사용자 인터페이스](/help/assets/configure-content-hub-ui-options.md)에 액세스할 수 있습니다.
...
### 사용자가 AEM Assets Content Hub에 액세스하는 방법과 할당할 수 있는 권한은 무엇입니까?

Adobe Admin Console을 통해 Content Hub의 관련 제품 프로필에 사용자를 할당하여 AEM Assets Content Hub에 사용자를 추가할 수 있습니다.

사용자는 다음 권한을 사용할 수 있습니다.

* Content Hub 사용자는 Content Hub 포털에서 브랜드 승인 에셋에 액세스할 수 있습니다.

* Content Hub 관리자는 Content Hub에서 승인된 에셋에 액세스, Content Hub에 에셋 업로드 및 이미지를 편집하기 위한 Adobe Express 통합에 대한 구성 사용자 인터페이스에 액세스할 수 있습니다(Adobe Express 권한이 있는 경우).

* 에셋을 추가할 수 있는 권한이 있는 Content Hub 사용자는 Content Hub 포털에서 브랜드 승인 에셋에 액세스할 수 있을 뿐만 아니라 Content Hub에 에셋을 업로드할 수 있습니다.

* 자산을 리믹스할 수 있는 권한이 있는 Content Hub 사용자는 Content Hub 포털에서 브랜드 승인 자산에 액세스할 수 있을 뿐만 아니라 Adobe Express(Adobe Express 권한이 있는 경우)에 액세스할 수 있습니다.

### AEM Assets Content Hub에서 다양한 유형의 사용자가 사용할 수 있는 다양한 제품 프로필은 무엇입니까?

제품 프로필은 AEM Assets Content Hub에서 다양한 유형의 사용자가 사용할 수 있습니다.

* Content Hub 사용자: AEM Assets 제한된 사용자

* Content Hub 관리자: AEM Assets 제한된 사용자 + AEM 관리자

* 에셋을 추가할 수 있는 권한이 있는 Content Hub 사용자: AEM Assets 제한된 사용자 + AEM 사용자

* 자산을 리믹스할 수 있는 권한이 있는 Content Hub 사용자: AEM Assets 제한된 사용자 + AEM 사용자

### 관리자는 어떻게 조직에 AEM Assets Content Hub을 활성화할 수 있습니까?

조직에서 AEM Assets Content Hub을 활성화하려면 관리자는 Cloud Manager에 로그인하고 프로그램을 선택(또는 생성)한 다음 솔루션 및 추가 기능 탭에서 Assets 및 Content Hub을 활성화하고 프로그램을 업데이트해야 합니다. 이렇게 하면 사용자 액세스를 관리할 수 있는 Content Hub 인스턴스가 Adobe Admin Console에 생성됩니다.
...
### AEM Assets 라이선스에서 사용 가능한 Content Hub 크레딧은 몇 개입니까?

사용 가능한 Content Hub 크레딧의 수는 AEM Assets 라이선스에 따라 다릅니다.

* Assets Ultimate에는 3개의 Content Hub 크레딧이 포함되어 있습니다.

* Assets Prime에는 Content Hub 크레딧 1개가 포함되어 있습니다.

* 기존 Assets as a Cloud Service 고객은 하나의 Content Hub 크레딧을 받습니다.

### AEM Assets Content Hub 크레딧은 어떻게 사용됩니까?

Content Hub이 활성화된 각 환경마다 하나의 Content Hub 크레딧이 사용됩니다. 예를 들어 프로덕션, 개발 및 스테이징 환경에서 Content Hub을 활성화하려면 3개의 크레딧이 필요합니다.

### 낮은 환경에서 Content Hub을 활성화할 수 있습니까?

예. 사용 가능한 AEM Assets 크레딧이 있는 경우 개발 또는 단계와 같은 하위 환경에서 Content Hub Content Hub을 활성화할 수 있습니다. 활성화된 각 하위 환경은 하나의 크레딧을 사용합니다.

### AEM Assets Content Hub에서 승인된 에셋에 액세스할 수 있는 권한은 어떻게 가질 수 있습니까?

AEM Assets Content Hub 사용자는 Content Hub 포털에서 브랜드 승인 에셋에 액세스할 수 있습니다. AEM 사용자가 되려면 Content Hub 제한 사용자 제품 프로필에 추가되어야 합니다.

### AEM Assets Content Hub에 에셋을 업로드할 권한을 보유하려면 어떻게 해야 합니까?

에셋을 추가할 수 있는 권한이 있는 AEM Assets Content Hub 사용자는 Content Hub 포털에서 브랜드 승인 에셋에 액세스할 수 있을 뿐만 아니라 Content Hub에 에셋을 업로드할 수 있습니다. 에셋을 추가할 권한이 있는 AEM 사용자가 되려면 Content Hub 제한 사용자 및 AEM 사용자 제품 프로필에 추가되어야 합니다.

### AEM Assets Content Hub에서 구성 사용자 인터페이스에 액세스할 수 있는 권한은 어떻게 가질 수 있습니까?

AEM Assets Content Hub 관리자는 브랜드 승인 에셋에 액세스, Content Hub에 에셋 업로드 및 Adobe Express 통합을 통해 이미지 편집을 할 수 있을 뿐만 아니라 Content Hub의 구성 사용자 인터페이스에 액세스할 수 있습니다(Adobe Express 권한이 있는 경우). Content Hub 관리자가 되려면 AEM 제한 사용자 및 AEM 관리자 제품 프로필에 추가되어야 합니다.

### AEM Assets Content Hub에서 Adobe Express을 사용하여 이미지를 편집할 수 있는 권한을 보유하려면 어떻게 해야 합니까?

자산을 리믹스할 수 있는 권한이 있는 AEM Assets Content Hub 사용자는 Content Hub 포털에서 브랜드 승인 자산에 액세스할 수 있을 뿐만 아니라 Adobe Express(Adobe Express 권한이 있는 경우)에 액세스할 수 있습니다. 자산을 리믹스할 권한이 있는 AEM 사용자가 되려면 Content Hub 제한 사용자 및 AEM 사용자 제품 프로필에 추가되어야 합니다.
