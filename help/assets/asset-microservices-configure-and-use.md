...
>[!NOTE]
>
>여기에 설명된 자산 처리는 이전 버전의 `DAM Update Asset`에 있는 [!DNL Experience Manager] 워크플로 모델을 대체합니다. 에셋 마이크로서비스 처리는 대부분의 표준 렌디션 생성 및 메타데이터 관련 단계를 대체하며, 사후 처리 워크플로우 구성은 나머지 단계(있는 경우)를 대체할 수 있습니다.
...
>[!VIDEO](https://video.tv.adobe.com/v/32998?captions=kor&quality=9)
-->
...
*그림: [!UICONTROL 서비스 매개 변수] 필드를 사용하여 사용자 지정 응용 프로그램에 빌드된 미리 정의된 매개 변수에 추가된 정보를 전달하십시오. 이 예제에서 캠페인 이미지가 업로드되면 이미지는 `Jumanji` 글꼴의 `Arial-BoldMT` 텍스트로 업데이트됩니다.*
