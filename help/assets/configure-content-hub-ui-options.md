...
1. 속성에 대한 레이블을 지정하고, **[!UICONTROL 메타데이터]** 필드를 사용하여 속성에 매핑한 다음 새 자산 메타데이터에 대한 적절한 입력 유형을 선택합니다. 입력 유형 레이블이 Adobe Experience Manager(AEM)에 맞게 업데이트됩니다. 사용 가능한 입력 유형에는 텍스트, 태그, 날짜, 숫자, 드롭다운, 다중 값 텍스트, 스마트 태그 및 스마트 색상 태그가 포함됩니다. 특정 메타데이터 필드(xcm:colorDistribution, xcm:machineKeywords)의 경우 입력 유형이 자동으로 할당되며 수정할 수 없습니다.
...
![Content Hub의 구성 UI 업로드 세부 정보](assets/filter-new.png)
...
1. 필터에 대한 레이블을 지정하고, **[!UICONTROL 메타데이터]** 필드를 사용하여 해당 레이블을 속성에 매핑한 다음 새 필터에 대한 적절한 입력 유형을 선택하십시오. 입력 유형 레이블이 Adobe Experience Manager(AEM)에 맞게 업데이트됩니다. 사용 가능한 입력 유형에는 텍스트, 태그, 날짜, 숫자, 드롭다운, 다중 값 텍스트, 스마트 태그 및 스마트 색상 태그가 포함됩니다. 특정 메타데이터 필드(xcm:colorDistribution, xcm:machineKeywords)의 경우 입력 유형이 자동으로 할당되며 수정할 수 없습니다.
...
![Content Hub의 구성 UI 필터](assets/add-filters.png)
...
1. **[!UICONTROL 일괄 검색]** 전환을 사용하도록 설정합니다. 기본 구분 기호 `[ , | \t | \r\n | \r | \n ]`이(가) 자동으로 표시됩니다. 또한 구분 기호를 추가할 수도 있습니다. 이렇게 하려면 `pipe symbol (|)`(으)로 구분된 입력 상자에 구분 기호를 지정합니다.
   ...
   ![대량 검색 구성](assets/edit-filter-new.png)
...
![Content Hub의 구성 UI 자산 세부 정보](assets/asset-detailsn.png)
...
다음 단계를 실행하여 **[!UICONTROL 자산 카드]**&#x200B;에 표시할 메타데이터 속성을 구성하십시오.
...
마찬가지로 각 사용 가능한 속성 옆에 있는 ![편집](/help/assets/assets/edit-content-hub.svg)을 클릭하여 필요한 수정 작업을 수행하거나 ![삭제](/help/assets/assets/delete-content-hub.svg)를 클릭하여 기존 메타데이터 속성을 삭제합니다. 변경 사항을 적용하려면 모든 수정 작업을 수행한 후 **저장**&#x200B;을 클릭하세요.
...
>[!CAUTION]
>
>검색 가능으로 표시한 필드를 신중하게 선택하는 것이 좋습니다. 사용자가 검색할 가능성이 높고 의미 있는 결과를 반환하는 데 도움이 되는 필드만 포함합니다. 날짜 및 숫자와 같은 필드는 검색 가능한 콘텐츠보다는 필터링 및 정렬에 더 적합합니다. 너무 많은 필드를 검색 가능으로 표시하면 검색 결과의 품질과 성능에 영향을 줄 수 있습니다.
...
**[!UICONTROL 기본 색상]:** 기본 색상 구성표는 선택 작업, 확인란, 검색 창, 토글 스위치와 같은 대화형 요소에 적용되며 [!DNL Content Hub] 홈 페이지 및 [!UICONTROL 구성] 페이지의 [!DNL Content Hub]에 적용됩니다. **[!UICONTROL 모든 Assets]** 및 **[!UICONTROL 컬렉션]** 페이지에서 사용할 수 있는 옵션과 같이 기본 [!DNL Content Hub] 인터페이스에서 사용할 수 있는 작업 옵션에도 적용됩니다.
