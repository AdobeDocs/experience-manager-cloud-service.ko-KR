...
## Experience Manager Assets as a Cloud Service 저장소의 모든 자산을 OpenAPI 기능이 포함된 Dynamic Media를 사용하여 검색하고 게재할 수 있습니까? {#assets-available-for-search}
...
## 관리자가 폴더에 추가된 신규 자산과 기존 자산을 승인된 것으로 표시하려면 어떻게 해야 합니까? {#add-assets-to-folder-as-approved}
...
## 게재 및 검색 경험을 보호하기 위해 Experience Manager 관리 보기에서 자산에 대한 역할을 설정하는 데 Adobe IMS(Adobe Identity Management Services) 사용자 또는 그룹 ID를 사용하도록 하려면 어떻게 해야 합니까? {#set-roles-secure-delivery-search}
...
## 자산의 승인 상태를 편집할 수 있는 권한을 얻으려면 어떻게 해야 합니까? {#permissions-edit-approval-status}
...
## OpenAPI 기능 포함 Dynamic Media와 Dynamic Media 솔루션의 차이점은 무엇입니까? {#dynamic-media-and-dynamic-media-with-openapi-differences}
...
| 비디오 스마트 자르기 기능을 지원하지 않습니다. | 비디오 스마트 자르기 기능을 지원합니다. |
...
| SEO를 손상시키지 않고 자산 UID를 사용하여 안전하고 난독화된 URL을 활성화합니다. | URL 난독화는 URL 쿼리 매개변수에만 사용할 수 있습니다. URL의 Assets ID(자산 이름)를 인식할 수 있습니다. |


## OpenAPI 기능 포함 Dynamic Media는 연결된 자산 기능의 한계를 어떻게 해결합니까? {#dynamic-media-openapi-addresses-connected-assets-limitations}
...
1. Adobe 지원 사례에 다음 세부 정보를 언급합니다.
