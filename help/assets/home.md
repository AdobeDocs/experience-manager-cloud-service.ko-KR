---
title: 클라우드 서비스로서의 Adobe Experience Manager 자산
description: 클라우드 서비스로서의 Adobe Experience Manager 자산 자습 리소스 및 설명서 링크
translation-type: tm+mt
source-git-commit: 068195919c4bf73c41b1156eadb47544e4c41e65

---


# 클라우드 서비스로서의 Adobe Experience Manager 자산 {#aem-assets-guide}

Adobe Experience Manager Assets as a Cloud Service에 대한 자체 도움말 리소스를 소개합니다.

## 주요 자산 아티클 {#key-articles}

* [클라우드 서비스로서의 자산 소개](overview.md)
* [클라우드 서비스로서의 자산 아키텍처](architecture.md)

## Adobe Experience Manager 가이드 {#aem-guides}

| 사용 안내서 | 설명 |
|---|---|
| [클라우드 서비스로서의 Experience Manager 홈](/help/landing/home.md) | 클라우드 서비스로서의 Experience Manager에 대한 전체 설명서 |
| [개요](/help/overview/home.md) | 클라우드 서비스로서의 Experience Manager 개요 및 용어집 |
| [릴리스 노트](/help/release-notes/home.md) | 릴리스 노트, 새로운 기능, 더 이상 사용되지 않거나 제거된 기능, 알려진 문제 |
| [핵심 개념](/help/core-concepts/home.md) | 이 안내서에서는 이 새로운 서비스의 아키텍처를 포함하여 클라우드 서비스로서의 Experience Manager의 핵심 개념을 소개합니다. |
| [보안 사용 안내서](/help/security/home.md) | 클라우드 서비스로서의 Experience Manager에 대한 보안 항목 |
| [온보딩](/help/onboarding/home.md) | Adobe Experience Manager를 클라우드 서비스로 사용하여 중요한 데이터에 액세스하고 보호할 수 있습니다. |
| [사이트 사용 안내서](/help/sites-cloud/home.md) | 클라우드 서비스로 Experience Manager Sites 관리 |
| [구현 사용 안내서](/help/implementing/home.md) | Adobe Experience Manager를 클라우드 서비스로 맞춤화, 개발 및 배포할 수 있습니다. |
| [커넥터 사용 안내서](/help/connectors/home.md) | Adobe Experience Manager를 클라우드 서비스로 통합하고 |
| [작업 사용 안내서](/help/operations/home.md) | 인덱싱 및 유지 관리 작업과 같은 클라우드 서비스로서 Experience Manager의 백엔드 작업. |

## 기타 Experience Manager 리소스 {#other-resources}

* [디스패처 설명서](/help/implementing/dispatcher/overview.md)
* [HTL 설명서](https://docs.adobe.com/content/help/en/experience-manager-htl/using/overview.html)
* [핵심 구성 요소 설명서](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/introduction.html)
* [Cloud Manager 설명서](https://docs.adobe.com/content/help/en/experience-manager-cloud-manager/using/introduction-to-cloud-manager.html)
* [GDPR 준비 완료](/help/onboarding/data-privacy-and-protection-readiness/aem-readiness.md)
* [클라우드 서비스로서의 Adobe Experience Manager 자습서](https://docs.adobe.com/content/help/en/experience-manager-learn/cloud-service/overview.html)
* [Experience League](https://guided.adobe.com/?promoid=K42KVXHD&mv=other#solutions/experience-manager)
* [AEM 커뮤니티 포럼](https://forums.adobe.com/community/experience-cloud/marketing-cloud/experience-manager)
