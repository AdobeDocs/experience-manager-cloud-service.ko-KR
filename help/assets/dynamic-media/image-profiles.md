...
생성하는 각 스마트 자르기 생성에는 추가 처리가 필요합니다. 예를 들어 5개 이상의 스마트 자르기 종횡비를 추가하면 에셋 수집률이 느려질 수 있습니다. 또한 시스템의 부하가 증가할 수 있습니다. Adobe 폴더 수준에서 스마트 자르기를 적용할 수 있으므로 필요한 *폴더*&#x200B;에서만 사용하는 것이 좋습니다.
...
1. 페이지의 오른쪽 상단 모서리에서 **[!UICONTROL 저장 및 닫기]**&#x200B;를 선택합니다. Folders that have a profile already assigned to it are indicated by the display of the profile&#39;s name directly below the folder name.
...
>[!IMPORTANT]
>
>에셋의 스마트 자르기 창을 수동으로 재정렬하거나 크기를 조정하면 나중에 에셋을 재처리하는 경우에도 해당 편집이 유지되고 보존됩니다. 그러나 이미지 프로필의 **[!UICONTROL 응답형 이미지 자르기]** 영역에서 폭, 높이 또는 두 가지 모두를 편집하면 해당 에셋이 재처리됩니다.
