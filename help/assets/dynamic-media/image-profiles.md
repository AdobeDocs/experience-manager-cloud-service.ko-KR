...
| 옵션 | 사용 시기 | 설명 |
| --- | --- | --- |
| **[!UICONTROL 픽셀 자르기]** | 차원만 기준으로 이미지를 벌크 자르십시오. | 에서 **[!UICONTROL 자르기 옵션]** 드롭다운 목록에서 **[!UICONTROL 픽셀 자르기]**.<br>이미지의 양쪽에서 자르려면 이미지의 모든 측면이나 각 측면에서 자를 픽셀 수를 입력합니다. 이미지가 잘리는 양은 이미지 파일의 ppi(인치당 픽셀) 설정에 따라 달라집니다.<br>이미지 프로필 픽셀 자르기는 다음과 같은 방식으로 렌더링됩니다.<br>・ 값은 위쪽, 아래쪽, 왼쪽 및 오른쪽입니다.・ 왼쪽 위 부분이 고려됨 `0,0` 여기서 픽셀 자르기가 계산됩니다.<br>・ 자르기 시작 지점: 왼쪽은 X이고, 상단은 Y입니다<br>・ 가로 계산: 원래 이미지의 가로 픽셀 크기 - 왼쪽 및 - 오른쪽<br>・ 세로 계산: 세로 픽셀 높이 - 위쪽, 아래쪽 빼기<br>예를 들어 4000 x 3000 픽셀 이미지가 있다고 가정합니다. 값을 사용합니다. Top=250, Bottom=500, Left=300, Right=700.<br>(4000-300-700, 3000-250-500 또는 3000,2250)의 채우기 공간을 사용하여 왼쪽 위(300,250) 자르기로부터 |
...
| **[!UICONTROL 대상 해상도 전체에서 자르기 콘텐츠 유지]** | 동일한 종횡비로 자르기 컨텐츠를 유지 관리하려면 | 스마트 자르기 프로필을 만들 때 을 사용합니다.<br>이 상자를 선택 취소하면 다양한 해상도에서 지정된 종횡비에 대해 초점을 유지하면서 새로운 자르기 콘텐츠가 생성됩니다.<br>이 상자의 선택을 취소하면 원본 이미지 해상도가 스마트 자르기 프로필에 대해 정의한 해상도보다 커야 합니다.<br><br>예를 들어 종횡비를 600 x 600(큰), 400 x 400(중간) 및 300 x 300(작은)으로 설정했다고 가정합니다. <br>사용 **[!UICONTROL 대상 해상도의 자르기 컨텐츠 유지]** 옵션 *체크*&#x200B;세 해상도에서 모두 동일한 자르기를 볼 수 있으므로, 다음 이미지 샘플 출력(예시적인 목적으로만 사용)이 발생합니다.<br>![옵션 선택](/help/assets/dynamic-media/assets/preserve-checked.png)<br><br>사용 **[!UICONTROL 대상 해상도의 자르기 컨텐츠 유지]** 옵션 *선택 안 함*&#x200B;로 지정하는 경우, 자르기 컨텐츠는 세 가지 해상도 모두에 대해 새로 만들어지므로 다음 샘플 이미지 출력이 수행됩니다(예시적인 경우에만).<br>![선택 사항 선택 취소](/help/assets/dynamic-media/assets/preserve-unchecked.png) |
