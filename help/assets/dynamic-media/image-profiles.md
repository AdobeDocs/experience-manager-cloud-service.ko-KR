...
이미지 프로필을 만들 때 **[!UICONTROL 언샵 마스크]** 다운샘플링된 최종 이미지에서 선명하게 하기 필터 효과를 미세 조정하는 옵션입니다. 효과의 강도, 효과의 반경(픽셀 단위) 및 무시되는 대비 임계값을 제어할 수 있습니다. 이 효과는 Adobe Photoshop의 &quot;언샵 마스크&quot; 필터와 동일한 옵션을 사용합니다.

>[!NOTE]
>
>언샵 마스크는 50% 이상 다운샘플링되는 PTIFF(피라미드형 tiff) 내의 축소된 렌디션에만 적용됩니다. 즉, ptiff 내의 최대 크기 렌디션은 언샵 마스크의 영향을 받지 않습니다. 반면 썸네일과 같은 작은 크기의 렌디션은 변경되며(그리고 언샵 마스크 표시).
...
<table>
 <tbody>
  ...
  <tr>
   <td>임계값</td>
   <td><p>언샵 마스크 필터가 적용될 때 무시할 수 있도록 대비 범위를 결정합니다. 즉, 이 옵션은 가장자리 픽셀로 간주하여 선명하게 되기 전에 선명해진 픽셀이 주변 영역과 얼마나 달라야 하는지 결정합니다. 소음이 발생하지 않도록 하려면 0-255 사이의 값으로 실험하십시오.</p> </td>
  </tr>
 </tbody>
</table>

선명하게 하기는에 설명되어 있습니다. [이미지 선명하게 하기](/help/assets/dynamic-media/assets/sharpening_images.pdf).
...
생성하는 각 스마트 자르기 생성에는 추가 처리가 필요합니다. 예를 들어 5개 이상의 스마트 자르기 종횡비를 추가하면 에셋 수집률이 느려질 수 있습니다. 또한 시스템의 부하가 증가할 수 있습니다. Adobe 폴더 수준에서 스마트 자르기 를 적용할 수 있으므로 폴더에서 사용하는 것이 좋습니다 *전용* 필요한 위치입니다.

**이미지 프로필의 스마트 자르기 정의 지침**
스마트 자르기 사용을 제어하고 자르기 처리 시간 및 저장을 최적화하려면 Adobe에서 다음 지침 및 팁을 권장합니다.

* 폭과 높이 값이 동일한 중복 스마트 자르기 프로필을 만들지 마십시오.
...
두 가지 이미지 자르기 옵션을 선택할 수 있습니다. 색상 및 이미지 견본 만들기를 자동화하거나 대상 해상도 전체에서 자르기 콘텐츠를 보존하도록 선택할 수도 있습니다.
...
| 옵션 | 사용 시기 | 설명 |
| --- | --- | --- |
| **[!UICONTROL 픽셀 자르기]** | 차원만 기준으로 이미지를 대량으로 자릅니다. | 다음에서 **[!UICONTROL 자르기 옵션]** 드롭다운 목록에서 다음을 선택합니다. **[!UICONTROL 픽셀 자르기]**.<br>이미지의 측면에서 자르려면 이미지의 모든 측면 또는 각 측면에서 자를 픽셀 수를 입력합니다. 이미지가 잘리는 정도는 이미지 파일의 ppi(인치당 픽셀) 설정에 따라 달라집니다.<br>이미지 프로필 픽셀 자르기는 다음과 같은 방식으로 렌더링됩니다.<br>· 값은 위쪽, 아래쪽, 왼쪽 및 오른쪽입니다.<br>· 왼쪽 상단이 고려됩니다. `0,0` 그리고 픽셀 자르기 결과는 거기에서 계산됩니다.<br>· 자르기 시작점: 왼쪽은 X이고 위쪽은 Y입니다.<br>· 수평 계산: 원본 이미지의 수평 픽셀 크기에서 [왼쪽]을 뺀 다음 [오른쪽]을 뺀 값입니다.<br>· 세로 계산: 세로 픽셀 높이에서 위쪽을 뺀 다음 아래쪽을 뺀 값입니다.<br>예를 들어 4000 x 3000픽셀 이미지가 있다고 가정합니다. Top=250, Bottom=500, Left=300, Right=700 값을 사용합니다.<br>왼쪽 위(300,250)에서 (4000-300-700, 3000-250-500 또는 3000,2250)의 채우기 공간을 사용하여 자릅니다. |
| **[!UICONTROL 스마트 자르기]** | 시각적 초점을 기반으로 이미지를 대량 자릅니다. | 스마트 자르기는 Adobe Sensei에서 인공 지능의 힘을 사용하여 일괄 이미지 자르기를 빠르게 자동화합니다. 스마트 자르기는 화면 크기에 관계없이 모든 이미지의 초점을 자동으로 감지하고 자르기하여 원하는 관심 영역을 얻습니다.<br>다음에서 **[!UICONTROL 자르기 옵션]** 드롭다운 목록에서 다음을 선택합니다. **[!UICONTROL 스마트 자르기]**&#x200B;을 클릭한 다음 의 오른쪽에 있는 **[!UICONTROL 응답형 이미지 자르기]**, 기능을 활성화(켜기)합니다.<br>기본 중단점 크기(**[!UICONTROL 크게]**, **[!UICONTROL Medium]**, **[!UICONTROL 작음]**)는 대부분의 이미지가 모바일 및 태블릿 장치, 데스크탑 및 배너에 사용되는 모든 범위의 크기를 다룹니다. 원하는 경우 [크게], [중간] 및 [작게]의 기본 이름을 편집할 수 있습니다.<br>중단점을 더 추가하려면 를 선택합니다. **[!UICONTROL 자르기 추가]**&#x200B;자르기를 삭제하려면 휴지통 아이콘을 선택합니다. |
| **[!UICONTROL 색상 및 이미지 견본]** | 벌크는 각 이미지에 대한 이미지 견본을 생성합니다. | **참고**: 스마트 색상 견본은 Dynamic Media Classic에서 지원되지 않습니다.<br>색상 또는 질감을 표시하는 제품 이미지에서 고품질 견본을 자동으로 찾아 생성합니다.<br>다음에서 **[!UICONTROL 자르기 옵션]** 드롭다운 목록에서 다음을 선택합니다. **[!UICONTROL 스마트 자르기]**. 그럼 오른쪽에 **[!UICONTROL 색상 및 이미지 견본]**, 기능을 활성화(켜기)합니다. 에 픽셀 값 입력 **[!UICONTROL 폭]** 및 **[!UICONTROL 높이]** 텍스트 상자.<br>렌디션 레일에서 모든 이미지 자르기를 사용할 수 있지만 견본은 **[!UICONTROL URL 복사]** 기능. 자신만의 보기 구성 요소를 사용하여 사이트에서 견본을 렌더링합니다. 이 규칙의 예외는 회전 배너입니다. Dynamic Media은 회전 배너에 사용되는 견본의 보기 구성 요소를 제공합니다.<br><br>**이미지 견본 사용**<br>&#x200B;이미지 견본의 URL은 간단합니다.<br>`/is/image/company/&lt;asset_name&gt;:Swatch`<br>위치 `:Swatch` 는 에셋 요청에 추가됩니다.<br><br>**색상 견본 사용**<br>&#x200B;색상 견본을 사용하려면 `req=userdata` 다음을 포함한 요청:<br>`/is/image/&lt;company_name&gt;/&lt;swatch_asset_name&gt;:Swatch?req=userdata`<br><br>예를 들어 다음은 Dynamic Media Classic의 견본 자산입니다.<br>`https://my.company.com:8080/is/image/DemoCo/Sleek:Swatch`<br>다음은 견본 에셋의 해당 항목입니다 `req=userdata` URL:<br>`https://my.company.com:8080/is/image/DemoCo/Sleek:Swatch?req=userdata`<br>다음 `req=userdata` 응답은 다음과 같습니다.<br>`SmartCropDef=Swatch`<br>`SmartCropHeight=200.0`<br>`SmartCropRect=0.421671,0.389815,0.0848564,0.0592593,200,200`<br>`SmartCropType=Swatch`<br>`SmartCropWidth=200.0`<br>`SmartSwatchColor=0xA56DB2`<br>다음을 요청할 수도 있습니다. `req=userdata` 다음 각 URL 예제와 같이 XML 또는 JSON 형식의 응답입니다.<br>·`https://my.company.com</code>:8080/is/image/DemoCo/Sleek:Swatch?req=userdata,json`<br>·`https://my.company.com:8080/is/image/DemoCo/Sleek:Swatch?req=userdata,xml`<br><br>**참고**: 색상 견본을 요청하고 구문을 분석하려면 자체 WCM 구성 요소를 만들어야 합니다. `SmartSwatchColor` 24비트 RGB 16진수 값으로 표현되는 속성입니다.<br>참조: [`userdata`](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/http-protocol-reference/command-reference/req/r-userdata.html) 를 참조하십시오. |
| **[!UICONTROL 대상 해상도 전체에서 자르기 콘텐츠 유지]** | 동일한 종횡비에 걸쳐 자르기 콘텐츠 유지 | 스마트 자르기 프로필을 만들 때 사용합니다.<br>다양한 해상도에서 지정된 종횡비에 대해 초점을 유지하면서 새로운 자르기 콘텐츠를 생성하려면 이 옵션의 선택을 취소합니다.<br>이 상자를 선택 해제하려면 원본 이미지 해상도가 스마트 자르기 프로필에 대해 정의한 해상도보다 커야 합니다.<br><br>예를 들어 종횡비를 600 x 600(큼), 400 x 400(중간) 및 300 x 300(작음)으로 설정했다고 가정합니다.<br>날짜 **[!UICONTROL 대상 해상도 전체에서 자르기 콘텐츠 유지]** 옵션은 다음과 같습니다. *선택됨*&#x200B;에서는 다음의 이미지 샘플 출력과 유사하게, 세 해상도 모두에 대해 동일한 자르기가 표시됩니다(설명 목적으로만 해당).<br>![옵션 선택됨](/help/assets/dynamic-media/assets/preserve-checked.png)<br><br>날짜 **[!UICONTROL 대상 해상도 전체에서 자르기 콘텐츠 유지]** 옵션은 다음과 같습니다. *선택되지 않음*, 자르기 콘텐츠는 이미지의 다음 샘플 출력과 유사한 세 가지 해상도 모두에 대해 새로운 것입니다(설명용으로만 해당).<br>![선택 취소됨](/help/assets/dynamic-media/assets/preserve-unchecked.png) |
...
다른 자산 유형에 대한 고급 처리 매개 변수를 정의하려면 다음을 참조하십시오. [자산 처리 구성](config-dm.md#configuring-asset-processing).

다음을 참조하십시오 [Dynamic Media 이미지 프로필 및 비디오 프로필 정보](/help/assets/dynamic-media/about-image-video-profiles.md).

참조: [처리 프로필 사용을 위한 디지털 자산 구성 우수 사례](/help/assets/organize-assets.md).

**Dynamic Media 이미지 프로필을 만들려면 다음을 수행하십시오.**

1. Adobe Experience Manager 로고를 선택하고 다음 위치로 이동합니다 **[!UICONTROL 도구]** > **[!UICONTROL 에셋]** > **[!UICONTROL 이미지 프로필]**.
1. 이미지 프로필을 추가하려면 **[!UICONTROL 만들기]**.
1. 언샵 마스크, 자르기, 색상 견본 또는 둘 다에 대한 프로파일 이름과 값을 입력합니다.

   팁: 의도한 목적에 맞는 프로필 이름을 사용하십시오. 예를 들어 견본만 생성하는 프로파일을 만들려고 한다고 가정합니다. 즉, 스마트 자르기가 비활성화되고(꺼짐), 색상 및 이미지 견본이 활성화됩니다(켜짐). 이러한 경우 &quot;스마트 색상 견본&quot;이라는 프로필 이름을 사용할 수 있습니다.
...
1. Experience Manager 로고를 선택하고 다음으로 이동 **[!UICONTROL 도구]** > **[!UICONTROL 에셋]** > **[!UICONTROL 이미지 프로필]**.
...
프로필이 할당된 폴더는 사용자 인터페이스에 표시되며, 프로필의 이름이 카드에 표시됩니다.

<!-- When you add smart crop to an existing Image Profile, you need to re-trigger the [DAM Update Asset workflow](assets-workflow.md) if you want to generate crops for existing assets in your asset repository. -->

이미지 프로필을 특정 폴더에 적용하거나 모든 에셋에 전역적으로 적용할 수 있습니다.

나중에 변경한 기존 이미지 프로필이 이미 있는 폴더에서 에셋을 재처리할 수 있습니다. 다음을 참조하십시오 [처리 중인 프로필을 편집한 후 폴더에서 에셋 재처리](/help/assets/dynamic-media/about-image-video-profiles.md#reprocessing-assets).
...
나중에 변경한 기존 비디오 프로필이 이미 있는 폴더에서 에셋을 재처리할 수 있습니다. 다음을 참조하십시오 [처리 중인 프로필을 편집한 후 폴더에서 에셋 재처리](/help/assets/dynamic-media/about-image-video-profiles.md#reprocessing-assets).
...
1. 다음 항목 선택 **[!UICONTROL Dynamic Media 처리 중]** 탭.
1. 아래 **[!UICONTROL 이미지 프로필]**, **[!UICONTROL 프로필 이름]** 드롭다운 목록에서 적용할 프로필을 선택합니다.
1. 페이지의 오른쪽 상단 모서리 근처에서 을 선택합니다. **[!UICONTROL 저장 및 닫기]**. Folders that have a profile already assigned to it are indicated by the display of the profile&#39;s name directly below the folder name.
...
>[!IMPORTANT]
>
>에셋의 스마트 자르기 창을 수동으로 재정렬하거나 크기를 조정하면 나중에 에셋을 재처리하는 경우에도 해당 편집이 유지되고 보존됩니다. 그러나 너비와 높이 중 하나 또는 둘 다를 **[!UICONTROL 응답형 이미지 자르기]** 이미지 프로필의 영역이면 해당 에셋은 재처리됩니다.
...
참조: [여러 이미지의 스마트 자르기 또는 스마트 색상 견본 편집](#editing-the-smart-crop-or-smart-swatch-of-multiple-images).

**단일 이미지의 스마트 자르기 또는 스마트 색상 견본을 편집하려면 다음을 수행하십시오.**

1. Experience Manager 로고를 선택하고 다음으로 이동 **[!UICONTROL 에셋]**&#x200B;를 클릭한 다음 스마트 자르기 또는 스마트 견본 이미지 프로필이 적용된 폴더로 이동합니다.
...
1. 다음 중 하나를 수행합니다.
   ...
   * 이미지 위에서 을 선택합니다.  **[!UICONTROL 되돌리기]** 편집을 모두 취소하고 원래 자르기 또는 색상 견본을 복원합니다.
...
**여러 이미지의 스마트 자르기 또는 스마트 색상 견본을 편집하려면 다음을 수행하십시오.**

1. Experience Manager 로고를 선택하고 다음으로 이동 **[!UICONTROL 에셋]**&#x200B;를 클릭한 다음 스마트 자르기 또는 스마트 견본 이미지 프로필이 적용된 폴더로 이동합니다.
1. 폴더에서 **[!UICONTROL 추가 작업]** (...) 아이콘을 클릭한 다음 을 선택합니다. **[!UICONTROL 스마트 자르기]**.

1. 다음에서 **[!UICONTROL 스마트 자르기 편집]** 페이지에서 다음 중 하나를 수행합니다.
...
폴더 내에서 이미지 프로필을 제거할 수 있습니다. **[!UICONTROL 도구]** 메뉴 또는 폴더에 있는 경우 **[!UICONTROL 속성]**.
...
1. 선택 **[!UICONTROL 폴더에서 처리 프로필 제거]** 에서 프로필을 제거하는 데 사용할 폴더 또는 여러 폴더를 선택하고 **[!UICONTROL 제거]**.
...
1. Experience Manager 로고를 선택하고 이동 **[!UICONTROL 에셋]** 그런 다음 이미지 프로필을 제거할 폴더로 이동합니다.
1. 폴더에서 확인 표시를 선택하여 선택한 다음 을 선택합니다 **[!UICONTROL 속성]**.
1. 다음 항목 선택 **[!UICONTROL 이미지 프로필]** 탭.
1. 다음에서 **[!UICONTROL 프로필 이름]** 드롭다운 목록에서 다음을 선택합니다. **[!UICONTROL 없음]**&#x200B;을 선택한 다음 을 선택합니다. **[!UICONTROL 저장 및 닫기]**.
