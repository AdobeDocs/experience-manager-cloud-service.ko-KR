...
이미지 세트는 해당 세트임을 나타내는 배너로 표시됩니다. `IMAGESET`. 또한 이미지 세트가 게시되면 **[!UICONTROL World]** 아이콘으로 표시된 게시 날짜가 배너에 있습니다. 또한 **[!UICONTROL 연필]** 아이콘으로 표시된 마지막 수정 날짜가 표시됩니다.
...
1. 세트에 자산을 추가하면 영숫자 순서로 자산이 자동으로 추가됩니다. 에셋을 추가한 후 수동으로 에셋을 재정렬하거나 정렬할 수 있습니다.
   ...
   썸네일 또는 스와치를 변경하려면 이미지 옆의 **+****썸네일** 아이콘을 클릭하고 원하는 썸네일 또는 스와치를 선택하세요. 모든 이미지를 선택한 후 **[!UICONTROL 저장]**&#x200B;을 클릭하세요.
