---
title: 뷰어 사전 설정 관리
description: Dynamic Media에서 뷰어 사전 설정을 만들고 관리하는 방법.
...
다음 표는 Dynamic Media과 함께 제공되는 미리 정의된 모든 미리 정의된 기본 뷰어 사전 설정을 나타냅니다.
...
새 뷰어 사전 설정을 만들도록 선택하는 경우, 저장한 후 뷰어 사전 설정 페이지에서 뷰어 상태가 자동으로 활성화됩니다(설정된 경우 **[!UICONTROL On]**). 이 상태는 Dynamic Media 구성 요소 및 대화형 미디어 구성 요소에서 볼 수 있고 이미지나 비디오를 미리 볼 때마다 볼 수 있음을 의미합니다.
...
뷰어 사전 설정의 상태를 활성화(또는 &quot;켜기&quot;)하는 것은 Dynamic Media 구성 요소, 대화형 미디어 구성 요소에서 그리고 자산을 볼 때마다 해당 사전 설정이 표시되도록 함을 의미합니다.

그러나 뷰어 사전 설정이 있는 에셋을 전달하려면 뷰어 사전 설정도 게시해야 합니다.* 자산에 대한 URL 또는 포함 코드를 얻으려면 모든 뷰어 사전 설정을 활성화하고 *게시해야 합니다. You must activate and publish all out-of-the-box viewer presets that comes with Dynamic Media. 만들고 추가하는 사용자 정의 뷰어 사전 설정은 자동 활성화되지만 게시되어야 합니다.
...
AEM을 WCM으로 사용하는 경우 페이지에서 바로 뷰어 사전 설정을 사용하여 자산을 추가할 수 있습니다. 페이지](/help/assets/dynamic-media/adding-dynamic-media-assets-to-pages.md)에 Dynamic Media 자산 추가를 참조하십시오.[