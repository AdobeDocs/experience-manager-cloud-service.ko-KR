...
## 자산 {#obtaining-a-url-for-an-asset}에 대한 URL 얻기
...
1. 복사하려는 이미지 사전 설정 URL 또는 뷰어 사전 설정 URL의 *게시된* 자산으로 이동하고 자산을 눌러 엽니다.
   ...
   URL은 *이후에*&#x200B;만 복사할 수 있습니다. 먼저 자산을 *게시된*&#x200B;합니다. 또한 뷰어 사전 설정 또는 이미지 사전 설정도 게시해야 합니다.
...
1. 선택한 자산에 따라 다음 중 하나를 수행합니다.
   ...
   * 이미지를 선택한 경우 드롭다운 메뉴에서 **[!UICONTROL 표현물]**&#x200B;을 누릅니다.
      ...
      **[!UICONTROL Dynamic]** 머리글 아래에서 사전 설정 이름을 탭하여 오른쪽 프레임에서 해당 변환을 봅니다. 동적 제목을 보려면 표현물 목록을 스크롤해야 할 수도 있습니다.

      왼쪽 레일 하단에서 **[!UICONTROL URL]**&#x200B;을 누릅니다.
   ...
   * 스핀 세트, 이미지 세트, 회전판 세트 또는 비디오를 선택한 경우 드롭다운 메뉴에서 **[!UICONTROL 뷰어]**&#x200B;를 누릅니다.
      ...
      왼쪽 레일의 맨 아래에서 **[!UICONTROL URL]**&#x200B;을 누릅니다.

      ![chlimage_1-271](assets/chlimage_1-271.png)

1. 텍스트를 선택하여 웹 브라우저에 복사하여 자산을 미리 보거나 웹 컨텐츠 페이지에 추가합니다.

   URL 창을 종료하려면 **[!UICONTROL X]**&#x200B;을 누르거나 **[!UICONTROL Close]**&#x200B;을 누릅니다.
...
1. 복사할 URL이 있는 *게시된 정적 자산으로 이동하고 자산을 눌러 엽니다.
   ...
   URL은 먼저 정적 자산을 *게시된*&#x200B;이후 *만 복사할 수 있습니다.*
...
1. 게시된 정적 자산의 URL을 얻으려면 다음 방법 중 하나를 사용하십시오.
   ...
   * **[!UICONTROL 자산 > 동적 표현물]**&#x200B;을 누른 다음 정적 자산의 동적 변환을 누르고 URL을 복사합니다.
...
1. **[!UICONTROL 등록 ID]**&#x200B;에서 ID의 마지막 부분에 있는 고객 이름을 복사합니다.
   ...
   예를 들어 등록 ID가 `87654321|MyCompany`이면 고객 이름이 `MyCompany`이 됩니다.
...
1. 사용 중인 비디오 전달 프로토콜을 기반으로 각 프로토콜 선택기를 복사합니다. 이 단계 후반부에 복사된 프로토콜 선택기가 필요합니다.
   ...
   <table>
    <tbody>
      ...
      <tr>
      <td><p>HTTP</p> <p>HTTP(비보안 비디오 전달)를 사용하는 경우 이전에 복사한 비디오 서비스 URL 값의 <code>https</code>을 <code>http</code>(으)로 변경해야 합니다.</p> </td>
...
1. 문자열 끝에 `.m3u8`을 추가하여 URL을 완료합니다. 예를 들어 이전 단계의 문자열에 `.m3u8`을 추가하면 전체 URL 경로가 다음과 같이 표시됩니다.
