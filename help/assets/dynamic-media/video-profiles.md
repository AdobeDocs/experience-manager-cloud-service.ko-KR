...
값이 동일하지 않으면 프로파일을 그대로 계속 만들 수 있습니다. 그러나 적응형 스트리밍은 불가능합니다. 대신 단일 비트 전송률 스트리밍이 가능합니다. 프로파일에 있는 개별 인코딩 사전 설정에서 동일한 값을 사용하도록 인코딩 설정을 편집하는 것이 좋습니다. [비디오 프로파일/사전 설정 편집기]는 &quot;적응 스트리밍용으로 인코딩&quot;이 활성화된 경우 적응형 비디오 인코딩 설정의 패리티를 적용합니다.

자세한 내용은 [점진적 스트리밍을 위한 비디오 인코딩 프로필 만들기](#creating-a-video-encoding-profile-for-progressive-streaming)를 참조하십시오.
...
**적응형 스트리밍을 위한 비디오 프로파일을 만들려면**,

1. Experience Manager 로고를 누르고 **[!UICONTROL 도구]** > **[!UICONTROL 에셋]** > **[!UICONTROL 비디오 프로필]**&#x200B;으로 이동합니다.
...
선택한 비디오 형식 코덱에 따라 더 많은 설명 또는 권장 설정을 보려면 각 옵션 옆에 있는 정보 아이콘을 누릅니다.
1. 비디오 크기 제목에서 **[!UICONTROL 종횡비 유지]**&#x200B;가 선택되어 있는지 확인합니다.
1. 비디오 프레임 크기 해상도를 픽셀 단위로 설정합니다. 소스 종횡비(폭과 높이 비율)에 맞게 자동으로 비율을 조정하려면 **[!UICONTROL 자동]** 값을 사용합니다. 예를 들어 자동 x 480 또는 640 x 자동을 선택합니다.

1. 다음 중 하나를 수행하십시오.

   * **[!UICONTROL 폭]** 필드에 **[!UICONTROL auto]**&#x200B;를 입력합니다. **[!UICONTROL 높이]** 필드에 픽셀 단위로 값을 입력합니다.

   * 비디오의 크기를 시각화하는 데 도움이 되도록 **[!UICONTROL 높이]**&#x200B;의 오른쪽에 있는 정보 아이콘 (i)을 눌러 크기 계산기 페이지를 엽니다. **[!UICONTROL 크기 계산기]**&#x200B;를 사용하여 원하는 비디오 크기(파란색 상자로 표시됨)를 설정합니다. 완료되면 오른쪽 상단 모서리의 **[!UICONTROL X]**&#x200B;을 누릅니다.

1. (선택 사항) **[!UICONTROL 고급]** 탭을 누르고 **[!UICONTROL 기본값 사용]** 확인란이 선택되었는지 확인합니다(권장). 또는 고급 비디오 및 오디오 설정을 수정합니다.
1. 페이지의 오른쪽 상단에서 **[!UICONTROL 저장]**&#x200B;을 눌러 사전 설정을 저장합니다.
1. 다음 중 하나를 수행하십시오.
   * 4-10단계를 반복하여 인코딩 사전 설정을 더 많이 만듭니다. 적응형 비디오 스트리밍에는 두 개 이상의 비디오 사전 설정이 필요합니다.
   * 다음 단계로 진행합니다.

1. (선택 사항) 이 프로파일이 적용된 비디오에 비디오 스마트 자르기를 추가하려면 다음을 수행합니다.
...
1. 페이지의 오른쪽 상단에서 **[!UICONTROL 저장]**&#x200B;을 다시 눌러 프로필을 저장합니다.
...
적응형 스트리밍에 대해 **[!UICONTROL 인코딩]** 옵션을 사용하지 않도록 선택하면 프로파일에 추가하는 모든 인코딩 사전 설정은 단일 비트 전송률 스트리밍 또는 점진적 비디오 전달에 대한 개별 비디오 변환으로 처리됩니다. 또한 모든 비디오 렌디션의 종횡비가 동일한지 확인할 수 없습니다.

지원되는 비디오 형식 코덱은 H.264(.mp4) 및 WebM입니다.

적응 스트리밍을 위한 비디오 인코딩 프로필 만들기](#creating-a-video-encoding-profile-for-adaptive-streaming)를 참조하십시오.[
...
Experience Manager에서 비디오 프로필을 만들거나 편집할 때 사용자 인터페이스에 없는 고급 비디오 인코딩 매개 변수를 활용하려면 기존 비디오 인코딩 프로필을 편집할 수 있습니다. minBitrate 및 maxBitrate와 같은 고급 매개 변수를 기존 프로파일에 사용자 정의할 수 있습니다.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>인코딩에 사용할 H.264 레벨입니다. 일반적으로 이 레벨은 사용 중인 인코딩 설정에 따라 자동으로 결정됩니다.</td>
   <td><code>String</code></td>
   <td><p>10 * h264 레벨</p> <p>예를 들어 3.0 = 30, 1.3 = 13)입니다.</p> <p>기본값이 없습니다.</p> </td>
  </tr>
  <tr>
   <td><code>keyframe</code></td>
   <td>키프레임 사이의 대상 프레임 수입니다. 2-10초마다 키프레임을 생성하려면 이 값을 계산합니다. 예를 들어 초당 30프레임에서 키프레임 간격은 60-300입니다.<br /> <br /> 키프레임 간격이 낮을수록 적응형 비디오 인코딩에 대한 스트림 검색 및 스트림 전환 동작이 향상되며 동작이 많은 비디오의 품질도 향상시킬 수 있습니다. 그러나 키프레임의 파일 크기가 커지므로 키프레임 간격이 낮을수록 일반적으로 지정된 비트 전송률에서 전반적인 비디오 품질이 떨어집니다.</td>
   <td><code>String</code></td>
...
1. CRXDE Lite 페이지의 왼쪽 상단에서 **[!UICONTROL Back Home]** 아이콘을 눌러 Experience Manager으로 돌아갑니다.
...
1. [비디오 인코딩 사전 설정] 머리글 아래에서 프로파일을 구성하는 비디오 인코딩 사전 설정을 추가, 편집 또는 삭제합니다.
   ...
   선택한 비디오 형식 코덱에 따라 더 많은 설명 또는 권장 설정을 보려면 **[!UICONTROL 기본]** 및 **[!UICONTROL 고급]** 탭의 각 옵션 옆에 있는 정보 아이콘을 누릅니다.
...
폴더에 비디오 프로파일을 할당하면 하위 폴더가 해당 상위 폴더의 프로파일을 자동으로 상속합니다. 따라서 폴더에 하나의 비디오 프로파일만 할당할 수 있습니다. 따라서 에셋을 업로드, 저장, 사용 및 보관하는 폴더 구조를 신중하게 고려하십시오.

폴더에 다른 비디오 프로필을 할당하면 새 프로필은 이전 프로필을 재정의합니다. 이전에 기존 폴더 에셋은 변경되지 않습니다. 새 프로파일은 나중에 폴더에 추가된 에셋에 적용됩니다.

프로파일이 할당된 폴더는 카드 이름에 나타나는 프로파일의 이름을 사용하여 사용자 인터페이스에 표시됩니다.
...
1. Experience Manager 로고를 누르거나 클릭하고 **[!UICONTROL 에셋]**&#x200B;으로 이동한 다음 비디오 프로필을 적용할 폴더로 이동합니다.
...
폴더에 프로파일을 적용하는 것 외에도 Experience Manager 에셋에 업로드된 모든 내용에 선택된 프로파일이 적용되도록 프로파일을 전체적으로 적용할 수도 있습니다.
