...
OpenAPI 기능이 있는 [!DNL Dynamic Media]은(는) DAM을 애자일 및 효율적인 콘텐츠 supply chain 에코시스템의 핵심으로 사용하여 에셋 거버넌스 및 전달을 보장합니다.
...
Cloud Manager에서 사용 가능한 기존 또는 새로운 환경에서 [!UICONTROL Dynamic Media with OpenAPI]의 기본 및 고급 기능을 사용하도록 [Dynamic Media](/help/assets/dynamic-media-open-apis-overview.md) 솔루션을 구성하십시오.
...
다음 단계를 실행하여 [!DNL Dynamic Media] 환경 내에서 구성할 수 있는 새 [!DNL AEM] 회사(계정)를 만들고 나열합니다.
...
1. Dynamic Media 관리자는 [ Cloud Service 환경에서  [!DNL Dynamic Media]](/help/assets/dynamic-media/config-dm.md#architecture-diagram-of-dynamic-media)구성[!DNL AEM] 회사를 시작하는 단계 목록이 포함된 시작 전자 메일을 사서함에서 확인합니다.
...
1. **[!UICONTROL CNAME 구성]** 단계를 실행하여 DNS 서비스 공급자의 DNS 레코드에 `cdn.adobeaemcloud.com`(CNAME 레코드)을(를) 매핑합니다. 이 매핑을 통해 사용자 정의 도메인에서 받은 요청을 Adobe의 CDN으로 리디렉션할 수 있습니다.
