...
* **[!UICONTROL 폭]**  - 이미지의 크기를 고정하려면 값을 픽셀 단위로 입력합니다. 이 값을 공백으로 두면 자산이 적응형으로 설정됩니다.
...
* **[!UICONTROL 종횡비 일치]**  활성화 - Dynamic Media에서 원본 이미지의 종횡비와 가장 잘 일치하는 종횡비로 스마트 자르기 변환을 선택할 수 있도록 하려면 이 옵션을 선택합니다.
...
인터랙티브 미디어 구성 요소는 스마트합니다.이미지를 추가하는지 아니면 비디오를 추가하는지에 따라 다양한 옵션이 제공됩니다. 또한 뷰어는 반응형으로 작동하므로 화면 크기가 자동으로 화면 크기에 따라 변경됩니다. 모든 뷰어는 HTML5 뷰어입니다.
