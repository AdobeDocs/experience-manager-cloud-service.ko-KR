...
Dynamic Media을 관리하는 경우 다음 항목이 유용합니다.
...
>**업그레이드하는 경우:**
>
>* Adobe [!DNL Experience Manager]을(를) 시작하고 실행한 후에는 업로드하는 모든 에셋이 자동으로 Dynamic Media을 사용하도록 설정되어 있습니다(시스템 관리자가 명시적으로 사용하지 않도록 설정하지 않은 경우). 업그레이드된 [!DNL Experience Manager] 인스턴스 중 Dynamic Media을 처음 사용하는 경우 자산을 다시 처리하여 Dynamic Media을 사용하도록 설정해야 합니다. [폴더에서 에셋 재처리](/help/assets/dynamic-media/about-image-video-profiles.md#reprocessing-assets)를 참조하십시오.
