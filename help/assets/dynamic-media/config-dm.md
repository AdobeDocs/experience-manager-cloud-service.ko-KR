...
**Dynamic Media Cloud 서비스를 구성하려면 먼저 다음을 수행하십시오**. Dynamic Media 자격 증명으로 프로비저닝 이메일을 받은 후 Dynamic Media Classic에 [로그인해야](https://www.adobe.com/marketing-cloud/experience-manager/scene7-login.html) 암호를 변경할 수 있습니다. 프로비저닝 이메일에 제공된 암호는 시스템에서 생성되며 임시 암호에만 사용됩니다. Dynamic Media Cloud 서비스가 올바른 자격 증명으로 설정되도록 암호를 업데이트해야 합니다.

다이내믹 미디어 클라우드 서비스를 구성하려면:

1. AEM에서 AEM 로고를 눌러 글로벌 탐색 콘솔에 액세스합니다.
1. 콘솔의 왼쪽에 있는 **[!UICONTROL 도구]** 머리글 아래에서 **[!UICONTROL 클라우드 서비스 > 다이내믹 미디어 구성을 누릅니다]**.
1. Dynamic Media 구성 브라우저 페이지의 왼쪽 창에서 **[!UICONTROL 글로벌]** ( **[!UICONTROL 전역]**&#x200B;왼쪽에 있는 폴더 아이콘을 탭하거나 선택하지 않음)을 탭한 다음 **[!UICONTROL 만들기를]**&#x200B;탭합니다.
1. [다이내믹 미디어 구성 만들기] 페이지에서 제목과 Dynamic Media 계정 이메일 주소, 암호를 입력한 다음 지역을 선택합니다. 이러한 서비스는 제공 이메일에서 Adobe가 제공합니다. 수신하지 못한 경우 지원 센터에 문의하십시오.
...
1. 연결이 성공하면 다음을 설정할 수 있습니다.
   ...
   * **[!UICONTROL 자산 게시]** - 다음 세 가지 옵션 중에서 선택할 수 있습니다.
      ...
      * **[!UICONTROL 선택적 게시]** (Selective Publish)를 사용하면 에셋이 보안 미리 보기만을 위해 자동으로 게시되며 공개 도메인에서 배달을 위해 DMS7에 게시하지 않고도 AEM에 명시적으로 게시할 수 있습니다. 향후 Adobe는 상호 배타적인 AEM에 자산을 게시하고 Dynamic Media에 자산을 게시하기 위한 이 옵션을 개선합니다. 즉, DMS7에 자산을 게시하여 스마트 자르기 또는 동적 표현물과 같은 기능을 사용할 수 있습니다. 또는 AEM에서만 자산을 게시하여 미리 볼 수 있습니다. 동일한 에셋은 공용 도메인에 전달되기 위해 DMS7에 게시되지 않습니다.
...
Dynamic Media Classic(Scene7) 사용자 인터페이스를 사용하여 동적 미디어 설정을 변경합니다.
...
AEM Assets에서 지원되지 않는 포맷에 대한 사용자 지정 MIME 유형을 추가할 수 있습니다. CRXDE Lite에 추가하는 새 노드가 AEM에서 삭제되지 않도록 하려면 MIME 유형을 먼저 이동해야 하며, 활성화된 값이 `image_` false로 ****&#x200B;설정되어 있어야 합니다.
...
1. 최대 **[!UICONTROL 병렬 작업]** 필드에서 숫자를 원하는 값으로 변경합니다.
   ...
   Adobe에서는 Dynamic Media Classic(Scene7)에 **[!UICONTROL 대용량 파일 업로드를 지원하기 위해 32개의 최대 병렬 작업을]** 구성할 것을 권장합니다.
