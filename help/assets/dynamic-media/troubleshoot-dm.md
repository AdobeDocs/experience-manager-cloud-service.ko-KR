...
CRXDE Lite에서 다음 에셋 속성을 검토하여 Adobe Experience Manager에서 Dynamic Media로의 에셋의 성공적인 동기화를 확인할 수 있습니다.
...
동기화 오류 및 문제가 `error.log`에 기록됩니다(Experience Manager 서버 디렉터리 `/crx-quickstart/logs/`). 충분한 로깅을 사용하여 대부분의 문제의 근본 원인을 확인할 수 있지만 자세한 정보를 수집하기 위해 Sling 콘솔(`com.adobe.cq.dam.ips`https://localhost[/system/console/slinglog:4502)을 통해 &#x200B;](https://localhost:4502/system/console/slinglog) 패키지의 DEBUG에 대한 로깅을 늘릴 수 있습니다.
...
1. Dynamic Media 동기화 폴더(예: `<sync-folder>/_CSS/_OOTB`) 내의 `/content/dam/_CSS/_OOTB` 폴더로 이동합니다.
...
1. 다음 속성 및 문자열 리터럴 값을 연결하여 Dynamic Media에서 직접 아트워크를 요청합니다.
...
### 문제: 대역폭 및 저장소 탭을 열 때 오류 #2046 {#error-2046-bandwidth-storage}

**디버깅하는 방법**

![Dynamic Media Classic의 대역폭 및 저장소 탭에 오류가 #2046](assets/2046-error.png)

* 사용자가 Dynamic Media Classic(Scene7) 데스크탑 애플리케이션에서 대역폭 및 저장소 탭을 열 때 오류 #2046이 발생합니다.
* 이 문제는 Adobe AIR 프레임워크에서 사용하는 캐시된 RSL(Runtime Shared Library)의 만료된 디지털 서명 인증서로 인해 발생합니다.
* 로컬 인증서를 다시 확인하는 동안 오류가 발생합니다.

**솔루션**

Adobe AIR에서 업데이트된 RSL(런타임 공유 라이브러리)을 강제로 다운로드하려면 로컬 캐시를 지우십시오.

**macOS**

1. 다음으로 이동합니다.
   `~/Library/Caches/Adobe/Flash Player/AssetCache/<folder>/`
2. `.swz` 및 `.heu` 파일을 모두 삭제합니다.

**Windows**

1. 다음으로 이동합니다.
   `%APPDATA%\Adobe\Flash Player\AssetCache\<folder>\`
2. 폴더 내의 모든 파일을 삭제합니다.

캐시를 지운 후 응용 프로그램을 다시 시작합니다.
...
1. 클라우드 서비스에서 [Dynamic Media 구성을 다시 만듭니다](/help/assets/dynamic-media/config-dm.md#configuring-dynamic-media-cloud-services).