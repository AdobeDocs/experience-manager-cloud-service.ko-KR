...
1. AEM 로고를 누르고 도구 > 자산 **[!UICONTROL > 메타데이터 프로필로]**&#x200B;이동한 다음 만들기를 **[!UICONTROL 탭합니다]**.
1. 메타데이터 프로필의 제목(예: 샘플 메타데이터)을 입력하고 제출을 **[!UICONTROL 탭합니다]**. 메타데이터 프로필에 대한 양식 편집이 표시됩니다.
...
설명 구성 요소에 대해 다음 속성을 **[!UICONTROL 편집합니다]** .
   ...
   * **[!UICONTROL 기본값]** - 이 속성을 사용하여 메타데이터 구성 요소의 기본값을 추가합니다. 예를 들어 &quot;My description&quot;을 지정하면 이 값이 자산의 메타데이터 `dc:desc` 노드의 속성에 할당됩니다.
      ...
      >[!NOTE]
      >
      >새 메타데이터 속성에 기본값 추가(에 이미 존재하지 않음) `/jcr:content/metadata` node) does not display the property and its value on the asset&#39;s Properties page by default. 자산의 속성 페이지에서 새 속성을 보려면 해당 스키마 양식을 수정합니다.

1. (선택 사항) 양식 작성 탭에서 양식 **[!UICONTROL 편집에 구성 요소를]** 더 추가하고 설정 **[!UICONTROL 탭에서 해당 속성을]** 구성합니다. 양식 작성 탭에서 다음 속성을 사용할 **[!UICONTROL 수]** 있습니다.
...
1. **[!UICONTROL Done]**&#x200B;을 누릅니다. 메타데이터 프로필은 메타데이터 프로필 **[!UICONTROL 페이지의 프로필 목록에 추가됩니다]** .

## 메타데이터 프로필 복사 {#copying-a-metadata-profile}

1. 메타데이터 **[!UICONTROL 프로필]** 페이지에서 메타데이터 프로필을 선택하여 복사본을 만듭니다.
1. 도구 **[!UICONTROL 모음에서]** 복사를 누릅니다.
1. 메타데이터 **[!UICONTROL 프로필 복사]** 대화 상자에서 메타데이터 프로필의 새 사본에 대한 제목을 입력합니다.
1. 복사를 **[!UICONTROL 누릅니다]**. 메타데이터 프로필 사본은 [메타데이터 프로필] 페이지의 프로필 목록에 **[!UICONTROL 나타납니다]** .
...
1. 폴더에서 확인 표시를 탭하거나 클릭하여 선택한 다음 속성을 탭하거나 **클릭합니다**.
1. 메타데이터 **[!UICONTROL 프로필]** 탭을 선택하고 드롭다운 메뉴에서 프로필을 선택한 다음 저장을 **누릅니다**. 프로파일이 이미 할당된 폴더는 폴더 이름 바로 아래에 프로필 이름을 표시하여 표시됩니다.
...
* 해당 프로필로 `https://<AEM server>/mnt/overlay/dam/gui/content/assets/foldersharewizard.html/content/dam` 이동하여 적용하고 저장을 **누릅니다**.

* CRXDE Lite에서 다음 노드로 이동합니다. `/content/dam/jcr:content`Adobe 속성을 `metadataProfile:/etc/dam/metadata/dynamicmedia/<name of metadata profile>` 추가하고 모두 저장을 **누릅니다**.
...
[도구] 메뉴 내에서 또는 폴더에 있는 **경우** [속성]에서 메타데이터 프로필을 제거할 수 **있습니다**. 이 섹션에서는 폴더에서 메타데이터 프로필을 제거하는 방법을 설명합니다.

### 프로필 사용자 인터페이스를 통해 폴더에서 메타데이터 프로필 제거 {#removing-metadata-profiles-from-folders-via-profiles-user-interface}

1. AEM 로고를 탭하거나 클릭하고 도구 > 자산 **[!UICONTROL > 메타데이터 프로필로 이동합니다]**.
...
1. 메타데이터 **[!UICONTROL 프로필]** 탭을 선택하고 드롭다운 **[!UICONTROL 메뉴에서]** 없음을 선택하고 저장을 **[!UICONTROL 클릭합니다]**. 프로파일이 이미 할당된 폴더는 폴더 이름 바로 아래에 프로필 이름을 표시하여 표시됩니다.
