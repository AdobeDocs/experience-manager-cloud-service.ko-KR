...
Adobe Express 및 AEM Assets Content Hub에 액세스할 수 있는 권한이 있고, 자산을 새 변형에 다시 혼합할 수 있는 권한이 있는 사용자는 AEM Assets Content Hub을 사용하여 이미지를 편집할 수 있습니다.

### AEM Assets Content Hub에서 Adobe Express을 사용하여 이미지를 편집하려면 어떻게 합니까?

AEM Assets Content Hub에서 이미지를 편집하려면 이미지의 에셋 카드에서 **Adobe Express에서 열기**&#x200B;를 클릭하거나 이미지의 세부 정보를 열고 Adobe Express 로고를 클릭하십시오. 이렇게 하면 AEM Assets Content Hub 내에 임베드된 Adobe Express 편집기가 로드되므로 플랫폼을 종료하지 않고도 편집할 수 있습니다.

### Adobe Express은 AEM Assets Content Hub 내에서 어떤 편집 기능을 제공합니까?

Adobe Express은 AEM Assets Content Hub 내의 다양한 이미지 편집 기능을 제공합니다. 여기에는 이미지 크기 조정, 배경색 제거 또는 변경, 이미지 자르기, AI 생성 이미지 또는 텍스트와 결합 등이 포함됩니다.

### AEM Assets Content Hub에서와 같이 편집한 이미지를 저장할 수 있는 파일 형식은 무엇입니까?

편집한 이미지를 AEM Assets Content Hub에서 PNG(고품질), JPG(작은 파일에 적합) 또는 PDF(문서에 적합) 형식으로 저장할 수 있습니다.

### AEM Assets Content Hub에서 편집된 이미지를 저장할 때 어떤 필드를 입력해야 합니까?

AEM Assets Content Hub에서 편집된 이미지를 저장할 때 **다른 이름으로 저장** 필드에 자산 이름을 지정하고 **캠페인 이름** 필드에 캠페인 이름을 지정해야 합니다. 또한 Adobe에서는 검색 기능 및 구성을 향상시키기 위해 키워드, 채널, 시간대 및 지역 과 같은 추가 필드에 값을 지정할 것을 권장합니다.

### AEM Assets Content Hub에서 편집 내용을 새 자산으로 저장할 수 있습니까?

예. 편집한 후 **새 자산으로 저장**&#x200B;을 클릭하여 변경 내용을 AEM Assets Content Hub의 새 자산으로 저장할 수 있습니다.
