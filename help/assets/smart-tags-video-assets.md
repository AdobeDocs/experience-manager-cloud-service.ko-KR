...
# 스마트 태그 지정 비디오 에셋 {#video-smart-tags}
...
스마트 태그 지정에 지원되는 비디오 파일 포맷(및 코덱은 MP4(H264/AVC), MKV(H264/AVC), MOV(H264/AVC, 모션 JPEG), AVI(indo4), FLV(H2) 64/AVC, vp6f) 및 WMV(WMV2) 또한 이 기능을 통해 최대 300MB의 비디오에 태그를 지정할 수 있습니다. 비디오 에셋의 자동화된 태깅은 비디오가 업로드된 후 또는 재처리가 트리거될 때 표준 에셋 처리(축소판 생성 및 메타데이터 추출 포함)로 이루어집니다. 스마트 태그는 자산 속성에서 [신뢰 점수](#confidence-score-video-tag) 내림차순으로 [!UICONTROL 표시됩니다]. 비디오 태그 지정은 기본적으로 Cloud Service [!DNL Adobe Experience Manager] 로 활성화됩니다. 그러나 폴더에서 비디오 스마트 태그 [를](#opt-out-video-smart-tagging) 옵트아웃할 수 있습니다.
...
적용된 스마트 태그는 [스마트 태그](#confidence-score-video-tag)내에서 개체 및 작업 태그에 대해 조합된 [!UICONTROL 신뢰 점수]내림차순으로정렬됩니다.
...
DAM의 기존 비디오 에셋은 자동으로 지능적인 태그가 지정되지 않습니다. 자산에 대한 스마트 태그를 [!UICONTROL 생성하려면 자산을] 수동으로 재처리해야 합니다.

자산 저장소에 이미 존재하는 자산의 스마트 태그 비디오 자산 또는 폴더(하위 폴더 포함)를 만들려면 다음 단계를 수행하십시오.

1. 로고를 [!DNL Adobe Experience Manager] 선택한 다음 탐색 [!UICONTROL 페이지에서 자산을] 선택합니다.

1. 파일 [!UICONTROL 을] 선택하여 자산 인터페이스를 표시합니다.

1. 스마트 태그를 적용할 폴더로 이동합니다.

1. 전체 폴더 또는 특정 비디오 자산을 선택합니다.

1. 자산 ![재처리 아이콘](assets/do-not-localize/reprocess-assets-icon.png) 자산 [!UICONTROL 재처리] 아이콘을 선택하고 [!UICONTROL 전체 프로세스] 옵션을 선택합니다.

![자산 재처리를 통해 기존 DAM 저장소에 태그 추가](assets/reprocess.gif)

프로세스가 완료되면 폴더 내의 비디오 [!UICONTROL 자산의 속성] 페이지로 이동합니다. 자동으로 추가된 태그는 [!UICONTROL 기본] 탭의 [!UICONTROL 스마트 태그] 섹션에표시됩니다. 이러한 적용된 스마트 태그는 [신뢰 점수의 내림차순으로 정렬됩니다](#confidence-score-video-tag).

## 태그 있는 비디오 검색 {#search-smart-tagged-videos}

자동 생성된 스마트 태그를 기준으로 비디오 자산을 검색하려면 Omnissearch를 [사용합니다](search-assets.md#search-assets-in-aem).
...
검색 결과는 메타데이터의 검색어 키워드와 검색된 키워드가 포함된 비디오 자산과 검색된 키워드로 스마트 태그가 지정된 비디오 자산의 조합입니다. 하지만 메타데이터 필드의 모든 검색어와 일치하는 검색 결과가 먼저 표시되고, 그 다음에 스마트 태그의 검색어와 일치하는 검색 결과가 표시됩니다. 자세한 내용은 스마트 태그를 [ [!DNL Experience Manager] 사용하여 검색 결과 이해를 참조하십시오](smart-tags.md#understandsearch).
...
자산에 대한 스마트 태그를 중재하는 방법에 대한 자세한 내용은 스마트 태그 [관리를 참조하십시오](smart-tags.md#manage-smart-tags-and-searches).

![비디오 스마트 태그 중재](assets/manage-video-smart-tags.png)

>[!NOTE]
>
>스마트 태그 [관리의 단계를 사용하여 중재되는](smart-tags.md#manage-smart-tags-and-searches) 태그는 자산의 재처리 시 기억되지 않습니다. 원래 태그 세트가 다시 표시됩니다.
...
1. 폴더 [!UICONTROL 속성에서 자산 처리] 탭을 [!UICONTROL 엽니다].

1. 비디오용 [!UICONTROL 스마트 태그] 메뉴에서 [!UICONTROL 상속됨] 옵션이 기본적으로 선택되고 비디오 스마트 태그가 활성화됩니다.

   [ [!UICONTROL 상속됨] ] 옵션을 선택하면 상속된 폴더 경로가 [사용] 또는 [사용 안 함]으로 설정되어 있는지 여부 [!UICONTROL 와 함께] [!UICONTROL 표시됩니다].

   ![비디오 스마트 태그 지정 비활성화](assets/disable-video-tagging.png)

1. 폴더에 업로드된 비디오의 스마트 태그 지정 [!UICONTROL 을] 옵트아웃하려면 비활성화를 선택합니다.

>[!IMPORTANT]
>
>업로드 시 폴더에 비디오에 태그 지정이 없고 업로드 후 비디오에 스마트 태그를 지정하려면 **[!UICONTROL 속성]** 및 자산 처리 [!UICONTROL 옵션 사용] 스마트Video에 스마트 태그를 추가하도록 선택한 Asset ProcessOption [!UICONTROL 의 [] 자산 처리 [ 탭에서 비디오에 대한] 스마트 태그](#smart-tag-existing-videos) 사용을선택합니다.
...
작업 및 개체 태그의 기본 임계값은 0.7(0과 1 사이의 값이어야 함) [!DNL Adobe Experience Manager] 입니다. 일부 비디오 자산에 특정 태그가 지정되어 있지 않은 경우 알고리즘이 예측된 태그에 70% 미만임을 나타냅니다. 기본 임계값이 모든 사용자에게 항상 최적의 것은 아닐 수 있습니다. 따라서 OSGI 구성에서 신뢰 점수 값을 변경할 수 있습니다.

Cloud Manager를 통해 Cloud Service으로 배포된 프로젝트에 신뢰 점수 OSGI 구성 [!DNL Adobe Experience Manager] 을 추가하려면:

* 프로젝트(Tranype 24 또는 이전 [!DNL Adobe Experience Manager] 에`ui.config` )에 `ui.apps`OSGi 구성 `config.author` 에 다음 내용이 포함된 구성 파일 `com.adobe.cq.assetcompute.impl.senseisdk.SenseiSdkImpl.cfg.json` 을 포함합니다.
