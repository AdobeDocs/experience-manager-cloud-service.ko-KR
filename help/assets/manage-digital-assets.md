...
이 문서에서는 Adobe Experience Manager Assets의 자산을 관리하고 편집하는 방법에 대해 설명합니다. 컨텐츠 조각을 관리하려면 컨텐츠 조각 [자산을](content-fragments/content-fragments.md) 참조하십시오.
...
1. 미리 보기 모드에서는 [지원되는 이미지 유형](/help/assets/file-format-support.md) (대화형 편집 포함)에 대해 확대/축소 옵션을 사용할 수 있습니다.
   ...
   재설정 **[!UICONTROL 을]** 눌러 보기를 원래 크기로 재설정합니다.
...
1. 자산을 복사할 위치로 이동합니다.
   ...
   >[!NOTE]
   >
   >동일한 위치에서 자산을 복사하면 AEM은 변형된 이름을 자동으로 생성합니다. 예를 들어 제목이 지정된 자산을 복사하면 AEM이 자동으로 복사본 제목 `Square`을 생성합니다 `Square1`.
...
가장 적절한 자산을 검색하고 사용하기 위한 단순, 고급 및 사용자 지정 검색에 대해서는 AEM에서 [자산 검색을 참조하십시오](/help/assets/search-assets.md).
