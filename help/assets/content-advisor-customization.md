---
title: 콘텐츠 관리자 사용자 지정
description: 함수를 사용하여 애플리케이션 내에서 Content Advisor를 사용자 지정합니다.
role: Admin, User
...
# 콘텐츠 관리자 사용자 지정 {#content-advisor-customization}

콘텐츠 관리자를 사용하면 환경 설정, 요구 사항 또는 기능 요구 사항에 따라 다양한 구성 요소를 사용자 지정할 수 있습니다. 다음 구성 요소를 사용자 지정할 수 있습니다. [Micro-Frontend 콘텐츠 관리자](#overview-asset-selector.md):
...
콘텐츠 관리자를 사용하여 만료된 에셋의 사용을 제어할 수 있습니다. 현재 날짜로부터 30일 이내에 만료되는 에셋에 대해 미리 알 수 있도록 **곧 만료** 배지로 만료된 에셋을 사용자 지정할 수 있습니다. 또한 이는 요구 사항에 따라 사용자 정의할 수 있습니다. 캔버스에서 만료된 에셋을 선택하거나 그 반대로 할 수도 있습니다. 만료된 에셋의 사용자 지정은 다양한 방법으로 일부 코드 조각을 사용하여 수행할 수 있습니다.
...
만료된 에셋의 사용을 선택 또는 선택 취소하도록 사용자 정의할 수 있습니다. 콘텐츠 관리자 캔버스에서 만료된 에셋을 드래그하여 놓을 것인지 여부를 사용자 정의할 수 있습니다. 이렇게 하려면 다음 매개 변수를 사용하여 캔버스에서 에셋을 선택할 수 없도록 합니다.
...
콘텐츠 관리자를 사용하여 태그 선택기 필터를 추가할 수 있습니다. 모든 관련 태그를 특정 태그 그룹에 결합하는 태그 그룹을 지원합니다. 또한 찾고 있는 자산에 해당하는 추가 태그를 선택할 수 있습니다. 또한 사용자가 주로 사용하는 상황별 호출 필터 아래에 기본 태그 그룹을 설정하여 이동 중에 액세스할 수도 있습니다.
...
## 컨텐츠 권고자에서 업로드 {#upload-in-content-advisor}

로컬 파일 시스템에서 Content Advisor로 파일이나 폴더를 업로드할 수 있습니다. 로컬 파일 시스템을 사용하여 파일을 업로드하려면 일반적으로 Content Advisor 마이크로 프론트엔드 애플리케이션에서 제공하는 업로드 기능을 사용해야 합니다. 콘텐츠 관리자에서 업로드를 호출하는 데 필요한 `upload`개의 다양한 코드 조각은 다음과 같습니다.
...
*추가 속성에는 `metadataSchema`, `onMetadataFormChange`, `targetUploadPath`, `hideUploadButton`, `onUploadStart`, `importSettings` `onUploadComplete`, `onFilesChange`,`uploadingPlaceholder`*&#x200B;이(가) 포함됩니다. 자세한 내용은 [콘텐츠 관리자 속성](/help/assets/content-advisor-properties.md)을 참조하세요.
