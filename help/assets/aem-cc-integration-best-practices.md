---
title: ' [!DNL Adobe Creative Cloud]과(와) 통합하는 우수 사례'
...
Adobe Experience Manager(AEM) Assets는 DAM 사용자가 크리에이티브 팀과 공동 작업을 할 수 있도록 Adobe Creative Cloud과 통합하여 콘텐츠 제작 프로세스의 공동 작업을 간소화할 수 있는 디지털 에셋 관리(DAM) 솔루션입니다.
...
>[!NOTE]
>
>AEM에서 Creative Cloud으로 폴더 공유가 이제 더 이상 사용되지 않으며 더 이상 아래에서 다루지 않습니다. Adobe에서는 크리에이티브 사용자가 AEM에서 관리되는 에셋에 액세스할 수 있도록 Adobe Asset Link 또는 AEM 데스크탑 앱과 같은 새로운 기능을 권장합니다.
...
|---|---|---|
| 크리에이티브 사용자는 Creative Cloud 앱을 종료하지 않고도 AEM에서 에셋을 검색하고, 열어 사용하고, AEM에 대한 변경 사항을 편집 및 업로드하고, 새로운 파일을 AEM에 업로드할 수 있습니다. | [Adobe Asset Link](https://helpx.adobe.com/kr/enterprise/using/adobe-asset-link.html) | Photoshop, Illustrator 및 InDesign |
...
| 사용 사례 | Adobe Asset Link | AEM Desktop App | 발언 또는 대체 방법 |
|----------------------------------------------------------|-----------------------------------------------------------------------------------|--------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------|
| Discover - AEM 폴더 찾아보기 | 예 | AEM 웹 UI + 데스크탑 작업 | 네트워크 공유를 검색할 때 에셋의 바이너리 파일을 다운로드하지 않도록 축소판을 끕니다. |
| Discover - AEM 컬렉션에 액세스 | 예 | AEM 웹 UI + 데스크탑 작업 |  |
| Discover - AEM에서 자산 검색 | 예 | AEM 웹 UI + 데스크탑 작업 |  |
| 사용 - 자산 열기 | 예 | 예 - 모든 앱 | [Finder에서 웹 ](/help/assets/manage-digital-assets.md#previewing-assets) 인터페이스에서 열기 |
| 사용 - AEM의 에셋을 문서로 가져오기 | 예 - 포함 | 예 - 링크 또는 포함 | AEM 데스크탑 앱을 사용하면 로컬 파일 시스템의 파일로 에셋에 액세스할 수 있습니다. 기본 앱에서 이러한 링크는 로컬 경로로 표시됩니다. |
...
| 편집 - AEM 외부에서 진행 중인 작업 | 예 - 데스크탑에 동기화되는 사용자의 Creative Cloud 저장소 계정에서 사용할 수 있는 에셋입니다. | 예 |  |
...
| 업로드 - 여러 파일/계층적 폴더 구조 | 아니오 | 예 | [웹 인터페이스를 통해 업로드](/help/assets/manage-digital-assets.md#uploading-assets);사용자 정의 스크립팅 또는 도구 |
...
* [AEM Assets 웹](/help/assets/manage-digital-assets.md) 은 AEM 액세스 제어 설정에 의해 보안이 설정되고 필요한 IT/네트워크 구성 조정을 통해 외부 당사자를 위한 영역과 상호 작용하여 외부 사용자가 AEM에 액세스할 수 있도록 합니다.
...
AEM 및 Creative Cloud 통합에 대한 모범 사례에 대한 간단한 요약입니다. 이 문서의 나머지 부분을 참조하여 이러한 내용을 자세히 파악하십시오.
...
일반적으로 사내 크리에이티브 팀 또는 내부 네트워크에 연결된 에이전시/크리에이티브 전문가는 AEM 로그인을 비롯한 DAM 인스턴스에 액세스할 수 있습니다. AEM 및 네트워크 인프라를 설정하여 외부 파트너(일반적으로 클라이언트 관련 에이전시 등 신뢰할 수 있는 조직)에 직접 액세스하면 VPN 또는 IP 허용 목록을 통해 AEM을 이용할 수 있습니다.
...
* 외부 파트너에게 자산을 안전하게 배포하려면 [AEM Assets 브랜드 포털](https://helpx.adobe.com/experience-manager/brand-portal/user-guide.html)을 사용합니다.
...
이 문서에서 설명한 바와 같이, 로컬 파일에 저장된 모든 편집 내용을 변경 사항으로 AEM에 업로드하지 않고도 진행 중인 작업이라고도 하는 자산에 대한 주요 업데이트를 수행하는 것이 좋습니다. 이를 통해 데스크탑 사용자의 작업 시간을 단축하고, 사용된 네트워크 대역폭을 제한하며, 에셋 타임라인을 정리하고 제어되고 주요 업데이트에 집중할 수 있습니다.
...
* 에셋이 백그라운드에서 다운로드되고 Creative Cloud 데스크탑 앱별로 디스크에 동기화된 사용자 Creative Cloud 계정에 넣으면 자산에 있는 AEM에서 체크아웃 플래그가 전환되어 편집 충돌을 최소화할 수 있습니다
...
* 크리에이티브 사용자가 변경 작업을 완료하면 Creative Cloud 응용 프로그램에서 해당 파일에 대한 체크 인 작업을 선택적으로 실행할 수 있습니다. AEM의 해당 에셋은 버전을 관리하고 새 바이너리로 업데이트합니다. 마케터나 LOB 사용자와 같은 AEM 사용자는 AEM 에셋 타임라인 UI를 통해 주요 에셋 변경 사항 또는 이정표에 액세스할 수 있습니다.

AEM 데스크탑 앱은 기본 앱에서 열린 자산에 대한 네트워크 공유를 제공합니다. 기본적으로 로컬에서 수행한 모든 변경 사항은 잠시 후 AEM에 자동으로 업로드됩니다. 이러한 구성을 통해 진행 중인 작업 중 빈번한 저장 작업은 모두 AEM에 업로드되고 버전을 관리하게 되므로 AEM에서 불필요한 버전은 물론 네트워크 트래픽과 잠재적인 확장성 문제를 야기합니다.

여기서 권장되는 방법은 AEM 데스크탑 앱의 옵션을 사용하여 자동화된 업데이트를 끄고, 자산에 대한 변경 사항을 AEM에 수동으로 업로드하여 앱의 에셋 상태 UI에서 변경 사항 업로드 작업을 이용하는 것입니다.
...
이 설명은 데스크톱 사용자의 워크플로우에서 정상적인 부분으로 파일(예: 매주 또는 모든 사진 촬영)을 원활하게 업로드하는 것을 말합니다. 대규모 자산 마이그레이션은 여기에서 다루지 않습니다.

다음과 같은 업로드 기능을 활용할 수 있습니다.

* 대용량/계층적 폴더를 일괄적으로 업로드하려면 [폴더 업로드](https://helpx.adobe.com/experience-manager/desktop-app/aem-desktop-app.html#bulkupload) 기능을 제공하는 AEM 데스크탑 앱을 사용합니다. 계층 폴더 구조를 업로드할 수도 있습니다. 에셋은 백그라운드에서 업로드되므로 웹 브라우저 세션과 연결되지 않습니다.
...
네트워크 파일 공유를 사용하여 디지털 자산을 관리하는 경우, AEM 데스크탑 앱이 매핑한 네트워크 공유를 사용하는 것만으로도 편리한 대체품으로 보일 수 있습니다. 네트워크 파일 공유에서 전환할 때 AEM 웹 인터페이스는 네트워크 공유(검색, 컬렉션, 메타데이터, 공동 작업, 미리 보기 등)에서 가능한 것 이상으로 풍부한 디지털 자산 관리 기능을 제공하고 AEM 데스크탑 앱은 서버측 DAM 저장소를 데스크탑에서 작업과 연결하는 편리한 링크를 제공합니다.
