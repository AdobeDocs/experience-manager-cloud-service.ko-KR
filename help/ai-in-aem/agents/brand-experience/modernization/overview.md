...
[Brand Experience Agent의 일부로](/help/ai-in-aem/agents/brand-experience/overview.md) Experience Modernation Agent는 웹 사이트 마이그레이션과 지속적인 발전을 빠르고 원활하게 만들어 Edge Delivery Services(AEM 작성 포함)의 모든 가치를 잠금 해제합니다.

초기 웹 사이트 온보딩을 위한 [사이트 생성 및 마이그레이션 기술](#creation-migration)과 지속적인 경험 개발을 위한 [개발 기능 차단](#block-development)(스타일 업데이트, 템플릿 개선, 랜딩 페이지 생성)을 결합합니다. 또한 호스팅된 AI 지원 개발 환경으로 [경험 현대화 콘솔](#console)을 직접 사용할 수 있습니다. 사용자는 해당 콘솔을 통해 직접 에이전트를 작동할 수 있지만, 개발자는 어떤 선박에 대한 모든 권한을 유지합니다.

또한 복잡한 마이그레이션의 성공을 보장하기 위해 Adobe은 [AOE(Agentic Outcome Engineer) 게재 모델](#delivery-model)을 제공합니다. 이 옵션은 특정 프로젝트 문제를 차단하는 데 도움이 되는 액셀러레이터 또는 전술적 서비스로 사용할 수 있습니다.

## 이점 {#benefits}

Experience 현대화 에이전트는 [Edge Delivery Services](/help/edge/overview.md) 채택의 가치 창출 시간을 단축하고 브랜드의 웹 경험을 조정할 수 있는 민첩성을 제공합니다.

* **빠른 속도**: AI 자동화는 반복적인 마이그레이션 작업(콘텐츠 가져오기, 블록 매핑, 디자인 시스템 응용 프로그램)을 처리하여 수개월의 노력을 수주로 압축합니다.
* **비용 효율적인**: 자동화는 반복적인 작업을 처리하므로 통합 및 전략적 의사 결정과 같은 고가치 작업에 전문 서비스를 사용할 수 있습니다.
...
* **지속적인 값**: 에이전트는 스타일 업데이트, 템플릿 세분화 및 랜딩 페이지 만들기를 포함하여 지속적인 사이트 진화를 지원합니다
...
* 모든 CMS, 레거시 AEM 또는 디자인 시스템(예: Figma)의 웹 사이트를 프로덕션 준비 Edge Delivery Services 프로젝트로 변환합니다
* 무제한 기능에 대한 AI 준비, 빠른 성능(핵심 웹 바이탈 최적화), 접근성(WCAG 2.1 AA), 모든 중단점에 대한 응답형 디자인, 콘텐츠+코드 민첩성 등 모든 Edge Delivery Services 약속을 제공합니다.
...
Experience Modernation Agent는 [`aemcoder.adobe.io`.](https://aemcoder.adobe.io)에서 웹 인터페이스로 노출되는 Edge Delivery Services용 호스팅 AI 지원 개발 환경을 제공합니다.
...
* 개발자가 일반적인 GitHub 검토 및 승인 프로세스를 통해 제공되는 내용에 대한 모든 권한을 보유함에 따라 기업 거버넌스가 시행됩니다.

셀프 서비스 경험 현대화 콘솔 은 일반적으로 사용할 수 있습니다. 관심이 있는 사용자는 원활한 온보딩 경험을 보장하기 위해 액세스 권한을 요청할 수 있습니다.

## 게재 모델 {#delivery-model}

복잡한 마이그레이션 또는 가속화된 결과를 위해 Adobe은 AOE(Agentic Outcome Engineer) 제공 모델을 제공합니다. Adobe 엔지니어가 사용자를 대신하여 AI 도구를 작동하는 선택적 서비스입니다.

* Adobe AOE는 AI 자동화와 전문가 지침을 결합하여 규모에 맞게 제작 준비가 된 결과를 제공하는 에이전트를 사용자와 함께 운영합니다.
* 따라서 구현 중단 또는 기존 현대화 과제에 직면한 기업을 위한 전략적 재설정 옵션이 제공됩니다.
* AOE 모델은 거버넌스, 품질 및 성공적인 결과를 보장하면서 AI 자동화를 활용하는 더 빠르고 위험이 적은 경로를 제공합니다.

AOE 게재 모델을 자세히 살펴보려면 다음 작업을 수행하십시오.
...
다음 요구 사항은 아직 전용 자동화 기술에서 다루지 않으며 수작업이 필요합니다.
