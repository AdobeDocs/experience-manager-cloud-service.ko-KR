...
1. **먼저 단일 페이지 마이그레이션을 실행하십시오.** - 일괄 가져오려는 템플릿의 대표 페이지 1개를 마이그레이션합니다.
   * 이렇게 하면 필요한 가져오기 인프라가 만들어집니다.
1. **작은 페이지 집합에서 일괄 가져오기를 실행합니다.** - 에이전트에게 일괄 가져오기를 실행하도록 요청하고 동일한 템플릿 다음에 오는 간단한 URL 목록을 제공합니다.
1. **결과를 검토하고 구체화합니다.** - 가져온 페이지를 검사합니다.
   * 잘못된 것으로 보이는 것이 있으면 에이전트에게 파서, 변환기 또는 가져오기 논리를 조정하도록 요청하십시오.
1. **확장** - 결과가 올바르게 표시되면 URL의 전체 목록을 제공합니다.
...
* 다음 워크플로우를 권장합니다.
   ...
   1. 페이지 기준을 실행하여 전체 유효성 검사를 수행합니다.

### 블록 마이그레이션 구성 {#figma-block-migration}

이 프롬프트를 사용하여 블록을 Figma 디자인에서 Edge Delivery Services으로 마이그레이션합니다.
...
* 이 스킬을 실행하면 호스팅 환경에서 다음 단계가 자동으로 수행됩니다.
   ...
   1. **Edge Delivery Services 콘텐츠 생성** — 에이전트는 Edge Delivery Services 블록 구조에 따라 Markdown 파일을 만듭니다
...
### Figma 파생 블록을 사용하여 마이그레이션 재디자인 {#figma-redesign-migration}

기존 웹 사이트를 재설계된 환경으로 마이그레이션하는 경우 이 프롬프트를 사용하십시오.

이 워크플로우에서는 먼저 Figure에서 대상 블록 컬렉션을 만듭니다. 그런 다음 사이트 마이그레이션은 라이브 소스 웹 사이트에 대해 실행되며 소스 콘텐츠를 Figma에서 만든 블록에 매핑합니다.

* **그림**&#x200B;은(는) 대상 디자인 및 블록 라이브러리 원본입니다.
* **라이브 웹 사이트**&#x200B;는 콘텐츠 소스로 유지됩니다.

#### 프롬프트 예 {#example-figma-redesign}

1. Figure에서 블록 컬렉션을 만듭니다.

   * &quot;다음 Figure 구성 요소에서 Edge Delivery Services 블록 컬렉션을 만듭니다. `https://figma.com/design/{fileKey}?node-id={nodeId}`&quot;

1. 소스 콘텐츠를 다음 블록으로 마이그레이션:

   * &quot;이러한 페이지를 마이그레이션하고 콘텐츠를 Figma 파생 블록 컬렉션(URL1, URL2, URL3)에 매핑합니다.&quot;

#### 알아 두어야 할 사항 {#wtk-figma-redesign}

* Figma는 재설계된 블록 세트를 설정하기 위해 먼저 사용됩니다.
* 그런 다음 사이트 마이그레이션은 실제 웹 사이트 콘텐츠를 해당 블록 세트에 매핑합니다.
* **콘텐츠 유효성 검사**&#x200B;는 원본 웹 사이트에 대해 수행됩니다.
* **시각적 유효성 검사**&#x200B;는 Figma 파생 블록 컬렉션 및 디자인 시스템에 대해 수행됩니다.
* 새 블록 변형은 기존 피규어 파생 블록이 소스 콘텐츠를 나타낼 수 없는 경우에만 만들어야 합니다.

#### 권장 워크플로우 {#figma-redesign-workflow}

1. 재설계된 사이트에 필요한 Figma 구성 요소를 식별합니다.
1. 이러한 구성 요소를 Edge Delivery Services 블록 또는 변형으로 마이그레이션합니다.
1. 생성된 블록 컬렉션 및 디자인 토큰을 검토합니다.
1. 대표 소스 페이지에서 사이트 마이그레이션을 실행합니다.
1. 소스 콘텐츠를 Figma에서 파생된 블록에 매핑합니다.
1. 소스 웹 사이트에 대해 콘텐츠의 유효성을 검사합니다.
1. 대상 Figma 디자인에 대해 시각적 출력의 유효성을 검사합니다.
1. 블록이나 매핑을 세분화한 다음 더 많은 페이지로 확장합니다.

### 그림에서 새 페이지 만들기 {#figma-new-page-from-figma}

페이지가 소스 웹 사이트에 존재하지 않고 Figure 페이지나 프레임이 새 Edge Delivery Services 페이지를 생성해야 하는 경우 이 프롬프트를 사용합니다.

#### 프롬프트 예 {#example-figma-new-page}

* &quot;이 그림 페이지를 Edge Delivery Services으로 마이그레이션: `https://figma.com/design/{fileKey}?node-id={nodeId}`&quot;

#### 알아 두어야 할 사항 {#wtk-figma-new-page}

* 이 프롬프트는 전체 파일이 아닌 **특정 그림 프레임 또는 페이지**&#x200B;에서 가장 잘 작동합니다.
* 프레임은 **페이지 섹션 지우기**(으)로 구성해야 합니다.
* 섹션은 기존 블록, 기본 컨텐츠 또는 새 변형에 매핑됩니다.
* 텍스트와 에셋은 Figma에서 가져옵니다.
* 검색, 계산기, 개인화 또는 스토어 로케이터와 같은 동적 기능을 사용하려면 Figma 마이그레이션이 생성하는 것 이상의 **개별 블록 개발**&#x200B;이 필요할 수 있습니다.
...
* 일반적인 문제에는 알려진 패턴이 있습니다.
   ...
   * **블록이 렌더링되지 않음**: Markdown에서 블록 이름 형식을 확인하고 `blocks/`에서 블록에 `.js` 및 `.css` 파일이 모두 있는지 확인하십시오.
