...
>[!NOTE]
>
> 통신 만들기 작업이 현재 알파에 있습니다. 참가하려면 공식 이메일 주소에서 [aem-forms-ea@adobe.com으로 요청을 보내십시오.](mailto:aem-forms-ea@adobe.com)
...
* **낮은 기술 장벽**: 고급 기술 전문가 또는 심층적인 제품 전문 지식 없이도 비즈니스 사용자가 쉽게 커뮤니케이션을 만들 수 있도록 허용합니다.
...
* **템플릿에서 만들기**: 승인된 조직 템플릿을 사용하여 브랜드 일관성 및 준수 표준을 보장할 수 있습니다. 이 작업은 기존 템플릿과 스타일 지침을 활용하여 규제 요구 사항을 충족하는 브랜드 내 서신을 만듭니다.

* **기존 이미지 및 문서를 대화형 통신으로 가져오기 및 변환**: 기존 문서를 대화형 통신으로 가져오고 변환할 수 있습니다. 이 작업은 업로드된 콘텐츠를 분석하여 필드를 감지하고 레이아웃을 보존하며 동적 콘텐츠 기능을 통해 데이터 기반 서신을 생성합니다. 지원되는 형식에는 PDF, 이미지(JPG, PNG) 및 손으로 그린 템플릿이 포함됩니다.
...
* **필드 및 콘텐츠 추가**: 자연어 프롬프트를 사용하여 통신 문서에 새 필드, 텍스트 블록, 이미지, 차트, 표 및 기타 구성 요소를 추가합니다. 이 작업은 지침을 해석하고 적절한 구조와 형식으로 적절한 요소를 삽입합니다.

* **필드 및 콘텐츠 편집**: 대화 명령을 통해 통신 문서 내의 기존 필드 및 콘텐츠를 수정합니다. 필드 속성을 업데이트하고, 텍스트 콘텐츠를 변경하고, 데이터 바인딩을 조정하고, 구성 요소 구성을 세분화합니다.

* **필드 및 콘텐츠 제거**: 자연어 지침을 사용하여 통신 문서에서 원치 않는 필드, 구성 요소 또는 섹션을 삭제합니다. 작업은 문서 구조 및 레이아웃 무결성을 유지하면서 지정된 요소를 제거합니다.
