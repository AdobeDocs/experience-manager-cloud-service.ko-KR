...
* **개발자**&#x200B;는 IDE 또는 채팅 응용 프로그램에서 콘텐츠 작업 및 워크플로를 조정할 수 있습니다.
* **실무자** 및 콘텐츠 설계자는 AEM의 기존 권한 모델 내에서 AI의 지원을 받아 사이트 및 콘텐츠 조각을 관리하고 에셋을 가져올 수 있습니다.
...
* **API 배관 대신 자연어 상호 작용** - MCP 도구는 사용 가능한 작업과 호출 방법을 설명합니다. LLM은 이러한 스키마를 사용하여 호출할 도구와 매개 변수를 결정합니다.
* **여러 응용 프로그램에서 일관된 경험** - 여러 MCP 호환 응용 프로그램에서 동일한 AEM MCP 도구를 사용할 수 있으므로 팀은 동일한 기본 AEM 기능을 호출하면서 가장 생산적인 위치에서 작업할 수 있습니다.
* **보안 및 거버넌스 유지** - AEM MCP 도구에 대한 요청은 인증된 사용자의 ID에서 실행되며, 각 도구는 사용자의 기존 AEM 권한을 적용합니다. AI 지원 작업은 AEM의 수동 작업과 동일한 액세스 규칙을 따릅니다.

## AEM 제공 MCP 서버 {#mcp-servers-provided-by-aem}

AEM은 MCP 서버를 HTTP 끝점으로 노출합니다. 아래 나열된 끝점은 `https://mcp.adobeaemcloud.com/adobe/mcp/`을(를) 기준으로 합니다.
...
1. **각 MCP 클라이언트 응용 프로그램을 구성**&#x200B;하여 응용 프로그램에서 AEM의 MCP 서버에 연결하고 OAuth 로그인을 수행하는 방법을 알 수 있도록 합니다.
...
[지원되는 MCP 응용 프로그램](#supported-mcp-applications)에 나열된 모든 응용 프로그램은 기본적으로 허용됩니다.
...
또한 관리자는 조직의 정책에 필요한 경우 특정 MCP 클라이언트 애플리케이션에 대한 액세스를 비활성화할 수 있습니다. Adobe에서 추가 MCP 클라이언트 제품에 대한 지원을 활성화하려면 제품 웹 사이트에 대한 링크를 보내십시오. 사용자 정의 MCP 클라이언트를 다운로드해야 하는 경우 연락하십시오.

모든 MCP 서버 관련 요청에 대해 언제든지 **`aemcs-mcp-feedback@adobe.com`**&#x200B;의 Adobe에 문의하십시오.

### MCP 클라이언트 애플리케이션 구성 {#mcp-client-application-configuration}

각 사용자는 이 단계를 수행하거나 MCP 클라이언트 애플리케이션의 관리자는 지원되는 경우 이를 수행할 수 있습니다. 구성 세부 정보는 애플리케이션마다 조금씩 다릅니다. MCP 클라이언트는 빠르게 발전하고 있으며 원격 MCP 서버에 대한 지원이 활발히 개발되고 있습니다. 원격 서버를 추가하기 위한 기능에 액세스하려면 개발자 모드를 활성화해야 할 수 있지만 일반적인 프로세스는 다음과 같습니다.

1. 하나 이상의 MCP 서버 URL을 추가합니다.
...
지원되는 응용 프로그램의 전체 목록은 [지원되는 MCP 응용 프로그램](#supported-mcp-applications)을 참조하세요.
...
* **높은 수준의 기능을 제공하지만 확실하지 않음** - LLM은 복잡한 작업을 수행할 수 있지만 가끔 오류가 발생할 수 있습니다. 동일한 프롬프트가 명확한 이유 없이 약간 다른 결과나 프레젠테이션을 산출할 수 있습니다. 프로덕션 콘텐츠에 변경 사항을 적용하기 전에 항상 출력을 검토하십시오.
* **진화하는 기능** - LLM 모델이 지속적으로 개선되고 있습니다. 시간이 지나면서 목표를 달성하기 위해 MCP 도구를 결합하는 새로운 방법을 발견하는 것이 더 똑똑해집니다. 오늘 여러 개의 프롬프트가 필요한 작업은 내일 하나의 프롬프트와 원활하게 작동할 수 있습니다.
* **사람의 감독이 필수적입니다** - LLM을 감독이 필요한 지식이 있는 도우미로 생각하십시오. 광범위한 지식을 가지고 있으며 창의적인 해결책을 고안할 수 있지만, 지도와 검토를 통해 이점을 얻을 수 있습니다. 특히 중요한 작업의 결과를 확인하고 결과가 예상과 일치하지 않을 때 피드백을 제공합니다.
* **도구 실행 자동 확인** - Claude와 같은 일부 MCP 클라이언트 응용 프로그램은 LLM에서 요청한 도구 실행을 자동으로 확인하는 옵션을 제공합니다. 이 옵션은 콘텐츠 검색 또는 검색과 같은 읽기 전용 작업에는 편리할 수 있지만, 콘텐츠를 업데이트하거나 삭제하는 도구에는 주의하십시오. AEM 환경을 수정하는 작업을 확인하기 전에 각 도구 실행 요청을 검토하십시오.

## 제한 사항 {#limitations}

AEM은 현재 [지원되는 MCP 응용 프로그램](#supported-mcp-applications)에 나열된 응용 프로그램에서 MCP 서버 구성을 지원합니다.

다른 MCP 클라이언트 응용 프로그램을 사용하려면 **`aemcs-mcp-feedback@adobe.com`**&#x200B;에 연락하여 추가 클라이언트 지원을 요청하거나 사용자 지정 클라이언트 응용 프로그램을 다운로드하십시오.
