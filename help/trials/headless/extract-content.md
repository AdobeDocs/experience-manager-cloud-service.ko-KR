...
>[!NOTE]
>
>을(를) 선택할 수 없는 경우 **AEM 데모 자산** 드롭다운에서 엔드포인트 액세스하려면 Adobe 고객 지원 센터에 문의하거나 [AEM 평가판 Slack 채널입니다.](https://adobe-dx-support.slack.com/)
...
## 지속되는 쿼리 {#persisted-queries}

지속되는 쿼리는 GraphQL API를 클라이언트 애플리케이션에 노출하기 위해 선호되는 메커니즘입니다. 쿼리가 유지되면 GET 요청을 사용하여 요청하고 빠르게 검색할 수 있도록 캐시할 수 있습니다.

클라이언트 애플리케이션에서 사용할 데이터를 포함하는 지속적인 쿼리를 만듭니다.

1. 이전에 컨텐츠 조각으로 만든 데이터를 사용하게 되므로, 이 경우 **프로젝트** 엔드포인트가 **끝점** 편집기의 오른쪽 위 모서리에 있는 드롭다운 메뉴.
...
1. 복사한 코드를 붙여넣어 쿼리 편집기의 기존 콘텐츠를 바꿉니다.
   ...
   >[!NOTE]
   >
   >이전 모듈에 설명된 것과 동일한 필드 설명을 사용하지 않은 경우 이 쿼리의 필드 이름을 업데이트해야 합니다.
   >
   >앞에서 설명한 대로 GraphQL 자동 완성(Ctrl+Space 또는 Option+Space) 기능을 사용하여 사용 가능한 속성을 식별할 수 있습니다.

1. 붙여넣기가 완료되면 쿼리 편집기 왼쪽 상단의 **재생** 버튼을 클릭하여 쿼리를 실행합니다.

1. 쿼리 편집기 옆의 오른쪽 패널에 결과가 표시됩니다. 쿼리가 잘못된 경우, 오른쪽 패널에 오류가 표시됩니다.

   ![자체 쿼리 만들기](assets/do-not-localize/own-query.png)

1. 쿼리에 만족하면 **다른 이름으로 저장** 쿼리 편집기 맨 위에 있는 단추를 클릭하여 쿼리를 유지합니다.

1. 에서 **쿼리 이름** 팝업에서 쿼리 이름을 지정합니다 `adventure-list`.

1. 탭 또는 클릭 **다른 이름으로 저장**.

   ![쿼리 유지](assets/do-not-localize/persist-query.png)

1. 이 쿼리는 화면 하단에 있는 배너 메시지에 의해 확인된 대로 유지됩니다. 이제 창의 지속되는 쿼리의 왼쪽 패널에도 쿼리가 나타납니다.

1. 지속형 쿼리를 공개적으로 사용하려면 컨텐츠 조각을 게시해야 하는 방식과 마찬가지로 게시해야 합니다. 을(를) 클릭합니다. **게시** 쿼리 편집기 오른쪽 상단의 단추를 클릭하여 쿼리를 게시합니다.

1. 게시가 배너 알림으로 확인됩니다.

이제 정의한 특정 속성 및 형식만 포함하는 새로운 지속된 쿼리가 있습니다.
