...
1. 다음 항목 선택 **정렬** 읽기 쉽도록 코드 서식을 자동으로 지정합니다.

1. 완료되면 다음을 선택합니다 **재생** 쿼리를 실행하려면 편집기 왼쪽 상단의 단추입니다. 편집기가 노란색으로 잠깐 강조되는 `items`를 자동 완성하면 쿼리가 실행됩니다.
...
1. 지속 쿼리를 공개적으로 사용할 수 있으려면 콘텐츠 조각을 게시해야 하는 것처럼 게시해야 합니다. 클릭 **게시** 쿼리를 게시할 쿼리 편집기의 오른쪽 상단
