...
1. **정렬** 버튼을 선택하여 읽기 쉽도록 코드 서식을 자동으로 지정할 수 있습니다.

1. 완료되면 쿼리 편집기 왼쪽 상단의 **재생** 버튼을 선택하여 쿼리를 실행합니다. 편집기가 노란색으로 잠깐 강조되는 `items`를 자동 완성하면 쿼리가 실행됩니다.
...
1. 복사한 코드를 붙여넣어 쿼리 편집기의 기존 콘텐츠를 바꿉니다.
   ...
   >[!NOTE]
   >
   >이전 모듈에서 설명한 것과 동일한 필드 설명을 사용하지 않은 경우 이 쿼리에서 필드 이름을 업데이트합니다.
   >
...
1. 지속 쿼리를 공개적으로 사용할 수 있으려면 콘텐츠 조각을 게시해야 하는 것과 마찬가지로 게시해야 합니다. 쿼리 편집기 오른쪽 상단의 **게시**&#x200B;를 클릭하여 쿼리를 게시합니다.
