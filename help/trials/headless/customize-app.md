...
1. 다음 항목 선택 **미리 보기** 편집기 화면의 오른쪽 상단에 있는 단추입니다.
...
1. 선택 **미리 보기** 변경 내용을 보려면 창의 오른쪽 상단에 있어야 합니다. 데모 앱의 미리보기가 새 탭에서 열립니다.
...
1. 다음 항목 선택 **X** 바이커 이미지를 제거합니다. 이 콘텐츠 조각 모델에 필요한 데이터이므로 이미지가 사라지고 편집기에 오류가 표시됩니다.

   ![조각에서 이미지 제거](assets/do-not-localize/swap-image-4.png)

1. 선택 **에셋 추가** 그런 다음 **에셋 찾아보기** 을 클릭합니다.

1. **자산 선택** 대화 상자가 열리고 **sample-wknd-app** > **en** > **이미지 파일** 경로가 자동으로 선택됩니다.

1. 이미지 선택 `biker-yellow.png` 다음을 선택합니다. **선택**.

1. 바이커 이미지가 선택한 이미지로 교체됩니다. 편집기는 변경 내용을 자동으로 저장합니다.

1. 선택 **미리 보기** 변경 내용을 보려면 창의 오른쪽 상단에 있어야 합니다. 데모 앱의 미리보기가 새 탭에서 열립니다. 브라우저에서 새로 고침을 클릭하면 앱에서 노란색 반바지가 있는 새 바이커 이미지를 볼 수 있습니다.

AEM Headless CMS를 사용하여 앱의 이미지와 에셋을 쉽게 업데이트할 수 있습니다.
...
1. 콘텐츠 조각 편집기에서 올바른 위치로 돌아가야 합니다. 편집기의 왼쪽 상단에 있는 이동 경로는 콘텐츠 계층에서 현재 위치를 보여 줍니다. 선택 **WKND 홈** 이동 경로에서 해당 페이지로 돌아갑니다.

1. **WKND Yellow를 타는 산악 바이커** 패널을 선택합니다.

1. **산악 바이킹 - 구매 가능** 레이어를 선택합니다.

1. 이 패널에서 콜아웃을 만들려면 구매 가능한 순간 콘텐츠 조각을 만듭니다. 다음 항목 선택 **+ 새 조각 만들기** 단추를 클릭합니다.
...
1. 선택 **미리 보기** 변경 내용을 보려면 창의 오른쪽 상단에 있어야 합니다. 데모 앱의 미리보기가 새 탭에서 열립니다. 브라우저에서 새로 고침을 클릭하여 위치를 테스트하고 편집기에서 필요에 따라 조정합니다.
