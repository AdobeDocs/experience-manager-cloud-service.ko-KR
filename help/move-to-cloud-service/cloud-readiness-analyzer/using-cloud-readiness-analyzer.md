...
* CRA는 사용자 또는 그룹의 사용자만 실행할 수 `admin` `Administrators` 있습니다.
...
* 보고서를 생성하는 데 필요한 상당한 시간 때문에 결과는 캐시에 저장되고 캐시가 만료되거나 보고서가 명시적으로 새로 고쳐질 때까지 나중에 보고 다운로드할 수 있습니다.
...
1. Adobe Experience Manager 사용자 인터페이스를 사용하여 도구 -> **작업** -> **클라우드 준비 분석기로 이동합니다**.

   >[!NOTE]
   >도구를 여는 즉시 보고서를 생성하기 위한 배경 프로세스가 CRA에 시작됩니다. 보고서가 준비될 때까지 보고서 생성이 진행 중임을 표시합니다. 브라우저 탭을 닫고 나중에 다시 방문하여 보고서가 완료되면 확인할 수 있습니다.

CRA 보고서가 생성되고 표시되면 도구 페이지의 오른쪽 상단 모서리에 있는 **CSV** 단추를 클릭하여 쉼표로 구분된 값(CSV) 형식으로 보고서를 다운로드하는 옵션이 있습니다.

왼쪽 상단의 &quot;보고서 새로 고침&quot; 단추를 클릭하여 CRA가 캐시를 지우고 보고서를 다시 생성하도록 할 수 있습니다.

### AEM 6.2 및 6.1 {#aem-specific-versions}

CRA 사용자 인터페이스는 CSV 보고서를 생성하고 다운로드하는 링크로 AEM 6.2에서 제한됩니다. AEM 6.1의 경우 사용자 인터페이스가 작동하지 않고 HTTP 인터페이스만 사용할 수 있습니다.

모든 버전에서 포함된 패턴 탐지기는 독립적으로 실행될 수 있습니다.

## CRA 요약 보고서 {#cra-summary-report}

CRA가 AEM 사용자 인터페이스에서 실행되면 보고서가 도구 창에 결과로 표시됩니다. 보고서의 형식은 다음과 같습니다.

* 보고서 개요: 보고서 자체에 대한 정보(보고서 생성 시기 포함)
...
작업의 대략적인 우선순위를 나타내기 위해 각 검색 결과에 중요도 수준이 지정됩니다. 사용된 중요도 수준은 다음과 같습니다.
...
&quot;CSV&quot; 단추를 누르면 CRA 보고서의 CSV 형식이 결과 캐시에서 만들어지고 브라우저에 반환됩니다. 브라우저 설정에 따라 이 보고서는 기본 이름이 인 파일로 자동으로 다운로드됩니다 `results.csv`. 캐시가 만료되면 CSV 파일을 빌드하고 다운로드하기 전에 보고서가 다시 생성됩니다.

아래 절차에 따라 AEM 인스턴스에서 요약 보고서의 CSV 형식을 생성합니다.
...
또한 HTTP 클라이언트 응용 프로그램 `curl` 또는 기타 `wget` 와 같은 명령줄 도구를 사용할 수 있습니다. 인증된 세션에서 브라우저 탭을 사용하지 않는 경우 주석의 일부로 관리 사용자 이름과 암호를 제공해야 합니다. 다음은 이 작업을 수행하는 방법의 예입니다.
