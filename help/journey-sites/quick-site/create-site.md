...
이 문서는 이러한 기본 사항을 기반으로 하므로 이를 통해 첫 번째 구성 단계를 수행하고 템플릿에 대한 사이트를 만들어 나중에 프론트엔드 도구를 사용하여 맞춤화할 수 있습니다.
...
1. 파일 브라우저에서 템플릿을 찾습니다 [이전에 다운로드했습니다.](#obtaining-template) 및 선택 **업로드**.

1. 업로드되면 사용할 수 있는 템플릿 목록에 표시됩니다. 해당 템플릿을 선택하고(오른쪽 열에 템플릿에 대한 정보도 표시됨) 선택합니다 **다음**.
...
1. 선택 **만들기** 그리고 사이트 템플릿에서 새 사이트가 만들어집니다.
...
1. 사이트 브라우저에서 사이트 마스터 언어의 홈 페이지로 이동한 다음 선택할 페이지를 선택하고 를 선택합니다 **편집** 메뉴 표시줄에서 을 클릭합니다.
...
1. 새 사이트의 루트를 선택한 다음 을 선택합니다. **테마 소스 다운로드** 사이트 레일에서 을 참조하십시오.
...
1. AEM 메인 탐색에서 로 이동합니다. **도구** > **보안** > **사용자**.
1. 사용자 관리 콘솔에서 을 선택합니다. **만들기**.
