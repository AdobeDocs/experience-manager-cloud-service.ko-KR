...
이 문서는 참조 데모 추가 기능의 템플릿을 기반으로 사이트를 만드는 방법을 이해하는 데 도움이 됩니다. 문서를 읽고 나면
...
1. AEM 메인 메뉴에서 을(를) 선택합니다. **사이트**.

   ![사이트에 액세스](assets/access-sites.png)

1. 사이트 콘솔에서 을 선택합니다. **만들기** 화면 오른쪽 상단에서 **템플릿의 사이트** 을 클릭합니다.

   ![템플릿으로 사이트 생성](assets/create-site-from-template.png)

1. 사이트 생성 마법사가 실행됩니다. 왼쪽 열에서 파이프라인에서 작성 인스턴스에 배포한 데모 템플릿을 볼 수 있습니다. 하나를 선택하여 오른쪽 열에 세부 정보를 표시합니다. AEM Screens을 테스트하거나 데모하려면 다음을 선택하십시오. **We.Cafe 사이트 템플릿**. **다음**&#x200B;을 선택합니다.
...
1. 페이지를 선택하여 선택한 다음 선택 **편집** 을 클릭합니다.
...
* AEM Screens 콘텐츠를 완전히 데모하고 테스트하려면 다음을 기반으로 사이트를 배포했는지 확인하십시오. **We.Cafe 사이트 템플릿** 앞에서 설명한 대로 계속합니다. [데모 사이트에 AEM Screens을 활성화합니다.](screens.md)
