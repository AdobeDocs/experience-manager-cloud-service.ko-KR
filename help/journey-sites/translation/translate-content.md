...
AEM Sites 번역 여정의 이전 문서에서, [번역 규칙 구성](translation-rules.md) AEM 번역 규칙을 사용하여 번역 콘텐츠를 식별하는 방법을 알아보았습니다. 이제 다음을 수행해야 합니다.
...
1. 다음으로 이동 **탐색** -> **Sites** 프로젝트를 탭하거나 클릭합니다.
1. 프로젝트의 언어 루트를 찾습니다. 예를 들어, 언어 루트가 영어로 된 경우, `/content/<your-project>/en`.
...
1. 에서 **프로젝트 만들기** 창, 추가 **제목** 을(를) 클릭한 다음 을(를) 탭하거나 클릭합니다 **만들기**.
...
1. 다음으로 이동 **탐색** -> **프로젝트**.
1. 이전 섹션에서 만든 프로젝트를 탭하거나 클릭합니다(다음 중 하나). [컨텐츠 경로를 기반으로 번역 프로젝트 자동 만들기](#automatically-creating) 또는 [컨텐츠를 선택하여 번역 프로젝트 수동으로 만들기](#manually-creating) 상황에 따라)
...
* **번역 작업** - 이 카드나 이 카드에 표시된 것처럼 상태, 자산 수 등을 포함한 실제 번역 작업에 대한 개요를 제공합니다. 일반적으로 ISO-2 언어 코드가 작업 이름에 첨부된 언어당 작업이 한 개 있습니다.
   * 다음의 경우에 주의하십시오 [번역 작업 자동 생성](#automatically-creating) AEM은 작업을 비동기식으로 만들며 프로젝트 내에 즉시 나타나지 않을 수 있습니다.
...
1. 설정 **요약** 카드 하단에 있는 생략 부호 단추를 탭하거나 클릭합니다.
1. 설정 **고급** 탭에서 옵션을 선택 취소합니다. **프로모션 후 실행 삭제**.
...
>[!TIP]
>
>번역 작업을 처리하는 데 시간이 오래 걸릴 수 있으며, 번역 항목이 **초안** to **번역 진행 중** to **검토 준비** 그들이 도착하기 전에 **승인됨** state. 이것은 예상될 것입니다.

>[!NOTE]
>
>프로젝트 옵션을 비활성화하지 않은 경우 **프로모션 후 실행 삭제** 로서의 [이전 섹션에 설명되어 있습니다.](#using-translation-project) 번역된 항목은 **삭제됨** state. 이는 번역된 항목이 도착하면 AEM에서 자동으로 번역 레코드를 삭제하므로 일반적입니다. 번역된 항목은 언어 사본으로 가져오었으며 더 이상 필요하지 않으므로 번역 레코드만 삭제되었습니다.
>
>확실하지 않더라도 걱정하지 마십시오. 이러한 내용은 AEM 작동 방식에 대한 심층적인 세부 정보이며 여정 이해에 영향을 주지 않습니다. AEM의 번역 처리 방법을 자세히 알아보려면 [추가 리소스](#additional-resources) 섹션 을 참조하십시오.
...
완료된 번역 작업으로 이동한 다음 확인란을 탭하거나 클릭하여 라인 항목을 선택하면 됩니다. 아이콘 **사이트에서 미리 보기** 이 도구 모음에 표시됩니다.
...
적절한 권한이 있으면 필요한 번역된 컨텐츠를 추가로 수정할 수 있지만 컨텐츠를 편집하는 것은 이 여정의 범위를 벗어납니다. 자세한 내용은 [추가 리소스](#additional-resources) 이 항목에 대한 자세한 내용은 이 문서의 끝 부분에 있는 섹션을 참조하십시오.
...
을 통해 이 폴더로 이동하는 경우 **탐색** -> **Sites**&#x200B;번역된 컨텐츠가 표시됩니다.
...
이 지식을 바탕으로 작성되고 다음에 문서를 검토하여 AEM Sites 번역 여정을 계속 진행합니다 [번역된 컨텐츠 게시](publish-content.md) 에서는 번역된 콘텐츠를 게시하는 방법과 해당 번역을 언어 루트 콘텐츠가 변경될 때 업데이트하는 방법을 알아봅니다.

## 추가 리소스 {#additional-resources}

문서를 검토하여 번역 여정의 다음 부분으로 이동하는 것이 좋습니다 [번역된 컨텐츠 게시,](publish-content.md) 다음은 이 문서에서 언급된 일부 개념을 자세히 설명하는 몇 가지 추가 선택적 리소스입니다. 여정을 계속 진행할 필요는 없습니다.
