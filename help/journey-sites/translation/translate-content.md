...
1. 다음으로 이동 **탐색** > **사이트** 프로젝트를 선택합니다.
1. 프로젝트의 언어 루트를 찾습니다. 예를 들어 언어 루트가 영어인 경우 `/content/<your-project>/en`을 선택합니다.
   * 처음 번역하기 전에 다른 언어 폴더는 빈 플레이스홀더입니다. 이는 일반적으로 콘텐츠 설계자에 의해 생성됩니다.
1. 프로젝트의 언어 루트를 찾습니다.
1. 레일 선택기를 선택하고 **참조** 패널.
1. 선택 **언어 복사**.
1. **언어 사본** 확인란을 선택합니다.
1. 참조 패널 하단의 **언어 사본 업데이트** 섹션을 확장합니다.
1. 다음에서 **프로젝트** 드롭다운 목록에서 다음을 선택합니다. **번역 프로젝트 만들기**.
...
1. 다음으로 이동 **탐색** > **프로젝트**.
1. 선택 **만들기** > **폴더** 을 클릭하여 프로젝트용 폴더를 만듭니다.
   * 이 작업은 옵션이지만 번역 작업을 구성하는 데 도움이 됩니다.
1. 다음에서 **프로젝트 만들기** 창, 추가 **제목** 을(를) 폴더에 대해 선택한 다음 을(를) 선택합니다 **만들기**.

   ![프로젝트 폴더 만들기](assets/create-project-folder.png)

1. 폴더를 선택하여 폴더를 엽니다.
1. 새 프로젝트 폴더에서 **만들기** > **프로젝트**.
1. 프로젝트는 템플릿을 기반으로 합니다. 다음 항목 선택 **번역 프로젝트** 템플릿을 선택하고 다음 선택 **다음**.
...
1. 다음에서 **고급** 탭에서 **타겟 언어** 드롭다운 목록에서 콘텐츠를 번역할 언어를 선택합니다. **만들기**&#x200B;를 선택합니다.

   ![프로젝트 고급 탭](assets/project-advanced-tab.png)

1. 선택 **열기** 확인 대화 상자에서 확인할 수 있습니다.
...
1. 다음으로 이동 **탐색** > **프로젝트**.
1. 이전 섹션에서 만든 프로젝트를 선택합니다(다음 중 하나). [콘텐츠 경로를 기반으로 자동으로 번역 프로젝트 만들기](#automatically-creating) 또는 [콘텐츠를 선택하여 수동으로 번역 프로젝트 만들기](#manually-creating) 상황에 따라).
...
* **번역 작업** - 이 카드 또는 이들 카드는 상태, 에셋 수 등을 포함하여 실제 번역 작업에 대한 개요를 제공합니다. 일반적으로 언어당 작업 이름에 ISO-2 코드가 추가된 하나의 작업이 있습니다.
...
1. 다음에서 **요약** 카드 하단에서 줄임표 버튼을 선택합니다.
...
번역 프로젝트를 자동으로 생성할 때 AEM은 이전에 정의한 번역 규칙을 기반으로 사용자가 선택한 경로의 콘텐츠를 평가합니다. 해당 평가를 기반으로 번역에 필요한 콘텐츠를 새 번역 프로젝트에 추출합니다.

이 프로젝트에 포함된 콘텐츠 세부 정보를 보려면 다음 작업을 수행하십시오.

1. 아래 줄임표 버튼을 선택합니다. **번역 작업** 카드.
1. **번역 작업** 창에 작업에 있는 모든 항목이 나열됩니다.

   ![번역 작업 세부 정보](assets/translation-job-detail.png)

1. 한 라인이 번역할 여러 콘텐츠 항목을 나타낼 수 있다는 점을 고려하여 해당 라인의 세부 정보를 보려면 라인을 선택합니다.
1. 라인 항목에 대한 선택 확인란을 선택하여 작업에서 항목을 삭제하거나 사이트 콘솔에서 볼 수 있는 옵션과 같은 추가 옵션을 확인합니다.

   ![번역 작업 옵션](assets/translation-job-options.png)

일반적으로 번역 작업의 콘텐츠는 **번역 작업** 창의 **상태** 열에 표시된 **초안** 상태에서 시작합니다.

번역 작업을 시작하려면 번역 프로젝트 개요로 돌아가 도구 모음 단추 **번역 작업** 카드 및 선택 **시작**.
...
>
>이것이 명확하게 이해되지 않아도 걱정하지 마십시오. 이는 AEM이 어떻게 작동하는지에 대한 심층적인 세부 정보이며, 여정에 대한 이해에 영향을 미치지 않습니다. AEM이 번역을 처리하는 방법에 대해 자세히 알아보려면 다음을 참조하십시오. [추가 리소스](#additional-resources) 이 문서 끝에 있는 섹션.
...
1. 다음 중 하나의 하단에서 줄임표 버튼을 선택합니다. **번역 작업** 카드.
1. 작업에 콘텐츠가 없는지 확인합니다. 다음 항목 선택 **추가** 창 맨 위에 있는 단추 **에셋/페이지** 드롭다운에서 을 클릭합니다.

   ![빈 번역 작업](assets/empty-translation-job.png)

1. 경로 브라우저가 열려 추가할 콘텐츠를 구체적으로 선택할 수 있습니다. 콘텐츠를 찾은 다음 선택하여 선택합니다.

   ![경로 브라우저](assets/path-browser.png)

1. 선택 **선택** 을 눌러 선택한 컨텐츠를 작업에 추가합니다.
1. 다음에서 **번역** 대화 상자에서 다음 작업을 수행하도록 지정합니다. **언어 사본 만들기**.
...
>[!TIP]
>
>경로 브라우저는 콘텐츠를 검색하고, 필터링하고, 탐색할 수 있는 강력한 도구입니다. 다음 항목 선택 **컨텐츠 전용/필터** 단추를 사용하여 사이드 패널을 토글하고 다음과 같은 고급 필터를 표시합니다. **수정한 날짜** 또는 **번역 상태**.
>
>[추가 리소스 섹션](#additional-resources)에서 경로 브라우저에 대해 자세히 알아볼 수 있습니다.

이전 단계를 사용하여 프로젝트의 모든 언어(작업)에 필요한 콘텐츠를 추가할 수 있습니다. 모든 콘텐츠를 선택한 후 번역을 시작할 수 있습니다.
...
[앞에서 보았던 것처럼,](#using-translation-project) 기계 번역 콘텐츠는 **승인됨** 상태로 AEM으로 다시 흐릅니다. 기계 번역을 사용 중이므로 사람의 개입이 필요 없다고 간주되기 때문입니다. 그러나 번역된 컨텐츠는 여전히 검토할 수 있습니다.

완료된 번역 작업으로 이동하여 확인란을 탭하거나 클릭하여 라인 항목을 선택하십시오. 도구 모음에 **사이트에서 미리보기** 아이콘이 표시됩니다.

![사이트에 표시](assets/reveal-in-sites.png)

해당 아이콘을 선택하여 콘솔에서 번역된 콘텐츠를 열고 번역된 콘텐츠의 세부 정보를 볼 수 있습니다.
...
다음을 통해 이 폴더로 이동하는 경우 **탐색** > **사이트**&#x200B;를 클릭하면 번역된 콘텐츠가 표시됩니다.
...
이 콘텐츠는 게시되지 않았으므로 사용할 수 없습니다. 이 작성자-게시 구조에 대해 알아보고 번역 여정의 다음 단계에서 번역된 콘텐츠를 게시하는 방법을 살펴볼 수 있습니다.
...
이 지식을 기반으로 다음 문서를 검토하여 AEM Sites 번역 여정을 계속하십시오 [번역된 콘텐츠 게시](publish-content.md) 여기서 번역된 콘텐츠를 게시하는 방법과 언어 루트 콘텐츠가 변경될 때 이러한 번역을 업데이트하는 방법을 배웁니다.
