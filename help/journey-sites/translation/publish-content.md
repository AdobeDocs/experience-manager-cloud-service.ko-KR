...
AEM Sites 번역 여정의 이전 문서 [컨텐츠 번역,](configure-connector.md)에서는 AEM 번역 프로젝트를 사용하여 콘텐츠를 번역하는 방법을 알아보았습니다. 이제 다음을 수행해야 합니다.
...
이 문서는 AEM에서 콘텐츠를 게시하는 방법과 번역을 최신 상태로 유지하기 위해 연속 워크플로우를 만드는 방법을 이해하는 데 도움이 됩니다. 이 문서를 읽은 후에는 다음을 수행해야 합니다.
...
번역된 콘텐츠의 상태가 만족되면 게시 후 액세스 및 소비될 수 있도록 해야 합니다. 이 작업은 일반적으로 번역 전문가의 책임이 아니지만 전체 워크플로우를 보여주기 위해 여기에 설명되어 있습니다.
...
번역을 게시하는 가장 간단한 방법은 프로젝트 폴더로 이동하는 것입니다.
...
1. **탐색** -> **사이트** -> **파일**&#x200B;로 이동하여 프로젝트 폴더를 엽니다.
...
이제 번역된 콘텐츠가 게시되었습니다! 이제 액세스하고 사용할 수 있습니다.
...
1. **탐색** -> **자산** -> **파일**&#x200B;로 이동합니다. AEM의 컨텐츠는 `/content` 아래에 저장됩니다.
1. 프로젝트의 언어 루트를 선택합니다. 이 경우 `/content/wknd/en` 을 선택했습니다.
...
새 카드에 나열된 통계는 다를 수 있습니다. 이것은 AEM이 마지막 번역 이후 변경된 사항을 인식하고 번역해야 하는 콘텐츠만 포함하기 때문입니다. 여기에는 업데이트된 컨텐츠의 재번역뿐만 아니라 새 컨텐츠의 최초 번역도 포함됩니다.
...
1. 창 상단에 있는 **추가** 단추를 탭하거나 클릭합니다.
1. **타일 추가** 창에서 **번역 작업** 및 **제출**&#x200B;을 탭하거나 클릭합니다.

   ![타일 추가](assets/add-translation-job-tile.png)

1. 새 번역 작업의 카드에서 카드 상단에 있는 V자 모양 단추를 탭하거나 클릭하고 **Target 업데이트**&#x200B;를 선택하여 새 작업의 대상 언어를 정의합니다.

   ![대상 업데이트](assets/update-target.png)

1. **Target 언어 선택** 대화 상자에서 드롭다운을 사용하여 언어를 선택하고 **완료**&#x200B;를 탭하거나 클릭합니다.

   ![대상 언어 선택](assets/select-target-language.png)

1. 새 번역 작업의 대상 언어가 설정되면 작업 카드 하단에 있는 줄임표 단추를 탭하거나 클릭하여 작업 세부 정보를 확인합니다.
1. 작업을 처음 만들면 작업이 비어 있습니다. **추가** 단추를 탭하거나 클릭하고 원래 번역 프로젝트를 만들 때 사용했던 경로 브라우저 [를 사용하여 작업에 컨텐츠를 추가합니다.](translate-content.md##manually-creating)
...
축하합니다! AEM Sites 번역 여정을 완료했습니다! 이제 다음을 수행해야 합니다.

* AEM 컨텐츠 관리 기능에 대한 기본적인 이해를 바랍니다.
* AEM 번역 기능 및 사이트 컨텐츠와 어떻게 관련되는지를 이해합니다.
* 자신만의 콘텐츠를 번역할 수 있습니다.

이제 AEM에서 자체 콘텐츠를 번역할 준비가 되었습니다. 그러나 AEM은 강력한 도구이며 다양한 추가 옵션을 사용할 수 있습니다. 이 여정에서 본 기능에 대해 자세히 알아보려면 [추가 리소스 섹션](#additional-resources)에서 사용할 수 있는 추가 리소스 중 일부를 확인하십시오.
